...
- 质量生产与发布分开：产出 candidate 的规范 lane 工作树 `make accept PUBLISH=1` 对同一 exact candidate 去重执行 required 本地 readiness、Lane Gate 源码闭集、默认 typed `not_required` Alpha/Beta（仅 `ALPHA=1`/`BETA=1` 才真跑 live），admit 并以 expected-old CAS 发布到 `origin/dev1.0` 读回 `after`；`make integrate ACCEPTANCE_BUNDLE=… PUBLISH=1` 是消费他树 bundle 的等价两段形态。本地 accept/bundle/integrate/hook 强制验真且不重跑完整套件。可按授权撤 dev 旧 `04. Lane Gate` required check、删除已可达已发布 dev 且未发布增量已保全的远端 lane，不等待专用 publisher/broker；hosted 资格强制仍是 daily-merge OPEN-004 `track`，不阻塞有效本地验收后的日常合入。服务端普通授权凭据仍可 FF dev，不能保证 Alpha 必经；dev 禁删/禁 non-FF 与 main promotion 强制继续保留。
