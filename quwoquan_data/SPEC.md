...
不属于 CLI / tools 原生能力的职责：

- 页面内 `main_entity / members / aliases` 的最终语义抽取
- 主实体 / 子实体 / 并列实体 / 待审的最终判定

上述语义结论统一由编程助手读取 normalization input 后输出结构化 result，CLI 只负责：

- 取页与 hydrate
- 构建 normalization input
- 校验 normalization output schema
- compile / materialize
- topic 与 package gate
...
python3 quwoquan_data/tools/cli.py data normalize-run-extract --batch-label "sichuan_catalog_20260512"   # 缺结果时生成 assistant_tasks/extract.json
python3 quwoquan_data/tools/cli.py data normalize-run-review --batch-label "sichuan_catalog_20260512"    # 缺结果时生成 assistant_tasks/review.json
python3 quwoquan_data/tools/cli.py data normalize-run-authority --batch-label "sichuan_catalog_20260512" # 缺结果时生成 assistant_tasks/authority.json
python3 quwoquan_data/tools/cli.py data build-entities-tags --phase normalize-prepare --spec quwoquan_data/runtime/specs/sichuan_chuanxi_attractions_full.yaml --batch-label "sichuan_catalog_20260512" --topics "poi_node_11405204678"
python3 quwoquan_data/tools/cli.py data build-entities-tags --phase normalize-validate --batch-label "sichuan_catalog_20260512" --stage extract
python3 quwoquan_data/tools/cli.py data build-entities-tags --phase compile --batch-label "sichuan_catalog_20260512"
python3 quwoquan_data/tools/cli.py data build-entities-tags --phase materialize --batch-label "sichuan_catalog_20260512" --catalog quwoquan_data/runtime/seed/sichuan_chuanxi_attractions_catalog.ndjson --output-name "normalized_entities.ndjson"
...
# 省级全量批次编排（默认四川；需稳定网络；见脚本内环境变量说明）:
...
- **脚本**：`quwoquan_data/scripts/util/clean_quwoquan_data_runtime_generated.sh`（默认交互确认输入 `YES`；无人值守：`FORCE=1`）
