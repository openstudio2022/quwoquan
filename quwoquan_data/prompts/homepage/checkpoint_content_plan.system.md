...
<constraints>
  <always>
    ...
    - 底稿中心 1:1：每个内容对象只绑定一个合格 source unit，同一底稿不得复用；按 effective spec 的 entityArticlesPerTarget/imageWorksPerTarget/videoWorksPerTarget 硬配额选择质量最高的底稿，不得把所有候选都写入 packet，也不得超额。writingIntent 是底稿派生的可选标签，不作为凑配额角度。
  ...
  <never>
    - 不得沿用旧 2+2/角度配额示例，也不得把 execution 配额解释为仅供参考的饱和值。
