...
模板，`.py`（经 `_common/prompt_render.py`）只负责「加载模板 + 计算动态数据块 + 校验占位符 + 组装」。
...
├── system/                   # 各环节系统提示词（静态，可缓存，放在前）
│   ├── article_author.system.md
│   ├── entity_homepage.system.md
│   ├── image_curation.system.md
│   └── review_repair.system.md
├── task/                     # 各环节任务/内容模板（动态，放在后）
│   ├── article_author.task.md
│   ├── entity_homepage.task.md
│   ├── image_curation.task.md
│   └── review_repair.task.md
├── partials/                 # 复用片段（被 system/task 经 {{> partials/x.md}} 包含）
...
- `{{> partials/x.md}}`：包含复用片段（递归展开，可含自己的 `{{var}}`）。
- 渲染后**不得残留** `{{` / `}}`（未闭合占位符 = 契约违例）。
...
- `python3 quwoquan_data/scripts/cli.py template prompt-lint`：占位符闭合 / vars 必填齐备 /
  系统/任务行数预算 / `scripts/**` 不得硬编码 prompt 正文（ratchet）。
- 渲染契约测试 `tests/local_contract/common/test_prompt_render__local_contract_test.py`：
  fixtures 渲染产物含正确分区与已填变量、缺必填变量报错；已接入 `verify_quwoquan_data.sh`。
