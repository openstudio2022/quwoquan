...
以及 caption 退化门与 content_evidence 繁简归一复用同一真相源（无第二套阈值/折叠表）。
可直接运行：python3 quwoquan_data/tests/common/test_localization_simplified_chinese.py
"""
...
from _common.localization import (  # noqa: E402
    fold_to_simplified,
...
def test_content_evidence_fold_reuses_shared_table():
    """content_evidence 繁简归一复用共享折叠表（无第二套表）。"""
    from _common import content_evidence as ce

    assert ce._fold_zh_variants("臺灣") == "台湾"
