...
from quality.dirty_data import delete_dirty_data, scan_dirty_data  # noqa: E402


def _seed_dirty_homepage() -> Path:
    entity_dir = _TMP / "runtime" / "tasks" / "旅行" / "地域" / "四川省" / "景区" / "景区全覆盖" / "entities" / "地点" / "景区" / "毕棚沟"
    ...
    write_json(entity_dir / "_entity.json", {"label": "毕棚沟", "domain": "地点", "type": "景区", "sourceTaskId": "t"})
...
def _seed_dirty_post() -> Path:
    post_dir = _TMP / "runtime" / "tasks" / "旅行" / "地域" / "四川省" / "景区" / "景区全覆盖" / "batches" / "b1" / "produce" / "posts" / "article" / "都江堰" / "1"
    ...
    write_json(
        post_dir / "manifest.json",
        {
            ...
            "sourceTaskId": "t",
            "assets": [{"assetId": "missing_cover", "fileName": "img_01.jpg"}],
            "articleAssetManifest": {"assets": [{"assetId": "missing_cover", "sha256": "sha256:" + "0" * 64}]},
        },
    )
    return post_dir
