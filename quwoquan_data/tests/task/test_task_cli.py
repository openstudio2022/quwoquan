...
def test_latest_post_outputs_points_to_runtime_article():
    ...
    tid = spec["taskId"]
    post_dir = store.runtime_task_root(tid) / "batches" / "b1" / "posts" / "article" / "九寨沟纪行" / "1"
    ...
    assert outputs[0]["articlePath"].endswith("batches/b1/posts/article/九寨沟纪行/1/article.md")
