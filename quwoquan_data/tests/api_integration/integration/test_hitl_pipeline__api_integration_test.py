...
可直接运行：python3 quwoquan_data/tests/api_integration/integration/test_hitl_pipeline__api_integration_test.py
"""
...
_RUNTIME_ROOT = Path(tempfile.mkdtemp(prefix="hitl_pipeline_rt_"))
...
def _run_pipeline() -> Path:
    ...
    write_batch_manifest(TASK, BATCH, command="workflow_run", content_type="article")
...
def test_mined_entity_enters_review_without_placeholder_homepage():
    ...
    candidate = CandidateRepository(_RUNTIME_ROOT / "governance").get(ent["candidateId"])
