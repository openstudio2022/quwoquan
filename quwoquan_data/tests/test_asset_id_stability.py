...
def test_readable_ascii_tokens_preserved():
    aid = asset_id_from_object_key("asset-seed/post/稻城亚丁_体验/detail_1.jpg")
    assert aid.startswith("data_asset_")
    # 可读 ASCII token 保留
    assert "asset_seed_post" in aid
    assert "detail_1_jpg" in aid


def test_no_version_segment():
    # 新 objectKey 不再含 /v1/，旧 id 也不应再出现 v1 段
    aid = asset_id_from_object_key("asset-seed/post/x/detail_1.jpg")
    assert "_v1_" not in aid


def test_stable_for_same_input():
    k = "asset-seed/post/洛绒牛场/cover.jpg"
...
def test_unique_for_distinct_chinese_topics():
    ...
    a = asset_id_from_object_key("asset-seed/post/稻城亚丁/cover.jpg")
    b = asset_id_from_object_key("asset-seed/post/洛绒牛场/cover.jpg")
    assert a != b
