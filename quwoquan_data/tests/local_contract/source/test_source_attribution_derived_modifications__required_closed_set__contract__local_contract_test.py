...
def test_retired_publication_admission_is_rejected_fail_closed() -> None:
    from content.release.canonical.pool_source_attribution import source_attribution_complete

    value = {**_attribution([]), "publicationAdmission": "research_release"}
...
def test_data_authoring_contract_has_no_object_classification_fields() -> None:
    import json

    schema = json.loads((DATA_ROOT / "schema/content/post_manifest.schema.json").read_text())
    attribution = schema["$defs"]["sourceAttribution"]
    assert "publicationAdmission" not in attribution["properties"]
    assert "publicationAdmission" not in attribution["required"]
    assert "variantPurpose" not in schema["properties"]
    assert "distributionDecision" not in schema["properties"]["assets"]["items"]["properties"]
