...
def test_frozen_publication_is_preserved_by_schema_and_pool_reader(publication: str) -> None:
    ...
    assert source_attribution_complete({"sourceAttribution": value}) is True
    with pytest.raises(ValueError, match="publicationAdmission"):
        canonical_source_attribution({**value, "publicationAdmission": "production_release"})
...
def test_data_and_service_source_attribution_authoring_contracts_agree() -> None:
    ...
    # Service 透传已冻结对象权利记录，不以 DTO enum 重新归类；Data authoring 校验原有词汇。
    assert fields["publicationAdmission"]["type"] == "string"
    assert set(attribution["properties"]["publicationAdmission"]["enum"]) == {"research_release", "commercial_release"}
