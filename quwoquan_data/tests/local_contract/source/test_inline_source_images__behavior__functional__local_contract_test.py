...
可直接运行：python3 quwoquan_data/tests/local_contract/source/test_inline_source_images__behavior__functional__local_contract_test.py
"""
...
def test_inline_extractor_merges_consecutive_images_into_figuregroup():
    ...
    """
    from core.figure_groups import iter_figure_groups, figure_image_count
...
def test_build_inline_image_candidates_maps_src_and_placeholder():
    ...
    out = build_inline_image_candidates(rows, entity_id="九寨沟")
...
def test_build_inline_image_candidates_empty_input():
    assert build_inline_image_candidates(None, entity_id="九寨沟") == []
    assert build_inline_image_candidates([], entity_id="九寨沟") == []
...
def test_inline_candidate_flows_through_gates_with_placeholder():
    ...
    try:
        ...
        patch("_assess_source_image", lambda temp, spec, execution_id=None: _OkVerdict())
