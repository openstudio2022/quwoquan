...
from content.release.canonical import rollout_milestone  # noqa: E402
...
def _closed_canary(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    ...
    evidence_paths = (
        ...
        output / "env/gamma/runs/data-release" / CANARY_RELEASE / "import-evidence" / "app_uat_cases.json",
    ...
    monkeypatch.setattr(
        rollout_milestone,
        "_assert_gamma_evidence",
        lambda **_kwargs: evidence_refs,
    )
    payload = {
...
def test_canary__attestation_binds_execution_and_gamma_evidence__local_contract(
    ...
    monkeypatch.setattr(rollout_milestone, "execution_root", lambda execution_id: roots[execution_id])
    monkeypatch.setattr(rollout_milestone, "execution_readiness_issues", lambda *_args, **_kwargs: [])
    monkeypatch.setattr(rollout_milestone, "homepage_media_completeness_report", lambda _id: {"passed": True})
    refs = ["地点/景区/普陀山", "地点/自然景观/东钱湖", "地点/景区/海螺沟"]
    _write(payload_file(release, "release.json"), {"schemaVersion": "quwoquan_data.release/3", "releaseId": CANARY_RELEASE, "releaseKind": "content", "executionIds": [ZHEJIANG, SICHUAN], "rolloutMilestone": "canary"})
    ...
    _write(import_root / "result.json", {"environment": "gamma", "releaseId": CANARY_RELEASE, "status": "completed", "appUatCasesRef": cases_ref})
    _write_full_sync_receipts(import_root, CANARY_RELEASE, refs)
    _write(import_root / "app_uat_cases.json", {"schemaVersion": "quwoquan_data.gamma_app_uat_case_manifest/1", "environment": "gamma", "releaseId": CANARY_RELEASE, "runId": "import-001", "importerReportRef": importer_ref, "generatedAt": "2026-07-14T00:00:00Z", "cases": [{"entityRef": ref, "homepageId": homepage_id, "title": ref.rsplit("/", 1)[-1]} for ref, homepage_id in mapping.items()]})
    ...
    _write(api_root / "run.json", {"environment": "gamma", "releaseId": CANARY_RELEASE, "kind": "homepage_api_verification"})
    ...
    _write(api_root / "homepage-api-verification.json", {"schemaVersion": "quwoquan_data.gamma_homepage_api_verification/1", "environment": "gamma", "releaseId": CANARY_RELEASE, "runId": "api-001", "sourceUatCasesRef": cases_ref, "apiBaseUrl": "https://gamma.example.test", "verifiedAt": "2026-07-14T00:00:00Z", "passed": True, "entities": [{"entityRef": ref, "homepageId": homepage_id, "title": ref.rsplit("/", 1)[-1], "detailStatus": 200, "introductionStatus": 200, "coverUrl": f"https://media.example.test/{homepage_id}.jpg", "sectionCount": 1} for ref, homepage_id in mapping.items()], "issues": []})

    app_report = output / "env/gamma/runs/patrol-001/report.json"
    _write(app_report, {"status": "passed", "runtimeEnv": "gamma", "apiContractEnv": "gamma", "dataSource": "remote", "releaseUatCasesPath": cases_ref, "runs": [{"exitCode": 0}]})
    ...
    report = rollout_milestone.build_rollout_milestone_attestation(
