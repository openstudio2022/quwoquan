...
def _manifest(*, digest: str, perceptual_hash: str, asset_id: str = "image-1") -> dict[str, object]:
    return {
        "contentType": "image",
...
def test_cross_execution_perceptual_duplicate_is_rejected(
    ...
    write_json(
        package / "object/manifest.json",
        _manifest(digest="sha256:" + "b" * 64, perceptual_hash="0" * 15 + "3", asset_id=asset_id),
    )
    monkeypatch.setattr(subject, "PUBLISH_ROOT", publish)
