...
def test_trigger_entity_reload_posts_metadata_route_and_writes_report(monkeypatch):
    """契约：POST {base}/homepages:reload（ReloadHomepageState），结果落审计报告。"""
    import content.release.environment.reload as mod

    calls: list[list[str]] = []

    def _fake_run(argv, input=None, capture_output=False, check=False):
        calls.append(list(argv))
        assert input == b"Authorization: Bearer local-release-token\n"
    ...
    with tempfile.TemporaryDirectory(prefix="qwq_ship_reload_") as tmp:
        ...
        report = mod.trigger_entity_reload(
            "http://localhost:18084/",
            authorization_header="Bearer local-release-token",
            release_id="rel_test_001",
        )

        assert report is not None and report.is_file()
        assert calls and "http://localhost:18084/homepages:reload" in calls[0]
        assert "local-release-token" not in " ".join(calls[0])
        data = json.loads(report.read_text(encoding="utf-8"))
        assert "local-release-token" not in json.dumps(data)
        assert data["ok"] is True
        assert data["httpStatus"] == "200"
        assert data["endpoint"].endswith("/homepages:reload")
...
def test_trigger_entity_reload_end_to_end_real_http_roundtrip(monkeypatch):
    ...
    ship --entity-reload-url → POST /homepages:reload → 内存合并 全链闭环。
    """
    import content.release.environment.reload as mod
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="qwq_ship_reload_") as tmp:
            monkeypatch.setattr(mod, "OUTPUT_ROOT", Path(tmp))
            report = mod.trigger_entity_reload(
                f"http://127.0.0.1:{port}",
                authorization_header="Bearer local-release-token",
                release_id="rel_test_e2e",
            )
            assert seen == {"path": "/homepages:reload", "method": "POST"}
            data = json.loads(report.read_text(encoding="utf-8"))
            assert data["ok"] is True
            assert data["httpStatus"] == "200"
