"""全国地点静态覆盖身份门契约测试（discovery_seed/2，WP1）。
...
from governance.coverage.master_list import coverage_identity_key
import verify.verify_coverage_static_identity as coverage_gate
from verify.verify_coverage_static_identity import (
...
def test_schema_contract_is_single_source():
    ...
    assert "generatedBy" not in contract["fileFields"]


def test_coverage_identity_key_is_stable_and_scope_bound():
    fields = {
        "country": "中国",
        "province": "四川省",
        "city": "成都市",
    ...
    assert coverage_identity_key(**fields) == coverage_identity_key(**fields)
    assert coverage_identity_key(**fields).startswith("coverage_")
    assert coverage_identity_key(**{**fields, "district": "青羊区"}) != coverage_identity_key(**fields)
...
def test_c2_missing_required_and_invalid_priority_blocked(tmp_path):
    root = _tmp_country_root(tmp_path)
...
def test_c10_derived_coverage_key_collision_blocked(tmp_path, monkeypatch):
    root = _tmp_country_root(tmp_path)
    second = {**_VALID_LEAF, "name": "青城山", "canonicalName": "青城山"}
    _write_master_file(root, leaves=[dict(_VALID_LEAF), second])
    monkeypatch.setattr(coverage_gate, "coverage_identity_key", lambda **_: "coverage_collision")
    errors = _scan(root)
    assert any(error.startswith("C10") and "coverage_collision" in error for error in errors), errors
