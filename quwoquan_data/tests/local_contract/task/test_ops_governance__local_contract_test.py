...
def test_runtime_protection_manifest_declares_protected_paths_and_lease():
    """跑批保护协议：清单必须声明 batch 根/lease/frozen plan 与 pid，供外部治理代理清理前读取。"""
    batch = "runtime_protection"
    path = og.write_runtime_protection_manifest(
        TASK, batch, plan_id="plan_guard", protected_paths=["/extra/protected"]
    )
    manifest = read_json(path)
    assert manifest["schema"] == og.RUNTIME_PROTECTION_SCHEMA
    assert manifest["taskId"] == TASK and manifest["batchId"] == batch
    assert manifest["planId"] == "plan_guard"
    assert manifest["pid"] == os.getpid()
    protected = manifest["protectedPaths"]
    assert str(og.batch_root(TASK, batch)) in protected
    assert str(og.controller_lease_path(TASK, batch)) in protected
    assert "/extra/protected" in protected
    assert manifest["leaseRef"] == str(og.controller_lease_path(TASK, batch))
    # 幂等重写：同批次重复声明不产生重复路径。
