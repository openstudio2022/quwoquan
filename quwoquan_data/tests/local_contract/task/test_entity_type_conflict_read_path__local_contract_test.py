...
3. audit_managed_batch 对多类型分区（batch etype 空）在 canonical 目录物化后可用，
   不得因空 hint 默认类型触发假冲突。
...
from _common.paths import batch_entity_object_dir, ensure_batch_layout  # noqa: E402
from _common.source_unit import resolve_entity_object_dir  # noqa: E402
from task import store  # noqa: E402

_BATCH = "etype_conflict_batch"
...
def _materialize(task_id: str, domain: str, etype: str, name: str) -> Path:
    obj = batch_entity_object_dir(task_id, _BATCH, domain, etype, name)
...
def test_unmaterialized_default_hint_does_not_raise_against_canonical():
    task_id = _mixed_type_task("类型冲突读路径批_未物化")
    ensure_batch_layout(task_id, _BATCH, "download")
    _materialize(task_id, "地点", "景区", "嵊泗列岛")
    # 空 hint → 默认打卡地路径；打卡地目录不存在，不得判为漂移共存。
    obj = resolve_entity_object_dir(task_id, _BATCH, "嵊泗列岛", etype_hint="")
    assert obj.name == "嵊泗列岛" and obj.parent.name == "打卡地", obj
    # 显式错误 hint 同理：只要错类型目录未物化，读解析不抛。
    obj2 = resolve_entity_object_dir(task_id, _BATCH, "嵊泗列岛", etype_hint="地点/打卡地")
    assert obj2 == obj


def test_dual_materialized_trees_still_raise():
    task_id = _mixed_type_task("类型冲突读路径批_双物化")
    ensure_batch_layout(task_id, _BATCH, "download")
    _materialize(task_id, "地点", "景区", "嵊泗列岛")
    _materialize(task_id, "地点", "打卡地", "嵊泗列岛")
    for hint in ("", "地点/打卡地", "地点/景区"):
        try:
            resolve_entity_object_dir(task_id, _BATCH, "嵊泗列岛", etype_hint=hint)
...
def test_audit_managed_batch_usable_on_mixed_type_partition():
    task_id = _mixed_type_task("类型冲突读路径批_审计")
    ensure_batch_layout(task_id, _BATCH, "download")
    _materialize(task_id, "地点", "景区", "嵊泗列岛")
    _materialize(task_id, "地点", "自然景观", "花鸟岛")
