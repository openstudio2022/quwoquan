...
def test_stable_production_proof_stdout_is_deterministic_and_read_only(tmp_path: Path) -> None:
    ...
    arguments = ("governance", "stable-production-proof", "--request", str(request_path), "--artifact-root", str(root))
    ...
    assert json.loads(first.stdout)["executionCount"] == 12
    assert before == after


def test_stable_production_proof_has_no_output_write_option(tmp_path: Path) -> None:
    request = _write_request(tmp_path / "request.json", {"schema": subject.REQUEST_SCHEMA, "fingerprint": "sha256:" + "f" * 64, "proofUnits": []})
...
def test_stable_production_proof_error_names_three_proof_units(tmp_path: Path) -> None:
    request = _write_request(tmp_path / "request.json", {"schema": subject.REQUEST_SCHEMA, "fingerprint": "sha256:" + "f" * 64, "proofUnits": []})
    ...
    assert "exactly three proof units" in result.stderr


def test_stable_production_proof_rejects_stale_current_fingerprint(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    request = {"schema": subject.REQUEST_SCHEMA, "fingerprint": "sha256:" + "e" * 64, "proofUnits": []}
...
def test_legacy_retirement_precheck_accepts_operationally_retired_without_delete(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    proof = {"schema": proof_subject.SCHEMA, "expectedFingerprint": "sha256:" + "f" * 64, "releaseIds": ["release-1", "release-2", "release-3"], "proofUnitCount": 3, "executionCount": 12, "verdict": "pass"}
