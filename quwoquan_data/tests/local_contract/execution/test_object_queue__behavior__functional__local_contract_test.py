...
def test_enqueue_does_not_downgrade_succeeded_job():
    ...
    oq.complete_job(batch, leased["jobId"], leased["lease"])
    ...
    assert read_json(oq._job_path(batch, job["jobId"]))["state"] == oq.STATE_SUCCEEDED
...
def test_reliabletask_backend_records_bridge_and_requires_envelope():
    ...
    assert job["reliableTaskRef"]["idempotencyKey"].count("|") == 2
    leased = oq.acquire_lease(batch, worker="w1", stage="author")
...
def test_malformed_governance_job_blocks_and_writes_failure_ledger():
    ...
    path = oq._job_path(batch, job["jobId"])
...
def test_lease_complete_lifecycle():
    ...
    done = oq.complete_job(BATCH, job["jobId"], job["lease"])
...
def test_governed_article_author_complete_requires_real_agent_draft():
    ...
    job = oq.acquire_lease(batch, worker="w1", stage="author")
    completed = oq.complete_job(batch, job["jobId"], job["lease"])
...
def test_failure_escalates_to_dead():
    ...
    r1 = oq.fail_job(batch, j1["jobId"], j1["lease"], error="boom1")
    assert r1["state"] == oq.STATE_FAILED  # attempt 1 < max 2
    # 失败退避：模拟退避窗口已过，job 可再次重取
    path = oq._job_path(batch, j1["jobId"])
    payload = read_json(path)
    ...
    r2 = oq.fail_job(batch, j2["jobId"], j2["lease"], error="boom2")
...
def test_lease_mismatch_rejected():
    ...
    job = oq.acquire_lease(batch, worker="w1", stage="author")
    try:
        oq.complete_job(batch, job["jobId"], "someone-else:0")
    except RuntimeError as exc:
...
def test_reconcile_completed_refs_marks_terminal_success():
    ...
    job = oq.acquire_lease(batch, worker="w1", stage="author")
    oq.fail_job(batch, job["jobId"], job["lease"], error="dead now")
...
def test_fail_sets_backoff_not_before():
    ...
    j1 = oq.acquire_lease(batch, worker="w1", stage="author")
    oq.fail_job(batch, j1["jobId"], j1["lease"], error="boom")
    payload = read_json(oq._job_path(batch, j1["jobId"]))
...
def test_startup_failure_uses_independent_startup_budget():
    ...
    payload = read_json(oq._job_path(batch, job1["jobId"]))
    payload["notBeforeEpoch"] = 0
    from core.io import write_json
    write_json(oq._job_path(batch, job1["jobId"]), payload)
    ...
    payload = read_json(oq._job_path(batch, job2["jobId"]))
    payload["notBeforeEpoch"] = 0
    write_json(oq._job_path(batch, job2["jobId"]), payload)
...
def test_stuck_detection_forces_dead_before_max_attempts():
    ...
    for _ in range(3):
        ...
        job = oq.acquire_lease(batch, worker="w1", stage="author")
        assert job is not None
        last = oq.fail_job(batch, job["jobId"], job["lease"], error="same issue", fingerprint=fp)
...
def test_usage_budget_exceeded_forces_dead():
    ...
    job = oq.acquire_lease(batch, worker="w1", stage="author")
    mid = oq.record_usage(batch, job["jobId"], job["lease"], tokens=400)
    assert mid["state"] == oq.STATE_LEASED  # 未超预算仍在跑
    res = oq.record_usage(batch, job["jobId"], job["lease"], tokens=700)
...
def test_complete_with_valid_envelope_succeeds():
    ...
    done = oq.complete_job_with_envelope(batch, job["jobId"], job["lease"], envelope_path=envelope_path)
    assert done["state"] == oq.STATE_SUCCEEDED
...
def test_complete_with_envelope_rejects_write_outside_assignment_roots():
    ...
    failed = oq.complete_job_with_envelope(batch, job["jobId"], job["lease"], envelope_path=envelope_path)
