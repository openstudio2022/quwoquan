"""execution/_shared 证据面瘦身契约（数据输出规范）。

单一真相源：`core.paths` 的
- `EXECUTION_SHARED_AUTHORITATIVE_ENTRIES`：不可重算真相源（readiness/审计只认它们）；
- `EXECUTION_SHARED_RECLAIMABLE_ENTRIES` + `tmp_*` 前缀：调试/过程层，可随时清理；
- 其余条目 unknown → 目录证据链门 BLOCK。

execution/_shared 最小证据面同样由 `EXECUTION_SHARED_ALLOWED_ENTRIES` 冻结。
"""
...
def test_role_partition_is_disjoint_and_tmp_prefix_reclaimable():
    overlap = (
        paths_mod.EXECUTION_SHARED_AUTHORITATIVE_ENTRIES
        & paths_mod.EXECUTION_SHARED_RECLAIMABLE_ENTRIES
    )
    assert overlap == set(), overlap
    assert paths_mod.execution_shared_entry_role("tmp_source_unit_image_checks") == "reclaimable"
    assert paths_mod.execution_shared_entry_role("made_up_second_truth.json") == "unknown"


def test_gate_blocks_unknown_shared_entry(tmp_path):
    execution = tmp_path / "execution"
    shared = execution / "_shared"
    ...
    issues = _execution_shared_issues(execution)
    assert len(issues) == 1
