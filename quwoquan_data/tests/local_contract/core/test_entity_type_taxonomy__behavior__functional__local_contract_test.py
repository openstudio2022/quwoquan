...
- 类型口径归一：execution 输入门的 known_entity_types 只来自 Entity 树一级节点；
  餐饮/住宿细分不再作为合法 entityType 主类型；
...
def test_known_entity_types_come_from_entity_tree_only():
    ...
    # 餐饮/住宿细分不可作 entityType 主类型（细分经 typeTagRefs 表达）
    for flat in ("地点/咖啡馆", "地点/茶馆", "地点/酒吧", "地点/火锅店",
                 "地点/民宿", "地点/客栈", "地点/酒店", "地点/度假村"):
        assert flat not in types, flat


def test_entity_subtypes_map_to_entity_tree_levels():
    """餐饮/住宿细分是 Entity 树子级节点，而非第二份类型目录。"""
...
def test_contract_tags_root_is_repo_path_not_runtime_root():
    ...
    assert CONTRACT_TAGS_ROOT == _REPO_DATA_ROOT / "control_plane" / "governance" / "taxonomy"
...
def test_required_entity_fields_subset_of_publish_schema():
    from content.homepage.homepage import _REQUIRED_ENTITY_FIELDS
