...
可直接运行 python3 quwoquan_data/tests/local_contract/core/test_content_object_router__behavior__functional__local_contract_test.py
"""
...
from content.post import object_index as co  # noqa: E402
...
def test_compute_coords_allows_empty_image_title_hint_and_uses_ref_for_routing():
    ...
    registered = co.register_from_brief(_IMAGE_TASK, ref, brief, "image")
...
def test_register_and_resolve_object_dirs():
    brief = {"titleHint": "峨眉山·行前安排", "carrier": "article", "writingIntent": "planning_consultation"}
    ref = "地点_景区__峨眉山"
    coords = co.register_from_brief(_TASK, ref, brief)
    assert coords["seq"] == 1
    assert coords["angle"] == "攻略"
    # 路由索引落 _shared
    idx = co.index_path(_TASK)
    assert idx.parent.name == "_shared"
    assert idx.parent.parent == execution_root(_TASK)
    # 对象目录 = posts/{type}/{angle}/{title}/{seq}
    obj = co.content_object_dir(_TASK, ref)
    rel = obj.relative_to(execution_root(_TASK)).as_posix()
    assert rel == "posts/article/攻略/峨眉山·行前安排/1", rel
    # 阶段目录带序号
    assert co.content_object_stage_dir(_TASK, ref, "3.compose").name == "3.compose"
    assert co.content_object_stage_dir(_TASK, ref, "4.draft").parent == obj


def test_register_idempotent_keeps_seq():
    brief = {"titleHint": "稻城亚丁·攻略", "templateId": "环线"}
    ref = "地点_景区__稻城亚丁"
    c1 = co.register_from_brief(_TASK, ref, brief)
    c2 = co.register_from_brief(_TASK, ref, brief)
...
def test_same_title_collision_gets_stable_seq():
    ...
    a = co.register_from_brief(_TASK, "ref_b", brief)
    b = co.register_from_brief(_TASK, "ref_a", brief)
    seqs = {"ref_a": co.content_coords(_TASK, "ref_a")["seq"],
            "ref_b": co.content_coords(_TASK, "ref_b")["seq"]}
...
def test_write_brief_same_title_does_not_orphan_existing_brief():
    ...
    first_path = co.write_brief_object(_TASK, first, brief)
    co.write_brief_object(_TASK, second, brief)

    assert first_path.is_file()
    assert (co.content_object_stage_dir(_TASK, first, "3.compose") / co.BRIEF_FILE).is_file()
    assert (co.content_object_stage_dir(_TASK, second, "3.compose") / co.BRIEF_FILE).is_file()


def test_unregistered_ref_raises():
    try:
        co.content_object_dir(_TASK, "ref_never")
    except KeyError:
        return
...
def test_register_content_object_rejects_empty_title():
    try:
        co.register_content_object(_TASK, "bad_ref", content_type="article", angle="攻略", title=" ")
    except ValueError as exc:
...
def test_register_content_object_rejects_cross_carrier_execution():
    try:
        ...
        assert "conflicts with immutable execution identity" in str(exc)
        return
    raise AssertionError("expected cross-carrier execution rejection")
...
def test_register_content_object_requires_execution_manifest():
    ...
    try:
        ...
        assert "execution manifest does not exist" in str(exc)
        return
    raise AssertionError("expected missing execution manifest rejection")
