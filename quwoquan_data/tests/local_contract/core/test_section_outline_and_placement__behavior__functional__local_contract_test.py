...
可直接运行：python3 quwoquan_data/tests/local_contract/core/test_section_outline_and_placement__behavior__functional__local_contract_test.py
"""
...
from core.asset_placement import caption_semantic_issues, place_assets_in_markdown, referenced_asset_ids  # noqa: E402
from core.section_outline import (  # noqa: E402
...
def test_outline_appendix_aliases_are_not_required_sections() -> None:
    source = (
        "## 历史\n\n"
        + ("历史事实。" * 80)
        + "\n\n## 参考来源\n\n"
    ...
    required = outline_required_sections(parse_section_outline(source), min_body_chars=120)
    assert [node.title for node in required] == ["历史"]
