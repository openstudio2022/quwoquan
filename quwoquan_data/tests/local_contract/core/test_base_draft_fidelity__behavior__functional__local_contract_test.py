...
def test_base_draft_allowed_lanes_decouples_by_carrier():
    ...
    assert base_draft_allowed_lanes("homepage") == {"homepage", "encyclopedia"}
    for invalid in (None, "", "unknown_carrier", "gallery"):
        with pytest.raises(ValueError):
            base_draft_allowed_lanes(invalid)


def test_assign_base_draft_article_carrier_excludes_image_lane_source():
    """三类解耦：文章载体底稿只从 article 研究 lane 认领，不误选 image 图库来源。"""
    obj = resolve_entity_object_dir(EXECUTION_ID, "螺髻山", etype_hint="景区")
...
def test_assign_base_draft_image_carrier_excludes_article_lane_source():
    """三类解耦：图片作品载体底稿只从 image lane 认领，不误选文章游记来源。"""
    obj = resolve_entity_object_dir(EXECUTION_ID, "邛海", etype_hint="景区")
    article_unit = write_source_unit(
        obj,
        ordinal=1,
        source_id="qunar_article_base",
    ...
    chosen = assign_base_draft(
        EXECUTION_ID,
        "post://邛海_画报",
        {"entityRefs": ["地点/景区/邛海"], "carrier": "image"},
    )
...
def test_assign_base_draft_declared_wrong_lane_ref_is_reassigned_to_correct_lane():
    ...
    obj = resolve_entity_object_dir(EXECUTION_ID, "泸山", etype_hint="景区")
    image_unit = write_source_unit(
        obj,
        ordinal=1,
...
def test_assign_base_draft_rejects_declared_reject_source():
    obj = resolve_entity_object_dir(EXECUTION_ID, "黄龙", etype_hint="景区")
    reject = write_source_unit(
        obj,
        ordinal=1,
        source_id="reject_probe",
...
def test_assign_base_draft_reassigns_when_declared_source_taken_by_peer():
    obj = resolve_entity_object_dir(EXECUTION_ID, "都江堰", etype_hint="景区")
...
def test_load_base_draft_text_prefers_source_clean():
    obj = resolve_entity_object_dir(EXECUTION_ID, "峨眉山", etype_hint="景区")
