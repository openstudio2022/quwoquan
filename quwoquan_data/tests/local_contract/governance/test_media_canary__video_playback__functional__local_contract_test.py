...
from governance import media_canary
...
def test_media_canary_freezes_minute_hour_and_rejection_boundaries() -> None:
    profile = load_media_canary_profile()

    assets = {asset.asset_id: asset for asset in profile.assets}
    minute = assets["media-canary-seek-125s"]
    near_hour = assets["media-canary-hour-boundary-3595s"]
    over_limit = assets["media-canary-over-limit-3605s"]
...
def test_media_canary_public_slices_are_canonical_and_unique() -> None:
    profile = load_media_canary_profile()
...
def test_media_canary_encoder_is_byte_deterministic(tmp_path: Path) -> None:
    profile = load_media_canary_profile()
    asset = MediaCanaryAsset(
        asset_id="media-canary-determinism-2s",
        asset_version=1,
        duration_ms=2_000,
        frames_per_second=2,
        expected_processing_status="ready",
        storyboard=False,
        public_slice_prefix="media/video/s/media-canary-determinism-2s/v1",
    )
...
def test_media_canary_validation_reprobes_descriptor_fields(
    tmp_path: Path,
    monkeypatch,
) -> None:
    base_profile = load_media_canary_profile()
    asset = MediaCanaryAsset(
        asset_id="media-canary-probe-5s",
        asset_version=1,
        duration_ms=5_000,
        frames_per_second=2,
        expected_processing_status="ready",
        storyboard=True,
        public_slice_prefix="media/video/s/media-canary-probe-5s/v1",
    )
    profile = replace(
        base_profile,
        assets=(asset,),
    ...
    monkeypatch.setattr(media_canary, "load_media_canary_profile", lambda: profile)

    media_canary.prepare_media_canary_assets(output_root=tmp_path)

    assert media_canary.validate_media_canary_assets(output_root=tmp_path) == []

    descriptor_path = tmp_path / asset.public_slice_prefix / "descriptor.json"
    descriptor = json.loads(descriptor_path.read_text(encoding="utf-8"))
    descriptor["durationMs"] = 1
    descriptor["probeHash"] = media_canary._descriptor_probe_hash(descriptor)
    descriptor_path.write_text(
        json.dumps(descriptor, ensure_ascii=False, indent=2, sort_keys=True),
        encoding="utf-8",
    )

    issues = media_canary.validate_media_canary_assets(output_root=tmp_path)
