...
可直接运行：python3 quwoquan_data/tests/local_contract/common/test_agent_executor_contract__local_contract_test.py
"""
...
from _common.agent_executor_contract import scan_agent_executor_contract  # noqa: E402
...
def test_flags_missing_delivery_guard_anchor():
    # 空目录缺少 materialize/draft_io/entity_extract 防线锚点 → 必须报缺失。
    tmp = Path(tempfile.mkdtemp())
    issues = scan_agent_executor_contract(tmp)
    assert any("materialize.py" in i for i in issues), issues
