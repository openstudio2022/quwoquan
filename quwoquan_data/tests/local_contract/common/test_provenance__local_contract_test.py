...
可直接运行：python3 quwoquan_data/tests/local_contract/common/test_provenance__local_contract_test.py
"""
...
def test_source_bundle_hash_accepts_runtime_relative_path_with_batch_prefix():
    ...
    with tempfile.TemporaryDirectory(prefix="source_bundle_hash_") as tmp:
        os.chdir(tmp)
        try:
            batch = Path(".qwq_output/data/local/runtime/batches/demo_batch")
