"""主清单共享库契约（WP2 枚举 SOP）：walk/统计/地理覆盖门/decompose 投影 单一真相源。
...
- fanout_plan.discovery_gate_issues 在 plan 声明 geoCoverage 时必须接线地理覆盖门。
"""
...
from _common import coverage_master_list as cml
...
def test_master_list_stats_counts_types_and_readiness(tmp_path):
    root = tmp_path / "coverage" / "中国"
    _write_city_file(root, "测试省", "甲市", {"一区": ["点A"], "二县": ["点B"]}, readiness="ready")
    ...
    assert stats["bySourceReadiness"] == {"ready": 2}
    assert stats["crossProvinceLeaves"] == []


def test_fanout_discovery_gate_wires_geo_coverage(tmp_path, monkeypatch):
    """plan 声明 geoCoverage 时，发现门必须叠加地理覆盖校验（唯一接线点）。"""
    calls: list[tuple] = []

    def _fake_geo(provinces, *, country="中国", **_kw):
        calls.append((tuple(provinces), country))
        return [f"geo-coverage: {provinces[0]} 缺市州文件 'X.yaml'"]

    monkeypatch.setattr(cml, "geo_coverage_issues", _fake_geo)
    plan = fp.new_plan("plan_geo_gate", "全国景点主页", "travel", partition_dimension="省")
    fp.add_partition(plan, "测试省")
    fp.add_leaves(plan, ["测试省"], [{"name": "点A"}])
    assert fp.discovery_gate_issues(plan) == []  # 未声明 geoCoverage 不触发
    plan["geoCoverage"] = {"country": "中国", "provinces": ["测试省"]}
    issues = fp.discovery_gate_issues(plan)
    assert any("geo-coverage" in i for i in issues), issues
    assert calls == [(("测试省",), "中国")]
