...
可直接运行：
    python3 quwoquan_data/tests/local_contract/download/test_research_entity_type_guard__local_contract_test.py
"""
...
from download.prepare import (  # noqa: E402
    prepare_source_plan,
    resolve_research_entity_types,
)
from task import store  # noqa: E402

_BATCH = "etype_guard_batch"
...
def test_canonical_type_overrides_wrong_single_value_hint():
    ...
    # 别名同样命中 canonical。
    assert resolve_research_entity_types(task_id, ["东沙古渔镇"])["东沙古渔镇"] == "地点/古镇"


def test_missing_type_fails_fast_instead_of_default_spot_dir():
    task_id = _mixed_type_task()
    try:
        resolve_research_entity_types(task_id, ["花鸟岛"], fallback_type="")
...
def test_prepare_source_plan_corrects_dir_and_refuses_default_drift():
    ...
    canonical_plan = (
        batch_entity_object_dir(task_id, _BATCH, "地点", "自然景观", "秀山岛")
        / "1.download"
        / "homepage_source_plan.json"
    )
    drift_dir = batch_entity_object_dir(task_id, _BATCH, "地点", "打卡地", "秀山岛")
    ...
    assert not batch_entity_object_dir(task_id, _BATCH, "地点", "打卡地", "花鸟岛").exists()
