...
- research_plan._curl_json/_curl_text 网络级退出码计入断路器并在打开后秒级短路；
...
from download.research.network_breaker import (  # noqa: E402
...
def test_curl_layer_reports_network_exit_codes_and_short_circuits(monkeypatch):
    import download.research_plan as rp
    ...
    monkeypatch.setattr("download.research.network_breaker.BREAKER", breaker)
    ...
    assert rp._curl_json(url) == {}
    assert rp._curl_json(url) == {}
    assert len(calls) == 2
    # 阈值已到：第三次不再执行 curl（秒级短路）。
    assert rp._curl_json(url) == {}
    assert rp._curl_text(url) == ""
...
def test_stage_no_progress_timeout_env_contract(monkeypatch):
    monkeypatch.delenv("QWQ_STAGE_NO_PROGRESS_TIMEOUT_SECONDS", raising=False)
    assert stage_no_progress_timeout_seconds() == 900.0
    monkeypatch.setenv("QWQ_STAGE_NO_PROGRESS_TIMEOUT_SECONDS", "120")
    assert stage_no_progress_timeout_seconds() == 120.0
    monkeypatch.setenv("QWQ_STAGE_NO_PROGRESS_TIMEOUT_SECONDS", "0")
    assert stage_no_progress_timeout_seconds() == 0.0
...
def test_write_auto_research_plans_no_progress_timeout_yields_resumable_outage(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("QWQ_STAGE_NO_PROGRESS_TIMEOUT_SECONDS", "2")
    import importlib

    import _common.paths as paths_mod
    importlib.reload(paths_mod)
    import download.research.auto_plan_public as app
    importlib.reload(app)

    release = threading.Event()

    def _stuck_impl(task_id, batch_id, entity_ids, **kwargs):
...
def test_wave_budget_short_circuits_curl_after_deadline(monkeypatch):
    ...
    import download.research_plan as rp
    from download.research import network_breaker as nb
    ...
    try:
        assert rp._curl_json("https://zh.wikipedia.org/w/api.php") == {}
        assert len(calls) == 1
