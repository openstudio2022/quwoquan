...
import produce.materialize as materialize  # noqa: E402
...
def test_resolve_semantic_mentions_merges_sidecar_into_manifest(tmp_path: Path, monkeypatch) -> None:
    ...
    merged = materialize._resolve_semantic_mentions("task", "batch", "post:1", compose_payload)
...
def test_resolve_semantic_mentions_tolerates_missing_sidecar(tmp_path: Path, monkeypatch) -> None:
    ...
    merged = materialize._resolve_semantic_mentions("task", "batch", "post:x", {})
    assert merged == []
