...
- search_queries: 数组；每项都包含 dimension、query、purpose、execution_purpose、entity_refs、tag_refs、target_domains
...
4. execution_purpose 只能取 source_page、image_asset、fact_gap、layout_reference，分别对应页面证据、图片素材、事实缺口、版式参考。
5. 只能规划公开可核验的网页证据；不要假设登录态、验证码绕过或受限接口。
6. 输出中必须保留 entity_refs 和 tag_refs，便于后续把证据落到 raw/ 与 facts.ndjson。
"""
