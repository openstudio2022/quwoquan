"""全国地点主清单门禁（discovery_seed/2，仿 verify_tag_tree R1-R12 模式）。
...
一文件自闭环、无总控 index——裁决 8），逐文件校验：
  ...
  C2  - schema 同口径结构门：schemaVersion const、必填字段、字段类型、
        未知字段阻断（必填集/enum/字段白名单从 schema/coverage/master_list.schema.json 读取，
...
标签契约树按「契约跟代码走」以仓内路径解析（_common.entity_type_taxonomy），
不随运行时 QWQ_DATA_ROOT 漂移。经 `qwq-data verify coverage-master-list` 暴露，
挂 verify_quwoquan_data.sh；本文件不提供直跑入口（CLI-first ratchet）。
"""
...
def _load_schema_contract(schema_path: Path) -> dict:
    ...
    return {
        ...
        "sourceReadinessEnum": set(leaf_schema["properties"]["sourceReadiness"]["enum"]),
    }


# 标签树节点判定唯一实现在 _common.coverage_master_list（单一真相源）。
...
def _check_leaf(
    ...
    *,
    ...
    readiness = str(leaf.get("sourceReadiness") or "")
    if readiness and readiness not in contract["sourceReadinessEnum"]:
        errors.append(f"C2: {label} sourceReadiness '{readiness}' 不在 {sorted(contract['sourceReadinessEnum'])}")
...
def main() -> int:
    ...
    print(f"[verify-coverage-master-list] files={file_count} leaves={leaf_count}")
    if errors:
        print(f"[verify-coverage-master-list] FAILED ({len(errors)} error(s)):")
        for item in errors[:100]:
            print(f"  ✗ {item}")
        if len(errors) > 100:
            print(f"  ... 还有 {len(errors) - 100} 条")
        return 1
    print("[verify-coverage-master-list] PASSED")
    return 0
