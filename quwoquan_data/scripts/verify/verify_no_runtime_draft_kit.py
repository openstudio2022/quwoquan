...
本门保证：除 `tests/` 外，任何 `scripts/ tasks/ runtime/` 下的 .py 都不得：
  ...
  - 复刻 kit 的固定段落骨架指纹句；
  - 调用 `write_agent_draft()`（除 `content/post/article/draft_io.py` 自身定义外）；正文写回只能由创作 agent/外部 runner 执行，
    ...
    for base in scan_roots:
        ...
        for path in base.rglob("*.py"):
            ...
            if _KIT_IMPORT_RE.search(text):
                ...
                and rel.as_posix() != "quwoquan_data/scripts/content/post/article/draft_io.py"
