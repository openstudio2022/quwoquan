...
_PRODUCT_ASSET_EXTS = frozenset({".jpg", ".jpeg", ".png", ".webp", ".gif"})
...
def _source_refs_issues(obj: Path, execution: Path) -> list[str]:
    """post `1.download/source_refs.json` 必须满足单底稿零参考宪法：
    ...
    if carrier == "video":
        if not isinstance(sources, list) or not sources:
            return [f"{rel}: video source_refs.json.sources 必须为非空数组"]
        for index, entry in enumerate(sources):
            if not isinstance(entry, dict):
                issues.append(f"{rel}: video sources[{index}] 必须为 object")
                continue
            if str(entry.get("role") or "") != "frame":
                issues.append(f"{rel}: video sources[{index}].role 必须为 frame")
            for field in ("sourceRef", "sourceAssetRef", "rightsRef", "sourceUrl", "sha256"):
                if not str(entry.get(field) or "").strip():
                    issues.append(f"{rel}: video sources[{index}] 缺 {field}")
            for field in ("sourceRef", "sourceAssetRef"):
                ref_value = str(entry.get(field) or "")
                if ref_value and not (execution / ref_value).is_file():
                    issues.append(f"{rel}: video {field} 源证据缺失：{ref_value}")
        return issues
...
def _finalization_report_issues(obj: Path, execution: Path) -> list[str]:
    rel = obj.relative_to(execution)
    is_video = (obj / "assets" / "video.mp4").is_file()
    has_final = (obj / "article.md").is_file() or (obj / "gallery.md").is_file() or is_video
    if not has_final:
        return []
    ...
    if is_video:
        required_refs = {
            "scriptRef": obj / "4.draft" / "video_script.json",
            "videoRef": obj / "assets" / "video.mp4",
            "posterRef": obj / "assets" / "poster.webp",
            "subtitlesRef": obj / "subtitles.vtt",
        }
        for field, expected_path in required_refs.items():
            value = str(data.get(field) or "")
            if not value:
                issues.append(f"{rel}: video finalization_report.{field} 缺失")
            elif not expected_path.is_file():
                issues.append(f"{rel}: video finalization_report.{field} 目标文件缺失")
        return issues
...
def _execution_assets_naming_issues(execution: Path) -> list[str]:
    """命名门：对象 assets/ 成品图片必须使用唯一可读 assetId 格式。"""
    issues: list[str] = []
