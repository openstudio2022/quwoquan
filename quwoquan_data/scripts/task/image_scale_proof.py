"""Open-license image scale proof for task preflight."""
...
from download.research_plan import _license_allows_app_publish  # noqa: PLC2701
from download.source_inputs import curated_images_for_entity
from task import store
from task.run import _coverage_entity_ids, _coverage_entity_type  # noqa: PLC2701
...
def build_open_license_scale_proof(task_id: str, batch_id: str) -> dict[str, Any]:
    spec = store.load_spec(task_id)
    entity_ids = _coverage_entity_ids(spec)
    entity_type = _coverage_entity_type(spec)
...
def write_open_license_scale_proof(report: Mapping[str, Any]) -> Path:
    path = (
        batch_root(str(report["taskId"]), str(report["batchId"]))
        / "_shared"
...
def apply_open_license_scale_proof_to_task(task_id: str, proof: Mapping[str, Any]) -> Path:
    spec = store.load_raw_spec(task_id)
    content = spec.setdefault("content", {})
    if not isinstance(content, dict):
        raise ValueError("task content must be a mapping")
    research = content.setdefault("research", {})
    if not isinstance(research, dict):
        raise ValueError("task content.research must be a mapping")
...
def handle_open_license_proof(args: argparse.Namespace) -> None:
    report = build_open_license_scale_proof(str(args.task), str(args.batch))
    path = write_open_license_scale_proof(report) if bool(args.write) else None
    if bool(args.apply_task):
        if not bool(report.get("passed")):
            raise SystemExit(
                "[task open-license-proof] proof failed; refusing to write task openLicenseScaleProof"
            )
        apply_open_license_scale_proof_to_task(str(args.task), report["proof"])
    print(
        "[task open-license-proof] "
        f"passed={bool(report.get('passed'))} "
        f"entities={report.get('proof', {}).get('preScreenedEntityCount')}/{report.get('requiredEntityCount')} "
        f"assets={report.get('proof', {}).get('publishableImageAssets')}/"
        f"min:{report.get('minimumPublishableImageAssets')} "
        f"desired:{report.get('desiredPublishableImageAssets')} "
        f"score={report.get('averageCompositeScore')}"
    )
    if path:
        print(f"  report: {path}")
    if not bool(report.get("passed")):
        failed = ", ".join(str(item) for item in (report.get("failedEntitySample") or [])[:20])
        if failed:
