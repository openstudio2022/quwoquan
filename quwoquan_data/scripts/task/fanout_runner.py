"""fan-out 多 worker runner（cursor-sdk）：把 object_queue 叶子 job 拉起为 cloud agent。

控制面执行引擎，经 `qwq-data task scaled-e2e author-runner` 暴露；与 `qwq-data task run
--mode fanout` 互补：dispatch 负责建 task/batch + enqueue 叶子，runner 负责把 CHECKPOINT
接缝替换为「lease packet → cloud agent 创作 → ref_review_gate → complete/fail」。
保留直跑入口（cli_first_allowlist 登记）仅供 scaled_e2e 用独立 SDK venv 解释器拉起。
...
# 直跑（独立 venv 解释器）时自举 sys.path；经 CLI import 时已就绪，setdefault 幂等。
_DATA_SCRIPTS = Path(__file__).resolve().parents[1]
_DATA_ROOT = _DATA_SCRIPTS.parent
...
from _common.cursor_credentials import cursor_api_key_file, is_cursor_auth_error, resolve_cursor_api_key  # noqa: E402
from _common.cursor_usage import extract_cursor_usage  # noqa: E402
from _common.execution_branch import resolve_execution_branch  # noqa: E402
...
from task.run_context import DEFAULT_CURSOR_AGENT_MODEL  # noqa: E402

# startup 失败/运行失败的退避基数（秒）。
STARTUP_BACKOFF_BASE = 5
MAX_STARTUP_RETRIES = 3
_RUN_MATRIX_LOCK = threading.Lock()
_LOCAL_BRIDGE_CACHE_LOCK = threading.Lock()
_LOCAL_BRIDGE_CLIENTS: dict[str, Any] = {}
...
def _is_retryable_startup_error(error: str | None) -> bool:
    """本地 bridge 启动类瞬时错误，允许在 startup budget 内同轮重试。"""
    lowered = str(error or "").casefold()
    return any(marker in lowered for marker in _RETRYABLE_STARTUP_MARKERS)


def _reuse_local_bridge_enabled() -> bool:
    return str(os.environ.get("QWQ_FANOUT_REUSE_LOCAL_BRIDGE", "1")).strip().lower() not in {
        "0",
        "false",
        "no",
    }
...
def _local_bridge_cache_key(cwd: str | None) -> str:
    return str(Path(cwd or os.getcwd()).resolve())


def _close_cached_local_bridges() -> None:
    with _LOCAL_BRIDGE_CACHE_LOCK:
        clients = list(_LOCAL_BRIDGE_CLIENTS.values())
        _LOCAL_BRIDGE_CLIENTS.clear()
...
atexit.register(_close_cached_local_bridges)


def _invalidate_local_bridge(cwd: str | None) -> None:
    key = _local_bridge_cache_key(cwd)
    with _LOCAL_BRIDGE_CACHE_LOCK:
        client = _LOCAL_BRIDGE_CLIENTS.pop(key, None)
...
def _cached_local_bridge_client(client_cls: Any, cwd: str | None):
    key = _local_bridge_cache_key(cwd)
    with _LOCAL_BRIDGE_CACHE_LOCK:
        client = _LOCAL_BRIDGE_CLIENTS.get(key)
        if client is not None:
            try:
                client.ping()
                return client
            except Exception:  # noqa: BLE001
                _LOCAL_BRIDGE_CLIENTS.pop(key, None)
                try:
                    client.close()
                except Exception:  # noqa: BLE001
                    pass
        client = client_cls.launch_bridge(
            workspace=cwd or os.getcwd(),
            timeout=10,
            allow_api_key_env_fallback=True,
        )
        _LOCAL_BRIDGE_CLIENTS[key] = client
        return client
...
def _default_cloud_repos(cwd: str | None) -> list[dict[str, Any]]:
    ...
    branch = resolve_execution_branch(cwd=repo_cwd) or _git_output(["branch", "--show-current"], cwd=repo_cwd)
    if not remote:
...
def _artifact_terminal_outcome(packet: Mapping[str, Any]) -> RunOutcome | None:
    """Return success when the agent wrote an approved ref gate but did not terminalize queue.

    Local Cursor runs can occasionally keep the SDK call open after all required
    artifacts have landed. The approved ref gate is the single-ref quality exit,
    so the runner may complete the queue job from those artifacts instead of
    waiting until maxWallClock.
    """
    task_id = str(packet.get("taskId") or "").strip()
    batch_id = str(packet.get("batchId") or "").strip()
    ref = str(packet.get("ref") or "").strip()
    if not task_id or not batch_id or not ref:
        return None
    try:
        review_gate_path = content_object.content_object_stage_dir(task_id, batch_id, ref, "5.review") / "ref_review_gate.json"
        gate = read_json(review_gate_path) if review_gate_path.is_file() else {}
    except Exception:  # noqa: BLE001
        return None
    if not (bool(gate.get("passed")) and str(gate.get("reviewDecision") or "") == "approved"):
        return None
...
def default_agent_runner(
    ...
    *,
    ...
    def _invoke_prompt() -> None:
        client = None
        close_client = True
        using_cached_local_bridge = False
        try:
            _patch_cursor_sdk_tool_callback_token()
            if runtime == RUNTIME_LOCAL and _reuse_local_bridge_enabled():
                client = _cached_local_bridge_client(Client, cwd)
                close_client = False
                using_cached_local_bridge = True
            else:
            ...
            if using_cached_local_bridge:
                _invalidate_local_bridge(cwd)
            holder["startup_error"] = err
        except Exception as exc:  # noqa: BLE001
            if using_cached_local_bridge and _is_retryable_startup_error(str(exc)):
                _invalidate_local_bridge(cwd)
            holder["error"] = exc
        finally:
    ...
    while not done.wait(timeout=0.5):
        ...
        if terminal is not None:
            return terminal
        terminal = _artifact_terminal_outcome(packet)
        if terminal is not None:
            return terminal
    ...
    return RunOutcome(
        ...
        tokens=int(usage.get("usedTokens") or 0),
    ...
    *,
    ...
    return {
        ...
        "executionContract": {
            ...
            "checkpointSemantics": {
                "download_plan": "按 hint 为每个 coverage 实体真实检索 ≥2 个可消费来源（URL+正文 body；图片 lane 需填真实 CC imageUrls），按 lane 写对应的 homepage/article/image_source_plan.json，禁止编造来源/纯色块图",
...
def _process_job(
    ...
    *,
    ...
    if not outcome.started:
        ...
        retryable_startup = bool(outcome.retryable) or _is_retryable_startup_error(outcome.error)
...
def run_fanout(
    ...
    *,
    ...
    matrix["executionBranch"] = resolve_execution_branch(cwd=_REPO_ROOT)
    _save_run_matrix(plan_id, matrix)
    ...
    final_matrix["summary"] = {
        ...
        "executionBranch": resolve_execution_branch(cwd=_REPO_ROOT),
        "credentialIngress": {
            "source": (
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--model", default=DEFAULT_CURSOR_AGENT_MODEL)
    parser.add_argument(
