...
from task.object_queue_jobs import enqueue_ref_job


def _clock_now() -> float:
    return object_queue_bridge.call("_now", _now)
...
def acquire_lease(
    ...
    *,
    ...
    """
    with _queue_lock(task_id, batch_id):
        now = _clock_now()
...
def _author_completion_issues(job: Mapping[str, Any]) -> list[str]:
    ...
    root = batch_root(str(job.get("taskId") or ""), str(job.get("batchId") or ""))
...
def _apply_failure(
    ...
    *,
    ...
    if stuck or exhausted:
        ...
        job["notBeforeEpoch"] = now + object_queue_bridge.call("_backoff_seconds", _backoff_seconds, startup_failure_count if startup_failure else attempt)
    job["updatedAt"] = store.now_iso()
    return job
