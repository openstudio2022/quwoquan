...
- **真相源外置**：所有 prompt 正文落 `quwoquan_data/prompts/{system,task,partials}/*.md`，
...
- **占位符 + 校验**：模板用 `{{var}}` 占位、`{{> partials/x.md}}` 复用片段；渲染器按
  `vars/{name}.vars.yaml` 的 required/optional 声明校验后填充；渲染后不得残留未闭合 `{{`/`}}`。
...
def system_template_path(name: str) -> str:
    return f"{SYSTEM_DIR}/{name}.system.md"


def task_template_path(name: str) -> str:
    return f"{TASK_DIR}/{name}.task.md"


def vars_schema_path(name: str) -> Path:
    return PROMPTS_ROOT / VARS_DIR / f"{name}.vars.yaml"


def _expand_partials(text: str, *, _seen: tuple[str, ...] = ()) -> str:
    """递归展开 `{{> partials/x.md}}` 包含（防自包含死循环）。"""
...
def load_vars_schema(name: str) -> dict[str, dict[str, list[str]]]:
    """读取 `vars/{name}.vars.yaml`：声明 system/task 区的 required/optional 变量。"""
    ...
    for section in (SYSTEM_DIR, TASK_DIR):
...
def render(
    ...
    *,
    ...
    system_block = _render_section(
        system_text, declared=schema[SYSTEM_DIR], values=system_vars or {}, section=SYSTEM_DIR, name=name
    )
    task_block = _render_section(
        task_text, declared=schema[TASK_DIR], values=task_vars or {}, section=TASK_DIR, name=name
    )
