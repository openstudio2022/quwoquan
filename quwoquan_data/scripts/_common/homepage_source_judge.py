...
def collect_title_evidence(
    ...
    *,
    ...
    if unit_dir is not None:
        try:
            from _common.source_layout import read_source_layout
...
ENTITY_PAGE_FAILURE_KINDS = frozenset(
    {
        "source_entity_mismatch",    # 底稿讲的不是目标实体（父级行政区/其它实体/门户）
        "source_insufficient",       # 底稿事实量不足以支撑主页
        "source_page_type_invalid",  # 底稿是列表/公告/聚合页等非介绍页
        "other",
    }
)
...
def entity_page_failure_issues(
    ...
    *,
    ...
    kind = str(failure.get("failureKind") or "")
    if kind not in ENTITY_PAGE_FAILURE_KINDS:
        issues.append(f"failureKind invalid: {kind!r}")
...
def render_judge_prompt(request: Mapping[str, Any]) -> str:
    ...
    from _common.prompt_render import fmt_bullets, render
