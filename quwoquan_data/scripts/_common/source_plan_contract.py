...
SOURCE_PLAN_RULE_SIGNATURE_VERSION = "quwoquan.source_plan_rules.v2"

_DATA_ROOT = Path(__file__).resolve().parents[2]
_ENTITY_SCOPED_REGISTRY_KEYS = {
    "knownOfficialSites",
    "knownHomepageSupportSites",
    "knownEntityAliases",
    "knownArticleSources",
}
_GLOBAL_RULE_RELATIVE_PATHS = (
    "scripts/download/research_plan.py",
    "scripts/_common/source_catalog.py",
    "templates/_registry/catalogs/source_catalog.yaml",
    "templates/_registry/catalogs/content_source_registry.yaml",
)
...
def source_plan_rule_signature(vertical: str, entity_id: str) -> dict[str, Any]:
    """Return a stable signature for the rules that affect one entity plan.
    ...
    known official/homepage/article rows are scoped by entity so adding one
    missing encyclopedia source does not invalidate the whole batch.
    """
