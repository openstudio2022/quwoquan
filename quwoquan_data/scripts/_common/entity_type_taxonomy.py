...
本模块是「类型口径归一」的共享消费面：task lint、主清单 verify 门、WP2 枚举
SOP 与 WP3 打标全部经此读取类型契约，禁止再出现第二处类型定义
（sop 拍平清单 / lint 内联集合 / verify 内联集合都不允许回归）。
...
CONTRACT_TAGS_ROOT = _REPO_DATA_ROOT / "publish" / "tags"
ENTITY_GROUP = "Entity"
PLACE_DOMAIN = "地点"
...
def known_entity_type_paths(*, tags_root: Path | None = None) -> set[str]:
    ...
    task lint 的 entityType 真相源：契约缺失（Entity 树不存在/为空）时显式抛错，
    禁止静默空集把所有类型误判为未知。
    """
...
def find_entity_type_node_path(domain: str, type_name: str, *, tags_root: Path | None = None) -> str | None:
    ...
    供 sop 拍平类型 ↔ Entity 树层级的一致性校验：`咖啡馆` → `Entity/地点/餐厅/咖啡馆`。
    """
