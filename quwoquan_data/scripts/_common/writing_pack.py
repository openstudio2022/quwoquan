...
def render_prompt_md(pack: Mapping[str, Any]) -> str:
    ...
    if carrier in ("image", "gallery"):
        ...
        lines.append(
            "- **章节均衡硬门**：任何单个章节去空白字数不得超过正文总量的约 60%；"
            "底稿某一主题（如历史沿革）篇幅极长时，按比例压缩为精炼概述（压缩属合法轻编辑，"
            "保真针对不脱离底稿另写/不编造，而非不许删减），不要让一段吞并其余应有章节。"
        )
        lines.append(
            "- **时间线归并硬门**：同章节内出现多条并列时间线（开发史 / 保护史 / 行政沿革等）时，"
            "必须按真实时间顺序归并为单一连贯叙事；禁止把不同时间线首尾拼接造成时间倒错（如 2007 年后又跳回 1979 年）。"
        )
        lines.append(
    ...
    if base_text:
        ...
        if adapt_base:
            ...
            lines.append(
                "- 删除仅限广告、保险、App 下载/积分、平台活动等**非内容噪声**；底稿写到的所有目的地/行程/景点段落"
                "都是正文内容，必须整篇保留（多目的地路书照样保留全部站点），**禁止**以「与本篇实体无关」为由删掉其它"
                "城市/景点段落，也不要改写成全新的攻略结构（单一底稿 1:1 成稿，实体只是标签不是裁剪边界）。"
            )
            lines.append(
