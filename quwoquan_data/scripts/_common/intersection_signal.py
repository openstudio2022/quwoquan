...
云侧 read model `IntersectionReason`（contracts/metadata/recommendation/rec_model/projections/
intersection_reason.yaml）是跨会话真相源：端只读展示、禁止本地拼装文案。

produce 是内容生产期、没有具体用户，因此只预生成「内容侧可交集锚点」intersectionHints——
...
SOURCES = ("tagRef", "geoTagRef", "entityRef", "followEdge", "contact")
...
def contract_field_names() -> frozenset[str]:
    ...
    return frozenset(str(f.get("name")) for f in fields if f.get("name"))
