...
- lane adapter 边界（载体差异只允许出现在此清单声明的产物与阶段差异）：
  ...
  - video：只冻结 schema（post_manifest.schema.json contentType=video +
    videoBindings）+ smoke，不放量、不接 produce_author。
...
class CarrierLane:
    ...
    # 是否进入 produce_author（bridge 受限段）。
...
# 单一真相源：content_supply.VALID_CONTENT_TYPES 与 site_supply packet 标注必须引用本表，
# 禁止再写内联映射 dict / 注释式映射。
CONTENT_MIX_TO_LANE: dict[str, str] = {
    ...
    "imagePost": "image",
    "videoPost": "video",
    # 知识卡是 article 载体的排产变体（SOP 走 article.md，预算独立）。
