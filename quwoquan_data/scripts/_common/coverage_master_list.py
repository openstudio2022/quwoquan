...
MASTER_LIST_SCHEMA_PATH = _REPO_DATA_ROOT / "schema" / "coverage" / "master_list.schema.json"
...
def master_list_stats(
    *,
    ...
    """主清单统计：文件/区县/叶子规模、类型分布、源就绪分布、跨省地点。"""
    stats: dict[str, Any] = {
        "files": 0,
        "districts": 0,
        "leaves": 0,
    ...
    for path in master_list_files(provinces=provinces, coverage_root=coverage_root):
        ...
        for district, leaf in iter_master_leaves(data):
            ...
            stats["bySourceReadiness"][readiness] = stats["bySourceReadiness"].get(readiness, 0) + 1
...
def discovery_partitions_from_master_list(
    ...
    *,
    ...
    leaf = {name: canonicalName, entityType}（ref 由 fanout_plan.leaf_ref 派生）。
