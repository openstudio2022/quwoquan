"""Canonical publish 与 execution 输出质量扫描。"""
...
from content.homepage.assets.repair import scan_homepages
...
def _runtime_root() -> Path:
    # 路径唯一真相源 core.paths：runtime 批次树在 QWQ_OUTPUT_ROOT（仓外），
...
def _post_roots() -> list[Path]:
    roots: list[Path] = []
    for execution_dir in _runtime_root().iterdir() if _runtime_root().is_dir() else []:
        if execution_dir.is_dir():
            posts = execution_dir / "posts"
            if posts.is_dir():
                roots.append(posts)
