...
def _pending_entity_mentions(refs: list[str], article_md: str, manifest: dict[str, Any]) -> list[dict[str, Any]]:
    """Keep unresolved entity mentions auditable without projecting active entity refs."""
...
class PublishFilterVerdict:
    ...
    def write_into(self, dst: Path) -> None:
        ...
        if _is_asset_only_manifest(self.manifest):
            (dst / "article.md").unlink(missing_ok=True)
        elif self.article_md:
...
def _is_asset_only_manifest(manifest: dict[str, Any]) -> bool:
    return str(manifest.get("contentType") or "") in {"image", "video"} or str(
        manifest.get("carrier") or ""
    ) in {"image", "gallery", "video"}
