...
碎片即时『随记』(moment) 与证据不足 (abandoned) 不进入 content_plan / post。
...
from content.post.article.evidence_text import anonymize_source_markdown, score_source_markdown
from core.content_source_registry import load_content_source_registry, resolve_source_tier
from core.paths import CONTROL_PLANE_CATALOGS_ROOT

WORKS_CLASSIFICATION_SCHEMA = "quwoquan_data.works_classification"
WORKS_CLASSIFICATION_CONFIG_PATH = CONTROL_PLANE_CATALOGS_ROOT / "works_classification.yaml"
_CONFIG_SCHEMA = "quwoquan.works_classification"
...
def classify_works(
    ...
    *,
    ...
    declared = str(declared_carrier or "").strip().lower()
    explicit_image_carrier = declared == "image"
