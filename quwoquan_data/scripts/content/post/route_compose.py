...
def build_route_writing_pack(
    ...
    publish_layout = "image" if carrier == "image" else "travel"
...
def _attach_base_draft_text(execution_id: str, pack: dict[str, Any]) -> None:
    ...
    if not base_ref:
        return
    pack["sourceUseMode"] = base_source_use_mode(execution_id, base_ref)
    text = load_base_draft_text(execution_id, base_ref).strip()
    if text:
