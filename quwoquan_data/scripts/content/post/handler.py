...
from content.post.entity_workflow import (
    build_entity_writing_pack,
    iter_entity_briefs,
    review_entity_draft,
)
from content.post.route_analysis import analyze_route_ref
from content.post.route_compose import build_route_writing_pack
from content.post.route_core import iter_route_briefs
from content.post.route_review import review_route_draft
...
def _stage_compose_brief(execution_id: str, refs, *, writer_group_size: int = 1) -> int:
    ...
    if writer_group_size > 1 and prepared_refs:
        ...
        print("[post] Next: 图片作品已由结构化 sourceCollection/assets/caption 准备；不生成 4.draft/draft.article.md。")
    print("[post] Next: task geo-homepages continues the execution through review and canonical promotion.")
...
def handle_post(request: PostStageRequest) -> None:
    ...
    print(f"[post] Execution: {execution_id}, Type: {content_type}, Stage: {stage}")
    print(f"[post] Work dir: {post_root}")

    from content.post.object_index import has_briefs

    if content_type != "article" or not has_briefs(execution_id):
        if request.materialize:
            paths = materialize_posts(execution_id, content_type)
            print(f"[post] Materialized {len(paths)} approved post package(s).")
        return
