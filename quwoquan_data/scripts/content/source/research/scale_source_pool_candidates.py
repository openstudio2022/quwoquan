"""Deterministic four-carrier candidate file assembled from audited projections."""
...
def build_scale_source_pool_candidates(
    *,
    ...
    homepage_article_projection: Mapping[str, Any],
    image_video_projection: Mapping[str, Any],
) -> dict[str, Any]:
    """Merge exact two-carrier projections into one CLI-consumable file."""
    ...
    if (
        _identity(homepage_article_projection) != expected_identity
        or _identity(image_video_projection) != expected_identity
    ):
    ...
    homepage_article = homepage_article_projection.get("rows")
    image_video = image_video_projection.get("candidates")
    if not isinstance(homepage_article, list) or not isinstance(image_video, list):
        _fail("projection candidate collections are invalid")
    candidates = [
        dict(item)
        for item in (*homepage_article, *image_video)
        if isinstance(item, Mapping)
    ]
    if len(candidates) != len(homepage_article) + len(image_video):
    ...
    if set(homepage_article_counts) != {"homepage", "article"}:
        _fail("homepage/article projection carrier boundary drift")
    if set(image_video_counts) != {"image", "video"}:
    ...
    declared_homepage_article = {
        ...
        for item in homepage_article_projection.get("rowCounts") or []
        if isinstance(item, Mapping)
    }
    if declared_homepage_article != dict(homepage_article_counts):
        _fail("homepage/article projection rowCounts drift")
    if int(image_video_projection.get("candidateCount") or -1) != len(image_video):
        _fail("image/video projection candidateCount drift")
    counts = Counter(str(item["carrier"]) for item in candidates)
    if any(counts[carrier] < 1 for carrier in _CARRIERS):
        _fail("four-carrier projections must each contain at least one candidate")
    projection_bindings = [
        {
            "kind": "homepage_article",
            "projectionDigest": str(homepage_article_projection["projectionDigest"]),
