...
_WIKIDATA_HTTP_TIMEOUT_SECONDS = active_runtime_policy().provider_timeouts.mediawiki_seconds
...
def _wiki_title(host: str, entity_id: str) -> str:
    ...
    exact = network_io.wiki_api(host, {"action": "query", "titles": entity_id, "redirects": "1", "format": "json"})
...
def _wikidata_item_for_zhwiki(title: str) -> str:
    data = network_io.wiki_api("zh.wikipedia.org", {
        "action": "query",
        "titles": title,
        "prop": "pageprops",
        "format": "json",
...
def _wikidata_item_for_entity_search(entity_id: str) -> str:
    ...
    weak city/province substitutes stay out of the content.execution.planning.
    """
    if not entity_id:
        return ""
    for term in _dedupe_terms(_entity_name_variants(entity_id), limit=5):
        data = network_io.curl_json(
            ...
            timeout=_WIKIDATA_HTTP_TIMEOUT_SECONDS,
        )
