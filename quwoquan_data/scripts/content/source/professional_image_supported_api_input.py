    ...
    *,
    ...
    for path in sorted(paths):
        ...
        if content_sha and phash and candidate_id and provider_asset_id:
            identities.append((content_sha, phash, candidate_id, provider_asset_id))
    return identities
    ...
    for root in roots:
        ...
        for evidence_path in sorted(
            ...
            if (
                ...
                or resolved_current in evidence_path.resolve().parents
            ):
                continue
            try:
                ...
                if request["contentSha256"] != evidence["contentSha256"] or request[
                    "originalAssetSha256"
                ] != file_sha256(asset_path):
                    continue
                provider_asset_id = str(evidence["providerAssetId"])
                previous = records.get(provider_asset_id)
                if previous is not None and file_sha256(previous[0]) != file_sha256(
                    asset_path
                ):
                    continue
...
def _assert_rebindable_provenance(
    *,
    ...
    drifted = [
        field
        for field, (current, prior) in checks.items()
...
def _manifest_item(
    ...
    return {
        ...
        "assetUrl": str(meta["originalAssetUrl"]),
        "manualFile": "",
        ...
        "authorizationProof": str(meta["sourcePageUrl"]),
        ...
        "safetyReview": {
            ...
            "reviewer": "semantic:"
            + str(judgment.get("reviewerRunId") or "bound-result"),
            "evidenceRef": safety_ref,
            "safetyEvidenceFileSha256": safety_sha,
        },
...
def prepare_supported_api_inputs(
    *,
    ...
    from content.source.professional_image_supported_api_preparation import (
        prepare_supported_api_inputs as _prepare,
    )
