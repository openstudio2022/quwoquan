"""Producer-owned canonical and immutable release consistency scanner."""
...
from content.release.canonical.release_media_consistency import (
    ...
    *,
    ...
    for kind, refs in (("posts", posts), ("entities", entities)):
        for ref in refs:
            ...
            try:
                read_object_sources(object_root, manifest)
            except (ObjectTransactionError, OSError, ValueError) as exc:
                issues.append(_issue("object_evidence_missing", str(exc), ref))
    ...
    if release_root is not None:
        ...
        if release_header_path.is_file():
            ...
            # Explicit cohorts may carry the complete admitted author closure.
            # poolDigest is the producer-owned proof that this is a pool release.
            if release_header.get("poolDigest") and pool_creator_refs == creators:
