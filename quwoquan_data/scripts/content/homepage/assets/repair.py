...
from core.image_safety import STATUS_UNSAFE, assess_image, is_low_texture_placeholder_graphic
...
def _runtime_root() -> Path:
    return paths.current_runtime_root()
...
def repair_homepage(issue: HomepageIssue) -> dict[str, Any]:
    ...
    execution_id = str(entity.get("executionId") or "").strip()
    if not execution_id and issue.root_kind == "runtime":
        execution_id = _runtime_task_id(entity_dir)
    if not execution_id:
        raise ValueError(f"canonical homepage missing executionId: {entity_ref}")
    entity["executionId"] = execution_id
