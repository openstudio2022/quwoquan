...
def _run_download_fetch(ctx: ExecutionContext) -> StageResult:
    ...
    from content.execution.recovery.download_repair import _apply_download_fast_fail, _record_download_repair
    ...
    if target_entity_ids != ctx.entity_ids:
        print(
            "[geo-homepages] download object repair/refresh: "
    ...
    if download_lane != "all":
        print(f"[geo-homepages] download lane-scoped repair: lane={download_lane}")
        ...
        if code not in (0,):
            ...
            if not issues:
                ...
                return StageResult(
                    ...
                    AUTO,
                    StageStatus.DONE,
                    "fetched sources after deterministic recovery",
                )
    ...
    if issues:
        ...
        if not issues:
            ...
            return StageResult(
                ...
                AUTO,
                StageStatus.DONE,
                    "fetched sources after deterministic source repair",
            )
        rendered_issues = issue_messages(issues)
        _record_download_repair(ctx, issues)
        return StageResult(
...
def _run_build_prepare(ctx: ExecutionContext) -> StageResult:
    from content.homepage.homepage import homepage_runtime_spec, prepare_entity_pages, validate_entity_page_inputs
...
def _run_homepage_independent_reviews(
    ...
    from content.execution.pipeline.homepage_authoring import _homepage_independent_review_issues
    ...
    review_model = str(os.environ.get("QWQ_HOMEPAGE_REVIEW_MODEL") or "").strip()
    review_model_family = str(
        os.environ.get("QWQ_HOMEPAGE_REVIEW_MODEL_FAMILY") or ""
    ).strip()
    author_model = str(ctx.model or "").strip()
    author_model_family = str(
        os.environ.get("QWQ_HOMEPAGE_AUTHOR_MODEL_FAMILY") or ""
    ).strip()
    if not review_model or not review_model_family or not author_model_family:
        return ["independent reviewer model contract is incomplete"]
    ...
    for target in ((runtime_spec.get("scope") or {}).get("coverageTargets") or []):
        ...
        if str(outcome.get("status") or "") != "finished" or payload is None:
            ...
            write_json(
                failure_path,
                {
                    "schemaVersion": "quwoquan_data.homepage_review_failure/1",
