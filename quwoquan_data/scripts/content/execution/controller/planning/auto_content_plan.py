"""Automatic content-plan construction for execution controllers."""

from __future__ import annotations

from content.execution.controller.content_plan import (
    Any,
...
def auto_content_plan(
    ...
    execution_policy = (
        active_spec.get("executionPolicy")
        if isinstance(active_spec.get("executionPolicy"), Mapping)
        else {}
    )
    ...
    for target in targets:
        ...
        object_dir = resolve_entity_object_dir(
            ctx.execution_id, target, etype_hint=target_etype
        )
        ...
        for source_dir in source_units:
            ...
            if lane == "article":
                if str(meta.get("sourceRole") or "") != "base":
                    continue
                if (
                    "support" in source_id.lower()
                    or "support" in source_dir.name.lower()
                ):
                    continue
                article_raw_count += 1
                if bool(meta.get("hasVideo")):
                    ...
                    rejects.reject_article(
                        "contains_video", source_id, "来源含内联视频，文章类放弃"
                    )
                    continue
                ...
                qunar_focus_verdict = (
                    str(meta.get("entityFocusVerdict") or "").strip() or focus_verdict
                )
                ...
                if qunar_block:
                    rejects.reject_article(
                        qunar_block,
                        source_id,
                        "Qunar 模板底稿不可作为当前实体 article base",
                    )
                    continue
                ...
                if not draft_title:
                    ...
                    rejects.reject_article(
                        "no_source_title", source_id, "底稿无法提取发布标题"
                    )
                    continue
                entity_tags = sorted(
                    {
                        *_coverage_targets_mentioned(
                            base_body, str(meta.get("title") or ""), targets
                        ),
                        target,
                    }
                )
                ...
                article_candidates.append(
                    {
                        ...
                        "sourceUseMode": str(
                            meta.get("sourceUseMode") or "factual_reference_only"
                        ),
                        "sourceClass": str(
                            meta.get("sourceClass") or meta.get("category") or ""
                        ),
                ...
                for row in rows:
                    ...
                    if not collection_id:
                        rejects.reject_image(
                            "missing_source_collection_id", source_id, asset_ref
                        )
                        continue
        ...
        image_candidates.sort(
            key=lambda row: (str(row["collectionId"]), str(row["assetRef"]))
        )
        ...
        source_diagnostics[target] = build_source_diagnostic(
            SourceDiagnosticInput(
                ...
                article_image_warning_examples=dict(
                    rejects.article_image_warning_examples
                ),
