"""External-input binding for pre-acquisition handoffs."""
...
def validate_carrier_external_input_requirements(
    ...
    *,
    ...
    if not observed or any(kind not in required for kind in observed):
        raise _typed(
            ...
            f"{carrier} requires pre-acquisition external inputs of kind "
            f"{', '.join(required)}",
        )
