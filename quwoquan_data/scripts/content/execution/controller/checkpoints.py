...
def _download_plan_scale_readiness(ctx: ExecutionContext) -> StageResult | None:
    ...
    if image_asset_strategy(spec) != OPEN_LICENSE_PUBLISH:
        return None
    report = build_open_license_scale_proof(ctx.execution_id)
    proof_path = write_open_license_scale_proof(report)
    issues = image_asset_strategy_scale_issues(spec, report.get("proof"))
    if not issues:
        return None
    issue = data_issue(
        ...
        message=issues[0],
        attributes={
            "requiredEntityCount": report.get("requiredEntityCount", 0),
            "failedEntityCount": report.get("failedEntityCount", 0),
...
def _checkpoint_download_plan(ctx: ExecutionContext) -> StageResult:
    ...
    try:
        ...
        issue = data_issue(
            ...
            stage=DataIssueStage.DOWNLOAD_PLAN,
            message="auto research boundary failed",
            ref=ctx.execution_id,
...
def _checkpoint_content_plan(ctx: ExecutionContext) -> StageResult:
    ...
    from content.execution.controller.homepage_authoring import _content_plan_done
