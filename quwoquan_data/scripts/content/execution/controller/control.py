...
from content.execution.support import Any, Callable, DataIssueCode, DataRecoveryAction, ExecutionContext, ExecutionStateTransition, Iterable, MANAGED_SCHEDULER_STALE_SECONDS, MAX_REACT_REWINDS, Mapping, Path, StageResult, contextmanager, data_issues, execution_root, execution_state_status, load_execution_state, os, save_execution_state, signal, store, subprocess, sys
...
def _stop_at_until(
    ...
    *,
    ...
    state.status = ExecutionStateStatus.STOPPED_AT_UNTIL
    state.stopped_at_stage = ctx.until
    state.next_action = next_stage or "execution complete"
    state.heartbeat_at = store.now_iso()
    save_execution_state(state)
    print(f"[task execute] stopped at --until {ctx.until}")
    return 0

def _react_rewind(ctx: ExecutionContext, state: ExecutionStateTransition, completed: set[str],
    ...
    latest_state = load_execution_state(ctx.execution_id)
    state.replace_with(latest_state)
    rewinds = state.react_rewinds
    key = result.stage
    used = int(rewinds.get(key, 0))
    if used >= MAX_REACT_REWINDS:
        print(f"[task execute] ReAct 回退已达上限({MAX_REACT_REWINDS}) @ {result.stage}; 转人工", file=sys.stderr)
        return completed, False
    rewinds[key] = used + 1
    ...
    if result.stage is ExecutionStage.POST_REVIEW:
        ...
        if target is ExecutionStage.POST_COMPOSE and not prepared:
            print(
                "[task execute] post_review failed only at batch/release packaging; "
                "no content object will be invalidated",
                file=sys.stderr,
            )
            return completed, False
        latest = load_execution_state(ctx.execution_id)
    ...
    save_execution_state(state)
    print(f"[task execute] ⟲ ReAct 回退 {result.stage} → {target} (第{used + 1}/{MAX_REACT_REWINDS}次)\n"
    ...
    *,
    ...
    for line in proc.stdout.splitlines():
        ...
        if (
            ...
            and ("task execute" in command)
            and (ctx.execution_id in command or ctx.execution_id in command)
        ):
            return True
    return False

def _recover_stale_agent_scheduler(ctx: ExecutionContext, state: ExecutionStateTransition) -> bool:
    """Clear orphaned waiting_agent state left by interrupted managed-local runs."""
    from content.execution.controller.metrics import _parse_iso_seconds
    scheduler = state.active_agent_scheduler
    if not isinstance(scheduler, Mapping):
        return False
    if str(scheduler.get("runtime") or ctx.runtime) != "local":
        return False
    if execution_state_status(state) is not ExecutionStateStatus.WAITING_AGENT:
        return False
    heartbeat = _parse_iso_seconds(
        state.heartbeat_at or scheduler.get("updatedAt") or scheduler.get("startedAt")
    )
    ...
    stage = str(scheduler.get("stage") or state.waiting_checkpoint or "")
    recovered = {
        "stage": stage,
    ...
    history = state.scheduler_recovery_actions
    history.append(recovered)
    state.scheduler_recovery_actions = history[-20:]
    state.active_agent_scheduler = None
    state.waiting_checkpoint = stage or state.waiting_checkpoint
...
def _recover_stale_auto_research(ctx: ExecutionContext, state: ExecutionStateTransition) -> bool:
    """Mark orphaned deterministic source discovery as an explicit checkpoint failure.
    Unlike managed Agent jobs, auto research runs inside the execution process.
    ...
    """
    from content.execution.controller.metrics import _parse_iso_seconds
    active = state.active_auto_research
    ...
    if status == "succeeded":
        ...
        return True
    heartbeat = _parse_iso_seconds(
        state.heartbeat_at or active.get("updatedAt") or active.get("startedAt")
    )
    now = _parse_iso_seconds(store.now_iso())
    ...
    recovered = {
        ...
        "reason": (
            ...
            if interrupted
                ...
                "orphaned running auto research without live execution process"
                if fresh_orphan
                else "stale auto research heartbeat without live execution process"
            )
        ),
        "previous": dict(active),
        "recoveredAt": recovered_at,
    }
    history = state.auto_research_recovery_actions
    history.append(recovered)
    state.auto_research_recovery_actions = history[-20:]
    state.active_auto_research = None
    state.waiting_checkpoint = "download_plan"
    state.last_failed_stage = "download_plan"
    ...
    state.next_action = "download_plan auto_research interrupted/stale; rerun execution to revalidate source readiness"
    state.heartbeat_at = recovered_at
    save_execution_state(state)
    return True

def _recover_stale_controller_yield(ctx: ExecutionContext, state: ExecutionStateTransition) -> bool:
    """Clear stale controllerYield left by a dead managed-local controller."""
    controller_yield = state.controller_yield
    if not isinstance(controller_yield, Mapping):
        return False
    stage = str(controller_yield.get("stage") or state.waiting_checkpoint or "")
    ...
    try:
        execution_live = _managed_agent_process_alive(ctx, include_workspace_bridge=False)
    except TypeError as exc:
        if "include_workspace_bridge" not in str(exc):
            raise
        execution_live = _managed_agent_process_alive(ctx)
    if lease_live or execution_live:
        return False
    recovered_at = store.now_iso()
    recovered = {
        "stage": stage,
        "reason": "stale controllerYield without live controller lease or execution process",
        "previous": dict(controller_yield),
        "previousLease": dict(lease or {}) if isinstance(lease, Mapping) else None,
        "recoveredAt": recovered_at,
    }
    history = state.controller_yield_recovery_actions
    history.append(recovered)
    state.controller_yield_recovery_actions = history[-20:]
...
def _mark_execution_interrupted(
    ...
    *,
    ...
    state.status = ExecutionStateStatus.MANUAL_REQUIRED
    state.failed_objects = [reason]
    state.next_action = f"{stage}: interrupted; rerun execution to revalidate checkpoint"
    ...
    if isinstance(active, Mapping):
        ...
        updated["interruptedAt"] = state.heartbeat_at
        updated["interruptReason"] = reason
        state.active_auto_research = updated
    save_execution_state(state)
...
def _managed_checkpoint_interruption_is_resumable(
    ...
    *,
    ...
    if not ctx.managed:
        return False
    if execution_state_status(state) is not ExecutionStateStatus.REPAIRING:
        return False
...
@contextmanager
def _execution_signal_guard(ctx: ExecutionContext):
    """Persist execution interruption before SIGTERM/SIGINT tears down the process."""
    previous: dict[int, Any] = {}
    def _handler(signum: int, _frame: object) -> None:
        state = load_execution_state(ctx.execution_id)
        active_auto = state.active_auto_research if isinstance(state.active_auto_research, Mapping) else {}
        stage = str(
            state.waiting_checkpoint
            or active_auto.get("stage")
            or state.last_failed_stage
            or "execution"
        )
        ...
        _mark_execution_interrupted(
            ...
            reason=f"{stage}: interrupted by signal {signum}; execution controller stopped",
        )
