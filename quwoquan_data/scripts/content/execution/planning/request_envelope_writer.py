...
from content.execution.planning.request_envelope_identity import (
    assert_one_handoff_identity as _assert_one_handoff_identity,
    assert_one_source_identity as _assert_one_source_identity,
)
from content.execution.planning.scale import campaign_workload_targets, resolve_campaign_scale
...
def _reconciliation_inputs(
    ...
    *,
    ...
    from content.execution.planning import submission_reconciliation as reconciliation

    if sequence == 1:
        raise ValueError(
            "campaign envelope sequence=1 forbids predecessor reconciliation"
        )
    receipt = reconciliation.load_submission_reconciliation_receipt(
        receipt_path,
