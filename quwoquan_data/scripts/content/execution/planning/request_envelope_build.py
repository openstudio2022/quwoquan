...
from content.execution.planning import request_envelope as owner
...
from content.execution.planning.retry_submission import (
    ...
    *,
    ...
    if all(value is not None for value in pool_inputs):
        ...
        (
            ...
            carrier=carrier,
            # SourcePool readiness uses the oversampled ``count`` as its
            # availability floor, but an execution freezes only the governed
            # user quota.  Keeping these axes separate makes M1 task-init and
            # proof identity exactly one while preserving a two-candidate pool.
            count=quota_value,
            minimum_candidate_count=count,
            source_revision=source_revision,
            source_digest=str(source["digest"]),
            entity_catalog_digest=catalog_digest,
