...
def bind_scale_source_pool(
    ...
    *,
    ...
    required_count = count if minimum_candidate_count is None else minimum_candidate_count
    if (
        isinstance(required_count, bool)
        or not isinstance(required_count, int)
        or required_count < count
        or len(ordered) < required_count
    ):
        raise ValueError(
            f"{SOURCE_POOL_SHORTFALL}: {carrier} candidateCount={len(ordered)} required={required_count}"
        )
