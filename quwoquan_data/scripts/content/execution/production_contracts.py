...
def build_token_ledger_entry(
    *,
    ...
    token_exceeded = int(budget_tokens) > 0 and int(used_tokens) > int(
        budget_tokens
    )
    cost_exceeded = (
        cost_usd is not None
        and float(cost_budget_usd) > 0
        and float(cost_usd) > float(cost_budget_usd)
    )
    effective_cost_issue = (
        cost_issue
        or ("GATE_BLOCK_COST_UNKNOWN" if cost_usd is None else "")
    )
    effective_object_ref = (
        content_object_ref
        or (job_id if scope == "content_object" else "")
    )
    return {
        ...
        "contentObjectRef": effective_object_ref or None,
        "creatorProfileId": creator_profile_id or None,
        ...
        "costKnown": cost_usd is not None,
        "costSource": cost_source or None,
        "costIssue": effective_cost_issue or None,
        "pricingRevision": pricing_revision or None,
        "retryCostUsd": float(retry_cost_usd),
        "passed": bool(passed),
        "cacheHits": dict(cache_hits or {}),
        "budgetExceeded": token_exceeded or cost_exceeded,
        "unitPassedCostUsd": float(cost_usd) if passed and cost_usd is not None else None,
    }
...
def validate_token_ledger_entry(entry: Mapping[str, Any]) -> list[str]:
    ...
    for key in ("executionId", "jobId", "runId", "contentType", "scope"):
        _expect_string(entry, key, issues, prefix="tokenLedger")
    if entry.get("scope") == "content_object":
        _expect_string(entry, "creatorProfileId", issues, prefix="tokenLedger")
        _expect_string(entry, "contentObjectRef", issues, prefix="tokenLedger")
    ...
    cost_budget = float(entry.get("costBudgetUsd") or 0.0)
    if (
        isinstance(cost, (int, float))
        and cost_budget > 0
        and float(cost) > cost_budget
        and entry.get("budgetExceeded") is not True
    ):
        issues.append(
            "tokenLedger.budgetExceeded must be true when costUsd exceeds costBudgetUsd"
        )
    if cost is None and entry.get("costIssue") != "GATE_BLOCK_COST_UNKNOWN":
        issues.append("tokenLedger unknown cost must use GATE_BLOCK_COST_UNKNOWN")
    return issues
