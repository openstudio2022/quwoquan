        ...
        *,
        ...
        with self._mutex:
            with campaign_snapshot_guard(
                ...
                evidence = process_evidence or {}
                for field in (
                    "sliceCount", "resumeCount", "elapsedSeconds", "maxRssBytes",
                    "lastExecutionHeartbeatAt", "heartbeatAgeSeconds",
                    "terminationOwner", "terminationSignal",
                ):
                    if field in evidence or field in previous:
                        payload[field] = evidence.get(field, previous.get(field))
