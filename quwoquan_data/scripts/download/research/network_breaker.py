...
def _threshold() -> int:
    raw = os.environ.get("QWQ_AUTO_RESEARCH_NETWORK_BREAKER_THRESHOLD", "")
    try:
        value = int(raw) if raw else _DEFAULT_THRESHOLD
    except ValueError:
...
_DEFAULT_STAGE_NO_PROGRESS_TIMEOUT_SECONDS = 900.0
_DEFAULT_WAVE_BUDGET_SECONDS = 1800.0
...
def wave_budget_seconds() -> float:
    ...
    """
    raw = os.environ.get("QWQ_AUTO_RESEARCH_WAVE_BUDGET_SECONDS", "")
    try:
        value = float(raw) if raw else _DEFAULT_WAVE_BUDGET_SECONDS
    except ValueError:
        value = _DEFAULT_WAVE_BUDGET_SECONDS
...
def stage_no_progress_timeout_seconds() -> float:
    ...
    唯一真相源：env `QWQ_STAGE_NO_PROGRESS_TIMEOUT_SECONDS`（默认 900）。
    消费方：auto_plan_public 并行 wave（相邻两次实体完成之间无任何进展超过
    该预算 → 以可续跑的 network/no-progress outage 中断）；task 层 run_context
    暴露同名常量给 run.py 编排消费。
    """
    raw = os.environ.get("QWQ_STAGE_NO_PROGRESS_TIMEOUT_SECONDS", "")
    try:
        value = float(raw) if raw else _DEFAULT_STAGE_NO_PROGRESS_TIMEOUT_SECONDS
    except ValueError:
        value = _DEFAULT_STAGE_NO_PROGRESS_TIMEOUT_SECONDS
    return max(0.0, value)
