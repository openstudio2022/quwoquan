...
输出: .qwq_output/data/local/workspace/taxonomy/relations/

用法:
  python3 quwoquan_data/scripts/cli.py governance taxonomy graph
  python3 quwoquan_data/scripts/cli.py governance taxonomy graph --min-cooccur 2
  python3 quwoquan_data/scripts/cli.py governance taxonomy graph --dry-run
"""
...
from core.paths import DATA_LOCAL_ROOT, PUBLISH_ROOT

CANONICAL_ROOT = PUBLISH_ROOT
REL_ROOT = DATA_LOCAL_ROOT / "workspace" / "taxonomy" / "relations"
...
def collect_tag_sets() -> tuple[list[dict], list[dict]]:
    ...
    for ef in CANONICAL_ROOT.rglob("_entity.json"):
        try:
        ...
        if geo:
            tags = tags + [geo]
        rel = str(ef.parent.relative_to(CANONICAL_ROOT))
        entities.append({"id": rel, "type": "entity", "tags": tags})

    for mf in CANONICAL_ROOT.rglob("manifest.json"):
        ...
        try:
            ...
            continue
        tags = data.get("tagRefs", [])
        rel = str(mf.parent.relative_to(CANONICAL_ROOT))
