"""Typed repository-owned profile for deterministic media probe fixtures."""
...
PROFILE_PATH = REPO_DATA_ROOT / "reference/media_probe/video_playback.yaml"


@dataclass(frozen=True, slots=True)
class MediaProbeAsset:
    asset_id: str
    asset_version: int
    duration_ms: int
...
class MediaProbeProfile:
    ...
    preview_columns: int
    assets: tuple[MediaProbeAsset, ...]


def _positive_int(value: object, *, label: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
        raise ValueError(f"media probe {label} must be a positive integer")
    return value


def load_media_probe_profile(path: Path = PROFILE_PATH) -> MediaProbeProfile:
    raw = yaml.safe_load(path.read_text(encoding="utf-8"))
    if not isinstance(raw, dict):
        raise ValueError("media probe profile must be an object")
    video = raw.get("video")
    audio = raw.get("audio")
    preview = raw.get("preview")
    assets_raw = raw.get("assets")
    if not isinstance(video, dict) or not isinstance(audio, dict) or not isinstance(preview, dict):
        raise ValueError("media probe video/audio/preview sections are required")
    if not isinstance(assets_raw, list) or not assets_raw:
        raise ValueError("media probe assets must be a non-empty list")
    if str(video.get("codec") or "") != "h264" or str(video.get("container") or "") != "mp4":
        raise ValueError("media probe requires H.264 MP4")
    if str(audio.get("codec") or "") != "aac":
        raise ValueError("media probe requires AAC audio")

    assets: list[MediaProbeAsset] = []
    seen: set[str] = set()
    for item in assets_raw:
        if not isinstance(item, dict):
            raise ValueError("media probe asset entry must be an object")
        ...
        if not asset_id or asset_id in seen:
            raise ValueError(f"media probe assetId is missing or duplicated: {asset_id}")
        if not prefix.startswith("media/video/s/") or ".." in prefix:
            raise ValueError(f"media probe public slice prefix is invalid: {prefix}")
        if status not in {"ready", "rejected"}:
            raise ValueError(f"media probe processing status is invalid: {status}")
        seen.add(asset_id)
        assets.append(
            MediaProbeAsset(
                asset_id=asset_id,
    ...
    if not profile.profile_id or not profile.processor_profile or not profile.audio_bitrate:
        raise ValueError("media probe profile identity and audio bitrate are required")
    if profile.keyframe_interval_ms > 2000:
        raise ValueError("media probe keyframe interval must not exceed 2000ms")
    for asset in profile.assets:
        ...
        if asset.expected_processing_status != expected:
            raise ValueError(
                f"media probe {asset.asset_id} status must be {expected} at the product boundary"
            )
        if asset.storyboard and asset.expected_processing_status != "ready":
            raise ValueError("rejected media probe cannot publish a storyboard")
    return profile


__all__ = ["MediaProbeAsset", "MediaProbeProfile", "load_media_probe_profile"]
