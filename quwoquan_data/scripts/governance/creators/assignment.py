...
def _registry_creator(creator_profile_id: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        # The compact-file fallback below is the canonical recovery path for
        # a registry that cannot be assembled as one object.
        profile = None
    try:
        from content.templates.registry import CREATORS_ROOT, iter_yaml_files, load_yaml
    except ImportError:
        return None
    ...
    *,
    ...
    if content_region or content_tag_refs:
        ...
        if kind in ("regional", "thematic", "regional_topic"):
            try:
                from content.templates.creator import coverage_range_fit, _tag_overlap
    ...
    *,
    ...
    if carrier:
        try:
            ...
            affinity = carrier_affinity(registered, carrier)
        except Exception:  # noqa: BLE001
            affinity = 0.0
        if affinity <= 0:
...
def resolve_registry_creator_assignment(
    ...
    *,
    ...
    assignment fields. Shared by managed content execution so
