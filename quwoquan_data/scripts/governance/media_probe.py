"""受控视频播放探针的确定性生成、真实探测与 storyboard 封装。"""
...
from governance.media_probe_profile import (
    MediaProbeAsset,
    MediaProbeProfile,
    load_media_probe_profile,
)
from governance.media_probe_storyboard_validation import validate_storyboard
...
def _run(command: list[str]) -> subprocess.CompletedProcess[str]:
    ...
    if completed.returncode != 0:
        ...
        raise RuntimeError(f"media probe command failed ({completed.returncode}): {detail}")
    return completed
...
def _asset_root(output_root: Path, asset: MediaProbeAsset) -> Path:
...
def _probe_media(
    *,
    ...
    if duration_match is None or video_match is None or audio_match is None:
        raise ValueError("media probe could not read duration/video/audio streams")
...
def prepare_media_probe_assets(
    *,
    output_root: Path,
    ...
    if not selected:
        raise ValueError("no media probe assets matched the requested asset IDs")
    ...
    for asset in selected:
        ...
        descriptor: dict[str, Any] = {
            "schema": "quwoquan.content.media_probe_descriptor",
            "assetId": asset.asset_id,
            "assetVersion": asset.asset_version,
            "processorProfile": profile.processor_profile,
    ...
    return {
        "schema": "quwoquan_data.media_probe_result",
...
def validate_media_probe_assets(
    *,
    output_root: Path,
    asset_ids: set[str] | None = None,
) -> list[str]:
