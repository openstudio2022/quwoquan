...
def assert_video_distribution_use_allowed(
    ...
    *,
    ...
    if publication_admission not in PUBLICATION_ADMISSIONS or publication_admission not in admissions:
        raise ValueError(
...
def verify_video_publication_admission(
    ...
    for row in matrix_rows:
        ...
        if admissions != PUBLICATION_ADMISSIONS:
            issues.append(f"video matrix {source_id}: production_release admission is required")
    return issues
