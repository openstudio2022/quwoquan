...
from core.controller_lease import (
    DEFAULT_ASSIGNMENT_DEADLINE_SECONDS,
    _now_epoch,
    _shared_dir,
    active_controller_issue,
    assignment_events_path,
    assignment_state_path,
    conflict_ledger_path,
    controller_lease,
    controller_lease_active,
    controller_lease_lock_path,
    controller_lease_path,
    failure_ledger_path,
    heartbeat_controller_lease,
    new_controller_run_id,
    pid_alive,
    quality_target_report_path,
    read_controller_lease,
    runtime_protection_manifest_path,
)
...
def write_runtime_protection_manifest(
    ...
    *,
    ...
    """运行保护协议：声明本次 execution 活跃期间禁止外部治理代理清理/终止的资源清单。
    ...
    - 任何外部清理动作前必须读取本清单；`pid` 存活且 lease 活跃时，清单内
      ...
      删除/覆盖，进程树不得 kill；
    - 清单与 controller lease 同目录同源（`_shared/`），execution 收口后随 execution
      证据留档，不需要显式解除。
    """
    from core.paths import RELEASE_ROOT

    defaults: list[str] = [
        str(execution_root(execution_id)),
        str(controller_lease_path(execution_id)),
    ]
    ...
    manifest = {
        ...
        "leaseRef": str(controller_lease_path(execution_id)),
        "note": note or (
            "active execution runtime protection: do not kill process tree or delete "
            "protected paths while pid is alive"
        ),
    }
    path = runtime_protection_manifest_path(execution_id, create=True)
...
def build_assignment(
    *,
    ...
    seed = f"{execution_id}|{execution_id}|{controller_run_id}|{'/'.join(path)}|{role}|{json.dumps(dict(scope), sort_keys=True, ensure_ascii=False)}"
...
def _assignment_state(execution_id: str) -> dict[str, Any]:
    path = assignment_state_path(execution_id)
    if not path.is_file():
        return {
    ...
    data.setdefault("schema", ASSIGNMENT_STATE_SCHEMA)
    data.setdefault("executionId", execution_id)
...
def append_assignment(execution_id: str, assignment: Mapping[str, Any]) -> dict[str, Any]:
    row = dict(assignment)
    row.setdefault("schema", ASSIGNMENT_SCHEMA)
    now = now_iso()
    row.setdefault("heartbeatAt", now)
    row.setdefault("createdAt", now)
    state = _assignment_state(execution_id)
    ...
    state["updatedAt"] = now
    write_json(assignment_state_path(execution_id), state)
    event = {
    ...
    _append_jsonl(assignment_events_path(execution_id), event)
    return row
...
def append_failure(
    ...
    *,
    ...
    return _append_jsonl(
        failure_ledger_path(execution_id),
        {
...
def append_conflict(
    ...
    *,
    ...
    return _append_jsonl(
        conflict_ledger_path(execution_id),
        {
            "schema": CONFLICT_LEDGER_SCHEMA,
            "executionId": execution_id,
...
def source_unit_id(
    *,
    ...
    可读契约（geo-content-trinity/execution §3）：提供 entity_name + source_kind 时
    返回 `{实体名}__{sourceKind}__{hash8}`（实体名保留中文，仅替换路径危险字符）。
    hash8 只由 canonical source identity（canonical URL）稳定派生，跨抓取快照与
    批次可复算；无 canonical URL 的非主页旧调用才回退 source ref。
...
def summarize_failure_ledger(execution_id: str) -> dict[str, Any]:
    rows = read_jsonl(failure_ledger_path(execution_id))
...
def summarize_conflict_ledger(execution_id: str) -> dict[str, Any]:
    rows = read_jsonl(conflict_ledger_path(execution_id))
...
def write_quality_target_report(
    ...
    *,
    ...
    failure_summary = summarize_failure_ledger(execution_id)
    conflict_summary = summarize_conflict_ledger(execution_id)
    goal = max(1, int(target_goal or 1))
    passed = max(0, int(quality_passed_count or 0))
    rate = round(passed / goal, 4)
    report = {
        "schema": QUALITY_TARGET_REPORT_SCHEMA,
        "executionId": execution_id,
        "targetGoal": goal,
    ...
    write_json(quality_target_report_path(execution_id), report)
    return report
