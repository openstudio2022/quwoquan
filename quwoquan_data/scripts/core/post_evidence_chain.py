...
def _normalized_ref(execution_id: str, raw: str, resolved: Path | None = None) -> str:
    text = str(raw or "").strip()
    if not text:
        return ""
    # 顶层批次目录名 = {intentLabel}-{taskHash}__{execution_id}（取自真实 batch 根，含任务消歧哈希）。
    batch_dir = execution_root(execution_id).name
    ...
    if resolved is not None:
        try:
            resolved.relative_to(execution_root(execution_id).resolve())
        except ValueError:
            return normalized
        return relative_execution_ref(resolved, execution_id)
    return normalized.lstrip("./")


def _resolve_source_path(execution_id: str, raw: str) -> Path | None:
    text = str(raw or "").strip()
    if not text:
        return None
    candidate = Path(text)
    ...
    if candidate.is_absolute():
        ...
        candidates.append(execution_root(execution_id) / normalized)
...
def _relative_ref_or_none(execution_id: str, path: Path | None) -> str | None:
    if path is None or not path.is_file():
        return None
    return _normalized_ref(execution_id, str(path), path.resolve())
...
def _build_base_source_entry(
    ...
    *,
    ...
    resolved = _resolve_source_path(execution_id, raw_ref)
    if resolved is None:
    ...
    source_unit_ref = _normalized_ref(execution_id, str(unit_dir), unit_dir.resolve())
    entry: dict[str, Any] = {
        "sourceRef": normalized_ref,
        "sourceUnitRef": source_unit_ref,
        "role": "base",
        "sourceFileSha256": sha256_file(resolved),
        "metaRef": _relative_ref_or_none(execution_id, meta_path),
    }
    if clean_path.is_file() and _is_text_source_file(clean_path):
        entry["cleanSourceRef"] = _relative_ref_or_none(execution_id, clean_path)
...
def build_source_refs_snapshot(
    ...
    *,
    ...
    """
    resolved = _resolve_source_path(execution_id, base_source_ref)
    normalized_base = _normalized_ref(execution_id, base_source_ref, resolved)
