...
- **职责**：基于已确认 demand 与只冻结目标对象身份的 immutable candidate bindings 调用 `task init`，随后核对并冻结本次 execution 的 scope、目标、载体与 quota；不要求 pre-init source/media admission。
- **输入**：confirmed carrier demand、identity-only immutable candidate bindings，以及 init 产出的三份 exact refs。
