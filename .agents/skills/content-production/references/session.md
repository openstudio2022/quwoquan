...
- E1–E4 按声明省份分片，V1–V2 按来源前沿分片；多省实体按 canonical 主归属省分配，别名先核对真实身份。`shardId` 唯一，executionId 含分片 slug；用户启动各会话，生产会话共用已点名的源码工作树与独立 publish 根，只写本片运行产物，不改源码或 Skill。
- 协调者在轮前分配各站点总请求预算和错峰顺序，并在本轮 `report.md` 写明；各片预算之和不得超过总预算。同站请求仍串行，Commons 同时最多两个取证会话、每次请求间隔至少 1.5 秒；头条百科每会话至少 2 秒，yt-dlp 每会话最多一个并发下载。响应挑战、429/503 或 provider 认证/额度错误只暂停对应站点或会话，保留原调用，不补发替代 actor。
- 共同 publish 写锁串行化各片对象事务；每片只逐个点名自己的对象，跨片复用先读池，不以目录名称或全池净增推断归属。协调者是唯一 finalize 与内容仓提交执行者，仍须具备各动作原有授权；各片保留独立 author/reviewer 与本片六段报告。
- 先完成计划点名的小轮来源和存储实测，再决定放量。每个视频轮前核对当前可用空间及原件、派生、execution、随体与保护副本的峰值；预算不足停止该轮，不自动削减目标或清理保护根。
