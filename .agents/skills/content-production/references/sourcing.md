...
- release 资产权利字段必须依当前 schema 在场并可回查，空/未知表达按 schema，不能伪造授权。当前发布只用 `production` 单轨及 `production_allowed/production_release` 记录值；实际许可、商业授权、作者与派生修改逐项保真，不从发布类别推导。旧记录只经显式 cutover 迁移或授权删除，不能作为普通生产输入；受保护历史审计原件不改写。
