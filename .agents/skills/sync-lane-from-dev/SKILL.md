...
1. PRE 校验 HEAD 是六条 `lane/*` 之一且工作区路径符合策略；在 `integration/` 触发时停止并交 `integrate-lane-to-dev`。确定 exact target，运行 `make feature-context TARGET=<exact-path>` 获取非授权 context（可 unresolved）；写入前声明 exact path scope 并核对活跃 claim。`git fetch origin` 成功后仅一次用 `git rev-parse --verify refs/remotes/origin/dev1.0^{commit}` 冻结 `published_sha`；记录 HEAD/SHA/ahead-behind/merge-base/status 与进行中 Git 操作。fetch 或远端 ref 解析失败立即阻断，绝不 fallback 本地 `dev1.0`。
...
current feature context/claim ref、同步前后 HEAD、冻结的 `origin/dev1.0` exact SHA、ahead/behind、采用的 `ff-only` 或显式授权 merge、merge commit SHA（若有）、冲突处置、focused 验证命令与退出码、WIP 保留证据。仅本地同步完成，不代表 dev 发布、Alpha、准出或 main 合入；不推远端 lane。

## 失败与停止

布局不符、fetch/远端 ref 不可用、进行中操作、claim/脏路径重叠、分叉未获 merge 授权、冲突无法保留双方语义或验证失败时 `GATE_BLOCK`。保留首个 typed blocker 与当前 Git 状态，不 reset/stash/clean/`--abort` 他人字节，不用 `-X ours/theirs` 整体覆盖，不把未验证 merge 冒充完成。
