...
2. 读取最近子树 `AGENTS.md`，best-effort 运行 `make feature-context TARGET=<exact-path>`：关联 context 时保存 context snapshot 并消费 feature chain、canonical contexts、OPEN 与 applicable agents；无 owner、多 owner 或解析失败时记录 typed 结果，基于当前 Git 快照继续只读。
...
交付 exact target、context manifest ref（若可用）、typed owner 解析结果、父链（若可解析）、范围、验收层、风险与建议下游；ref unavailable 时必须明确记录，证据来自当前 Git 快照和可用的 exact ref，不来自固定路径或对话印象。
...
确认需要写规格、设计或实现后，分别交接 prd、design 或 dev，并传递 exact target 与 context snapshot（若可用）；下游进入 mutation 前仍须取得context query（允许 unresolved 或多关联）。持久交接语义见 continue Skill。
