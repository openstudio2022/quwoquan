...
- [MUST] 改动落在 PRE feature context 指定的对象与契约边界内。
...
- [MUST] 命名、注释与测试失败能直接指向业务语义。
  check: 读取新增命名/注释/断言消息；只表达阶段或复述代码时判失败。新增或重命名的方法、类型、属性、变量、测试名与错误码把可变闭集的当前成员数写成身份，或函数体内平行维护与 canonical 闭集等价的成员表时判失败；冻结协议 id、已发布节点目录名与契约字面量按身份处理，不判失败。
