...
- [ ] 内容卡与所有内容消费面只有 `赞 / 评 / 转` 三动作；全 App 无「收藏 / 关注内容 / 稍后看」入口与文案；`favorite` 契约与代码 grep 零残留（点赞心形图标 `Icons.favorite*` 豁免）——**防回归项**（基线修正已达成）
- [ ] 旧交集 kind grep 零残留：`mutualFriend|friendVisited|friendInCircle|coCollectedEntity` 全仓清零（词典退场清单章节豁免；对应 WP1·T1/T4）
- [x] group 概念用户侧仅出现「讨论」；禁用词（空间/频道/论坛/群组/趣群，群概念语境）清零
...
- [x] 内容卡交集理由位于图下、标题上，无图上覆盖（WP2 反断言测试在 CI 绿）

### 2.4 数据诚信

- [x] 抽查影响模块数字（TA的影响/我的影响力/圈子影响）每条可点开枚举来源；无数据模块收起
...
1. [ ] 推荐页看到交集 spotlight（≥3 卡非空窗，**非空依赖 WP1·T3 空窗治理**）与瀑布卡交集理由（gamma 真实 API）
...
7. [ ] 他人视角阅读该文：交集理由 + 提及可点；在文下评论触发「共同讨论」交集（WP1·T2）；该文自动进入读者「我的足迹」（**足迹列表页由 WP1·T5 交付**），且足迹不产生任何交集
8. [ ] 作者「我的主页 → 我的影响力」出现可枚举增量；产生新交集后收到小趣提醒 → AppMessage 通过结构化 route/query 跳转我的连接收件箱对应维度
9. [ ] 内容交集详情或对象页交集卡点击「问问小趣」→ 小趣只基于透传 reason / object 上下文解释推荐理由，不编造收藏、稍后看或未提供事实

辅助旅程：

- [ ] 搜索「讨论」分组正常；Web 宽屏精品按定稿规格渲染（`featured/all -> work`、`image -> photo`、`video -> video`、`article -> article`，无「精品队列」hero/rail，点击作品进入统一 `workBrowser`）；消息页两行布局/索引/星标回归
- [ ] Web 精品 context tabs 与移动端 Work Browser 更多菜单筛选同源于 `ContentUIConfig.workFormatFilters`；移动端文章深色纸张与阅读设置无回退、无白闪
- [ ] 登录入口双目标契约回归（`make verify-app-login-entry-loop-contract`）

## 4. 文档与 CR 收口

- [x] 各包 dev_log（含 verify 命令证据）已追加至 `CR-20260611-033`
...
- WP7 专题 / 圈子精选卡位（Core 不交付；如启用 Plus，需 metadata 驱动开关、UI 卡位、跳转对象页/圈子页与 T2/T4 证据）
