...
| 趣信 | `chat_page.dart`、`chat_conversation_page.dart`、`chat_settings_page.dart`、群管理页 | `chat.conversation`、`chat.message`、`chat.member`、`chat.user_state`、`chat.group_settings`、`chat.contact_row` | `chat_core`、`chat_settings_core`、`chat_contacts_core`、`chat_group_flow_core` |
...
| 首页圈子/圈子列表 | `AlphaCircleQueryReader` 从 `circle_core` 与 `circle_home_feed_core` typed fixture 初始化 | Remote typed Circle query facets + `listHomeCircleDiscoveryFeed` 聚合圈子 feed | `GET /circles`、`GET /circles/{circleId}/feed` |
