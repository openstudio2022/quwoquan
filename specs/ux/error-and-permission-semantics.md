...
- 首屏网络失败：标题“暂时连不上”，说明“可能是网络问题。”，主操作“再试一次”。
...
#### 1.12.1 面向用户的文案与退出路径（强制）

错误界面只回答三件事，且三者不得重复或互相矛盾：

1. **标题**（必选）：说明发生了什么，不包含“重试”“稍后”等恢复指令。
2. **主操作或退出指引**（必选其一）：告诉用户此刻如何离开错误态。
3. **说明**（可选）：只补充用户无法从标题或操作推断出的原因、替代路径或登录续接。

默认使用“标题 + 一个主操作”的最简结构。说明不是必填内容；若不增加新的决策信息，必须省略。

| 规则 | 要求 |
|------|------|
| 操作唯一归属 | “再试一次”“去登录”“去设置”等恢复动词只能出现在可点击控件上；正文不得写“点击重试”“点一下再试”。 |
| 时间语义一致 | 有可点“再试一次”时，标题、说明与 `user_message` 禁止出现“稍后”“稍后再来”“请稍后重试”。 |
| 可点可辨 | 标题是静态文本；操作必须通过容器、tint 或链接色清晰表达可点击，最小热区为 44pt。 |
| 宿主退出唯一 | 栈页的返回、模态的关闭、沉浸浏览的滑走由宿主提供；错误组件不复制“返回”或关闭 CTA。 |
| 无效操作禁止 | 不可恢复或不支持的失败不得展示“再试一次”；改用“可以先看看别的内容”等非按钮替代路径。 |

所有错误载体必须具备下列可观察退出路径：

| 场景 | 错误层主路径 | 宿主路径 |
|------|-------------|----------|
| 可重试视频播放失败 | 唯一 CTA “再试一次” | 滑走下一条或返回 |
| 视频不可用 / 不支持 | “可以先看看别的内容” | 滑走或返回 |
| 首屏无数据加载失败 | 唯一 CTA “再试一次” | 顶栏返回 |
| 局部区块失败 | “再试一次” | 继续使用页面其余内容 |
| 分页追加失败 | 可点单行“后面没加载出来，轻点重试” | 继续上滑可再次触发 |
| 已有内容的刷新失败 | 无按钮的短暂提示 | 滚动或再次下拉 |
| 动作提交失败 | “再试一次”与“取消” | 关闭弹窗即放弃该动作 |
| 登录 / 权限门禁 | “去登录”或“去设置” | 宿主返回或稍后关闭 |

#### 1.12.2 媒体内联播放失败

媒体播放失败在 feed、沉浸浏览和独立播放器中使用同一 `VideoPlaybackFailureOverlay`，不替换页面或叠加第二个错误层。

| 场景 | 标题 | 说明 | 操作 |
|------|------|------|------|
| 网络不稳定 | 网络不太稳定 | — | 再试一次 |
| 临时不可用 / 服务繁忙 | 暂时无法播放 | — | 再试一次 |
| 内容不可用 | 这条视频暂时无法观看 | 可以先看看别的内容 | — |
| 当前设备不支持 | 这条视频暂不支持播放 | 可以先看看别的内容 | — |

- 深色媒体面使用静态 medium 标题与半透明白色胶囊主操作；按钮文字为 semiBold，标题不得使用同等强调。
- 可重试场景不渲染说明行；不可重试场景不渲染伪重试按钮。
- 服务繁忙是观测细节，不增加不同于“暂时无法播放”的用户决策。
...
- `AppInlineFieldError` 与 `AppFormErrorCard` 必须复用同一透明内联错误行：16px 圆形感叹号、14px 常规错误文字、图文间距 6px；浅色 `#E5484D`、深色 `#FF6B6B`，图标、文字和字段错误边框同色。
- 内联错误行不得使用胶囊、色块、卡片、阴影、警示圆点或独立标题；默认单行，窄屏或 200% 动态字体可自然换为最多两行。独立恢复动作只在主按钮无法承担恢复时保留，并且不改变错误行本身的透明样式。
- 同一失败只允许一个主反馈，不同时展示 Toast、caption、dialog 或第二张卡片；新错误通过 live region 播报一次，用户编辑、重试或成功后清除。
- 当系统已经自动完成降级或切换到可恢复表单时，使用无嵌套动作的紧凑 `AppFormErrorCard`；目标表单的主按钮唯一承担后续动作，禁止错误行复制“重试/登录/获取验证码”。
