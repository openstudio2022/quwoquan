...
- 文章 `MUST NOT` 显示图片/视频式点状页码指示器；文章页码 `MUST` 使用“返回 + 当前页/总页数”的顶部阅读导航语义。
- 文章阅读器 `MUST` 以全屏书页舞台呈现，正文页从顶部安全区与阅读导航下方开始，`MUST NOT` 侵入顶部工具栏区域。
- 文章书页舞台 `MUST` 左右占满屏幕；允许出现书页实体边缘、书脊、纸张厚度，但这属于书本外观，不属于普通卡片留白。
- 文章若存在封面，封面 `MUST` 进入第一页作为扉页式上半屏素材；`MUST NOT` 默认变成纯封面独占页。
- 文章内图片组若进入子查看模式，页码 `SHOULD` 与文章页码复用同一顶部导航位置语义，即显示在返回按钮之后，而不是另起一套右下角或点状指示器。
...
- 关注流文章卡 `MUST` 支持“有封面”和“无封面”两种正式样式；无封面时 `MUST NOT` 保留空白图片区。
- 圈子双列文章卡 `MUST` 支持“上封面下标题摘要”与“高质感文字卡”两种形式，并保持瀑布流中的卡片几何稳定。
- 文章分发卡 `MUST NOT` 直接复用图片/视频 post 的大图预览逻辑；封面是作者显式选择的分发素材，而不是正文首图自动顶替。
...
- 文章编辑器 `MUST` 提供封面、模板、字体三类正式展示设置；它们属于文章 presentation system，而不是临时装饰开关。
- 文章编辑页、预览页与沉浸式阅读页 `MUST` 共用同一分页几何与模板语言；编辑态允许保留控件 chrome，但 `MUST NOT` 换成另一套纸面逻辑。
- 文章模板 `MUST` 至少覆盖手帐、精装书、画册/相簿、科技手册、轻刊专栏五类真实书本风格，并允许按圈子频道给出默认推荐。
...
| 底层亮度 | 统一 modal brightness layer | 同 compact | 同 compact | `ColorType.modalScrim` 只允许在 presenter 内解析；route barrier 固定透明，面板独立转场，必须保留背后页面上下文 |
