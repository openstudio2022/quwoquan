...
实体主页（`HomepageDetailShell`，school/place/enterprise/sight 归一 `homepageDetail`）与「我的主页/他人主页」同壳同语义 token 重做，统一结构为「身份 → 我的交集 → 这里打动的人 → 关于这里 → 记录·讨论·相关圈子」。规格真相源见 [intersection-definition-and-application.md](../../../../product/intersection-definition-and-application.md) §17.7 与 §17.8（双模块口径）。两个核心维度：

- **我的交集 = 我与这里客观存在、可枚举、可解释、可行动的真实连接点**（你认识的人在这里、你来过、你关注的话题在这里发生）。
- **这里打动的人 = 这里帮助他人产生连接、内容传播、讨论沉淀的能力**，同样可证、可枚举、可解释、可行动；`impact` 仅保留为内部机器名。
...
- 打动卡（新增）：标题「这里打动的人」，与「我打动的人」（`AuthorImpactCard`）/圈子打动卡同构，渲染 `IntersectionStatementCard` + `entityImpactProvider`；逐条只读云侧 `EntityImpactItem.primaryText/primarySpans`，句内数字可点击下钻「影响明细」sheet（来源摘要 + 样本视觉）；无可枚举影响事实时不展示，不用主观营销语。
...
- A2：打动卡标题「这里打动的人」，逐条只读云侧 `EntityImpactItem`，句内数字可下钻「影响明细」；无可枚举影响事实不展示，无主观营销语。
