...
- `TA打动的人` 去好友化文案（历史「TA帮助了很多人」口径在 V5 废止）。

### S2b 我的主页（Mine）

- 同壳头部与 tab/过滤规则与 S2a 一致。
- `MyIntersectionInboxCard` 升级为 `我的交集` 列表入口（红点聚合并入「查看更多」）；主标题固定为「我的交集」，时间窗口按用户活跃节奏动态解释，最小粒度可细到今日、最大可覆盖上次登录以来。
- `我打动的人` `isMine` 视角，去好友化/去收藏。
...
- A3：`我与TA的交集` / `我的交集` 为列表入口，每行单结论句 + 至多一条辅助说明；我的主页主标题固定为「我的交集」，不回退为「今日交集」。
- A4：打动文案去好友化（`认识了新朋友` → `建立了新连接`），无收藏文案；他人/我的标题统一为 `TA打动的人` / `我打动的人`。
