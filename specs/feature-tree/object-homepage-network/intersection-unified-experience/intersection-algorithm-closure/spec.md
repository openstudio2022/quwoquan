...
- 证据层：local_contract 固定衰减时钟、重复/乱序与来源交错；api_integration 通过真实 User/Content command、自动 worker 和交集 reader 读回；user_acceptance 在主页/Feed 证明当前受限事实不再展示，不以 UI 单独证明后台贡献已清理。
...
- 证据层：local_contract 验证预算退出与续跑；api_integration/容量证据证明实际 reader 和物化成本，真机少量展示不代替容量。
