...
- [`JNY-011 / SCN-029`](../spec.md#scn-029)
  - 本领域负责：在“可行动对象进入会话”中，提供被分享对象的公开事实与其可导航承接页。
  - 进入条件：用户在会话中分享内容、主页、圈子或 Gathering 且权限前置成立。
  - 交付给下游的结果：可被行动提示引用的对象事实与承接页，供 `recommendation-platform` 继续处理。
  - 不负责：不写入会话事实，也不决定 card 的行动分流。
