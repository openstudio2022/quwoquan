...
- **职责**：多场景推理 API（POST /score、scenario 路由）、模型加载（从 ModelRegistry/OSS）、与 Go 集成（HTTP 契约、CascadeScorer 兜底）；部署形态为**常驻在线服务**（FastAPI）。
...
| L4 | 说明 |
|----|------|
| inference-api | POST /score、scenario 路由、多场景模型加载与推理 |
