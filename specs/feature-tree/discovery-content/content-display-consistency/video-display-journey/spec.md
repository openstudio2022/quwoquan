...
- 显式横向模式按 [`works-immersive-viewer` REQ-022](../../dual-rail-discovery-redesign/works-immersive-viewer/spec.md#req-022) 只局部旋转查看器、控件和内部弹层 90°，媒体在完整横向视口 contain，不请求系统横屏、不随传感器切换，也不新增浏览器系统全屏能力。顶部仅返回/标题/更多，中央显式播放/暂停；作者头像/名称/关注与赞转评仍在底部，右下独立收起；进度条位于整组工具栏上方，所有已知有效时长（含 ≤30000ms）随控件显示，中性白/灰轨道与白色拖动点，不用红色。
