...
> 设计引用：[L2 DEC-001](../design.md#dec-001)、[DEC-004](../design.md#dec-004)、[DEC-005](../design.md#dec-005)、[DEC-006](../design.md#dec-006)
...
- 优先级：`P0`
- 准出影响：`block`
- 影响或价值：`REQ-001`～`REQ-004` 的当前语义已替换“两个 bool 即足够”和“本地到期直接失败”；尚缺能证明 actor 分区、逐对象 hydrate、persist 成功与权威同键终结的当前证据，旧 `GWT-001` 绑定不能替代。
...
- 优先级：`P0`
- 准出影响：`block`
- 影响或价值：`REQ-005`、`GWT-005`、`GWT-006` 尚缺 owner typed slice、严格 mapper、REST/公开 GraphQL 与私有组合及设备 UI 反例；不能把合法公共缺席或 HTTP 200 当互动完整成功。
