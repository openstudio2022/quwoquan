# L3 Story：实时推荐流 (`realtime-feed-baseline`)

> 所属能力：[`feed-orchestration-recommendation`](../spec.md)

> Journey / Scenario：[`JNY-003 / SCN-007`](../../../spec.md#scn-007)

> 设计归属：[L2 DEC-001](../design.md#dec-001)

## 1. 用户价值

作为内容创作者或浏览者，
我希望统一 sessionId / feedRequestId 归因，
从而完成可恢复的内容创作、发现或互动。

## 2. 范围与非目标

### In Scope

- “实时推荐流”的输入、可观察主路径、失败语义以及与父能力的交接。
- App 行为上报到 HotPath session 信号。
- 多路召回、预排、规则/模型级联打分、重排去重与冷启动保底。
- Feed 延迟、空结果、fallback、重复曝光的 SLO 基线。
- 深度排序模型、双塔向量召回、广告竞价、搜索排序。
- Thompson Sampling 与大规模曝光状态结构实现。

### Out of Scope

- 父能力中由其他 Story 独立拥有的行为、能力级架构决定和实现任务。

## 3. 行为要求

<a id="req-001"></a>
### REQ-001 实时推荐流

- 统一 sessionId / feedRequestId 归因。

<a id="req-002"></a>
### REQ-002 RuleScorer 为默认安全底座，任何模型上线必须支持一键回退到规则

- RuleScorer 为默认安全底座，任何模型上线必须支持一键回退到规则。
- 统一 sessionId / feedRequestId 归因。

## 4. 契约引用

- canonical：`quwoquan_service/services/content-service/contracts/content/post/operations.yaml`
- canonical：`quwoquan_service/services/recommendation-service/contracts/recommendation/recommendation_model_release/operations.yaml`
- canonical：`quwoquan_service/services/recommendation-service/config/schema.yaml`

## 5. 验收场景

<a id="gwt-001"></a>
### GWT-001 实时推荐流

- GIVEN 内容创作者或浏览者具备有效身份，且父能力声明的输入与上游事实成立。
- WHEN 参与者执行“实时推荐流”对应的公开行为。
- THEN 统一 sessionId / feedRequestId 归因。
- AND 失败时返回 canonical failure，且不产生伪成功事实。

## 6. 依赖

- 前置要求：[`feed-orchestration-recommendation`](../spec.md) 的范围、要求与 SIT。
- 下游结果：本 Story 声明的 GWT 可观察结果。
- 父级设计：[L2 DEC-001](../design.md#dec-001)
