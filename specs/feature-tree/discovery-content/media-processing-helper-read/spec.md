...
<a id="open-002"></a>
### OPEN-002 media 上下文无 typed 枚举，且 contentType 与 content 域同名异义

- 类型：`capability_gap`
- 优先级：`P1`
- 准出影响：`track`
- 影响或价值：缺少 typed 枚举引用。`media/media_asset/fields.yaml` 没有任何一个 typed 枚举——`mediaType`、`contentType`、`accessPolicy`、`processingStatus`、`coverStrategy`、`processorProfile`、`imageDeliveryContentType`、`videoCodec`、`videoContainer`、`videoAudioCodec` 全是裸 string，而 `object.yaml` 的 `lifecycle.states` 已写明 `[processing, ready, rejected, deleted]`，且 `MediaCoverStrategy` 等枚举在 `_shared/types.yaml` 已存在，属纯退化。`filter_catalog_release` 与 `media_image_reprocess_run` 的 status 同样退化，直接削弱了它们各自精心设计的状态机（后者 6 状态 8 迁移），codegen 拿不到状态集，非法迁移只能靠运行时字符串比较兜底。此外有一个必须避免"顺手统一"的陷阱：`contentType` 在 `content/post` 是内容形态（image/video/micro/article），在 `media_asset`/`media_upload_session` 是 MIME 类型，是同名异义，比类型退化更危险。
- 完成判定：三处 status 与 `coverStrategy` 补 `enum_ref`，枚举值取自各自 `lifecycle.states`；media 侧 `contentType` 改名 `mimeType` 消除同名异义。由 runtime-governance `OPEN-002` 的 lifecycle 一致性门禁防回归。
