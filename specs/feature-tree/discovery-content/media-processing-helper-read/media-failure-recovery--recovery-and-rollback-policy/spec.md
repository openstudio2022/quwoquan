...
- 带有效 cursor 的损坏 outbox 记录必须先写入 durable dead letter，再推进 checkpoint；
  dead letter 写失败时保留 checkpoint，缺失 cursor 时 fail-closed，禁止猜测跳过位置。
...
- poison event 持久化后后续事实可继续消费；死信写入失败不会错误推进 checkpoint。
