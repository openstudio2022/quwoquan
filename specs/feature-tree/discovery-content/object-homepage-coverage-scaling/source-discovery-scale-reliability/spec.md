...
>
> 设计归属：[L2 DEC-001](../design.md#dec-001)、[L2 DEC-028](../design.md#dec-028)
...
- 仓内 source discovery worker、slot、heartbeat、claim、runner、fleet、execution-state 或 capacity/calibration authority。
- 由代码自动选择替代来源、解释业务终态、重试失败或推进后继 stage。
- 恢复旧 scheduler/progress/heartbeat/capacity schema、sequence-017、shim 或 dual-read。
...
- Skill 十阶段是唯一业务工作说明；`sources` 只写逐 target source plan，`1.download` 才创建 source unit、source ref 与取得字节。
...
- 仓库不保存 source discovery worker、slot、heartbeat、claim、runner、fleet、execution-state 或 capacity/calibration authority。
...
- THEN 仓库中不存在 worker/slot/heartbeat/claim/runner/fleet/execution-state/capacity authority，后继只由 Skill 固定。
...
- 优先级：`P1`
- 准出影响：`track`
- 影响或价值：旧 scheduler/worker/fleet、slot/heartbeat/claim 与 capacity authority 已物理删除，并由 post-delete architecture/public CLI live-import gates 持续锁定；但当前尚缺硬切后多 target 来源发现的行为证据，不能仅凭删除门宣称逐 target 计划、下载与局部失败隔离已经实现或闭合。
- 尚缺验收证据：同一 candidate-backed execution 至少包含一个成功 target 与一个 typed blocked target，直接证明计划先于下载、结果 identity 不串线、局部失败不覆盖其它 target，且无旧控制面 import/reference。
...
- 依赖：当前 source plan、atomic acquisition 与 stage OPEN/CLOSE 边界；不得恢复旧 scheduler/worker/fleet 来补证据。
