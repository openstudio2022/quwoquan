...
- 实体口径是「用户感兴趣、愿意去的一切地方」：景区景点、秘境、网红打卡地、古镇、露营地、温泉、观景台、徒步节点、探险探秘地，不限 A 级评定，不设数量上限；实体类型只从 taxonomy `Entity/地点/*` 现有叶子中选取。homepage 主源必须是百科闭集成员：zh.wikipedia 或头条百科（`www.baike.com`，`sourceKind=toutiao_baike`），百度百科因 robots 全站禁止不在闭集。实体发现以携程景点榜（热度分、点评数、「必打卡」标签、类型筛选）、zh.wikipedia 分层类目（5A/4A/3A、一级博物馆、历史文化名镇名村、国家级自然保护区、各省文保单位）与 AI 按地域给出的秘境名单为前沿，逐条以百科页存在与正文厚度核验。
- Article 的主来源可以是同一实体条目以不同 `publishAngle` 切入，也可以是主题条目页；事实参考可追加携程游记/攻略、zh.wikivoyage 与 robots 允许的公开博客，全部按 `factual_reference_only` 登记——AI 只取事实、路线、费用与贴士并亲笔原创，不搬运来源表达，来源 URL 全部进入 `sourceUrls`。Article 分类必须覆盖 `摄影`；摄影文章与攻略、游记等使用同一 Post/Article 契约与质量准入，不创建第二套载体。
- Image 来源按「开放许可专业摄影池优先」运行：Flickr 官方 API（CC 许可筛选，`interestingness` 作排序）、Wikimedia Commons（条目图与 Commons 类目，Quality/Featured 优先）、头条百科标注 `CC BY-SA` 的自有图片、Unsplash/Pexels/Pixabay 官方 API（自有免费许可，按白名单派生 `unverified`）。最终来源、creator、原始资产、取得方式与 license/terms 必须回到作品页或官方 API；Pinterest（robots 全站禁止）、图虫（正文纯脚本渲染且数据接口被 robots 禁止）、500px 不进入自动化路径，只能以「人工提供文件」登记。
- Video 来源按「切题实景、可落实体、热度作排序」运行：Wikimedia Commons 视频类目与 `filetype:video` 全文检索、YouTube Data API 以 `videoLicense=creativeCommon` 发现并按其 CC BY 许可取得、Pexels/Pixabay/Vimeo 的 CC 或自有免费许可片段、Bilibili 只在前述不足时作补位（版权保留，`unverified`）。旅行摄影博主的其他作品只保留逐条通过实体/地点/摄影相关性与负面主题过滤的命中，不构造作者全集；无法取得逐作品来源证据时只保留本次命中。抖音、快手、小红书因登录墙与服务条款不进入任何路径。
- 热点信号（携程热度/点评数、维基 pageviews、Flickr interestingness/faves、YouTube viewCount）与逐载体候选级质量筛选（百科正文厚度与结构化事实、游记长度与实用信息项数与时效、图片分辨率与主体切题与水印类型、视频时长与清晰度与实景比例）只作候选排序与可重放的候选级放弃判据，不是 execution 内的门禁，也不落台账。
- 所有站点、搜索和 creator shard 只允许公开直链、平台支持接口或人工提供文件，不得规避登录、付费墙、验证码、访问控制、DRM 或 robots/服务条款限制；新增站点前先读其 robots 与服务条款，`Disallow: /` 对通用 agent 生效的站点不进入任何自动化路径。来源侧出网只由宿主 AI 以通用工具（检索、读页、`curl`/`yt-dlp` 下载、HTML→text 转换、`jq`、看图）执行；仓内不存在任何来源侧网络客户端、抓取器、限速器、HTML 或 MediaWiki 抽取器，API key 只存在于本机环境而不进仓库；礼貌节流是 Skill 对宿主 AI 的明文约束（使用声明产品与联系方式的合规 User-Agent、同一来源站点串行并遵守其 Crawl-delay、429/503 退避并放弃同一轮次剩余候选）而不是代码保证。单来源的 typed failure 只阻断自身，不阻断同 carrier 其他来源。
