...
- 权利边界：acquire 机械派生逐资产 `rightsStatus`（白名单 license → verified；其它可读 license → unverified 并写 `rightsIssues`；不可读 → unknown），并保留 `commercialAuthorizationStatus`、`authorizationProof`、`license`、水印、`accessPolicy` 与 `authorizationRequired` 等真实权利事实；publish 事务与 release build 只把这些事实写入唯一 manifest 与包内 `sources/<unit>/source.json`，header 只派生计数，pool record 只绑定包与 review 摘要，不复制完整权利投影，不据此拒绝对象。新对象及现役 schema/writer/reader 彻底退役 `distributionDecision`、`publicationAdmission`、对象级 pool `usageScope=research|commercial` 与 `variantPurpose=commercial_variant`，不得以 `production`、`default` 或其它单值替代。资产用途 `usageScope=internal_reference|app_publish|editorial` 是另一语义并继续保留。历史 canonical/release/review 原件不原地改写；包格式转换按 `DEC-023` 生成 current new version 与摘要，原 review 与历史字节保留，不能以转换伪造重审。
