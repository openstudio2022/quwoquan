...
- 可观察面：local_contract 构造同时含未达门槛候选与判定未完成候选的实体，断言归并得到 `探测失败`。携带未归类原因的实体同样以 `探测失败` fail closed 并点名该原因。四类分子、分母与占比可由实体级聚合算出，第四类单独计量。三阶段逐实体对账由 api_integration 经一次真实 execution 的 selection、auto_research 与 content_plan receipt 完成，ready 数下降时能精确列出出局实体及其首要原因。
- 关联要求：`REQ-001`
...
- 可观察面：local_contract 让两条抽取路径分别停在未达 quota 的终态，断言各自返回 typed stop reason 而不抛错。对同一终态分别注入两种处置取值，前者阻断、后者以实际合格集合继续，合格数为零时两种取值都阻断。处置入参缺失即 fail closed，stop reason 不出现在 verdict receipt 与零合格原因值对象中。
- 关联要求：`REQ-001`
...
- 关联要求：`REQ-001`
- 影响 Story：[`multi-carrier-release`](./multi-carrier-release/spec.md) 的共享选择路径
...
- 约束与影响：准入出局不是失败。补采循环把它计为该轮未产出并在后继轮次补齐，因此它只改变补采轮次数，不改变 quota，也不改变 `DEC-002` 的三值分离与 `DEC-010` 的补采轮次机制。
...
- 可观察面：local_contract 让准入挡下若干 `在场可用` 实体，断言选择证据可逐实体列举出局实体、约束与原因，实体键与 verdict receipt 逐字相同，且 verdict receipt 无任何准入字段。零准入使 spec 不被冻结时，选择证据仍存在且出局集合可读。闸门未运行时省略该键，闸门运行且零出局时该键在场且集合为空，两者不被读成同一结果。
- 关联要求：`REQ-001`
