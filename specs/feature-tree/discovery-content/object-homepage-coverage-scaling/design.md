...
- 对象边界：`ContentProductionTaskView`、`ContentItemVersionView`、`EnvironmentReleaseOrderView`、`ReviewDecisionTimeline`、`ReleaseSelectionView`、`TargetAcceptanceView` 都是 projection/query view，不是 aggregate、process manager 或 evidence owner。`ContentProductionTaskView` 的 owner 是现役 carrier demand/execution manifest/stage receipts；旧 WorkRequest schema 不在依赖闭包。`ContentItemVersionView` 与 `ReviewDecisionTimeline` 的 owner 是 canonical object transaction/pool record 及其已绑定 review facts。`ReleaseSelectionView` 的 owner 是 `ContentRelease`。`EnvironmentReleaseOrderView` 与 `TargetAcceptanceView` 对 Alpha/Beta/Gamma 只读 operation/EAF v2，对 Prod 只读 RC Qualification、`ReleaseTagAdmissionFact`、`ProdActivationAdmissionFact` 与 hosted lifecycle facts；view 不复制 owner payload 或生命周期，也不构造 Prod EAF。
...
- 对象边界：M100 使用 exact release homepage/article/image/video=`100/100/100/10`。Data-owned `ReleaseUatSamplePlan` 冻结同 release 的 entry surface × carrier required cells；runner 逐 cell 写绑定同一 exact integration candidate 的唯一 raw `ReadinessCaseResult`。Environment Ops scheduler 是 EAF 唯一 producer，以同 candidate request 签发 Gamma `EnvironmentAcceptanceFact` v2，并在 `caseResultRefs` 之外完整绑定 `impactPlanDigest`、8 个 named closure refs、Beta predecessor、有效期、`nonPromotable` 与 DSSE signer；Data 不写 EAF。
- 环境顺序：EAF 只保持 Alpha→Beta→Gamma predecessor authority。M100 目标增量硬终点是在必要 Alpha/Beta predecessor 完成后取得 Gamma acceptance；Prod 不在 EAF 环境闭集，Prod activation、Commercial release 与商业验收明确 out of scope，不得用它们替代或阻断目标增量终点。
...
- 失败恢复与回滚：M100 任一 import/readback、required raw UAT、scheduler named closure 或 EAF v2 acceptance 失败时保留 previous active，按环境 owner 追加 rollback/readback facts；不得预启动 M1000。M1000 首 slot 的 init/0.plan 失败只保留 typed blocker，不改 M100 acceptance。
- SLI/SLO：gate 的唯一通过判据是同一 candidate 的 Gamma EAF v2 exact-byte closure，其中 `caseResultRefs` 绑定当前 release 的 fresh raw results，全部 named evidence、predecessor 与 DSSE 均有效；counts、旧 receipt、workflow success、Alpha-only UAT 或 projection verdict 均不得代填。M1000 pre-gate mutation count 必须恒为 0。
- 可测试面：local_contract 覆盖二维矩阵、raw 单写、scheduler-only EAF writer、`smoke|integration|release` profile、8 个 named closure refs、Alpha/Beta/Gamma predecessor、DSSE、零副作用与只读 query；api_integration 覆盖 Alpha/Beta 前序和 Gamma import/readback；registered physical device user_acceptance 生成 fresh raw facts。M1000 start test 断言首 slot 只到 `0.plan pass -> sources`。
...
- 新架构验收：删除后用全新 homepage/article/image/video 各 M1 execution，经 Skill 十阶段、逐对象 publish、显式 `1/1/1/1` Research cohort，到 Alpha ship apply/import/readback/health，并以 `m1_api_consumer` 内容 intent 选择 16-cell API consumer runner。runner 只消费既有 release/import/verify readiness、content-consumer health 与 sample plan，执行只读 API 请求并产出 canonical raw CaseResult；不写 EAF。若 Alpha completion 要求 EAF，则同 exact integration candidate 的 Environment Ops scheduler request 必须以 canonical `profile=smoke|integration|release` 中调用方冻结的取值，完整绑定 `candidate`、`impactPlanDigest`、`caseResultRefs`、8 个 named closure refs、`predecessor=null`、有效期、`nonPromotable` 与 DSSE signer 后签发 v2；任何一项缺失都保持 `GATE_BLOCK`。该验收证明新架构，不授权删除。
- Prod acceptance：RC Qualification factory 以 package/provider/UAT/supply-chain facts 汇总 `QualificationFact`；stable `ReleaseTagAdmissionFact` 再成为 `ProdActivationAdmissionFact` 的唯一入口，hosted rollout/readback/soak facts闭合生产结果。Prod 不生成或消费 EAF，Data ship 也不写 Prod acceptance。
- 可测试面：零 legacy 路径/import/CLI/schema/reference；OPEN/CLOSE create-once；全新 M1 receipts 不引用旧 proof；API consumer raw facts 与 scheduler EAF v2 不互相代填；Prod admission 明确拒绝 EAF selector。
- 关联要求：[`multi-carrier-release`](./multi-carrier-release/spec.md) 的 `REQ-006`、`REQ-007`
- 影响 Story：[`multi-carrier-release`](./multi-carrier-release/spec.md)
...
<a id="dec-042"></a>
### DEC-042 Content 发布以最小候选集合完成 verified-before-active

- 对象边界：Content importer 以独立 `posts_candidate` 集合暂存一个 immutable release 的 Post 闭包；候选键由 `environment + sourceOwner + releaseId + manifestDigest + postId` 唯一确定，文档保持与 `posts` 同形但固定 `lifecycleStatus=candidate`。它不是第二 active store，公开 feed/detail/search/recommendation 不读取它；候选读回是 content-service `cmd/import --phase verify` 的受保护存储读回（`GetReleaseCandidateReadback` 只是该 importer 入口的 operation 名），不新增线上 HTTP API。`data_release_state` 的 `environment + sourceOwner` 单文档 CAS 仍是唯一 activation authority；一步式 `ApplyImportedPostRelease` 只保留给显式 `--require-replay` 的 exact-active replay/repair 轨道，普通发布不得调用。
- 顺序与事务：`stage` 只校验 immutable 输入、向 `posts_candidate` 写候选、向 `media_assets.sourceReleaseIds` 做 additive membership，并写 prepared/imported/projected candidate receipt；它不写 `posts`、active pointer 或生命周期 outbox。`verify` 精确绑定 `releaseId + manifestDigest + candidateRevision`，从受保护读回证明候选数量与 sourceHash 身份闭包，并复核 stage 阶段按 manifest sha256 逐文件校验过的媒体同步证据；失败只写 typed terminal。`activate` 在一个 Mongo transaction 内重验 verified evidence 和 expected active revision，按稳定 Post `_id` 把候选复制进 `posts` 并把 `version` 重写为本次激活解析出的 `sourceVersion`（候选阶段的 `version` 是 `candidateRevision`，两者必然不同），应用本 release 的 tombstone，追加 outbox，最后 CAS `data_release_state` 并写 active receipt。任一步失败时 transaction 全部回滚且 previous active 仍可读。Search/Recommendation 只从 activate 时写入的 outbox 派生，因此其读回只能在 activate 之后由 consumer verify 完成；这是否决 Search/Recommendation 候选索引的直接推论。
- 引用对象与媒体：Creator 与 Homepage 在 activate 前只允许 `upsert`，任何删除或下线在 Content pointer 成功后执行；Tag taxonomy importer 没有 upsert/sync 之分，它在 stage 阶段就写入按 `releaseId` 保留的 snapshot 并切换 taxonomy 自己的 active pointer（previous snapshot 保留、可重新 Activate，因此对 rollback 可逆但并非"激活前不动"）。媒体同步同样只 additive copy。`upsert` 对新增对象是纯加法，但会在激活前覆盖同一 stable ID 的既存 Creator/Homepage 投影，与 Tag pointer 先切一样都是已接受的精简取舍并由 `OPEN-022` 跟踪，不得据此声称引用型对象已具备私有 staging。post-activate 收尾（Creator/Homepage `sync` 删除下线、媒体 prune）发生在 Content pointer 切换之后、consumer verify/UAT 之前；若随后 UAT 失败，fresh rollback 会重新 additive 物化 previous 的引用对象与媒体，代价是恢复时间而不是数据丢失。prune 只允许删除既不属于 current active、也不属于 stage 回执记录的 previous active 闭包的 public slice；首次激活（stage 回执 `previousReleaseId` 为空）才允许只按新闭包回收，previous 身份已知但本机不可得时不删除任何文件。共享 CAS 资产的 release membership 是集合，最近写入的单值只作诊断，不得参与授权；`--require-replay` 轨道同样以 additive 方式补齐存量 active release 的 membership。research 验收对每个 avatar 与每个首屏图片资产各做一次原图短签取回，配额按 viewer × asset 计，不再只采样单个资产。
- 候选保留与激活收尾：`posts_candidate` 只服务于一次激活，`activate` 事务提交后按精确候选身份删除；清理失败只记录诊断，残留候选不被任何读面消费，且同身份再次 stage 会覆盖。`activate` 不提供 dry-run：Go 侧 `--dry-run` 只证明 immutable 输入可加载，不能替代 verified evidence 与 pointer CAS 的前置校验。pointer 切换之后的收尾任一步失败都必须写出带 `revision/candidateRevision/failedStage` 的 failed result（`DATA.DELIVERY.POST_ACTIVATE_FAILED`），重跑 `activate` 会命中 `CANDIDATE_ALREADY_ACTIVE`，此时只能走 consumer verify 触发的 fresh rollback 或显式 `--require-replay` 修复轨。consumer verify 的 restore 只在 verifier 真实失败并写出 failed result 后触发，参数错误与证据缺失不得引发回滚。
- scale bound：单次 activate 的 transaction 目标规模为 M100 量级，候选 copy、tombstone、outbox 与 pointer CAS 必须在现有 Mongo transaction 预算内完成。超过受测事务预算时 fail closed 并由 OPEN-021 承接分批代际方案；在取得证据前不引入 generation-keyed Post identity、跨库两阶段提交、Search/Recommendation 候选索引或环境蓝绿。
- 失败恢复：activate 前失败只清理失败候选，不触碰 previous active。activate 后任一 required UAT 失败以 immutable previous release 发起 fresh `import -> verify -> activate`，随后对 feed、search、recommendation、direct/object route 做 release identity readback；旧 readiness 只提供目标 identity，不可代替 fresh verification。超过 300 秒、pointer 未恢复或任一入口混合 identity 时终态为 `rollback_failed`。
- 可测试面：Mongo integration 证明 stage 后 active pointer/posts/outbox bytes 不变，未 verified 无法 activate，activate 单事务成功，stale revision 失败且 previous active 不变；Data local/api integration 证明 apply/verify/activate 顺序、引用对象先加后减、媒体 prune 窗口及 fresh rollback 四入口同 identity。
- 被否决方案：代际键重写（会波及 comment/interaction 的 stable Post 引用）、跨库两阶段提交、环境蓝绿、Search/Recommendation 候选索引，以及把 public active API 对 candidate 的失败误称为 pre-activation verification。
- 关联要求：[`multi-carrier-release`](./multi-carrier-release/spec.md) 的 `REQ-015`
- 影响 Story：[`multi-carrier-release`](./multi-carrier-release/spec.md) 的 Content import、activation 与 rollback 面
- 关联验收：[`multi-carrier-release`](./multi-carrier-release/spec.md) 的 `GWT-016`、`GWT-031`
- 关联 OPEN：[`multi-carrier-release`](./multi-carrier-release/spec.md) 的 `OPEN-018`（四入口 release identity 读回）、`OPEN-021`（事务规模）、`OPEN-022`（引用型对象私有 staging）
