...
<a id="dec-041"></a>
### DEC-041 默认单一发布消费链、对象级分类维退役、权利事实与环境配置分离

- 决策：producer 与 Data/Service/App/Ops 消费默认只有一条链，不拥有发布类别、运行类别或命名消费轨道。删除 `releaseClass`、`productLifecycleState` 以及以 `readinessPhase` 为载体的类别选择，连同 CLI 参数、enum、默认值、结果和指纹投影一并去除；pool/review/canonical 新 writer 同时删除对象级 `usageScope=research|commercial`、`publicationAdmission=research_release|commercial_release`、`distributionDecision=research_allowed|commercial_allowed|blocked` 与 `variantPurpose=commercial_variant`。不以 `production`、`default` 或其它常量替代；另一套资产用途 `usageScope=internal_reference|app_publish|editorial` 保留且不得扩展为发布类别。具体字段闭集由 Data release schemas、Service contracts 与共享 App launch metadata 拥有，按 authoring source → verify/codegen → 实现和测试推进；跨 owner 实现缺口由 [`multi-carrier-release` OPEN-024](./multi-carrier-release/spec.md#open-024) 承接。
...
- 权利边界：acquire 机械派生逐资产 `rightsStatus`（白名单 license → verified；其它可读 license → unverified 并写 `rightsIssues`；不可读 → unknown）并保留 `commercialAuthorizationStatus`、`authorizationProof`、`license`、水印、`accessPolicy`、`authorizationRequired`、来源与 `rightsIssues`；publish 事务与 release build 只把这些取值当作记录事实写入唯一 manifest 与包内 `sources/<unit>/source.json`，header 只派生计数，pool record 只绑定包与 review 摘要，不复制完整权利投影，不据此拒绝对象。`blocked` 不再作为 `distributionDecision`，受限只由 `rightsStatus=restricted`、`authorizationRequired` 与 typed 权利事实表达。新对象及现役 schema/writer/reader 彻底退役 `distributionDecision`、`publicationAdmission`、对象级 pool `usageScope=research|commercial` 与 `variantPurpose=commercial_variant`，不得以 `production`、`default` 或其它单值替代。资产用途 `usageScope=internal_reference|app_publish|editorial` 是另一语义并继续保留。历史对象/review/receipt/release 原字节不改；仍活跃对象只按 `DEC-023` 的全量显式清单、expected-before CAS 与引用闭包校验生成删除退役字段的递增新版本及新 `payloadDigest`，保留原 review 结论、对象摘要和全部权利事实，不原地篡改、不伪造重审。
- 媒体交付：release 只物化 canonical 公开媒体引用，导入投影按同一媒体契约声明公开访问。当前非商用开发验证阶段，四入口与媒体直链统一开放，不基于缺少授权记录隐藏或拒绝，不新增运营审批/放行开关；权利计数与精确资产事实原样保留。商用前的可见性治理由 [`multi-carrier-release` OPEN-026](./multi-carrier-release/spec.md#open-026) 承接，其唯一 owner 为 `product-ops-growth`（或 owner registry 中唯一等价的下游内容可见性与运营 owner），只读权利事实并结合运营/环境配置裁决，不回写 producer 或恢复双类别。
...
- 失败与恢复：旧类别字段、参数或对象级 enum 不进入现役严格契约，新 reader 遇到即 fail closed；不得通过补值、映射、改旧摘要或降级投影把历史对象/release 冒充新契约，也不得 dual-read。活跃 canonical 只重放 `DEC-023` 的同一显式清单与 exact predecessor，清单漏对象、before version/digest 漂移、退役字段残留、权利或 review binding 不等价均零写并保留旧 active 状态；成功后 producer 再基于新对象版本生成 release/handoff。跨 release、环境、activation/verify 或 lease 的身份错绑仍 fail closed，不改写既有结果。
- 可测试观察面：静态/local_contract 证明新 writer 零退役字段/enum、资产用途三值继续合法、`restricted + authorizationRequired` 表达限制、新 reader 对每个旧 enum fail closed，并拒绝 `production/default` 替代；`DEC-023` fixture 证明 before 历史字节不变、after version 与 `payloadDigest` 递增、退役字段消失且 review/rights 等价。api_integration 对同一 active release 的四入口与公开 GET/HEAD/Range 媒体字节读回，覆盖授权记录缺失但不触发开发期隐藏；环境配置及缓存身份隔离继续验证。编译/安装/启动、runtime health 与双端用户可见结果分别形成新证据，旧命名类别测试不代表当前验收。
...
- 被否决方案：把多类别缩成单一枚举值、把标签改名为 `production/default`、在配置中继续选择内容类别、保留成功别名或 dual-read——均未删除类别维度；将旧 enum 映射为权利事实、保留 `blocked` distributionDecision、原地删字段并刷新旧摘要、为 cutover 伪造重审或由 producer 写回运营可见性——混淆 authority 或破坏 immutable 历史；删除普通认证、原图授权或环境观测——扩大了公开交付裁决。按 `DEC-023` 生成新版本不再是否决项，而是与历史原件隔离的唯一受治理 cutover。
...
<a id="dec-050"></a>
### DEC-050 legacy sealed release 以只读源和双根新 identity 原子重物化

- 决策与对象边界：一次性、受治理的 Data repackage command 把受支持 legacy sealed release membership 作为只读来源，并结合 canonical pool fresh readback 构造不同 `releaseId` 的 current-schema output release、cohort 与 producer handoff。独立 publish 仓 `releases/<newReleaseId>/` 另持有 cohort/handoff terminal copy。旧对象级 `research|commercial` 分类值只允许由 `quwoquan_data/schema/release/legacy_release_cohort_v1.schema.json` 与 `quwoquan_data/schema/release/legacy_content_pool_handoff_query_v1.schema.json` 两个 exact input schema 解析，不得用目录或命名模式扩张。它不是普通 finalize、claim-batch、reader compatibility、migration daemon 或删除工具；普通 reader/finalize/verify/current schema 继续拒绝 legacy schema，candidate 与 rollback/recovery current terminal 均不携带旧分类，历史 terminal bytes 永不改写。字段、schema 闭集、operation 与 typed error 只由 Data canonical authoring source 拥有。
- source repository identity：source input 绑定 source release root、releaseId、handoff/cohort digests 与独立 source repository evidence。legacy wire 缺 repositoryId 时只从与 source terminal 摘要绑定、可独立验真的 evidence 取得 identity，不读 target current repository、不用目录名或默认配置代填。unknown、缺证据或 digest drift 在 current terminal 签发前 fail closed。
- command/query 分流与转换：command 只接受冻结输入与正式 authority。query 只读 source seal/membership，并从 canonical pool fresh readback package、原 content_review 与 publish proof。builder 对 fresh bytes 运行 current canonicalizer，重新生成 artifact entries、counts、object/query bindings 与 treeDigest。新 lineage 绑定 source repository evidence 及 source release/handoff/cohort identities/digests。原 review/proof 仅按 exact ref/digest 引用且 bytes 不动，不制造 reviewer、attestation、execution、receipt、重新审核或重新发布事实。
- terminal location 与跨根一致性：output release 是不可提升的构建产物，不能单独证明 producer terminal。publish repository 的 `releases/<newReleaseId>/` 以 create-or-same 保存与 output exact 相同的 current cohort/handoff。若两根不能共享单事务，协议固定为 output staging/验证/原子 candidate → publish terminal create-once → output/publish 双 readback。只有两者 identities、bytes、digests 与 lineage 全部一致才成功。任一步失败保留首错与可 exact replay 状态，不把半闭合结果签为成功。
- authority 与 task context：允许 migration-only iteration/shard claim，scope exact 覆盖 legacy cohort target refs，但不创建 batch scope、不 claim/rebind legacy 或 existing execution、不要求 batch nonces。调用必须携带真实 user authorization ref、唯一 director/global closer、current generation、output/publish/library/carried 四根 binding 与 exact WriteFenceToken。task 只能由现役 schema/入口绑定真实 confirmationRef、confirmedBy 与唯一动作 repackage。手写 JSON、自报 digest 或环境变量拼装无 authority。
- deployment roster：优先复用真实已授权且完整的现役 deployment。generic register-deployment 若强制无关角色，不得用固定七角色、占位 actor、虚构账号或兼岗补齐。director-only migration deployment 只有在 canonical authoring source、受支持入口、真实 actor 与授权 readback 全部落地后才可使用；任一条件缺失即为 typed engineering gap，不以测试 fixture 代偿。
- 并发与锁：coordination claim 只证明本系统内 scope，不证明 native writer 缺席。执行前在协调 writer gate 内核 current generation/task/root/fence 与跨 iteration target occupancy，并取得/核验现役 release operation lock。DB 为空、无 batch row、无 execution 或无本 iteration claim 均不得推断无其它 native writer。锁序保持协调 gate → SQLite 短只读/CAS 后关闭 → release/publish lock → 文件 staging/publish，禁止反向取锁。任一 native writer、release lock 或 target occupancy 冲突在业务写前 fail closed。
- 原子性与幂等：target create-once。same target、same frozen input、exact same output/publish bytes 为 replay，任一绑定或字节不同为 conflict。fault injection 覆盖 source read、canonicalization、staging、output rename、publish terminal write 与双 readback。失败不得返回 success，source tree digest 与 existing execution actor/generation/receipts/release binding before/after 相同。
- retention、恢复与回滚：source 与任何已成功 output/publish terminal 均 immutable。失败或取消只清理未发布且不在保护集的 staging。成功后不提供普通 delete rollback。若不再采用，复用现有受治理 retirement/保护集；该能力缺失时继续保留，不执行 rm。candidate 与 rollback/recovery 是两个不同且均不同于 source 的新 release identities。恢复只做 same-input exact replay 或用另一新 identity 重建，不删除 candidate 来伪装 rollback。
- 观测与 SLO：只复用现有 command result/audit，报告 source/new identities/digests、source repository evidence ref/digest、output/publish readback、authority/lock result、terminal outcome 与首个 typed blocker。不建 ledger、dual-read、周期扫描或后台队列。正确性 SLO 是成功时双根 terminal exact readback，失败时无可提升半闭合成功、source 与 existing execution 不变。延迟只作诊断，不新增业务承诺。
- 测试 seam：Data local_contract/api_integration 通过 `CLI -> governed_repackage_call -> fenced_governed_repackage` 覆盖 [`GWT-062.t1`](./multi-carrier-release/spec.md#gwt-062) 至 [`GWT-062.t14`](./multi-carrier-release/spec.md#gwt-062)。必须包含 unknown source repository evidence、source/pool/review/proof drift、双根 create-once/readback、fault injection、director/global closer/current/stale generation、四根/task/fence、真实 roster、unset `QWQ_CONTENT_BATCH_NONCES` 成功、existing execution 未 rebind、跨 iteration occupancy/native writer/release lock conflict 与 retention/recovery。fixture 或直接调用内核不替代正式入口证据。
- 被否决方案：普通 reader 接受旧 schema、原地补历史 handoff、以 target repository 代填 source identity、复制旧摘要、output 单根冒充 terminal、伪造 reviewer/execution/receipts、用空 DB 推断无 writer、伪造七角色、手写 task JSON、长期 dual-read/ledger/daemon、成功后 rm 或覆盖同一 candidate identity。
- 关联要求：[`multi-carrier-release REQ-030`](./multi-carrier-release/spec.md#req-030)
- 影响 Story：[`multi-carrier-release`](./multi-carrier-release/spec.md)
- 关联验收：[`GWT-062`](./multi-carrier-release/spec.md#gwt-062)。实现与真实双 handoff 缺口由 [`OPEN-020`](./multi-carrier-release/spec.md#open-020) 持续阻断。
...
<a id="dec-051"></a>
### DEC-051 immutable target descriptor 单写过程定位到 canonical 身份与版本映射

- 决策与 owner：task init 为每个稳定 process `targetRef` create-once 写 immutable target descriptor；descriptor 是 `process ref -> canonical logical entityRef/entityId -> independent contentVersion` 映射及其 canonical digest 的唯一 authority。process ref 只定位 execution 物理证据，不是对象 identity 或 version；目录末级序号只承担 locator/publish sequence，不进入版本语义。初始 canonical 对象固定从 content version 1 开始；homepage correction 必须对 current descriptor/manifest/pool record 做 expected-current CAS，并只创建 `expectedCurrentVersion + 1` 的新 descriptor/package/review/record。article/image/video 的 post `publishSeq` 仅表示 append/publish 顺序，独立 `contentVersion` 才表示同一逻辑作品 revision。字段、canonical bytes、摘要算法与 typed error 闭集由 Data schema/contract owner metadata-first 定义，本设计不复制 wire。
- command/query 与单一读取路径：init command 以 identity-only candidate 和冻结 target 身份创建 descriptor；seal query 仍按 process ref 找到 `1.download`、draft、review input 与 receipts 的物理 bytes，再以 descriptor 的 canonical target 写 `content_review.objectRef`、object revision 与 disposition。publish/revision/replay/handoff 都调用同一个 descriptor reader/canonicalizer，禁止各自实现等价映射、从 publish tree/locator/末级 seq 猜版本、接受 reviewer 自报版本为 authority，或在 handoff 中按路径、最近对象、默认 owner fallback。current reader/writer 对 descriptor 缺失、重复、歧义或 unsupported schema typed fail closed，不 alias legacy process ref，也不 dual-read/dual-write。
- revision tuple 与四方 CAS：review 的完整 revision tuple 必须与 descriptor 严格一致，不能只比较 `contentRevision`；descriptor `contentVersion`、canonical manifest `version`、append-only pool record `contentVersion` 与 review object revision 四方在同一 single-object transaction 内做 expected-before CAS。source unit、candidate、package identity 分别绑定同一 descriptor mapping digest；任一 target/entity/version/digest 漂移、latest-by-dimension 拼接或重复 mapping 都在 review/disposition、canonical package 或 pool record 可见前失败。source/package mapping digest 不能由调用方自报替换。
- seal 与 receipt chain：seal 只用 process ref 定位 physical evidence，并以 canonical target identity/version 形成 review/disposition；receipt chain 同时验证 physical ref/digest 与 payload 内 identity/version/mapping digest，二者缺一不可且不得 alias。`001-1.download`、`002-4.draft`、`003-5.review` 的历史 sealed bytes、既有 descriptor、review、manifest、pool record 与 release/handoff 均保持原字节；新 revision 只追加新的 versioned facts，不补写或重摘要旧事实。
- 并发、TOCTOU 与幂等：init、seal、publish、homepage correction、exact replay 与 handoff 在进入长期文件操作前冻结 descriptor/ref/digest 与 expected-current，写入前在共同 canonical 写锁内重新读回 descriptor、current manifest/pool head、review 与 package identities。锁内任何变化返回 typed conflict 且零业务写；禁止先查后裸写。同一 process ref、candidate identity 与 mapping canonical bytes 的 init 是 create-or-same replay，任一差异是 conflict；同一 publish/revision 输入和 exact bytes 重放只返回原结果，不增加 version、publishSeq 或 pool record。事务以 staging 全量校验后原子暴露 package+record；故障注入任一点都不得出现 descriptor/review/manifest/record 部分推进。
- typed failures、恢复与回滚：至少区分 descriptor missing/ambiguous/duplicate、mapping digest drift、physical evidence drift、payload identity drift、revision tuple mismatch、expected-current conflict、package/source binding drift、receipt-chain mismatch 与 handoff owner unresolved；保留首个最深层 blocker 及 exact refs，不把 rejected 内容与 identity/integrity failure 混为一类。恢复只允许 same-input exact replay，或在重新读取 current 后以新 execution/process ref 和 expected-current+1 创建新 revision；不得修改旧 seal、降级猜测 owner/version、补 process `/1`，也不得删除新版本伪装回滚。业务回滚只选择一个已封存且完整一致的旧 canonical version 作为新的显式 current selection（若原 owner contract 支持），不倒改历史对象或 pool append facts；该跨服务 active selection/rollback wire 若尚未由 runtime owner 定义，必须作为 scope 外 blocker，不在 Data descriptor 中发明。
- SLI/SLO、观测与告警：只在现有 command result/audit/metric 边界记录 operation、process ref 摘要、canonical target 摘要、contentVersion、mapping digest、expected/current version、physical/payload 验证结果、终态、首错 code 与 latency，不记录内容 bytes、secret 或新可写 ledger。正确性 SLO 为成功事务 `100%` 四方 identity/version/digest exact readback 且失败事务 `100%` 零部分写；幂等 SLO 为 same-input replay `100%` 不增版本/seq/record。任何 mapping/receipt/payload identity mismatch、部分写 readback 或 rollback selection 绑定漂移立即告警；延迟仅作诊断，未在冻结 spec 中承诺新的时延目标。
- 真实测试 seam：Data local_contract 通过同一正式 reader/canonicalizer 覆盖 descriptor create-once、版本 1 初始化、process ref 不加 `/1`、publishSeq/locator 与 contentVersion 正交、四方 CAS、receipt physical+payload 双校验、source/package mapping digest、TOCTOU/fault injection、typed failures、零部分写与 exact replay；api_integration 必须走正式 `task-init -> acquire seal -> author seal -> independent review seal -> publish` 产出 v1，再对 homepage 以 expected-current 创建 v2 correction，并以正式入口验证 exact replay 与 immutable handoff。负例覆盖 mapping/digest/version/revision tuple 漂移、并发双修正只有一个 winner、reviewer 自报版本、publish tree 猜版本、handoff owner fallback 与历史 sealed bytes before/after digest 不变；fixture 直调内核或静态字段断言不替代该贯通证据。
- 理由与被否决方案：显式 descriptor 把过程定位、业务身份与内容版本解耦，并让所有边界消费同一可摘要 authority。否决给 process ref 补 `/1`、从 publish tree/locator/末级 seq 隐式猜版本、把 reviewer 自报版本当 authority、handoff fallback 猜 owner、各阶段复制 mapping、兼容 alias、fallback 或 dual-read；这些方案会使同一对象出现多份身份/版本真相源，无法闭合并发 CAS、exact replay 与历史不可变性。
- 关联要求：[`multi-carrier-release REQ-001`](./multi-carrier-release/spec.md#req-001)
- 影响 Story：[`multi-carrier-release`](./multi-carrier-release/spec.md) 的 task-init、seal、publish、homepage correction、replay 与 producer handoff；跨服务 active selection/rollback 仍归 runtime contract owner。
- 关联验收：[`GWT-020`](./multi-carrier-release/spec.md#gwt-020)、[`GWT-023`](./multi-carrier-release/spec.md#gwt-023)、[`GWT-024`](./multi-carrier-release/spec.md#gwt-024)。
