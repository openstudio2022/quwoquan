...
作为内容运营者，我希望 invalid canonical identity（如 payload digest 漂移）被呈现为互斥、可裁决的对象治理状态，并通过显式人工命令修复或退役；该治理不得成为内容十阶段的自动 recovery 或 scheduler。
...
- WHEN 运营者执行 pool inspection、backfill planning 与 release/publish readback。
