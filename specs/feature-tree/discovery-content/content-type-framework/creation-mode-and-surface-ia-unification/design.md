...
当前草稿按 `moment/photo/video/article` 存储。第一版需要迁移到统一结构。

迁移策略：

- 读取旧草稿时：
  - `moment` -> 尝试映射到文字为主编辑器；若存在视频则映射到媒体编辑器视频态，若存在多图则映射到媒体编辑器图片态
  - `photo` -> 媒体编辑器图片态
  - `video` -> 媒体编辑器视频态
  - `article` -> 文字为主编辑器
- 写入新草稿时统一使用 `draftVersion=v2`

要求：

- 旧草稿可读
- 新草稿只写新结构
- 灰度期间允许读旧写新，不允许同时继续扩散旧结构
...
- 旧 `moment/photo/video/article` 草稿读取适配到 `v2`
- 新草稿统一写入 `draftVersion=v2`
