...
<a id="req-007"></a>
### REQ-007 建立三类 Post（图片/视频/文章）统一创作与发布契约，补齐端云发布链路

- 建立三类 Post（图片/视频/文章）统一创作与发布契约，补齐端云发布链路。
- 支持作者在发布后变更圈子分发关系（追加/移除），并通过 canonical 设置命令变更可见性与助手使用策略；正文、类型和媒体本体不可修改（仅允许删除），圈子分发不并入 Post 设置写入。
...
<a id="req-010"></a>
### REQ-010 该能力需被图片、视频、文章三类 Post 发布流程统一复用

- 该能力需被图片、视频、文章三类 Post 发布流程统一复用。
...
<a id="req-013"></a>
### REQ-013 发布后设置只由作者合法变更并可回读

- 作者可独立或同时变更 `visibility` 与 `assistantUsePolicy`，合法值、允许状态和幂等/并发约束只由 Post canonical contracts 定义；未提交的设置保持原值。
- 非作者、未认证或非法值请求不能改变 Post、设置版本或成功回执；失败不得对外报告成功。允许变更必须持久化并由权威读取确认，不能仅回显请求。
- 设置变更不得改写已发布正文、内容类型或媒体身份；公开资格与助手消费按同一权威设置生效，不绕过现有审核/安全状态，不隐式变更 CirclePostPlacement。

## 4. 契约引用

- canonical：`quwoquan_app/lib/service/content_service/media/media_upload_session/presentation/create_media_picker_page.dart`
- canonical：`quwoquan_app/lib/service/content_service/media/media_upload_session/application/public/media_picker_port.dart`
...
- canonical：`quwoquan_app/lib/service/content_service/content/post/application/content_repository_contract.dart`
- canonical：`quwoquan_app/lib/service/content_service/content/post/adapters/create_page_remote_helpers.dart`
- canonical：`quwoquan_app/lib/service/content_service/media/media_upload_session/presentation/video_editor_page.dart`
...
<a id="gwt-009"></a>
### GWT-009 作者变更发布后设置、鉴权与持久化回读

- GIVEN 图片、视频与文章 Post 已发布，存在合法作者和非作者身份，且 canonical 契约明确允许的可见性/助手使用策略及当前版本。
- WHEN 作者分别单独和同时修改两项设置、重复提交同一幂等请求，再由新读取会话回读；另分别提交未认证、非作者、非法值与过期版本请求。
- THEN 合法变更持久化后由权威读取返回相同 `visibility` 与 `assistantUsePolicy`；未提交的设置保持原值，重复同一幂等请求不产生额外变更或重复成功事实。
- AND 未认证、非作者、非法值、过期版本或同一幂等键不同载荷请求返回各自 canonical 失败，既有设置、对象版本与成功回执不被改变。
- AND 正文、类型、媒体身份与圈子分发关系不变；公开读取和助手消费使用更新后的权威设置，收紧后不得继续从旧缓存放行，放宽也不得绕过审核或已终止的安全状态。
- AND 持久化失败不返回成功设置；App 保留可操作的原状态并按 canonical recovery 恢复，重试成功后再次回读同一 Post，不以本地乐观值冒充确认。
...
<a id="open-009"></a>
### OPEN-009 发布后可见性与助手使用策略的真实设置证据

- 类型：`capability_gap`
- 优先级：`P0`
- 准出影响：`block`
- 影响或价值：尚缺发布后三类 Post 的合法设置变更、越权/非法值拒绝、幂等并发、持久化及公开/助手消费回读的完整证据；举报审核证据不能证明作者设置命令。
- 完成判定：`GWT-009` 各结果由真实 local_contract、带持久化的 api_integration 与 App 交互测试分层直接绑定；readiness 只引用实际覆盖子句，不把服务成功回显当作完整用户验收。
