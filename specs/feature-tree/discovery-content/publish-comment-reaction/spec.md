...
| 子特性 (L3) | 说明 | 状态 |
|-------------|------|------|
| **comment-thread** | 商用级评论系统：2 级层级、平铺文章内联评论、沉浸式上压评论、默认/最新/最多赞排序、评论赞踩、图片与 @ 输入、个人主页评论管理、先发后审、Persona 身份、10 万+容量承载、端云配置统一 | V2 PRD 已基线化 |
...
### 评论端云一体化（V2 PRD 2026-06-04）

comment-thread 已从统一底部弹窗扩展为端云一体的完整评论体验，覆盖：

- **核心交互**（F1~F14）：2 级层级、9 入口打通、平铺文章内联评论、沉浸式上压评论、游标分页、默认/最新/最多赞排序、回复分页、评论赞踩、删除审计、长评论折叠、作者标识、相对时间
- **扩展功能**（F15~F22）：先发后审、Persona 身份切换、个人主页"我发出的/收到的评论"、字数限制端云一致、评论通知骨架、图片附件、emoji 与 @ 输入
- **非功能规格**：首屏 P95 < 800ms、提交 P95 < 500ms、10 万+评论容量、乐观更新 + 最终一致、弱网降级、热帖缓存防护
- **配置统一**：业务规则参数（字数限制/回复预览/回复展开/默认排序/附件上限/热帖阈值/缓存 TTL）统一由 config.yaml 管理，端侧通过 App Config 同步
- **灰度发布**：Canary → 1% → 50% → 100% 四阶段，SLO + 回滚条件

详见 `comment-thread/spec.md` 与 `comment-thread/acceptance.yaml`。
