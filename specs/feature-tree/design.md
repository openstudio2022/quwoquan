...
- 决策：`productLifecycleState` 独立于环境名和共享内容工作流；acquisition、semantic、review 与 canonical pool 保持 class-neutral。显式 immutable release selection 冻结 `releaseClass/productLifecycleState`。四环境可承载 Research release，但必须关闭匿名内容/媒体、公开 CDN、分享、导出与索引，并使用白名单身份、内部签名、研究态标识、短期签名 URL 和访问审计。选择 `commercial` 时构建新 release，只投影 `rightsStatus=verified && distributionDecision=commercial_allowed`，不得就地改写或复用 Research release/receipt。
