...
### 方案 B：以 user-service 提供 `SubAccountProfileView`，content-service 继续拥有互动活动读契约，由 App 侧直接编排
...
`SubAccountProfileView` 字段建议至少包括：
...
- `UserProfile` 仍保留 owner 管理视角；`SubAccountProfileView` 才是主页 UI 视角。
...
两条写入链都改为消费 `SubAccountProfileMutation`，新增字段建议：
...
- 读模型 `SubAccountProfileView` 在 user-service 侧完成 owner 基线 + subAccount override 的合成。
...
- user-service 的 `SubAccountProfileView`
...
- 新增 `SubAccountProfileView` 作为读模型，新增 `SubAccountProfileMutation` 作为写模型。
...
- **读路径**：新 UI 优先消费 `GetMeProfile / GetSubAccountProfile -> SubAccountProfileView`。
...
flowchart LR
  ...
  SubjectView[SubAccountProfileView]
