...
- 用户名字可设置并即时刷新
- 身份、作品与影响力加载独立终止，并由页面唯一裁决器合并重复错误
...
<a id="req-003"></a>
### REQ-003 主页关键身份与可选区块独立终止

- 身份是关键首屏；身份尚未成功或失败时，不显示作品和影响力自己的等待或错误。
- 身份、作品和影响力不得由无超时 `Future.wait` 绑定；每个范围使用 3 秒慢提示与 6 秒终态预算，并拒绝旧 generation 回写。
- 身份成功后，一个可选区块失败由区块显示；作品与影响力两个区块失败时由 `AppPageLoadArbiter` 合并为一个页面提示并隐藏重复 CTA。
- 有缓存时保留档案或作品；页面恢复动作只重新请求失败范围，登录成功后重新请求因会话失效而失败的范围。
...
<a id="gwt-002"></a>
### GWT-002 主页只显示一个可恢复状态

- GIVEN 身份、作品和影响力正在独立加载，且其中一个或多个请求失败。
- WHEN 身份尚未终止、仅一个可选区块失败或两个可选区块同时失败。
- THEN 身份未终止时只显示关键首屏状态。单区块失败由区块显示。双区块失败只显示一个页面提示。
- AND 页面不同时出现顶部错误、作品裸转圈和影响力错误；重新加载只重试被裁决为失败的范围。
