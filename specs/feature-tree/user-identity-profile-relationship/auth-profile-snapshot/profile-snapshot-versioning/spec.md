...
- 同一缓存权威任期内，已生效失效必须拒绝更早旧源读取的迟到回填。代际元数据缺失或被驱逐时产生新代际，不复用固定初始值；值、索引、旧代际回收与总容量有上限。
...
- AND 双设备 warm 页、另一端操作、切主体或前台恢复后，同事实读回仍保持当前主体与已确认互动。
...
- 完成判定：[GWT-003](./spec.md#gwt-003)、[GWT-004](./spec.md#gwt-004) 均有直接完整 `spec_ref` 和分层真实结果：`local_contract` 固定源时钟、代际变化、期限、并发屏障，并验证网络/hydrate 时序与源期限继承；`api_integration` 在真实 Redis 同 slot 原子操作及旧代际提升反例验真，并通过真实公开/私有 reader、cache hit 和 authority 证明隔离及失效顺序；`user_acceptance` 以同候选双设备 warm 页、另一端操作、切主体/前台恢复和同事实读回验证呈现。只清空/重启缓存或使用 fake 不证明故障切换边界。User 先冻结缓存资源/源有效性/资料新鲜度与读合同，再由真实 Redis 并发和旧代际提升、公开/私有 reader、App 网络/hydrate 与双设备读回证明。无故障授权、设备或当前来源证据时保持对应准出阻断，不把本地时序测试当作 Redis 原子/HA 通过。
