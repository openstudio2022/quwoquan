...
- 同一缓存权威任期内，已生效失效必须拒绝此前旧源读取的迟到回填。代际元数据缺失或被驱逐时产生新代际，不复用固定初始值；值、索引、旧代际回收与总容量有上限。
...
- 证据层：local_contract 固定源时钟、代际变化、期限与并发屏障；api_integration 在真实 Redis 同 slot 原子操作及旧代际提升反例验真。只清空/重启缓存或使用 fake 不证明故障切换边界。
...
- 证据层：local_contract 验证网络/hydrate 时序与源期限继承；api_integration 通过真实公开/私有 reader、cache hit 和 authority 证明隔离及失效顺序；user_acceptance 以同候选双设备 warm 页、另一端操作、切主体/前台恢复和同事实读回验证呈现。
