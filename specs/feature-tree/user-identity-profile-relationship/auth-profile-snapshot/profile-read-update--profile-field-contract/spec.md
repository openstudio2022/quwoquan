...
- tag-service 新增公共只读层级接口 `GET /tag/children?parentTagRef=...&limit=500`，与地图/POI 同属公共能力，但语义归属标签系统。
...
- User API：`PATCH /user/profile` 使用 `regionTagRef`，不接受客户端提交任意非空 `region`；`ProfileEditSnapshotWire` 返回 `region + regionTagRef`。
...
- api_integration：
  - tag-service 真实 Mongo seed 后调用 `/tag/children` 验证广东/北京样例。
