...
<a id="open-002"></a>
### OPEN-002 refresh token 轮换成功，logout 后旧 token 不可复用

- 类型：`capability_gap`
- 优先级：`P1`
- 准出影响：`track`
- 影响或价值：尚缺实现或直接 `spec_ref`；目标：服务端 contract、App session store 与错误码语义一致，不再存在“token 已失效但客户端仍保留 authenticated”的灰区。
