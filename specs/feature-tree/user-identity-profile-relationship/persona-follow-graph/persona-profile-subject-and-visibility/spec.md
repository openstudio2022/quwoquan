...
- 历史内容、评论、聊天消息、通知保留不可变作者快照。
- 停用不会把历史内容重绑到 owner 或其它分身。
- 停用后的 `ProfileSubject` 是否继续开放公开页，由 user 域可见性策略统一决定；第一版允许公开页关闭，但历史对象仍应使用快照正常渲染。
...
- 停用分身不得继续作为新动作主体，但其历史归因必须可追踪、可渲染、可审计。
