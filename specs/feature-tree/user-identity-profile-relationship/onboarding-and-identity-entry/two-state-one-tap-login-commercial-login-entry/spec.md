...
- 窗口键盘几何由唯一页面骨架边界消费：合法底部 inset 保持原值用于键盘避让；负值或非有限值不得传入布局 Padding，不得关闭键盘避让、改 SDK 或吞掉断言。键盘收起过渡不得产生非法布局约束。
- 正文最小高度须扣除滚动区上下 padding 且不小于零；小屏键盘展开时输入焦点仍可用，正文和恢复动作可滚动到达。
...
<a id="gwt-005"></a>
### GWT-005 验证码键盘过渡与小屏几何

- GIVEN 登录页通过完整 `AppScaffold` 组合路径显示，键盘底部 inset 在展开、零值与负值/非有限值之间变化，或正文剩余高度小于上下 padding。
- WHEN 输入第六位、整串粘贴触发验证，错误返回后恢复输入，或在小屏键盘展开时滚动恢复动作。
- THEN 骨架布局满足 `REQ-003` 的合法几何与键盘避让要求。
- AND 验证码交互满足 `REQ-006/007/013` 的焦点恢复与单次请求要求。
...
- 优先级：`P1`
- 准出影响：`block`
- 影响或价值：尚缺 `GWT-005` 的真实设备键盘过渡与焦点体验证据（注入非法 inset 的 local_contract 不证明设备根因），以及 `GWT-004` 在真实运营商、短信与联合登录 Provider 下的一键、OTP、手机号绑定及失败恢复闭环；现有本地状态与截图证据不能证明 production Remote 登录或 continuation 单次消费。
