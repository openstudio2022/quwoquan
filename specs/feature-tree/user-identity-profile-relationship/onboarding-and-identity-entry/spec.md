...
- 欢迎页是身份入口，不只是品牌 splash；必须知道“未登录 / 已登录 / 登录失效 / 多Persona待选择”四种状态。
...
- 登录成功后不能直接把 `OwnerAccount` 暴露给应用世界；必须进入某个具体 `Persona` 上下文。
- 仅有一个 `Persona` 时，默认自动进入该Persona；拥有多个 `Persona` 时，必须支持安全、显式、可恢复的选择或新建。
...
- 完成判定：生产发布密钥系统注入四类正式 provider 凭据与受控 SDK，`prod-hosted` 真实网络真机完成登录、首登资料同步、失败恢复与回滚验证，使 `SIT-001` 的完整身份进入链路与微信/支付宝/QQ/本机号一键入口子句由正式入口而非降级路径满足，并由 [`four-environment-commercial-login-maturity`](./four-environment-commercial-login-maturity/spec.md#gwt-001) 的直接 `spec_ref` 留证。
