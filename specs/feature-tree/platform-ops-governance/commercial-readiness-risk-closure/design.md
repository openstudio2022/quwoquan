...
- 约束与影响如下。
  ...
  - 旧运行面回收只允许匹配声明前缀且处于 `Created/Exited` 的容器和未使用镜像；专用 service 账号遗留的 Buildah external working container 还必须同时满足声明名称正则、`storage`、`PID=0` 和至少七天年龄才可按 ID 删除。容器存在依赖时对同一受控集合迭代删除，禁止用 `--depend` 或 `--force` 扩大范围。禁止删除任何 volume 或恢复容器。
