...
- 决策：预验证与正式 release transaction 使用物理隔离的运行面和证据链。
  ...
  - GHCR 认证、digest descriptor、不可变 pull 与 push 的瞬时传输失败按相同 context、Dockerfile、build args、tag、SBOM 和 provenance 参数最多尝试三次，并使用 5 秒、15 秒退避；矩阵关闭 `fail-fast` 以保留全部服务结论。三次均失败或成功尝试未返回合法 digest 时仍硬失败，ReleaseManifest 只能引用成功尝试产生的 digest。
