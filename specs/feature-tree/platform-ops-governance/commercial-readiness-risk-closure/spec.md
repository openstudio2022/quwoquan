...
- THEN 受限单机的当前可用空间与可回收空间分别报告，只可删除声明匹配且未运行的旧容器。
- THEN Buildah external working container 必须同时满足 `storage`、`PID=0`、声明名称和最小年龄约束；随后清理未使用镜像，禁止删除 volume，并在任何镜像传输前复验回收后的实际空间。
