...
<a id="open-002"></a>
### OPEN-002 非 HTTP 面对象缺可派生的 SLI 与 SLO 口径

- 类型：`capability_gap`
- 优先级：`P1`
- 准出影响：`track`
- 影响或价值：当前只声明运行时入口而没有 HTTP operation 的对象拿不到 route 维度切片，其运行时入口也没有任何可观测声明，因此这批投影与订阅链路当前完全没有可派生的 SLI 与 SLO 口径，故障只能靠人工发现。
- 关闭方式必须 contracts-first：先为运行时入口新增可观测与 SLO 声明语义，再由生成链路派生口径，禁止在 Ops 侧先手写规则反推契约。
- 建议为投影与订阅类入口定义 checkpoint 滞后、重建重放与死信三类 SLI；这些入口已带 checkpoint、rebuild、tombstone 与幂等字段，是天然挂载点。
- 完成判定：`GWT-001` 对应行为满足且真实测试 `spec_ref` 有效。
