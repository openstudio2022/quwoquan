...
- `stackctl app-dependency-sync` 是 App 依赖 bundle 的唯一显式同步入口。调用方可显式选择 iOS、Android 或双平台；缺省选择双平台，平台执行分支并行，同平台共享资源仍互斥。受管消费者必须传递其实际所需平台，不得隐式扩大到另一平台。
- 一次 attempt 必须完整构建所选平台的依赖闭包：共同的 production Pub、Patrol Pub，以及 iOS 的两份 Pods 或 Android 的 Gradle；双平台需要全部五组件。单平台不得调用另一平台工具链或要求另一平台组件，所选闭包内部不得部分更新或跨 attempt 混合。
...
- 条件：调用方发起同步且所选平台资源可取得，平台计划明确。
- 可观察结果：成功 attempt 产出所选平台完整组件、同 attempt 的不可变快照、先落盘的 sync receipt 与最后原子推进的对应 active pointer，active readback 与本次 attempt 一致。
