...
- 为已删除的旧 Data ReliableTask fleet 恢复启停、端口、role 或生命周期入口；业务 release 激活、App 页面旅程以及 teardown 的具体实现方式。
...
- 本地 workload 与调试依赖的 host 端口都必须引用同一个 `local_env_port_manifest.yaml` 中互不重叠的 role；任何独立控制面不得借用 Alpha、Beta、Gamma 环境 workload 的 MongoDB、Redis 或 Elasticsearch role。
- 本地 runtime teardown 与用于裁定其残留的 receipt 修复路径，必须从目标 canonical receipt、candidate runtime plan 与 canonical 端口清单合成唯一资源所有权结果；端口位于同一 profile 不构成目标 runtime 所有权。
- 旧 Data ReliableTask fleet 已硬删除，不得以端口 owner、独立生命周期、teardown 例外或 receipt 修复排除项回潮；目标 runtime 的 named volume 必须保留。
...
- GIVEN 目标本地 runtime 的 canonical receipt 与 candidate runtime plan 可注入，canonical port manifest 可解析为唯一且互不重叠的所有权结果。
- WHEN 目标 runtime teardown 或 receipt 残留修复通过公开运维入口裁定资源收敛。
- THEN 目标 runtime 自有资源全部收敛，目标 runtime 的 named volume 保持在场，满足其他准入条件后成功终态可回读；旧 Data ReliableTask fleet、端口 owner 或生命周期引用均不存在。
