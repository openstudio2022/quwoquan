...
- `L3_story`: `homepage-review-read-and-score-summary`
...
- 总评分摘要（真实均分 + 评分数 + 亮点标签，由 HomepageReview 事件投影聚合）。
- 口碑列表（keyset 分页，active 过滤）与我的评价（编辑/删除/软删复活预填）。
- 写口碑入口（1-5 星 + 正文 + 亮点标签 sheet）。

## Out of Scope

- 维度分摘要（2026-07-19 B6 裁决砍除：V1 摘要 = 均分 + 计数 + 亮点标签）。
- 认领方回应机制。
- 口碑治理（举报/下架，归 Ops portal）。
