...
1. 把已认领主页当作经营者私有主页，允许直接改写用户口碑、历史内容和评分结果。
...
- 历史主页名称、地址等主档字段直接沿用，不做额外结构迁移；
- 历史用户内容与评分数据不被 maintenance flow 回写。
