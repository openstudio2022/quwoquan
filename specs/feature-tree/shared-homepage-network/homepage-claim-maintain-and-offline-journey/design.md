...
3. 认领后如果不冻结可维护字段，维护者会直接越权触碰用户口碑和记录内容。
4. 下线如果没有正式合同，极易回到硬删除或彻底隐藏，直接破坏记录可信度和内容锚点。
...
| 输入 | 当前结论 |
|---|---|
| `shared-homepage-network/spec.md` | 已冻结候选/待校验/已发布/已下线，认领分层和软下线记录保留合同 |
...
| 4 个 L3 scenario spec / acceptance | 已明确最小实施单元：候选建档、认领审核、认领后维护、下线与记录保留 |
...
| 大众点评 | 已关闭商户保留记录主页和用户评论 | 不吸收纯目录平台治理方式 |

### 内部对标

| 文档 / 能力 | 可复用点 |
|---|---|
| `shared-homepage-network/design.md` | 候选来源、认领分层、软下线与记录保留大方向 |
| `platform-ops-governance` 类设计 | 审核状态、SLA、回滚和观测的治理写法 |
| `content` / `circle` 现有读模型 | 已下线主页继续保留记录内容和相关群组摘要的消费方式 |
...
- 记录导入条目统一回填为 `candidate` 或 `published`，由证据完整度决定
- 已存在的“关闭/停业”数据若可信，可回填为 `offline` 并保留原因标签
- 记录认领数据若不具备材料，不自动升级为 `claimed`

### 双读 / 双写

- **禁止 wire / 状态双读双写**：只认当前 `status` / claim / report 字段形状
- 旧文案状态必须一次性迁出；不得保留「短期双读」兼容窗口
- UI、搜索与内容聚合只读新状态字段；旧键解码与正向兼容测试一律拒绝
...
- 一级回滚：关闭 claim 入口，但保留已认领主页与记录状态展示
- 二级回滚：关闭 offline report 入口，但不得破坏已下线主页记录页
- 不允许回滚到硬删除主页
...
- 支持主页合并，但必须在独立 Journey 中定义记录迁移合同，不能提前侵入本 Journey
