...
- 当前上下文：Circle Gathering 已是活动身份、Participation、准入、会话与 Outcome 的唯一 owner；travel-service 的进程、contracts、generated client、App DI、旧页面/路由/Surface 与 Skill 绑定均已删除，历史源对象仅保留脱敏 crosswalk 和签名 receipt。
- 设计要解决的问题：在已完成 target-only 收敛的基础上，把旅行体验完整装配为 Gathering 上的 Plan、Map、Calendar、Experience 组合；Assistant、Chat、Content 与 Integration 只消费目标 owner 公开事实。
...
- 历史 crosswalk：legacy `TripPlan`、Revision、Membership、Moment、Placement、ShareSnapshot、Template、GuideAssignment 只存在于已签名映射与迁移 receipt 中，生产运行时没有源 Reader、Writer、route 或 fallback。
...
<a id="dec-002"></a>
### DEC-002 travel-service 按逐对象 target-only 流迁移和退役

- 决策：travel-service 的逐对象 target-only 迁移与退役已经完成；legacy 对象通过确定性映射、ID crosswalk、幂等 importer 与 parity receipt 进入 Circle/Chat/Content/Integration 目标，生产调用只读目标。
...
- 约束与影响：签名 receipt 持续证明 count/digest/orphan/collision 与 privacy trimming 结果；任何审计不一致阻断后续目标数据发布，但不得恢复源进程、contract、App 入口或写入。运行回滚只允许上一目标应用/config 或目标数据快照。
- 关联要求：`REQ-003`
...
- 迁移 SLO：parity 必须 100%，orphan/collision 为零。
