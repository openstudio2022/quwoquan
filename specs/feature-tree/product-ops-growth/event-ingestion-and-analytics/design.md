...
- **local_contract**：规格、事件字典、字段分级、SLO 与设计方案文档评审。
- **local_contract**：metadata/schema/contract 测试与样例事件校验。
- **api_integration**：端到云接入、OLAP 可查询、反馈链路可复盘。
- **user_acceptance**：大促/弱网/高频播放/高并发聊天/实验切桶等生产级验证。
