...
- 副本重启会丢失全部窗口状态，因此同一来源可以通过触发或等待副本轮换重新取得完整配额，限流结论不具有跨副本或跨重启的可复现性。
...
- 完成判定：匿名恢复异常入口的来源准入由 api-edge 侧的共享限流状态裁决，多副本下同一来源的实际准入上限与声明上限一致，并有直接绑定该行为的 local_contract 与真实边界收据。
