...
<a id="open-001"></a>
### OPEN-001 circle experience redesign 能力 SIT

- 类型：`capability_gap`
- 优先级：`P1`
- 准出影响：`track`
- 影响或价值：尚缺实现或直接 `spec_ref`。
- 目标：按群组类型提供一致的发现、详情与协作入口。
- 完成判定：`SIT-001` 对应行为满足且真实测试 `spec_ref` 有效

<a id="open-002"></a>
### OPEN-002 圈子主页频道聚合与分页体验

- 类型：`capability_gap`
- 优先级：`P1`
- 准出影响：`track`
- 影响或价值：尚缺实现或直接 `spec_ref`。
- 目标：推荐频道首次进入只读取 recommended 聚合切片；用户显式切换我的频道后才读取 mine 切片，匿名状态保持安全空态和登录续接。
