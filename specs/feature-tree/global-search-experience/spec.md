...
现有 App 端已具备 `chat.* / circle.* / content.post / entity.homepage / integration.location_poi`
等搜索对象，但「真实连接强度、交集原因、已互动状态」并非所有 provider 都已返回稳定字段。
在服务端读模型补齐前，客户端首版实现允许使用可审计的确定性分组规则作为 UI 编排层：

- `chat.contact / chat.conversation / chat.message` 视为已连接。
- `circle.group` 视为已加入/已连接讨论，`circle.circle` 视为圈子发现结果。
- `integration.location_poi` 视为地点发现结果；已关注地点待 user/entity 关注读模型接入。
- `content.post` 按 `contentType` 映射图片/视频/长文；已互动状态待 interaction projection 接入。
- `entity.homepage` 仅作为地点/人等对象卡的兼容来源，不得恢复「主页」为一级用户 Tab。

该过渡规则不得成为第二套搜索 contract；服务端字段补齐后必须继续通过 canonical
`search(request)` 与 typed payload 替换 UI 推断。
