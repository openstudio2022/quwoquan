...
“朋友”已经在 PRD 中被正式更名过，但最新两段式 UX 又把当前入口层的“人”结果进一步收敛成可直接聊天的“联系人”分段。设计阶段必须把这层语义冻结清楚，否则 Search UI 和 metadata 仍会继续挂错域。

## 最新实现基线（2026-03-22）

本 Scenario 以后续实现以下口径为准：

- 当前搜索入口层展示的是“联系人”，不是泛化的“社交关系主页列表”。
- 联系人结果必须带会话绑定信息，点击后直接进入单聊或群聊。
- `messages` 提供可行动联系人 contract，`user` 只补充展示身份信息与后续扩展能力。
