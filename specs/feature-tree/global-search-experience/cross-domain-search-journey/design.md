...
- **全屏 route-driven 搜索页**
- **App `SearchCoordinator` 并发编排四个域搜索**
...
| `global-search-experience/design.md` | 能力级已选定 route-driven + App coordinator + 域级 typed contract |
...
- `/design` 准入满足。
- 本 Journey 的设计顺序固定为：`_shared metadata -> 域级 Search* contract -> codegen -> shell/coordinator -> history/voice/handoff -> tests`。
- G1 已实际执行：
...
### 方案 C：route-driven 壳层 + App `SearchCoordinator` + 域级 typed 搜索接口

核心思路：

- `GlobalSearchPage` 作为统一全屏壳层。
- App `SearchCoordinator` 并发调用域 Repository。
- 每个域补齐自己的 `Search*` contract 与 DTO。
- assistant 仅做 typed handoff，不混排 AI 结果。

优点：

- 与现有仓库结构和 metadata-first 约束最一致。
- 壳层、结果模型、历史、助手 handoff 可以一次性统一。
- 保留未来下沉到聚合接口的空间。
...
2. 它不会让全局搜索被某一个域或某一个后端聚合实现绑死。
3. 它满足助手设计约束：问小趣 handoff 通过 typed contract 完成，不进入 runtime 特判。
...
### KD2：App 使用单一 `SearchCoordinator` 统一搜索会话
...
### KD3：Journey 只冻结域级 `Search*` contract，不直接设计统一 `SearchAll`
...
| `social/circle` | `SearchCircles` | 返回圈子项 + facet buckets |
...
- `circle_facets` 只作为 `circles` 的补强分组或筛选来源
- 不新增 `channel` 主域
- 不新增 `assistant` 结果分组
...
| `_shared/request_context.yaml` | 新增 landing/result page ids 与四域 `Search*` page ids | `*_request_page_ids.g.dart` |
...
| `social/circle/fields.yaml + service.yaml` | 新增 `CircleSearchItemView`、`CircleFacetBucketView`、`SearchCircles` | circle generated DTO/API metadata |
...
2. 四域 `Search*` operation 与搜索 DTO
3. recent search sync contract
4. assistant handoff contract
5. 运行 G1：
   - `make -C quwoquan_service verify-metadata`
   - `make codegen`
   - `make codegen-app`
6. 再进入 App Repository / SearchCoordinator / UI
...
| `J2` | `T1/T2/T3/T4` | 域级 Search* contract、结果分组、局部降级 |
...
| Slice | 目标 | 对应验收 | 主要证据 |
|---|---|---|---|
| `P1` | 冻结 `_shared` 与四域 search metadata | `J1/J2` | `T1_schema` |
...
| `P4` | 落地 `SearchCoordinator` 与结果分组 | `J2` | `T2_module_interaction`, `T3_cross_service_integration` |
...
- 后续可把 `SearchCoordinator` 下沉为统一聚合 `SearchAll`，但不改变 UI / result model contract。
