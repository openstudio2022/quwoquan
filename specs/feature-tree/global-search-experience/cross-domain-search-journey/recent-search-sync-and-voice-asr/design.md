...
最近搜索和语音输入都属于“搜索入口能力”，但它们的生命周期与搜索结果完全不同。设计阶段需要把这两个子问题单独收口，避免后续把 `小趣搜` continuation、聊天语音发送链路或临时本地缓存混进来。

## 最新实现基线（2026-03-22）

本 Scenario 的最新实现基线如下：

- 搜索首页初始态以 `最近在搜` 双列卡片展示记录，并支持展开与记录管理态。
- 用户可删除单条或清空全部，且本地与云端语义保持一致。
- `小趣搜` 不单独新增 AI query 记录模型，只复用当前全局搜索 query。
- 语音仍只承担 ASR 转词，不直接触发 assistant 推理。
