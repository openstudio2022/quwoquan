...
以下设计口径以用户最新确认的 `latest_two_stage_ux` 为准；若下文仍出现“综合结果 / 问小趣快捷入口 / 指定搜索内容”等记录表述，均以上述新基线覆盖：

- 搜索体验是两段式：`初始记录态 -> 输入后实时联想态 -> 独立网络结果页`。
...
| 输入 | 当前结论 |
|---|---|
| `global-search-experience/spec.md` | 已冻结能力边界、领域服务、数据生命周期、NFR 与记录节点清理口径 |
