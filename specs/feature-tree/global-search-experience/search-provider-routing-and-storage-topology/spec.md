...
2. **准确性与可重复性硬门槛**
   ...
   - 测试：`runtime/search/sort_stable_test.go`、`services/search-service/internal/application/*experiments*test.go`、`.qwq_output/local/gamma-local/search_repeatability_golden_diff.json`；真集群新增 api_integration。
...
3. **高并发与弹性 measured 准出**
   ...
   - 测试：`.qwq_output/runs/search-load/**` + 真集群 report；`stackctl verify --env gamma|prod --kind all --tier all`。
...
5. **搜索词热力与推荐闭环运营化**
   ...
   - 测试：`search_signal_consumer_test.go`、`runtime/recommendation/*scorer*test.go`、`.qwq_output/local/gamma-local/search_signal_t3_report.json`、`.qwq_output/runs/search-obs/search_observability_ab_recommendation_report.md`。
...
| api_integration integration | SIT | stackctl gamma：package/up/health/verify + `.qwq_output/local/gamma-local/search_smoke_report.json`（`/v1/search` 200、`/v1/search/feedback` 202） | gamma 真实冒烟已绿；推荐信号真实 Redis 双服务 api_integration 已绿（WP-F，`.qwq_output/local/gamma-local/search_signal_t3_report.json`）；真集群/长稳待 WP-E |
| user_acceptance journey | UAT | 搜索 Journey 端到端（埋点 / 降级 / 弱网 / 权限 / 可重复性） | 🟢 已 recorded：`cross_domain_search_journey_test.dart`（suggest 本地两阶段、result 云侧固定 Tab、本地对象不进 result、最近搜索水合、单域降级不阻塞整页、整页错误态可重试、默认页/结果页 `referralSource=search`+`feedRequestId` 归因链）；高并发负载模型/背压/缓存/可重复性已冻结并有证据（`.qwq_output/runs/search-load/**`、`.qwq_output/local/gamma-local/search_repeatability_golden_diff.json`）；故障/回滚演练 `.qwq_output/runs/gamma/search_rollback_rehearsal.md` |
