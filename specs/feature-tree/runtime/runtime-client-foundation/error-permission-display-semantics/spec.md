...
| `connectionUnavailable` | 服务暂时连不上 | 这次请求没有到达我们的服务，通常是我们这边的问题，请稍后再试。 | 重新加载 |
| `requestTimedOut` | 服务响应超时 | 我们的系统处理太慢，这次请求已停止等待。这是系统问题，不是你的操作导致的。 | 重新加载 |
| `serviceUnavailable` | 系统出了问题 | 我们的服务暂时没能处理这次请求。这是系统问题，不是你的网络或操作导致的，请稍后再试。 | 重新加载 |
...
- AND 离线、远端连接失败、超时、服务不可用、响应不可显示和游客会话失败分别呈现用户可理解且不重复的说明；远端连接失败、超时与服务不可用的标题和说明直接归因为我们的系统问题，页面不显示品牌名称、技术字段或圈子跳转动作。
...
<a id="open-001"></a>
### OPEN-001 整页阻断错误在 App 回前台后不会自动复核，陈旧错误页可停留数小时

- 类型：`capability_gap`
- 优先级：`P2`
- 准出影响：`track`
- 影响或价值：Alpha 实测中，首页与视频书在服务侧故障时进入 `requestTimedOut`/`serviceUnavailable` 整页错误，用户切后台数小时后回前台仍看到同一错误页，而服务早已恢复，只能手动点"重新加载"。`HomeMultiFormFeed` 与 `WorksImmersiveViewer` 的阻断错误只响应显式重试，尚缺对 `AppLifecycleState.resumed` 的自动复核，陈旧错误页因此可无限停留。
- 尚缺实现：在 REQ-012 的 generation-bound 恢复语义内，为整页阻断错误增加"回前台且距上次失败超过阈值时自动发起一次重试"，重试结果仍只能是 `recovered|stillBlocked|superseded|cancelled`，不得重复叠加请求或绕过 `reloadLater` 兜底；需为首页与视频书各绑定 `local_contract`。
- 完成判定：GWT-009 增加"回前台自动复核"子句并由真实测试 `spec_ref` 绑定后关闭。
