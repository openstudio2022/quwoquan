...
| `lib/app/shell/web_main_app_shell.dart` | T1 | ✓ | — | — | ✓ | — | ✓ | ✓ | ✓ | PC Web 独立宽屏壳；顶部短欢迎区复用移动端 `WelcomeFlowerMark` 花瓣动效并居中展示品牌簇，不放登录/下载提示，内容页在欢迎区下方并随滚动推入/拉回，工具栏吸顶后再出现 `趣我圈` 图标/名称；右侧五个一级操作仅显示同尺寸图标并保留语义 label；壳层自身无新增云契约，首页/精品内容区复用 `discoveryFeedMapProvider` + `PostBaseDto` + 多候选媒体 URL 加载；P7 走 `PlatformCapabilities.wideScreenLayout`/`AppSpacing.wideBreakpoint`，P8 走既有 `AppColors`/`AppTypography`/`AppSpacing` token |
