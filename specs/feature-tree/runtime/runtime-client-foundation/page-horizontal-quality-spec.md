...
| **T7** | **壳内子视图**或 **当前未挂路由的遗留页面文件**（须标记后续处理） |
...
| **P3** | **端云一体化** | 涉及云的领域使用 **`Mock*Repository` + `Remote*Repository`** + `appDataSourceModeProvider`（或等价）切换；禁止 UI 直连裸 HTTP | 无网络、纯本地状态页 |
| **P4** | **统一埋点** | 页面级 **`open/close/停留`** 或等价事件进入 **统一观测管道**（`AppLogService` / 后续 Analytics 实现）；或登记 **豁免原因** | 极短 transient 路由页可 **—** 但须在备注说明 |
| **P5** | **组件复用** | 设置表单走 **`SettingsInsetForm*`**；对话态半屏走 **`settings_conversation/`**；禁止重复造同类壳 | 强定制全屏沉浸且已登记模板的页面可 **—** |
...
| **v2** | `scripts/verify_page_horizontal_quality_matrix.py`（路径存在性 + **P1–P8** 符号）+ `scripts/verify_page_matrix_scan_complete.py`（磁盘↔矩阵↔缺口清单 **无漏页**）；已接入 `make gate` 的 app 段；列数扩展时同步改脚本 |
