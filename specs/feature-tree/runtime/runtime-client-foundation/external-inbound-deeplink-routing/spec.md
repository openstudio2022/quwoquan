...
<a id="req-007"></a>
### REQ-007 内容深链服从对象边界与能力终态

- Post 目标仅承接图片、视频、文章三类，实体主页是独立目标而不是第四类 Post；纯文字与富文图文混排文章都使用文章目的面。
- 深链反查后只消费云端权威目的面，不能从附件嗅探类型或把不支持的目的面换成另一个面。
- 客户端不支持内容类型或目的面时，显示需升级的 typed 终态并可返回，不误报删除、不默认展示封面卡；已知目标的真实删除和权限失败仍保持各自 canonical 语义。
- 运行时不复活退役内容身份；历史链接的永久 HTTP 行为只由获批准的 canonical 契约决定，本 Story 不规定新重定向、404 或兼容路由。
...
<a id="gwt-005"></a>
### GWT-005 已知内容直达与未知能力升级终态

- GIVEN 已安装 App 分别收到图片、视频、纯文字文章、富文文章与实体主页链接，以及指向客户端不支持类型或目的面的链接。
- WHEN 冷启动完成路由就绪重放，或热启动解析并反查目标。
- THEN 已知目标按权威目的面直达同一对象，实体主页不进入 Post 媒体浏览；两种文章不因附件差异被改派。
- AND 未知能力显示需升级终态并可返回，不显示默认卡、不误报删除、不改写目的面；真实删除或无权限目标仍显示各自 canonical 失败。
...
<a id="open-006"></a>
### OPEN-006 三类 Post 直达与未知能力终态证据

- 类型：`capability_gap`
- 优先级：`P0`
- 准出影响：`block`
- 影响或价值：尚缺冷启动与热启动的真实端到端证据，未知能力升级终态、返回路径及其与真实删除/权限失败的区分未获行为证明。
- 依赖：需升级终态的错误、恢复动作与文案由 canonical 契约声明；退役链接的永久 HTTP 行为由 canonical owner 批准，未声明前不得新增重定向或兼容入口。
- 完成判定：`GWT-005` 由三类 Post、独立实体主页及未知能力的真实测试绑定，已知失败与需升级终态可区分；无媒体嗅探和目的面替换。

<a id="open-001"></a>
### OPEN-001 已安装设备点击外链直达受支持目标

- 类型：`capability_gap`
- 优先级：`P1`
- 准出影响：`track`
- 影响或价值：尚缺实现或直接 `spec_ref`；目标：Post（文章、图片、视频）、circle、user 与独立实体主页都能直达；退役类型不再是受支持目标。
