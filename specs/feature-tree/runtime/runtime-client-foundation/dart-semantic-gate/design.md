...
硬编码视觉字面量（如 width: 44、leadingSize: 44）在 PRD/design/dev/deliver/gate 全链路未被拦截，根因：
...
采用四层策略：L1 脚本→L2 规则→L3 PRD/Design 前置→L4 元数据（未来）。
