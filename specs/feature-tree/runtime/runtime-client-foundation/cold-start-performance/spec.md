...
- 以下 S0–S3 版本检查与 Web 动作仅适用于已验证配置允许联网且具有受信恢复地址的 Remote 来源；S0 固定显示“应用暂时无法启动／正在检查可用版本／正在检查…／使用网页版”，网页版从首帧起可用。
- Alpha 离线快照发生已确认启动 fatal 时直接显示“应用暂时无法启动／离线版本无法在线恢复，请重新安装或重新构建／查看恢复指引”；指引仅在本地说明重新安装或经 canonical launcher 重新构建，不检查版本、不提供 Web/更新跳转、不重试启动。配置缺席、校验或 activation 失败仍独立进入 REQ-002 配置错误页，不得折叠为离线 fatal。
...
- AND 已验证 Remote 首帧立即提供网页版。有新版且存在当前平台可安装通道时，Android 经官网 HTTPS 端点下载正式签名 APK，公众 iOS 继续使用 PWA/网页版。已最新、没有合规原生通道或检查未完成时提供网页版且不存在启动重试；可见截止时间独立于请求回调，配置/URL 早退不得遗留检查中状态。Alpha 离线 fatal 直接进入本地指引终态，无 Web/更新动作且不联网；配置失败独立阻断，不创建 fatal marker。
