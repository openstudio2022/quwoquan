...
| 外部命令                                                              | 阶段        | 内部命令 / 脚本                                                                                                                                                                                                                                |
| ----------------------------------------------------------------- | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `python3 quwoquan_data/scripts/cli.py data explore ...`             | 数据规格探索阶段  | 规格收敛与可选探测脚本                                                                                                                                                                                                                              |
| `python3 quwoquan_data/scripts/cli.py data baseline ...`            | 数据规格基线阶段  | 文档 / 配置 / schema 存在性；可选 `--geo-band-rules`，与 `--catalog-config` 同传时校验与 `geo_band_rules_path` 解析一致                                                                                                                                           |
| `python3 quwoquan_data/scripts/cli.py data build-entities-tags ...` | 生成实体和标签阶段 | `build_geo_poi_catalog`、`merge_overpass_poi_catalog`、`semantic_entity_resolution`、`crawl tag-catalog-build`、`crawl entity-catalog-build`                                                                                               |
| `python3 quwoquan_data/scripts/cli.py data download ...`            | 下载与来源发现阶段 | `crawl instruction-build`、`crawl entities-by-tag`、`crawl spec-build`、`crawl authority-sync`、`crawl authority-review`、`crawl pool-bootstrap`、`crawl spec-discovery`、`crawl fetch-source`、`crawl content-discover`、`crawl content-hydrate` |
| `python3 quwoquan_data/scripts/cli.py data normalize-prepare-topic-inputs ...` | 编程助手抽取输入准备 | 将已 hydrate 的 topic 页面镜像为 normalization bundle，并批量生成 extract input，供编程助手消费 |
| `python3 quwoquan_data/scripts/cli.py data normalize-run-extract ...` | extract 阶段门禁 | 检查 extract result 是否已由编程助手写回；缺失时生成 assistant task manifest，存在时立即校验 schema |
| `python3 quwoquan_data/scripts/cli.py data normalize-run-review ...` | review 阶段门禁 | 检查 review result 是否已由编程助手写回；缺失时生成 assistant task manifest，存在时立即校验 schema |
| `python3 quwoquan_data/scripts/cli.py data normalize-run-authority ...` | authority 阶段门禁 | 检查 authority result 是否已由编程助手写回；缺失时生成 assistant task manifest，存在时立即校验 schema |
...
### 3.5 `task execute`（任务产线）与 `content_plan`
