...
对外暴露 `python3 quwoquan_data/tools/cli.py data ...`，内部调用现有 `crawl` handlers。
...
- **观景台**：优先与 OSM `tourism=viewpoint` 及同类目录过滤对齐。  
- **打卡地 / 小众秘境**：百科缺失概率高，默认走 **post_evidence_track**；若后续补齐百科/维基则 **可升级为 authority_track** 并回写主名。  
- **未开发区域**：实体与图文均须遵守 **风险提示与合规展示**（不由数据层代替法务/产品结论）；数据中打标便于下游过滤。

## 图文规模与精品子集

- **目标**：每实体 **20～100** 篇可 hydrate 的攻略/游记/日记/摄影向图文（合规前提下上限尽量用尽）。  
- **精品约 30%**：在 `process-content` 后打 `contentTier: curated` 或等价字段；可取 **规则分 top 30%**（长度、图片数、权威来源引用、去重后信息增益）与 **人工抽检** 组合，比例按实体桶统计，在 acceptance 中写明允许误差（如 ±5% 或按批次文档化）。
