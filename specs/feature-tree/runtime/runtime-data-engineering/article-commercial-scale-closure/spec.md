...
- article 必须从 confirmed demand 绑定的 canonical entity catalog 独立选择 target；不得依赖旧 campaign、homepage execution 或已发布主页批次。
...
- THEN review 中断不改写已有 reviewer result/attestation 或 stage receipt；OPEN 无 CLOSE 按同一冻结输入重做，CLOSE blocked 后新 `retryOf` 显式消费 exact failed refs 与 typed feedback，通过对象不进入新 execution，且不存在 controller/campaign/reconciliation 自动派生路径。
