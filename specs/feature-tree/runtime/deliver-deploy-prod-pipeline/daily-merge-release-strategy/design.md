...
旧方案依赖每日定时把 `dev1.0` 自动 merge 到 `main`，存在等待不可控、重复重跑和职责分散的问题。现改为：用户显式发起 PR，所有进入 `main` 的代码统一经过 merge queue 阻断。

## 适用场景与约束

- **适用**：分支开发与 trunk development 并存的主干治理
- **约束**：需要 GitHub merge queue / branch protection；self-hosted runner 需在线且可见设备至少一台
...
| **B：显式 PR + merge queue** | 用户创建 PR，GitHub merge queue 跑 required checks | 路径清晰、阻断可见、与 trunk/dev1.0 模式统一 | 需配置 required checks 与 queue | ✓ 选定 |
| **C：人工本地 merge 再 push main** | 本地 merge 后直推 | 简单 | 绕过主干阻断，风险高 | 不选 |

**选型**：B — 统一采用显式 PR + merge queue，不再保留任何定时自动 merge。
...
| **B：重验证前移到 merge queue** | `03/04/05` 只在 merge queue / 手动执行 | 先验阻断、避免 main 后重跑 | merge 前成本上升 | ✓ 选定 |

**选型**：B — `03/04/05` 前移到 merge queue；`02/07` 保留 main 后动作；`06/08` 为手动。
...
- `main`：禁止 direct push，仅允许通过 PR + merge queue
- `dev1.0`：是否要求 PR 由团队决定，但不得再依赖定时自动推进到 `main`

## 未来演进

- **审批门控**：merge queue 前可补充 approve / CODEOWNERS
