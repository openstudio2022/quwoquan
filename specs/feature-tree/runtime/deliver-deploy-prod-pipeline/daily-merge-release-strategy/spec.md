...
- Alpha/Beta `EnvironmentAcceptanceFact` 的签发位置是产出 Data release handoff 的 lane 工作树（`make accept`，当前分支为该 lane，`--mode acceptance` 终态 `accepted`，不 admit、不写 `dev1.0`）；integration 工作区（分支 `dev1.0`）只以这些事实为 predecessor 做 admit → publish，并独占 Gamma 与 prod（含 rehearsal）。lane 已裸 fast-forward 落地后仍可对同一 head 验收，须显式 `--baseline <上一个已验收基线>`，否则 `INTEGRATION_RUN.NOTHING_TO_ACCEPT`；不得为在 `dev1.0` 分支上跑通 ship admission 而放宽 `validate_current` 或复制他方 lane 的证据字节（见 [L2 DEC-014](../design.md#dec-014)）。
...
- 影响或价值：`make integrate` 已真实跑到 `alpha.data-release`：`qwq-data ship apply` 以 `release_header … source identity set requires a pool selection` 拒绝硬切前产出的 intersection-flywheel immutable release；硬切前本机全部 immutable release（product-mainline 与 data-engineering 产出）的 `selectionScope` 都是 `all_publishable`，无一满足 dev1.0 当前 `content.release.canonical.release_header` 合同。health 的 `release_active` 层依赖该导入回执，因此 Alpha 不能在没有合同兼容 release 的情况下签发 passed 事实；不得放宽合同或伪造回执。现状：product-mainline 已从仓内 tracked `quwoquan_data/publish` 池以 `qwq-data release pool-build`（`selectionScope=explicit_cohort`）重建一对 research release（candidate 与 rollback 目标各一份，`release_header` 与 `verify release-integrity` 均通过），但尚未经 `make integrate` 在 Alpha 上真实 apply/verify；硬切删除了 `release baseline` 入口，dev1.0 当前没有 empty_baseline 的 writer，rollback 目标只能是另一份 content release。
- 完成判定：`GWT-001.t3` 以 DEC-041 单一 production 类别下产出的一对（candidate、rollback）immutable release 为输入——`RELEASE_ATTESTATION`/`ROLLBACK_RELEASE_ATTESTATION`（`releaseClass=productLifecycleState=production`，供 stackctl package 候选绑定）与 candidate release 的 `RELEASE_HANDOFF_REF`（authoritative handoff-ref-v1，现役 `qwq-data ship apply → activate → verify --readiness-phase production` 的唯一准入身份）——在产出该 handoff 的 lane 工作树以 `make accept`（`--mode acceptance`，见 [L2 DEC-014](../design.md#dec-014)）对 exact candidate 完整产出 Alpha（及条件 Beta）`EnvironmentAcceptanceFact`，summary 各分段耗时齐全、终态 `accepted`；research/commercial attestation 一律以 `INTEGRATION_RUN.INPUT_INVALID` 拒绝，不得以 release id 隐式选择 release。
...
<a id="open-008"></a>
### OPEN-008 integration 工作区尚不能以 lane 侧已签发的 Alpha/Beta 事实为 predecessor 做 admit

- 类型：`capability_gap`
- 优先级：`P1`
- 准出影响：`track`
- 影响或价值：[L2 DEC-014](../design.md#dec-014) 把 Alpha/Beta 签发放到 lane 工作树（`make accept`），但 `integration_run.py --mode integrate` 仍自己重跑环境并在同一进程内 admit；它还没有「传入既有 `alphaFactRef`/`betaFactRef`、校验其 candidate commit/tree 与 ImpactPlan digest 等于本次 candidate 后直接 admit → publish」的输入面。于是 lane 已验收的 candidate 到 integration 工作区仍会再跑一遍环境，而该工作区在 `dev1.0` 分支上又无法通过 ship handoff admission（`CANDIDATE.OWNER_DRIFT`），带资格的发布链事实上断在两端之间；当前 dev1.0 的合入只能走裸 fast-forward push，不携带资格。
- 目标：`integration_run.py --mode integrate` 增加 `--alpha-fact-ref/--beta-fact-ref`：存在时跳过 `_run_environment`，重放 `environment_scheduler` 对事实的 exact 校验（candidate、impactPlanDigest、signer、有效期、predecessor 链），再进入 admit → publish；缺失或漂移 fail closed。`make integrate` 相应暴露 `ALPHA_FACT_REF`/`BETA_FACT_REF`。
- 完成判定：`GWT-001.t3` 与 `GWT-001.t4`——lane 工作树 `make accept` 产出的两份事实被 integration 工作区 `make integrate PUBLISH=1` 消费后 admit 并 fast-forward 发布、读回 `after`，publish result 的 predecessor 指向这两份事实；local contract 覆盖事实 candidate/plan digest 漂移、过期与缺失 predecessor 的 typed 拒绝。
- 依赖：[L2 DEC-014](../design.md#dec-014)；`quwoquan_ops/ci/scoped_candidate/core.py#create_publish_admission` 对外部事实 ref 的接受面。
