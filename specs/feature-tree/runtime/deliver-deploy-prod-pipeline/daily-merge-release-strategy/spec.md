...
- 集成深度仍唯一由 `quwoquan_ops/ci/impact_planner_core.py` 的 `derive_integration_depth` 派生（`data|topology` → `abg_release_sensitive`；`app|service|portal` → `alpha_integration`；五 scope 全空 → `no_live`，不启动环境），但它只决定 Beta `not_required` 的原因码，不再强制 Beta 真跑：Alpha 始终真跑；Beta 只在显式 `--beta` 时真跑，否则签 `status=not_required`，`reasonCode` 为闭集之一——ImpactPlan 无需 live Beta 时 `IMPACT_PLAN.NO_LIVE_ENVIRONMENT_REQUIRED`，ImpactPlan 判定 `abg_release_sensitive` 但未 opt-in 时 `ACCEPTANCE.BETA_OPTIONAL_BY_POLICY`；两种原因码在签发（`environment_scheduler`）、schema（`environment_acceptance_fact.schema.json`）与 admission（`create_publish_admission`）三处同一闭集，其余原因码 fail closed，Alpha/Gamma 不得 `not_required`。`integrationEligibility` 是本地 readiness 正交维度（producer=`trusted_integration_publisher`，状态 `not_evaluated|eligible|blocked`），只在 exact candidate 的 publish admission 绑定 passed source fact 与 Alpha/Beta 事实之后由 publisher/integration FF 通道写成 `eligible`；裸直推、L0/L1/L2 source readiness 与 Environment Ops 的 `environmentReadiness` 都不得推导该维度。
...
- AND lane 工作树 `make accept` 以 lane head 为 exact candidate（scope 等于相对 baseline 的全部 changed paths，baseline 非祖先时拒绝），Alpha 真跑、Beta 只在 `--beta` 时真跑，否则以闭集原因码（`IMPACT_PLAN.NO_LIVE_ENVIRONMENT_REQUIRED` | `ACCEPTANCE.BETA_OPTIONAL_BY_POLICY`）签 typed `not_required`；终态 `accepted` 产出 acceptance bundle，不 admit、不写 `dev1.0`；用户显式合并多 lane 时 `--merged-lanes` 中每个 lane 都必须是 candidate 祖先。
...
- 完成判定：`GWT-001.t6..t7` 以 DEC-041 单一 production 类别下产出的一对（candidate、rollback）immutable release 为输入——`RELEASE_ATTESTATION`/`ROLLBACK_RELEASE_ATTESTATION`（`releaseClass=productLifecycleState=production`，供 stackctl package 候选绑定）与 candidate release 的 `RELEASE_HANDOFF_REF`（authoritative handoff-ref-v1，现役 `qwq-data ship apply → activate → verify --readiness-phase production` 的唯一准入身份）——在产出该 handoff 的 lane 工作树以 `make accept`（`--mode acceptance`，见 [L2 DEC-014](../design.md#dec-014)）对 exact candidate 真实产出 Alpha `EnvironmentAcceptanceFact`（Beta 缺省 typed `not_required`，`BETA=1` 时真跑），summary 各分段耗时齐全、终态 `accepted` 并写出 acceptance bundle；随后 `GWT-001.t9..t12` 由 integration 工作区 `make integrate ACCEPTANCE_BUNDLE=… PUBLISH=1` 消费该 bundle 发布并读回 `after`。research/commercial attestation 一律以 `INTEGRATION_RUN.INPUT_INVALID` 拒绝，不得以 release id 隐式选择 release。
