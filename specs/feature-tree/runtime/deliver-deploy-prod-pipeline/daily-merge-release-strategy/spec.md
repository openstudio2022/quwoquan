...
- 完成判定：`GWT-002.t1` 的 ruleset 事实由 `ruleset_fact` 以 `bypassActorsObservable` 显式区分"观测为空"与"不可见"，不可见时不输出 `bypassActors: []`，`requiredCheckEnforced` 只声明 required_status_checks 形状；main ruleset 的 bypass 为空由 admin 侧 `--require-bypass-observable` 读回或等价 write 权限读回签发，并在 `test_promotion_hosted` 以缺席/null/非空三态锁定；`GWT-002` 的 required context 在只读 token 下不再写出未观测到的值；`CI_CD_SECRETS.md` 同步措辞。
