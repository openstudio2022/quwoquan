...
- 完成判定：`GWT-001.t3` 以 DEC-041 单一 production 类别下产出的一对（candidate、rollback）immutable release 为输入——`RELEASE_ATTESTATION`/`ROLLBACK_RELEASE_ATTESTATION`（`releaseClass=productLifecycleState=production`，供 stackctl package 候选绑定）与 candidate release 的 `RELEASE_HANDOFF_REF`（authoritative handoff-ref-v1，现役 `qwq-data ship apply → activate → verify --readiness-phase production` 的唯一准入身份）——`make integrate PUBLISH=0` 对 exact candidate 完整产出 Alpha（及条件 Beta）`EnvironmentAcceptanceFact`，summary 各分段耗时齐全；research/commercial attestation 一律以 `INTEGRATION_RUN.INPUT_INVALID` 拒绝，不得以 release id 隐式选择 release。
