...
- **集成主路径**：日常开发在 lead lane 分支提交，以 `lane/* -> dev1.0` PR 合入；同一精确 merge SHA 的 required integration checks 全部成功后，`dev1.0` 成为该增量的集成终态。canonical `integration_branch_activation.state` 为 `bootstrap|active` 闭集，当前成熟仓库为 `active`；active 下 `dev1.0` 只接受 lane PR merge 或受管 system fast-forward backsync。bootstrap 仅允许远端 integration ref 不存在时，由匹配本地 `dev1.0` create 远端 `dev1.0`，且无 release eligibility；它不是永久隐式例外。失败增量只能在 retained lead lane 追加修复，不得创建白名单外旁路分支。
...
- **direct push 与 activation 边界**：普通 lane 只允许更新同名远端，不要求一次 push 全部 lane。bootstrap 下仅上述 create-only integration 初始化例外通过，已有远端 `dev1.0` 的 update 必须拒绝；显式 transition CLI 只能在当前 state 为 bootstrap 时 create-once 写出 untracked evidence、active proposal 与人工提交指令，不得改 tracked policy。tracked policy 经人工提交为 active 后，`dev1.0` direct push 拒绝，只由合法 lane PR merge 或受管 system backsync 更新；`main` direct push 拒绝。Hosted PR gate 只验证事件提供的 remote head/base 事实，不枚举本地 lane。Actions 与 release governance 必须拒绝缺少合法 promotion 事实的 SHA，在缺少 GitHub 原生保护时不得声称远端 ref 一定未被先行修改。
...
- THEN bootstrap 只接受远端 ref 不存在时匹配本地 `dev1.0` 的 create-only 初始化；active（当前 canonical state）拒绝本地 direct update，并只接受 `lane/* -> dev1.0` PR merge 或可证明的 system fast-forward backsync。匹配同名远端的单条普通 lane push、`lane/* -> dev1.0` PR 与 `dev1.0 -> main` PR 合法；lane push 不要求 all lanes，Hosted PR 只验证 remote 事件事实。
- AND 白名单外任何分支、lane 之间的 PR、`lane/* -> main`、人工 `main -> dev1.0`、缺失 head/base、`main` direct push、bootstrap 对既有 `dev1.0` 的 update、active direct update、非匹配 lane push 以及非受管 backsync 均返回无 traceback 的 typed blocker；transition evidence 已存在时拒绝覆盖，tracked policy 只能由人提交。
