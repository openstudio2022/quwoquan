...
`local-gamma-mirror` 是提交前本地预测试环境：在开发机通过 Docker 镜像栈、DNS/TLS 反代、gamma seed manifest 与真机/模拟器 runner，把 `local_contract -> user_acceptance` 验证尽量左移到本地完成。它复用云侧 gamma 的配置语义、seed 来源、旅程注册表和报告字段，但**不再作为进入 `main` 的独立 merge gate**。
...
- A1：规格、设计、验收与计划已落地，明确本地 `local_contract -> user_acceptance` 左移覆盖与云端不可替代边界。
...
- A4：本地 `api_integration` runner 使用 `app_gamma_seed_manifest.json` 验证真实 API、真实存储副作用、错误响应和 RemoteRepository 边界。
- A5：本地 `user_acceptance` runner 统一 App 与测试进程 endpoint，至少在一台模拟器或真机完成 Patrol 核心旅程。
