...
四类逻辑环境（alpha、beta、gamma、prod）与**代码一套、配置分环境**，以及 `alpha-local -> beta-local -> gamma-hosted -> prod-hosted` 的固定自动推进主链。`prod` 内部仍允许灰度/放量 wave，但这些 wave 只属于 `prod`，不是额外环境。
...
- A4：文档与 workflow 都明确保持 `alpha-local / beta-local / gamma-hosted / prod-hosted`，不引入 `beta-hosted` 或 `prod-gray`
