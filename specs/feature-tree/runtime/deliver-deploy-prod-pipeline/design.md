...
- Beta 显式 opt-in：`derive_integration_depth` 仍派生 `abg_release_sensitive|alpha_integration|no_live`，但只决定 Beta `not_required` 的原因码。Alpha 始终真跑；Beta 只在 `--beta`（`BETA=1`）时真跑，否则签 `status=not_required`，`reasonCode` 为 `IMPACT_PLAN.NO_LIVE_ENVIRONMENT_REQUIRED`（ImpactPlan 无需 live Beta）或 `ACCEPTANCE.BETA_OPTIONAL_BY_POLICY`（判定敏感但未 opt-in）；该闭集由 `environment_acceptance_fact_contract.NOT_REQUIRED_REASON_CODES` 唯一声明，签发（`environment_scheduler`）、schema、admission（`create_publish_admission`）三处消费同一常量。named evidence 的 `source.basis` 与 EAF `reasonCode` 同值；ReadinessCaseResult 合同规定 passed 结果不携带 `reasonCode`，因此原因只落在 EAF 与 named evidence。
...
- 测试 seam：`test_integration_run_production_release__local_contract_test.py` 覆盖 `--mode` 闭集、`make accept` 含 `--beta`/`MERGED_LANES` 且不含 `--publish`、`make integrate` 只含 `--acceptance-bundle`、acceptance/integrate 互斥输入的 typed 拒绝、Beta 原因码分流、bundle round-trip（23 个 exact store 文件）与 digest/manifest/commit/parent/占位漂移拒绝、integrate 相位闭集 `preflight → import-bundle → admit`；`test_environment_scheduler__local_contract_test.py` 覆盖 `ACCEPTANCE.BETA_OPTIONAL_BY_POLICY` 的签发与 schema 及未知原因码拒绝。
