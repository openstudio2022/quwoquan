...
- 理由：验收职责和ref发布权限分离不要求两个物理工作树。复用同一执行器和事实闭包，使integration自主修复、验收与发布而不依赖上游发起；来源字段不是资格，环境签名、Data admission、scope/currentness与CAS仍分别验证。单树复用与多树四环境执行的覆盖策略由REQ-002演进，不能仅凭来源分支判断验收通过。
