...
- [`daily-merge-release-strategy`](./daily-merge-release-strategy/spec.md)：scoped candidate 在 lane 完成 required checks 与 Alpha（Beta 显式 opt-in）并形成 bundle，唯一 integration 工作区验真 admission 后由受信发布通道 non-force fast-forward 更新 dev；`dev1.0 -> main` 只做可用源码 promotion。
...
- 对象与 owner：六条本地 lane 仅是检出/验收 identity，不再有远端 lane 或 `lane/* -> dev1.0` PR 交付边。`local-continuous-integration` 拥有 `04. Lane Gate` 检查集合：静态治理、ImpactPlan/changed boundary、canonical full Code Health Delta 与 ops 合同；它们在 lane `make accept` 的同一 exact candidate 上完整、去重执行并进入 source fact。Alpha 必跑、Beta 显式 opt-in 的 EAF 与这些 source facts 一起形成 DEC-014 的 portable bundle。integration 只验真并发布，不重跑完整套件。
- Command/query 分流：本地 accept 是质量事实生产者，本地 bundle/integrate/hook 是受管入口的强制验真边界。最终本地 publisher 在执行 expected-old non-force FF 前独立重算 admission 自摘要并复验前驱引用、签名/有效期、candidate commit/tree、before/after、规范 integration 路径与目标 remote/ref；布尔环境变量、自报 success、check 名称或 workflow conclusion 均不是 authority。hosted 保留 dev 禁删/禁 non-FF 与 main promotion 强制，但普通已授权写凭据仍可绕过本地流程 FF dev，服务端不能保证 Alpha 必经。专用 publisher/broker 与只验 exact 证据的 hosted 资格强制尚缺执行面，保持 OPEN-004 `track`，不得宣称不可绕过。
- 时机与节奏：先 lane `make accept` 产出 current bundle；integration 在移动 HEAD 前校验 bundle 与远端 parent，通过才本地 FF、admit/publish，并在发布前再次复验。无效 bundle、stale parent 或缺 required 检查在本地 ref/远端 mutation 前停止。publish 只有读回 `after` 才产出 publish result，再冻结该已发布 SHA 回同步本地 lane。没有 source-only 发布或先发远端 lane 再取得 check 的依赖。
...
- 测试 seam：accept planner/runner 的 local contract 对照既有 Lane Gate 全检查集合，证明缺项/失败不得 accepted、同一 exact candidate 只执行一次；admission/publisher seam 覆盖仅 env=1、伪造/过期/漂移前驱、错路径/remote/tree/OID 与 non-FF 零写；hosted seam 如实证明旧 required check 已撤、普通授权凭据可 FF 而不能保证 Alpha，并验证 dev 禁删/禁 non-FF 与 main promotion 仍强制；专用 broker 的缺证据拒绝 seam 保持 OPEN-004，ruleset seam 保留适用规则与 bypass 可见性负例；system backsync seam 证明 MainSourceSeal 与 expected-before 绑定。源码合同不能替代 current hosted 执行回执。
...
- 纯验真前置：accept 默认完整 `scope` 与 `local_readiness_scope`，full required/Review/`deferred=[]` 在 Alpha 前验证；validate-only 与 admission 复用同一纯校验，在 HEAD/index/WIP 变化前拒绝 fast-only/缺 scope、required 或 Review，预检不 import/admit，正式 publish 再复核。`no_live` 仅诊断，不可 accepted/bundle。显式与自动 candidate reuse 共用 caller owner/claim 校验，不因同 SHA 复用他方身份。
...
- Beta 显式 opt-in：`derive_integration_depth` 仍派生 `abg_release_sensitive|alpha_integration|no_live`，但不决定 Beta 真跑或政策跳过的原因码。可发布验收 Alpha 必跑；Beta 选择延伸至 Data milestone→Gamma 全链：未选只验政策跳过与同 release Alpha/内容前驱，Gamma 自身完整检查仍 required；已选失败阻断，不自动 opt-out，新选择保留旧历史。改为 opt-in 时只在完整 runtime 前驱可恢复时复用 Alpha 补跑 Beta，否则重验。Beta 只在 `--beta`（`BETA=1`）时真跑，否则无论集成深度为何都签 `status=not_required`、`reasonCode=ACCEPTANCE.BETA_OPTIONAL_BY_POLICY`。事实合同仍由 `environment_acceptance_fact_contract.NOT_REQUIRED_REASON_CODES` 唯一声明有效原因码闭集，保留 ImpactPlan 自身免环境结论的 `IMPACT_PLAN.NO_LIVE_ENVIRONMENT_REQUIRED`，但本通道不得用它代替政策原因；签发（`environment_scheduler`）、schema、admission（`create_publish_admission`）三处消费同一常量。named evidence 的 `source.basis` 与 EAF `reasonCode` 同值；ReadinessCaseResult 合同规定 passed 结果不携带 `reasonCode`，因此原因只落在 EAF 与 named evidence。
