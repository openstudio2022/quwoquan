...
- [x] 新增 `quwoquan_app/scripts/runtime/verify_module_package_mapping.py`。
...
- [x] 接入 `Makefile`、`make verify`、`make gate`、`agent_ops/gate/gate_repo.sh --scope service`。
...
- [x] 本地可执行验证（2026-05-03）：`python3 -m py_compile agent_ops/avatar/run_chat_avatar*.py agent_ops/deploy/gamma/run_local_gamma_avatar_e2e.py quwoquan_app/scripts/gamma/run_local_gamma_t3.py`、`bash -n quwoquan_app/scripts/gamma/start_local_gamma_mirror.sh`、`go test ./runtime/reliabletask`、`go test ./services/chat-service/tests -run TestGroupAvatar`、`flutter test test/cloud/realtime/realtime_avatar_sync_handler_test.dart` 均通过。
- [ ] 真实环境矩阵证据：`beta`、`local-gamma`、`cloud-gamma-pre`、`cloud-gamma-prod-smoke` 在可访问网关、ECS 凭据与本机或 self-hosted Android/iOS 就绪后运行 **非 dry-run** 报告；**且** `make verify-chat-avatar-commercial-matrix COMMERCIAL_MATRIX_MANIFEST=artifacts/commercial-matrix/chat-avatar/manifest.yaml`（或等价 `python3 agent_ops/avatar/verify_chat_avatar_commercial_matrix_evidence.py`）**退出码 0**。未满足前不得声明商用矩阵完成。
  ...
  - 一键编排：`bash agent_ops/avatar/run_chat_avatar_commercial_matrix_orchestrator.sh` / `make run-chat-avatar-commercial-matrix-local`。
  - 自检：`python3 agent_ops/avatar/check_avatar_commercial_matrix_prereqs.py --strict`（不替代 manifest 校验）。
