...
- 裸 `flutter run` 与 IDE 直接 Flutter profile 不是受支持入口；Xcode/Gradle phase 必须在安装前失败并只提示 `./run.sh -d <device>`，不得为首次冷构建合成 Alpha handoff、endpoint、Dart defines、证书信任或 effective manifest。
...
- 该入口固定编译 production `lib/main_prod.dart` 连接完整 Remote topology，并把同一 handoff 的 entrypoint 与全部 `--dart-define` 交给 resident compiler；禁止 alpha runner、fixture override 或只提供 mock/public-plane 子集的本地进程。
- Gradle/Xcode 只验证 launcher 已提供完整 runtime package、制品摘要与设备证明；不得获取 Android lease、准备环境、静默推断 Alpha/Beta/Gamma/Prod、复制配置或吞掉准备脚本失败。
...
- GIVEN 开发者通过 `quwoquan_app/run.sh -d <device>` 在指定设备启动 Alpha App。
...
- AND 裸 iOS Simulator Debug `flutter run` 与其他缺失 canonical handoff 的 direct build 在安装前失败并指向 `./run.sh -d <device>`，用户不得看到由开发配置缺失制造的启动恢复页。
...
- 优先级：`P0`
- 准出影响：`block`
- 影响或价值：尚缺公网 DNS/证书签发与 Alpha/Beta/Gamma/Prod-sim live runtime，因而 live health、release activation 和三个 release-bound exact query 仍无通过证据；四环境 Remote App/Service package、target 隔离、public base、host purity 与产物纯度已可从空输出重建并通过。
- 目标：在 OPEN-002 的 DNS/TLS 前置满足后启动四环境 Remote topology，补齐 release activation、live health 与 consumer/commercial exact-query 矩阵。
...
- 优先级：`P0`
- 准出影响：`block`
- 影响或价值：仓库已具备 loopback DNS plan/apply/verify、CAA、邮件防护与 DNS-01 证书签发链路，并隔离 DNS provisioning token 与非生产 challenge-only token；但当前会话未提供 Cloudflare token/zone id 与 ACME account email，不能伪造 live DNS/TLS 成功证据。
- 目标：通过受保护变量提供 `QWQ_DNS_PROVISIONING_API_TOKEN`、`QWQ_ACME_DNS_API_TOKEN`、`QWQ_DNS_ZONE_ID`、`QWQ_ACME_ACCOUNT_EMAIL`，执行 apply、证书签发、四个本地 target HTTPS/WSS 与 prod-hosted 外部公共 CA 验证并保存 receipt。
