...
- 日常与 CI 构建只消费已锁定依赖；Dart lock、Flutter plugin podspec、Podfile.lock、Pods/Manifest.lock 与 CocoaPods executable/version 任一漂移时在编译前返回 typed blocker，启动路径不得自动 update 锁定声明或联网修复。唯一有界例外是依赖 staleness 的交互式同步恢复：live worktree 的外层 canonical launcher 在创建 private workspace projection 前检出 active dependency bundle 与当前 source/toolchain identity 漂移时，先输出 canonical `APP.DEPENDENCY.bundle_stale`（detail 只携带白名单字段名，如 `field=nativeResolutionInputDigest`）；仅当 stdin 与 stderr 均为 TTY、处于 live workspace 且同一次 launcher 调用内尚未执行过同步时，才允许自动执行一次 canonical `stackctl app-dependency-sync`，成功且 active readback 与本次 sync attempt 一致后仅重试一次 projection。同步失败、activation ambiguous、第二次 stale、非交互/CI/UAT 或 private projection 内一律 fail-closed，首个 stale blocker 必须先输出且不得被替换；该例外绝不更新任何锁定声明。
...
- `quwoquan_app/run.sh --mode content-live --env alpha|beta|gamma -d <device>` 是内容联调、Hot Restart 与完整启动证据链的 canonical 开发启动执行体；具名激活入口把受管 bin 目录注入 PATH 后，`run.sh` 在任意工作目录全局可调用，PATH wrapper 只 `exec` 仓库内同一脚本，不复制参数、状态或第二套逻辑。受控制的 IDE Run/Debug（`workspace_ide_debug`）是其薄包装。
- 受管 PATH 中的字面 `flutter run` 是 managed one-command 入口：launcher `flutter` dispatcher 只对本 App 工作区的 `run` 子命令归一化进入 canonical launcher（launch provenance=`canonical_launcher`），固定 environment=alpha、target=`alpha-local`、mode=`content-live`、launch policy=`test_live`。managed 路径按固定顺序执行：解析 exact device → 启动或复用 alpha full mutable runtime → 获取真实 consumer lease/transport → 以同一 lease 安装并验证 device trust → 解析并绑定 exact 当前内容（禁止 latest 猜测）→ 严格 preflight（TLS、api-edge、user-service、integration-service、SMS Provider/relay identity、真实 OTP journey，及含 homepage、Creator 头像、图片、typed video、premium stream 与媒体字节的严格 content preflight）→ 写 private managed preparation receipt → 前台 exec canonical `run.sh` 完成 build/install/activation/attach。managed 路径中上述任一 readiness 不可用都是 typed blocker，在 Flutter build 前 fail-closed；direct `run.sh`、`make app-dev`/dev-session 维持既有 `test_live` warning+degraded 语义不变。其他 Flutter 子命令（analyze/test/pub/doctor/build 等）与其他项目由 dispatcher 以 exact argv/env/cwd 透传真实 SDK。非 alpha 的 ambient 环境选择器对字面命令 typed 拒绝。Alpha 当前 Research release 的匿名隔离保持：managed 启动只要求登录服务可用，真实内容验收仍要求已登记白名单账号登录。
...
- `run.sh` 前台会话与并发语义：TTY 下 r/R/q 分别桥接为同一 attach 会话的 hot reload、hot restart 与停止，非 TTY 保持无键盘面。跨设备并行 canonical run 互不阻塞——deploy work state 按 run/设备隔离，Android `adb reverse` 幂等且不清理他会话映射；同设备重复启动即重启既有实例。
...
- `acceptanceProfile=m1_api_consumer` 的 `requiredRawResults` 仍引用 canonical `ReadinessCaseResult` envelope，但结果层固定为 API 集成 consumer readback，不是 raw App UAT，也不携带 UAT/promotion authority；这不创建第二种 CaseResult，更不得让 API result 顶替上述 environment-promotion physical slots。
...
- `acceptanceProfile=m1_api_consumer` 只允许 `environment=alpha`、`target=alpha-local`、`predecessorAcceptance=null`、`prodReleaseFacts=null` 与 `targetBindingRefs=[]`。它必须绑定同一 M1 Research `releaseId + releaseDigest + importRunId + verifyRunId` 的 Alpha import/readback/activation exact evidence，且 `ReleaseUatSamplePlan` 的 4 entry × 4 carrier 共 16 个 cell 全部 required；`requiredRawResults` 恰好直接绑定这 16 个 fresh API 集成 consumer readback CaseResult refs/exact-byte digests，并逐 slot 证明相同 release/import/verify identity、canonical entry/carrier、runner 与 `status=passed`。跨 run/release、缺 cell、重复 slot、非 API 集成、App 用户验收结果或摘要漂移均 `GATE_BLOCK`。
...
- 两个 profile 的 `EnvironmentAcceptanceFact` 都是 append-only Ops acceptance fact，必须直接绑定本 profile 的全部 required raw refs/exact-byte digests、Data readiness、fresh active CAS/readback、environment lifecycle `Exit`、Provider/observability/rollback readiness及相同 release/import/verify identity。`environment_promotion` 的 Beta、Gamma、Prod 还必须绑定前一环境 `environment_promotion` fact exact-byte digest；Data readiness、retired `appUatEnvelopeDigest` field、父 report、bundle digest或 `m1_api_consumer` fact 均不能替代前环境 acceptance 或任一 required raw result。
- 任何本 profile required slot 缺失或结果为 `failed/blocked/skipped`、任一 exact-byte digest/readback 漂移、lifecycle 未到 `Exit`、Provider/observability/rollback 未就绪，均不得创建通过 fact，并返回 `GATE_BLOCK`。`environment_promotion` 只能按 Alpha→Beta→Gamma→Prod 顺序追加且后继环境 exact-byte 绑定前一环境同 profile fact；任何 legacy aggregate verdict 均无写 authority、无 promotion consumer。
...
- `GWT-002` 证据绑定：`local_contract` 覆盖 launcher/lease/activation/receipt、`app-dev`/`app-uat` 薄适配边界、managed 字面 `flutter run` dispatcher 的子命令分流/readiness fail-closed 顺序/非 alpha 选择器拒绝与 raw SDK 旁路负例、依赖 bundle stale 的单次有界同步恢复与非交互 fail-closed、`run.sh` 全局 wrapper 与设备选择、PATH 注入投影/回退、attach 键位桥、并发隔离、frozen CocoaPods binding、父 report 无 verdict 与 typed blocker；`api_integration` 覆盖真实 stackctl 委托、attempt-1/retry 同 binding、runtime package、CAS/readback、Remote 服务与 lifecycle；`user_acceptance` 覆盖 Android/iOS 的 `run.sh`、受管终端字面 `flutter run` one-command 启动、IDE 启动、Hot Restart、并行双设备、内容 outcome 与恢复动作。
...
- `GWT-006` 证据绑定：`local_contract` 覆盖同一 EAF schema/validator 的 `environment_promotion|m1_api_consumer` 条件分支、create-once/append-only、exact-byte refs、空/非空 target binding 约束、非 promotion consumer 拒绝、父投影不可写回与 J 标签纯视图；`api_integration` 覆盖 CAS/readback、Data readiness、Exit、Provider/observability/rollback、前环境 digest，以及 Alpha M1 同 release/import/verify 的 16 个 API consumer readback slots；`user_acceptance` 仅覆盖 `environment_promotion` required App raw results 对正式 acceptance 的直接贡献。
...
- GIVEN 开发者通过全局可调用的 `quwoquan_app/run.sh --mode content-live|ui-only --env alpha|beta|gamma [-d <device>]`、受版本控制的 IDE 薄包装入口（`workspace_ide_debug`），或在受管终端执行字面 `flutter run`（launcher `flutter` dispatcher 归一化进入 canonical launcher，固定 alpha/content-live，launch provenance=`canonical_launcher`）；canonical 入口在未显式选择环境时默认 Alpha，并由 launcher 生成 canonical handoff 与待激活 package。
...
- AND live worktree 的外层 `run.sh` 在创建 private workspace projection 前检出 active dependency bundle 与当前 source identity 漂移时，先输出 canonical stale blocker（detail 只含白名单字段名）；stdin/stderr 双 TTY 的交互会话自动完成一次 canonical 同步并在 active readback 与本次 sync attempt 一致后重试一次 projection 即恢复启动，非交互调用、同步失败、activation ambiguous 或第二次 stale 保持失败且首个 blocker 不被替换，锁定声明不被更新。
...
- THEN 两个 profile 由同一 schema、validator、factId 派生与 append-only store 条件校验；fact 直接列出本 profile 全部 required raw refs/exact-byte digests、Data readiness、active CAS/readback、lifecycle `Exit`、Provider/observability/rollback readiness。retired `appUatEnvelopeDigest` field、Data readiness 或父 report/bundle 不能替代其中任一项。
...
- AND `m1_api_consumer` 只在 `alpha/alpha-local` 成立，`targetBindingRefs=[]`，同一 M1 release/importRunId/verifyRunId 的 16 个 fresh API integration consumer readback CaseResult 对 4 entry × 4 carrier 精确一一覆盖；任一 App 用户验收结果、TargetUatBinding/device 字段、跨 identity、重复/缺失 cell 或非 passed result 均 `GATE_BLOCK`。
...
- 优先级：`P0`
- 准出影响：`block`
- 影响或价值：required human surfaces 收敛为全局可调用的 `run.sh`、受控制 IDE Run/Debug 与受管 PATH 中经 launcher `flutter` dispatcher 进入 canonical launcher 的字面 `flutter run`（managed one-command 入口，固定 alpha/content-live）。当前仍缺 managed 字面命令一命令到达 alpha 首页内容的双端现场证据、dispatcher 的 readiness fail-closed 顺序与非 alpha 选择器拒绝、依赖 bundle stale 的单次有界同步恢复、`run.sh` 无 `-d` 交互设备选择、attach 键位桥、并行双设备与同设备重复运行重启的现场证据；facade/carrier 退役后的 PATH 注入等价性、`make app-dev`/`make app-uat` 薄适配与 test_live readiness 也尚未同轮闭环。
- 尚缺实现：launcher `flutter` dispatcher（`run` 子命令归一化进入 canonical launcher、其余 exact 透传）、managed 路径的固定执行顺序与严格 preflight、private managed preparation receipt、嵌入默认供给与 `native_flutter_run` 消费面的物理退役、`run.sh` 外层 bundle stale 检测与单次交互式同步恢复、PATH wrapper 与 TTY 设备选择、attach r/R/q 键位桥、deploy work state 按 run/设备隔离与 `adb reverse` 幂等化。iOS UAT attempt-1/retry 仍复用 parent 预冻结 binding，direct `run.sh` 的 test_live warning 不得跳过真实编译、安装、activation 与启动。
