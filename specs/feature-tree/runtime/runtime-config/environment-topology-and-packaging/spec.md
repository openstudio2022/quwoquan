...
- 四环境默认使用同一内容发布、激活和就绪验证链，不选择或传递内容类别；环境差异只由显式环境配置表达，不由环境名在业务代码中分支。内容 readiness 必须绑定同一份 immutable release 的 `releaseId + manifestDigest + sourceOwner=qwq_data`，并校验 discovery `identity=work`、视频书 `identity=work&type=video` 与 `premium_stream` 的 release-bound 非空读回；缺少 Data readiness receipt 或任一 exact query 为空时不得产生通过回执。
...
- Debug-nonprod 构建期自供给（`runtimeConfigSupplyMode=build_time_self_supply`）：开发启动的最基础动作只能依赖仓库与 SDK，不得依赖仓库外 PATH 注入、用户 shell 状态或某个具体 worktree 的绝对路径。iOS `Debug-nonprod` 与 Android debug/nonprod 的构建阶段在既有 trust gate 判否前，若无外部 canonical handoff（`QWQ_IOS_RUNTIME_CONFIG_TRUST_PATH` / `QWQ_ANDROID_RUNTIME_CONFIG_ASSET_ROOT` 缺席），必须以当前源码树（`SRCROOT` / Gradle 项目根，不读任何用户级配置）调用仓内 canonical handoff builder（`quwoquan_app/scripts/device/build_launcher_handoff.py --env alpha --target alpha-local --launch-provenance workspace_ide_debug --launch-policy test_live --runtime-config-supply-mode build_time_self_supply`）现场签发 nonprod trust envelope 与 alpha `test_live` package，并把 `runtime_config_activation_request`（`expectedActiveDigest` 留空）与 trust envelope 一并嵌入制品 `qwq_runtime/`；材料每次构建重生成、不缓存、不进源码树。原生 gate 冷启动在无外部 activation 参数时消费该嵌入请求：仅当 active package 缺席、过期，或 active receipt 同为 `build_time_self_supply` 且 `requestDigest` 与嵌入请求不同（重建后刷新）时，以当前 active digest 为 CAS 前值经同一 `activate`/receipt 路径激活；外部 `external_runtime_package` 已激活且新鲜时保持不变。自供给激活失败只记录 typed 码并继续既有 trust/config 阻断，不得静默回退。Profile/Release、prod buildProfile、`prod-sim`、`prod-hosted` 与 beta/gamma 不得物化或消费自供给，无 canonical handoff 时仍以 `APP.LAUNCH.runtime_config_trust_missing` fail-closed。`native_flutter_run`/`workspace_flutter_run` provenance、ZDOTDIR shim 与 terminal carrier receipt 不得恢复；raw 直装启动的 receipt 以 `launchProvenance=workspace_ide_debug` 记账；`app_launch_attempt` 的 terminalCarrierReceiptDigest/terminalCarrierReceiptRef 固定为空值。
...
- AND 环境无激活内容 release 时 App 只接受 canonical `outcome=empty + emptyReason=no_active_release` 或 typed unavailable，不以普通空列表冒充成功；环境已激活 release 时 App 从 Content API 响应解析 `releaseId + manifestDigest`，UAT 以环境侧期望 release 比对读回身份，App 制品不内嵌内容身份。Prod 发布准出仍绑定 active candidate、内容 readiness 与 rollback/replay 的环境侧证据，任一缺失均阻断准出，但不改变 App 运行时行为。
