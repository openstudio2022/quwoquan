...
| `/design` | ① 每服务 `default/alpha/beta/gamma/prod` 目录齐；② 加载顺序与 APP_ENV 校验有测试；③ 门禁脚本可执行 | 阻断 apply，先补实现与测试 |
...
- [x] C1 定义配置目录规范（default/alpha/beta/gamma/prod）
...
- [x] C5 落地自动化测试矩阵（alpha/beta/gamma/prod 配置加载）
- [x] C5.1 新建服务自动配置引导
  - 新增 `scripts/bootstrap_service_config_layout.sh`
  - S04 新建服务流程自动调用，生成 `default/alpha/beta/gamma/prod/config.yaml`
...
- [x] C7.1 增加部署拓扑门禁
  - `deploy/shared/process_domain_mapping.yaml` 声明四态映射
  - `scripts/verify_deployment_domain_mapping.sh` 校验 domain 唯一归属与 beta/gamma/prod 一致
