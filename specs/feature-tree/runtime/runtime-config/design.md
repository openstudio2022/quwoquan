...
- 对象边界：`AppArtifact`、不可变 `RuntimeConfigPackage` 与平台私有容器中的单槽 `ActiveRuntimeConfigPointer` 是三个独立事实。runtime package 不作为 AppArtifact 的 owned entity，也不得进入 APK、AAB、IPA、`.app`、Flutter kernel、Mach-O 或 DEX；不存在任何构建期嵌入例外（`embedded_default_package` 已退役），全部 buildMode 与 buildProfile 制品维持零嵌入。active pointer 只引用一份已验证 package digest，不内嵌历史或无界 ACK 集合。
...
- 被否决方案：environment-specific flavor/scheme、endpoint `--dart-define`、把 target package 注入已签名制品后重签、构建期物化嵌入默认 package 的 bootstrap 旁路（`embedded_default_package`），以及 bundle package 与私有容器双读。
