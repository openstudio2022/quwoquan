...
- 角色入口边界：面向人类的 `make app-dev`、面向 AI/自动化的 `make app-uat`、`run.sh`、fresh Workspace Terminal 字面 `flutter run` 与受控制 IDE 虽承担不同交互角色，但都只能薄提交同一 canonical contract；Make 与 IDE 不拥有设备发现、env/target 扩展、状态机、provenance、receipt 或第二套 activation 协议，自动化入口也不替代 required human surfaces。
- Terminal carrier 边界：全部代理 user zsh startup files 完成后的 final `PATH` 与 live toolchain identity 由入口/toolchain owner 校验；只有 facade→real Flutter→exact Pod 顺序及 Flutter/physical CocoaPods identity 均成立后，terminal carrier receipt 才可写入。旧 PTY、提前 receipt、`agents-window` diagnostics 或用户全局 Cursor settings 修改均不构成通过。
...
| Alpha/Beta/Gamma | Android/iOS：fresh Workspace Terminal 字面 `flutter run`（`workspace_flutter_run`） | required surface；final facade→real Flutter→exact Pod PATH 与 live identity validation 后才写 terminal carrier receipt，facade 再归一化进入 canonical launcher；[`GWT-002`](./environment-topology-and-packaging/spec.md#gwt-002)、[`UAT-003 install-launch-equivalence`](../../spec.md#install-launch-equivalence) | facade 透传/接管、startup files final 顺序、receipt 时机、SDK 单轨/无递归、设备选择与 raw backend 负例 | fresh terminal 回读 Flutter/physical Pod identity后完成 trust、compile/install/activation/launch receipt；iOS attempt-1/retry 复用同一 frozen binding | fresh Workspace Terminal 真实执行 `flutter run -d <device>` 完成同一 attempt；`agents-window` 仅为 optional diagnostics，不是 required literal Flutter/UAT surface，也不修改用户全局 Cursor settings |
