...
- [`agent-skill-review-context-organization`](./agent-skill-review-context-organization/spec.md)：规则按唯一职责渐进加载，开发与 Review 共用 context manifest，评审受固定角色预算约束，Cursor/Codex 同源加载。
...
- [`governance-pipeline-observe-only`](./governance-pipeline-observe-only/spec.md)：从 context manifest 的 immutable exact ref 开始，只读汇聚 readiness、Review、Human、Objective、环境和发布 owner 的独立 readback，并维持零 authority/零 mutation 边界。
...
- 决策：`explore` 与 `plan-next` 不产生送审交付件。`continue` 复用被恢复 workflow，`review` 不递归自审，`commit` 只做 lane 提交、不要求 Review evidence。这五个控制型 workflow 在 registry 中声明零 Reviewer；其他 workflow 在 registry 保留 primary/specialist 配置以供显式 `/review` 与准出派发，但开发期 POST 默认不派审——Review 触发点只在用户显式请求或准出（lane acceptance/integration 发布、handoff、release），硬门不进入日常开发。
...
- 决策：spec 路径按目录父链直接定位 Feature context；代码路径通过 dependency evidence 关联零个或多个 context。context 查询不授予 mutation 权限，unresolved 或多 context 不阻断写入；写入边界由用户授权、整文件 exact-path claim 与 current actual diff/ImpactPlan 决定。Review 直接消费 candidate changed paths、workspace 与 ImpactPlan，profile 只由 current `changed_paths + deliverable` 派生。
...
- 决策：交付来源闭集从 branch policy 的本地 lane 与 integration branch 派生，排除 release branch；candidate 的 delivery/source 两字段必须一致。producer 和离线结构 validator 共用同一判据，current consumer 另验实际分支与 policy digest。integration 可生产自身候选，不冒用 lane、不改变 Feature context、不引入绝对工作树路径。既有字段形状保持，政策变更由已绑定的 governance/branch 摘要使旧 current evidence 失效；错误来源按既有 typed terminal 拒绝，不设绕过参数。测试覆盖 integration 正向、main/未知/detached 拒绝及来源字段不一致，失败只重建证据，不移动 ref。
- 决策：candidate 采用单轨升级的紧凑 manifest 与一个完整 owner/path 对象；对象以 canonical JSON exact bytes 内容寻址，绑定 PRE owner 前驱及全部稳定排序的 owner groups。manifest 只保存 exact ref、字节摘要、长度、路径/owner 数量和 scope 摘要，既有 manifest 预算保持不变；该引用是无损对象边界，不是分片候选或上下文截断。
- 决策：producer 复用 descriptor-relative create-once writer，先发布依赖对象并安全读回，再发布 manifest；同值 replay 幂等，目录项替换、symlink/multilink、不同字节竞争均 fail closed。consumer 读取前按契约资源上限与声明长度限制 fd 读取，再校验 canonical bytes、ref/digest、计数、前驱、路径覆盖与 current resolver/workspace/ImpactPlan；不通过反序列化后的摘要自报绕过原始字节验证。
- 决策：Review dispatcher、evidence runner 和 handoff 共用同一闭包验证入口；handoff 的 authoritative exact bytes 携带 candidate、owner 前驱及路径对象闭包，跨 runner 离线验证读取传输的 exact bytes，不读取原宿主 inventory，current admission 仍另验当前工作树 freshness。旧 schema 以既有 migration terminal 拒绝，不做双读或静默补取。
- 恢复与观测：缺失、篡改、资源超限或陈旧对象保留 typed blocker，只能从 current context/完整范围重建；未发布依赖可留作可删除输出，不回滚他方字节。测试 seam 覆盖正式大候选入口、consumer 缺失/替换/前驱/范围漂移及并发发布；字节数、路径计数与 exact refs 是诊断事实，不签发准出 authority。
...
- 影响 Story：[`agent-skill-review-context-organization`](./agent-skill-review-context-organization/spec.md)（context manifest 与 adapter）
- 关联验收：`GWT-002`、`GWT-005`

<a id="dec-005"></a>
### DEC-005 上下文治理门以全 Story 约束为输入
- 决策：上下文预算门同时校验规则分层、context manifest、Review Board、adapter 单轨和 Claude 活跃入口退休；它是约束执行器，不复制各 Requirement 正文。负例合同与 gate 使用同一实现字节，任何预算或注册表语义变化必须同步失败。
...
- 决策：Local CI 的 planner/runner 只根据 changed paths、context manifest 与 canonical profile 调度仓库既有 checks；queue、cache、receipt 与 inspect 输出均是绑定 EvidenceFingerprint 的可删除投影，不登记 check 真相、完成状态或 release 状态。当前无宿主自动 edit readiness 接线，queue 在真实 producer/consumer SLO 闭合前按版本化 contract 仅作 advisory；Git hooks 保留 staged/branch boundary，不消费 readiness receipt，也不自行复制全面测试编排。pre-push 拒绝远端 lane 写入；唯一 integration 工作区只有持验真 acceptance admission 才可匹配 `dev1.0 -> dev1.0` non-force fast-forward，并用 update line before/after OID 调用 Git ancestry authority。最终 publisher 独立复验 admission 自摘要、exact 前驱/签名/有效期、candidate/tree、OID、规范路径与 remote；仅 env=1、缺证据/authority、非快进、force/delete、来源不匹配、lane→dev 与 main direct push 均 fail closed。`integrationEligibility` 只能由 exact admission 建立，push 不重新签发环境及后续资格。Lane Gate 完整检查集合左移到 accept 的同一 exact candidate 并去重，本地 bundle/integrate/hook 强制验真，不重跑完整套件。按授权撤除 dev 旧 `04. Lane Gate` required check、删除已可达已发布 dev 且增量保全的远端 lane，不等待专用 publisher/broker；hosted 资格强制缺口归 daily-merge OPEN-004 `track`，不阻塞有效本地合入。普通授权凭据仍可 FF dev，服务端不能保证 Alpha；dev 禁删/禁 non-FF 与 main promotion 强制不变。
...
- 决策：满足 local-CI `REQ-003` 廉价扫描条件的检查在显式 L0 首判，readiness 通过既有 `STATIC_COMMANDS` 消费同一 id/命令；`retired_terms_zero` 复用现有四工程树 `source_changed` 判断及扫描器，不新增分类分支或 check registry。`gate_repo.sh` 将它上提到所有合法 scope/phase 共用的参数校验之后、重型检查之前，删除 `run_app` 旧调用；既有 companion 随 Lane Gate 集合在 accept 执行，hosted 只验 exact 结果，不新增重复扫描入口。L0 选择来自调用方 index 的路径，但该扫描与其他全仓静态门一样读取当前工作树，不能据此声称已验证 staged-only candidate；clean candidate 的 exact 字节验证仍由准出 consumer 负责。
- 失败、恢复与观测：扫描退出非零、超时或输入漂移沿用 L0/static 与 readiness 的既有失败/失效终态，不新增 receipt schema；日志保留 check id、文件/行/标识、退出码和耗时。修正真实输入后重跑，不能复用 stale PASS 或放宽豁免；回滚本增量须连同规格和全部接线一起回退，保留原扫描器与 hosted companion，禁止只删本地阻断。测试 seam 覆盖选择器去重/文档非触发、真实扫描正负例、shell case 失败传播、readiness 不生成 PASS 与公共前缀先失败；预算沿用现有 L0 deadline，不因左移提高阈值。
...
- 决策：candidate request是canonical create-once事实，绑定claim、context identity、parent/commit/tree、paths、ImpactPlan和source evidence。trusted publisher先验证Alpha/Beta及remote-before，再调用受限ref port执行一次CAS；本地Git adapter只用于local contract，不授予Hosted authority。
...
- Cursor/Codex 当前只接线 observe-only/fail-open 的 shell/worktree context 与会话提醒，不是安全边界；Cursor 没有 after-edit hook，Codex PostToolUse 也未接线。保留的 after-edit 脚本仅供显式验证或未来 producer 使用。Git `pre-commit`/`pre-push` 只守 staged/branch 边界，lane acceptance/integration 发布、handoff、promotion 与 release 的 required checks 才是准出 hard gate。
...
| SKILL:integrate-lane-to-dev | 唯一 integration 工作区消费 exact acceptance bundle 发布 dev 并回同步本地 lane；C-NL/C-Skill/C-Cmd | C-NL[D]、C-Skill[D]、C-Cmd[V(repo)]；Codex discovery[D]/Cmd[U] | 根规则 + [integrate-lane-to-dev Skill](../../../../.agents/skills/integrate-lane-to-dev/SKILL.md) + canonical branch/worktree policy；owner 解析按 Skill PRE | 规范 integration/dev1.0、无进行中操作；本地 HEAD 移动前先验 bundle/candidate 与已发布 remote parent | 验真 bundle → 本地 FF → admit → 授权后 expected-old publish/readback；仅 after 后以冻结已发布 SHA 回同步 | 分层报告 bundle/admission/publish、before\|after\|other 与各 lane resync；交接 Gamma→IQF→main promotion，dev 发布不等于 main 合入 | 缺 bundle、stale、签名/摘要/路径/remote 错误、非 FF 或未知 readback 均阻断；无 source-only/env=1 旁路 | 控制型操作本身零 Reviewer；准出 Review 按 exact candidate 与既有 review Skill | H-D；发布仍需授权，不能伪造资格；跨 lane 只 FF，不 merge 他人字节 | 被跳过 lane 交其自身会话处理；candidate stale 则同步已发布 dev 后重新 accept | integrate 不重跑完整套件；成功 publish 后下一次 delta 排除已发布差异，未发布增长不保证常量，见 daily-merge OPEN-009 |
...
| SKILL:prd | 用户价值/范围/验收的 spec mutation | C-NL[D]、C-Skill[D]、C-Cmd[V(repo)]；Codex discovery[D]/Cmd[U] | 根规则 + [prd Skill](../../../../.agents/skills/prd/SKILL.md) + compact owner ref + canonical contracts | 唯一价值、scope、target/owner/ref 与可测结果 | 在最低节点更新 REQ/GWT/OPEN，不实现代码、不复制 wire | 运行 feature-tree 验证，报告 ref/evidence，默认未评审 | 价值/scope/owner/contract 不唯一时写前阻断 | 显式 Review 或 lane acceptance/integration 发布/handoff 准出；primary + 最多一专审 | H-D；产品范围/验收冻结必须 H-P/H-R；Agent 负责中性规格与可测性 | 回 explore 补事实/owner，或补 contract/验收 | 防止规格正文、矩阵与测试排列组合重复；默认 context ≤8KiB |
| SKILL:sync-lane-from-dev | 把已发布 origin/dev1.0 exact SHA 同步进当前 lane；C-NL/C-Skill/C-Cmd | C-NL[D]、C-Skill[D]、C-Cmd[V(repo)]；Codex discovery[D]/Cmd[U] | 根规则 + [sync-lane-from-dev Skill](../../../../.agents/skills/sync-lane-from-dev/SKILL.md) + canonical branch/worktree policy；owner 解析按 Skill PRE | 固定 lane/路径、无进行中 merge/rebase、脏路径不重叠；fetch 后冻结已发布 SHA，不回落本地 dev | 常规 FF-only；分叉须显式授权由当前 lane 合并同一已发布 SHA，保留自身增量并重新验收 | 报告 frozen SHA、ff/merge、冲突处置、focused verify 与 ahead/behind | 脏重叠、进行中操作或 authority 不可证零写；禁止 reset/stash/clean/`-X ours` | 零 Reviewer；不 push lane、不代入 integration | H-D；仅本 lane 解决授权冲突，发布另授权 | 重叠 WIP 由其 owner 处置，重跑前重新冻结已发布 SHA | 只加载冲突文件及 canonical contracts，不做全仓验证；未发布本地 dev 不得传播 |
...
| BOUNDARY:commit | Git commit 边界（通常经 commit Skill） | Git hook[V(repo)]；Cursor/Codex 只负责发起命令 | commit Skill + staged path identity；不加载完整 Review | 精确 staging、secret/PII/generated/cache/branch 检查 | pre-commit staged boundary；成功后原子 commit | post-commit 仅轻量 mark-due/fail-open；回读 SHA | pre-commit 是本地 hard boundary，但不消费 Review/readiness/本矩阵 gate | 零；lane acceptance/integration 发布 再审 | 明确用户提交意图；Agent 不混入他会话字节 | 修复 staged boundary 后重试，不 clean/reset | 秒级目标；不得运行全仓 context gate、全量 inventory 或普通 commit Review |
| BOUNDARY:push | integration 验真 admission 后 dev 同名 FF publish / 受管 system backsync | Git pre-push[V(repo)]；最终 publisher 独立验真 | branch policy + exact update/admission 或 MainSourceSeal | 回读 ref、actor、before/after、admission exact 前驱与 ancestry | 只执行 canonical 受信更新；env=1 非 authority，lane 远端写入拒绝；不在 hook 重跑 readiness | 报告 exact before/after/readback，push 不推导后续资格 | 缺/漂移 admission、错路径/remote、non-FF、force/delete、lane 写入与 main direct push 均阻断 | push 自身零；current candidate 准出按 review Skill | 外部发布另授权；不把 hook allow 当资格 | 修复事实/身份，stale candidate 从已发布 parent 重新 accept | branch policy 与验真保持有界，不重跑完整套件 |
| BOUNDARY:lane-pr | 本地 lane exact candidate → portable bundle → integration publish | 本地 accept 检查与 hosted 替代验真门的 current 执行证据分别报告；未接线[OPEN] | owner/candidate refs、ImpactPlan、branch policy、Review artifacts | 验 candidate/parent/owner/scope 与 required 检查完整性，不依赖 lane PR | accept 对同一 exact candidate 去重执行 Lane Gate 全集合与 Alpha/Beta 政策；hosted 只验 exact 证据/受信身份 | integration 先验 bundle 再本地 FF/admit/publish；after 读回后冻结已发布 SHA 安全 resync | required 缺项/失败拒绝本地 accepted/integrate；dev 旧 check 可撤、可达 dev 的远端 lane 可删；hosted 强制 OPEN-track 不阻日常合入，普通凭据可 FF 不保证 Alpha，见 daily-merge OPEN-004 | current candidate 准出 Review；primary + 最多一专审 + 人工签收 | 工程/质量接受；保留 dev 禁删/禁 non-FF 与 main promotion，hosted 资格强制缺口如实 track | 修复本 candidate 事实，parent 漂移后重新验收；不恢复 lane PR/source-only | 完整套件只执行一次；hosted 不探测本机六 worktree |
| BOUNDARY:handoff | 跨会话/人/host、环境发布、外部阻断或证据复用 | producer/consumer[V(repo)]；自动宿主触发桥[U] | current context/candidate、named evidence、Review consolidation、OPEN | 仅 canonical 触发时创建；普通闭环使用 `no_persistent_handoff` | create-once manifest，exact refs/fingerprint，不转抄结论 | consumer 重验 freshness 并回读接手结果 | triggered handoff 是 hard boundary；stale/missing refs fail-closed | 触发准出 Review；范围为 handoff exact candidate | Agent 生产/验签；人工签收接手；正式决定仍需 H-P/H-R | 重跑 stale evidence、重建 current handoff，不能改写旧 manifest | 防止每轮都持久化、重复 evidence 和大日志；仅存 refs/摘要 |
| BOUNDARY:dev1.0-main | `dev1.0 → main` promotion PR | branch/source gate[V(repo)]；Hosted protection/live authority[D] | promotion SHA、required checks、Review/release evidence、main reachability | 确认唯一合法 promotion 边与 exact candidate | Hosted checks/人工签收；不执行正式 Prod apply | merge 后 main 为发布来源，system backsync 需 CAS readback | promotion 是 release-source hard gate；main direct push 永远拒绝 | 必触发 release 范围 Review；不复用 lane acceptance/integration 发布 的 stale结论 | release/engineering/quality 分责；PR approval 不等于 Prod H-R | 修复 checks/evidence 或新 candidate；禁止 force/自动 merge | promotion 全量但应去重 common checks；Hosted API 不可读即 fail-closed |
...
- 本节由 `make verify-agent-context-budget` 的 `HOTL 运行矩阵` 检查验证：单一 marker 区段存在、`SKILL:*` 场景与 `.agents/skills` 当前发现的 Workflow Skill 闭集一一对应（不多不少，不硬编码数量）、9 个 `BOUNDARY:*` 场景齐全、列头完整且区段不超过 32KiB。该检查属于按需/准出上下文治理，不得进入普通 commit 的 staged static selection。
