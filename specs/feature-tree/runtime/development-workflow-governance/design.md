...
- 决策：交付来源闭集从 branch policy 的本地 lane 与 integration branch 派生，排除 release branch；candidate 的 delivery/source 两字段必须一致。producer 和离线结构 validator 共用同一判据，current consumer 另验实际分支与 policy digest。integration 可生产自身候选，不冒用 lane、不改变 Feature owner、不引入绝对工作树路径。既有字段形状保持，政策变更由已绑定的 governance/branch 摘要使旧 current evidence 失效；错误来源按既有 typed terminal 拒绝，不设绕过参数。测试覆盖 integration 正向、main/未知/detached 拒绝及来源字段不一致，失败只重建证据，不移动 ref。
...
- 恢复与观测：缺失、篡改、资源超限或陈旧对象保留 typed blocker，只能从 current owner/完整范围重建；未发布依赖可留作可删除输出，不回滚他方字节。测试 seam 覆盖正式大候选入口、consumer 缺失/替换/前驱/范围漂移及并发发布；字节数、路径计数与 exact refs 是诊断事实，不签发准出 authority。
