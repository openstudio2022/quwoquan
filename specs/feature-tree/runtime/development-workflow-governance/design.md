...
| SKILL:integrate-lane-to-dev | 在唯一 integration 工作区把 lane head 合入 `dev1.0` 并回同步各 lane；C-NL/C-Skill/C-Cmd | C-NL[D]、C-Skill[D]、C-Cmd[V(repo)]；Codex discovery[D]/Cmd[U] | 根规则 + [integrate-lane-to-dev Skill](../../../../.agents/skills/integrate-lane-to-dev/SKILL.md) + `branch_policy.yaml`/`worktree_policy.yaml`；不加载 Feature owner manifest | HEAD 为 `dev1.0`、工作区为 integration 目录、`dev1.0` 是 lane 祖先、无进行中 merge | `merge --ff-only <lane>` → `make integrate REUSE=1`（复用同 exact candidate 的 readiness/Alpha 事实）→ 有 admission 走 expected-old 快进发布，外部阻断且用户授权时裸 push 源码 → `make lane-resync-execute` 三态回同步 | 分层报告源码推送、readiness、Alpha 事实/typed blocker、`before\|after\|other` 读回与每条 lane 的 resync JSON | ff 失败、非祖先、读回 `before`/`other`、pre-push 拒绝硬阻断；readiness/Alpha 失败且未授权裸推时阻断；分叉/重叠脏树 lane 零写只报告 | 零 Reviewer；publish result 是 Gamma/IQF 前驱，准出由后续 release 流程触发 | H-D；裸 push 与发布须用户明确授权，Agent 不签发任何资格；跨 lane 只 ff-only、不 merge 他人字节 | 被跳过的 lane 交其会话运行 sync-lane-from-dev；Alpha 阻断保留首个 typed blocker 重跑 | readiness/Alpha 耗时以分钟计且随远端落后膨胀（OPEN-008）；必须复用事实、不得为通过而缩 scope |
...
| SKILL:sync-lane-from-dev | 把本地 `dev1.0` 同步进当前 lane 工作树；C-NL/C-Skill/C-Cmd | C-NL[D]、C-Skill[D]、C-Cmd[V(repo)]；Codex discovery[D]/Cmd[U] | 根规则 + [sync-lane-from-dev Skill](../../../../.agents/skills/sync-lane-from-dev/SKILL.md) + `branch_policy.yaml#persistent_lane_admission`；不加载 Feature owner manifest | HEAD 为固定 lane、工作区与布局一致、无进行中 merge/rebase、脏文件与同步路径不重叠 | lane 是 `dev1.0` 祖先则 `merge --ff-only`，否则一次普通 merge 并只在本 lane 解冲突；冲突路径跑 focused verify | 报告 ff/merge 结果、冲突文件与解决方式、验证退出码、同步后 ahead/behind | 脏树重叠、进行中 merge、冲突无法保留双方语义时零写阻断；禁止 reset/stash/clean/`-X ours` | 零 Reviewer；不 push、不合入 dev | H-D；Agent 只动冲突文件字节，push/合入需用户另行授权 | 修复冲突或先提交/搬走重叠脏改动后重跑 | merge 冲突数可达百级（历史 158 处）；只加载冲突文件与其 contracts，不做全仓验证 |
...
| BOUNDARY:lane-pr | `lane/* → dev1.0` exact candidate | Hosted PR/CI[V(repo)]：`04. Lane Gate` 在 exact PR head 重跑静态治理、ImpactPlan/boundary、code-health delta 与 ops 合同分片，并读回 dev1.0 ruleset 证明自身是 hosted required check（`local-continuous-integration#gwt-005`） | owner/candidate refs、impact checks、branch policy、Review artifacts | 验 head/base、candidate SHA、owner/scope、required checks 可达 | CI 重跑 required checks；不得信任本地 PASS 或开发机 worktree 状态 | 合入由 integration 通道以 Alpha/Beta admission 快进到已过检 lane head（`daily-merge-release-strategy`），hosted 侧标记 PR merged；retained lane 随后 fast-forward resync | 这是集成 hard gate；ruleset 未把本 check 设为 required 或有可见的 bypass actor 时 check 自身转红（只读 token 下 bypass 不可见，以 `bypassActorsObservable` 留痕，见 `local-continuous-integration#gwt-005`），不宣称已自动准出 | 必触发 current candidate Review；primary + 最多一专审 + 人工签收 | 工程/质量接受；正式 hosted authority 缺失时对应声明 fail-closed | 修复 candidate/evidence，更新同一 lane PR；合入后 resync | 避免每 shard 重跑 common governance；Hosted 不探测本机六 worktree |
