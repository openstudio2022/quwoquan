...
- lane 工作树 `make accept` 的 Alpha 准入在 `health` 之后、runtime 仍在线时增加 `alpha.app-launch` 阶段：在一台 iOS 模拟器上以 canonical launcher（`quwoquan_app/run.sh --env alpha -d <udid>`）真实编译、安装、激活、启动，观察到 `QWQ_APP_LAUNCH_PHASE status=launched` 且 `ios_startup_safe_terminal surface=router_shell … configurationState=complete`；随后对同一 Alpha 网关做首页 feed（`GET /content/feed?sort=recommend&channelId=recommend`）与视频书（`GET /content/feed?identity=work&type=video`）只读 readback，要求 HTTP 200 且集合非空。两者各形成一份 raw `ReadinessCaseResult`（`app-launch:ios-simulator`、`content-readback:home-feed+video-book`）进入 Alpha `EnvironmentAcceptanceFact.caseResultRefs`；无可用模拟器（`INTEGRATION_RUN.APP_LAUNCH_DEVICE_UNAVAILABLE`）、启动未达终态（`INTEGRATION_RUN.APP_LAUNCH_FAILED`）或任一 readback 失败（`INTEGRATION_RUN.CONTENT_READBACK_FAILED`）都是 typed blocker，不得降级为 PASS，也不得以服务 health、契约测试、facade status 或旧 receipt 替代。
