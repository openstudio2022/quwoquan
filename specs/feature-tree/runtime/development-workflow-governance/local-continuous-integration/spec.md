...
- 依赖：`environment-topology-and-packaging` 对 compile-only canonical 入口的定义；`quwoquan_ops/policies/local_readiness_contract.yaml` 的 scope_build 声明；Gradle 依赖的 precise-input cache 供给。
