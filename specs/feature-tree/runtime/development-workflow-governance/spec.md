...
迁移前仓库的 SDD 主线、特性树、命令、门禁和测试规则已经很多，但真正的问题不是“缺少规范”，而是**规范之间的层级模型不一致**：

1. **特性树曾混用多套层级**：索引、`acceptance.yaml`、脚本和命令文案同时存在 `L3_component`、`L4_detail`、`L5_leaf`、`L4_story` 等旧新混合语义。
2. **命令曾以四层/五层思维驱动**：`/explore`、`/prd`、`/design`、`/dev` 把 `L4` 当实施叶子、把 `L5` 当兼容层，导致新增特性和存量治理口径持续漂移。
3. **任务层和树层曾未彻底解耦**：历史节点把任务继续下钻成目录或树内计划文件，形成规格之外的第二真相源。
4. **测试层和特性树层曾相互污染**：主流程与 deploy 文案曾把 `L3/L4` 用作测试层表达，导致“层级”和“验证方式”混用。
5. **治理目标已冻结**：不再采用四层树，也不再保留兼容迁移层，仓库已切换到严格三层治理模型。
...
- R1.2：目录树保留到 `L3_story`；Task 只存在于当前会话、PR 拆分或外部执行台账，不进入正式特性树。
...
- R5.1：`tree_index.yaml`、tree index 生成器与相关校验脚本必须统一到三层模型，不保留第二套树镜像。
- R5.2：`quwoquan_ops/gate/scaffold/new_feature_fullstack.sh` 必须退出旧 `changes/` 五层脚手架模式，改为直接生成新的三层特性节点。
- R5.3：`quwoquan_ops/gate/scaffold/verify_feature_traceability.sh`、`quwoquan_ops/gate/scaffold/verify_feature_tree_refactor.sh` 等脚本必须改为零容忍旧层级。
...
- OS5：Task 不落树内文档或 `tasks.yaml`；本规格只冻结其“不进入正式特性树”的原则。
...
- 当前会话、PR 拆分或外部执行台账：短期 Task 执行状态；不得成为规格第二真相源

禁止行为：

- 新建树镜像，或在脚手架、校验脚本、命令文案中继续维护另一套与三层树不一致的层级定义
...
- 本次重构应在长期工作分支 `dev1.0` 的单一切换窗口完成，不允许半兼容状态进入主干。
- 合入前必须完成：
  - 新旧层级残留扫描
  - `tree_index.yaml` 重建与校验
  - 命令/规则/流程文案一致性校验
  - 代表性样例节点的三层迁移验证
- 若门禁或索引重建失败，必须撤销本次治理增量，不允许局部带病合入。
