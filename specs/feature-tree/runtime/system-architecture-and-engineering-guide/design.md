...
- 决策：L0 只运行无需安装且 p95 不超过 30 秒的 changed-file 快判（staged 对 HEAD）；L1/L2 与 `make verify-code-health-delta` 以 HEAD 与 dev1.0 的 merge-base 为 base 执行完整 delta，让已提交的 lane 分歧同样进入判罚，缺少 dev1.0 引用时 fail-closed；hosted 复算有两层：`04. Lane Gate` 在 lane PR 的 exact head 上以 `verify_code_health_delivery.py` 复算并作为 required check 阻断合入，`code-health-integration` 对每次 dev1.0 快进的 exact before/after 复算并发布 report-only OCI fact（5 分钟超时），后者是否成为 promotion required evidence 归交付链 owner；每周任务执行全仓 report-only 增长与 `change-frequency × health` 热点，以 OCI fact 保存历史、对照上期给出棘轮方向与 hotspot 连续在榜周数，不阻断 PR、不提交 snapshot。
...
- 决策：保留 PostgreSQL（PG）、MongoDB（Mongo）、Elasticsearch（ES）、Redis 与 Object Storage 五类引擎能力；它们是可选能力而非默认优先级。每个持久或运行态资源的引擎选择必须同时回答业务不变量、访问与更新形态、检索能力、保留期限、可重建性和失败后恢复来源，不能仅凭现有样板、数据长得像 JSON、团队熟悉度或某引擎已经接入来决定。Mongo 不再是新对象或新服务的默认目标；没有逐项选择依据时判否，不以 Mongo 先落地再迁移作为合法过渡。
- 决策：`database-agnostic` 的边界止于 spec、object、operation 与 typed port：这些层声明业务结果、对象身份、命令/查询语义、原子性与可见性要求，不出现驱动、表、集合、索引或物理集群概念。adapter、DDL、索引、查询计划与 migration 必须 engine-specific，并以真实引擎能力证明语义；不得用最低公分母 repository 抹平事务、并发控制、检索或保留机制。跨 engine 变更是受控发布迁移，包含源/目标版本、复制或重放、完整性 readback、切换与恢复步骤，不是运行期热配置，也不得在 admission 开放时替换 binding。
- 决策：逻辑资源身份从 `service/context/object/resource` 派生，不另建人工数据库名册。同一 `service × engine` 默认只拥有一个 namespace 与一个 typed slot，对象资源在该 namespace 内保持各自逻辑身份；只有隔离级别、数据驻留、保留策略、吞吐/容量、加密边界或故障域确有不同且能独立验证时，才新增表达该差异的具名 slot。扩容、换集群、endpoint 变化或暂时迁移本身都不是新增 slot 的理由，`primary`、`secondary`、`legacy` 之类只描述位置或阶段的名字不得成为长期逻辑身份。
- 决策：跨对象、跨服务以及权威对象到 projection 的一致性默认是 eventual。聚合内不变量由 owning object 在一次原子提交中维持；需要其他 owner 参与的同步裁决由 operation 在提交前经 typed port 完成，但该裁决不把远端读取或兄弟对象扩成同一事务，裁决后的写入、超时与并发失效必须有明确终态。relationship 的 `strong` 只允许表示 same aggregate / same commit，不承诺某数据库隔离级别、跨请求 read-your-writes（RYW）或 projection 立即可见；若 operation 对 RYW、版本栅栏或同步可见性有承诺，必须由 operation 单独声明并由 reader 实现，不能从 relationship 或 engine 名称推断。
- 决策：一致性语义按层唯一归属：spec 拥有调用方可观察的业务结果与可接受延迟；object 拥有聚合边界和聚合内不变量；operation 拥有同步 arbitration、命令幂等边界以及 query/read visibility；storage 拥有实际事务、CAS、锁、隔离、持久化和保留机制；projection 拥有事件 ordering、去重、checkpoint、freshness 与重放收敛。下层可以实现上层承诺但不得扩大承诺，上层也不得以 `transactional`、`strong` 或引擎名字虚构下层没有提供的机制。
- 决策：command 只经 owning object 的 typed writer 写入其权威资源，聚合提交与本聚合发出的 durable publication 同属该对象的提交语义；跨 owner event 消费和 projection 更新在提交后收敛，不伪装成分布式原子提交。query 必须选择与 operation 可见性承诺相符的 authoritative reader 或 projection reader。幂等窗口与去重事实由 operation 的重试边界和 storage 的真实原子机制共同保证；Redis 可以承载带明确 TTL、容量上限、丢失后终态与权威恢复来源的 session、lease、rate-limit、短期去重等有界运行态，但不得承载长期领域权威、不可重建历史事实或唯一恢复依据。
- 决策：物理 binding 只在部署期由 composition/Ops 将逻辑 typed slot 解析为具体 namespace、endpoint、credential 与 engine adapter，并把已解析句柄交给 adapter；spec、object、operation、domain/application 代码和 App 均不得出现 `clusterRef`、selector、节点地址或按环境选择集群的分支。缺失、歧义、类型不符或与发布清单不一致的 binding 在开放流量前 fail-closed，服务不得以默认本地实例、另一 slot 或旧集群继续运行。
- 可判定规则（contract gate 执行位，`make -C quwoquan_service verify-metadata` 的 commercial ratchet 阻断）：对象触达的物理引擎数大于 1 时必须在 `storage.yaml#resources` 逐引擎声明具名逻辑资源（含自身权威引擎），否则 `CONTRACT.STORAGE.RESOURCE_DECLARATION_REQUIRED`；声明 `engine: elasticsearch, role: query_projection` 的对象必须有一个 `projections/*.yaml` read_model 给出完整 `consistency_policy`（`source_version_field`、`apply_mode: strictly_newer`、`delete_mode`、`rebuild_strategy`），否则 `CONTRACT.PROJECTION.CONSISTENCY_POLICY_REQUIRED`；`transaction.mechanism` 与 `guarantees` 词表必须对应、participants 必须指向真实存在的兄弟集合，否则 `CONTRACT.STORAGE.TRANSACTION_*`；`redis_cache` 的 `fence: token_value / current_counter` 必须成对且共享 `hash_tag` 与 `atomic_keys`（`CONTRACT.STORAGE.REDIS_FENCE_PAIR_INCOMPLETE`），`durability: ephemeral` 且 `eviction_consequence: fail_closed` 的键必须在 description 写明恢复来源（`CONTRACT.STORAGE.REDIS_RECOVERY_SOURCE_REQUIRED`）。Elasticsearch 写传输只保留 `runtime/search/es.VersionedWriter`（`UpsertVersioned` / `TombstoneVersioned`），无版本 Upsert/Delete 面已删除并由静态防回潮测试守住。
- 理由：引擎多样性有价值的前提是每种机制服务于可说明的不变量；把 Mongo 当默认库或把所有存储包装成同一种 repository，只会把真实差异推迟到事故与迁移时暴露。逻辑身份与物理 binding 分离后，常规扩缩容和集群替换不改变业务合同，而真正改变事务、查询、保留或恢复语义的跨 engine 迁移会被正确升级为发布事件。
- 引擎选择反例：因为 payload 是 JSON 就选择 Mongo，而对象实际需要跨记录唯一约束与严格事务裁决；因为需要全文检索就让 ES 同时成为不可重建权威；因为访问很热就把长期事实只放 Redis；因为 Object Storage 成本低就让高频条件更新与逐记录事务直接依赖对象扫描；因为 PG 能保存 JSON 就忽略检索、保留与大对象访问形态。以上都只命中单一表象而没有闭合不变量、访问、检索、保留与恢复，必须改为有依据的权威存储加必要 projection/asset 组合，或判否。
- 被否决方案：统一迁到单一数据库以获得“标准化”；它把标准化误放在物理机制，牺牲检索、对象资产、短期运行态与不同保留期的真实需求。被否决方案：保留 Mongo-first 脚手架再允许服务自行例外；默认值会吞掉逐对象裁决，例外清单则成为第二注册表。被否决方案：用配置开关在两种 engine 间热切或长期 dual-read/dual-write；它让同一事实同时拥有两个权威、无法判定回滚方向，并把迁移完整性问题隐藏成流量比例。
- 失败、恢复与回滚：选择依据或一致性 owner 不完整时由架构/契约门返回 `GATE_BLOCK`；部署 binding 无法唯一解析时保持 not-ready 且不开放 admission；迁移复制、重放或 readback 出现差异时停止切换并保持源 engine 为唯一 writer；提交已成功而 projection 延迟时呈现“已提交但尚未达到声明可见性”的真实状态，由 checkpoint 重放收敛，不把已提交写入改报失败。跨 engine 迁移始终只有一个 canonical writer：切换前可回滚整个 release 并继续使用源端；切换后若已有新写入，只能执行经完整性证明的反向迁移或前向修复，不得靠热改 selector 指回旧端。Redis 有界状态丢失时按该状态声明的 fail-closed 或从权威来源重建路径恢复，不得合成长期领域事实。
- 观测与 SLO：观测维度使用逻辑资源身份、typed slot、engine 与发布/binding digest，物理集群细节只留在部署与 adapter 遥测。至少分别观测 command 提交延迟/失败/冲突与重试、query 可见性违约、projection lag/乱序/重复/checkpoint 停滞、Redis 过期/驱逐/容量压力、保留与容量逼近、migration copy/replay/readback/cutover。operation 与 projection 的延迟预算来自其 spec 可观察结果，不在本 DEC 复制统一数字；binding 在 admission 前唯一解析率和迁移切换前 readback 一致率均必须为 100%，任一不满足即阻断。持续超过各 owner 声明预算、projection 不再收敛、Redis 无恢复来源或 binding digest 漂移均告警到对应 service/context/object/resource owner。
- 测试 seam：`local_contract` 用 engine-choice policy 的正反例、逻辑身份派生、默认 namespace/slot 与具名差异、typed port、relationship `strong` 的窄语义、Redis 有界性和 App/应用层零物理 selector 证明分层；`api_integration` 在真实 PG/Mongo/ES/Redis/Object Storage 边界证明所选事务/CAS/索引/检索/TTL/保留机制、部署 binding、engine-specific migration、readback、单 writer 切换以及 projection 重放/ordering/freshness；涉及调用方可见延迟与恢复终态时由现有 `SIT-001`、`SIT-002`、`SIT-003` 的结果证据承接。测试替身只能实现 typed port，不能以 in-memory 行为证明真实 engine 的隔离、持久化、检索或迁移语义。
- 约束与影响：本决策不复制具体字段 schema，不冻结每个对象的引擎清单，也不建立中央 storage registry；选择与 binding 从当前对象资源声明、服务身份和部署清单派生。影响 Story：[`app-cloud-business-object-commercial-closure`](./app-cloud-business-object-commercial-closure/spec.md) 承接对象 storage contract、typed port、真实 adapter/migration、投影可见性与端云零物理 binding 泄漏。
- 关联要求：`REQ-001`、`REQ-002`、`REQ-003`
- 关联验收：`SIT-001`、`SIT-002`、`SIT-003`

<a id="dec-034"></a>
### DEC-034 标识符描述角色，可变闭集的基数只存在于唯一声明源

- 决策：手写源码与测试的标识符只描述角色或能力，不把可变闭集的当前成员数写成身份。判据是一句可回答的话：该数字变化后操作语义是否仍成立；仍成立即属于可变基数，必须移出标识符。适用面是方法、函数、类型、属性、变量、测试名、错误码与 CLI flag，不含规格散文。
- 决策：改名与发现是同一条要求的两层，缺一仍是耦合。名字不含基数只解决可读性；消费方还必须按唯一 canonical 源发现成员，不得在函数体内平行维护一份与该源等价的成员表。两者只做前者时，那张平行表就是第二真相源，闭集变化时它不会报错，只会静默给出旧答案。
- 决策：豁免边界按「它是身份还是当前计数」划分，不按字面是否含数字划分。冻结协议 id（`qwq.three-layer-case-results`）、已发布节点目录名（`four-environment-commercial-login-maturity`、`three-layer-evidence`）、契约字面量与分桶枚举成员（`two_to_five`）都是身份，改名等于改契约，不在本决策约束内。三层测试金字塔是冻结的证据协议，环境集合是可增减的部署闭集，两者不同类。
- 理由：first instance 是实测的漂移而非假想。`four_env_build_images` 名字声明四环境，实现遍历的却是五个 runtime target（三个本地、`prod-sim` 与 `prod-hosted`），并在函数体内硬编码了一张 target 到 runtime manifest 的映射表——该表与 `quwoquan_ops/cli/lib/environment_topology.py` 已声明的闭集重复。名字、实现与真相源三者已经不一致，而新增环境本不应该改动这个函数的签名。
- 理由：本决策与 [`REQ-001`](spec.md#req-001) 的「数量由契约扫描派生，不在规格、门禁或注册表重复登记」同源，只是作用面不同：那条管规格不冻结数量，本条管标识符不冻结数量。同一个坏味道在两个平面各有自己的落点，合并会让规格扫描规则与命名规则互相污染。
- 被否决方案：为本决策建全仓词法扫描加豁免名单。扫描 `two|three|four|five` 一类前缀会对上述身份类标识符产出压倒性误报，而为它们建名单与 [DEC-031](#dec-031) 禁止路径豁免的裁决直接冲突；先加扫描再加名单换来的是一个会被绕过的绿灯。确定性执行位的缺口按 Story `OPEN-015` 承载，不以名单形式提前兑现。
- 被否决方案：把基数换成 `all_`、`every_` 一类全集前缀。`all` 仍暗示调用方知道全集边界，且对 `four_env_build_images` 这种真实处理 target 而非 env 的函数，换前缀不会纠正被搞错的那一层抽象。
- 约束与影响：本决策对增量新增或重命名的标识符强制，存量随触及文件收敛，不建立改名台账。唯一执行位是 developer 主审 checklist 的客观 `check:`；它不产生确定性 terminal，因此开发期零 Reviewer 的 POST 不检查本条，该缺口在 `OPEN-015` 显式保留，不得以「已有 checklist」声称已闭环。
- 关联要求：`REQ-009`
- 关联验收：`SIT-008`
