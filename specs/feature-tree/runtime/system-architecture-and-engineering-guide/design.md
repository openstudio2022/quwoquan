...
- 决策：Go 的指针面只减不增。`infrastructure` 可用 `*T` 与 `sql.Null*` 承载列可空事实，需要三态的写入面使用专用 change set。
- 决策：`domain`、`application` 与出站 DTO 的可选标量使用值类型，缺席以省略键表达。
- Wire 约束：HTTP 边界上的值类型 `bool` 禁止 `omitempty`，因为它会让在场的 `false` 表现为缺席；必填字段还会因此触发端侧 fail-closed 解码失败。
- Wire 约束：确需区分「未设置」与「false」时使用 `*bool`，由指针准确表达三态。
...
- 被否决方案：全局禁用可空类型、把可缺字符串统一塌陷为空串，或把全部 `NULLABLE` 标量映射为 Go 指针。
- 被否决方案：引入 `Result`/`Either` 第二套错误模型，或在字段级裁决前由生成器批量删除契约声明的 `default`。
- 被否决方案：把 `catch` 内 `return null` 一律判死，或按目录名近似出站边界。
- 被否决方案：对出站列表批量删除 `omitempty`，因为 nil 切片会转而序列化成 `null`，仍违反同一要求。
...
- 关联要求：结果四态与门禁可证伪对应 `REQ-001`、`REQ-002`。
- 关联验收：`SIT-001`
