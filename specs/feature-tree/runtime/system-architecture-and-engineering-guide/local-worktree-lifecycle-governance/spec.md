...
>
> 设计引用：[L2 DEC-026](../design.md#dec-026)
...
- hook 面的任何阻断（deny/ask）。执行面 hook 只注入上下文，判断权留给执行体；hook 运行在开发者本机且执行体有权改写环境变量与仓内文件，本就不构成安全边界。真正的硬门只在准出：lane `make accept` → integration 消费 bundle 发布 `origin/dev1.0`、交接、发布。
...
- canonical lane worktree 形态：显式 `-b <fixed lane>`、显式 path 与 start-point，start-point 解析为 fetch 后冻结的已发布 `origin/dev1.0` exact SHA（不存在或不可证时阻断，不回落本地未发布 `dev1.0`），本地 lane upstream 指向 `origin/dev1.0`，不使用 `--detach`、force 与 force-create；可从任意 worktree 发起，不要求当前 worktree 在 `dev1.0`。非 canonical 形态只附带模板提醒（`OPS.WORKTREE.INVALID_ADD`），不阻断；创建后的 lane clean/HEAD 一致性由准出门禁 `verify_local_worktree_lifecycle.py` 验证。
...
- 事实必须归属到真正持有它的工作副本。stash 存放在仓库 common dir，全部 linked worktree 与主 worktree 共享同一份，因此 stash 只归属独立 clone；未合入提交必须以主仓库已发布 `origin/dev1.0` 的可证明 exact SHA 为基准判定，不得采用本地未发布 `dev1.0` 或副本自身可能陈旧的远程引用；authority 不可证时如实诊断，不能声称已同步。错误归因会让刚创建的干净副本立刻显示为已滞留，几次假报警之后整条提醒就不再被阅读。
...
- `worktree_policy.yaml`是物理布局唯一真相源：project root下必须恰有一个bare hub、一个可编辑、可提交且仅允许在持有 acceptance publish admission 时把匹配本地`dev1.0` non-force fast-forward到远端同名分支的`integration/ -> dev1.0`，以及六条仅持本地检出/验收 identity 的 lane worktree，upstream 统一指向 `origin/dev1.0`。同步/回同步冻结已发布 exact SHA，只 FF 安全祖先，分叉、脏重叠或进行中操作零写；不传播本地未发布 dev，不推远端 lane。分支闭集和其余更新通道只读 `branch_policy.yaml`，路径 ownership 只读 `lane_ownership.yaml`，不得复制。
...
- 默认门禁验证已发现 lane 的路径身份并单独要求唯一 integration clean；全量身份门必须精确六条 lane 均 clean 且 HEAD 等于该次冻结的已发布 `origin/dev1.0` canonical exact SHA（不存在或不可证明则阻断，不回落本地 dev），integration 也必须 clean 且同 HEAD。
...
- 设备与local-runtime属于跨worktree共享资源，互斥锁必须host-scoped；holder evidence至少包含`pid/worktree/lane/head`。`integration/ -> dev1.0`可编辑、提交并按branch policy在持有 admission 时从匹配本地ref执行non-force fast-forward；push不得force/delete、写`main`或写远端`lane/*`，且只提交已验收源码；env=1 不证明 admission，`integrationEligibility` 由 exact admission 建立，不由 push 或 runtime 承载推导，push 不重新签发 Alpha/Beta/Gamma、`IntegrationQualificationFact`、promotion、release 或 Prod authority。承载runtime时必须显式声明host身份。
...
- AND 该判定只在显式运行门禁（本地 `make verify-local-worktree-lifecycle`、准出 CI）时生效，不挂在任何执行面 hook 或普通 commit gate 的无条件 static checks 上；改动 worktree 治理实现/策略时，commit gate 只选择 lifecycle focused local_contract。门禁 recovery 要求长期 lane 本地 fast-forward resync（不推远端 lane）并保留 worktree，clone 或额外废弃副本才由人工决定是否删除。
- AND 设备与local-runtime锁跨worktree互斥并产出holder evidence；integration可按scoped candidate协议编辑，也可在持有 admission 时从匹配`dev1.0`执行non-force fast-forward，但不能因承载runtime获得force/delete、写`main`、写远端`lane/*`或任何集成/晋级/发布资格。
