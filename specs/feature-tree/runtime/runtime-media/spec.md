...
2. 用户头像与群头像有清晰业务归属
3. 群头像采用服务端预合成，对外统一下发非空 `avatarUrl`
...
- App：消费统一 `avatarUrl/sync patch`，不推导底层存储路径
...
- **用户头像**：长期保留当前有效版本，过往版本本延迟清理；数据库主存 `avatarAssetId + avatarVersion`
...
- 第二阶段：`chat-service` 产出服务端预合成 `avatarUrl/groupAvatarVersion`
- 第三阶段：App 切换到只消费 `avatarUrl`
...
- 若群头像预合成失败率异常，服务端保留上一版 `avatarUrl` 并暂停推进新版本
