...
**环境**：视频矩阵与群头像相同，依赖 **阿里云 ECS gamma onebox**（见 `deploy_gamma_ecs.sh`）与 **本机或 self-hosted Runner** 上的 Flutter/Patrol；`GAMMA_BASE_URL` 必须为 **Caddy 网关端口**。路由自检：`python3 scripts/verify_gamma_public_gateway_routing.py`。
