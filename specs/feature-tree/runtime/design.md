...
<a id="dec-006"></a>
### DEC-006 手机页面方向是全局不变量，不是媒体租约

- 决策：runtime L1 持有 App 自有手机页面固定 `portraitUp` 的唯一写入口。Android 启动门与主 Activity 在原生声明正向竖屏，透明回调继承后方 Activity；iPhone 原生支持方向只声明正向竖屏。Flutter bootstrap 在配置水合及首帧之前执行同一固定请求，并由 bootstrap 只登记一次的全局方向 observer 在 resumed 上重申该请求；热重启重新建立同一入口。
- 决策：方向命令无可变参数、传感器查询、历史快照、媒体 lease 或恢复队列，所有并发/迟到请求都只能携带同一不可变方向集合。媒体内部旋转、页面与路由状态不依赖方向 API 完成或窗口方向读回；不修改相机 capture orientation、EXIF 或视频编码元数据。
- 理由：全 App 手机页面方向是跨业务壳层不变量；以媒体进入时观测值恢复会让退出、前后台和迟到回调重新放开方向。删除可变写轨比在每个媒体回调补 fence 更直接可测。
- 被否决方案：四方向基础策略加临时横屏锁、按传感器恢复原方向、为媒体旋转请求系统横屏、吞掉平台失败当作 confirmed、为系统弹窗强制旋转，以及擅自关闭 iPad 多任务或绕过 Android 大屏平台限制。
- 失败恢复与预算：固定请求最多等待 2 秒；平台异常或超时保留为异常，bootstrap 复用既有启动失败恢复，resumed 异常进入 Flutter 统一错误观测。重试仍只写正向竖屏；迟到完成不安排补偿/解锁请求，原生启动声明始终不变。不用方向循环轮询阻塞首帧或业务页面，也不把请求完成记录为真机方向成功。
- 回滚：原生声明与 Flutter 入口作为同一候选一起回滚，不单独恢复旧租约或动态方向队列；设备验证失败保留首个 blocker 与 `OPEN-005`，不以平板被系统忽略请求为手机策略成功证据。
- 可测试观察面：`local_contract` 截获真实 `SystemChrome` 平台消息验证唯一 `portraitUp` 参数，使用真实生命周期通知覆盖 resumed，用延迟完成/异常/超时与重入证明不存在历史方向重放；原生 XML/plist 和 bootstrap 前后顺序另作源码契约检查。设备观察指标为手机横放启动至 Flutter 首帧期间非正向竖屏帧数（目标 0）及页面/前后台/热重启后非正向竖屏次数（目标 0）；本地消息测试不签发这些设备事实。
- 关联要求与验收：[`REQ-003`](spec.md#req-003)、[`DOM-002`](spec.md#dom-002)、[`DOM-003`](spec.md#dom-003)、[`DOM-004`](spec.md#dom-004)。影响既有启动壳与媒体展示协作者，不转移内容模式、素材或系统 UI 的 owner。
