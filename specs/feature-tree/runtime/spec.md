...
- Ops：`Makefile`（仓库级薄构建、测试与门禁编排门面；不拥有其调用的 Feature 行为、契约或测试）
...
- 完成判定：形成可观察的 Skill owner、发布状态机和回滚验收后，再建立独立 L2/L3，且新增节点后 `DOM-001` 的 L1/L2 与共享 runtime 包工程映射子句仍然成立。
