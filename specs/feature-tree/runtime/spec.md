...
- Agent：`AGENTS.md`（全仓执行不变量）、`specs/feature-tree/README.md`（Feature Tree 结构与 owner 算法）、`.agents`（Workflow Skill 唯一 authoring source/宿主发现面与 Reviewer 中性真相源）、`.codex`、`.cursor`（仅两宿主的一行命令入口与 Reviewer projection；Workflow 语义只归 `.agents/skills`）
