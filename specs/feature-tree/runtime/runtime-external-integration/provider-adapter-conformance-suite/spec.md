...
- 对任一 target `t`，required 集合 `R_t` 必须由执行器重新解析当前候选 generated target Binding 的 exact bytes，取 `required=true` Capability 后按 canonical identity 去重排序；预期 cell 集合由 `R_t × applicable layers` 动态派生，规格、聚合器与 gate 均不得固化 Capability 名单或数量。同一环境本次命令新产生的全部预期格若绑定该环境 active immutable candidate、由测试拥有的 CaseResult/cleanup/observability 可复核，则可满足该环境的 Debug-local functional readiness；该结论不要求 CI HMAC，也不得提升商业或发布 readiness。
...
- Debug-local functional readiness 只消费当前 `stackctl provider-conformance --environment-matrix` invocation 按 `R_t × {local_contract,api_integration,user_acceptance}` 新生成的全部预期格；聚合结果必须显式声明 `readinessScope=local_functional`、`releasePromotionClaimed=false`，且 `attemptEvidenceCount.executed` 精确等于本次动态 expected cell count。历史输出、其它环境 evidence、重复/额外 cell 或缺 active candidate receipt 任一出现均阻断当前环境，不得为了复用历史结果扫描聚合成通过。
- Alpha/Beta/Gamma 全部动态预期格若与 iOS Simulator + Android Emulator 的 `emulator_only` matrix 同轮绑定，只能支持 `ALPHA_BETA_GAMMA_EMULATOR_ONLY_FUNCTIONAL_GREEN`；该回执必须 `nonPromotable=true`，不得补写 Android 真机覆盖、不得关闭正式 provider-release 或 Prod Remote blocker。
- `prod-sim` 必须从其 package-bound generated target Binding 动态派生 `R_prod-sim`，运行完整第一方服务拓扑和 target-scoped Port 对等 protocol substitute/local infrastructure；每个 required Capability 至少有一次经其 owner 第一方 Service/Integration 黑盒入口到 Provider 的非零调用，并同源产出 invocation、effect/readback、cleanup 与 logs/traces/metrics evidence。结果只能标记 `readinessScope=local_rehearsal|local_functional`、`releasePromotionClaimed=false`、`nonPromotable=true`，且不受 legal-static 占位是否消除影响。
- 正式 provider-release producer 必须分别从 Alpha/Beta/Gamma 与 `prod-hosted` 的 generated target Binding 动态派生各自 `R_t`：非生产 target 对每个 required Capability 执行三层，`prod-hosted` 对每个 required Capability 执行 native/operator `user_acceptance`。正式 expected cell count 精确等于 `Σ(nonprod |R_t|×3) + |R_prod-hosted|`；缺失、重复、额外、legacy evidence、local trust 或非 active receipt 任一出现即阻断，run-attempt 使用独立输出根，禁止迁移历史 evidence。
- `prod-hosted` cell 只接受真实厂商 Provider、真实凭据与正式 hosted rollout/health/switch/callback-drain/last-good/rollback receipt，必须拒绝 fixture/protocol substitute/local infrastructure；`prod-sim` evidence 永不提升 Prod Adapter/Capability readiness，也不能关闭该 receipt 缺口。

<a id="req-003"></a>
### REQ-003 假报告、动态跳过、输出越界和敏感信息均 fail-closed

- 所有负例有自动化测试且 gate_repo/CI 执行同一检查。每个 expected cell 必须包含至少一次实际 Provider invocation；零调用、required skip、旧 receipt、只探测 `/healthz`、缺 effect/readback、缺 cleanup 或缺 logs/traces/metrics 任一出现均判 FAIL。
...
- THEN 聚合器重新解析各 target 当前候选的 generated target Binding，并证明实际 cell key 集合与动态 expected 集合完全相等；不得以固定 Capability 数量、缺失、重复或额外 cell 通过，且每格 Provider、网络边界、数据和环境语义匹配。
...
- AND 当前环境本次 invocation 的全部动态预期格在 active immutable candidate、selected Binding、测试源、CaseResult、cleanup 和 observability 全部同源时，允许以 `local-sha256` 满足该环境 functional readiness，且无需 CI attestation key。
- AND 三环境全部动态预期格与双模拟器 UAT 即使全部通过，也只能形成独立的 emulator-only non-promotable claim；正式准出仍等待 Android 真机、CI-attested provider-release 全量动态 expected set 与 Prod Remote receipt。
...
- WHEN 对 `prod-sim` 执行 package-bound Provider rehearsal。
- THEN 从 `R_prod-sim` 动态派生的每项 required Capability 都必须经完整第一方 Service/Integration 黑盒入口实际调用 target-scoped Port 对等 Provider，并取得同一 attempt 的 invocation、effect/readback、cleanup 与 logs/traces/metrics；expected invocation count 为零、required skip、旧 receipt、重复补格或只探测 `/healthz` 均失败。
- AND `prod-sim` 结果固定为 `local_rehearsal|local_functional`、`releasePromotionClaimed=false`、`nonPromotable=true`，不得生成或提升 Prod Adapter/Capability readiness。
...
- WHEN 执行生产商用准出。
- THEN `R_prod-hosted` 的每个 required Capability 另有一个绑定真实厂商 Prod Provider Workload、真实凭据、Prod environment artifact 与 hosted topology 的 Remote `user_acceptance` receipt，且包含正式 rollout/health/switch/callback-drain/last-good/rollback readback；不接受 Alpha/Beta/Gamma nonprod matrix、`prod-sim`、fixture 或 local workload 作为替代。
- AND 正式 artifact 的 actual cell key 集合与 `Σ(nonprod R_t×3) ∪ R_prod-hosted` 动态 expected 集合完全相等；`provider-conformance-readiness` 的 `issues/sourceCoverageIssues` 均为空，每个 target Binding 的 required Capability 均达到其对应 scope 的 readiness，全部 raw evidence exact bytes 由 manifest/finalizer 收集并由 environment-stability final acceptance 重新推导验证。
...
- 优先级：`P0`
- 准出影响：`block`
- 影响或价值：由各 target 当前 generated Binding 动态派生的全部 executable source 必须被覆盖且 `sourceCoverageIssues=[]`，不得在规格或聚合器中固化会随 Capability/Adapter 变更而漂移的 source 名单或数量。Prod `user_acceptance` 均须有真实 harness，push/message/RTC 另含 Provider two-device native readback。当前真正缺口是 Alpha/Beta/Gamma 尚未在同一 active candidate 上执行出各自完整动态 expected set，`prod-sim` 尚未逐 required Capability 形成完整第一方黑盒 invocation/effect/cleanup/observability local rehearsal，正式 provider-release 也尚未执行出 CI-attested 全量动态 expected set，且没有真实厂商 Prod hosted Remote receipt，因此 local Green 与 `gate-release` 均必须阻断。source/harness 存在不得冒充执行通过。
...
- 完成判定：`GWT-002` 对应行为满足；每个 target Binding 动态枚举出的 required Capability/Adapter/layer 都有自描述原生 harness，在同一候选版本完成 Alpha/Beta/Gamma 动态九格、`prod-sim` local rehearsal 与真实厂商 Prod hosted Remote receipt，并通过 `--require-ready gamma` 与 `--require-ready prod`。
