...
- 取赞撤销此前同一关系的当前贡献，包括按原贡献口径衰减后的剩余部分，不是固定减一次，也不是 dislike 或新的负反馈。合法历史训练事实按所属合同保留，不将当前 coLiked 与行为历史混为一谈。
...
- 证据层：local_contract 证明归因/贡献 oracle；api_integration 必须证明 production 自动 worker、持久幂等收据与真实读回，手动 drain 或 UI 乐观态不能替代。
...
- 证据层：local_contract 验证版本交错；api_integration 绑定实际候选、评分输入、自动 consumer 及源提交证据；负载未冻结或证据缺席时不得判达标。
...
- 证据层：local_contract 覆盖时序与预算；api_integration 验证真实 User 确认、consumer、窗口续接及权限裁决；端侧行为由 Feed 的集成验收补齐。
...
- 完成判定：[SIT-002](./spec.md#sit-002)、[SIT-003](./spec.md#sit-003)、[SIT-004](./spec.md#sit-004) 均由同候选真实测试直接绑定完整 `spec_ref`；各 owner 完成事件/来源版本/因果读面 authoring 与生成，真实 worker、候选和 scorer 输入读回、窗口安全续接及声明负载新鲜度全部通过。仅源码或合同通过不能关闭。
