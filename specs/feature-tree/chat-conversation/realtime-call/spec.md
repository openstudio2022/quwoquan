...
- Provider 请求、厂商结果与设备展示分别以事务 outbox、幂等 inbox 和 presentation ACK 记账；双设备 Provider UAT 必须把两次反向真机呼叫的 delivery timeline、Realtime、唯一 Chat 通话记录和终态 QoE 绑定到同一组 call digest。
