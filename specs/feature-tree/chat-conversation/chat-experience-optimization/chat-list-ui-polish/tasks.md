...
- [ ] M1: 更新 `quwoquan_service/contracts/metadata/messages/conversation/projections/chat_inbox.yaml`，补 `mentionUnreadCount`，并确认 `avatarUrl` 是唯一头像字段 → A2 A5 A8 A10
...
- [ ] B5: inbox 头像只消费服务端 `avatarUrl`，不接入端侧九宫格拼图 → A2 A5
...
- [ ] P2: 群成员头像变更驱动服务端重算 `avatarUrl` 并通过头像 patch 刷新
