...
> ⚠️ **命名回滚（`00_GLOBAL_TERMINOLOGY.md` §18.6）**：chat-conversation 群会话前台词由 WP5 的「讨论」回滚为 **「群聊」**。本文档中作为「多人群会话」语境出现的「讨论」（消息/联系二级胶囊、群会话主页、群管理文案）一律读作 **「群聊」**；技术对象名（`GroupHome` / `CircleGroup` / `Discussion`）与 wire/filter 机器值 `group` 不变。仍保留「讨论」的是：实体/圈子**内容**讨论分区、全局检索聚合分区、交集「共同讨论」。
...
- 群聊：仅展示已加入的群聊，不展示聊天内容或公告，按最近活跃排序。
