...
- 优先级：`P0`
- 准出影响：`block`
- 影响或价值：当前仍缺 `assistant.public.search` 等 Provider 的 conformance 执行收据，以及真实 Provider 与 Android/iPhone 真机执行收据。material 已按防腐层规范以 `ext.search.protocol_fixture` 等登记进 alpha `externalBindings`。`web_search/web_open/web_find` 已由 canonical Tool Catalog 注册并接入 AgentLoop。Alpha canonical 启动与 health 29/29 已取得（run `20260813T160518410673Z-d38bed3e0945497980753e19bcb051e5-health-alpha-local`）。剩余缺 Gamma 同 digest 与全量 conformance 执行回执。
