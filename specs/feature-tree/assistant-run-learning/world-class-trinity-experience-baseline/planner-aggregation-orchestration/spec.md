...
<a id="open-001"></a>
### OPEN-001 编排与聚合裁决尚未实现

- 类型：`capability_gap`
- 优先级：`P1`
- 准出影响：`track`
- 影响或价值：尚缺编排与聚合裁决实现。当前下一步动作只有调用工具与回答两种可能，缺少向用户反问的路径，因此关键信息缺失时会直接硬答。重规划判断仅在工具观察为空时触发，不评估证据充分性。执行记录、子代理计划与聚合状态的契约均已定义但无实现，多领域问题无法分头处理，过程事件类型也不足以表达编排阶段。
- 完成判定：`GWT-001`、`GWT-002` 与 `GWT-003` 由真实测试直接 `spec_ref`。
