...
- 已收敛：assistant 手写树的同名镜像/死副本已全部清零（含 `citation_destination_resolver.dart` 曾经的 domain/public 双真相源分叉，runtime 真相为 application/public 版）。现行判据：同名或疑似副本必须以精确解析 import 指向确认零引用，并用 git 状态与 mtime 排除并行会话活跃区后才可删除；禁止保留「暂时并存」的第二份。
- 剩余方向是自底向上的真 typed 化三路径，另有两项受限。
  ...
  - Row 开放袋已全部收敛，业务弱袋归零——退役键（`dialogueState`/`uiActions`/`assistantBoundaryOutcome`）由 `kTranscriptRetiredKeys` 承载丢弃语义防止经 `extra` 回流；`uiUsageStats` 为 typed `AssistantUiUsageStatsViewData`（`toProtocolMap` canonical 序列化出口）；`uiReferences` 为 `List<AssistantCitation>`（`fromReferenceMap`/`toReferenceMap` 单轨编解码，解析失败 fail-closed 丢弃）；`runArtifacts` 为 generated `RunArtifacts?`（含 `presentationDocument: AssistantPresentationDocumentWire?` 与 partitioned diagnostics，结构解析收敛到 Codec 边界 fail-soft 置 null，bubble / run lifecycle / answer content 直读 typed 字段，projection 写入 typed 构造、运行期诊断键按 partition 机制落 extensions 登记扩展槽）。Row 仅剩 `extra` 一个弱 Map，为协议开放兜底槽。当前棘轮计数 map 146 / Object? 20 / bare 126，只减不增。
  - `Map<String, Object?>` 桶 20 处已完成合法驻留判据裁决，分两列：合法驻留 18 处——skill 配置槽 9 处（契约 `skill_user_setting/fields.yaml` 的 `nData` 声明为「仅按 nSchemaDigest 指向的 active package schema 解释的 canonical JSON 文档」，端侧动态表单 bag 属契约登记的动态 schema 开放槽）、presentation action dispatcher digest 4 处（`SplayTreeMap` wire payload 规范化算法）、citation wire 解析入口签名 2 处、learning fact outbox generated encoder 输出边界 2 处、stream event wire 列表解析 1 处；待收敛残量 2 处——`task_graph_contract.dart` 的 serde，随 TaskGraph schema 收编消除。驻留清单不得当作预算容纳新债：新增 `Map<String, Object?>` 仍触发棘轮回归。
  - 后续方向：`TaskGraph`/`SessionOrchestratorState`/`TurnSynthesisState` 的 schema 收编；`understanding_snapshot_codec.dart` 与 `aggregation_state.dart`/`profile_update_proposal.dart` 手写解析器。另两项受限：`run_artifacts_map_partition.dart` 唯一调用方是 generated `run_artifacts.g.dart`，改签名需 Go 生成器协同；手写 `TaskGraph` 与 `AssistantTaskGraphWire` 存在双轨，合并需产品裁决。
