...
<a id="open-001"></a>
### OPEN-001 提案创建的可审计输入与审核边界

- 类型：`capability_gap`
- 优先级：`P1`
- 准出影响：`track`
- 影响或价值：仍缺 canonical contract 和 user-service 聚合记录把 change reason、source evidence、impact scope、owner/operator context 固化为同一可审核提案。现有状态机和界面只能确认或拒绝既有记录。
- 完成判定：metadata-first proposal create command 以稳定 clientRequestId 幂等创建 owner-scoped record。record 持久化 typed reason、evidence refs、impact scope、review status 和 trace/actor audit context。local/API/App 证明重复提交不重复创建、非法或跨 owner 输入 fail-closed、审核页展示同一事实且无伪成功。`GWT-001` 对应行为满足且真实测试 `spec_ref` 有效。
