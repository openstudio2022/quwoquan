...
<a id="open-001"></a>
### OPEN-001 原子应用、审计快照与可控回滚

- 类型：`capability_gap`
- 优先级：`P1`
- 准出影响：`track`
- 影响或价值：当前 apply 路径未以 canonical record 同时持久化 pre/post snapshot、actor/trace、target version、rollback window 和结果 receipt；响应丢失或重试时不能证明恰好一次应用及可审核回滚。
- 完成判定：apply 使用 CAS 和 clientRequestId receipt 原子写 target change 与 immutable audit record。重复 apply 返回同一 receipt，失败不写半成功事实。rollback 仅对可回滚且版本匹配的应用恢复 pre-snapshot 并生成新审计记录。local/API/App 覆盖 response loss、owner/权限拒绝、快照差异和恢复。`GWT-001` 对应行为满足且真实测试 `spec_ref` 有效。
