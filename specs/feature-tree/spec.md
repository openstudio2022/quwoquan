...
- Alpha/Beta/Gamma required 验收绑定受管非生产租户的非内存 Provider，Prod 完成正式 Provider、实时 SLO、灰度和回滚验证；任何环境 App 均不含 seed/Mock/Memory/Noop 或故障数据源切换；Alpha 离线的公开许可内容不模拟这些真实 Provider，本地演练结果必须标注模拟且不得签发在线资格。
...
- App 在线访问只经统一 Gateway base URL 和 generated operation，不感知服务进程、存储或内部 URL；Alpha 离线 typed adapter 读取制品快照并在本地演练空间执行 App 可达 command，不假设 HTTPS endpoint、不伪造可发往真实 transport 的凭据。
...
- THEN command 经过 aggregate owner，query 读取 named Slice；App 在线只访问 generated Gateway operation，Alpha 经相同 typed ports 读取 canonical 快照并在本地演练空间提交 App 可达 command，不产生伪远程写入。
- THEN 每条 Journey 至少跨两个真实业务对象，并验证权限、错误恢复、幂等、副作用、投影收敛和推荐/运营回流。
- THEN 所有页面通过 light/dark、多屏、无障碍、语义 token、性能、弱网和 capability 降级检查。
- THEN Alpha 离线与 Beta/Gamma/Prod Remote 通过同一页面及 typed ports 返回 canonical 内容、Creator、实体和媒体；source 仅在组合根选择，离线 source 身份不冒充服务端 active release，具体等价与能力差异按 environment-topology-and-packaging GWT-007/GWT-008 验证。
...
- AND Alpha/Beta/Gamma required 验收绑定受管非生产租户的非内存 Provider，Prod 完成正式 Provider、实时 SLO、灰度和回滚验证；任何环境 App 均不含 seed/Mock/Memory/Noop 或故障数据源切换；Alpha 离线的公开许可内容不模拟这些真实 Provider，本地演练结果必须标注模拟且不得签发在线资格。
