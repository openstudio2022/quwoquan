...
STATE_DIR="$ROOT_DIR/state/local/prod_sim_stack"
MEDIA_DIR="$ROOT_DIR/quwoquan_service/contracts/metadata/_shared/test_fixtures/media"

eval "$(python3 "$ROOT_DIR/agent_ops/deploy/print_local_port_profile.py" --profile prod-sim --format shell-defaults)"
...
LOCAL_API_HOST="prod-api.localhost"
LOCAL_PRODUCT_OPS_HOST="prod-product-ops.localhost"
LOCAL_MEDIA_HOSTS=(
  "prod-avatar.localhost"
  "prod-image.localhost"
  "prod-video.localhost"
  "prod-upload.localhost"
)
...
MEDIA_BASE_URL="https://prod-image.quwoquan-env.test:${MEDIA_EDGE_PORT}"
...
case "$ACTION" in
  ...
  status)
    ...
    status_one media-edge "prod-image.quwoquan-env.test" "$MEDIA_EDGE_PORT" "/media/image/s/archived-image/post/fixture_photo_001/v1/cover.png"
    if command -v curl >/dev/null 2>&1; then
