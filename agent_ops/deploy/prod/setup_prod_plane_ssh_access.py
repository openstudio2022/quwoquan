...
DEFAULT_STATE_DIR = ROOT / "state" / "local" / "prod-ssh"
    ...
    *,
    ...
    lines.extend(
        [
            ...
            "推荐一键命令：",
            "",
            "`bash agent_ops/deploy/prod/setup_prod_plane_ssh_access.sh --mode all --include-relay --include-readonly --github-prune-obsolete-secrets`",
            "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="输出账号/host/key 路径映射 JSON（默认 state/local/prod-ssh）",
    )
    parser.add_argument(
        "--instructions-out",
        default=str(DEFAULT_STATE_DIR / "runner_key_setup.md"),
        help="输出 self-hosted runner / 导出说明 Markdown（默认 state/local/prod-ssh）",
    )
    parser.add_argument(
        "--host",
        default=None,
    ...
    parser.add_argument(
        ...
        help="加密 bundle 输出路径（默认 state/local/prod-ssh/prod_ssh_keys.tar.enc）",
    )
    parser.add_argument(
