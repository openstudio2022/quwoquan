...
#   PROD_SSH_KEY_DIR=~/.ssh/quwoquan-prod agent_ops/deploy/prod/deploy_to_prod.sh
#   或显式指定：
#   PROD_SERVICE_SSH_KEY_FILE=~/.ssh/quwoquan-prod/prod-service-svc agent_ops/deploy/prod/deploy_to_prod.sh
set -euo pipefail
...
deploy_plane() {
  ...
  if [[ "$plane" == "service" ]]; then
    local render_dir="state/local/prod-plane-stack/service-${INSTANCE_SUFFIX}"
    python3 agent_ops/deploy/prod/render_prod_plane_stack.py \
      --plane service \
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
      ...
      python3 agent_ops/deploy/prod/load_prod_plane_images.py \
        --plane service \
        --host "$PROD_SSH_HOST" \
      ...
      if [[ "$PROD_IMAGE_DELIVERY_MODE" == "skip" ]]; then
        ...
        bash agent_ops/deploy/prod/sync_prod_build_workspace.sh \
          --host "$PROD_SSH_HOST" \
          --key-dir "$PROD_SSH_KEY_DIR" \
          --services "${governed_csv}" >/dev/null
        bash agent_ops/deploy/prod/build_prod_plane_images_remote.sh \
          --host "$PROD_SSH_HOST" \
          --key-dir "$PROD_SSH_KEY_DIR" \
          --services "${governed_csv}"
      else
        if ! python3 agent_ops/deploy/prod/load_prod_plane_images.py \
          --plane service \
          --host "$PROD_SSH_HOST" \
          --key-dir "$PROD_SSH_KEY_DIR" \
          --services "${governed_csv}" \
      ...
      bash agent_ops/deploy/prod/sync_prod_plane_stack.sh \
        --plane service \
        --host "$PROD_SSH_HOST" \
...
podman compose --env-file \"\$env_file\" -f \"\$compose_file\" -p '${project}' up -d --force-recreate --remove-orphans ${startup_services}
