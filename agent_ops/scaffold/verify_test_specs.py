    ...
    *,
    ...
    if isinstance(record, dict):
        ...
        if file_path:
            ...
            return
        if artifact:
            if bucket_name == "recorded" and str(artifact).startswith("artifacts/") and not evidence_path_exists(str(artifact)):
...
def evidence_path_exists(record_text: str) -> bool:
    ...
    if not path_text.startswith(("quwoquan_app/", "quwoquan_service/", "quwoquan_data/", "agent_ops/", "apps/")):
        candidates.extend(
            [
                ROOT / "quwoquan_service" / path_text,
                ROOT / "quwoquan_data" / path_text,
                ROOT / "agent_ops" / path_text,
            ]
        )
