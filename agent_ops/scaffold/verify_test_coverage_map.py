    ...
    if path_text.startswith("quwoquan_data/tests/"):
        for layer in ("local_contract", "api_integration", "user_acceptance"):
            if path_text.startswith(f"quwoquan_data/tests/{layer}/"):
                return layer
        return None
    if path_text.startswith("agent_ops/tests/local_contract/"):
        return "local_contract"
    if path_text.startswith("agent_ops/acceptance/api_integration/"):
        return "api_integration"
    if path_text.startswith("agent_ops/acceptance/user_acceptance/"):
            ...
            for item_id, item in group.items():
                ...
                for record in recorded:
                    ...
                    if kind == "artifact":
                        ...
                        if not (value.startswith("artifacts/tests/") and value.endswith("report.json")):
                            failures.add(
                                f"{path.relative_to(ROOT)} {group_name}.{item_id} recorded artifact must be artifacts/tests/**/report.json: {value}"
                            )
