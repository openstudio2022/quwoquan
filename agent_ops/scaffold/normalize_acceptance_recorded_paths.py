#!/usr/bin/env python3
"""Rewrite acceptance recorded refs to canonical test paths and artifacts/tests reports."""
...
def normalize_record(record: Any, mapping: dict[str, str]) -> tuple[dict[str, str] | None, str | None]:
    ...
    if kind == "artifact":
        if value.startswith("artifacts/tests/") and value.endswith("report.json"):
