...
- `quwoquan_app/`、`quwoquan_service/`、`quwoquan_data/`、`quwoquan_ops/` 源码树不得生成或保留 `__pycache__/`、`*.pyc`、`*.pyo`、`.pytest_cache/`。所有 Python/pytest/Make/gate runner 必须禁写或显式将缓存重定向到 `.qwq_output/env/repo/local/**`；根布局门禁必须阻断回潮。
