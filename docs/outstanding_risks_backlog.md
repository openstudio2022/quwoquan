...
- [ ] R-LEGAL-001 商业化能力上线前必须补充专项条款并通过 legal-static 发布门禁
  ...
  - 原因: 当前 `deploy/legal/versions/2026-06` 首版协议按免费社区版本拟定，明确不覆盖付费交易、广告投放、电商履约、退款发票和自动续费等商业化权利义务。
  - 影响: 若商业化能力先于专项条款上线，将造成用户告知不足、同意版本不完整、双端审核材料缺失、争议处理与退款/发票/续费规则无依据。
  - 涉及文件: `deploy/legal/manifest.yaml`、`deploy/legal/versions/**`、`agent_ops/deploy/legal_static.py`、`quwoquan_app/lib/core/auth/auth_legal_config.dart`
...
- [ ] R-PLAZA-001 同频/广场原型 P2/P3 端云一体与后端 aggregate 待方向确认后升级
  - 区域: App / Service
  - 域: `connection` / `circle-community/companionship-and-nearby-connection`
  - 原因: 2026-06-26 交付 Mock 驱动可点击原型（底栏同频 tab、连接中心四 tab、三子页、实体结伴模块、发现流连接徽章）；`ConnectionRepository` Remote 暂返回 structured unavailable；矩阵 P2/P3 登记 ○；`trip/meetup` 业务后端、附近 LBS、联系人标签端云、surface 三层测试未上线。
  - 影响: beta/gamma/prod Remote 模式下同频/广场不可用；重行动（附近碰头/结伴）无真实后端与风控闭环。
  - 涉及文件: `quwoquan_app/lib/ui/plaza/**`、`quwoquan_app/lib/cloud/services/connection/**`、`specs/product/intersection-action-deepening-and-social-ia.md`、`specs/gates/metadata_driven_ui_gap_inventory.yaml`（connection 域 partial）
  - 验收: 方向确认后逐页 P2/P3 ○→✓；circle 域 trip/meetup metadata+Go 实现；`make verify-app-page-horizontal-quality` + api_integration + user_acceptance 19 条旅程关键路径
  - 状态: 待办（2026-06-26；原型 Mock 闭环证据：`connection_hub_page_widget_test.dart` + `connection_repository_test.dart` + `flutter analyze lib/ui/plaza` 绿）
...
- [ ] R-IX04 精品池召回源（featured / 高完成率内容专用候选通道）——前置缺失：无 featuring 写入能力
  ...
  - 状态: 待办（排序侧 premium 预设[场景路由 + 完成率/停留/相关性加权]已上线、圈内精选写入已补、product-ops 全局精品写入前置已补，P1d-2 本地事件投影与读路径基座已补；剩余召回侧闭环为真实跨服务 api_integration、PremiumPoolSource 真实数据启用证明、回滚/过期/下架联动、replay/AB 分桶与 Gamma/UAT。归属 content/product-ops 前置能力轨，不阻塞主链路）
- [ ] R-IX05 四主页云侧真实数据 + DDD 收口（WP-6，客户端+跨服务，字段漂移已收口）
  ...
  - 衍生待办（2026-06-16 gamma-local 实测）: 当时 `docker-compose.gamma-local.yaml` 只含 content/chat/user/assistant/product-ops/tag/search/rec-model 服务，不含 `entity-service` 与 `circle-service`；经网关 `/v1/homepages/*` 返回 404「local-gamma mirror route is not ready」，故四主页 detail（`/v1/homepages/{id}/object-page-bundle`）与圈子 impact（`/v1/circles/{id}/impact`）的 gamma-local api_integration 暂不可冒烟。content 交集 GET 路由（`/v1/content/intersections/object|summary`、`/v1/content/intersections`）已在运行栈内并强制 viewer 鉴权；populated 交集分组（connectionState / intersectionReason.primaryText）需网关可识别的真实 token + 已 seed 的 viewer 关系，当前匿名探测返回「需要登录」。证据：`artifacts/local-gamma/search_intersection_smoke.json`（`/v1/search` 200 ES-backed 杭州西湖、`/v1/content/feed` 200、`/v1/content/feed/intersections` 200 空、交集 GET 路由 viewer 鉴权）。本轮（gamma 远端退役真相源收敛，2026-06-16）：按「gamma 已取消远端、合入 local-gamma mirror + prod 生产灰度」口径，`deploy/shared/environment_topology_manifest.yaml` 的 `gamma` 块（publicBases/hostAllowlist/artifactPolicy.allowLocalHosts/distribution/forbiddenHostTokens）与 `quwoquan_app/configs/gamma/app_runtime.yaml`、`quwoquan_service/services/chat-service/configs/gamma/config.yaml` 已从远端 `118.31.239.122:1900x` 收敛为本地 `127.0.0.1:1900x`，并重打包 git 纳管的 gamma app/service env artifact（chat/platform-ops/product-ops，残留远端 IP 归零），与文档既定口径（`environment_matrix.md`、`prod_plane_access_isolation.yaml`、environment-ops SKILL）一致。证据：`verify_environment_topology_manifest`/`verify_gamma_local_prod_isomorphism`/`verify_env_artifact_isolation`/`verify_prod_package_purity` + `content_media_url_test`(8/8) + `verify_retired_terms_zero`/`verify_concept_naming` 全绿。结论：entity/circle 的接入目标明确为 **local-gamma compose**（非远端 gamma），真实远端集成由 **prod gray-initial** rollout stage 承接。
  - 复核（2026-06-25 continue-dev）: 当前仓库已静态包含 `entity-service` 与 `circle-service`：`docker-compose.gamma-local.yaml` 有两服务 build/image/health/port 配置，`deploy/local-gamma/Caddyfile` 有 `/v1/homepages*` 与 `/v1/circles*` 网关路由，stackctl package 输出包含两服务；`bash quwoquan_service/scripts/contract/verify_contract_metadata.sh` 通过（仅保留既有 search/user_profile response_entity warnings），`docker compose -f quwoquan_service/docker-compose.gamma-local.yaml config` 通过。风险不能关闭：`python3 agent_ops/deploy/stackctl.py verify --env gamma --kind all --tier all` 仍在 `gamma-local-t3` 因 gateway health `SSL: UNEXPECTED_EOF_WHILE_READING` gate_block，`gamma-local-environment-page-smoke` 因缺少 Patrol CLI 失败；api_integration 尚未证明 `/v1/homepages/{homepageId}/object-page-bundle`、`/introduction`、`/related-groups`、`/v1/circles/{circleId}/impact`、`/v1/content/intersections/object?objectType=homepage|circle` 的 populated 结果与 viewer 鉴权 seed。
  ...
  - 状态: 待办（字段漂移/客户端编译断点/Pin·Feature 持久化/PostCount 跨服务回写/WeeklyActive 窗口回写/Impact Explain 归一已解决并验证绿；entity/circle 已静态纳入 gamma-local compose + 网关路由，但剩余 gateway health、seed viewer 关系、viewer 鉴权、homepage/circle/related-groups/impact/object-intersections api_integration populated 探针、user_acceptance/UAT 与观测证据，真实远端集成归 prod gray-initial rollout stage，非本会话单点可闭）
...
- [ ] R-CS10 文章图文不同源 → 退化为 text_only（图文混排丢失，P0）
  ...
  - 证据: `artifacts/scale_fix_stage_p5_gates.md`（27 source.md 仅 1 含内联图、7/7 文章 text_only）；RC3 提取器契约测试 `test_inline_source_images`（gate 绿，证明提取逻辑对 fixture 生效）；真实 qunar lazy-load 重下载端到端验证待补。
  - 状态: 待办（2026-06-30 用户确认登记 RC4 图文不同源 P0；下一轮需对真实去哪儿 youji（lazy-load `data-*`）重新 download，端到端证明图文混排进入 source.md 与文章配图同源，而非退化 text_only）
...
- [x] R-PAGEFLIP-002 翻页通用引擎沉淀 components 并删除 `ui/content/pageflip` 旧根
  ...
  - 验证证据:
    ...
    - 四环境 packaging 低层验证：`python3 agent_ops/deploy/stackctl.py verify --env alpha|beta|gamma|prod --kind packaging --tier t1 --report-dir /tmp/qwq-pageflip-stackctl-<env>-t1` 全部 PASS。
  - 涉及文件:
