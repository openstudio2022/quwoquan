...
- `PersonalAssistantConversationPage`
  - 作为唯一助手对话页展示云端 stream journey、叙事过程与最终答案。
  - 首次打开时可恢复本地已保存的时间轴记录，避免空白起步。

- `PersonalAssistantStreamController`
  - 作为唯一对话主链，负责时间轴恢复、turn 创建、stream 投影与错误态收口。

- `AssistantSkillCenterPage`
  - 作为“找私助”的配套管理页存在，不再与“找小趣”并列为对话入口。

- `AppMessageClient`
  - 接入统一应用消息通道，支撑私助提醒与消息详情跳转。

### 5.3 端侧入口收口策略

当前策略已经从“双入口并行”收敛为“找私助唯一入口”：

- `/assistant`、底部导航 assistant tab、聊天内助手打开动作统一进入 `PersonalAssistantConversationPage`。
- 既有“找小趣”会话只保留为本地只读时间轴恢复数据源，不再保留独立页面、Controller 或本地执行引擎。
- 全局搜索中的 `xiaoqu` 结果 tab 继续保留，它属于搜索交互，不属于助手对话入口。

验收要求：

- 端侧助手主链只使用 `PersonalAssistantConversationPage + PersonalAssistantStreamController + AssistantRepository`。
- 打开“找私助”时，如果本地存在旧会话，可以直接看到恢复后的 query、过程和答案。
- 仓库主链不再回引 `AssistantConversationPage`、本地 AgentLoop / OpenClaw / runs API。
...
目标：以“找私助”作为唯一助手入口，并让端侧只保留 UX、上下文、管理和消息能力。

准入条件：

- M1-M6 的 metadata、assistant-service turn stream、AppMessage 基础能力已有可运行或可测试证据。
- “找私助”入口必须只消费云端 typed stream，不推进端侧 AgentLoop 主链路。
- 既有“找小趣”会话仅作为只读时间轴恢复来源，不再保留独立入口。
- 页面 route、surface、operation、request header 来源必须来自 metadata/codegen。

范围：

- 统一 `/assistant` 与聊天内助手打开动作到“找私助”。
- 以 `PersonalAssistantConversationPage`、Provider、Repository 与 stream client 作为唯一主链。
- 助手页展示云端 journey / final answer / runtime failure。
- 打开页面时恢复本地旧会话时间轴的 query、过程和答案。
