...
|---|---|---|---|---|---|---|---|
| `/explore` | “先看看这个需求归属哪里” | 只读探索 workspace，定位树归属、风险和触发规则 | 按 AGENTS + command 语义输出归属，不写实现 | `AGENTS.md`、`docs/agent_context_contract.md`、`.cursor/commands/explore.md` | 写代码、改文档、跳过 L1/L2/L3 | Journey/Scenario、L1/L2/L3、UAT/SIT/GWT/contract、T1~T4、风险 | `make verify-agent-context-contract` |
| `/prd` | “把这个需求规格冻结” | 更新或建议更新对应 spec/acceptance | 检查规格、验收、CR 和 registry 是否需刷新 | `.cursor/commands/prd.md`、特性树 spec/acceptance | 未明确 Out of Scope、缺 SLO/权限/生命周期 | `spec.md`、`acceptance.yaml`、registry/CR 更新说明 | `bash agent_ops/scaffold/verify_acceptance_standard.sh` |
| `/design` | “给出架构设计并冻结方案” | 在 AppRoot/L1/L2 层设计，不给 Story 新增 design | 对齐 metadata、回滚、观测、T1~T4 | `.cursor/commands/design.md`、相关 `design.md` | Story 层建 design、绕过 metadata、缺回滚 | 设计边界、依赖、数据流、观测、回滚、测试矩阵 | `bash agent_ops/scaffold/verify_specs_l1_hierarchy.sh` |
| `/baseline` | “需求稳定，做基线” | 一次冻结 spec、acceptance、必要 design 和 CR | 检查方案是否收敛，再进入基线 | `.cursor/commands/baseline.md`、`specs/00_MASTER_DEVELOPMENT_FLOW.md` | 未收敛就冻结、用基线掩盖分歧 | spec/acceptance/design/CR/registry 对齐 | `bash agent_ops/scaffold/verify_feature_tree_refactor.sh` |
...
| `/verify` | “收口检查 / 验一下是否完成” | 对照 acceptance、T1~T4、门禁和 E2E 证据 | 输出通过/缺口/需重跑命令/剩余风险 | `.cursor/commands/verify.md`、acceptance、gate 输出 | 用新计划掩盖未完成、只看 diff | 规格达成、T1~T4、E2E、UX、观测、门禁、风险 | `make verify-agent-context-contract` |
| `/plan-review` | “开发前再审一遍计划” | 多角色检视规划，不写实现 | 刷新目标、规格、任务清单和验收标准 | `.cursor/commands/plan-review.md`、`13-coding-discipline.mdc` | 空泛建议、无任务落点、跳过阻断项 | 角色检查结果、不符合项台账、刷新后任务清单 | `bash agent_ops/scaffold/verify_acceptance_standard.sh` |
...
| `/deploy` | “部署 / 发布 / 放量 / 回滚” | 以 stackctl 为唯一环境入口，prod 操作需确认 | 读取 AGENTS + environment skill + stackctl 证据 | `.cursor/commands/deploy.md`、`agent_ops/AGENTS.md` | prod-gray、手写 URL/端口、破坏性 repair 自行执行 | stackctl verify/health/inspect/deploy 报告、回滚证据 | `python3 agent_ops/deploy/stackctl.py verify --env <env> --kind all --tier all` |
| `/infra` | “环境巡检 / 拓扑 / 健康检查” | 通过 stackctl 和 manifests 检查环境 | 不创建第二套拓扑或部署脚本 | `.cursor/commands/infra.md`、`environment-ops` skill | 手写 host/port、绕过 manifests | topology/config/packaging/health/inspect 证据 | `python3 agent_ops/deploy/stackctl.py verify --env <env> --kind all --tier all` |
| `/obs` | “补观测 / 查指标 / 加告警” | 从业务验收反推指标、日志、trace、SLO 与告警 | 不把日志当完成，要求可查询、可告警、可回滚 | `.cursor/commands/obs.md`、`obs-*.md`、触达区域 AGENTS | 无 SLO、无采样/保留、无 dashboard/report | 指标/日志/trace 字段、告警阈值、查询或看板、T1~T4 证据 | 触达范围 `verify_*` + `make verify-agent-context-contract` |
...
| `crawl` | “跑数据抓取总控 / 内容候选到发布闭环” | runtime/publish 为真相源，CLI 驱动总控 | 保证 discovery/hydrate/compose/audit/publish/feedback 闭环 | `.cursor/commands/crawl.md`、data skill | mock 产物凑数、版权不清图片、旧 runtime 根 | authenticity gate、package gate、publish manifest、feedback | `python3 quwoquan_data/scripts/cli.py verify --scope current` |
| `crawl-topic` | “处理这个 topic / 单个 topic worker” | 单 topic worker，先证据后创作后审核 | 保留 needs_more_evidence，不强行发布 | `.cursor/commands/crawl-topic.md`、data AGENTS | Pinterest 直接发布、抄第三方原文、无证据 topic 发布 | source_pool、asset_manifest、review、manifest、gate 结果 | `python3 quwoquan_data/scripts/cli.py verify --scope current` |
...
- Pre-work Reflection：检查是否触发 `errors.yaml`、`CloudException.runtimeFailure`、`RuntimeRecoveryPolicy`、UI l10n、T2 widget/provider、T3 Remote 映射。
- Cursor voice：我会在当前 App workspace 查登录页、mapper、测试和 lints，不手写错误码字符串。
- Codex voice：我会按 AGENTS 层级加载 App 规则，优先用生成错误枚举与验证命令形成证据。
- 出口证据：T2 覆盖文案/恢复按钮/关闭安全态，必要时 T3 覆盖 Remote 错误映射。
- 应跑门禁：`dart tools/runtime_error_codegen/bin/check_runtime_error_cutover.dart`、相关 `flutter test`、`make verify-app-login-entry-loop-contract`。
...
- Codex voice：我会按 `errors.yaml -> verify/codegen -> RuntimeErrorResponse -> CloudException -> UI -> telemetry -> T1~T4` 链路执行。
- 出口证据：T1 metadata/codegen，T2 mapper/UI，T3 HTTP/Remote 映射，必要时 T4 用户旅程。
...
- 区域规则：`quwoquan_data/AGENTS.md`、data skill、`agent_ops/AGENTS.md`。
- Pre-work Reflection：必须 CLI-first、Agent-only 正文、七角色准出、ship sample bundle、service importer 幂等、gamma stackctl 验证。
- Cursor voice：我会读取 runtime/publish 产物与 gate report，不把离线文件生成当完成。
...
- 区域规则：`agent_ops/AGENTS.md`、environment skill、deploy command。
...
- Codex voice：我会收集 `artifacts/stackctl/prod/**` 和 release state 作为证据。
...
- Pre-work Reflection：不得用下一轮规划掩盖本轮未完成；检查是否缺 T1~T4、E2E、观测、门禁。
