...
19 垂类统一目录规范：18 主垂类 + 1 大搜兜底垂类，定义输入输出契约、安全边界、学习信号、WebSearch 子链路与对标清单。供 personal-assistant 引擎与 assistant-template-runtime 调用。
...
#### Scenario: 记录满意度参与垂类决策
- **WHEN** 某垂类执行新一轮检索与答案生成
- **THEN** 必须参考该垂类记录雷同问题的满意度结果，显式输出策略沿用或策略切换原因
...
#### Scenario: 垂类学习信号可复用
- **WHEN** 用户再次进入同类问题
- **THEN** 规划层可直接消费上次学习信号，优先采用记录高满意策略
