...
usage() {
  cat <<'EOF'
Usage:
  scripts/new_service_fullstack.sh --name <service-name> [--port <port>]

Examples:
  scripts/new_service_fullstack.sh --name user-service --port 18081
  scripts/new_service_fullstack.sh --name chat-service
...
Notes:
  ...
  - Script always bootstraps env-split config layout by calling:
      scripts/bootstrap_service_config_layout.sh --service <service-name>
EOF
}
...
bash "$ROOT/scripts/bootstrap_service_config_layout.sh" --service "$SERVICE_NAME"
