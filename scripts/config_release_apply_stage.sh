...
bash "$ROOT/scripts/config_release_gray_rollout.sh" \
  --service "$SERVICE" \
...
set +e
gate_output="$(bash "$ROOT/scripts/config_release_slo_gate.sh" \
...
case "$gate_code" in
  ...
  20)
    ...
    bash "$ROOT/scripts/config_release_rollback.sh" --service "$SERVICE" --to-config-version "$FROM_CONFIG"
    ;;
  *)
