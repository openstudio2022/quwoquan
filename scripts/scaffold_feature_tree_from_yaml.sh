...
tree_file="${1:-}"
if [[ -z "$tree_file" ]]; then
  echo "usage: bash scripts/scaffold_feature_tree_from_yaml.sh <three-layer-yaml>" 1>&2
  echo "example: bash scripts/scaffold_feature_tree_from_yaml.sh tmp/runtime-stories.yaml" 1>&2
  exit 2
fi
...
ruby -ryaml -e '
  ...
  root = Dir.pwd
  script = File.join(root, "scripts/new_feature_fullstack.sh")
