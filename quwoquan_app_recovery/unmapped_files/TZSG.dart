...
class _MyProfileContentState extends ConsumerState<_MyProfileContent> with TickerProviderStateMixin {
  ...
  // 下拉吸顶效果相关
  double _pullOffset = 0.0; // 用户原始下拉距离
    ...
    if (notification is ScrollUpdateNotification) {
      ...
      setState(() {
        if (scrollOffset < 0) {
          // 下拉状态：记录下拉距离并限制
          _isPullingDown = true;
          final rawPullOffset = -scrollOffset;
          final maxContentPull = _getContentPullSpace();
          
          // 🔑 关键：限制下拉不超过内容可拉伸空间
          _pullOffset = rawPullOffset.clamp(0.0, maxContentPull);
          
          // 调试日志
          debugPrint('=== 下拉状态 ===');
          debugPrint('原始下拉: $rawPullOffset');
          debugPrint('限制后下拉: $_pullOffset');
  ...
  double _getContentPullOffset() {
    ...
    return _pullOffset.clamp(0.0, maxPull);
  }
  ...
  void _setupPullListener() {
    _scrollController.addListener(() {
      if (_scrollController.position.pixels < 0) {
        setState(() {
          final rawPullOffset = -_scrollController.position.pixels;
          final maxContentPull = _getContentPullSpace();
          // 🔑 限制下拉不超过内容可拉伸空间
          _pullOffset = rawPullOffset.clamp(0.0, maxContentPull);
          _isPulling = true;
