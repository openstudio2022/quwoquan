...
class _AuthorProfileState extends ConsumerState<AuthorProfile> with TickerProviderStateMixin {
  ...
  bool _handleScrollNotification(ScrollNotification notification) {
    if (notification is ScrollUpdateNotification) {
      ...
      setState(() {
        if (scrollOffset < 0) {
          // 下拉状态：应用拉伸效果
          _pullOffset = -scrollOffset * 0.5; // 拉伸系数
        } else {
          _pullOffset = 0;
        }
      });
  ...
  double _getBackgroundImageHeight() {
    ...
    return screenQuarter + _getPaddingTop();
  }
  ...
  double _getBackgroundTop() {
    ...
    if (_pullOffset > 0) {
      ...
      // 上推：背景图跟随上移
      return -(scrollOffset.clamp(0.0, _getBackgroundImageHeight()));
    } else {
      // 正常状态：固定在屏幕顶部
      return 0;
    }
  ...
  double _getPlaceholderHeight() {
    final backgroundHeight = _getBackgroundImageHeight();
    final scrollOffset = _scrollController.hasClients ? _scrollController.offset : 0.0;
    
    if (scrollOffset < 0) {
      // 🔑 精确对齐公式：背景高度 + 视觉拉伸量 + 滚动偏移量
      // 注意：scrollOffset 是负值，这会抵消 CustomScrollView 的物理下移，保持卡片紧贴背景底部
      // 视觉拉伸量：因为背景图用 Transform.scale 拉伸，对齐公式需要考虑视觉底部的位移
      return backgroundHeight + (_pullOffset / 2) + scrollOffset;
    } else {
                ...
                Row(
                  ...
                  children: [
                    OutlinedButton(
                      ...
                      child: Text(UITextConstants.back),
                    ),
    ...
    return Positioned(
      ...
      child: NotificationListener<ScrollNotification>(
        ...
        child: CustomScrollView(
          ...
          slivers: [
          // ============ 方案K-2：背景+用户区Stack架构 ============
          // 第一个SliverToBoxAdapter：背景图+用户区Stack（不设固定高度，自适应）
          SliverToBoxAdapter(
            child: Builder(
              builder: (context) {
                final double screenQuarter = MediaQuery.of(context).size.height * 0.25;
                ...
                // 🔑 关键：用户区顶部位置始终等于背景图底部位置
                final double userContentTop = backgroundHeight;
                
                // 🔑 关键：计算缩放比例，用于图像拉伸效果
                final double scaleRatio = _pullOffset > 0 
                    ? 1.0 + (_pullOffset / backgroundHeight)
                    : 1.0;
                
                print('=== 背景图布局调试信息 ===');
                print('屏幕1/4高度: $screenQuarter');
                print('状态栏高度: $paddingTop');
                print('背景图容器高度（固定）: $backgroundHeight');
                print('背景图缩放比例: $scaleRatio');
                print('用户区顶部位置（固定）: $userContentTop');
                print('下拉偏移量: $_pullOffset');
                print('==========================');
                
                // 计算用户内容区域的高度（预估）
                final double estimatedUserContentHeight = 200.h; // 预估用户内容高度
                
                // 🔑 关键：Stack 高度需要包含下拉偏移，防止用户区突破背景图底部
                // 下拉时，CustomScrollView 的 scrollOffset 是负值，整个 Stack 会向上移动
                // 为了让用户区始终在背景图底部下方，需要增加 Stack 高度来补偿
                final double scrollOffset = _scrollController.hasClients 
                    ? _scrollController.offset 
                    : 0.0;
                final double pullCompensation = scrollOffset < 0 ? -scrollOffset : 0.0;
                final double totalStackHeight = userContentTop + estimatedUserContentHeight + pullCompensation;
                
                print('=== Stack 高度计算 ===');
                print('用户区顶部: $userContentTop');
                print('预估用户内容高度: $estimatedUserContentHeight');
                print('滚动偏移: $scrollOffset');
                print('下拉补偿: $pullCompensation');
                print('总 Stack 高度: $totalStackHeight');
                print('====================');
                
                return Container(
                  height: totalStackHeight, // 🔑 关键：包含下拉补偿，防止用户区突破背景图底部
                  child: Stack(
                    children: [
                      // 1. 背景图（下拉时顶部固定，通过Transform.scale实现拉伸）
                      Positioned(
                        top: 0, // 🔑 关键：始终固定在Stack顶部，下拉时也不移动
                        left: 0,
                        right: 0,
                        height: backgroundHeight, // 🔑 关键：高度固定，不随下拉变化
                        child: Transform.scale(
                          scale: scaleRatio, // 🔑 关键：通过缩放实现拉伸效果
                      ...
                      // 2. 用户区（紧贴背景图底部，根据内容自适应高度）
                      Positioned(
                        top: userContentTop, // 🔑 关键：始终等于背景图底部位置（固定）
                        left: 0,
                        right: 0,
                        child: Container(
                          decoration: BoxDecoration(
          ...
          // 第二个SliverToBoxAdapter：Tab导航 + 网格内容
          SliverToBoxAdapter(
            child: Container(
              color: AppColorsFunctional.getColor(isDark, ColorType.backgroundPrimary),
              child: Column(
