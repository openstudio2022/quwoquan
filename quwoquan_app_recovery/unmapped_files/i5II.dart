...
class _MyProfileContentState extends ConsumerState<_MyProfileContent> with TickerProviderStateMixin {
  ...
  double _getCurrentContentTop() {
    ...
    final result = staticTop + pullOffset;
    
    // 调试：验证最终内容区位置
    final maxTop = _getContentMaxTop();
    debugPrint('=== 最终内容区位置验证 ===');
    debugPrint('静态顶部: $staticTop');
    debugPrint('下拉偏移: $pullOffset');
    debugPrint('最终顶部: $result');
    debugPrint('最大顶部: $maxTop');
    debugPrint('是否超限: ${result > maxTop}');
    debugPrint('超限距离: ${result - maxTop}');
  ...
  double _getPlaceholderHeight() {
    ...
    // 🔑 关键：确保占位高度不会超过屏幕1/2
    final result = contentTop.clamp(0.0, maxTop);
    
    // 调试：验证占位高度限制
    debugPrint('=== 占位高度限制验证 ===');
    debugPrint('内容顶部: $contentTop');
    debugPrint('最大顶部: $maxTop');
    debugPrint('占位高度: $result');
    debugPrint('是否超限: ${contentTop > maxTop}');
    debugPrint('超限距离: ${contentTop - maxTop}');
