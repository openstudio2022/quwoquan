...
class _MyProfileContentState extends ConsumerState<_MyProfileContent> with TickerProviderStateMixin {
  ...
  double _getCurrentBackgroundHeight() {
    ...
    debugPrint('=== 背景高度计算 ===');
    debugPrint('屏幕高度: $screenHeight');
    debugPrint('状态栏高度: $paddingTop');
    debugPrint('背景高度: $backgroundHeight');
    debugPrint('总高度: ${paddingTop + backgroundHeight}');
  ...
  double _getCurrentContentTop() {
    ...
    debugPrint('=== 内容区顶部位置 ===');
    debugPrint('背景容器高度: ${_getCurrentBackgroundHeight()}');
    debugPrint('内容区顶部: $result');
  ...
  double _getPlaceholderHeight() {
    ...
    // 🔑 关键：占位高度 = 背景容器高度 + 下拉偏移补偿
    // 下拉时（scrollOffset < 0），需要额外占位空间防止内容突破背景底部
    final extraSpace = scrollOffset < 0 ? -scrollOffset : 0.0;
    final totalHeight = backgroundHeight + extraSpace;
    
    debugPrint('=== 占位高度计算 ===');
    debugPrint('背景容器高度: $backgroundHeight');
    debugPrint('滚动偏移: $scrollOffset');
    debugPrint('额外空间: $extraSpace');
    debugPrint('总占位高度: $totalHeight');
    
    // 🔑 关键：确保占位高度不会过大，导致内容区域被推到屏幕外
    final screenHeight = _getScreenHeight();
    final maxHeight = screenHeight * 0.5; // 最大不超过屏幕一半
    
    return totalHeight.clamp(0.0, maxHeight);
  }
