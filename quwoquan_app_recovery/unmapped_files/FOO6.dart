          ...
          SliverToBoxAdapter(
            child: Builder(
              builder: (context) {
                ...
                // 下拉补偿量（当 scrollOffset 为负时，此值为正）
                final double pullOffset = scrollOffset < 0 ? -scrollOffset : 0.0;
                
                // 🔑 关键：内容区顶部位置始终等于背景图底部基准位置
                // 注意：它会随着 Sliver 物理下移，所以视觉上在 backgroundHeight + pullOffset
                final double userContentTop = backgroundHeight;
                
                // 计算缩放比例（可选，用于增强视觉拉伸感）
                final double scaleRatio = 1.0; // 我们直接通过 height 拉伸，不再依赖 scale
                
                print('=== 背景图精准补偿调试 ===');
                print('滚动偏移: $scrollOffset');
                print('拉伸补偿: $pullOffset');
                print('背景图容器高度: ${backgroundHeight + pullOffset}');
                print('==========================');
                
                // 计算用户内容区域的高度
                final double estimatedUserContentHeight = 800.h; // 给足够的高度
                ...
                return Container(
                  ...
                  child: Stack(
                    ...
                    children: [
                      // 1. 背景图（通过 top 和 height 的联动实现顶部固定和底部对齐）
                      Positioned(
                        top: scrollOffset < 0 ? scrollOffset : 0, // 🔑 抵消物理下移，保持顶部在屏幕顶
                        left: 0,
                        right: 0,
                        height: backgroundHeight + pullOffset, // 🔑 高度随下拉增加
                        child: Container(
                          decoration: BoxDecoration(
                      ...
                      // 2. 用户区（固定在 backgroundHeight，随 Sliver 物理下移）
                      Positioned(
                        top: userContentTop, // 始终在背景图基准底部
                        left: 0,
                        right: 0,
                        child: Container(
                          decoration: BoxDecoration(
