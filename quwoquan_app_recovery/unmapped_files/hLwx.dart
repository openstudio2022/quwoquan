          ...
          SliverToBoxAdapter(
            child: Builder(
              builder: (context) {
                ...
                // 🔑 关键：获取滚动偏移量，用于动态调整用户区位置
                final double scrollOffset = _scrollController.hasClients 
                    ? _scrollController.offset 
                    : 0.0;
                
                // 🔑 关键：用户区顶部位置需要动态调整
                // 下拉时（scrollOffset < 0），整个 Stack 向上移动，用户区会突破背景图底部
                // 需要增加用户区的 top 值来补偿，使其始终紧贴背景图底部
                final double userContentTop = backgroundHeight + (scrollOffset < 0 ? -scrollOffset : 0.0);
