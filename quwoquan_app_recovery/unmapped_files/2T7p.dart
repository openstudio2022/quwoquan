...
class _AuthorProfileState extends ConsumerState<AuthorProfile> with TickerProviderStateMixin {
  ...
  double _calculateUserContentMargin() {
    // 静止状态：内容区紧贴背景图底部（背景图高度 = 屏幕1/4 + padding.top）
    final double screenQuarter = MediaQuery.of(context).size.height * 0.25;
    final double backgroundHeight = screenQuarter + MediaQuery.of(context).padding.top;
    
    // 拉伸状态：内容区跟随背景图拉伸下移
    return backgroundHeight + _pullOffset;
  }
    ...
    return Positioned(
      ...
      child: NotificationListener<ScrollNotification>(
        ...
        child: CustomScrollView(
          ...
          SliverToBoxAdapter(
            child: Container(
              height: _getBackgroundImageHeight() + MediaQuery.of(context).padding.top + _pullOffset,
              child: Stack(
                children: [
                  // 背景图片
                  Positioned.fill(
