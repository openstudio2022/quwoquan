...
class _PostListSectionState extends ConsumerState<PostListSection> 
  ...
  void didUpdateWidget(PostListSection oldWidget) {
    ...
    if (oldWidget.category != widget.category) {
      ...
      // 切换到图片tab时，根据当前位置决定是否显示二级tab
      if (_shouldShowImageSubTabs()) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (_scrollController.hasClients) {
            final currentOffset = _scrollController.offset;
            if (currentOffset > 30.0 && _showSubTabs) {
              setState(() {
                _showSubTabs = false;
              });
              _tabOpacityController.reverse();
            } else if (currentOffset <= 30.0 && !_showSubTabs) {
              setState(() {
                _showSubTabs = true;
              });
              _tabOpacityController.forward();
        ...
        // 切换到非图片tab时，重置为显示状态（虽然不会显示）
        if (!_showSubTabs) {
          setState(() {
            _showSubTabs = true;
          });
          _tabOpacityController.value = 1.0;
        }
      }
    }
