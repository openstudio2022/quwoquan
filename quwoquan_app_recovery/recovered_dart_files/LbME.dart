...
class _PostListSectionState extends ConsumerState<PostListSection> {
  ...
  Widget _buildPostList(BuildContext context, List<Map<String, dynamic>> posts, bool isDark, dynamic responsive, WidgetRef ref) {
    ...
    // 根据category过滤内容类型
    final filteredPosts = _filterPostsByCategory(validPosts, widget.category);
    
    if (filteredPosts.isEmpty && !_shouldShowStories()) {
    ...
    return RefreshIndicator(
      ...
      child: Container(
        ...
        child: ListView.builder(
          ...
          itemCount: filteredPosts.length + (_shouldShowStories() ? 1 : 0),
          itemBuilder: (context, index) {
            ...
            // 第一个Post与Stories/Tab之间无间距，其他Post之间间距一致
  ...
  /// 根据category过滤post内容类型
  List<Map<String, dynamic>> _filterPostsByCategory(List<Map<String, dynamic>> posts, String category) {
    switch (category) {
      case 'following':
        // 关注tab：显示所有类型的内容（图片、视频、文章、动态）
        return posts;
        
      case 'recommended':
        // 推荐tab：显示所有类型的内容（图片、视频、文章、动态）
        return posts;
        
      case 'images':
        // 图片tab：只显示图片类型
        return posts.where((post) => post['type'] == ContentTypeConstants.image).toList();
        
      case 'video':
        // 视频tab：只显示视频类型
        return posts.where((post) => post['type'] == ContentTypeConstants.video).toList();
        
      case 'articles':
        // 文章tab：只显示文章类型
        return posts.where((post) => post['type'] == ContentTypeConstants.article).toList();
        
      case 'moments':
        // 动态tab：只显示动态类型
        return posts.where((post) => post['type'] == ContentTypeConstants.moment).toList();
