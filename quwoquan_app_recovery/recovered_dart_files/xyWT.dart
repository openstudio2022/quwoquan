...
class _MultiImageContentState extends State<_MultiImageContent> {
  ...
  bool _handleScrollNotification(ScrollNotification notification) {
    if (notification is ScrollUpdateNotification) {
      ...
      final isAtStart = _currentIndex == 0 && metrics.pixels <= 0;
      final isAtEnd = _currentIndex == widget.images.length - 1 && 
                      metrics.pixels >= metrics.maxScrollExtent;
      
      // 检查滑动方向
      final scrollDelta = notification.scrollDelta ?? 0;
      final isScrollingLeft = scrollDelta < 0; // 向左滑动
      final isScrollingRight = scrollDelta > 0; // 向右滑动
      
      // 在边界且继续向边界方向滑动时，阻止事件传播
      if ((isAtStart && isScrollingRight) || (isAtEnd && isScrollingLeft)) {
        return true; // 阻止事件继续传播
      }
      
      // 如果PageView正在滑动，也阻止事件传播
