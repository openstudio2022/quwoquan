    ...
    return Positioned(
      ...
      child: NotificationListener<ScrollNotification>(
        ...
        child: CustomScrollView(
          ...
          slivers: [
          // 方案E：单个Sliver，Tab和网格作为用户内容区的一部分
          SliverToBoxAdapter(
            child: Builder(
              builder: (context) {
                final double screenQuarter = MediaQuery.of(context).size.height * 0.25;
                ...
                print('内容区顶部位置: ${screenQuarter + _pullOffset}');
                print('_pullOffset: $_pullOffset');
                print('========================');
                
                // 使用Column的自适应高度，然后用LayoutBuilder获取实际高度
                return LayoutBuilder(
                  builder: (context, constraints) {
                    return Stack(
                      clipBehavior: Clip.none,
                      fit: StackFit.passthrough, // 允许Stack根据子组件自适应
                      children: [
                    // 1. 背景图容器（底层，固定高度）
                    Positioned(
                      top: 0,
                    ...
                    // 2. 用户内容区（中层，包含所有内容：用户信息 + Tab + 网格）
                    Positioned(
                      top: screenQuarter + _pullOffset,
                      left: 0,
                      right: 0,
                      // 不设置 bottom，让 Column 自适应高度
                      child: Container(
                        decoration: BoxDecoration(
                          color: AppColorsFunctional.getColor(isDark, ColorType.backgroundPrimary),
                          borderRadius: BorderRadius.only(
