...
class _ImageViewerState extends ConsumerState<ImageViewer> with TickerProviderStateMixin {
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        _getUsernameWithoutPrefix(widget.post),
                        style: TextStyle(
                          color: Colors.white,
                          fontSize: 16.sp,
  ...
  Widget _buildInteractionButtons(bool isDark) {
    return Row(
      children: [
        ...
        // 评论按钮
        _buildInteractionButton(
          icon: Icons.mode_comment_outlined, // 使用简洁的评论图标（无中间三横）
        ...
        // 收藏按钮
        _buildInteractionButton(
          icon: _isSaved ? Icons.star_rounded : Icons.star_border_rounded,
          count: _savesCount,
          isActive: _isSaved,
        ...
        GestureDetector(
          ...
          child: Container(
            padding: EdgeInsets.all(8.w),
            child: Icon(
              Icons.redo_rounded, // 使用圆角转发图标，对应图一分享图标
              color: Colors.white,
              size: AppSpacing.iconMedium, // 使用统一的图标大小
            ),
          ),
        ),
      ],
  ...
  /// 获取用户名，优先使用displayName，去掉"用户"前缀，直接使用post传入的数据
  String _getUsernameWithoutPrefix(dynamic post) {
    final name = post['displayName'] ?? post['username'] ?? '';
    if (name == null || name.toString().isEmpty) {
      return '';
    }
    // 去掉"用户"前缀，直接使用post传入的名字
    return name.toString().replaceAll('用户', '');
  }
