...
class _PostListSectionState extends ConsumerState<PostListSection> 
  ...
  late AnimationController _tabOpacityController;
  ...
  void initState() {
    super.initState();
    // 初始化ScrollController和AnimationController
    _scrollController = ScrollController();
    _scrollController.addListener(_handleScroll);
    
    _tabOpacityController = AnimationController(
      duration: const Duration(milliseconds: 280), // 使用稍长的动画时长，让效果更平滑（参考Instagram等应用）
      vsync: this,
    );
    
    // 使用CurvedAnimation实现更自然的缓动效果（参考业界最优实践）
    _tabAnimation = CurvedAnimation(
      parent: _tabOpacityController,
      curve: Curves.fastOutSlowIn, // 业界标准缓动曲线，让动画更自然平滑
    );
    
    _tabOpacityController.value = 1.0; // 初始状态为显示
  ...
  /// 处理滚动事件，控制二级tab的显示/隐藏
  /// 使用更平滑的滚动阈值判断，实现自然的推拉效果
  void _handleScroll() {
    if (!_scrollController.hasClients) return;
    
    // 只有在图片tab时才处理二级tab的显示/隐藏
    if (!_shouldShowImageSubTabs()) return;
    
    final currentOffset = _scrollController.offset;
    final scrollDelta = currentOffset - _lastScrollOffset;
    
    // 使用更敏感的阈值，让交互更自然（参考Instagram等应用）
    // 向上滑动（scrollDelta > 0）且滚动超过阈值时淡出
    // 向下滑动（scrollDelta < 0）或滚动到顶部附近时淡入
    if (scrollDelta > 3.0 && currentOffset > 40.0) {
      // 向上滑动，淡出（被推出去）
      if (_showSubTabs) {
        setState(() {
          _showSubTabs = false;
        });
        _tabOpacityController.reverse();
      }
    } else if (scrollDelta < -3.0 || currentOffset <= 25.0) {
      // 向下滑动（包括下拉刷新）或接近顶部，淡入（被拉出来）
      if (!_showSubTabs) {
        setState(() {
          _showSubTabs = true;
        });
        _tabOpacityController.forward();
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        // 二级tab导航 - 支持平滑的推拉效果（参考业界最优动效，如Instagram等）
        if (shouldShowImageSubTabs)
          AnimatedBuilder(
            animation: _tabOpacityController, // 直接使用controller，避免late初始化问题
            builder: (context, child) {
              final tabHeight = AppSpacing.subTabNavigationHeight.h;
              // 使用CurvedAnimation计算值，实现平滑的缓动效果
              final opacity = Curves.fastOutSlowIn.transform(_tabOpacityController.value);
              
              // 使用Transform实现向上推拉的效果
              // 当淡出时，tab向上移动，实现"被推出去"的视觉效果
              // 当淡入时，tab从上方滑入，实现"被拉出来"的视觉效果
              // 移动距离与opacity成反比，让动画更自然
              final translateY = -tabHeight * (1.0 - opacity) * 0.7; // 适度减少移动距离，让效果更微妙自然
              
              // 同时控制高度和位置，让列表内容能够上移填充空间
              return ClipRect(
                child: SizedBox(
                  height: tabHeight * opacity, // 高度同步变化，让列表内容能够上移填充
                  child: Transform.translate(
                    offset: Offset(0, translateY),
                    child: Opacity(
                      opacity: opacity,
                      child: SizedBox(
                        height: tabHeight, // 内部容器保持原始高度
                        child: ImageSubTabNavigation(
      ...
      child: Container(
        ...
        child: NotificationListener<ScrollNotification>(
          ...
          child: ListView.builder(
            controller: _scrollController, // 使用ScrollController监听滑动
            // 移除垂直间距，让Post无缝连接
