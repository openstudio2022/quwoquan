...
class _PostListSectionState extends ConsumerState<PostListSection> 
  ...
  void initState() {
    ...
    _tabOpacityController = AnimationController(
      duration: const Duration(milliseconds: 250), // 使用稍长的动画时长，让效果更平滑
      vsync: this,
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        // 二级tab导航 - 支持平滑的推拉效果（参考业界最优动效）
        if (shouldShowImageSubTabs)
          AnimatedBuilder(
            animation: _tabOpacityController,
            builder: (context, child) {
              final tabHeight = AppSpacing.subTabNavigationHeight.h;
              final opacity = _tabOpacityController.value;
              
              // 使用Transform实现向上推拉的效果
              // 当淡出时，tab向上移动，实现"被推出去"的视觉效果
              // 当淡入时，tab从上方滑入，实现"被拉出来"的视觉效果
              // 使用非线性映射，让动画更自然（参考Instagram等应用）
              final translateY = -tabHeight * (1.0 - opacity) * 0.8; // 稍微减少移动距离，让效果更微妙
              
              // 同时控制高度和位置，让列表内容能够上移填充
              return ClipRect(
                child: AnimatedContainer(
                  duration: const Duration(milliseconds: 250),
                  curve: Curves.fastOutSlowIn, // 使用业界标准的缓动曲线，让动画更自然
                  height: tabHeight * opacity, // 高度同步变化，让列表内容能够上移
