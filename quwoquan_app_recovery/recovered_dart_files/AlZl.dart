...
class _PostListSectionState extends ConsumerState<PostListSection> 
  ...
  void _handleScroll() {
    ...
    if (scrollDelta > 5.0 && currentOffset > 50.0) {
      // 向上滑动，淡出
      if (_showSubTabs) {
        setState(() {
          _showSubTabs = false;
    ...
    } else if (scrollDelta < -5.0 || currentOffset <= 30.0) {
      // 向下滑动（包括下拉刷新）或接近顶部，淡入
      if (!_showSubTabs) {
        setState(() {
          _showSubTabs = true;
        });
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        // 二级tab导航 - 支持滑动淡出/淡入效果，高度也同步变化
        if (shouldShowImageSubTabs)
          AnimatedBuilder(
            animation: _tabOpacityController,
            builder: (context, child) {
              // 使用AnimatedContainer同时控制高度和透明度
              return AnimatedContainer(
                duration: const Duration(milliseconds: 200),
                curve: Curves.easeInOut,
                height: _tabOpacityController.value * AppSpacing.subTabNavigationHeight.h,
                child: Opacity(
                  opacity: _tabOpacityController.value,
                  child: _tabOpacityController.value > 0
                      ...
                      : const SizedBox.shrink(), // 完全隐藏时不渲染内容
                ),
              );
            },
          ),
