...
class _AuthorProfileState extends ConsumerState<AuthorProfile> with TickerProviderStateMixin {
  ...
  double _calculateUserContentMargin() {
    ...
    // 计算静止状态下的背景图高度（基于宽高比）
    final double backgroundImageHeight = _getBackgroundImageHeight();
    final double staticBackgroundHeight = backgroundImageHeight + MediaQuery.of(context).padding.top;
    
    double calculatedMargin;
    
    if (_pullOffset <= 0) {
      // 静止状态：内容区顶部对齐到屏幕1/4和背景图高度的最小值
      calculatedMargin = screenQuarter < staticBackgroundHeight ? screenQuarter : staticBackgroundHeight;
    } else {
      // 拉伸状态：内容区跟随背景图拉伸下移
      calculatedMargin = staticBackgroundHeight + _pullOffset;
    }
    
    // 边界限制（可选）
    return calculatedMargin.clamp(0.h, MediaQuery.of(context).size.height * 0.5);
  }
    ...
    return Positioned(
      ...
      child: NotificationListener<ScrollNotification>(
        ...
        child: CustomScrollView(
          ...
          SliverToBoxAdapter(
            child: Container(
              height: MediaQuery.of(context).size.height * 0.25 + MediaQuery.of(context).padding.top + _pullOffset,
              child: Stack(
                children: [
                  // 背景图片
                  Positioned.fill(
                    child: Container(
                      decoration: BoxDecoration(
                        image: DecorationImage(
                          ...
                          fit: BoxFit.cover,
                          alignment: Alignment.topCenter, // 确保图片顶部固定
                          onError: (exception, stackTrace) {
                            print('背景图片加载失败: $exception');
                          },
                        ),
          ...
          // 第二个Sliver：用户内容区域（使用Container占据真实高度）
          SliverToBoxAdapter(
            child: Container(
              margin: EdgeInsets.only(top: _calculateUserContentMargin()),
              child: Stack(
