          ...
          SliverToBoxAdapter(
            child: Builder(
              builder: (context) {
                ...
                final double backgroundHeight = screenQuarter + paddingTop + _pullOffset;
                final double userContentTop = screenQuarter + _pullOffset;
                
                // 计算缩放比例
                final double scaleRatio = _pullOffset > 0 
                    ? (screenQuarter + paddingTop + _pullOffset) / (screenQuarter + paddingTop)
                    : 1.0;
                
                print('=== 方案K-2 布局调试信息 ===');
                print('屏幕1/4高度: $screenQuarter');
                print('背景图容器高度: ${screenQuarter + paddingTop}');
                print('背景图缩放比例: $scaleRatio');
                print('用户区顶部位置: $userContentTop');
                print('_pullOffset: $_pullOffset');
                print('==========================');
                
                // 计算用户内容区域的高度（预估）
                final double estimatedUserContentHeight = 200.h; // 预估用户内容高度
                final double totalStackHeight = userContentTop + estimatedUserContentHeight;
                
                // 获取滚动偏移量，用于下拉时保持背景图顶部固定
                final double scrollOffset = _scrollController.hasClients 
                    ? _scrollController.offset 
                    : 0.0;
                
                return Container(
                  height: totalStackHeight, // 🔑 给Stack一个明确的高度约束
                  child: Stack(
                    children: [
                      // 1. 背景图（下拉时顶部固定，整体缩放拉伸）
                      Positioned(
                        top: _pullOffset > 0 ? -scrollOffset : 0, // 🔑 下拉时补偿滚动偏移，保持顶部固定
                        left: 0,
                        right: 0,
                        height: screenQuarter + paddingTop + _pullOffset.clamp(0.0, double.infinity), // 🔑 包含拉伸高度
                        child: Container(
                          decoration: BoxDecoration(
                      ...
                      Positioned(
                        top: userContentTop, // 紧贴背景图底部：screenQuarter + _pullOffset
                        left: 0,
                        right: 0,
                        child: Container(
                          decoration: BoxDecoration(
