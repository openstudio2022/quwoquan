...
class _PostListSectionState extends ConsumerState<PostListSection> 
  ...
  void didUpdateWidget(PostListSection oldWidget) {
    ...
    // 当category或subCategory改变时，重新加载数据
  ...
  void _loadData() {
    ...
    setState(() {
      _dataFuture = dataService.getDataList(
        endpoint: '/posts',
        params: params,
        limit: 20,
      );
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<Map<String, dynamic>>>(
      key: ValueKey('${widget.category}_${widget.subCategory}'), // 添加key，确保subCategory改变时重新构建
      future: _dataFuture,
      ...
      child: Container(
        ...
        child: NotificationListener<ScrollNotification>(
          ...
          child: ListView.builder(
            ...
            itemCount: validPosts.length + (_shouldShowStories() ? 1 : 0) + (_shouldShowImageSubTabs() ? 1 : 0),
            itemBuilder: (context, index) {
            // 图片tab的二级导航
            if (_shouldShowImageSubTabs() && index == 0) {
              return ImageSubTabNavigation(
                key: const ValueKey('image_sub_tab_navigation'), // 添加稳定的key，防止刷新时重建
            ...
            final storiesIndex = _shouldShowImageSubTabs() ? 1 : 0;
            if (_shouldShowStories() && index == storiesIndex) {
            ...
            // 调整Post索引
            final postIndex = _shouldShowStories() ? index - storiesIndex - 1 : index - (_shouldShowImageSubTabs() ? 1 : 0);
            final post = validPosts[postIndex];
            
            // 第一个Post与Stories/Tab之间无间距，其他Post之间使用分割线
            final isFirstPost = _shouldShowStories()
                ? index == storiesIndex + 1  // 有Stories的Tab，第一个Post索引是storiesIndex + 1
                : index == (_shouldShowImageSubTabs() ? 1 : 0); // 没有Stories的Tab，第一个Post索引是图片tab ? 1 : 0
            ...
            return Column(
              children: [
                ...
                // 计算总item数：validPosts + (有Stories ? 1 : 0) + (有图片tab ? 1 : 0)
                if (index < validPosts.length + (_shouldShowStories() ? 1 : 0) + (_shouldShowImageSubTabs() ? 1 : 0) - 1)
                  Divider(
                    height: 1.h,
                    thickness: 0.5,
