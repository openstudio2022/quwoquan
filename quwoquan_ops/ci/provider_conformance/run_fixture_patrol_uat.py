"""Run the Provider two-device Remote journey for local-substitute Providers."""
...
_PATROL_TARGET = (
    ...
    "provider_fixture_call_provider__user_acceptance_test.dart"
)
...
def _request_json(
    *,
    ...
    headers = {
        ...
        "X-Client-Page-Id": "rtc.provider.fixture_uat",
    ...
    try:
        ...
        exc.read()
        raise ValueError(
            f"Provider two-device fixture API request failed: {method} {path} HTTP {exc.code}"
        ) from exc
    try:
        decoded = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise ValueError(
            f"Provider two-device fixture API response is not JSON: {method} {path}"
        ) from exc
    if not isinstance(decoded, dict):
        raise ValueError(f"Provider two-device fixture API response is not an object: {path}")
    return decoded
...
def _wait_process(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(f"Provider two-device fixture {role} Patrol timed out") from exc
    if process.returncode != 0:
        raise ValueError(f"Provider two-device fixture {role} Patrol failed")
    return output or ""
...
def main() -> int:
    ...
    try:
        ...
        raise ValueError(
            "Provider two-device fixture UAT only supports Alpha/Beta substitutes"
        ) from exc
    ...
    caller_token = _required("QWQ_PROVIDER_UAT_CALLER_AUTH_TOKEN")
    caller_refresh = _required("QWQ_PROVIDER_UAT_CALLER_REFRESH_TOKEN")
    caller_owner = _required("QWQ_PROVIDER_UAT_CALLER_OWNER_ID")
    caller_persona = _required("QWQ_PROVIDER_UAT_CALLER_PERSONA_ID")
    callee_token = _required("QWQ_PROVIDER_UAT_CALLEE_AUTH_TOKEN")
    callee_refresh = _required("QWQ_PROVIDER_UAT_CALLEE_REFRESH_TOKEN")
    callee_owner = _required("QWQ_PROVIDER_UAT_CALLEE_OWNER_ID")
    callee_persona = _required("QWQ_PROVIDER_UAT_CALLEE_PERSONA_ID")
    ...
    initiated = _request_json(
        ...
        idempotency_key=f"provider-fixture-{uuid.uuid4().hex}",
        body={
            "callType": "video",
    ...
    if not call_id:
        raise ValueError("Provider two-device fixture initiation response omitted call id")

    started = time.monotonic()
    caller: subprocess.Popen[str] | None = None
    callee: subprocess.Popen[str] | None = None
    try:
        with tempfile.TemporaryDirectory(prefix="qwq-provider-fixture-") as temp_dir:
            ...
            for role, output in (
                ...
                if (
                    f"QWQ_PROVIDER_UAT_FIXTURE_MEDIA_CONNECTED:{role}:{call_id}"
                    not in output
                    or f"QWQ_PROVIDER_UAT_FIXTURE_CALL_ENDED:{role}:{call_id}"
                    not in output
                ):
                    raise ValueError(
                        f"Provider two-device fixture {role} Patrol omitted lifecycle evidence"
                    )
    finally:
        ...
        try:
            _request_json(
                ...
                owner_id=caller_owner,
                persona_id=caller_persona,
                idempotency_key=f"provider-fixture-cleanup-{call_id}",
            )
    ...
    if _find_string(final_call, frozenset({"status"})) != "ended":
        raise ValueError("Provider two-device fixture call did not reach terminal ended state")
    if time.monotonic() - started <= 0:
        raise ValueError("Provider two-device fixture execution duration is invalid")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"[provider-fixture-uat] {exc}", file=sys.stderr)
