"""Run the source-owned B10 Prod Remote two-device Patrol journey.
...
_PATROL_TARGET = (
    ...
    "b10_prod_remote_call_provider__user_acceptance_test.dart"
)
...
def _request_json(
    *,
    ...
    try:
        ...
        exc.read()
        raise ValueError(
            f"B10 Prod Remote API request failed: {method} {path} HTTP {exc.code}"
        ) from exc
    except urllib.error.URLError as exc:
        raise ValueError(f"B10 Prod Remote API request failed: {method} {path}") from exc
    try:
        decoded = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise ValueError(
            f"B10 Prod Remote API response is not JSON: {method} {path}"
        ) from exc
    if not isinstance(decoded, dict):
        raise ValueError(f"B10 Prod Remote API response is not an object: {path}")
    return decoded
...
def _require_call_digest_set(value: object, *, expected: set[str], field: str) -> None:
    if (
        ...
        raise ValueError(f"B10 operator readback {field} must bind every executed call")
...
def _validate_operator_evidence(
    ...
    *,
    ...
    if not isinstance(timelines, list) or len(timelines) != len(expected_call_digests):
        raise ValueError("B10 operator readback must include one delivery timeline per call")
    ...
    for item in timelines:
        if not isinstance(item, Mapping) or set(item) != {
            ...
            raise ValueError("B10 operator delivery timeline has an invalid shape")
        ...
        if (
            ...
            raise ValueError("B10 operator delivery timeline is incomplete")
        observed_timeline_calls.add(call_digest)
    if observed_timeline_calls != expected_call_digests:
        raise ValueError("B10 operator delivery timelines do not bind every executed call")
    ...
    if not isinstance(realtime, Mapping) or set(realtime) != {
        ...
        raise ValueError("B10 operator realtime readback has an invalid shape")
    ...
    if (
        ...
        raise ValueError("B10 operator realtime readback is incomplete")

    chat = payload.get("chatProjection")
    if not isinstance(chat, Mapping) or set(chat) != {"systemCallLogs"}:
        raise ValueError("B10 operator chat projection has an invalid shape")
    logs = chat.get("systemCallLogs")
    if not isinstance(logs, list) or len(logs) != len(expected_call_digests):
        raise ValueError("B10 operator chat projection must include one log per call")
    ...
    for item in logs:
        if (
            ...
            raise ValueError("B10 operator chat system-call log is invalid")
        observed_chat_calls.add(item["callIdDigest"])
    if observed_chat_calls != expected_call_digests:
        raise ValueError("B10 operator chat projection does not bind every executed call")
    ...
    if not isinstance(qoe, Mapping) or set(qoe) != {
        ...
        raise ValueError("B10 operator QoE readback has an invalid shape")
    if not isinstance(qoe.get("effectiveSampleCount"), int) or qoe["effectiveSampleCount"] < 50:
        raise ValueError("B10 operator QoE readback requires at least 50 terminal samples")
    for field in ("alertReceiptRef", "rollbackReceiptRef"):
        if not isinstance(qoe.get(field), str) or not _RECEIPT_REF_RE.fullmatch(qoe[field]):
            raise ValueError("B10 operator QoE readback requires receipt references")
    calls = qoe.get("calls")
    if not isinstance(calls, list) or len(calls) != len(expected_call_digests):
        raise ValueError("B10 operator QoE readback must include one session per call")
    ...
    for item in calls:
        if not isinstance(item, Mapping) or set(item) != {
            ...
            raise ValueError("B10 operator QoE session readback has an invalid shape")
        if (
            ...
            raise ValueError("B10 operator QoE session readback is incomplete")
        observed_qoe_calls.add(item["callIdDigest"])
    if observed_qoe_calls != expected_call_digests:
        raise ValueError("B10 operator QoE sessions do not bind every executed call")
...
def _load_operator_receipts(*, call_ids: tuple[str, ...]) -> dict[str, Any]:
    ...
    path = Path(_required("QWQ_B10_OPERATOR_READBACK_PATH")).expanduser()
    try:
    ...
    for field, expected in expected_digests.items():
        if not _SHA256_RE.fullmatch(expected) or payload.get(field) != expected:
            raise ValueError(
                "B10 operator readback must bind the active image, config, "
    ...
    if _SENSITIVE_VALUE_RE.search(json.dumps(payload, sort_keys=True)):
        raise ValueError("B10 operator readback contains sensitive runtime material")
    return payload
...
def _validate_hosted_release_receipts(payload: Mapping[str, Any]) -> None:
    ...
    if not isinstance(readiness, Mapping):
        raise ValueError("B10 operator readback releaseReadiness is invalid")
    candidate = {
        "imageDigest": _required("QWQ_PROVIDER_CONFORMANCE_EXPECTED_IMAGE_DIGEST"),
        "configDigest": _required("QWQ_PROVIDER_CONFORMANCE_CONFIG_DIGEST"),
        "contractGraphDigest": _required(
    ...
    for field, purpose in (
        ...
        if match is None:
            raise ValueError(
                f"B10 operator readback {field} must be a hosted receipt reference"
            )
        command = [
            sys.executable,
        ...
        if result.returncode != 0:
            raise ValueError(
                "B10 hosted release receipt verification failed: "
                f"{result.stderr.strip() or result.stdout.strip()}"
            )
        try:
            verified = json.loads(result.stdout)
        except json.JSONDecodeError as error:
            raise ValueError("B10 hosted release receipt verification was not JSON") from error
        if (
            ...
            raise ValueError("B10 hosted release receipt verification is inconsistent")
...
def _wait_for_marker(
    ...
    *,
    ...
    while time.monotonic() < deadline:
        ...
        if marker in line:
            return
    raise ValueError(f"B10 Prod Remote {role} Patrol omitted marker {marker}")
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            captured.process.wait(timeout=15)
        raise ValueError(f"B10 Prod Remote {role} Patrol timed out") from exc
    ...
    if captured.process.returncode != 0:
        raise ValueError(f"B10 Prod Remote {role} Patrol failed")
...
def _patrol_command(platform: str, *, role: str) -> list[str]:
    command = [
        ...
        "--define-key",
        "QWQ_PROVIDER_UAT_B10_PHASE",
    ]
    if role == "caller":
        command.extend(("--define-key", "QWQ_PROVIDER_UAT_B10_CALL_ID"))
    else:
        command.extend(
            ("--define-key", "QWQ_PROVIDER_UAT_B10_EXPECTED_CALLER_NAME")
        )
    return command
    ...
    environment.update(
        {
            ...
            "QWQ_PROVIDER_UAT_B10_PHASE": phase,
            "QWQ_PROVIDER_UAT_B10_CALL_ID": call_id,
            "QWQ_PROVIDER_UAT_B10_EXPECTED_CALLER_NAME": expected_caller_name,
...
def _participant(prefix: str) -> dict[str, str]:
    return {
        "token": _required(f"QWQ_B10_{prefix}_AUTH_TOKEN"),
        "refresh": _required(f"QWQ_B10_{prefix}_REFRESH_TOKEN"),
        "owner": _required(f"QWQ_B10_{prefix}_OWNER_ID"),
        "persona": _required(f"QWQ_B10_{prefix}_PERSONA_ID"),
        "display_name": _required(f"QWQ_B10_{prefix}_DISPLAY_NAME"),
    }
...
def _assert_markers(
    ...
    *,
    ...
    if missing:
        raise ValueError(
            f"B10 Prod Remote {role} Patrol omitted lifecycle evidence: {', '.join(missing)}"
        )
...
def _run_direction(
    *,
    ...
    try:
        ...
        _wait_for_marker(
            callee_process,
            marker=f"QWQ_B10_REMOTE_CALLEE_READY:{phase}",
            role=f"{phase}/callee",
            timeout_seconds=120,
        )
        initiated = _request_json(
            ...
            idempotency_key=f"b10-prod-{phase}-{uuid.uuid4().hex}",
            body={
                "callType": "video",
        ...
        if not call_id:
            raise ValueError("B10 Prod Remote initiation response omitted call id")
        ...
        _assert_markers(
            ...
            phase=phase,
            required_markers=(
                "QWQ_B10_REMOTE_MEDIA_CONNECTED",
                "QWQ_B10_REMOTE_CALL_ENDED",
            ),
        )
        callee_markers = (
            "QWQ_B10_REMOTE_PUSH_PRESENTED",
            "QWQ_B10_REMOTE_MEDIA_CONNECTED",
            "QWQ_B10_REMOTE_CALL_ENDED",
        )
        if phase == "ios_to_android":
            callee_markers += (
                "QWQ_B10_REMOTE_SCREEN_SHARE_COMPLETED",
                "QWQ_B10_REMOTE_PIP_HANGUP",
            )
        _assert_markers(
            callee_output,
            role="callee",
        ...
        if _find_string(final_call, frozenset({"status"})) != "ended":
            raise ValueError("B10 Prod Remote call did not reach terminal ended state")
        ...
        if call_id:
            try:
                _request_json(
                    ...
                    owner_id=caller["owner"],
                    persona_id=caller["persona"],
                    idempotency_key=f"b10-prod-cleanup-{call_id}",
                )
            except ValueError:
...
def _build_readback(
    *,
    ...
    assertion_ids = json.loads(_required("QWQ_PROVIDER_CONFORMANCE_ASSERTION_IDS"))
    if not isinstance(assertion_ids, list) or not all(
        isinstance(value, str) and value for value in assertion_ids
    ):
        raise ValueError("QWQ_PROVIDER_CONFORMANCE_ASSERTION_IDS must be a string list")
    ...
    if not isinstance(observability, Mapping) or not all(
        ...
        raise ValueError("B10 operator readback observabilityRefs are incomplete")
    data_digest = _sha256(
        json.dumps(
            {
        ...
        "assertions": [
            {
                ...
                "status": "passed",
                "logRef": observability["logs"][0],
                "traceRef": observability["traces"][0],
                "metricRefs": list(observability["metrics"]),
            }
...
def main() -> int:
    if _required("QWQ_PROVIDER_CONFORMANCE_ENVIRONMENT") != "prod":
        raise ValueError("B10 Prod Remote Patrol only supports prod")
    ...
    output_path = Path(_required("QWQ_B10_REMOTE_UAT_READBACK_PATH"))
...
if __name__ == "__main__":
    try:
        ...
        print(f"[b10-prod-remote-patrol] {exc}", file=sys.stderr)
