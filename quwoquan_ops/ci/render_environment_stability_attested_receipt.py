...
def _bound_production_release(
    ...
    for relative, expected_digest in sorted(common):
        ...
        if (
            ...
            and payload.get("releaseClass") == "production"
            and payload.get("productLifecycleState") == "production"
    ...
    if len(identities) != 1:
        raise ValueError(
            "manifest must bind exactly one production pilot-003 release in all environments"
        )
...
def _identity(
    ...
    *,
    ...
    release = _bound_production_release(root, manifest)
    return {**actual, **release}


def _verify_release_binding(manifest_path: Path, release_attestation: Path) -> None:
    root, manifest = _manifest(manifest_path)
    expected = _bound_production_release(root, manifest)
    supplied = _read_object(
        release_attestation,
        label="runtime production release attestation",
    )
    actual = {
    ...
    if supplied.get("schema") != "quwoquan_data.release_attestation" or actual != expected:
        raise ValueError(
            "runtime production release bytes differ from manifest-bound pilot-003"
        )
