...
1. Prepare new config version file in `quwoquan_service/services/<service>/v*.yaml`
2. Run stage rollout:
...
Rollback is idempotent and writes an audit line to `.qwq_output/local/release-state/<service>.audit.log`.
