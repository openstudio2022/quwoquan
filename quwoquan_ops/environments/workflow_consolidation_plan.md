...
- **Profile 统一**：所有门禁按 `quwoquan_ops/environments/gamma_validation_suites.json` 中的 profile 执行，不再各自硬编码行为。
...
- gamma 旅程以 `quwoquan_ops/environments/gamma_validation_suites.json` 为单源；当前基线是 `assistant_main_chain` 与 `chat_avatar_sync`，后续按业务对象继续补齐。
