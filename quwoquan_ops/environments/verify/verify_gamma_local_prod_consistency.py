...
其服务名册/对外 DNS 名必须与 prod（quwoquan_ops/environments/workload_topology_inventory.yaml）对齐。

真相源：
  - quwoquan_ops/environments/workload_topology_inventory.yaml  (prod 三态 wired workloads)
  - quwoquan_ops/environments/compose/docker-compose.gamma-local.yaml  (gamma-local 统一 compose)
