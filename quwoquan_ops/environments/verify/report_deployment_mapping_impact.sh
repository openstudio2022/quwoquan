...
Behavior:
  - Reads quwoquan_ops/environments/process_domain_mapping.yaml.
  - Compares with <base-ref>:quwoquan_ops/environments/process_domain_mapping.yaml when available.
...
' "$ROOT/quwoquan_ops/environments/process_domain_mapping.yaml" \
  "$(git show "${BASE_REF}:quwoquan_ops/environments/process_domain_mapping.yaml" 2>/dev/null || true)" \
  "$BASE_REF"
