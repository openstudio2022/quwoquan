...
- 官方自动化入口统一为 `quwoquan_ops/cli/stackctl.py`（包装脚本：`quwoquan_ops/cli/stackctl.sh` / `quwoquan_ops/cli/stackctl`）。底层脚本可保留，但只作为实现细节。
...
`quwoquan_ops/environments/gamma_validation_suites.json` 是多环境 promotion 期间 local-gamma / self-hosted 验证 profile 的唯一真相源（已无远端 gamma profile；远端真实集成复验由 prod `gray-initial` 承接）：
...
- 共享 `quwoquan_ops/environments/gamma_validation_suites.json` 中的 suite 定义与报告字段。
...
| `gamma-local` | `python3 quwoquan_ops/cli/stackctl.py up --env gamma` | local-gamma mirror + App 达到 steady state，并生成 `.qwq_output/local/gamma-local/report.json` / `.qwq_output/runs/gamma/**` |
...
- 本轮已验证的 gray/full 证据分别见 `.qwq_output/runs/prod/20260617T164119Z-deploy-prod-hosted`（`gray-initial` 成功，`4/4 healthy`）与 `.qwq_output/runs/prod/20260617T172537Z-deploy-prod-hosted`（`full` 成功，`4/4 healthy`，doctor 无问题）。
...
| 范围 | 命令 / 条件 | 通过判据 |
|------|-------------|----------|
| `local-gamma mirror` | `make gate-local-gamma` / `python3 quwoquan_ops/cli/stackctl.py up --env gamma` | `T1/T2` 本地门禁、`T3` 本地真实 API/存储、`T4` 共享 gamma patrol/chat-avatar/environment-smoke 旅程通过并生成 `.qwq_output/local/gamma-local/report.json` 或 `.qwq_output/runs/gamma/**` |
| `prod-sim` | `python3 quwoquan_ops/cli/stackctl.py up --env prod-sim` → `python3 quwoquan_ops/cli/stackctl.py verify --target prod-sim --kind topology --tier t4` | 本地 `api-edge/product-ops-edge/media-edge/media-origin` 可探测，页面 smoke 覆盖首页、我的、他人主页、记录列表、视频流，产物归档到 `.qwq_output/runs/prod/**/environment-page-smoke/report.json` |
