...
- **统一门禁**：无论采用哪种模式，进入 `main` 前都必须通过：
  ...
  - `04. Pre-Release Gate`：验证 profile 解析与 required-check 汇总；远端复验由 prod `gray-initial` 承接。
  - `05. App Env Device Matrix`：本地 self-hosted alpha/beta/gamma Android/iOS 设备矩阵与证据校验。
...
- `gamma-local` 使用 `APP_ENV=gamma` / `APP_RUNTIME_ENV=gamma` 与
  `quwoquan_ops/environments/gamma_validation_suites.json`；它负责提交前自检，不作为独立 merge gate。
...
- `quwoquan_ops/environments/ci_cd_end_to_end_design.md`
