...
- `local-gamma mirror` 与云侧 gamma 共用 `APP_ENV=gamma` / `APP_RUNTIME_ENV=gamma`、同一份 seed 语义与 `quwoquan_ops/environments/gamma_validation_suites.json`；它负责提交前自检，不再作为独立 merge gate。
