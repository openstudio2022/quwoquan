...
class StackctlUpRuntimeTest(unittest.TestCase):
    def test_parser_accepts_report_dir_after_subcommand(self) -> None:
        ...
        cases = [
            ["package", "--env", "alpha", "--report-dir", ".qwq_output/runs/alpha/package"],
            [
                "package",
                "--env",
                "alpha",
                ...
                "legal-static",
                "--report-dir",
                ".qwq_output/runs/alpha/legal-package",
            ],
            [
                "verify",
                "--env",
            ...
            [
                ...
                "legal-static",
                "--report-dir",
                ".qwq_output/runs/alpha/legal-verify",
            ],
            ["up", "--target", "alpha-local", "--report-dir", ".qwq_output/runs/alpha/up"],
            [
                "health",
                "--target",
                "alpha-local",
            ...
            [
                ...
                "all",
                "--report-dir",
                ".qwq_output/runs/alpha/inspect",
            ],
        ]
        for argv in cases:
            with self.subTest(argv=argv):
                args = parser.parse_args(argv)
                self.assertTrue(getattr(args, "report_dir", "").startswith(".qwq_output/runs/"))

    def test_parser_keeps_global_report_dir_before_subcommand(self) -> None:
        parser = stackctl.build_parser()
        args = parser.parse_args(
            ["--report-dir", ".qwq_output/runs/alpha/global", "package", "--env", "alpha"]
        )
        self.assertEqual(args.report_dir, ".qwq_output/runs/alpha/global")
    ...
    def test_legal_static_package_builds_current_pointer(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            package_dir = output_root / "alpha" / "2026-07"
