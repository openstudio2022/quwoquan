...
def test_bound_production_release_does_not_reintroduce_commercial_rights_admission(
    tmp_path: Path, rights: dict,
) -> None:
    manifest = _bound_release_fixture(tmp_path, **rights)
    result = receipt_renderer._bound_production_release(tmp_path, manifest)
...
def test_bound_release_rejects_retired_lifecycle(
    ...
    with pytest.raises(ValueError, match="exactly one production"):
        receipt_renderer._bound_production_release(tmp_path, manifest)
