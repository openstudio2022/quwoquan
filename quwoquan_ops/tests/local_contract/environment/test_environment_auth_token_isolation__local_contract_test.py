...
def test_research_consumer_parameters_are_rejected_before_probe(tmp_path: Path) -> None:
    with mock.patch.object(stackctl, "_run_script_probe") as run_probe:
        with pytest.raises(TypeError, match="research_consumer_token"):
            stackctl._run_environment_integration_probe(
                stackctl.load_environment_topology(), "alpha-local", tmp_path,
                research_consumer_token="retired-bearer",
                research_consumer_attestation="retired-attestation",
