...
def test_ios_exact_registrant_policy_keeps_runner_closed() -> None:
    policy = projection_seal._load_policy(
        _REPO_ROOT / "quwoquan_ops/policies/app_build_projection_policy.json",
        FLUTTER_IOS_3_47_COCOAPODS_1_16_POLICY_ID,
    )
    for suffix in ("h", "m"):
        projection_seal._admit(policy, projection_seal._Node(
            f"quwoquan_app/ios/Runner/GeneratedPluginRegistrant.{suffix}", "file", 0o644, 1, _digest("a")))
    with pytest.raises(ValueError, match="rejected"):
        projection_seal._admit(policy, projection_seal._Node(
            "quwoquan_app/ios/Runner/Injected.m", "file", 0o644, 1, _digest("a")))
    with pytest.raises(ValueError, match="kind rejected"):
        projection_seal._admit(policy, projection_seal._Node(
            "quwoquan_app/ios/Runner/GeneratedPluginRegistrant.h", "symlink", 0o644, 1, _digest("a")))


def test_registrant_bytes_are_replayed_from_sdk_and_locked_declaration(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    projection = tmp_path / "projection"
    app = projection / "quwoquan_app"
    capsule = tmp_path / "capsule"
    sdk = tmp_path / "sdk"
    plugin = b"name: sample\nversion: 1.0.0\nflutter:\n  plugin:\n    platforms:\n      ios:\n        pluginClass: SamplePlugin\n"
    relative = "quwoquan_app/vendor/plugins/sample/pubspec.yaml"
    _write(projection, relative, plugin)
    _write(projection, "quwoquan_app/pubspec.lock", b"packages:\n  sample:\n    source: path\n    version: 1.0.0\n    description:\n      path: vendor/plugins/sample\n")
    _write(projection, "quwoquan_app/.dart_tool/package_config.json", json.dumps({"packages": [
        {"name": "sample", "rootUri": "../vendor/plugins/sample/"}]}).encode())
    _write(projection, "quwoquan_app/.flutter-plugins-dependencies", json.dumps({"plugins": {"ios": [
        {"name": "sample", "path": str(app / "vendor/plugins/sample"), "native_build": True}]}}).encode())
    _write(capsule, "dependencies/dart-pub-cache-manifest.json", json.dumps({
        "flutterVersion": "3.47.0", "flutterCommandResolutionDigest": _digest("a"), "dependency": {"entries": []}}).encode())
    executable = _write(sdk, "bin/flutter")
    _write(sdk, "packages/flutter_tools/lib/src/flutter_plugins.dart", b"const _objcPluginRegistryHeaderTemplate = '''\n{{framework}}\n''';\nconst _objcPluginRegistryImplementationTemplate = '''\n{{#methodChannelPlugins}}\n{{name}} {{prefix}}{{class}}\n{{/methodChannelPlugins}}\n''';\n")
    monkeypatch.setattr(projection_seal, "resolved_flutter_identity", lambda env: {
        "executable": str(executable), "flutterVersion": "3.47.0", "commandResolutionDigest": _digest("a")})
    header = _write(projection, "quwoquan_app/ios/Runner/GeneratedPluginRegistrant.h", b"Flutter\n")
    implementation = _write(projection, "quwoquan_app/ios/Runner/GeneratedPluginRegistrant.m", b"sample SamplePlugin\n")
    manifest = {"entries": [{"kind": "file", "logicalPath": relative, "digest": "sha256:" + hashlib.sha256(plugin).hexdigest()}]}
    projection_seal._verify_ios_registrants(projection, capsule, manifest)
    implementation.write_bytes(b"sample SamplePlugin\nvoid injected() {}\n")
    with pytest.raises(ValueError, match="exact generated bytes"):
        projection_seal._verify_ios_registrants(projection, capsule, manifest)
    implementation.write_bytes(b"sample SamplePlugin\n")
    (projection / relative).write_bytes(plugin.replace(b"SamplePlugin", b"OtherPlugin"))
    with pytest.raises(ValueError, match="declaration differs"):
        projection_seal._verify_ios_registrants(projection, capsule, manifest)
    assert header.read_bytes() == b"Flutter\n"
    (projection / relative).write_bytes(plugin)
    graph_path = app / ".flutter-plugins-dependencies"
    graph = json.loads(graph_path.read_bytes())
    graph["plugins"]["ios"].append({"name": "injected", "native_build": True})
    graph_path.write_text(json.dumps(graph))
    with pytest.raises(ValueError, match="plugin set drifted"):
        projection_seal._verify_ios_registrants(projection, capsule, manifest)


def test_swiftpm_private_cache_does_not_relax_external_symlink_boundary(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    manifest, projection = _source_projection(tmp_path, monkeypatch)
    cache = projection / "quwoquan_app/.dart_tool/qwq_ios_cocoapods_dependency/production/user-home/.config/swiftpm/cache"
    cache.mkdir(parents=True)
    (cache / "local-cache").write_text("rebuildable")
    seal_projection_build(manifest, projection, policy_id=FLUTTER_IOS_3_47_COCOAPODS_1_16_POLICY_ID)
    (cache / "local-cache").unlink()
    cache.rmdir()
