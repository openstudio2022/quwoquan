...
def test_prod_sim_rehearsal_missing_source_fails_closed_and_writes_report(
    ...
    conformance.source_for_cell.return_value = None
    with tempfile.TemporaryDirectory() as temporary:
        ...
        with (
            ...
            mock.patch.object(
                subject,
                "_prod_sim_rehearsal_identity",
                return_value=(identity, _composition()),
            ),
            mock.patch.object(
                subject,
                "_prod_sim_provider_rehearsal_environment",
                side_effect=AssertionError("missing source must stop before runtime binding"),
            ),
        ):
    ...
    assert report["completedCapabilities"] == []
    assert "no source-declared first-party blackbox runner" in report["issues"][0]
...
def test_prod_sim_first_party_sources_are_keyed_apart_from_nine_grid() -> None:
    ...
    for source in first_party:
        ...
        assert sources[key]["testSource"] == source["testSource"]
        assert "/service_ops/" in source["testSource"]
        uat_key = (
            str(source["capabilityId"]),
            str(source["adapterId"]),
            "user_acceptance",
        )
        if uat_key in sources:
            assert sources[uat_key]["testSource"] != source["testSource"]
        subject._prod_sim_first_party_source(
            source,
            capability_id=str(source["capabilityId"]),
        )


def test_prod_sim_first_party_assistant_reuses_command_idempotency_key() -> None:
    source = (
        Path(stackctl.__file__).resolve().parents[2]
        / "quwoquan_ops/tests/acceptance/user_acceptance/service_ops/prod_sim_first_party_blackbox.py"
    ).read_text(encoding="utf-8")
    assert 'body={"summary": prompt, "clientRequestId": session_key}' in source
    assert '"clientRequestId": run_key,' in source
    assert '_idempotency(seed + "/session-id")' not in source
    assert '_idempotency(seed + "/run-id")' not in source
    assert '_owner_authed(' in source
    assert '"assistant-service"' in source
    assert '"user-service"' in source
    assert "/owner/credentials/carrier-phone/bind" in source
    assert "/auth/login/wechat" in source
    assert '"product-ops-service"' in source
    assert "/ops/runtime-logs/summary" in source
    assert '"/content/posts:publish"' in source
    assert '"Idempotency-Key": intent' in source
    assert '"publishIntentId": intent' in source
    assert "content_outbox" in source
    assert "PostPublished" in source
    assert "_seed_content_embedding_projection" in source


def test_prod_sim_up_publishes_assistant_policy_after_compose() -> None:
    source = (
        Path(stackctl.__file__).resolve().parents[2]
        / "quwoquan_ops/cli/commands/up_runtime.py"
    ).read_text(encoding="utf-8")
    assert "publish_prod_sim_assistant_policy" in source
    assert "package-bound-assistant-policy-publication" in source
    assert 'requested_target == "prod-sim"' in source
