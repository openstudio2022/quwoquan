...
def _candidate() -> CandidateBinding:
    return CandidateBinding(
        ...
        release_posts=(
            ...
            BusinessObjectRef("Post", "post-2"),
            BusinessObjectRef("Post", "post-3"),
        ),
        release_creators=(BusinessObjectRef("Creator", "creator-1"),),
        release_entities=(BusinessObjectRef("Entity", "entity-1"),),
        release_homepages=(BusinessObjectRef("EntityHomepage", "entity-1"),),
        release_tags=(BusinessObjectRef("Tag", "tag-1"),),
        release_media_assets=(BusinessObjectRef("MediaAsset", "media-1"),),
    )
...
class TestDataControlPlaneContractTest(unittest.TestCase):
    def test_actor_and_immutable_release_handles_reject_identity_drift(self) -> None:
        with self.assertRaisesRegex(ValueError, "UserAccount"):
            ActorHandle(
                ...
                account=BusinessObjectRef("Persona", "wrong-account-kind"),
                persona=BusinessObjectRef("Persona", "persona-a"),
                session_handle="session-a",
            )
        actor = ActorHandle(
            role=ActorRole.PRIMARY,
            account=BusinessObjectRef("UserAccount", "account-a"),
            persona=BusinessObjectRef("Persona", "persona-a"),
            session_handle="session-a",
        )
        with self.assertRaisesRegex(ValueError, "at most once"):
            AcceptanceActorSet((actor, actor))
        with self.assertRaisesRegex(ValueError, "release digest"):
            ImmutableReleaseHandle(
                ...
                entities=(BusinessObjectRef("Entity", "entity-1"),),
                homepages=(BusinessObjectRef("EntityHomepage", "entity-1"),),
                tags=(BusinessObjectRef("Tag", "tag-1"),),
                media_assets=(BusinessObjectRef("MediaAsset", "media-1"),),
            )
    ...
    def test_serial_baseline_policy_disables_candidate_cache(self) -> None:
        runtime = DataRuntime(candidate_cache_enabled=False)

        for _ in range(2):
            cached, hit, owner = runtime.candidate_cache_get(
                candidate_binding_digest=_candidate().digest,
                capability_key="immutable-cache-entry",
                params_identity="sha256:" + "5" * 64,
            )
        ...
        self.assertEqual(runtime.cache_hits, 0)
    ...
    def test_same_instance_is_exclusive_and_retry_advances_fencing_generation(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            first_manager.bind_actor_set(
                generation=first_lease.generation,
                actor_set_digest=actor_set.identity_digest,
            )
            ...
            drifted = ReceiptJournal(
                root=root,
                case_id="exclusive-instance",
                test_data_instance_id="instance-1",
                candidate_binding_digest=_candidate().digest,
            )
            drifted_manager = ActorLeaseManager(drifted)
            drifted_lease = drifted_manager.acquire(
                lease_id="lease-1",
                case_run_id="instance-1",
            )
            different_actor_set = AcceptanceActorSet(
                (
                    ActorHandle(
                        role=ActorRole.PRIMARY,
                        account=BusinessObjectRef("UserAccount", "account-b"),
                        persona=BusinessObjectRef("Persona", "persona-b"),
                        session_handle="session-b",
                    ),
                )
            )
            with self.assertRaisesRegex(RuntimeError, "different ActorSet"):
                drifted_manager.bind_actor_set(
                    generation=drifted_lease.generation,
                    actor_set_digest=different_actor_set.identity_digest,
                )
            drifted_manager.quarantine(
                generation=drifted_lease.generation,
                reason="recovery actor binding drift",
            )
            drifted.close()
    ...
    def test_chat_only_request_loads_only_user_and_chat_and_writes_timings(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            case_document = executed.document(
                receipt_path_base=Path(temporary),
            )
            self.assertTrue(case_document["readbackReceiptDigests"])
            self.assertTrue(case_document["cleanupReceiptDigests"])
            for field in ("provisionReceiptPath", "testBodyReceiptPath"):
                receipt_path = Path(str(case_document[field]))
                self.assertFalse(receipt_path.is_absolute())
                self.assertTrue((Path(temporary) / receipt_path).is_file())
            for field in ("readbackReceiptPaths", "cleanupReceiptPaths"):
                self.assertTrue(case_document[field])
                self.assertTrue(
                    all(
                        (Path(temporary) / Path(str(receipt_path))).is_file()
                        for receipt_path in case_document[field]
                    )
                )
            ...
            self.assertTrue(
                {"provision", "readback", "test-body", "cleanup"}.issubset(
                    receipt_kinds
                )
            )
            test_body = next(
                json.loads(path.read_text(encoding="utf-8"))["payload"]
                for path in Path(temporary).rglob("*-test-body.json")
            )
            self.assertEqual(
                test_body["provisionReceiptDigest"],
                executed.provision_receipt.digest,
            )
            for field in (
                "requestCollectionMs",
                "providerDiscoveryMs",
