...
def exact_fingerprint_document(entries: list[dict[str, object]]) -> str:
    return json.dumps(
        {
            "_governance": COMPLETE,
            "schema": "single-track-exact-fingerprint-baseline",
            "fingerprintAlgorithm": "sha256(category\u0000path\u0000normalized-detail)-v1",
...
def fingerprint_entry(path: str, fingerprint: str, count: int) -> dict[str, object]:
    return {
...
def test_list_exact_fingerprint_count_growth_blocks(
    gate_root: Path, monkeypatch: pytest.MonkeyPatch, capsys
) -> None:
    path = gate_root / "single_track_exact_fingerprint_baseline.json"
    current = fingerprint_entry("some/file.dart", "sha256:" + "a" * 64, 2)
    previous = fingerprint_entry("some/file.dart", "sha256:" + "a" * 64, 1)
    path.write_text(exact_fingerprint_document([current]), encoding="utf-8")
    monkeypatch.setattr(
        gate, "head_revision", lambda _: exact_fingerprint_document([previous])
    )
...
def test_list_exact_fingerprint_replacement_blocks(
    gate_root: Path, monkeypatch: pytest.MonkeyPatch, capsys
) -> None:
    path = gate_root / "single_track_exact_fingerprint_baseline.json"
    current = fingerprint_entry("some/file.dart", "sha256:" + "b" * 64, 1)
    previous = fingerprint_entry("some/file.dart", "sha256:" + "a" * 64, 1)
    path.write_text(exact_fingerprint_document([current]), encoding="utf-8")
    monkeypatch.setattr(
        gate, "head_revision", lambda _: exact_fingerprint_document([previous])
    ...
    assert gate.main() == 1
    assert "sha256:" + "b" * 64 in capsys.readouterr().out


def test_list_exact_fingerprint_reduction_passes(
    gate_root: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    path = gate_root / "single_track_exact_fingerprint_baseline.json"
    current = fingerprint_entry("some/file.dart", "sha256:" + "a" * 64, 1)
    previous = fingerprint_entry("some/file.dart", "sha256:" + "a" * 64, 2)
    path.write_text(exact_fingerprint_document([current]), encoding="utf-8")
    monkeypatch.setattr(
        gate, "head_revision", lambda _: exact_fingerprint_document([previous])
    )
...
def test_list_duplicate_entries_cannot_hide_growth(
    gate_root: Path, monkeypatch: pytest.MonkeyPatch, capsys
) -> None:
    entry = fingerprint_entry("some/file.dart", "sha256:" + "a" * 64, 1)
    path = gate_root / "single_track_exact_fingerprint_baseline.json"
    path.write_text(
        exact_fingerprint_document([entry, dict(entry)]),
        encoding="utf-8",
    )
    monkeypatch.setattr(
        gate,
        "head_revision",
        lambda _: exact_fingerprint_document([entry]),
    )
