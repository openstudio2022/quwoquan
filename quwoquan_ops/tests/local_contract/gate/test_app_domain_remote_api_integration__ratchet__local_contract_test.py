...
class AppDomainRemoteApiIntegrationContractTest(unittest.TestCase):
    ...
    def test_entity_and_notification_use_typed_environment_harnesses(self) -> None:
        cases, discovery_issues = evidence.discover_cases(ROOT)
        selected = [
            case for case in cases if case.domain in {"entity", "notification"}
        ]
        validated, validation_issues = evidence.validate_cases(
            ROOT,
            selected,
            required_domains=("entity", "notification"),
        )

        self.assertEqual(discovery_issues, [])
        self.assertEqual(validation_issues, [])
        self.assertEqual(len(validated), len(selected))
        for case in validated:
            test_source = (ROOT / case.test_path).read_text(encoding="utf-8")
            harness_source = (ROOT / case.harness_path).read_text(encoding="utf-8")
            self.assertNotIn("dart:io", test_source)
            self.assertNotIn("package:http/", test_source)
            self.assertNotIn("/adapters/", test_source)
            self.assertIn("api_contract_environment.dart", harness_source)
            self.assertIn("ApiContractEnvironment.resolve()", harness_source)
        ...
        with tempfile.TemporaryDirectory() as temporary_dir:
            ...
            with (
                ...
                mock.patch.object(
                    ...
                    return_value=mock.Mock(api_base="https://api.gamma.test"),
                ),
                mock.patch.object(
                    stackctl,
    ...
    def test_focused_message_case_uses_canonical_runner_and_candidate_preflight(
        ...
        message_path = (
            "quwoquan_app/test/api_integration/service/chat_service/chat/message/"
            "message_remote__api_integration_test.dart"
        )
        ...
        with tempfile.TemporaryDirectory() as temporary_dir:
            ...
            self.assertEqual(report["skipped"], 0)
            self.assertEqual(report["domains"], ["chat"])
            self.assertTrue(
                any(
                    "gamma-local active immutable candidate is required" in item
                    for item in payload["details"]
                )
            )
            self.assertTrue(
                any(
                    "gamma-local full startup receipt is required" in item
                    for item in payload["details"]
                )
            )
            self.assertFalse(
                any("GAMMA_BASE_URL" in item for item in payload["details"])
            )
