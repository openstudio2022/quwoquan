...
class CanonicalServiceTestDirectoryContractTest(unittest.TestCase):
    ...
    def test_cmd_local_contract_suffixed_test_is_accepted(self) -> None:
        failures = self._verify("cmd/api/main__local_contract_test.go")
        self.assertEqual(failures, [])

    def test_cmd_plain_go_test_is_rejected(self) -> None:
        failures = self._verify("cmd/api/main_test.go")
        self.assertTrue(
            any(
                "must end with '__local_contract_test.go'" in failure
                for failure in failures
            ),
            failures,
        )

    def test_cmd_api_integration_bypass_is_rejected(self) -> None:
        failures = self._verify("cmd/api/main__api_integration_test.go")
        self.assertTrue(
            any(
                "api_integration tests belong to a canonical" in failure
                for failure in failures
            ),
            failures,
        )
    ...
    def test_runtime_api_integration_test_outside_canonical_root_is_rejected(self) -> None:
        ...
        self.assertTrue(
            any(
                "api_integration tests belong to a canonical tests/api_integration tree"
                in failure
                for failure in failures
            ),
            failures,
