...
from quwoquan_ops.gate.app_architecture import report as vaa_report

GATE_REPO_PATH = ROOT / "quwoquan_ops/gate/gate_repo.sh"
GATE_SOURCE_PATH = ROOT / "quwoquan_ops/gate/verify_app_architecture.py"
# 实现单轨在 app_architecture/ 包内；源码形态断言指向包内对应模块。
GATE_PACKAGE_PATH = ROOT / "quwoquan_ops/gate/app_architecture"
GATE_CONSTANTS_SOURCE_PATH = GATE_PACKAGE_PATH / "constants.py"
...
def test_entry_file_rule_has_exactly_one_definition() -> None:
    ...
    唯一赋值处在实现包的 `constants.py`；薄入口只 re-export，不再出现赋值。
    """
    tree = ast.parse(GATE_CONSTANTS_SOURCE_PATH.read_text(encoding="utf-8"))
    bindings = [
        node.value
...
def test_retired_baseline_must_remain_absent(
    ...
    # verify_retired_baseline_absent 读的是实现包 report 模块的全局。
    monkeypatch.setattr(vaa_report, "BASELINE_PATH", baseline_path)
...
def test_cli_has_no_baseline_write_or_allowance_path() -> None:
    # 覆盖薄入口与实现包全部模块，防止吸收入口藏进包内。
    source = "\n".join(
        [GATE_SOURCE_PATH.read_text(encoding="utf-8")]
        + [
            module_path.read_text(encoding="utf-8")
            for module_path in sorted(GATE_PACKAGE_PATH.glob("*.py"))
        ]
    )
...
def test_strict_zero_cli_blocks_observed_violations(
    ...
    # main 经实现包 report 模块查找这些全局，patch 必须落在包模块上。
    monkeypatch.setattr(vaa_report, "BASELINE_PATH", tmp_path / "retired.json")
    monkeypatch.setattr(vaa_report, "load_roster", lambda: roster)
    monkeypatch.setattr(
        vaa_report,
        "evaluate",
        lambda _: {
