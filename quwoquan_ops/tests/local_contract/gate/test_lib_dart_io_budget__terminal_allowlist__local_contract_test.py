...
GATE_PATH = ROOT / "quwoquan_app" / "scripts" / "runtime" / "platform" / "verify_lib_dart_io_budget.py"
...
class LibDartIoBudgetTerminalAllowlistTest(unittest.TestCase):
    ...
    def test_write_baseline_cannot_resurrect_the_allowlist_to_excuse_a_violation(
        self,
    ) -> None:
        """退役后不得靠重建基线把新违规洗成豁免。

        规则 14 对这条门禁写的是「不接受豁免登记」。只要 ``--write-baseline`` 还能重建
        文件，加一个 ``dart:io`` 导入再跑一次就能让门禁转绿，规则等于没有。
        """
        self._write_importer("content/content/post/presentation/page.dart")
        ...
        self.assertEqual(result, 2)
        self.assertIn("cannot be rebuilt", stderr)
        self.assertFalse(self.allowlist.exists())
