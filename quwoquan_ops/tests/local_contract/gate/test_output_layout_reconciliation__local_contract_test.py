...
class OutputLayoutReconciliationContractTest(unittest.TestCase):
    ...
    def test_confirmed_apply_resolves_the_plan_ref_against_the_repo_runs_root(
        ...
        """带确认的 apply 必须真的能解析计划路径。

        其余用例都在到达计划解析之前就返回了（缺确认、锁冲突），于是
        `_canonical_output_layout_plan_ref` 里一个未 import 的 `repo_runs_root`
        长期没被执行到：整条 apply 路径一调用就 NameError，而计划生成看着是好的。
        """
        with tempfile.TemporaryDirectory() as temporary_dir:
            # macOS 的 /var/folders 临时目录本身是软链，解析器会因此判定路径含别名。
            root = Path(temporary_dir).resolve()
            runs_root = root / "runs"
            plan_path = runs_root / "run-id" / reconciliation.PLAN_FILENAME
            plan_path.parent.mkdir(parents=True)
            plan_path.write_text("{}", encoding="utf-8")

            with mock.patch.object(stackctl, "repo_runs_root", return_value=runs_root):
                resolved = stackctl._canonical_output_layout_plan_ref(str(plan_path))

            self.assertEqual(resolved, plan_path.resolve())

            outside = root / reconciliation.PLAN_FILENAME
            outside.write_text("{}", encoding="utf-8")
            with (
                mock.patch.object(stackctl, "repo_runs_root", return_value=runs_root),
                self.assertRaises(reconciliation.OutputLayoutReconciliationError),
            ):
                stackctl._canonical_output_layout_plan_ref(str(outside))
