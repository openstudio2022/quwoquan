...
class UiMockIsolationStructuralCriteriaTest(unittest.TestCase):
    ...
    def test_test_support_cross_domain_fixture_reader_is_caught(self) -> None:
        gate = _load_gate()
        source = (
            "import 'dart:io';\n"
            "final seed = objectContractExampleReader"
            ".requireExample('entity', 'entity_homepage_core');\n"
        )
        violations = gate.test_support_fixture_reader_violations(
            Path("test/support/runtime/fixtures/example_reader.dart"),
            source,
        )
        self.assertTrue(any("dart:io" in item for item in violations))
        self.assertTrue(any("requireExample" in item for item in violations))
        self.assertTrue(
            any("objectContractExampleReader" in item for item in violations)
        )

    def test_test_support_document_domain_lookup_is_caught(self) -> None:
        gate = _load_gate()
        violations = gate.test_support_fixture_reader_violations(
            Path("test/support/service/entity/homepage_builder.dart"),
            "final seed = reader.document('entity');\n",
        )
        self.assertEqual(
            violations,
            ["禁止 test/support 通过 document(domain) 选择跨域场景"],
        )

    def test_typed_builder_without_io_is_allowed(self) -> None:
        gate = _load_gate()
        source = (
            "final seed = HomepageWireExample("
            "homepageId: 'homepage-1', title: '示例');\n"
        )
        self.assertEqual(
            gate.test_support_fixture_reader_violations(
                Path("test/support/service/entity/homepage_builder.dart"),
                source,
            ),
            [],
