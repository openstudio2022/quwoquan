...
from quwoquan_ops.gate.verify_runtime_media_playback_evidence import (
    MATRIX_SCHEMA,
    MATRIX_SCENARIO,
    REPORT_SCHEMA,
    SCENARIO,
...
class RuntimeMediaPlaybackEvidenceContractTest(unittest.TestCase):
    ...
    def test_rejects_missing_or_noncanonical_release_receipt_identity(self) -> None:
        ...
        evidence["release"]["readinessReceiptRef"] = "parallel/playback.json"  # type: ignore[index]
    ...
    def test_artifact_validation_rejects_preflight_release_drift(self) -> None:
        ...
        self.assertTrue(
            any("Range report.release.importRunId 与 playback release 不一致" in issue for issue in issues),
            issues,
        )
    ...
    def test_artifact_validation_recomputes_observability_qoe_thresholds(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_dir:
            artifact_root = Path(temporary_dir)
            evidence = _report()
            _materialize_valid_artifacts(artifact_root, evidence)
