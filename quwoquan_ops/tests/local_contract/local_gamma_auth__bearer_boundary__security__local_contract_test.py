...
from quwoquan_ops.cli.lib import local_gamma_auth
...
class LocalGammaAuthBearerBoundarySecurityLocalContractTest(unittest.TestCase):
    def test_acceptance_session_uses_standard_issuer_and_keeps_bearer_out_of_repr(self) -> None:
        ...
        auth = local_gamma_auth.LocalGammaAuth(
        ...
        with (
            ...
            mock.patch.object(local_gamma_auth, "prepare_local_gamma_auth", return_value=auth),
            mock.patch.object(local_gamma_auth.subprocess, "run", return_value=completed) as run,
        ):
            session = local_gamma_auth.open_local_gamma_acceptance_session(
    ...
    def test_authenticated_request_has_only_bearer_identity(self) -> None:
        session = local_gamma_auth.LocalGammaAcceptanceSession(
        ...
        with mock.patch.object(
            local_gamma_auth.request,
            "build_opener",
            return_value=opener,
        ):
            payload = local_gamma_auth.request_local_gamma_json(
                "https://gamma-api.quwoquan-env.test:19000",
    ...
    def test_run_scoped_subject_rejects_noncanonical_identity(self) -> None:
        with self.assertRaisesRegex(ValueError, "subject is invalid"):
            local_gamma_auth.open_local_gamma_acceptance_session(
                "https://gamma-api.quwoquan-env.test:19000",
    ...
    def test_acceptance_session_rejects_invalid_issuer_response(self) -> None:
        ...
        with (
            ...
            mock.patch.object(local_gamma_auth, "prepare_local_gamma_auth", return_value=auth),
            mock.patch.object(local_gamma_auth.subprocess, "run", return_value=completed),
        ):
