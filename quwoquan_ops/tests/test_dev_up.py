...
class DevUpTest(unittest.TestCase):
    ...
    def test_gamma_local_mirror_persists_caddy_state_on_host(self) -> None:
        ...
        self.assertIn(
            '${LOCAL_GAMMA_CADDY_DATA_ROOT:-../../../.qwq_output/local/gamma-local/caddy/data}:/data',
            gamma_compose,
        )
        self.assertIn(
            '${LOCAL_GAMMA_CADDY_CONFIG_ROOT:-../../../.qwq_output/local/gamma-local/caddy/config}:/config',
            gamma_compose,
        )
