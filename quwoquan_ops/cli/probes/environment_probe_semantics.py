...
CONTENT_FEED_CHECK_NAMES = frozenset(
    {"content_feed", "homepage_recommend", "video_book_feed", "premium_feed"}
)
FEED_MEDIA_SOURCE_CHECK_NAMES = CONTENT_FEED_CHECK_NAMES
...
def _feed_media_slice_urls(payload: str, media_origin: str) -> dict[str, str]:
    ...
    def _add(raw: object, kind: str) -> None:
        ...
        # 同一视频可能同时出现在 mediaUrls 和 videoUrl；播放职责优先，不能
        # 因图片列表先遍历就丢掉 Range 验证。
        if kind == "video" or absolute not in urls:
            urls[absolute] = kind
