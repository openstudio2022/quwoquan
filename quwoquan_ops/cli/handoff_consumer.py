...
def validate_named_evidence_ref(
    ...
    *,
    ...
    validate_named_evidence_ref_payload(
        receipt, plan=plan, registry=registry, label=relative
    )
...
def validate_handoff_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    owner_manifest_ref = normalize_repo_relative_path(
        payload["owner_manifest_ref"], ROOT
    )
    plan_ref, plan = _load_json_ref(payload["review_plan_ref"], label="review plan")
    if owner_manifest_ref != (plan.get("owner_manifest_identity") or {}).get("ref"):
        raise HandoffConsumerError("handoff owner manifest ref 与 plan 不一致")
    try:
    ...
    if consolidation.get("terminal", {}).get("status") != "PASS":
        raise HandoffConsumerError("handoff review consolidation 非 PASS")
    if (
        ...
        raise HandoffConsumerError("handoff consolidation exact identity chain mismatch")
    identity = {
...
def consume(path: Path) -> dict[str, Any]:
    _, payload = _load_json_ref(path.as_posix(), label="handoff manifest")
