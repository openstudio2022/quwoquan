...
def _record_receipt(
    ...
    replay = next((
        ...
        and receipt.get("platformRecordId") == platform_record_id
        and receipt.get("readbackMethod") == row["readback"]
        and receipt.get("readbackDigest") == _sha256_file(evidence_path)
    ), None)
    if replay is not None:
        replay_digest = "sha256:" + hashlib.sha256(_canonical_bytes(replay)).hexdigest()
        replay_path = root / channel_id / (replay_digest.removeprefix("sha256:") + ".json")
        return {"exitCode": 0, "summary": f"identical receipt already recorded for {channel_id}",
                "details": [str(replay_path)], "receipt": replay, "receiptPath": str(replay_path)}
