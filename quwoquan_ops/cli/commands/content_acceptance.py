...
- `register_content_readiness_parser` / `register_uat_parsers`：三个子命令的
  argparse 表面（content-readiness 与 content-uat/account-enforcement-uat 在
  build_parser 中不相邻，因此拆成两个注册函数，注册顺序保持不变）；
