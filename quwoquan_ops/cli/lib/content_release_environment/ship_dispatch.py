    ...
    *,
    ...
    try:
        with release_operation_guard(
            ...
            locked_admission = admit_environment_release(
                args, repo_root=repo_root, output_root=output_root, release_root=release_root,
            )
            if locked_admission != admission:
                raise ValueError("DATA.RELEASE.HANDOFF_AUTHORITY_DRIFT: release changed before operation lock")
