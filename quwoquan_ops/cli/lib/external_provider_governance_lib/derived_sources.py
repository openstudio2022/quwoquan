...
def _service_roots() -> list[Path]:
    ...
    return sorted(
        path
        for path in _entry.SERVICES_ROOT.iterdir()
...
def _descriptor_output(service_id: str, object_root: Path) -> Path:
    service_root = _entry.SERVICES_ROOT / service_id
...
def _find_adapter_source(adapter_id: str) -> Path | None:
    ...
    if source.is_relative_to(_entry.SERVICES_ROOT):
        relative = source.relative_to(_entry.SERVICES_ROOT)
        service_root = _entry.SERVICES_ROOT / relative.parts[0]
