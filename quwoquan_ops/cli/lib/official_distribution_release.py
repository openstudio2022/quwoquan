...
_KINDS = {
    "web": "publicWeb",
    "app-release": "androidOfficialRelease",
}
...
def deploy_official_distribution(
    *,
    ...
    receipt = {
        "schema": "qwq.official-distribution.receipt.v1",
        "artifactKind": kind,
...
def inspect_official_distribution(
    *,
    ...
    latest_path = root / "downloads" / "android" / "latest.json"
    ...
    return {
        "schema": "qwq.official-distribution.inspection.v1",
...
def _deploy_web(
    *,
    ...
    if manifest.get("schema") != "qwq.public-web.release.v1":
...
def _deploy_android(
    *,
    ...
    if manifest.get("schema") != "qwq.android.official-release.v1":
    ...
    relative_apk = Path("downloads") / "android" / version / build / artifact_name
    ...
    latest_path = distribution_root / "downloads" / "android" / "latest.json"
    previous = ""
    if latest_path.is_file():
...
def _release_manifest(path: Path) -> tuple[dict[str, Any], str]:
    ...
    if (
        ...
        raise OfficialDistributionReleaseError("release manifest is not deployable")
    declared = str(manifest.get("manifestDigest") or "")
    unsigned = dict(manifest)
    unsigned.pop("manifestDigest", None)
    actual = "sha256:" + hashlib.sha256(_canonical_bytes(unsigned)).hexdigest()
    if declared != actual or _SHA256.fullmatch(declared) is None:
        raise OfficialDistributionReleaseError("release manifest digest mismatch")
...
def _verify_component_binding(
    *,
    ...
    components = release_manifest.get("artifacts")
    ...
    if component.get("manifestSHA256") != digest:
        raise OfficialDistributionReleaseError(
            f"release manifest distribution digest mismatch: {component_key}"
        )
    expected_schema = str(component.get("schema") or "")
