...
def validate_contract(value: object) -> None:
    ...
    if value.get("schema_id") != "governance-pipeline-admission-contract" or value.get("schema_version") != 2:
    ...
    expected_sources = {
        "workflow_resolve_source": "quwoquan_ops/policies/workflow_resolution_contract.yaml",
    ...
    expected_schemas = {
        ...
        "evidence_bundle_receipts": ("owner_manifest", "workflow_resolve", "local_scope_ready", "local_release_ready", "review_plan", "named_evidence", "review_consolidation", "handoff", "human_calibration", "objective_inspect", "hotl_inspect", "hosted_authority_source", "external"),
    ...
    if not isinstance(layers, Mapping) or len(layers) < 20:
    ...
    expected_current = {"owner_manifest_root", "owner_manifest_target", "local_readiness_mode", "local_readiness_paths", "evidence_bundle_root", "managed_identity_paths", "named_evidence_layers", "provider_adapters", "external_provider_interfaces"}
    ...
    if not isinstance(current.get("provider_adapters"), Mapping) or not isinstance(current.get("external_provider_interfaces"), Mapping):
