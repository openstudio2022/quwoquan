...
2. 对同一 Alpha 网关做首页 feed 与视频书（video works）的 Remote readback，HTTP 200
   且 items 非空。
...
# 首页（推荐 feed）与视频书（video works）各一条只读查询；与 content-api-consumer 同源。
CONTENT_READBACK_QUERIES: tuple[tuple[str, dict[str, str], str], ...] = (
    ("home-feed", {"sort": "recommend", "channelId": "recommend", "limit": "20"}, "objectCards"),
    ("video-book", {"identity": "work", "type": "video", "limit": "20"}, "items"),
)
...
def content_readback(*, target: str = "alpha-local", timeout_seconds: float = 12.0) -> dict[str, Any]:
    """对 Alpha 网关做首页 feed 与视频书 readback；任一非 200 或空结果即 typed blocker。"""
    ...
    for name, query, collection in CONTENT_READBACK_QUERIES:
        try:
            observation = _default_http_request(
                api_base=api_base, ca_file=ca_file, bearer_token="", attestation_token="",
    ...
    return {"apiBase": api_base, "results": results, "failures": failures, "passed": not failures}
