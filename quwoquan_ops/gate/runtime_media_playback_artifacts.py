"""runtime-media playback 归档证据的可重放校验。"""
...
from quwoquan_ops.gate.runtime_media_playback_qoe import validate_qoe_payload
...
def _validate_range_artifact(
    ...
    for field in (
        ...
        if release.get(field) != report_release.get(field):
            issues.append(
                f"{prefix}.Range report.release.{field} 与 playback release 不一致"
            )
    ...
    if payload.get("target") != report_environment.get("target"):
        issues.append(f"{prefix}.Range report.target 与 playback target 不一致")
    ...
    for evidence_field, report_field in (
        ...
        if video.get(evidence_field) != report_media.get(report_field):
            issues.append(
                f"{prefix}.Range report.video.{evidence_field} 与 playback media 不一致"
            )
    if video.get("postId") != report_post.get("postId"):
        issues.append(f"{prefix}.Range report.video.postId 与 playback post 不一致")
