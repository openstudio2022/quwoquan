...
def main() -> int:
    ...
    retired_error_contract = "contracts" + "/error_codes.md"
    if retired_error_contract in arch_text:
        issues.append(
            ".cursor/rules/01-arch-constraints.mdc must not point error codes to "
            + retired_error_contract
        )
    if "quwoquan_service/contracts/metadata/{domain}/{entity}/errors.yaml" not in arch_text:
