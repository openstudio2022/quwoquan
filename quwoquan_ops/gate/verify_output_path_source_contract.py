...
TEXT_SUFFIXES = frozenset({".py", ".sh", ".yaml", ".yml", ".md"})
...
RETIRED_PATTERNS = (
    ...
    re.compile(r"data/release(?:/|\\b)"),
    re.compile(r"data-releases"),
    re.compile(r"(?:QWQ_OUTPUT_ROOT|\.qwq_output)/(?:local|runs|release|observability|repo)(?:/|\\b)"),
    re.compile(r"env/(?:alpha|beta|gamma|prod|repo)/(?:packages|runtime|cache|tmp)(?:/|\\b)"),
    re.compile(r"(?:QWQ_OUTPUT_ROOT|\.qwq_output)/data/(?:runtime|runs|objects|cache|tmp|observability)(?:/|\\b)"),
    re.compile(r"env/prod/control(?:/|\\b)"),
    re.compile(r"env/repo/local/test-cache(?:/|\\b)"),
    re.compile(
