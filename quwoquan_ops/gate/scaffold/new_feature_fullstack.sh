...
if [[ -z "$domain" || -z "$capability" || -z "$story" ]]; then
  echo "usage: bash quwoquan_ops/gate/scaffold/new_feature_fullstack.sh <domain-service> <business-capability> <story>" 1>&2
  echo "example: bash quwoquan_ops/gate/scaffold/new_feature_fullstack.sh discovery-content feed-orchestration-recommendation unified-items-cursor" 1>&2
  exit 2
fi
