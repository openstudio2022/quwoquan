#!/usr/bin/env python3
"""校验 runtime-media 视频播放非 dry-run 证据的最小商用合同。"""
...
from quwoquan_ops.gate.runtime_media_playback_artifacts import validate_report_artifacts

REPORT_SCHEMA = "runtime-media-video-playback-evidence-report"
MATRIX_SCHEMA = "runtime-media-video-playback-evidence-matrix-report"
SCENARIO = "runtime_media.video_playback_evidence"
MATRIX_SCENARIO = "runtime_media.video_playback_evidence_matrix"
...
def validate_evidence_document(
    ...
    *,
    ...
    """返回所有违反 runtime-media 视频播放证据合同的原因；空列表表示通过。"""

    if not isinstance(document, dict):
        return ["runtime-media playback evidence 根对象必须为 JSON object"]
    ...
    if require_matrix and not is_matrix:
        return [
            "发布级播放门禁必须提供四环境矩阵报告，单环境报告不能作为商用准入证据",
        ]
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--evidence", required=True, help="playback JSON report 或 matrix manifest")
    parser.add_argument(
        "--require-matrix",
        action="store_true",
...
def main() -> int:
    ...
    if not path.is_file():
        print(f"[verify_runtime_media_playback_evidence] FAIL: evidence file not found: {path}")
        return 2
    try:
        document = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        print(f"[verify_runtime_media_playback_evidence] FAIL: unable to read JSON: {exc}")
        return 2
    ...
    if issues:
        print("[verify_runtime_media_playback_evidence] FAIL")
        for issue in issues:
            print(f"  - {issue}")
        return 1
    print("[verify_runtime_media_playback_evidence] OK")
    return 0
