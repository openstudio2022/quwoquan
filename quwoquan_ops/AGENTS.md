...
- 本地运行使用 [`multi-environment-instance-isolation`](../specs/feature-tree/runtime/deliver-deploy-prod-pipeline/multi-environment-instance-isolation/spec.md) 的跨 worktree host-target 单实例、共享使用租约与 target executor fence；不同 target 可并行，矩阵及失败清理不得停止或改写未拥有的实例、设备、信任或转发资源。
