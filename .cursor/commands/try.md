...
**仅豁免**：不要求提前创建特性树节点（spec.md/design.md/plan.yaml/acceptance.yaml）。
...
|------|--------------------------------|------|
| 特性树分解（spec/design/plan/acceptance） | **必须** | **豁免** |
| DDD 分层约束 | 必须 | **必须** |
| metadata-first（新实体先更新 YAML） | 必须 | **必须** |
| operation / surface / route 由 metadata + codegen 唯一生成 | 必须 | **必须** |
...
- 若原型涉及 Repository、请求头、埋点 context 或导航路径，仍必须先收口到 metadata，不得在原型代码里手写 route/page/surface/operation 字符串作为长期实现
...
原型也会产生真实的实体和数据结构。如果原型绕过 metadata，数据模型、请求上下文与导航契约都无法被 codegen 保护，也无法在 `/land` 时自动生成正确的基线制品。**以正确方式完成原型，后续基线化成本更低**。
