...
| 5 | 基线漂移已处理 | `/opsx-verify` 漂移报告中无 BLOCKING 项（D1 SPEC_DRIFT、D2 IMPL_DRIFT、D3 DESIGN_DRIFT 已解决） |
| 6 | gate-full 通过 | `make gate-full` 全部通过，含特性树一致性检查 |

**若不满足**：不执行归档；输出**补全列表**，引导用户按列表补全后再执行 `/opsx-archive`。

### 补全列表（前置不满足时输出）

```
前置条件不满足，请先完成以下项后再执行 /opsx-archive：

□ [ ] 四类文档齐全：目标节点具备 spec.md、design.md、tasks.md、acceptance.yaml（见 00_FEATURE_TREE_STANDARD.md）
□ [ ] tasks 当前交付任务已完成：tasks.md 中当前交付任务全部 [x]；未完成项请先执行 /opsx-apply
□ [ ] acceptance 无 pending：所有 A1~An status 改为 implemented/waived/deferred，不得有 pending
□ [ ] acceptance tests 链接有效：status=implemented 的 An 的 tests[].file 已写入实际测试文件路径
□ [ ] 漂移已处理：执行 /opsx-verify，解决所有 BLOCKING 漂移项（SPEC/IMPL/DESIGN drift）
□ [ ] gate-full 通过：执行 make gate-full，修复所有失败项（metadata、结构、契约测试、特性树一致性）

补全后重新执行：/opsx-verify（验证通过）→ /opsx-archive
```
...
4. 读取 `acceptance.yaml` 中 `tests[]`，逐一确认文件存在（参见步骤 3 的 gate 自动验证）
5. 确认已执行 `/opsx-verify` 并无 BLOCKING 漂移项
...
G3 通过后，AI Agent **必须回写以下两处**，使 gate 的 lifecycle 检查可通过：
...
将目标节点（及其所有子节点，若也已完成）的 `status` 改为：
```yaml
status: completed
```
...
下一步：/submit-with-gate
```

3. 下一步：`/submit-with-gate` 提交入库
