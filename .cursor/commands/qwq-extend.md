...
```
自动执行：
1. 创建 `services/{name}-service/` 目录结构（含 cmd/internal/configs/Makefile）
2. 创建 `contracts/metadata/{domain}/` 目录
3. `make verify-metadata`
4. `make codegen --service={name}` → 完整服务骨架
