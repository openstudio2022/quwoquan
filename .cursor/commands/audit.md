...
```bash
cd /path/to/quwoquan && bash quwoquan_ops/gate/scaffold/verify_feature_tree_refactor.sh
```
...
自然语言等价触发：用户直接描述与本命令目标相同的需求时，也按 `/audit` 语义执行；执行前仍需按 `docs/agent_context_contract.md` 完成 Spec Entry / Pre-work Reflection，完成后按 Exit Review 收口。
