...
```
自动执行：
1. 创建 `errors.yaml` 骨架（code/http_status/recovery action/user_message.zh/en/dart_const/go_const）
...
5. 若 `bump-version`：在 `AssistantSessionManager.load()` 中添加迁移逻辑，过往版本本降级为存量
