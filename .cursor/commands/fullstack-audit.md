...
id: fullstack-audit
category: Quality
description: 端云全栈一致性审计（语义 + 结构 + metadata + ArchUnit）
---

执行全栈一致性审计，覆盖端侧语义审计 + 云侧结构约束 + metadata↔代码同步 + DDD 层级约束。
...
**1.3 cloud/ 生成代码一致性**
```bash
# 检查 cloud/models/ 中的 DTO 是否包含 DO NOT EDIT 标记
...
for svc in pathlib.Path("services").iterdir():
    ...
    for go_file in (svc / "internal").rglob("*.go") if (svc / "internal").exists() else []:
        if "/infrastructure/" in str(go_file) or "/tests/" in str(go_file):
            continue
        content = go_file.read_text(encoding="utf-8")
        # 检查是否在非 infrastructure 层直接使用数据库驱动
