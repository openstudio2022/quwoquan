...
### 工程规范规格（若涉及以下维度必须在 spec 中冻结）

| 维度 | spec 中必须明确的内容 |
|------|----------------------|
| **DDD 分层** | 涉及哪些域层，新增代码归属的 `domain / application / adapters / infrastructure` 边界 |
| **强类型** | 新增数据结构的端侧 DTO 类和云侧 struct，禁止 `Map<String, dynamic>` / `interface{}` |
| **存储选型** | 存储引擎选型理由（Mongo/PG/Redis/S3）、成本效率、TTL/归档/弹性策略 |
| **存储无关** | Repository interface 定义位置和存储实现可替换性 |
| **端云一致** | 需要同步的字段列表、枚举值、错误码，以及 metadata YAML 变更计划 |

### 可观测与指标规格（若需求涉及用户可见功能必须冻结）

| 维度 | spec 中必须明确的内容 |
|------|----------------------|
| **埋点需求** | 涉及页面的行为埋点方案（曝光/停留/深度/来源/互动） |
| **成功指标** | 用什么黄金指标或二层指标衡量需求成功 |
| **推荐回流** | 新行为信号是否需要回流推荐特征，回流路径和时效 |
| **性能基线** | 关键路径的 TTI / API P99 / 帧率目标值 |
| **AB 实验** | 是否需要 AB，实验层（recall/rank/rerank）和评估指标 |
