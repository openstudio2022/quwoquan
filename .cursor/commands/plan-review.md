...
| 测试与质量专家 | 验收意图 UAT/SIT/GWT/contract 完整；三层测试 证据矩阵可形成；每条证据声明 test_object 与 quality_facet；Mock↔Remote 一致（api_integration 断言在 local_contract 有对应）；`acceptance.yaml` 声明的测试路径可落地；门禁可绿。 |
| 运维与运营专家 | SLO/KPI、告警阈值、采样与 TTL；灰度与回滚演练；新页面/新 API/新行为信号的**埋点与归因链**（曝光/停留/异常 + referralSource/feedRequestId）；异常恢复、性能、安全隐私、可观测、可靠性/可用性、数据一致性质量维度完整；AB 可分桶；env-seed-first（alpha/beta/gamma）。 |
...
2. **刷新 `acceptance.yaml`**：将不符合项转化为可测 GWT/UAT/SIT/contract，绑定三层测试、磁盘测试路径、test_object、quality_facet、SLO/观测/安全引用。
...
- 异常、性能、安全、观测、可靠性或数据一致性适用但没有证据路径。
