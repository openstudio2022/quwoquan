...
> 真相源：`deploy/shared/process_domain_mapping.yaml`（domain 归属）、`deploy/shared/module_package_mapping.yaml`（包→模块）、`deploy/shared/workload_topology_inventory.yaml`（部署形态三态）。
...
- 跨技术栈服务（如 Python recommendation）从第一天即独立 workload，不退化为 sidecar——`deploy/service/recommendation-service/kustomize/**` 是已落地的标准参考模板。
...
| `standalone-workload` | 第一天即独立 Deployment/StatefulSet | `recommendation-service`、`realtime-gateway`、`rtc-service`、`livekit-sfu`、`coturn`、`entity/tag/ops/assistant-service` |
| `split-candidate` | 当前并入 seed-box，将来按本手册抽出 | `content`、`chat`、`user`、`circle`、`integration`、`notification` |
...
3. **建独立 kustomize**（照搬模板，见第 6 节）：`deploy/service/D-service/kustomize/{base,overlays/{dev,integration,beta,prod}}`，保持 Service 名与 `/v1/<domain>/*` 上游不变。
4. **wire 进 root**：把 `D-service` prod overlay 加入 `deploy/kustomization/{cloud}-prod`，inventory 置 `wired_to_prod_root: true`。
...
- `deploy/service/D-service/kustomize/base/deployment.yaml` —— 单业务容器 + 标准 probe（readiness/liveness/startup）+ requests/limits + topologySpreadConstraints。
- `deploy/service/D-service/kustomize/base/service.yaml` —— `metadata.name: D-service`（对外稳定标识）。
- `deploy/service/D-service/kustomize/base/hpa.yaml` —— `scaleTargetRef` 指向同名 Deployment，CPU/内存 target。
- `deploy/service/D-service/kustomize/base/pdb.yaml` —— `minAvailable: 1`。
- `deploy/service/D-service/kustomize/base/kustomization.yaml` —— 聚合上面四件。
- `deploy/service/D-service/kustomize/overlays/<env>/kustomization.yaml` —— 独立 `configMapGenerator`（命名 `D-deploy-params`，避免与 seed-box `deploy-params` 冲突）+ `images.newTag` + `replacements`（APP_ENV/CONFIG_VERSION/IMAGE_VERSION/REPLICAS/HPA 阈值）。
