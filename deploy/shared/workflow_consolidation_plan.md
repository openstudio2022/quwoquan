...
| Workflow | 阶段 | 说明 |
|----------|------|------|
| **delivery-gate** | PR/合并入库 | L1+L2 质量门，阻塞合入 |
| **pre-release-gate** | v*-rc* 发布前 | L1+L2 → deploy → L3 → L4（L3 统一在此执行） |
...
### 4.3 Service Pipeline — 只做构建与部署校验，不做 L2

- **触发**：PR、push → main/master（paths: quwoquan_service/**, deploy/**）
- **调整**：
  - **移除** `build-go` 中的 `make test-unit`（L2 由 gate 负责）
  - **保留** `make build`、Python 镜像构建、`validate-deploy`
- **说明**：PR 时 gate 已跑 L2，Service Pipeline 只负责构建与 kustomize 校验。
