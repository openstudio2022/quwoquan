...
| 03 | Delivery Gate | Merge Queue | merge queue 快速主门禁：拓扑 + L1 + L2 |
| 04 | Pre-Release Gate | Integration / Merge Queue | deploy integration → L3 → L4 → gamma smoke |
| 05 | App Env Device Matrix | Merge Queue / Self-hosted | self-hosted 动态设备矩阵唯一入口 |
...
### 4.1 delivery-gate — merge queue 主门禁（L1+L2）

- **触发**：`merge_group`、`workflow_dispatch`
...
- **说明**：merge queue 已由 `03` 兜底，01 不再参与主干阻断。
...
### 4.4 pre-release-gate — merge queue 发布验证
...
| 5 | `pre-release-gate.yml` | 改为 merge_group；移除重复 gate；保留 deploy/L3/L4/gamma smoke | ✅ |
| 6 | `app-env-device-matrix-self-hosted.yml` | 升级为唯一 05 设备矩阵入口；支持 merge_group / workflow_call / 手动 | ✅ |
| 7 | `deploy/shared/ci_cd_end_to_end_design.md` | 更新 workflow 名称、触发与主干门禁说明 | ✅ |
| 8 | `merge-dev1.0-to-main.yml` | 删除；以显式 PR + merge queue 取代 | ✅ |

---

## 7. 风险与回退

- **风险**：merge queue required checks 配置不完整，会出现代码进入 `main` 前未真正经过 `03/04/05`。
