...
| `vars.MEDIA_AVATAR_CDN_BASE_URL` | 可选 | 可选 | — | 可选 | chat-avatar 媒体基址 |
...
| `beta` | 本地启动网关与服务，App 注入 `APP_RUNTIME_ENV=beta` + `APP_DATA_SOURCE=remote` | 本地 Android/iOS 设备矩阵通过 |
| `gamma` | `04` 触发 ECS gamma hosted pre 链 + 本地 gamma Android/iOS assistant/avatar 旅程 | hosted/self-hosted 全绿并带证据 |
