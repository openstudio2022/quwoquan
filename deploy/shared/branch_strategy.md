...
> 与 `deliver_to_production_runbook.md`、`ci_cd_end_to_end_design.md` 对齐。当前策略已取消“定时 dev1.0 → main 自动合并”，统一改为**用户显式发起 PR，并受 main 的 pull request 合入规则保护**。
...
| **main** | 发布主干 | 仅通过 PR 更新；阻断项为 `03` / `04` / `05` |
...
- **统一门禁**：无论采用哪种模式，进入 `main` 前都必须通过 `main` 的 **pull request required checks**，并跑通：
  - `03. Delivery Gate`
  - `04. Pre-Release Gate`
...
- 进入 `main` 的动作必须由用户显式发起 PR，并由分支保护规则放行；禁止保留第二条“脚本等绿后自动 merge”路径。
