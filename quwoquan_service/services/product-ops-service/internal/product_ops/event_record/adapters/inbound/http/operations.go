...
func (s *OperationsHandler) handleReportRuntimeLogBatch(w http.ResponseWriter, r *http.Request) {
	startedAt := time.Now()
	result := "rejected"
	recordCount := 0
	defer func() {
		if s.onIngest != nil {
			s.onIngest(result, recordCount, time.Since(startedAt))
		}
	}()
	...
	if err != nil {
		switch {
		case errors.Is(err, eventapp.ErrInvalidRuntimeLogBatch):
			result = "invalid"
			writeEventAppError(w, r, productopsgenerated.AppErrorFromRuntimeLogBatchInvalid(err.Error()))
		case errors.Is(err, eventapp.ErrBatchInProgress):
			result = "unavailable"
...
func (s *OperationsHandler) handleInternalRuntimeLogIngest(w http.ResponseWriter, r *http.Request) {
	startedAt := time.Now()
	result := "rejected"
	recordCount := 0
	defer func() {
	...
	if err != nil {
		if errors.Is(err, eventapp.ErrInvalidRuntimeLogBatch) {
			result = "invalid"
			writeEventAppError(w, r, productopsgenerated.AppErrorFromRuntimeLogBatchInvalid(err.Error()))
			return
		}
...
func (s *OperationsHandler) handleGetEventSummary(w http.ResponseWriter, r *http.Request) {
	...
	query := eventapp.EventSummaryQuery{
...
func (s *OperationsHandler) handleGetRtcMediaQoeSummary(
...
func (s *OperationsHandler) handleGetEventDrilldown(w http.ResponseWriter, r *http.Request) {
	limit := 50
	...
	query := eventapp.EventDrilldownQuery{
		LogType:       strings.TrimSpace(r.URL.Query().Get("logType")),
		EventType:     strings.TrimSpace(r.URL.Query().Get("eventType")),
		PageName:      strings.TrimSpace(r.URL.Query().Get("pageName")),
		AppVersion:    strings.TrimSpace(r.URL.Query().Get("appVersion")),
...
func (s *OperationsHandler) handleGetRuntimeLogSummary(w http.ResponseWriter, r *http.Request) {
	if s.runtimeLogs == nil {
	...
	out, err := s.runtimeLogs.GetRuntimeLogSummary(r.Context(), eventapp.RuntimeLogSummaryQuery{
...
func (s *OperationsHandler) handleGetRuntimeLogDrilldown(w http.ResponseWriter, r *http.Request) {
	if s.runtimeLogs == nil {
		writeEventAppError(w, r, productopsgenerated.AppErrorFromRuntimeLogstoreUnavailable("runtime diagnostic query is not configured"))
		return
	}
	...
	out, err := s.runtimeLogs.GetRuntimeLogDrilldown(r.Context(), eventapp.RuntimeLogDrilldownQuery{
		Signal:            strings.TrimSpace(r.URL.Query().Get("signal")),
		Severity:          strings.TrimSpace(r.URL.Query().Get("severity")),
		ErrorCode:         strings.TrimSpace(r.URL.Query().Get("errorCode")),
		Fingerprint:       strings.TrimSpace(r.URL.Query().Get("fingerprint")),
