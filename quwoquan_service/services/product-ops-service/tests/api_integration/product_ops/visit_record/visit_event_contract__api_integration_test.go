...
var (
	...
	productOpsEmbeddedPG    *testsupport.EmbeddedPostgres
)
...
func TestMain(m *testing.M) {
	...
	realVisitStore = visitpersistence.NewMongoVisitStore(telemetryMongoDB)
	...
	if postgresDSN == "" {
		productOpsEmbeddedPG, postgresDSN, err = testsupport.StartEmbeddedPostgres()
		if err != nil {
			panic(err)
		}
	}
	...
	if err := productOpsEmbeddedPG.Stop(); err != nil {
		panic(err)
	}
	cancelCleanup()
	os.Exit(code)
