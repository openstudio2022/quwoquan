# Source: circle/circle_management/gathering/events.yaml; event_ref: circle.gathering.GatheringParticipationChanged
# Code generated by tools/codegen_rec_model_python. DO NOT EDIT.

from __future__ import annotations

from datetime import datetime
from typing import Any

from pydantic import BaseModel, ConfigDict

from enum import Enum
from pydantic_core import core_schema


class _ContractEnum(str, Enum):
    @classmethod
    def __get_pydantic_core_schema__(cls, source_type, handler):
        return core_schema.no_info_before_validator_function(
            cls._validate_wire,
            handler(source_type),
            serialization=core_schema.plain_serializer_function_ser_schema(cls._serialize_wire),
        )

    @classmethod
    def _serialize_wire(cls, value):
        return cls._validate_wire(value).value

    @classmethod
    def _validate_wire(cls, value):
        if isinstance(value, cls):
            return value
        if type(value) is not str:
            raise ValueError("enum wire value must be a string")
        return cls(value)


class GatheringLifecycleStatus(_ContractEnum):
    VALUE_DRAFT = "draft"
    VALUE_PUBLISHED = "published"
    VALUE_CANCELLED = "cancelled"
    VALUE_COMPLETED = "completed"


class GatheringParticipationState(_ContractEnum):
    VALUE_INVITED_PENDING = "invited_pending"
    VALUE_APPLICATION_PENDING = "application_pending"
    VALUE_ACTIVE = "active"
    VALUE_CLOSED = "closed"


class GatheringEventPayload(BaseModel):
    """事务日志与 outbox 共用的 typed Gathering event payload。"""
    gatheringId: str
    aggregateVersion: int
    lifecycleStatus: GatheringLifecycleStatus
    actorPersonaId: str | None = None
    participantPersonaId: str | None = None
    participationState: GatheringParticipationState | None = None
    revisionId: str | None = None
    revisionNumber: int | None = None
    revisionDigest: str | None = None
    occurredAt: datetime

    model_config = ConfigDict(extra="forbid")
