...
from generated.recommendation.ranked_recommendation_window.models.request_response import ReleaseQueryReadinessProof, ReleaseQueryClassEvidence
...
class MongoReleaseCandidateOps:
    ...
    def read_release_readiness(self, binding, snapshot_digest, *, now=None):
        ...
        for post in event.snapshot.posts:
            ...
            if (row["ownerStatus"] == "active" and admission["status"] == "active" and admission["scope"] == "global" and admission["qualityAdmission"] == "approved" and admission["qualityScore"] >= .75 and until > now + timedelta(seconds=1) and post.contentIdentity == "work" and post.contentType == "video" and post.videoUrl and post.durationMs > 0):
        ...
        def evidence(kind, values):
            return ReleaseQueryClassEvidence(queryClass=kind, objectSetDigest=digest([{"objectType": "content.post", "objectId": p.identity.objectId} for p in values]), documentsDigest=digest([{"objectId": p.identity.objectId, "documentDigest": p.documentDigest} for p in values]))
        proof = ReleaseQueryReadinessProof(binding=canonical(binding), sourceClosureDigest=event.snapshot.sourceClosureDigest, objectSetDigest=event.snapshot.objectSetDigest, snapshotDigest=event.snapshot.snapshotDigest, documentsDigest=digest([canonical(p) for p in event.snapshot.posts]), checkpointVersion=event.sourceVersion, queryClasses=[evidence("home", posts), evidence("premium", premium), evidence("required_detail", event.snapshot.posts)], premiumAdmissionDigest=digest(admissions), premiumObjectSetDigest=evidence("premium", premium).objectSetDigest, verifiedAt=now, validUntil=expiry, proofDigest="")
        proof.proofDigest = digest(proof, "proofDigest")
        if canonical(binding) != canonical(self._release_runtime_binding(binding.release)):
            raise ReleaseNotReady("managed generation changed during proof query")
        return proof
