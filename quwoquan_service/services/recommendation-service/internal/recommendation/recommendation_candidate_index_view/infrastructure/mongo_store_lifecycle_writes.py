"""内容候选生命周期与 premium 准入投影写入（``MongoCandidateIndexStore`` mixin）。

拆分自原 ``mongo_store.py``（行数治理）：候选 upsert/remove、实体标签刷新、
tombstone、源事件收件箱幂等、premium 准入投影与源消费失败记录。
"""
...
class MongoCandidateLifecycleWriteOps:
    """候选生命周期写操作；集合属性由组合根 ``__init__`` 装配。"""
