...
def test_candidate_transaction_replay_readiness_and_retained_release(mongo_database, real_redis):
    ...
    proof = store.read_release_readiness(a.binding, a.snapshot.snapshotDigest)
    assert {x.queryClass for x in proof.queryClasses} == {"home", "premium", "required_detail"}
    assert proof.proofDigest == digest(proof, "proofDigest")
    ...
    assert store.read_release_readiness(a.binding, a.snapshot.snapshotDigest).snapshotDigest == a.snapshot.snapshotDigest
    ...
    assert store.read_release_readiness(b.binding, b.snapshot.snapshotDigest)
    ...
    ranked = store.list_release_for_ranking(fence, scenario="premium_stream", subject_id="viewer", limit=20)
    assert [row["contentId"] for row in ranked] == ["p"]
