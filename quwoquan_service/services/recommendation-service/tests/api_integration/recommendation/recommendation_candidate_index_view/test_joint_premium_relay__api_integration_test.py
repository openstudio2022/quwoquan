...
def test_real_approval_relay_candidate_query_and_revocation(joint_owner, mongo_database):
    ...
    assert ready("A").premiumAdmissionDigest
    ...
    assert ready("B").snapshotDigest == events["B"].snapshot.snapshotDigest
    ...
    with TestClient(api) as query_client:
        ...
        proof = response.json()
        assert proof["binding"] == query_body["binding"] and proof["snapshotDigest"] == query_body["snapshotDigest"]
        assert {r["queryClass"] for r in proof["queryClasses"]} == {"home", "premium", "required_detail"}
