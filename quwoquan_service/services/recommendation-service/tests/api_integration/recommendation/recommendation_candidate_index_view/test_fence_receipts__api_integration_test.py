...
def test_fence_checks_real_prepared_candidate_without_mutation(mongo_database):
    ...
    source, home, premium_posts = candidates.read_release_supply_projection(event.binding, event.snapshot.snapshotDigest)
    assert source.snapshot.snapshotDigest == event.snapshot.snapshotDigest
    ...
    with pytest.raises(ReleaseNotReady, match="safety authority or proof-time policy unavailable"):
        runner.apply(values)
    assert receipts.collection.count_documents({}) == 0
