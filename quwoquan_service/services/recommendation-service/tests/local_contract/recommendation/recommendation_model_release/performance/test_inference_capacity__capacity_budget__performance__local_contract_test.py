...
from generated.recommendation.recommendation_model_release.models.request_response import (  # noqa: E402
    CandidateInput,
    CandidateScore,
    ModelScoreRequest,
    ModelScoreResponse,
...
def _service_root() -> Path:
    for parent in Path(__file__).resolve().parents:
        if (parent / "build" / "Dockerfile").is_file():
            return parent
    raise AssertionError("recommendation-service root not found")
...
def test_dockerfile_uses_configurable_multi_worker_startup() -> None:
    dockerfile = _service_root() / "build" / "Dockerfile"
