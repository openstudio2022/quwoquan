...
def test_home_is_full_source_with_nonempty_premium_subset_and_query_is_read_only():
    ...
    source, home, premium = store.read_release_supply_projection(event.binding, event.snapshot.snapshotDigest, now=_NOW)
    assert source.snapshot.objectSetDigest == event.snapshot.objectSetDigest
    assert [post.identity.objectId for post in home] == ["a", "b"]
    assert [post.identity.objectId for post in premium] == ["a"]
...
def test_one_premium_revoked_recomputes_subset_without_invalidating_source():
    ...
    source_before, home_before, premium_before = store.read_release_supply_projection(event.binding, event.snapshot.snapshotDigest, now=_NOW)
    ...
    source_after, home_after, premium_after = store.read_release_supply_projection(event.binding, event.snapshot.snapshotDigest, now=_NOW)
    assert source_after.snapshot.objectSetDigest == source_before.snapshot.objectSetDigest
    assert [post.identity.objectId for post in home_after] == [post.identity.objectId for post in home_before] == ["a", "b"]
    assert [post.identity.objectId for post in premium_before] == ["a", "b"]
    assert [post.identity.objectId for post in premium_after] == ["b"]
    with pytest.raises(ReleaseNotReady, match="safety authority or proof-time policy unavailable"):
        _proof(store, event)
