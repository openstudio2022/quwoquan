...
def _post_values(**overrides) -> dict[str, str]:
    payload = {
        ...
        "environment": None, "sourceOwner": None, "releaseId": None, "manifestDigest": None, "releaseDigest": None, "sourceVersion": 1,
        "gatheringRef": "gathering-001",
    }
    payload.update(overrides.pop("payload", {}))
    if overrides.get("eventType") == "PostDeleted":
        payload = {key: payload[key] for key in ["postId", "authorId", "contentType", "contentIdentity", "environment", "sourceOwner", "releaseId", "manifestDigest", "releaseDigest", "sourceVersion"]}
