...
- 包输出：`.qwq_output/release/legal-static/<env>/<version>/`
...
发布包会生成 `public/legal/...` 静态目录、`manifest.json`、`checksums.json`、`release_metadata.json`，并刷新 `.qwq_output/release/legal-static/<env>/current` 指针。gateway/CDN 只挂载 `current/public`，不读取业务服务镜像。
