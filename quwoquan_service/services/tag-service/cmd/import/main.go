...
// the prior activate mode remains only for non-Data taxonomy compatibility.
package main
...
func main() {
	...
	activationMode := flag.String("activation-mode", "", "required immutable release action: stage-only|activate (activate is prior non-Data only)")
	...
	if strings.TrimSpace(*releaseRoot) != "" {
		...
		log.Fatal("prior taxonomy import requires --activation-mode=activate")
	}
	if *dryRun {
