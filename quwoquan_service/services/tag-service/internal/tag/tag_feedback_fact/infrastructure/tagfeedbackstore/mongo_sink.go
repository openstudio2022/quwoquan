// Package tagfeedbackstore 是 TagFeedbackFact 的 Mongo append sink。
...
const feedbackCollection = "tag_feedback_fact"
	...
	_, err := s.feedback.Indexes().CreateMany(ctx, []mongo.IndexModel{
		{
			...
			Options: options.Index().SetName("idx_tag_feedback_fact_actor_idempotency").SetUnique(true),
		},
		{
			Keys:    bson.D{{Key: "tagRef", Value: 1}, {Key: "recordedAt", Value: -1}},
			Options: options.Index().SetName("idx_tag_feedback_fact_tag_recorded"),
		},
		{
			Keys: bson.D{
			...
			Options: options.Index().
				SetName("idx_tag_feedback_fact_pending_event"),
		},
	})
	return err
}
