...
func (f *fakeObjReader) FindObjectsByTagRefSubtree(_ context.Context, tagRef, objectType string, _ int64) ([]model.ObjectTagIndex, error) {
	out := make([]model.ObjectTagIndex, 0)
	for _, o := range f.objs {
		for _, t := range o.TagRefs {
			if (t == tagRef || strings.HasPrefix(t, tagRef+"/")) && (objectType == "" || o.ObjectType == objectType) {
				out = append(out, *o)
				break
			}
		}
...
func TestInverted(t *testing.T) {
	svc := newServiceWithFixtures()
	view, err := svc.Inverted(context.Background(), "Topic/摄影", "", 0, false)
	if err != nil {
	...
	// objectType 过滤
	postOnly, err := svc.Inverted(context.Background(), "Topic/旅行", "post", 0, false)
	if err != nil {
...
// TestInvertedIncludeDescendants 祖先展开契约：省级 geo 标签反查必须聚合区县级
// 叶子对象（查询侧子孙展开）；关闭时保持精确匹配语义不变。
func TestInvertedIncludeDescendants(t *testing.T) {
	nodes := &fakeNodeReader{nodes: map[string]*model.TagNode{}}
	objs := &fakeObjReader{objs: map[string]*model.ObjectTagIndex{
		"hp1|homepage": {ObjectID: "hp1", ObjectType: "homepage", TagRefs: []string{"Topic/地理/行政区/中国/浙江省/杭州市/西湖区"}},
		"hp2|homepage": {ObjectID: "hp2", ObjectType: "homepage", TagRefs: []string{"Topic/地理/行政区/中国/浙江省/舟山市/岱山县"}},
		"hp3|homepage": {ObjectID: "hp3", ObjectType: "homepage", TagRefs: []string{"Topic/地理/行政区/中国/四川省/乐山市/沙湾区"}},
	}}
	svc := NewTagService(nodes, objs)

	// 精确匹配：省级标签本身无对象直接引用。
	exact, err := svc.Inverted(context.Background(), "Topic/地理/行政区/中国/浙江省", "", 0, false)
	if err != nil {
		t.Fatalf("inverted exact error: %v", err)
	}
	if exact.ObjectCount != 0 {
		t.Fatalf("expected 0 exact objects for province tag, got %d", exact.ObjectCount)
	}

	// 子孙展开：省级标签聚合全部区县级叶子对象，且不跨省。
	expanded, err := svc.Inverted(context.Background(), "Topic/地理/行政区/中国/浙江省", "", 0, true)
	if err != nil {
		t.Fatalf("inverted descendants error: %v", err)
	}
	if expanded.ObjectCount != 2 {
		t.Fatalf("expected 2 objects under 浙江省 subtree, got %d: %+v", expanded.ObjectCount, expanded.ObjectIds)
	}
	for _, id := range expanded.ObjectIds {
		if id == "hp3" {
			t.Fatalf("四川省 object must not appear in 浙江省 subtree query")
		}
	}
}
