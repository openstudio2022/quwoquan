...
// relatedScanCap 限制 related / cooccurrence / related-objects 的倒排扫描规模，
// 防止单标签热点对象集导致 O(N·k) 在线计算失控（启动子集足够，规模化由数据工程预算共现表）。
const relatedScanCap = 500

// TagSearchResultView 对齐 service.yaml SearchTags 响应。
...
// RelatedTagView 对齐 service.yaml RelatedTags 响应。
...
// TagObjectMatchView 对齐 service.yaml SearchByTags 响应。
...
// TagCooccurrenceView 对齐 service.yaml TagCooccurrence 响应。
...
// RelatedObjectView 对齐 service.yaml RelatedObjects 响应。
...
func (s *TagService) RelatedTags(ctx context.Context, tagRef string, limit int) ([]RelatedTagView, error) {
	...
	for t, c := range counts {
		view := RelatedTagView{TagRef: t, CooccurCount: c}
