...
func TestTagCandidateReceiptFoundAndNotFoundAreExact(t *testing.T) {
	...
	for _, prior := range []string{"expectedNodeCount", "projectedNodeCount"} {
		if strings.Contains(string(raw), prior) {
			t.Fatalf("receipt still exposes flat prior field %q: %s", prior, raw)
		}
	}
