...
func TestContentCandidateStageBDoesNotActivateLegacyA(t *testing.T) {
	cleanReleases(t)
	ctx := context.Background()
	legacy := newReleaseFacade(t)
	if _, err := legacy.Stage(ctx, taxonomyrelease.StageCommand{
		ReleaseID: "legacy-a", SourceOwner: "legacy_taxonomy",
		CanonicalDigest: "legacy-a", ReleaseKind: releasemodel.ReleaseKindContent, NodeCount: 1,
	}); err != nil {
		t.Fatal(err)
	}
	seedSnapshot(t, "legacy-a", 1)
	if _, err := legacy.Activate(ctx, "legacy-a"); err != nil {
		t.Fatal(err)
	}
	...
	if err != nil || !found || active.ReleaseID != "legacy-a" {
	...
	if count, err := mongoDB.Collection("tag_taxonomy_releases").CountDocuments(ctx, bson.M{"_id": "content-b"}); err != nil || count != 0 {
		t.Fatalf("Data candidate leaked into legacy release store count=%d err=%v", count, err)
	}
}
