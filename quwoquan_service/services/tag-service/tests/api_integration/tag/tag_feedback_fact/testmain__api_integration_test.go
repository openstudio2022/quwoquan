...
func TestMain(m *testing.M) {
	...
	mongoRuntime, err := testinfra.StartRealMongo(
		startupCtx,
		testinfra.UniqueDatabaseName("tag_feedback_fact_api_integration"),
	)
	startupCancel()
	if err != nil {
		panic("TagFeedbackFact api_integration requires real MongoDB: " + err.Error())
	}
...
func cleanCollections(t *testing.T) {
	t.Helper()
	for _, collection := range []string{"tag_nodes", "tag_feedback_fact", "tag_taxonomy_releases"} {
...
func seedLaunchSubset(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("seed TagFeedbackFact tag node: %v", err)
	}
