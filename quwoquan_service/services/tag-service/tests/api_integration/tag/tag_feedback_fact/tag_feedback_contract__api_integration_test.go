...
func TestTagFeedbackAppendDedupe(t *testing.T) {
	cleanCollections(t)
	if _, err := mongoDB.Collection("tag_feedback_fact").DeleteMany(context.Background(), bson.M{}); err != nil {
		t.Fatalf("clean tag_feedback_fact: %v", err)
	}
	seedLaunchSubset(t)
	...
	count, err := mongoDB.Collection("tag_feedback_fact").CountDocuments(context.Background(), bson.M{
