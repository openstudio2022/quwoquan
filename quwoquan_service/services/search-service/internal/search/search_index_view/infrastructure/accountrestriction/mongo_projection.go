...
func (projection *MongoAccountRestrictionProjection) FinalizeClosure(
	ctx context.Context,
	closure application.SubjectClosure,
) error {
	if projection == nil || projection.db == nil {
		return errors.New("search account restriction closure requires MongoDB")
	}
	now := time.Now().UTC()
	accountDigest := searchAccountRestrictionDigest(closure.AccountID)
	if _, err := projection.watermarks.UpdateOne(
		ctx,
		bson.M{"_id": accountDigest},
		bson.M{
			"$max": bson.M{
				"accountVersion": closure.AccountVersion,
				"closedAt":       closure.ClosedAt.UTC(),
			},
			"$set": bson.M{
				"eventDigest": closure.EventDigest,
				"terminal":    true,
				"updatedAt":   now,
			},
			"$setOnInsert": bson.M{"createdAt": now},
	...
	accountID := strings.TrimSpace(closure.AccountID)
	if _, err := projection.states.DeleteMany(
		ctx,
	...
	if _, err := projection.inbox.DeleteMany(
		ctx,
		bson.M{"$or": bson.A{
