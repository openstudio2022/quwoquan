...
type RecentSearchHandler struct {
	...
	observer application.RecentSearchObserver
}

func NewRecentSearchHandler(
	facade *recentsearch.Facade,
	observers ...application.RecentSearchObserver,
) *RecentSearchHandler {
...
func (h *RecentSearchHandler) Register(mux *http.ServeMux) {
	mux.HandleFunc(mustOperationPattern(listRecentOperation), h.handleList)
	mux.HandleFunc(mustOperationPattern(upsertRecentOperation), h.handleUpsert)
	mux.HandleFunc(mustOperationPattern(deleteRecentOperation), h.handleDelete)
	mux.HandleFunc(mustOperationPattern(clearRecentOperation), h.handleClear)
}
...
func (h *RecentSearchHandler) observe(operation, status string, started time.Time) {
	...
	h.observer.ObserveRecentSearch(application.RecentSearchObservation{
		Operation: operation,
		Status:    status,
