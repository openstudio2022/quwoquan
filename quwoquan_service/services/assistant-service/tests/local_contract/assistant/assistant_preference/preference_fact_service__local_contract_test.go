...
func (s *migratedPreferenceFactServiceMemoryPreferenceStore) List(
	...
	for _, fact := range s.facts {
		...
		if filter.SessionID != "" &&
			fact.SessionID != filter.SessionID {
			continue
		}
...
type migratedPreferenceFactServiceOwnedSessionReader struct {
	userID    string
	sessionID string
}

func (r migratedPreferenceFactServiceOwnedSessionReader) OwnedSessionExists(
	_ context.Context,
	userID string,
	sessionID string,
) (bool, error) {
...
func TestPreferenceFactSessionLifecycleAndOwnerIsolation(t *testing.T) {
	...
	commands := NewCommandFacade(
		store,
		migratedPreferenceFactServiceOwnedSessionReader{
...
func TestFactualLongTermMemoryRequiresConfirmationAndReusesRevokeRestore(t *testing.T) {
	...
	if fact.Kind != preferencemodel.KindDietaryRestrictions ||
		fact.SourceSessionID != "asn_memory_source" ||
...
func TestPreferenceFactRestoreWindowExpiresFailClosed(t *testing.T) {
	store := migratedPreferenceFactServiceNewMemoryPreferenceStore()
	commands := NewCommandFacade(store, migratedPreferenceFactServiceOwnedSessionReader{})
