...
func TestRuntimeFoundationSmoke(t *testing.T) {
	...
	transport.Publish(turnID, migratedRuntimeFoundationRoundTripEnvelope(t, envelope))
...
func migratedRuntimeFoundationRoundTripEnvelope(t *testing.T, envelope streaming.Envelope) streaming.Envelope {
	t.Helper()
