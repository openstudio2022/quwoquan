...
func TestAssistantMentionedConsumerProcessesAndAcks(t *testing.T) {
	...
	handler := &migratedAssistantMentionedConsumerMentionHandlerSpy{}
	consumer := NewAssistantMentionedConsumerWithTransport(
		migratedAssistantMentionedConsumerNewTestMessageTransport(t, redis),
		handler,
		"worker-1",
		nil,
	)
...
func TestAssistantMentionedConsumerDeadLettersFailedMessage(t *testing.T) {
	ctx := context.Background()
	redis := rtredis.NewMemoryClient()
	handler := &migratedAssistantMentionedConsumerMentionHandlerSpy{err: errors.New("boom")}
	consumer := NewAssistantMentionedConsumerWithTransport(
		migratedAssistantMentionedConsumerNewTestMessageTransport(t, redis),
		handler,
		"worker-1",
...
func migratedAssistantMentionedConsumerNewTestMessageTransport(
	t *testing.T,
