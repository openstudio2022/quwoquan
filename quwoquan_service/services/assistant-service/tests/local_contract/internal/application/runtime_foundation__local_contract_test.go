...
func TestRuntimeFoundationLocalContractTest(t *testing.T) {
	...
	cmd := exec.Command(
		...
		"-run",
		"^(TestRuntimeFoundationSmoke)$",
		"-count=1",
	)
	cmd.Env = append(os.Environ(), "QWQ_OUTPUT_ROOT="+filepath.Join(repoRoot(), ".qwq_output"))
	output, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("source bridge failed for quwoquan_service/services/assistant-service/internal/application/runtime_foundation_test.go: %v\n%s", err, output)
	}
}
