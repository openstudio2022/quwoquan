...
#
# 用法：
#   bash quwoquan_service/services/assistant-service/tests/ops/gate/run_pa_core_tests.sh          # 跑全部桶（A+B+C）
#   bash quwoquan_service/services/assistant-service/tests/ops/gate/run_pa_core_tests.sh --bucket-a  # 只跑桶 A（最快、最稳定）
#
# Gate 策略：
#   - 桶 A 失败 → 立即退出（基础协议契约）
#   - 桶 B 失败 → 立即退出（引擎集成契约）
