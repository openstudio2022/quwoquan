...
func assistantHTTPHandlerWithTurnView(
	...
	turnviewhttp.NewHandler(turnviewapplication.NewQueryFacade(
		turnviewpersistence.NewMongoReader(integrationMongoDB),
	)).RegisterRoutes(mux)
...
func TestAssistantRunEventStoreRejectsSequenceCorruption(t *testing.T) {
	...
	if err := integrationSessionRunStore.AppendRunEvent(ctx, runID, first); err != nil {
		t.Fatalf("append first Mongo run event: %v", err)
	}
	if err := integrationSessionRunStore.AppendRunEvent(ctx, runID, first); err != nil {
		t.Fatalf("replay identical Mongo run event: %v", err)
	}
	if err := integrationSessionRunStore.AppendRunEvent(
		ctx,
		runID,
	...
	events, err := integrationSessionRunStore.ListRunEvents(ctx, runID, 0, 10)
	if err != nil {
...
func TestAssistantRunPersistsTrustedTransportContextWithoutEchoingIt(t *testing.T) {
	...
	stored, found, err := integrationSessionRunStore.GetTurn(
		context.Background(),
		turnID,
	)
	if err != nil || !found {
...
func TestSkillConsentRevokeImmediateEnforcement(t *testing.T) {
	...
	if _, err := commands.Grant(
		ctx, "enforce-grant", "enforce-user", "personal_content_access", "personal_content_access",
	); err != nil {
		t.Fatalf("grant consent: %v", err)
	}
	create := assistantAPIRequest(t, handler, http.MethodPost, "/assistant/sessions",
		"enforce-user", map[string]any{
			"summary": "consent gate", "clientRequestId": "consent-gate-session",
		})
	var session assistant.AssistantSession
	if err := json.Unmarshal(create.Body.Bytes(), &session); err != nil {
		t.Fatalf("decode session: %v", err)
