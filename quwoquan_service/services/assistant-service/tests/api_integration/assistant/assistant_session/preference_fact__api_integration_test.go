...
func TestAssistantPreferencePersistenceRunSnapshotAndRestore(t *testing.T) {
	...
	create := assistantAPIRequest(
		...
		"preference-owner",
		map[string]any{
			"summary": "偏好闭环", "clientRequestId": "preference-fact-session",
		},
	)
	...
	createOtherSession := assistantAPIRequest(
		...
		"preference-owner",
		map[string]any{
			"summary": "偏好隔离", "clientRequestId": "preference-fact-session-other",
		},
	)
	...
	if startedRun.InputText != "保持原问题" ||
		len(startedRun.SessionPreferenceFacts) != 1 ||
		startedRun.SessionPreferenceFacts[0].PreferenceID != fact.PreferenceID {
	...
	if otherRun.InputText != "另一个会话保持原问题" ||
		len(otherRun.SessionPreferenceFacts) != 0 {
		t.Fatalf(
	...
	if len(revokedSnapshot.SessionPreferenceFacts) != 0 {
		t.Fatalf(
			"revoked preference leaked into run snapshot: %#v",
			revokedSnapshot.SessionPreferenceFacts,
		)
	}
	...
	if len(restoredSnapshot.SessionPreferenceFacts) != 1 ||
		restoredSnapshot.SessionPreferenceFacts[0].PreferenceID != fact.PreferenceID {
		t.Fatalf(
			"restored preference missing from run snapshot: %#v",
			restoredSnapshot.SessionPreferenceFacts,
		)
	}
}
