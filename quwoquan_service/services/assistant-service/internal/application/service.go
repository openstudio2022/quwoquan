...
func (s *AssistantService) SearchXiaoquResults(ctx context.Context, req assistant.SearchRequest) (_ assistant.AssistantSearchResultView, err error) {
	...
	// Migrated to the unified retrieve contract: the AI only declares targets +
	// terms; relationships and visibility are resolved by the backend. The
	// active backend (native by default) is transparent to the tool.
	backend := rtsearch.NewSliceBackend(assistantRetrieveDocuments(query))
	retrieveResp, retrieveErr := rtsearch.Retrieve(ctx, rtsearch.RetrieveRequest{
		Targets: []rtsearch.Target{
			...
			rtsearch.TargetCircle, rtsearch.TargetGroup,
		},
		Terms: assistantQueryTerms(query),
		Page:  rtsearch.PageRequest{Limit: 8},
	}, backend, rtsearch.Viewer{})
	if retrieveErr != nil {
		err = retrieveErr
		return assistant.AssistantSearchResultView{}, err
	}
	citations := make([]assistant.AssistantSearchCitationView, 0, len(retrieveResp.Hits)+1)
	for _, hit := range retrieveResp.Hits {
		target := string(hit.Target)
		citations = append(citations, assistant.AssistantSearchCitationView{
			CitationID:    "cite_" + target + "_" + hit.ObjectID,
			ObjectType:    target,
			ObjectID:      hit.ObjectID,
			Title:         hit.Title,
			Snippet:       hit.Snippet,
			BadgeLabel:    assistantTargetBadge(hit.Target),
			SourceDomain:  assistantTargetDomain(hit.Target),
			Score:         hit.Score,
			RecallSource:  retrieveResp.Provenance.Provider,
			ObjectTypeRef: target,
		})
	}
	// web_search equivalent is a citation supplement, not a business target.
	citations = append(citations, assistantWebSupplementCitation(query))
	summary := fmt.Sprintf("小趣已通过统一检索围绕“%s”找到 %d 条网页与站内对象线索，可继续追问或打开引用查看。", query, len(citations))
