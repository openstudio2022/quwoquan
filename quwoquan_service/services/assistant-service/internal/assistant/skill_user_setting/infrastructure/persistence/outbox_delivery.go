...
WITH head AS (
  SELECT event_id
  FROM skill_user_setting_outbox
  WHERE dispatched_at IS NULL
  ORDER BY occurred_at ASC, event_id ASC
  LIMIT 1
), claimed AS (
  UPDATE skill_user_setting_outbox AS target
...
FROM claimed`, ownerID, now, now.Add(-lease)).Scan(
	...
	if err != nil {
		return ports.OutboxEvent{}, false, unavailable("claim setting outbox", err)
	}
...
UPDATE skill_user_setting_outbox
...
WHERE event_id=$1 AND claim_owner=$2 AND dispatched_at IS NULL`,
	...
	if err != nil {
		return unavailable("mark setting outbox published", err)
	}
...
WHERE event_id=$1 AND claim_owner=$2 AND dispatched_at IS NULL`,
	...
	if err != nil {
		return unavailable("schedule setting outbox retry", err)
	}
	if tag.RowsAffected() != 1 {
		return ports.ErrOutboxClaimLost
	}
