...
type AssistantRunContextTurn struct {
...
type AssistantRunContextSummary struct {
...
func (process AssistantRunVisibleProcess) Clone() *AssistantRunVisibleProcess {
	cloned := process
	cloned.AcceptedReferences = append(
		[]AssistantRunVisibleReference(nil),
		process.AcceptedReferences...,
	)
	return &cloned
}
...
func IsKnownAssistantFeedbackContextDecision(value string) bool {
...
func (snapshot AssistantFeedbackContextSnapshot) Clone() AssistantFeedbackContextSnapshot {
	snapshot.Metrics = append(
		[]AssistantFeedbackMetricSummary(nil),
		snapshot.Metrics...,
	)
	snapshot.Reasons = append(
		[]AssistantFeedbackReasonSummary(nil),
		snapshot.Reasons...,
	)
	return snapshot
}
...
func (snapshot AssistantRunTerminalSnapshot) Clone() *AssistantRunTerminalSnapshot {
	cloned := snapshot
	cloned.Processes = make([]AssistantRunVisibleProcess, len(snapshot.Processes))
	for index, process := range snapshot.Processes {
		cloned.Processes[index] = *process.Clone()
	}
	...
	if snapshot.SelectedPolicyRef != nil {
		policy := *snapshot.SelectedPolicyRef
		cloned.SelectedPolicyRef = &policy
	}
	return &cloned
}
...
type AssistantTurn struct {
	...
	ContextTurns            []AssistantRunContextTurn                     `json:"contextTurns,omitempty" bson:"-"`
	ContextSummary          *AssistantRunContextSummary                   `json:"-" bson:"-"`
	...
	SessionPreferences      []preferencemodel.AssistantPreferenceSnapshot `json:"sessionPreferences,omitempty" bson:"sessionPreferences,omitempty"`
	LongTermPreferences     []preferencemodel.AssistantPreferenceSnapshot `json:"longTermPreferences,omitempty" bson:"longTermPreferences,omitempty"`
