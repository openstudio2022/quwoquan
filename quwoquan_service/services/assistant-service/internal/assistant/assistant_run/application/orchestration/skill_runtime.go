...
type SkillSelection struct {
	...
	// PresentationProfile is frozen together with the selected Skill package.
	// Durable execution must pass it to Presentation instead of resolving the
	// active catalog again after model inference.
...
func (rt DefaultSkillRuntime) SelectSkillWithin(
	...
	loader := rt.Loader
	if loader == nil {
		return SkillSelection{}, skillpkg.ErrCatalogUnavailable
	}
...
func (r ModelDrivenSkillRuntime) SelectSkillWithin(
	...
	if model != nil && strings.TrimSpace(turn.Input.Text) != "" {
		...
		if usageErr := consumeExecutionModelResponse(ctx, resp); usageErr != nil {
			return SkillSelection{}, usageErr
		}
		if err == nil {
	...
	// Degradation is deterministic and remains inside the same frozen active
	// package snapshot. No caller-injected runtime may switch catalogs or
	// bypass the already applied candidate policy.
	selection, err := ManifestSkillRuntime{
		Loader: immutableCatalogLoader{catalog: catalog},
	}.SelectSkillWithin(ctx, turn, allowedSkillIDs)
	if err == nil {
...
// immutableCatalogLoader only reuses the already resolved immutable catalog
// inside one routing decision. It never discovers manifests or supplies a
// built-in fallback.
type immutableCatalogLoader struct {
	catalog []skillpkg.Manifest
}

func (loader immutableCatalogLoader) Load(ctx context.Context) ([]skillpkg.Manifest, error) {
...
func (r ManifestSkillRuntime) SelectSkillWithin(
	...
	routed := skillpkg.NewRouter(catalog).Route(turn)
	if strings.TrimSpace(routed.SkillID) == "" {
		return SkillSelection{}, ErrNoEligibleSkill
	}
	return selectionFromManifest(routed), nil
}
