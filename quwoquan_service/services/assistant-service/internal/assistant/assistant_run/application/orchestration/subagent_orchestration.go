...
type SubagentPlan struct {
	...
	TimeoutMs       int
	// The following budgets are internal request-scoped allocations derived
	// from the root ReasoningProfile. They are not public SubagentPlan wire
	// fields and prevent parallel branches from each spending the root budget.
	TokenBudget    int64
	CostUnitBudget int64
	SourceBudget   int
}
...
func (planner ModelSubagentPlanner) PlanSubagents(
	...
	maxPlans := maxSubagentPlans
	if policy, ok := executionPolicyFromContext(ctx); ok {
		maxPlans = policy.maxSubagentCount(len(candidates))
		if maxPlans < 2 {
			return nil, nil
		}
	}
	response, err := planner.Model.Complete(ctx, frozenPolicyModelRequest(turn, primary, ModelRequest{
		...
		Prompt:               buildSubagentPlanPrompt(candidates, maxPlans),
	...
	shape, plans := subagentPlansFromModel(
		response,
		candidates,
		rootTools,
		maxPlans,
...
func buildSubagentPlanPrompt(candidates []skillpkg.Manifest, maxPlans int) string {
	var b strings.Builder
	b.WriteString("候选技能如下，判断这个问题需要几个子任务并行完成。\n")
	b.WriteString(fmt.Sprintf("并行子任务上限为 %d；不得为了凑满上限而拆分。\n", maxPlans))
	for _, manifest := range candidates {
		b.WriteString("- ")
