...
func FormatForPrompt(result *AssemblyResult) (string, error) {
	if result == nil {
		return "", nil
	}
	lines := []string{
	...
	skillContext, err := skillcontext.FormatForPrompt(result.SkillContextSnapshot)
	if err != nil {
		return "", err
	}
	if skillContext != "" {
		lines = append(lines, skillContext)
	}
