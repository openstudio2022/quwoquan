...
const (
	...
	IdempotencyContentAddressed = "content_addressed"
...
type Metadata struct {
	...
	Research             ResearchPolicy     `json:"research"`
...
// Clone returns an isolated metadata snapshot for one AgentLoop execution.
// Registry schemas are maps, so a shallow copy would let a hook or adapter
// mutate the policy observed by another Run.
func (m Metadata) Clone() (Metadata, error) {
	encoded, err := json.Marshal(m)
	if err != nil {
		return Metadata{}, fmt.Errorf("encode tool metadata %q: %w", m.ToolName, err)
	}
	var cloned Metadata
	if err := json.Unmarshal(encoded, &cloned); err != nil {
		return Metadata{}, fmt.Errorf("decode tool metadata %q: %w", m.ToolName, err)
	}
	return cloned, nil
}

const (
	ResearchOperationNone     = "none"
	ResearchOperationDiscover = "discover"
	ResearchOperationNavigate = "navigate"
	ResearchOperationInspect  = "inspect"
)

// ResearchPolicy describes how a read tool participates in bounded evidence
// exploration. AgentLoop consumes these semantic bindings instead of knowing
// concrete tool names or vertical implementations.
type ResearchPolicy struct {
	Operation                 string   `json:"operation"`
	ParallelInputField        string   `json:"parallelInputField"`
	TargetInputField          string   `json:"targetInputField"`
	TargetKindField           string   `json:"targetKindField"`
	TargetValueField          string   `json:"targetValueField"`
	ReusableSourceTargetKinds []string `json:"reusableSourceTargetKinds"`
...
func (p ResearchPolicy) ResolvedOperation() string {
	...
	case ResearchOperationDiscover:
		return ResearchOperationDiscover
	case ResearchOperationNavigate:
		return ResearchOperationNavigate
	case ResearchOperationInspect:
		return ResearchOperationInspect
	default:
		return ResearchOperationNone
	}
}
...
// FailurePolicy binds adapter-neutral dependency failures to the canonical
// Assistant error contract. The coordinator consumes this policy without
// knowing a provider, vertical, or concrete tool name.
type FailurePolicy struct {
	ProviderFailureCode string `json:"providerFailureCode"`
}
...
// RequiresIdempotencyKey reports whether the canonical tool contract requires
// the adapter to deduplicate a mutating invocation by the Tool Fabric identity.
func (m Metadata) RequiresIdempotencyKey() bool {
	return strings.TrimSpace(m.Idempotency) == IdempotencyKey
}
...
// LookupCanonicalMetadata 返回 generated catalog 中的不可变工具元数据。
// 声明/预算/恢复语义以 catalog 为真相源；Registry 只负责可执行 handler 绑定。
func LookupCanonicalMetadata(toolName string) (Metadata, bool) {
...
func WeatherLookupMetadata() Metadata {
	return mustCanonicalMetadata("weather_lookup")
}

func FinanceQuoteMetadata() Metadata {
	return mustCanonicalMetadata("finance_quote")
}
...
func mustCanonicalMetadata(toolName string) Metadata {
	metadata, ok := LookupCanonicalMetadata(toolName)
	if !ok {
