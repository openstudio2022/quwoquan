...
func (c *Client) ListMessages(
	...
	for _, item := range payload.Items {
		out = append(out, ports.ChatGroundingMessage{
			...
			ObjectRef:  item.groundingObjectRef(),
		})
	}
	return out, nil
}
...
func (m messageWire) groundingObjectRef() *ports.ChatGroundingObjectRef {
	if m.Card == nil || m.Card.ObjectRef == nil {
		return nil
	}
	ref := m.Card.ObjectRef
	if strings.TrimSpace(ref.ObjectTypeRef) == "" || strings.TrimSpace(ref.ObjectID) == "" ||
		strings.TrimSpace(ref.RouteID) == "" {
		return nil
	}
	return &ports.ChatGroundingObjectRef{
