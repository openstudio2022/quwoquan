...
const (
	...
	// assistantDefaultPolicyID 是助手运行的策略标识；技能候选集合与冻结模板必须取自同一策略。
	assistantDefaultPolicyID = "assistant-default"
)
...
func (s *AssistantService) requireSessionRunStore() (
	ports.SessionRunStore,
	error,
) {
	if s.sessionRuns == nil {
		return nil, assistantSessionStorageUnavailable(
			"session/run store is not configured",
		)
	}
	return s.sessionRuns, nil
}

func (s *AssistantService) CreateSession(ctx context.Context, userID string, input assistant.CreateSessionInput) (_ assistant.AssistantSession, err error) {
	ctx, span := rtobs.StartBusinessSpan(ctx, "assistant.CreateSession",
	...
	if userID == "" {
		return assistant.AssistantSession{}, rterr.NewInvalidArgument(rterr.ModuleAssistant, "userId 不能为空", "missing userId")
	}
	...
	if err != nil {
		return assistant.AssistantSession{}, rterr.NewUnavailable(rterr.ModuleAssistant, "生成对话 ID 失败", err.Error())
	}
	now := s.now()
...
func (s *AssistantService) GetSession(ctx context.Context, userID, sessionID string) (_ assistant.AssistantSession, err error) {
	ctx, span := rtobs.StartBusinessSpan(ctx, "assistant.GetSession",
		attribute.String("user.id", userID),
...
// ListSessions 返回 owner 的会话切片（updatedAt desc keyset 分页）。
func (s *AssistantService) ListSessions(ctx context.Context, userID string, limit int, cursor string) (_ assistant.AssistantSessionListView, err error) {
	ctx, span := rtobs.StartBusinessSpan(ctx, "assistant.ListSessions",
		attribute.String("user.id", userID))
	defer func() { rtobs.EndSpan(span, err) }()
	...
	userID = strings.TrimSpace(userID)
	if userID == "" {
		return assistant.AssistantSessionListView{}, rterr.NewInvalidArgument(rterr.ModuleAssistant, "userId 不能为空", "missing userId")
	}
	items, nextCursor, err := store.ListSessions(ctx, userID, limit, strings.TrimSpace(cursor))
	if err != nil {
		if strings.Contains(err.Error(), "invalid sessions cursor") {
			return assistant.AssistantSessionListView{}, rterr.NewInvalidArgument(rterr.ModuleAssistant, "分页游标无效", err.Error())
		}
		return assistant.AssistantSessionListView{}, assistantSessionStorageUnavailable(err.Error())
	}
	return assistant.AssistantSessionListView{Items: items, NextCursor: nextCursor}, nil
}
...
func (s *AssistantService) CancelRun(ctx context.Context, userID, turnID string) (_ assistant.AssistantTurn, err error) {
	...
	if err := store.UpdateSessionTurnPointer(ctx, turn.SessionID, "", turn.TurnID, now); err != nil {
		return assistant.AssistantTurn{}, assistantSessionStorageUnavailable(err.Error())
	}
	return stored, nil
}

func (s *AssistantService) CreateTurn(ctx context.Context, userID, sessionID string, input assistant.CreateTurnInput) (_ assistant.AssistantTurn, err error) {
	...
	userID = strings.TrimSpace(userID)
	sessionID = strings.TrimSpace(sessionID)
	if strings.TrimSpace(input.Input.Text) == "" && !hasTrustedTriggerInput(input) {
	...
	if !found || session.UserID != userID {
		return assistant.AssistantTurn{}, assistantSessionNotFound()
	}
	...
	candidateSkillID, candidateDomainID := s.routeCandidateSkill(
		ctx,
		input,
		assistantDefaultPolicyID,
	...
	assistantChannel := channelpkg.Resolve(turnType, trigger)
	if _, err := assistantChannel.ResolveIdentity(channelpkg.IdentityInput{
		ActorID:   userID,
		PersonaID: requestContext.PersonaID,
	}); err != nil {
		return assistant.AssistantTurn{}, AssistantRunInvalidArgument(err.Error())
	}
	...
	if s.preferenceSnapshots != nil {
		...
		if assistantChannel.ContextPersistence() == channelpkg.ContextPersistenceChannelOnly {
	...
	if err := store.UpdateSessionTurnPointer(ctx, sessionID, stored.TurnID, stored.TurnID, now); err != nil {
		return assistant.AssistantTurn{}, assistantSessionStorageUnavailable(err.Error())
	}
	return stored, nil
}

func hasTrustedTriggerInput(input assistant.CreateTurnInput) bool {
	if strings.TrimSpace(input.TurnType) != "proactive" || input.Trigger.Envelope == nil {
		return false
	}
	envelope := input.Trigger.Envelope
...
func (s *AssistantService) executeTurn(ctx context.Context, userID, turnID string) ([]streaming.Envelope, error) {
	...
	contextTurns, contextSummary, err := s.sessionContext(ctx, userID, turn)
	if err != nil {
		return nil, err
	}
	turn.ContextTurns = contextTurns
	turn.ContextSummary = contextSummary
	loop := s.agentLoop
	if loop == nil {
	...
	if cancelledByCommand := runCtx.Err() != nil && ctx.Err() == nil; cancelledByCommand {
		...
		projector := assistantstreaming.NewStreamProjectorAt(turn, s.now, lastSeq)
		envelope, eventErr := projector.Event(assistantstreaming.AssistantStreamEventCancelled, map[string]any{
			"status": "cancelled",
		})
		if eventErr != nil {
			return nil, eventErr
	...
	if stored.Status != completed.Status {
		if stored.Status == "cancelled" {
			projector := assistantstreaming.NewStreamProjectorAt(stored, s.now, lastSeq)
			cancelledEvent, eventErr := projector.Event(assistantstreaming.AssistantStreamEventCancelled, map[string]any{
				"status": "cancelled",
			})
			if eventErr != nil {
				return nil, eventErr
	...
	if stored.Status == "completed" {
		recentTurnLimit := channelpkg.Resolve(
			stored.TurnType,
			stored.Trigger,
		).ContextWindow().RecentTurnLimit
		if summaryErr := s.advanceSessionSummary(
			ctx,
			stored.UserID,
			stored.SessionID,
			recentTurnLimit,
			...
			slog.WarnContext(
				ctx,
				"assistant session summary advance deferred",
				slog.String("sessionId", stored.SessionID),
				slog.String("turnId", stored.TurnID),
				slog.String("error", summaryErr.Error()),
			)
		}
	}
	if err := store.UpdateSessionTurnPointer(ctx, turn.SessionID, "", turn.TurnID, completedAt); err != nil {
		return nil, assistantSessionStorageUnavailable(err.Error())
	}
...
func (s *AssistantService) followRunEvents(
	...
	for {
		...
		for _, envelope := range events {
			...
			emittedCount++
			if envelope.EventType == string(assistantstreaming.AssistantStreamEventCancelled) {
...
func (s *AssistantService) failTurnExecution(
	...
	projector := assistantstreaming.NewStreamProjectorAt(turn, s.now, lastSeq)
	envelope, err := projector.Failure(assistantstreaming.AssistantStreamEventFailed, map[string]any{
		"status": "failed",
	}, failure)
	if err != nil {
		return
	...
	if err := store.UpdateSessionTurnPointer(ctx, turn.SessionID, "", turnID, completedAt); err != nil {
