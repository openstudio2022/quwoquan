...
func (s *AssistantService) TickIntersectionReminders(ctx context.Context, input IntersectionReminderTickInput) (_ IntersectionReminderTickResult, err error) {
	...
	for _, reason := range reasons {
		...
		message, err := s.publishNotificationAppMessage(ctx, ports.NotificationAppMessageCommand{
			...
			Destination:    ports.NotificationAppMessageDestination{Type: "user", ID: userID},
			Title:          "小趣提醒",
			Summary:        intersectionReminderSummary(reason),
			Target: ports.NotificationAppMessageTarget{
				TargetType: "route",
...
func normalizeIntersectionReminderReason(
	reason ports.IntersectionReminderReason,
	userID string,
) ports.IntersectionReminderReason {
...
func intersectionReminderSummary(reason ports.IntersectionReminderReason) string {
