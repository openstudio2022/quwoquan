...
func FormatFactualMemoriesForPrompt(
	longTerm []preferencemodel.Snapshot,
) string {
	lines := []string{
	...
	for _, fact := range longTerm {
		if !preferencemodel.IsFactualKind(fact.Kind) {
			continue
		}
...
func factualMemoryLabel(kind preferencemodel.Kind) string {
	switch kind {
...
func FormatModelContextForPrompt(turns []assistant.AssistantSessionContextTurn) string {
	if len(turns) == 0 {
		return ""
	}
