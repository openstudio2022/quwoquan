...
const defaultScenarioFixtureName = "quwoquan_service/services/assistant-service/tests/support/contract_fixtures/scenarios/assistant_scenarios.json"
...
func LoadPlan(environment string, refsOverride []string) (Plan, error) {
	...
	manifestPath := fmt.Sprintf("quwoquan_service/contracts/metadata/_shared/test_fixtures/app_%s_seed_manifest.json", environment)
	manifest, err := contractfixture.LoadRepositoryJSON[AppSeedManifest](manifestPath)
	if err != nil {
...
func LoadAssistantScenarioPackAt(fixturePath string) (AssistantScenarioPack, error) {
	pack, err := contractfixture.LoadRepositoryJSON[AssistantScenarioPack](strings.TrimSpace(fixturePath))
	if err != nil {
