...
func (s *AssistantService) normalizeSkillSubscriptionInput(userID string, input assistant.CreateSkillSubscriptionInput, subscriptionID string) (assistant.SkillSubscription, error) {
	...
	if destination.DestinationType == "user" && destination.DestinationID == "" {
	...
	if destination.DestinationType != "user" && createdByPersonaID == "" {
		return assistant.SkillSubscription{}, rterr.NewInvalidArgument(
			rterr.ModuleAssistant,
			"会话投递缺少创建者 Persona",
			"conversation destination requires the creator persona",
		)
	}
...
func (s *AssistantService) requireSkillSubscriptionDestinationAccess(
	...
	if !current {
		return skillgenerated.AppErrorFromSubscriptionDestinationForbidden(
			"subscription creator or assistant skill is not a current conversation member",
		)
	}
	return nil
}
...
func (s *AssistantService) createProactiveTurnMessage(
	...
	conversation, err := s.CreateConversation(ctx, subscription.Owner.OwnerID, assistant.CreateConversationInput{
		...
		ClientRequestID: deliveryID + ":conversation",
	})
	if err != nil {
	...
	default:
		message, err := s.publishNotificationAppMessage(ctx, ports.NotificationAppMessageCommand{
			...
			Target:         ports.NotificationAppMessageTarget{TargetType: "assistant_turn", TargetID: turn.TurnID},
			Provenance: ports.NotificationAppMessageProvenance{
...
func (s *AssistantService) loadProactiveInterestProfile(
	ctx context.Context,
	userID string,
) *ports.ProactiveInterestProfile {
