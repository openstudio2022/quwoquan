...
func ProjectAssistantRunTerminalSnapshot(
	...
	if selectedPolicyRef != nil &&
		...
		strings.TrimSpace(selectedPolicyRef.Version) != "" &&
		...
		value := assistant.AssistantSelectedPolicyRef{
			...
			Version:  strings.TrimSpace(selectedPolicyRef.Version),
	...
	for _, event := range ordered {
		...
		switch AssistantStreamEventType(event.EventType) {
		case AssistantStreamEventProcessReplace:
			snapshot.Processes = visibleProcessesFromPayload(event.Payload["processes"])
		case AssistantStreamEventProcessAppend, AssistantStreamEventProcessCommit:
		...
		case AssistantStreamEventAnswerDelta:
			snapshot.AnswerText += stringValue(event.Payload["text"])
		case AssistantStreamEventCompleted:
		...
		case AssistantStreamEventFailed:
