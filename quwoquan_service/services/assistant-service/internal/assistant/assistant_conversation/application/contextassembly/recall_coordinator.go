...
func (authorizedTurnRecallSource) Recall(
	...
	for _, memory := range turn.LongTermPreferenceFacts {
		if !preferencemodel.IsFactualKind(memory.Kind) {
			continue
		}
	...
	for index := len(turn.ContextTurns) - 1; index >= 0; index-- {
		...
		hints = append(hints, RecallHint{
			HintID:   fmt.Sprintf("%s:conversation:%d", turn.TurnID, index),
			DomainID: request.DomainID,
			Text:     text,
			Score:    recallScore(queryTerms, text, 0.45+0.25*recency),
			Source:   "conversation_" + strings.TrimSpace(item.Role),
		})
	}
	return hints, nil
}
