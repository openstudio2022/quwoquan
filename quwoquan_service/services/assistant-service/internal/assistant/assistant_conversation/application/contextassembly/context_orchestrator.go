...
func NewContextOrchestrator() ContextOrchestrator {
	return ContextOrchestrator{Recall: NewRecallCoordinator(), Router: DefaultDomainRouter{}}
}
...
func (o ContextOrchestrator) Assemble(
	...
	slots, fillTasks := resolveSlots(input, domainID, hints)
	...
	longtermNeed := len(slots.MissingSlots) > 0 &&
		...
		channel.MemoryScope() == channelpkg.MemoryScopePrivateLongTerm
	result := AssemblyResult{
		...
		MemoryScope:         string(channel.MemoryScope()),
