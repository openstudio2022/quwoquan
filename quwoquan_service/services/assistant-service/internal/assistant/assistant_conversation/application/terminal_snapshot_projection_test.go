...
func TestProjectAssistantRunTerminalSnapshotUsesTypedTerminalFacts(t *testing.T) {
	...
	events := []streaming.Envelope{
		{
			Seq:       3,
			EventType: string(AssistantStreamEventProcessCommit),
			Payload:   map[string]any{"process": process},
		},
		{
			Seq:       1,
			EventType: string(AssistantStreamEventAnswerDelta),
			Payload:   map[string]any{"text": "草稿"},
		},
		{
			Seq:       4,
			EventType: string(AssistantStreamEventCompleted),
			Payload:   map[string]any{"finalAnswer": "最终回答"},
		},
		{
			Seq:       2,
			EventType: string(AssistantStreamEventProcessAppend),
	...
	snapshot := projectAssistantRunTerminalSnapshot(events, nil)
...
func TestProjectAssistantRunTerminalFailureDropsInternalMaterial(t *testing.T) {
	...
	snapshot := projectAssistantRunTerminalSnapshot([]streaming.Envelope{
		{
			Seq:            1,
			EventType:      string(AssistantStreamEventFailed),
