...
func TestValidateExecutableEntryRejectsSelectedFieldSetDrift(t *testing.T) {
	...
	if err := ownerinfra.ValidateExecutableEntry(missing); err == nil {
		t.Fatal("registry selectedFields missing an exact response field must fail closed")
	}

	extra := contentPostBaseEntry()
	extra.AppClientBundle.SelectedFields = append(extra.AppClientBundle.SelectedFields, "ownerPrivateField")
	if err := ownerinfra.ValidateExecutableEntry(extra); err == nil {
		t.Fatal("registry selectedFields containing an unbound response field must fail closed")
	}

	unbound := contentPostBaseEntry()
	unbound.AppClientBundle = nil
	if err := ownerinfra.ValidateExecutableEntry(unbound); err == nil {
		t.Fatal("content post runtime entry without selectedFields must fail closed")
	}
}
...
func contentPostBaseEntry() domain.Entry {
	...
	entry.CostPlan.BaseComplexity = 60
	entry.CostPlan.MaxResponseBytes = 64 * 1024
	entry.AppClientBundle = contentPostAppClientBundle(entry.OperationName)
	syncCostPlan(&entry)
	return entry
}
...
func contentPostBundleEntry(operationName, hash string) domain.Entry {
	...
	entry.AppClientBundle = contentPostAppClientBundle(operationName)
	return entry
}

func contentPostAppClientBundle(operationName string) *domain.AppClientBundle {
	selected := map[string][]string{
		...
		"ContentPostDetailArticleRenderAssets": {
			"articleAssetManifestSummary", "articleAssets", "articleFontPreset", "articleMarkdown",
			"articleMarkdownDigest", "articleRenderProfileSummary", "articleTemplate", "contentType",
			"contentVertical", "markdownDialect", "postId",
		},
		"ContentPostDetailArticleEntities": {"contentType", "entityMentions", "postId"},
	}
	...
	if operationName == "ContentPostDetailArticleRenderAssets" {
		bundle.AssemblyMappings = []domain.AssemblyMapping{
			{TargetField: "articleAssetManifest", PresenceSourceField: "articleAssetManifestSummary", Sources: []domain.AssemblySource{
			...
			{TargetField: "articleRenderProfile", Sources: []domain.AssemblySource{
				{SourceField: "articleRenderProfileSummary", Strategy: "replace"},
			}},
		}
	}
	return bundle
