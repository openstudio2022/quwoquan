...
type CircleGroupConversationBindingFailureStore interface {
...
type CircleGroupConversationBindingConsumer struct {
	...
	failures    CircleGroupConversationBindingFailureStore
	consumer    string
	logger      *slog.Logger
	mu          sync.RWMutex
...
func (c *CircleGroupConversationBindingConsumer) processMessage(
	...
	startedAt := time.Now()
	fact, err := decodeCircleGroupConversationBindingEvent(durableFieldValues(message.Fields))
	...
	attempts, recordErr := c.failures.RecordCircleGroupConversationBindingFailure(
		ctx,
		message.ID,
		strings.TrimSpace(durableFieldValues(message.Fields)["eventId"]),
