...
func NewMongoMemberCountProjector(
	database *mongo.Database,
	cache circleports.CacheInvalidator,
) *MongoMemberCountProjector {
	if database == nil || cache == nil {
		panic("CircleMembership member-count projector requires MongoDB and cache invalidator")
	}
...
func (projector *MongoMemberCountProjector) Apply(ctx context.Context, event circleapp.DerivedCountEvent) error {
...
func (projector *MongoMemberCountProjector) apply(ctx context.Context, event circleapp.DerivedCountEvent, circleID string) error {
	delta := int64(0)
...
var _ circleapp.DerivedCountProjection = (*MongoMemberCountProjector)(nil)
