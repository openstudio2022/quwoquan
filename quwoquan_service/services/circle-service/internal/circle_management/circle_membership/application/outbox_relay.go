...
// OutboxRelay gives every CircleMembership sink an independent durable
// checkpoint. A failed sink never advances another sink's waterline.
type OutboxRelay struct {
	reader      membershipports.OutboxReader
	checkpoints membershipports.ProjectionCheckpointStore
	publisher   membershipports.OutboxPublisher
	consumer    string
	mu          sync.RWMutex
...
func NewOutboxRelay(reader membershipports.OutboxReader, checkpoints membershipports.ProjectionCheckpointStore, publisher membershipports.OutboxPublisher, consumer string) *OutboxRelay {
...
func (relay *OutboxRelay) Drain(ctx context.Context, limit int) (int, error) {
	if relay == nil || relay.reader == nil || relay.checkpoints == nil || relay.publisher == nil || relay.consumer == "" {
		return 0, fmt.Errorf("CircleMembership outbox relay is not fully configured")
	}
	checkpoint, err := relay.checkpoints.LoadCheckpoint(ctx, relay.consumer)
	if err != nil {
		return 0, fmt.Errorf("load CircleMembership checkpoint: %w", err)
	}
	events, err := relay.reader.ReadAfter(ctx, checkpoint, limit)
	if err != nil {
		return 0, fmt.Errorf("read CircleMembership outbox: %w", err)
	}
	for index, event := range events {
		if strings.TrimSpace(event.Checkpoint) == "" {
			return index, fmt.Errorf("CircleMembership event %q has no checkpoint", event.EventID)
		}
		if err := relay.publisher.Publish(ctx, event); err != nil {
			return index, fmt.Errorf("publish CircleMembership event %q: %w", event.EventID, err)
		}
		if err := relay.checkpoints.SaveCheckpoint(ctx, relay.consumer, event.Checkpoint); err != nil {
			return index, fmt.Errorf("save CircleMembership checkpoint for %q: %w", event.EventID, err)
		}
	}
...
func (relay *OutboxRelay) Healthy(maxStaleness time.Duration) error {
	if relay == nil {
		return fmt.Errorf("CircleMembership outbox relay is not configured")
	}
	if maxStaleness <= 0 {
	...
	if relay.lastSuccess.IsZero() {
		return fmt.Errorf("CircleMembership outbox relay has not completed a scan")
	}
	if relay.lastFailure != nil {
		return fmt.Errorf("CircleMembership outbox relay last failure: %w", relay.lastFailure)
	}
	if time.Since(relay.lastSuccess) > maxStaleness {
		return fmt.Errorf("CircleMembership outbox relay heartbeat is stale")
	}
	return nil
}
