...
func (store *MongoAggregateStore) ReadAfter(ctx context.Context, checkpoint string, limit int) ([]membershipports.OutboxEvent, error) {
	if limit <= 0 {
		limit = 100
	}
	if limit > 1000 {
	...
	if strings.TrimSpace(checkpoint) != "" {
		sequence, err := parseMembershipCheckpoint(checkpoint)
		if err != nil {
			return nil, err
		}
	...
	if err != nil {
		return nil, fmt.Errorf("read CircleMembership outbox: %w", err)
	}
	defer rows.Close(ctx)
	var events []membershipports.OutboxEvent
	for rows.Next(ctx) {
		var document struct {
			ID               string    `bson:"_id"`
		...
		if !json.Valid(payload) {
			return nil, fmt.Errorf("CircleMembership outbox %q contains invalid payload JSON", document.ID)
		}
		events = append(events, membershipports.OutboxEvent{
...
func (store *MongoAggregateStore) LoadCheckpoint(ctx context.Context, consumer string) (string, error) {
	...
	if consumer == "" {
		return "", fmt.Errorf("CircleMembership consumer is required")
	}
	var document struct {
		Sequence int64 `bson:"sequence"`
	}
	err := store.checkpoints.FindOne(ctx, bson.M{"_id": "circle-membership:" + consumer}).Decode(&document)
...
func (store *MongoAggregateStore) SaveCheckpoint(ctx context.Context, consumer, checkpoint string) error {
	...
	_, err = store.checkpoints.UpdateOne(ctx, bson.M{"_id": "circle-membership:" + consumer},
		bson.M{"$max": bson.M{"sequence": sequence}, "$set": bson.M{"updatedAt": time.Now().UTC()}},
		options.UpdateOne().SetUpsert(true))
	return err
}

func parseMembershipCheckpoint(checkpoint string) (int64, error) {
	sequence, err := strconv.ParseInt(strings.TrimSpace(checkpoint), 10, 64)
	if err != nil || sequence <= 0 {
		return 0, fmt.Errorf("invalid CircleMembership checkpoint")
	}
	return sequence, nil
}

var (
	_ membershipports.OutboxReader              = (*MongoAggregateStore)(nil)
	_ membershipports.ProjectionCheckpointStore = (*MongoAggregateStore)(nil)
)
