...
func NewCircleGroupStreamPublisher(transport runtimemessaging.DurableRecordAppender) *CircleGroupStreamPublisher {
...
func (publisher *CircleGroupStreamPublisher) Publish(ctx context.Context, event groupports.OutboxEvent) error {
	...
	if err := runtimemessaging.AppendDurableRecord(ctx, publisher.transport, CircleGroupStream, map[string]string{
