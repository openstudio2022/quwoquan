...
func (store *MongoAggregateStore) ReadAfter(ctx context.Context, checkpoint string, limit int) ([]groupports.OutboxEvent, error) {
	if limit <= 0 {
		limit = 100
	}
	if limit > 1000 {
	...
	if strings.TrimSpace(checkpoint) != "" {
		sequence, err := parseGroupCheckpoint(checkpoint)
		if err != nil {
			return nil, err
		}
	...
	if err != nil {
		return nil, fmt.Errorf("read CircleGroup outbox: %w", err)
	}
	defer rows.Close(ctx)
	var events []groupports.OutboxEvent
	for rows.Next(ctx) {
		var document struct {
			ID               string    `bson:"_id"`
		...
		if !json.Valid(payload) {
			return nil, fmt.Errorf("CircleGroup outbox %q contains invalid payload JSON", document.ID)
		}
		events = append(events, groupports.OutboxEvent{
...
func (store *MongoAggregateStore) LoadCheckpoint(ctx context.Context, consumer string) (string, error) {
	...
	if consumer == "" {
		return "", fmt.Errorf("CircleGroup consumer is required")
	}
	var document struct {
		Sequence int64 `bson:"sequence"`
	}
	err := store.checkpoints.FindOne(ctx, bson.M{"_id": "circle-group:" + consumer}).Decode(&document)
...
func (store *MongoAggregateStore) SaveCheckpoint(ctx context.Context, consumer, checkpoint string) error {
	...
	_, err = store.checkpoints.UpdateOne(ctx, bson.M{"_id": "circle-group:" + consumer},
		bson.M{"$max": bson.M{"sequence": sequence}, "$set": bson.M{"updatedAt": time.Now().UTC()}},
		options.UpdateOne().SetUpsert(true))
	return err
}

func parseGroupCheckpoint(checkpoint string) (int64, error) {
	sequence, err := strconv.ParseInt(strings.TrimSpace(checkpoint), 10, 64)
	if err != nil || sequence <= 0 {
		return 0, fmt.Errorf("invalid CircleGroup checkpoint")
	}
	return sequence, nil
}

var (
	_ groupports.OutboxReader              = (*MongoAggregateStore)(nil)
	_ groupports.ProjectionCheckpointStore = (*MongoAggregateStore)(nil)
)
