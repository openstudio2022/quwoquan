...
type CircleGroupMembershipStreamPublisher struct {
	transport runtimemessaging.DurableRecordAppender
}

func NewCircleGroupMembershipStreamPublisher(transport runtimemessaging.DurableRecordAppender) *CircleGroupMembershipStreamPublisher {
...
func (publisher *CircleGroupMembershipStreamPublisher) Publish(ctx context.Context, event ports.OutboxEvent) error {
	...
	if err := runtimemessaging.AppendDurableRecord(ctx, publisher.transport, CircleGroupMembershipStream, map[string]string{
