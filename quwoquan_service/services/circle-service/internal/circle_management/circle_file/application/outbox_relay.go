...
type OutboxRelay struct {
	reader      fileports.OutboxReader
	checkpoints fileports.ProjectionCheckpointStore
	publisher   fileports.OutboxPublisher
	consumer    string
	mu          sync.RWMutex
...
func NewOutboxRelay(reader fileports.OutboxReader, checkpoints fileports.ProjectionCheckpointStore, publisher fileports.OutboxPublisher, consumer string) *OutboxRelay {
...
func (relay *OutboxRelay) Drain(ctx context.Context, limit int) (int, error) {
	if relay == nil || relay.reader == nil || relay.checkpoints == nil || relay.publisher == nil || relay.consumer == "" {
		return 0, fmt.Errorf("CircleFile outbox relay is not fully configured")
	}
	checkpoint, err := relay.checkpoints.LoadCheckpoint(ctx, relay.consumer)
	if err != nil {
		return 0, fmt.Errorf("load CircleFile checkpoint: %w", err)
	}
	events, err := relay.reader.ReadAfter(ctx, checkpoint, limit)
	if err != nil {
		return 0, fmt.Errorf("read CircleFile outbox: %w", err)
	}
	for index, event := range events {
		if strings.TrimSpace(event.Checkpoint) == "" {
			return index, fmt.Errorf("CircleFile event %q has no checkpoint", event.EventID)
		}
		if err := relay.publisher.Publish(ctx, event); err != nil {
			return index, fmt.Errorf("publish CircleFile event %q: %w", event.EventID, err)
		}
		if err := relay.checkpoints.SaveCheckpoint(ctx, relay.consumer, event.Checkpoint); err != nil {
			return index, fmt.Errorf("save CircleFile checkpoint for %q: %w", event.EventID, err)
		}
	}
...
func (relay *OutboxRelay) Healthy(maxStaleness time.Duration) error {
	if relay == nil {
		return fmt.Errorf("CircleFile outbox relay is not configured")
	}
	if maxStaleness <= 0 {
	...
	if relay.lastSuccess.IsZero() {
		return fmt.Errorf("CircleFile outbox relay has not completed a scan")
	}
	if relay.lastFailure != nil {
		return fmt.Errorf("CircleFile outbox relay last failure: %w", relay.lastFailure)
	}
	if time.Since(relay.lastSuccess) > maxStaleness {
		return fmt.Errorf("CircleFile outbox relay heartbeat is stale")
	}
	return nil
}
