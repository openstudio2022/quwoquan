...
func NewCircleFileStreamPublisher(transport runtimemessaging.DurableRecordAppender) *CircleFileStreamPublisher {
...
func (publisher *CircleFileStreamPublisher) Publish(ctx context.Context, event fileports.OutboxEvent) error {
	...
	if err := runtimemessaging.AppendDurableRecord(ctx, publisher.transport, CircleFileStream, map[string]string{
