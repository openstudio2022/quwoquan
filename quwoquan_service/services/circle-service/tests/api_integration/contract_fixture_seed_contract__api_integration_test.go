...
func TestContractFixtureSeedContractApiIntegrationTest(t *testing.T) {
	...
	cmd := exec.Command(
		...
		"..",
		"-run",
		"^(TestContractFixtureSeed_CircleAlphaReadsViaHandler)$",
		"-count=1",
	)
	cmd.Env = append(os.Environ(), "QWQ_OUTPUT_ROOT="+filepath.Join(repoRoot(), ".qwq_output"))
	output, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("source bridge failed for quwoquan_service/services/circle-service/tests/contract_fixture_seed_contract_test.go: %v\n%s", err, output)
	}
}
