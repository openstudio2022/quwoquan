...
func TestContentPostConsumerProjectsTypedStreamFactAndAcks(t *testing.T) {
	...
	if spy.events[0].FeedItem == nil ||
		spy.events[0].FeedItem.ContentType != "article" ||
		spy.events[0].FeedItem.Title != "West Lake" ||
		len(spy.events[0].FeedItem.MediaURLs) != 1 {
		t.Fatalf("typed CircleFeedItem snapshot drift: %#v", spy.events[0].FeedItem)
	}
...
func postLifecycleValues(eventID string, version int64) map[string]string {
	return map[string]string{
		...
		"payload": "{\"postId\":\"post-1\",\"authorId\":\"persona-1\"," +
			"\"status\":\"published\",\"visibility\":\"public\",\"moderationStatus\":\"approved\"," +
			"\"contentType\":\"article\",\"contentIdentity\":\"work\",\"title\":\"West Lake\"," +
			"\"mediaUrls\":[\"https://media.example/post-1.jpg\"]," +
			"\"createdAt\":\"2026-07-14T08:00:00Z\",\"updatedAt\":\"2026-07-14T09:00:00Z\"," +
