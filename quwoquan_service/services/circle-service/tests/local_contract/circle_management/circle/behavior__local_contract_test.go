...
import (
	"errors"
	. "quwoquan_service/services/circle-service/internal/circle_management/circle/domain/model"
	"strings"
	"testing"
	"time"
)
...
func TestCircleGovernanceTextRejectsOversizeInput(t *testing.T) {
	now := time.Date(2026, 7, 21, 0, 0, 0, 0, time.UTC)
	name := "超长圈规测试"
	oversizeRules := strings.Repeat("规", 1<<16)
	...
	oversizeWelcome := strings.Repeat("迎", 1<<16)
