...
const (
	...
	PushProviderAlphaFake    = "alpha_fake"
...
// AlphaFakePushProvider 只用于 APP_ENV=alpha 的 composition。它仍执行完整 payload
// 校验，但不会解析或记录 endpoint token。
type AlphaFakePushProvider struct{}

func (AlphaFakePushProvider) Send(
	_ context.Context,
	request reliabletask.ExternalInteractionRequest,
	_ reliabletask.ReliableAsyncTask,
	...
	if _, err := ParsePushDeliveryMessage(request); err != nil {
		failure := &PushProviderFailure{
			Code:      generated.ErrPushDeliveryInvalidRequest.Error(),
			Provider:  PushProviderAlphaFake,
			Retryable: false,
			Cause:     err,
		}
		return failedPushResult(request, PushProviderAlphaFake, failure), failure
	}
	return reliabletask.ExternalInteractionResult{
		RequestID:         request.RequestID,
		Operation:         request.Operation,
		Status:            reliabletask.ExternalInteractionStatusSentUnconfirmed,
		Provider:          PushProviderAlphaFake,
		ProviderRequestID: "alpha-" + request.RequestID,
		CallbackURL:       request.CallbackURL,
		OccurredAt:        time.Now().UTC(),
	}, nil
}
