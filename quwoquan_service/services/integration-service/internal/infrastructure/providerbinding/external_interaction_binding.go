...
// ResolveSMSBinding materializes the only release SMS adapter. It never
// interprets runtime provider names or falls back to test transports.
...
// ResolvePushBinding materializes the only release Push dispatch adapter.
...
func resolveExternalInteractionBinding(
	...
	if binding.AdapterID != expectedAdapterID || binding.TimeoutMilliseconds <= 0 {
		return ExternalInteractionBinding{}, fmt.Errorf(
