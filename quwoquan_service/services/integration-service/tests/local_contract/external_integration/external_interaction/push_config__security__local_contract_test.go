...
func TestPushConfigAcceptsLocalRecorderOnlyOnAlphaBeta(t *testing.T) {
	...
	for _, appEnv := range []string{"alpha", "beta"} {
	...
	for _, appEnv := range []string{"gamma", "prod"} {
		cfg.Environment = appEnv
		if err := integrationconfig.Validate(cfg); err == nil ||
			!strings.Contains(err.Error(), "only permitted in alpha/beta") {
...
func TestPushObservedEndpointsRedactTokenAndEndpointRef(t *testing.T) {
	...
	if got := provider.ObservedEndpoint(apnsRequest); got != "/3/device/{token}" {
	...
	if got := provider.ObservedEndpoint(secretRequest); got != serviceclients.UserPushEndpointSecretPathTemplate {
