...
import (
	...
	maperrors "quwoquan_service/services/travel-service/generated/travel/trip_map_view"
...
const getOperation = "travel.trip_map_view.GetTripMap"
...
func NewHandler(reader *application.Reader) *Handler {
	if reader == nil {
		panic("TripMapView HTTP handler requires reader")
	}
...
func (handler *Handler) RegisterRoutes(mux *stdhttp.ServeMux) {
	if mux == nil {
		panic("TripMapView HTTP handler requires ServeMux")
	}
...
func (handler *Handler) handleGet(writer stdhttp.ResponseWriter, request *stdhttp.Request) {
	...
	httpcodec.WriteJSON(writer, stdhttp.StatusOK, view, "travel_trip_map_view")
}
...
func queryActor(request *stdhttp.Request) (string, error) {
	...
	if !ok || current.Actor.Validate(operation.ActorPersona) != nil {
		return "", triperrors.AppErrorFromTripPermissionDenied(
			"TripMapView query requires a verified persona operation context",
		)
	}
...
func mapApplicationError(err error) error {
	...
	case errors.Is(err, ports.ErrProjectionUnavailable), errors.Is(err, ports.ErrNotFound):
		return maperrors.AppErrorFromTripMapProjectionUnavailable(err.Error())
	...
	default:
		...
		return maperrors.AppErrorFromTripMapProjectionFailed(err.Error())
	}
}
...
func mustPattern(operationID string) string {
	...
	panic("missing generated TripMapView operation descriptor: " + operationID)
}
