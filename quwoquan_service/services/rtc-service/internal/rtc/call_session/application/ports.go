...
type CallStore interface {
	...
	// FindActiveCallsForUsers returns terminal-security candidates in bounded
	// batches. The caller repeats until empty so every persona-owned active
	// CallSession is closed without relying on cache state.
	FindActiveCallsForUsers(
		ctx context.Context,
		userIDs []string,
		limit int,
...
type CallStateCache interface {
	...
	DeleteCallState(ctx context.Context, callID string) error
}
...
// AccountSecurityTerminalCloser applies an already-validated durable account
// security fact. It is intentionally narrower than CallOrchestrator so the
// stream adapter cannot invoke user-originated call commands.
type AccountSecurityTerminalCloser interface {
	ApplyAccountSecurityTerminalEvent(
		ctx context.Context,
		event AccountSecurityTerminalEvent,
