...
func (f *ImportFacade) Reconcile(ctx context.Context, request ImportRequest) (ImportReport, error) {
	...
	if request.SourceOwner == "" || request.SourceReleaseID == "" {
		return ImportReport{}, generated.AppErrorFromInvalidArgument(
			"homepage import source owner and release id are required",
		)
	}
	...
	for _, input := range request.Inputs {
		...
		meta := CommandMeta{
			...
			IdempotencyKey: "import:" + request.SourceReleaseID + ":" + entityRef,
		}
		if !found {
...
func (f *ImportFacade) offlineStale(
	...
	for {
		...
		for _, snapshot := range page.Items {
			...
			meta := CommandMeta{
				ActorID:        request.SourceOwner,
				IdempotencyKey: "import-sync:" + request.SourceReleaseID + ":" + snapshot.ID,
			}
