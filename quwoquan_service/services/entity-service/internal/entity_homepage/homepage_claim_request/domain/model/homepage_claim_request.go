...
var (
	...
	ErrInvalidMaterialURL  = errors.New("homepage claim material URL is invalid")
...
func (r *HomepageClaimRequest) validate() error {
	...
	if (r.businessLicenseURL != "" && !isCanonicalHTTPSURL(r.businessLicenseURL)) ||
		(r.identityCardFrontURL != "" && !isCanonicalHTTPSURL(r.identityCardFrontURL)) ||
		(r.identityCardBackURL != "" && !isCanonicalHTTPSURL(r.identityCardBackURL)) {
		return ErrInvalidMaterialURL
	}
