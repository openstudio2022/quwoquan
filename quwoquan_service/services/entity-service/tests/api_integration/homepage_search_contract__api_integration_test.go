...
func TestHomepageSearchContractApiIntegrationTest(t *testing.T) {
	...
	cmd := exec.Command(
		...
		"..",
		"-run",
		"^(TestHomepageSearchUsesCanonicalSearchSignals)$",
		"-count=1",
	)
	cmd.Env = append(os.Environ(), "QWQ_OUTPUT_ROOT="+filepath.Join(repoRoot(), ".qwq_output"))
	output, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("source bridge failed for quwoquan_service/services/entity-service/tests/homepage_search_contract_test.go: %v\n%s", err, output)
	}
}
