...
import (
	...
	"encoding/json"
	"errors"
	. "quwoquan_service/services/entity-service/internal/entity_homepage/homepage_claim_request/application"
	"sort"
	"strings"
	"sync"
	"testing"
	...
	if commit.ExpectedVersion == 0 {
		for _, candidate := range s.claims {
			if candidate.HomepageID == snapshot.HomepageID &&
				...
				return claimports.CommitResult{},
					claimgenerated.AppErrorFromDuplicatePendingClaim("duplicate pending claim")
			}
		}
		if exists {
			return claimports.CommitResult{},
...
func TestClaimCreateReceiptDuplicateAndCanonicalEvent(t *testing.T) {
	...
	if _, err := facade.Create(commandContext("claim-duplicate"), command); !hasCode(
		err, claimgenerated.ErrDuplicatePendingClaim,
	) {
...
// spec_ref: specs/feature-tree/shared-homepage-network/homepage-claim-maintain-and-offline/homepage-claim-request-and-review/spec.md#gwt-001
func TestClaimRejectsUnsafeMaterialURLs(t *testing.T) {
	for name, mutate := range map[string]func(*CreateCommand){
		"license javascript scheme": func(command *CreateCommand) {
			command.BusinessLicenseURL = "javascript:alert(document.domain)"
		},
		"identity front http scheme": func(command *CreateCommand) {
			command.IdentityCardFrontURL = "http://assets.test/identity-front"
			command.IdentityCardBackURL = "https://assets.test/identity-back"
		},
		"identity back embedded credentials": func(command *CreateCommand) {
			command.IdentityCardFrontURL = "https://assets.test/identity-front"
			command.IdentityCardBackURL = "https://operator:secret@assets.test/identity-back"
		},
	} {
		t.Run(name, func(t *testing.T) {
			...
			command := validCreateCommand()
			mutate(&command)
			if _, err := facade.Create(commandContext("claim-unsafe-url-"+name), command); !hasCode(
				err,
				claimgenerated.ErrInvalidClaimMaterialURL,
			) {
				t.Fatalf("unsafe claim material URL must be rejected: %v", err)
			}
		})
	}
}
