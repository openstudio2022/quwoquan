...
import (
	...
	"encoding/json"
	"errors"
	. "quwoquan_service/services/entity-service/internal/entity_homepage/homepage_status_report/application"
	"sort"
	"strings"
	"sync"
	"testing"
...
// spec_ref: specs/feature-tree/shared-homepage-network/homepage-claim-maintain-and-offline/homepage-offline-report-and-history-retention/spec.md#gwt-001
func TestStatusReportRejectsUnsafeEvidenceURLs(t *testing.T) {
	for name, evidenceURL := range map[string]string{
		"javascript scheme":               "javascript:alert(document.domain)",
		"http scheme":                     "http://assets.test/offline-proof",
		"https with embedded credentials": "https://operator:secret@assets.test/offline-proof",
	} {
		t.Run(name, func(t *testing.T) {
			facade, _ := newFacadeForTest(t)
			command := validCreateCommand()
			command.EvidenceURLs = []string{evidenceURL}
			if _, err := facade.Create(commandContext("report-unsafe-url-"+name), command); !hasCode(
				err,
				reportgenerated.ErrInvalidStatusReportEvidenceURL,
			) {
				t.Fatalf("unsafe status-report evidence URL must be rejected: %v", err)
			}
		})
	}
}
