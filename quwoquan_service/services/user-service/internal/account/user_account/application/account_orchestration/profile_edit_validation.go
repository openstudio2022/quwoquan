...
var ErrProfileTaxonomyReleaseConflict = errors.New("profile taxonomy release conflict")
...
func (PathProfileTagValidator) ValidateProfileTags(
	_ context.Context,
	expectedTaxonomyReleaseID string,
	occupationTagRef string,
	interestTagRefs []string,
) error {
	if strings.TrimSpace(expectedTaxonomyReleaseID) == "" {
		return fmt.Errorf(
			"%w: expectedTaxonomyReleaseId is required",
			ErrProfileTaxonomyReleaseConflict,
		)
	}
