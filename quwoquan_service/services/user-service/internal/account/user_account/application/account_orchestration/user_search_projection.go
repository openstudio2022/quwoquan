...
// UserProfileSearchEligible defines the discoverable user set: an active account
// with an active status. Anonymous / suspended / deleted accounts are not
...
func UserProfileSearchEligible(profile model.UserProfile) bool {
	return strings.EqualFold(strings.TrimSpace(profile.AccountState), "active") &&
		strings.EqualFold(strings.TrimSpace(profile.Status), "active")
}
