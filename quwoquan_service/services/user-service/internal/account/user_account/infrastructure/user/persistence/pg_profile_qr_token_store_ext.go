...
func (s *PgProfileQrTokenStore) FindActiveByOwnerAndHandle(ctx context.Context, ownerUserID, userHandle string) (*model.ProfileQrToken, error) {
	return generated.ScanProfileQrToken(s.pool.QueryRow(ctx,
		`SELECT `+generated.ProfileQrTokenCols+` FROM profile_qr_tokens
		 WHERE owner_user_id = $1 AND user_handle = $2 AND status = 'active' AND revoked_at IS NULL
		 ORDER BY created_at DESC
		 LIMIT 1`,
		ownerUserID, userHandle))
}
