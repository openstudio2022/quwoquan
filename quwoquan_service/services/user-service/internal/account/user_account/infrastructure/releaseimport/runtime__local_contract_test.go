...
func creatorReleaseFixture(t *testing.T) string {
	...
	writeReleaseTestFile(
		...
		`{"schema":"quwoquan_data.creator_profile","creatorId":"creator-a","userId":"author-a","authorId":"author-a","subAccountId":"author-a","displayName":"Creator A","userHandle":"creator_a","avatarAsset":{"assetId":"avatar-a","kind":"avatar","sha256":"`+testAvatarSHA+`"},"headline":"headline","bio":"bio","creatorArchetype":"guide","publicProfileTagRefs":[]}`,
	)
...
func TestLoadCreatorsResolvesAvatarFromReleaseMediaAuthority(t *testing.T) {
	...
	state, creators, err := loadCreators(root, "https://avatar.example.com")
	if err != nil {
		t.Fatal(err)
	}
...
func TestLoadCreatorsRejectsLegacyAndInconsistentAvatarBindings(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, _, err = loadCreators(root, "https://avatar.example.com")
