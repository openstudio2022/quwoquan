...
func (store *CloseStore) CommitClose(
	...
	accountAuthEpoch := state.authEpoch
	if !alreadyClosed {
		accountVersion++
		accountAuthEpoch++
	}
...
func loadAccountForClose(
	...
	err := tx.QueryRow(ctx, `
SELECT account_state, auth_epoch, updated_at, profile_version, phone
FROM user_profiles
WHERE user_id = $1
FOR UPDATE`, accountID).Scan(
	...
	if state.authEpoch <= 0 {
		return accountCloseState{}, errors.New(
			"load account for close: invalid auth epoch",
		)
	}
	if phone != nil {
		state.phone = *phone
