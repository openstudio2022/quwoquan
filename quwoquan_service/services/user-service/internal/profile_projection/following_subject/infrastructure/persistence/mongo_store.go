...
func (s *MongoFollowingSubjectStore) DeleteForClosedSubjects(
	ctx context.Context,
	accountID string,
	personaIDs []string,
) error {
	...
	subjectIDs := append([]string{strings.TrimSpace(accountID)}, personaIDs...)
	...
	if err != nil {
		return fmt.Errorf("delete closed subject FollowingSubject projections: %w", err)
	}
	return nil
}
