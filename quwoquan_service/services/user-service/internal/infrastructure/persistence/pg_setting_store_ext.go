...
var _ repository.UserSettingsStore = (*PgSettingStore)(nil)
...
func (s *PgSettingStore) FindByUserID(ctx context.Context, userID string) (*model.UserSetting, error) {
	row := s.pool.QueryRow(ctx, `
		SELECT user_id, enable_push, enable_marketing, quiet_hours_start, quiet_hours_end,
		       ...
		       appearance_updated_at, blocked_keywords, updated_at
		  FROM user_settings
		 WHERE user_id = $1
	`, userID)
	...
	if err := row.Scan(
		...
		&st.QuietHoursStart,
		&st.QuietHoursEnd,
		&st.DefaultIncomingCallRingtoneID,
		&st.AllowCallerRingtoneOverride,
		&st.EnableCallVibration,
		&st.EnableGroupCallRing,
		&st.AllowStrangerMsg,
		&st.ProfileVisibility,
		&st.ContentLanguage,
		&st.FeedPreference,
		&st.AssistantEnabled,
		&st.DefaultThemeMode,
		&st.DefaultFontSizePreset,
		&st.AppearanceVersion,
		&st.AppearanceUpdatedAt,
		&st.BlockedKeywords,
		&st.UpdatedAt,
	); err != nil {
