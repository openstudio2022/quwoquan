...
type GreetingCommandStore interface {
	LoadCommandReceipt(ctx context.Context, actorSubAccountID, idempotencyKey, operation string) (*model.GreetingRequest, bool, error)
	CommitCommand(ctx context.Context, commit GreetingCommit) error
	CountRecentByRequester(ctx context.Context, requesterSubAccountID string, window time.Duration) (int64, error)
}
