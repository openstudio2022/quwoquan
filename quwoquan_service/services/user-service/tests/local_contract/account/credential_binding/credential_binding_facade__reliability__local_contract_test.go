...
func TestCredentialBindingFacadeUsesUniqueConstraintForConflictAndNoop(t *testing.T) {
	...
	if err == nil || rterr.NormalizeError(err).Code.String() != bindinggenerated.ErrCredentialConflict.Error() {
...
func TestCredentialBindingFacadeProtectsLastCredentialAndRetriesCAS(t *testing.T) {
	...
	if err == nil || rterr.NormalizeError(err).Code.String() != bindinggenerated.ErrLastCredential.Error() {
