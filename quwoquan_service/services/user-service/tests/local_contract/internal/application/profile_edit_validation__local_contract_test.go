...
func TestProfileCareerInterestTagValidation(t *testing.T) {
	...
	if err := validator.ValidateProfileTags(context.Background(), "Audience/用户/职业/产品运营", nil); err == nil {
		t.Fatalf("category occupation tag should be rejected")
	}
	if err := validator.ValidateProfileTags(context.Background(), "Audience/用户/职业/旧分类/产品经理", nil); err == nil {
		t.Fatalf("unknown occupation category should be rejected")
	}
	if err := validator.ValidateProfileTags(context.Background(), "", []string{"Topic/兴趣/旅行"}); err == nil {
	...
	if err := validator.ValidateProfileTags(context.Background(), "", tooMany); err == nil {
