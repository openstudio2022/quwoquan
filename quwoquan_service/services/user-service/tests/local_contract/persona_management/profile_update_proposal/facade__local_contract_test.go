...
func migratedFacadeReceiptKey(proposalID, idempotencyKey string) string {
	return proposalID + "\x00" + idempotencyKey
}

func (s *migratedFacadeMemoryProposalStore) Replay(_ context.Context, proposalID, idempotencyKey, digest string) (ports.CommitReceipt, bool, error) {
	record, ok := s.receipts[migratedFacadeReceiptKey(proposalID, idempotencyKey)]
	if !ok {
...
func (s *migratedFacadeMemoryProposalStore) RecordNoopReceipt(
	...
	key := migratedFacadeReceiptKey(proposal.ID, idempotencyKey)
...
func (s *migratedFacadeMemoryProposalStore) Commit(_ context.Context, expectedVersion int64, changes ports.ChangeSet) (ports.CommitReceipt, error) {
	...
	key := migratedFacadeReceiptKey(changes.Proposal.ID, changes.IdempotencyKey)
	if record, ok := s.receipts[key]; ok {
...
func (w *migratedFacadeRecordingPersonaWriter) ApplyProfileProposal(_ context.Context, command personaports.ApplyProfileProposalCommand) error {
...
func TestFacadeOwnerAndIdempotentApply(t *testing.T) {
	...
	writer := &migratedFacadeRecordingPersonaWriter{version: 8, proposals: map[string]personaports.ApplyProfileProposalCommand{}}
