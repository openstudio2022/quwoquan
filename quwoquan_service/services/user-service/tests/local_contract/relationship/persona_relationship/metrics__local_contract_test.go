...
import (
	. "quwoquan_service/services/user-service/internal/relationship/persona_relationship/domain/telemetry"
	"testing"
	"time"
)

type migratedMetricsRecordingSink struct {
...
func (sink *migratedMetricsRecordingSink) ObserveCommandLatency(milliseconds float64) {
	sink.commandLatencies = append(sink.commandLatencies, milliseconds)
}

func (sink *migratedMetricsRecordingSink) ObserveListLatency(milliseconds float64) {
	sink.listLatencies = append(sink.listLatencies, milliseconds)
}

func (sink *migratedMetricsRecordingSink) ObserveCounterProjectionLag(milliseconds float64) {
	sink.projectionLatencies = append(sink.projectionLatencies, milliseconds)
}

func (sink *migratedMetricsRecordingSink) IncrementCounter(metricName string) {
	sink.counters[metricName]++
}

func TestMetrics_ForwardsProductionSamplesAndPreservesSnapshot(t *testing.T) {
	sink := &migratedMetricsRecordingSink{counters: map[string]int{}}
	metrics := &Metrics{}
