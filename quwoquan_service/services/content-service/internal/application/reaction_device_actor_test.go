...
func TestReactionActorKey_UserFirstThenDevice(t *testing.T) {
	if got := reactionActorKey("user_1", "dev_abc"); got != "user_1" {
		t.Fatalf("账号维度应优先 userId，got %q", got)
	}
	if got := reactionActorKey("", "dev_abc"); got != deviceActorKeyPrefix+"dev_abc" {
		t.Fatalf("游客应回落到设备维度键，got %q", got)
	}
	if got := reactionActorKey("  ", "  "); got != AnonymousFallbackSubAccountID {
		t.Fatalf("二者皆空应回落匿名常量，got %q", got)
	}
	// 账号维度键与设备维度键键空间不相交（命名空间化），保证天然独立计数。
	if reactionActorKey("dev_abc", "") == reactionActorKey("", "dev_abc") {
