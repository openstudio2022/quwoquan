...
type feedIntersectionProvider interface {
	Feed(ctx context.Context, userID, channel string, limit int) ([]IntersectionReasonView, error)
}
...
func attachFeedIntersections(views []FeedItemView, reasons []IntersectionReasonView, userID string) {
	if len(views) == 0 || len(reasons) == 0 {
		return
	}
	var lightPool, heavyPool []IntersectionReasonView
	...
	for i := range views {
		...
		case bucket < feedIntersectionHeavyPercent && len(heavyPool) > 0:
			views[i].IntersectionReasons = []IntersectionReasonView{
