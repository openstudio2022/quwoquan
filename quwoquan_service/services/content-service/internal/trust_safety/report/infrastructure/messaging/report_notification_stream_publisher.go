...
const (
	ReportNotificationStream          = "events.content.report_lifecycle"
	ReportNotificationStreamRetention = 7 * 24 * time.Hour
)
...
func (p *ReportNotificationStreamPublisher) Publish(
	...
	if err := p.redis.XTrimOlderThan(ctx, ReportNotificationStream, ReportNotificationStreamRetention); err != nil {
		return fmt.Errorf("trim report notification stream retention: %w", err)
	}
	if err := p.redis.Expire(ctx, ReportNotificationStream, ReportNotificationStreamRetention); err != nil {
		return fmt.Errorf("bound inactive report notification stream retention: %w", err)
	}
	return nil
}
