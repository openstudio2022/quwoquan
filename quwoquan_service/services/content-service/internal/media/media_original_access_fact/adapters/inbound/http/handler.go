...
type Handler struct{ service *originalaccessapp.Service }

func NewHandler(service *originalaccessapp.Service) *Handler {
	if service == nil {
		panic("MediaOriginalAccessFact HTTP handler requires service")
	}
	return &Handler{service: service}
}

func (handler *Handler) Request(writer http.ResponseWriter, request *http.Request) {
	...
	result, err := handler.service.Request(request.Context(), originalaccessapp.Command{
