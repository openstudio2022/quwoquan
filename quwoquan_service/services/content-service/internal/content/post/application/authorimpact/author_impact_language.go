...
// DecorateAuthorImpact only attaches deterministic metadata derived from the
// evidence-backed aggregate. Every row receives a cloud-generated conclusion
// sentence so the app never has to synthesize text or render a false empty
// state. Actor identity stays anonymous unless a separate privacy-safe
...
func DecorateAuthorImpact(summary ports.AuthorImpactSummary, viewerIsAuthor bool) ports.AuthorImpactSummary {
	...
	for i := range summary.Items {
		if strings.TrimSpace(summary.Items[i].PrimaryText) == "" {
			summary.Items[i].PrimaryText = rtimpact.EvidenceText(
