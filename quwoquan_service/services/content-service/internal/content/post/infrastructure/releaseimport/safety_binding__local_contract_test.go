...
func TestCandidatePostSafetyIdentityIndexModel(t *testing.T) {
	models := candidatePostIndexes()
	if len(models) != 3 {
		t.Fatalf("candidate Post index count = %d, want 3", len(models))
	}
	safety := models[2]
	wantKeys := bson.D{{Key: "environment", Value: 1}, {Key: "sourceOwner", Value: 1}, {Key: "postId", Value: 1}}
	if !reflect.DeepEqual(safety.Keys, wantKeys) {
		t.Fatalf("safety index keys = %#v, want %#v", safety.Keys, wantKeys)
	}
	resolved := options.IndexOptions{}
	for _, apply := range safety.Options.List() {
		if err := apply(&resolved); err != nil {
			t.Fatal(err)
		}
	}
	if resolved.Name == nil || *resolved.Name != "idx_data_release_candidate_post_safety_identity" {
		t.Fatalf("safety index name = %#v", resolved.Name)
	}
	if resolved.Unique != nil && *resolved.Unique {
		t.Fatal("safety identity index must be non-unique")
	}
}
...
func TestDataSafetyBindingAndModerationMongo(t *testing.T) {
	...
	result, err := activateImportedPostReleaseTransaction(ctx, db, "alpha", a, ExpectedActiveRelease{Empty: true, SourceOwner: "qwq_data"}, now)
	if err != nil {
	...
	if _, err = activateImportedPostReleaseTransaction(ctx, db, "alpha", b, ExpectedActiveRelease{SourceOwner: "qwq_data", ReleaseID: a.ReleaseID, ManifestDigest: a.ManifestDigest, Revision: result.Active.Revision}, now.Add(2*time.Second)); err == nil {
