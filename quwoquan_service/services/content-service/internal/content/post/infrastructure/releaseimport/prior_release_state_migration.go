...
const (
	ContentPriorReleaseStateMigrationReceiptSchema = "quwoquan.content_prior_release_state_migration_receipt"
	PriorReleaseStateIndexSetV1                    = "content_release_state_prior_v1"
	PriorReleaseStageReceiptIndexSetV1             = "content_release_stage_receipt_prior_v1"
	...
	priorReleaseStateCandidateIndexName  = "uq_data_release_state_environment_candidate"
	priorReleaseStateActiveIndexName     = "idx_data_release_state_active_pointer"
...
var allowedPriorReleaseStateFields = map[string]struct{}{
...
var allowedPriorReleaseStageReceiptFields = map[string]struct{}{
	"_id": {}, "environment": {}, "sourceOwner": {}, "releaseId": {},
...
// PriorReleaseStateMigrationExpectation binds the migration to every mutable
// identity field of the one expected active pointer. PriorIndexSet must name
// the exact historical index definitions; it is not an open-ended version.
type PriorReleaseStateMigrationExpectation struct {
	Environment          string
	SourceOwner          string
	ReleaseID            string
	...
	PriorIndexSet        string
	PriorReceiptIndexSet string
	ExpectedReceiptCount int64
	AllowReplay          bool
}

// PriorReleaseStateMigrationResult is safe to serialize: it intentionally
// contains no Mongo URI, document body, readback payload, or collection data.
type PriorReleaseStateMigrationResult struct {
	Status                      string
	Environment                 string
	SourceOwner                 string
	ReleaseID                   string
...
type priorReleaseStateDocument struct {
	ID                any       `bson:"_id"`
...
type priorReleaseStageReceiptDocument struct {
	ID                any       `bson:"_id"`
	Environment       string    `bson:"environment"`
	SourceOwner       string    `bson:"sourceOwner"`
	ReleaseID         string    `bson:"releaseId"`
...
func priorReleaseStateIndexDefinitions() []releaseStateIndexDefinition {
	return []releaseStateIndexDefinition{
		{
			Name: priorReleaseStateActiveIndexName,
			Keys: bson.D{{Key: "environment", Value: int32(1)}, {Key: "status", Value: int32(1)}, {Key: "activatedAt", Value: int32(-1)}},
		},
		{
			Name:   priorReleaseStateCandidateIndexName,
...
func priorReleaseStageReceiptIndexDefinitions() []releaseStateIndexDefinition {
	return []releaseStateIndexDefinition{
		{
...
// PriorReleaseStateExpectedIndexDigest returns the stable digest operators can
// use to independently bind an approval to the hard-coded prior definition.
func PriorReleaseStateExpectedIndexDigest() string {
	return releaseStateIndexSetDigest(priorReleaseStateIndexDefinitions())
}
...
// PriorReleaseStageReceiptExpectedIndexDigest binds the migration to the two
// exact historical receipt index definitions.
func PriorReleaseStageReceiptExpectedIndexDigest() string {
	return releaseStateIndexSetDigest(priorReleaseStageReceiptIndexDefinitions())
}
...
func validatePriorReleaseStateMigrationExpectation(expectation PriorReleaseStateMigrationExpectation) (PriorReleaseStateMigrationExpectation, error) {
	...
	expectation.PriorIndexSet = strings.TrimSpace(expectation.PriorIndexSet)
	expectation.PriorReceiptIndexSet = strings.TrimSpace(expectation.PriorReceiptIndexSet)
	...
	if !expectation.ActivatedAt.Equal(expectation.ActivatedAt.Truncate(time.Millisecond)) {
		return PriorReleaseStateMigrationExpectation{}, fmt.Errorf("prior release-state expected activatedAt must have MongoDB millisecond precision")
	}
	if expectation.PriorIndexSet != PriorReleaseStateIndexSetV1 {
		return PriorReleaseStateMigrationExpectation{}, fmt.Errorf("prior release-state index expectation must be %q", PriorReleaseStateIndexSetV1)
	}
	if expectation.PriorReceiptIndexSet != PriorReleaseStageReceiptIndexSetV1 {
		return PriorReleaseStateMigrationExpectation{}, fmt.Errorf("prior release-stage receipt index expectation must be %q", PriorReleaseStageReceiptIndexSetV1)
	}
	if expectation.ExpectedReceiptCount < 0 {
		return PriorReleaseStateMigrationExpectation{}, fmt.Errorf("prior release-stage receipt expected row count must be non-negative")
	}
	return expectation, nil
}

// MigratePriorContentReleaseState converts the only data_release_state row and
...
func MigratePriorContentReleaseState(
	ctx context.Context,
	database *mongo.Database,
	expectation PriorReleaseStateMigrationExpectation,
) (PriorReleaseStateMigrationResult, error) {
	if ctx == nil || database == nil {
		return PriorReleaseStateMigrationResult{}, fmt.Errorf("prior release-state migration context and database are required")
	}
	expectation, err := validatePriorReleaseStateMigrationExpectation(expectation)
	if err != nil {
		return PriorReleaseStateMigrationResult{}, err
	}
	state := database.Collection("data_release_state")
	receipts := database.Collection("data_release_stage_receipts")
	if err := requireEmptyPriorReleaseCandidateCollections(ctx, database); err != nil {
		return PriorReleaseStateMigrationResult{}, err
	}
	_, document, stateCurrent, err := inspectOnlyPriorReleaseStateDocument(ctx, state, expectation)
	if err != nil {
		return PriorReleaseStateMigrationResult{}, err
	}
	stateIndexes, err := inspectPriorReleaseStateIndexes(ctx, state)
	if err != nil {
		return PriorReleaseStateMigrationResult{}, err
	}
	beforeStateDigest := releaseStateIndexSetDigest(stateIndexes.Definitions)
	if !stateCurrent {
		if !isExactPriorReleaseStateIndexSet(stateIndexes) {
			return PriorReleaseStateMigrationResult{}, fmt.Errorf("GATE_BLOCK: prior data_release_state indexes differ from %s (expected digest %s, actual %s)", PriorReleaseStateIndexSetV1, PriorReleaseStateExpectedIndexDigest(), beforeStateDigest)
		}
	} else {
		if !expectation.AllowReplay {
			return PriorReleaseStateMigrationResult{}, fmt.Errorf("GATE_BLOCK: current or partially migrated release state requires explicit --allow-replay")
		}
		if !isRecoverableCurrentReleaseStateIndexSet(stateIndexes) {
			return PriorReleaseStateMigrationResult{}, fmt.Errorf("GATE_BLOCK: current release-state index set is not a recognized recoverable migration phase")
		}
	}

	receiptIndexes, err := inspectPriorReleaseStageReceiptIndexes(ctx, receipts)
	if err != nil {
		return PriorReleaseStateMigrationResult{}, err
	}
	beforeReceiptDigest := releaseStateIndexSetDigest(receiptIndexes.Definitions)
	receiptDocuments, beforeReceiptRows, receiptsCurrent, err := inspectPriorReleaseStageReceiptDocuments(ctx, receipts, expectation)
	if err != nil {
		return PriorReleaseStateMigrationResult{}, err
	}
	if int64(len(receiptDocuments)) != expectation.ExpectedReceiptCount {
		return PriorReleaseStateMigrationResult{}, fmt.Errorf("GATE_BLOCK: release-stage receipt row count differs from exact expectation: expected %d, found %d", expectation.ExpectedReceiptCount, len(receiptDocuments))
	}
	receiptsMixed := false
	if !receiptsCurrent {
	...
	if len(receiptDocuments) == 0 {
		receiptsCurrent = !isExactPriorReleaseStageReceiptIndexSet(receiptIndexes)
	}
	if !stateCurrent {
		if receiptsCurrent || receiptsMixed {
			return PriorReleaseStateMigrationResult{}, fmt.Errorf("GATE_BLOCK: current or mixed release-stage receipts with prior release state are not a recognized migration phase")
		}
		if !isExactPriorReleaseStageReceiptIndexSet(receiptIndexes) {
			return PriorReleaseStateMigrationResult{}, fmt.Errorf("GATE_BLOCK: prior data_release_stage_receipts indexes differ from %s (expected digest %s, actual %s)", PriorReleaseStageReceiptIndexSetV1, PriorReleaseStageReceiptExpectedIndexDigest(), beforeReceiptDigest)
		}
	} else if !isExactPriorReleaseStageReceiptIndexSet(receiptIndexes) &&
		!isRecoverableCurrentReleaseStageReceiptIndexSet(receiptIndexes) {
		return PriorReleaseStateMigrationResult{}, fmt.Errorf("GATE_BLOCK: current release-stage receipt index set is not a recognized recoverable migration phase")
	}
	...
	stateChanged, receiptsChanged, err := migratePriorReleaseControlDocuments(
	...
	if err := migratePriorReleaseStateIndexes(ctx, state, &stateIndexes, &steps); err != nil {
		return PriorReleaseStateMigrationResult{}, err
	}
	if err := migratePriorReleaseStageReceiptIndexes(ctx, receipts, &receiptIndexes, &steps); err != nil {
		return PriorReleaseStateMigrationResult{}, err
	}
	...
	_, readback, readbackCurrent, err := inspectOnlyPriorReleaseStateDocument(ctx, state, expectation)
	if err != nil {
		return PriorReleaseStateMigrationResult{}, fmt.Errorf("revalidate migrated release-state document: %w", err)
	}
	if !readbackCurrent || readback.Kind != releaseActivePointerKind || readback.Revision != 1 {
		return PriorReleaseStateMigrationResult{}, fmt.Errorf("GATE_BLOCK: migrated release-state readback is not current")
	}
	finalStateIndexes, err := inspectPriorReleaseStateIndexes(ctx, state)
	if err != nil {
		return PriorReleaseStateMigrationResult{}, fmt.Errorf("revalidate migrated release-state indexes: %w", err)
	}
	if !isExactCurrentReleaseStateIndexSet(finalStateIndexes) {
		return PriorReleaseStateMigrationResult{}, fmt.Errorf("GATE_BLOCK: migrated release-state indexes failed exact readback")
	}
	readbackReceipts, afterReceiptRows, readbackReceiptsCurrent, err := inspectPriorReleaseStageReceiptDocuments(ctx, receipts, expectation)
	if err != nil {
		return PriorReleaseStateMigrationResult{}, fmt.Errorf("revalidate migrated release-stage receipt documents: %w", err)
	}
	if !readbackReceiptsCurrent || len(readbackReceipts) != len(receiptDocuments) {
		return PriorReleaseStateMigrationResult{}, fmt.Errorf("GATE_BLOCK: migrated release-stage receipt readback is not current")
	}
	...
	if releaseStageReceiptRowSetDigest(afterReceiptRows, expectation.SourceOwner) != beforeReceiptPreservationDigest {
		return PriorReleaseStateMigrationResult{}, fmt.Errorf("GATE_BLOCK: migrated release-stage receipt non-owner row-set digest changed")
	}
	finalReceiptIndexes, err := inspectPriorReleaseStageReceiptIndexes(ctx, receipts)
	if err != nil {
		return PriorReleaseStateMigrationResult{}, fmt.Errorf("revalidate migrated release-stage receipt indexes: %w", err)
	}
	if !isExactCurrentReleaseStageReceiptIndexSet(finalReceiptIndexes) {
		return PriorReleaseStateMigrationResult{}, fmt.Errorf("GATE_BLOCK: migrated release-stage receipt indexes failed exact readback")
	}
...
func inspectPriorReleaseStageReceiptDocuments(
	ctx context.Context,
	receipts *mongo.Collection,
	expectation PriorReleaseStateMigrationExpectation,
) ([]priorReleaseStageReceiptDocument, []bson.Raw, bool, error) {
	...
	documents := make([]priorReleaseStageReceiptDocument, 0, len(rawDocuments))
	priorCount := 0
	for _, raw := range rawDocuments {
		elements, err := raw.Elements()
		if err != nil {
			return nil, nil, false, fmt.Errorf("decode prior release-stage receipt fields: %w", err)
		}
		for _, element := range elements {
			if _, allowed := allowedPriorReleaseStageReceiptFields[element.Key()]; !allowed {
				return nil, nil, false, fmt.Errorf("GATE_BLOCK: release-stage receipt contains unexpected field %q", element.Key())
			}
		}
		if err := validatePriorReleaseStageReceiptRawTypes(raw); err != nil {
			return nil, nil, false, err
		}
		var document priorReleaseStageReceiptDocument
		if err := bson.Unmarshal(raw, &document); err != nil {
			return nil, nil, false, fmt.Errorf("decode prior release-stage receipt: %w", err)
		}
		...
		if err := validatePriorReleaseStageReceiptDocument(document); err != nil {
			return nil, nil, false, err
		}
	...
	byAttempt := make(map[string][]priorReleaseStageReceiptDocument)
	for _, document := range documents {
	...
	return documents, rawDocuments, priorCount == 0, nil
}

func validatePriorReleaseStageReceiptRawTypes(raw bson.Raw) error {
...
func validatePriorReleaseStageReceiptDocument(document priorReleaseStageReceiptDocument) error {
...
func migratePriorReleaseControlDocuments(
	ctx context.Context,
	state *mongo.Collection,
	receipts *mongo.Collection,
	expectation PriorReleaseStateMigrationExpectation,
	stateDocument priorReleaseStateDocument,
	receiptDocuments []priorReleaseStageReceiptDocument,
	stateCurrent bool,
	...
	if err != nil {
		return false, false, fmt.Errorf("start prior release control CAS transaction: %w", err)
	}
	...
	_, err = session.WithTransaction(ctx, func(txCtx context.Context) (any, error) {
		if !stateCurrent {
			...
			if result.MatchedCount != 1 || result.ModifiedCount != 1 {
				return nil, fmt.Errorf("GATE_BLOCK: prior Content release-state expected current CAS conflict")
			}
		}
		if !receiptsCurrent {
			if err := migratePriorReleaseStageReceiptDocuments(txCtx, receipts, expectation, receiptDocuments); err != nil {
				return nil, err
			}
		}
		return nil, nil
...
func migratePriorReleaseStageReceiptDocuments(
	ctx context.Context,
	receipts *mongo.Collection,
	expectation PriorReleaseStateMigrationExpectation,
	documents []priorReleaseStageReceiptDocument,
) error {
	...
	if err != nil {
		return fmt.Errorf("bulk CAS prior release-stage receipt owner: %w", err)
	}
	if result.MatchedCount != int64(len(models)) || result.ModifiedCount != int64(len(models)) {
		return fmt.Errorf("GATE_BLOCK: prior release-stage receipt owner CAS conflict")
	}
	return nil
}

func migratePriorReleaseStateIndexes(
	ctx context.Context,
	state *mongo.Collection,
	...
	if indexes.PriorActive {
		if err := state.Indexes().DropOne(ctx, priorReleaseStateActiveIndexName); err != nil {
			return fmt.Errorf("drop prior active-pointer index (replay with --allow-replay is safe): %w", err)
		}
		indexes.PriorActive = false
		*steps = append(*steps, "drop-prior-active-pointer-index")
	}
	if indexes.CandidateKind == "prior" {
		if err := state.Indexes().DropOne(ctx, priorReleaseStateCandidateIndexName); err != nil {
			return fmt.Errorf("drop prior candidate index (replay with --allow-replay is safe): %w", err)
		}
		indexes.CandidateKind = ""
		*steps = append(*steps, "drop-prior-candidate-index")
	}
...
func migratePriorReleaseStageReceiptIndexes(
	ctx context.Context,
	receipts *mongo.Collection,
	...
	for _, phase := range phases {
		...
		if *phase.kind == "both" {
			if err := receipts.Indexes().DropOne(ctx, phase.canonicalName); err != nil {
				return fmt.Errorf("drop prior release-stage receipt index %s (replay with --allow-replay is safe): %w", phase.canonicalName, err)
			}
			*phase.kind = "temporary"
			*steps = append(*steps, "drop-prior-"+phase.canonicalName)
		}
...
func inspectOnlyPriorReleaseStateDocument(
	ctx context.Context,
	state *mongo.Collection,
	expectation PriorReleaseStateMigrationExpectation,
) (bson.Raw, priorReleaseStateDocument, bool, error) {
	cursor, err := state.Find(ctx, bson.D{}, options.Find().SetLimit(2))
	if err != nil {
		return nil, priorReleaseStateDocument{}, false, fmt.Errorf("inspect data_release_state: %w", err)
	}
	defer cursor.Close(ctx)
	var documents []bson.Raw
	if err := cursor.All(ctx, &documents); err != nil {
		return nil, priorReleaseStateDocument{}, false, fmt.Errorf("decode data_release_state: %w", err)
	}
	if len(documents) != 1 {
		return nil, priorReleaseStateDocument{}, false, fmt.Errorf("GATE_BLOCK: migration requires exactly one data_release_state document; found %d", len(documents))
	}
	raw := documents[0]
	elements, err := raw.Elements()
	if err != nil {
		return nil, priorReleaseStateDocument{}, false, fmt.Errorf("decode prior release-state fields: %w", err)
	}
	for _, element := range elements {
		if _, allowed := allowedPriorReleaseStateFields[element.Key()]; !allowed {
			return nil, priorReleaseStateDocument{}, false, fmt.Errorf("GATE_BLOCK: prior release-state contains unexpected field %q", element.Key())
		}
	}
	var document priorReleaseStateDocument
	if err := bson.Unmarshal(raw, &document); err != nil {
		return nil, priorReleaseStateDocument{}, false, fmt.Errorf("decode prior release-state document: %w", err)
	}
...
func requireEmptyPriorReleaseCandidateCollections(ctx context.Context, database *mongo.Database) error {
...
func inspectPriorReleaseStateIndexes(ctx context.Context, state *mongo.Collection) (inspectedReleaseStateIndexes, error) {
	...
	prior := priorReleaseStateIndexDefinitions()
	...
	for _, raw := range rawIndexes {
		...
		switch index.Name {
		case priorReleaseStateCandidateIndexName:
			switch {
			case releaseStateIndexDefinitionEqual(definition, prior[1]):
				inspected.CandidateKind = "prior"
		...
		case priorReleaseStateActiveIndexName:
			if !releaseStateIndexDefinitionEqual(definition, prior[0]) {
				return inspectedReleaseStateIndexes{}, fmt.Errorf("GATE_BLOCK: prior active release-state index definition drifted")
			}
			inspected.PriorActive = true
...
func inspectPriorReleaseStageReceiptIndexes(
	ctx context.Context,
	receipts *mongo.Collection,
	...
	prior := priorReleaseStageReceiptIndexDefinitions()
	...
	for _, raw := range rawIndexes {
		...
		case releaseStageReceiptAttemptIndexName:
			switch {
			case releaseStateIndexDefinitionEqual(definition, prior[0]):
				inspected.AttemptKind = "prior"
	...
	inspected.AttemptKind = normalizeReleaseStageReceiptIndexPhase(inspected.AttemptKind, inspected.Definitions, prior[0], current[0])
	inspected.TimelineKind = normalizeReleaseStageReceiptIndexPhase(inspected.TimelineKind, inspected.Definitions, prior[1], current[1])
	return inspected, nil
}
...
func normalizeReleaseStageReceiptIndexPhase(
	kind string,
	definitions []releaseStateIndexDefinition,
	prior releaseStateIndexDefinition,
	...
	for _, definition := range definitions {
		hasPrior = hasPrior || releaseStateIndexDefinitionEqual(definition, prior)
	...
	switch {
	case hasPrior && !hasCurrent && hasTemporary:
		return "both"
	case !hasPrior && !hasCurrent && hasTemporary:
		return "temporary"
	case !hasPrior && !hasCurrent && !hasTemporary:
...
func isExactPriorReleaseStageReceiptIndexSet(indexes inspectedReleaseStageReceiptIndexes) bool {
	return indexes.AttemptKind == "prior" && indexes.TimelineKind == "prior" && len(indexes.Definitions) == 2
}
...
func isRecoverableCurrentReleaseStageReceiptIndexSet(indexes inspectedReleaseStageReceiptIndexes) bool {
	phase := map[string]int{"prior": 0, "both": 1, "temporary": 2, "absent": 3, "current-and-temporary": 4, "current": 5}
...
func isExactPriorReleaseStateIndexSet(indexes inspectedReleaseStateIndexes) bool {
	return indexes.CandidateKind == "prior" && indexes.PriorActive && !indexes.CurrentActive && len(indexes.Definitions) == 2
}

func isExactCurrentReleaseStateIndexSet(indexes inspectedReleaseStateIndexes) bool {
	return indexes.CandidateKind == "current" && !indexes.PriorActive && indexes.CurrentActive && len(indexes.Definitions) == 2
}
...
func isRecoverableCurrentReleaseStateIndexSet(indexes inspectedReleaseStateIndexes) bool {
	...
	if indexes.CandidateKind == "prior" {
		return (indexes.PriorActive && !indexes.CurrentActive && len(indexes.Definitions) == 2) ||
			(indexes.PriorActive && indexes.CurrentActive && len(indexes.Definitions) == 3) ||
			(!indexes.PriorActive && indexes.CurrentActive && len(indexes.Definitions) == 2)
	}
	return indexes.CandidateKind == "" && !indexes.PriorActive && indexes.CurrentActive && len(indexes.Definitions) == 1
}
...
func currentCandidateReleaseStateIndexModel() mongo.IndexModel {
	return mongo.IndexModel{
		...
		Options: options.Index().SetName(priorReleaseStateCandidateIndexName).SetUnique(true).
			SetPartialFilterExpression(bson.D{{Key: "kind", Value: releaseCandidateKind}}),
	}
}

type PriorReleaseStateMigrationCommand struct {
	mongoURI   string
	Database   string
	ReportPath string
	Expected   PriorReleaseStateMigrationExpectation
}

func ParsePriorReleaseStateMigrationCommand(args []string) (PriorReleaseStateMigrationCommand, error) {
	set := flag.NewFlagSet("migrate-content-release-state", flag.ContinueOnError)
	set.SetOutput(io.Discard)
	var command PriorReleaseStateMigrationCommand
	...
	set.StringVar(&command.Expected.PriorReceiptIndexSet, "expected-prior-receipt-index-set", "", "exact prior release-stage receipt index expectation")
	...
	if err := set.Parse(args); err != nil {
		return PriorReleaseStateMigrationCommand{}, fmt.Errorf("parse prior release-state migration flags: %w", err)
	}
	if set.NArg() != 0 {
		return PriorReleaseStateMigrationCommand{}, fmt.Errorf("migrate-content-release-state does not accept positional arguments")
	}
	...
	if command.mongoURI == "" || command.Database == "" || command.ReportPath == "" || strings.TrimSpace(activatedAt) == "" {
		return PriorReleaseStateMigrationCommand{}, fmt.Errorf("--mongo-uri, --database, --expected-activated-at, and --report must be non-empty")
	}
	parsedAt, err := time.Parse(time.RFC3339Nano, strings.TrimSpace(activatedAt))
	if err != nil {
		return PriorReleaseStateMigrationCommand{}, fmt.Errorf("--expected-activated-at must be RFC3339: %w", err)
	}
	command.Expected.ActivatedAt = parsedAt.UTC()
	command.Expected, err = validatePriorReleaseStateMigrationExpectation(command.Expected)
	if err != nil {
		return PriorReleaseStateMigrationCommand{}, err
	}
	return command, nil
}

type ContentPriorReleaseStateMigrationReceipt struct {
	...
	PriorIndexSet               string    `json:"priorIndexSet"`
	PriorIndexSetDigest         string    `json:"priorIndexSetDigest"`
	BeforeIndexSetDigest        string    `json:"beforeIndexSetDigest"`
	AfterIndexSetDigest         string    `json:"afterIndexSetDigest"`
	PriorReceiptIndexSet        string    `json:"priorReceiptIndexSet"`
	PriorReceiptIndexSetDigest  string    `json:"priorReceiptIndexSetDigest"`
...
func RunPriorReleaseStateMigration(ctx context.Context, args []string) error {
	if ctx == nil {
		return fmt.Errorf("prior release-state migration context is required")
	}
	command, err := ParsePriorReleaseStateMigrationCommand(args)
	if err != nil {
		return err
	}
	if mode := strings.TrimSpace(os.Getenv("QWQ_STORAGE_MIGRATION_MODE")); mode != QuiescedAtomicStorageMigrationMode {
		return fmt.Errorf("prior release-state migration requires QWQ_STORAGE_MIGRATION_MODE=%s", QuiescedAtomicStorageMigrationMode)
	}
	if confirmation := strings.TrimSpace(os.Getenv("QWQ_CONTENT_RELEASE_STATE_QUIESCED")); confirmation != ContentReleaseStateQuiescedConfirmation {
		return fmt.Errorf("prior release-state migration requires QWQ_CONTENT_RELEASE_STATE_QUIESCED=%s", ContentReleaseStateQuiescedConfirmation)
	}
	resolvedReport, err := validatePriorReleaseStateMigrationReportDestination(command.ReportPath)
	if err != nil {
		return err
	}
	client, err := mongo.Connect(options.Client().ApplyURI(command.mongoURI))
	if err != nil {
		return redactedPriorMigrationMongoError("connect Content MongoDB", command.mongoURI, err)
	}
	defer client.Disconnect(context.Background())
	if err := client.Ping(ctx, nil); err != nil {
		return redactedPriorMigrationMongoError("ping Content MongoDB", command.mongoURI, err)
	}
	result, err := MigratePriorContentReleaseState(ctx, client.Database(command.Database), command.Expected)
	if err != nil {
		return redactedPriorMigrationMongoError("migrate Content prior release state", command.mongoURI, err)
	}
	receipt := ContentPriorReleaseStateMigrationReceipt{
		...
		PriorReceiptIndexSet:        command.Expected.PriorReceiptIndexSet,
		PriorReceiptIndexSetDigest:  PriorReleaseStageReceiptExpectedIndexDigest(),
	...
	return writePriorReleaseStateMigrationReceipt(resolvedReport, receipt)
}

func validatePriorReleaseStateMigrationReportDestination(path string) (string, error) {
	resolved := filepath.Clean(strings.TrimSpace(path))
	if resolved == "." || resolved == string(filepath.Separator) {
		return "", fmt.Errorf("prior release-state migration report path is invalid")
	}
	if _, err := os.Lstat(resolved); err == nil {
		return "", fmt.Errorf("prior release-state migration report already exists")
	} else if !errors.Is(err, os.ErrNotExist) {
		return "", fmt.Errorf("inspect prior release-state migration report destination: %w", err)
	}
	parentInfo, err := os.Lstat(filepath.Dir(resolved))
	if err != nil {
		return "", fmt.Errorf("inspect prior release-state migration report directory: %w", err)
	}
	if !parentInfo.IsDir() || parentInfo.Mode()&os.ModeSymlink != 0 {
		return "", fmt.Errorf("prior release-state migration report directory must be a non-symlink directory")
	}
	return resolved, nil
}

func writePriorReleaseStateMigrationReceipt(path string, receipt ContentPriorReleaseStateMigrationReceipt) error {
	encoded, err := json.MarshalIndent(receipt, "", "  ")
	if err != nil {
		return fmt.Errorf("encode prior release-state migration receipt: %w", err)
	}
	encoded = append(encoded, '\n')
	file, err := os.OpenFile(path, os.O_WRONLY|os.O_CREATE|os.O_EXCL, 0o600)
	if err != nil {
		return fmt.Errorf("create prior release-state migration receipt: %w", err)
	}
	remove := true
	defer func() {
		_ = file.Close()
	...
	if _, err := file.Write(encoded); err != nil {
		return fmt.Errorf("write prior release-state migration receipt: %w", err)
	}
	if err := file.Sync(); err != nil {
		return fmt.Errorf("sync prior release-state migration receipt: %w", err)
	}
	if err := file.Close(); err != nil {
		return fmt.Errorf("close prior release-state migration receipt: %w", err)
	}
	remove = false
	return nil
}

func redactedPriorMigrationMongoError(prefix, mongoURI string, err error) error {
	message := err.Error()
	for _, secret := range priorMigrationURISecrets(mongoURI) {
		message = strings.ReplaceAll(message, secret, "<redacted>")
	}
	return fmt.Errorf("%s: %s", prefix, message)
}

func priorMigrationURISecrets(mongoURI string) []string {
