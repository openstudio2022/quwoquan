...
func TestFenceJointContentHarness(t *testing.T) {
	...
	mux.HandleFunc("/fixture/activate", func(w http.ResponseWriter, r *http.Request) {
		...
		result, e := activateImportedPostReleaseTransaction(ctx, db.Database, "alpha", target, expected, time.Now().UTC())
		if e != nil {
			http.Error(w, e.Error(), 500)
			return
		}
		eventID := public.ReleaseFenceEventID(releaseWireFence(result.Active))
		var record importedOutboxDocument
