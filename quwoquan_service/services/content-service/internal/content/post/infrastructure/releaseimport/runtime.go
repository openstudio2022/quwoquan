...
type ImportOptions struct {
	...
	// ReleaseClass 是 release.json 声明的唯一 production 类别，
	// 随导入落进 data_release_state，用于验证同一不可变发布绑定。
	ReleaseClass              string
	ReleaseKind               string
	Mode                      string
	DeletePolicy              string
...
func ImportedPostBindings(posts []PostDoc) ([]ImportedPostBinding, error) {
	...
	for _, post := range posts {
		...
		if storagePostRef == "" || postID == "" || contentType == "" || authorID == "" ||
			...
			post.Admission.UsageScope != "production" {
...
func ImportPoolCounts(posts []PostDoc, entitiesLoaded int) bson.M {
	...
	for _, post := range posts {
		...
		if post.Admission.UsageScope == "production" {
			counts["productionLoaded"] = counts["productionLoaded"].(int) + 1
		}
	}
	return counts
}
