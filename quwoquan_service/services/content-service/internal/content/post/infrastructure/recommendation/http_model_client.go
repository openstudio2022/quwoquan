...
const recommendationScoringOperationID = "recommendation.recommendation_model_release.ScoreRecommendationCandidates"
...
// SetTransport injects the HTTP transport while preserving timeout, auth,
// retry and circuit-breaker policy.
func (c *HTTPModelServiceClient) SetTransport(transport http.RoundTripper) {
	if transport == nil {
...
// Predict sends the request to recommendation-service /score and returns the response.
