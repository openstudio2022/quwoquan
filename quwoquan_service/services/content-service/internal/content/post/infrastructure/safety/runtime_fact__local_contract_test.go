...
func TestPythonAuthorizationExactBytesVerifyInGo(t *testing.T) {
	...
	pae := []byte("DSSEv1 " + strconv.Itoa(len(runtimeAuthorizationPayloadType)) + " " + runtimeAuthorizationPayloadType + " " + strconv.Itoa(len(canonicalPayload)) + " ")
	...
	if err = strictCanonicalRuntimeAuthorizationJSON(raw, &authorization); err != nil {
		t.Fatal("Python fixed-microsecond canonical bytes rejected", err)
	}
	if err = verifyRuntimeAuthorizationWithKey(raw, authorization, "fixture-authority", base64.StdEncoding.EncodeToString(publicKey)); err != nil {
		t.Fatal("Python canonical signature rejected", err)
	}
	if err = strictCanonicalRuntimeJSON(raw, &authorization); err == nil {
		t.Fatal("fixture no longer exercises Go trailing-zero timestamp remarshal mismatch")
	}
}
...
func TestRetainedGammaAuthorizationExactBytesVerifyInGo(t *testing.T) {
	...
	if err = verifyRuntimeAuthorization(raw, authorization); err != nil {
...
func TestRuntimeDTORejectsMissingAndDuplicateFields(t *testing.T) {
	...
	for _, bad := range [][]byte{missing, []byte(`{"mode":"new_runtime","mode":"restored"}`), append(raw, []byte(` {}`)...)} {
		if err := StrictRuntimeJSON(bad, &target); err == nil {
	...
	if err := StrictRuntimeJSON(unknown, &target); err == nil {
		t.Fatal("unknown accepted")
	}
	object["binding"] = json.RawMessage(`null`)
	delete(object, "unknown")
	null, _ := json.Marshal(object)
	if err := StrictRuntimeJSON(null, &target); err == nil {
...
func TestRuntimePhysicalUUIDChangesOnRecreation(t *testing.T) {
	...
	db := runtime.Database
	if _, err = ReadPhysicalSafetyIdentity(ctx, db); err == nil {
		t.Fatal("missing collection treated as created")
	}
	if err = db.CreateCollection(ctx, Collection); err != nil {
		t.Fatal(err)
	}
	first, err := ReadPhysicalSafetyIdentity(ctx, db)
	if err != nil {
		t.Fatal(err)
	}
	if _, err = db.Collection(Collection).InsertOne(ctx, bson.M{"objectDigest": "test"}); err != nil {
		t.Fatal(err)
	}
	afterWrite, err := ReadPhysicalSafetyIdentity(ctx, db)
	...
	second, err := ReadPhysicalSafetyIdentity(ctx, db)
...
func TestRuntimeFilesRejectUnsafeMaterials(t *testing.T) {
	...
	if _, err = files.ReadDigest("secrets/post-safety.key", runtimeBytesDigest(raw)); err != nil {
		t.Fatal(err)
	}
...
func TestProductionRuntimeAuthorityBlocksListenerOnMaterialDrift(t *testing.T) {
	...
	if err = runtime.Database.CreateCollection(ctx, Collection); err != nil {
		t.Fatal(err)
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.mutate(caseRoot)
			_, key, loadErr := LoadRuntimeAuthority(ctx, runtime.Database, "gamma", caseRoot, "current.json", "fact.json", "secrets/post-safety.key", active)
			if tc.name == "valid" {
				if loadErr != nil {
	...
	authority, key, err := LoadRuntimeAuthority(ctx, runtime.Database, "gamma", root, "current.json", "fact.json", "secrets/post-safety.key", active)
	if err != nil {
		t.Fatal(err)
	}
	manager, err := New(runtime.Database, "gamma", key, authority)
	clear(key)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = runtime.Database.Collection(Collection).Drop(ctx); err != nil {
		t.Fatal(err)
	}
	if err = runtime.Database.CreateCollection(ctx, Collection); err != nil {
		t.Fatal(err)
	}
	if _, key, err := LoadRuntimeAuthority(ctx, runtime.Database, "gamma", root, "current.json", "fact.json", "secrets/post-safety.key", active); err == nil {
		clear(key)
		t.Fatal("recreated collection retained listener authority")
	}
	if _, key, err := LoadRuntimeAuthority(ctx, runtime.Database, "prod", root, "current.json", "fact.json", "secrets/post-safety.key", active); err == nil {
		clear(key)
...
func TestDataRuntimeAuthorityAcceptsNoAccountReaderButContentRemainsClosed(t *testing.T) {
	...
	authority, key, err := LoadRuntimeAuthority(ctx, runtime.Database, "gamma", root, "current.json", "fact.json", "secrets/post-safety.key", nil)
	if err != nil {
...
func TestDataRuntimeAuthorityInitializesOnlyWithCurrentRecoveryMaterials(t *testing.T) {
	...
	root, _ := writeSignedRuntimeFixture(t, ctx, runtime.Database, "gamma")
	newManager := func(t *testing.T, fixtureRoot, environment string) (*Manager, *RuntimeAuthority) {
		t.Helper()
		authority, key, loadErr := LoadRuntimeAuthority(ctx, runtime.Database, "gamma", fixtureRoot, "current.json", "fact.json", "secrets/post-safety.key", activeAccountAuthority{})
		if loadErr != nil {
			t.Fatal(loadErr)
		}
		manager, newErr := New(runtime.Database, environment, key, authority)
		clear(key)
		if newErr != nil {
			t.Fatal(newErr)
		}
	...
	runInitialize := func(manager *Manager, owner, sourceID string) error {
...
func writeSignedRuntimeFixture(t *testing.T, ctx context.Context, db *mongo.Database, environment string) (string, wire.PostSafetyDeploymentStartupMaterial) {
	...
	write := func(name string, raw []byte) wire.PostSafetyRuntimeEvidence {
		...
		return wire.PostSafetyRuntimeEvidence{Ref: name, Digest: runtimeBytesDigest(raw)}
	}
	...
	keyID, err := RuntimeKeyIdentity(key)
	if err != nil {
		t.Fatal(err)
	}
