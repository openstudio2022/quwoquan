...
func (cleaner *RedisPersonalDataCacheCleaner) VerifyNoPersonalDataResidual(
	ctx context.Context,
	subjectIDs []string,
	keys []string,
) error {
	if cleaner == nil ||
		...
		cleaner.subjectDigestor == nil {
		return errors.New(
			"UserAccountClosed Redis residual verifier is not configured",
		)
	}
	for _, subjectID := range uniqueStrings(subjectIDs) {
		closed, err := cleaner.IsSubjectClosed(ctx, subjectID)
		if err != nil {
			return err
		}
		if !closed {
			return errors.New(
				"closed-account Redis subject tombstone is missing",
			)
		}
		open, err := cleaner.IsSubjectKnownOpen(ctx, subjectID)
		if err != nil {
			return err
		}
		if open {
			return errors.New(
				"closed account remains in the open-subject Redis cache",
			)
		}
	}
	for _, key := range uniqueStrings(keys) {
		...
		if err != nil {
			return fmt.Errorf(
				"verify closed-account Redis cache deletion: %w",
				err,
			)
		}
		return errors.New(
			"closed-account personal Redis cache key still exists",
		)
	}
	return nil
}
