...
func collectActivityClosureRows(
	...
	orFilters := bson.A{
		...
		bson.M{"actorSubAccountId": bson.M{"$in": subjectIDs}},
		bson.M{"counterpartSubAccountId": bson.M{"$in": subjectIDs}},
		bson.M{"targetSubAccountId": bson.M{"$in": subjectIDs}},
		bson.M{"displaySubAccountId": bson.M{"$in": subjectIDs}},
	}
	if len(postIDs) > 0 {
		orFilters = append(
			orFilters,
