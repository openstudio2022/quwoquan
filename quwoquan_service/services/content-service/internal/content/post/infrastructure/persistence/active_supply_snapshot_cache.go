...
type activeSupplyCacheKey struct {
	...
	// 激活版本与 revision 参与缓存身份，旧 snapshot 不得跨激活复用。
...
// after the caller has read the current release identity. Only ContentReady
// snapshots are cached; failures and incomplete readbacks invalidate the
// selected key.
