...
// Comment transaction storage is owned by the Comment aggregate.
...
func NewMongoCommentDataAdapter(
	db *mongo.Database,
	mediaReferences referencefence.Fence,
) *MongoCommentDataAdapter {
	if db == nil || mediaReferences == nil {
		panic("MongoCommentDataAdapter requires database and MediaAsset reference fence")
	}
...
func (s *MongoCommentDataAdapter) Commit(
	...
	_, err = session.WithTransaction(ctx, func(txCtx context.Context) (any, error) {
		...
		if record.Status != string(commentmodel.StatusDeleted) &&
			record.Status != string(commentmodel.StatusTombstoned) {
			if err := s.mediaReferences.AllowReferences(
				txCtx,
				commentMediaReferences(record),
			); err != nil {
				if errors.Is(err, referencefence.ErrDeletionInProgress) ||
					errors.Is(err, referencefence.ErrReferenceUnavailable) {
					return nil, commenterrors.AppErrorFromCommentAttachmentNotReady(
						"media asset became unavailable before Comment commit",
					)
				}
				return nil, err
			}
...
func commentMediaReferences(
	record commentAggregateDocument,
) []referencefence.Reference {
	references := make(
		[]referencefence.Reference,
		0,
		len(record.AttachmentMediaIDs),
	)
	for _, assetID := range record.AttachmentMediaIDs {
