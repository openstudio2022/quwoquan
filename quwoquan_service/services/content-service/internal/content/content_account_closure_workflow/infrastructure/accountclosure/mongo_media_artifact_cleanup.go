...
func NewMediaArtifactWorkDocument(
	...
	historicalImageArtifacts := make(
		...
		0,
		len(row.ImageDescriptorRevisions),
	)
	...
	work := mediaprocessing.PlanArtifactCleanup(
		eventID,
		mediaprocessing.ArtifactCleanupSource{
			AssetID:                      row.ID,
			ObjectKey:                    row.ObjectKey,
			ImageNormalizedObjectKey:     row.ImageNormalizedObjectKey,
			ImagePublicSliceKey:          row.ImagePublicSliceKey,
			VideoPublicSliceKey:          row.VideoPublicSliceKey,
			CoverPublicSliceKey:          row.CoverPublicSliceKey,
			PreviewTrackManifestSliceKey: row.PreviewTrackManifestSliceKey,
	...
	return MediaArtifactWorkDocument{
		...
		PublicSliceKeys:   work.PublicSliceKeys,
		PublicPrefixes:    work.PublicPrefixes,
		PrivateObjectKeys: work.PrivateObjectKeys,
		PrivatePrefixes:   work.PrivatePrefixes,
	}
}
