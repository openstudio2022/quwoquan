...
const (
	...
	MediaArtifactWorkCollection      = "user_account_closed_media_artifact_work"
...
type inboxDocument struct {
	...
	State          WorkflowState `bson:"state"`
...
type failureDocument struct {
	...
	EventDigest    string     `bson:"eventDigest"`
	ErrorDigest    string     `bson:"errorDigest"`
	Attempts       int64      `bson:"attempts"`
	LastFailedAt   time.Time  `bson:"lastFailedAt"`
...
func NewMongoStore(
	...
	if db == nil || subjectDigestor == nil || objectFences == nil {
		return nil, errors.New(
			"UserAccountClosed Mongo store requires database, subject digestor, and media object deletion fence",
		)
	}
	return &MongoStore{
		db:                db,
		...
		mediaArtifactWork: db.Collection(MediaArtifactWorkCollection),
...
func (store *MongoStore) EnsureIndexes(ctx context.Context) error {
	...
	if _, err := store.mediaArtifactWork.Indexes().CreateMany(
		ctx,
		[]mongo.IndexModel{
			{
				Keys: bson.D{
				...
				Options: options.Index().
					SetName("idx_user_account_closed_media_artifact_work"),
			},
			{
				Keys: bson.D{{Key: "expireAt", Value: 1}},
				Options: options.Index().
					SetName("idx_user_account_closed_media_artifact_work_expire").
					SetExpireAfterSeconds(0),
			},
		},
	); err != nil {
		return fmt.Errorf(
			"create UserAccountClosed media-artifact-work indexes: %w",
			err,
		)
	}
...
func (store *MongoStore) MarkCompleted(
	...
	pendingMediaArtifacts, err := store.mediaArtifactWork.CountDocuments(
		ctx,
		bson.M{
			"eventId": event.EventID,
			"doneAt":  bson.M{"$exists": false},
	...
	if err != nil {
		return fmt.Errorf(
			"count UserAccountClosed media artifact work: %w",
			err,
		)
	}
	if pendingMediaArtifacts != 0 {
		return errors.New(
			"UserAccountClosed media artifact work is not complete",
		)
	}
	now := time.Now().UTC()
...
func (store *MongoStore) RecordFailure(
	...
	err := store.failures.FindOneAndUpdate(
		ctx,
		bson.M{
			"_id":            failureID(stream, messageID),
...
func (store *MongoStore) IsDeadLettered(
	ctx context.Context,
	stream string,
	messageID string,
) (bool, error) {
...
func (store *MongoStore) MarkDeadLettered(
	ctx context.Context,
	stream string,
	messageID string,
) error {
	...
	result, err := store.failures.UpdateOne(
		...
		bson.M{
			...
			// A terminal marker must outlive the ordinary retry-record TTL.
			// The original source PEL cannot be released or retried without an
			// explicit recovery action, even if it takes longer than routine
	...
	if err != nil {
		return fmt.Errorf(
			"mark UserAccountClosed dead-letter state: %w",
			err,
		)
	}
	if result.MatchedCount != 1 {
		return errors.New("UserAccountClosed dead-letter failure state is missing")
	}
	return nil
}
