...
func importedImageDetail(t *testing.T) postports.PostDetailSlice {
	...
	posts, err := postimport.LoadPosts(root, map[string]bool{"image/图集/测试/1": true}, "production")
	if err != nil {
		t.Fatal(err)
	}
	if len(posts) != 1 {
...
func TestImportedArticleEntityMappingReachesTypedGraphQL(t *testing.T) {
	...
	binding := postimport.ReleaseBinding{ReleaseID: "mentions", SourceOwner: "qwq_data", ManifestDigest: "sha256:" + strings.Repeat("a", 64), ReleaseClass: "production"}
	...
	posts, err := postimport.LoadPosts(root, map[string]bool{strings.TrimPrefix(postPath, "posts/"): true}, "production")
	if err != nil {
		t.Fatal(err)
	}
	...
	document, err := postimport.BuildCanonicalImportedPostDocument(posts[0], time.Now().UTC(), postimport.ImportOptions{ReleaseClass: "production"}, "active")
	if err != nil {
		t.Fatal(err)
	}
