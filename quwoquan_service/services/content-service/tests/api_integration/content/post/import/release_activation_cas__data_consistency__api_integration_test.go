...
func TestMongoReleaseStateRejectsPriorShapeAndEnforcesUniquePointer(t *testing.T) {
	db, cleanup := testDB(t)
	defer cleanup()
	ctx := context.Background()
	...
	if _, err := state.InsertOne(ctx, bson.M{
		...
		"activeReleaseId": "prior",
	}); err != nil {
		t.Fatal(err)
	}
	if _, err := ReadActiveImportedPostRelease(ctx, db, "alpha", "qwq_data"); err == nil || !strings.Contains(err.Error(), "prior") {
		t.Fatalf("prior shape did not fail closed: %v", err)
	}
...
func TestMongoCanonicalControlIndexesReenterAndRejectNamedDrift(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if err == nil || !strings.Contains(err.Error(), ReleasePriorStateMigrationRequiredCode) {
...
func TestMongoVerifiedCandidateQueryRejectsMissingRequiredIndex(t *testing.T) {
	...
	if _, err := ReadVerifiedImportedPostReleaseCandidate(
		ctx, db, "alpha", opts.SourceOwner, opts.ReleaseID, opts.ManifestDigest,
	); err == nil || !strings.Contains(err.Error(), ReleasePriorStateMigrationRequiredCode) {
