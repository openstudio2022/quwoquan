...
func TestMongoPriorReleaseStateWriterFailsClosed(t *testing.T) {
	db, cleanup := testDB(t)
	defer cleanup()
	err := UpsertReleaseState(
		context.Background(), db.Collection("data_release_state"), "alpha",
		ImportOptions{ReleaseID: "prior", ManifestDigest: "sha256:" + strings.Repeat("a", 64)},
		time.Now().UTC(), bson.M{"postsUpserted": 1},
	)
	if err == nil || !strings.Contains(err.Error(), "StageImportedPostRelease") {
		t.Fatalf("prior blind active writer did not fail closed: %v", err)
	}
}
