...
func priorReleaseStateExpectation() PriorReleaseStateMigrationExpectation {
	return PriorReleaseStateMigrationExpectation{
		Environment: "alpha", SourceOwner: "qwq_data", ReleaseID: "release-research-prior",
		...
		PriorIndexSet: PriorReleaseStateIndexSetV1, PriorReceiptIndexSet: PriorReleaseStageReceiptIndexSetV1,
		ExpectedReceiptCount: 5,
	}
}

func seedPriorReleaseState(t *testing.T, db *mongo.Database, expectation PriorReleaseStateMigrationExpectation) bson.M {
	t.Helper()
	...
	if _, err := state.InsertOne(ctx, prior); err != nil {
		t.Fatalf("seed prior release state: %v", err)
	}
	if _, err := state.Indexes().CreateMany(ctx, []mongo.IndexModel{
		...
		t.Fatalf("seed prior release-state indexes: %v", err)
	}
	return prior
}

func seedPriorReleaseStageReceipts(t *testing.T, db *mongo.Database, expectation PriorReleaseStateMigrationExpectation, releases int) []bson.Raw {
	t.Helper()
	ctx := context.Background()
	...
	for releaseIndex := range releases {
		...
		if releaseIndex != 0 {
			releaseID = "release-research-prior-" + string(rune('a'+releaseIndex))
	...
	if _, err := receipts.InsertMany(ctx, rows); err != nil {
		t.Fatalf("seed prior release-stage receipts: %v", err)
	}
	if _, err := receipts.Indexes().CreateMany(ctx, []mongo.IndexModel{
		...
		t.Fatalf("seed prior release-stage receipt indexes: %v", err)
	}
...
func TestMongoPriorReleaseStateMigrationPreservesFactsRebuildsIndexesAndReplays(t *testing.T) {
	db, cleanup := testDB(t)
	defer cleanup()
	ctx := context.Background()
	expectation := priorReleaseStateExpectation()
	expectation.ExpectedReceiptCount = 15
	prior := seedPriorReleaseState(t, db, expectation)
	priorReceipts := seedPriorReleaseStageReceipts(t, db, expectation, 3)

	result, err := MigratePriorContentReleaseState(ctx, db, expectation)
	if err != nil {
		t.Fatalf("migrate prior release state: %v", err)
	}
	if result.Status != "migrated" || result.ReceiptCount != 15 ||
		result.BeforeIndexSetDigest != PriorReleaseStateExpectedIndexDigest() ||
		result.AfterIndexSetDigest != CurrentReleaseStateExpectedIndexDigest() ||
		result.BeforeReceiptIndexSetDigest != PriorReleaseStageReceiptExpectedIndexDigest() ||
	...
	for _, field := range []string{"activatedAt", "createdAt", "updatedAt", "mode", "deletePolicy", "counts", "readback", "status"} {
		if !bsonValueEquivalent(current[field], prior[field]) {
			t.Fatalf("migrated field %s changed: got=%#v want=%#v", field, current[field], prior[field])
		}
	}
...
func TestMongoPriorReleaseStateMigrationCommandWritesCreateOnceRedactedReceipt(t *testing.T) {
	db, cleanup := testDB(t)
	defer cleanup()
	expectation := priorReleaseStateExpectation()
	...
	args := []string{
		...
		"--expected-prior-index-set", PriorReleaseStateIndexSetV1,
		"--expected-prior-receipt-index-set", PriorReleaseStageReceiptIndexSetV1,
		"--expected-receipt-count", "5",
		"--report", report,
	}
	if err := RunPriorReleaseStateMigration(context.Background(), args); err != nil {
	...
	if receipt.Schema != ContentPriorReleaseStateMigrationReceiptSchema || receipt.Status != "migrated" ||
		receipt.Database != db.Name() || receipt.ReleaseID != expectation.ReleaseID || receipt.Revision != 1 ||
		receipt.PriorIndexSetDigest != PriorReleaseStateExpectedIndexDigest() ||
		receipt.AfterIndexSetDigest != CurrentReleaseStateExpectedIndexDigest() ||
		receipt.PriorReceiptIndexSetDigest != PriorReleaseStageReceiptExpectedIndexDigest() ||
	...
	if err := RunPriorReleaseStateMigration(context.Background(), args); err == nil || !strings.Contains(err.Error(), "already exists") {
	...
	var replay ContentPriorReleaseStateMigrationReceipt
...
func TestMongoPriorReleaseStateMigrationRejectsCandidateAmbiguityAndDriftWithoutMutation(t *testing.T) {
	tests := []struct {
		name   string
		mutate func(*testing.T, *mongo.Database, PriorReleaseStateMigrationExpectation)
		want   string
	}{
		{
			name: "candidate collection is non-empty",
			mutate: func(t *testing.T, db *mongo.Database, expectation PriorReleaseStateMigrationExpectation) {
		...
		},
		{
			...
			mutate: func(t *testing.T, db *mongo.Database, _ PriorReleaseStateMigrationExpectation) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, err := MigratePriorContentReleaseState(context.Background(), db, expectation)
			...
			if _, hasKind := unchanged["kind"]; hasKind {
				t.Fatalf("rejected migration changed prior row: %#v", unchanged)
			}
		})
	}
}

func TestMongoPriorReleaseStateMigrationResumesAfterStateOnlyCrash(t *testing.T) {
	db, cleanup := testDB(t)
	defer cleanup()
	ctx := context.Background()
	expectation := priorReleaseStateExpectation()
...
func TestMongoPriorReleaseStateMigrationRejectsReceiptDriftWithoutMutation(t *testing.T) {
	tests := []struct {
		name   string
		mutate func(*testing.T, *mongo.Database, PriorReleaseStateMigrationExpectation)
		want   string
	}{
		{name: "unexpected environment", mutate: func(t *testing.T, db *mongo.Database, _ PriorReleaseStateMigrationExpectation) {
		...
		{name: "mixed owner", mutate: func(t *testing.T, db *mongo.Database, _ PriorReleaseStateMigrationExpectation) {
		...
		{name: "ambiguous release tuple", mutate: func(t *testing.T, db *mongo.Database, _ PriorReleaseStateMigrationExpectation) {
			var row bson.M
		...
		{name: "unexpected shape", mutate: func(t *testing.T, db *mongo.Database, _ PriorReleaseStateMigrationExpectation) {
		...
		{name: "owner binding collision", mutate: func(t *testing.T, db *mongo.Database, expectation PriorReleaseStateMigrationExpectation) {
			receipts := db.Collection("data_release_stage_receipts")
		...
		{name: "row count drift", mutate: func(t *testing.T, db *mongo.Database, _ PriorReleaseStateMigrationExpectation) {
			if _, err := db.Collection("data_release_stage_receipts").DeleteOne(context.Background(), bson.D{}); err != nil {
				t.Fatal(err)
			}
		}, want: "row count differs"},
		{name: "index drift", mutate: func(t *testing.T, db *mongo.Database, _ PriorReleaseStateMigrationExpectation) {
			ctx := context.Background()
			receipts := db.Collection("data_release_stage_receipts")
...
func TestMongoPriorReleaseStateMigrationResumesReceiptDDL(t *testing.T) {
	db, cleanup := testDB(t)
	defer cleanup()
	ctx := context.Background()
	expectation := priorReleaseStateExpectation()
...
func TestMongoPriorReleaseStateMigrationResumesRecognizedDDLPhases(t *testing.T) {
	db, cleanup := testDB(t)
	defer cleanup()
	ctx := context.Background()
	expectation := priorReleaseStateExpectation()
...
func assertReleaseStageReceiptMigration(t *testing.T, db *mongo.Database, before []bson.Raw, expectation PriorReleaseStateMigrationExpectation, count int64) {
	t.Helper()
	ctx := context.Background()
	receipts := db.Collection("data_release_stage_receipts")
	...
	if got, err := receipts.CountDocuments(ctx, bson.M{"sourceOwner": bson.M{"$exists": false}}); err != nil || got != 0 {
		t.Fatalf("prior release-stage receipts remain=%d err=%v", got, err)
	}
	assertReleaseStageReceiptIndexNames(t, db, []string{"_id_", "idx_data_release_stage_receipt_timeline", "uq_data_release_stage_receipt_attempt"})
