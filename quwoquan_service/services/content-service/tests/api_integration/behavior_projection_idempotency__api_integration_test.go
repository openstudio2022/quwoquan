...
func TestBehaviorEventIdempotencyIndexExcludesLegacyEmptyClientEventIDs(t *testing.T) {
	ctx := t.Context()
	if mongoClient == nil {
		t.Fatal("content-service tests require mongoClient")
	}
	db := mongoClient.Database("content_behavior_event_index_migration")
	t.Cleanup(func() {
		if err := db.Drop(context.Background()); err != nil {
			t.Errorf("drop behavior event index migration database: %v", err)
		}
	})
	...
	if _, err := events.Indexes().CreateOne(ctx, mongo.IndexModel{
		...
		Options: options.Index().SetName("uq_behavior_events_user_client_event").SetUnique(true),
	}); err != nil {
		t.Fatalf("create legacy behavior event idempotency index: %v", err)
	}
	...
	if _, err := events.InsertMany(ctx, []any{
		bson.M{"_id": "legacy-empty-1", "userId": "legacy-user"},
		bson.M{"_id": "legacy-empty-2", "userId": "legacy-user", "clientEventId": ""},
	}); err != nil {
		t.Fatalf("legacy empty clientEventId rows must remain writable: %v", err)
	}
