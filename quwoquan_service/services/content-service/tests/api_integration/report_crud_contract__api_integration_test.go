...
func TestReportCrudContractApiIntegrationTest(t *testing.T) {
	...
	cmd := exec.Command(
		...
		"..",
		"-run",
		"^(TestCreateAndResolveReportLifecycle)$",
		"-count=1",
	)
	cmd.Env = append(os.Environ(), "QWQ_OUTPUT_ROOT="+filepath.Join(repoRoot(), ".qwq_output"))
	output, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("source bridge failed for quwoquan_service/services/content-service/tests/report_crud_contract_test.go: %v\n%s", err, output)
	}
}
