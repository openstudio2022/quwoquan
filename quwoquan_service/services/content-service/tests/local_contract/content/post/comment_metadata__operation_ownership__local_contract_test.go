...
func TestCommentMetadataOwnsAllCanonicalOperationsWithReadyTransport(t *testing.T) {
	metadataRoot := filepath.Join(
		...
		"services/content-service/contracts/content/comment",
	)
	...
	raw, err := os.ReadFile(filepath.Join(metadataRoot, "operations.yaml"))
	if err != nil {
		t.Fatal(err)
	}
	var service commentMetadataService
	if err := yaml.Unmarshal(raw, &service); err != nil {
		t.Fatalf("decode comment operations metadata: %v", err)
	}
