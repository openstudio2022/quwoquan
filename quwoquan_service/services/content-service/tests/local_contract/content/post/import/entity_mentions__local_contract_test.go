...
func homepageProofFixture(t *testing.T) (string, string, ReleaseBinding, map[string]bool) {
	t.Helper()
	root := t.TempDir()
	binding := ReleaseBinding{ReleaseID: "release-mentions", SourceOwner: "qwq_data", ManifestDigest: "sha256:" + strings.Repeat("a", 64)}
...
func TestImportedArticleMentionsUseExplicitIdentityAndTypedQuery(t *testing.T) {
	...
	document, err := BuildCanonicalImportedPostDocument(posts[0], time.Now().UTC(), ImportOptions{}, "active")
	if err != nil {
		t.Fatal(err)
	}
