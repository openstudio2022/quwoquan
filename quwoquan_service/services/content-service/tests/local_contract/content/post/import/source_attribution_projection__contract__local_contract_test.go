...
func TestLoadRejectsMissingOrInvalidSourceAttributionFacts(t *testing.T) {
	for _, invalid := range []string{
		...
		t.Run(invalid, func(t *testing.T) {
			...
			if _, err := LoadPosts(root, nil); err == nil || !strings.Contains(err.Error(), "sourceAttribution") {
...
func TestLoadSourceAttributionRejectsMissingNullAndInvalidDataFacts(t *testing.T) {
	...
	for _, admission := range []string{"research_release", "commercial_release", "production_release"} {
		t.Run("preserve/"+admission, func(t *testing.T) {
			...
			facts["publicationAdmission"] = admission
			raw, err := json.Marshal(map[string]any{"contentType": "article", "publishedAt": "2026-09-09T00:00:00Z", "sourceAttribution": facts})
			if err != nil {
				t.Fatal(err)
			}
			root := t.TempDir()
