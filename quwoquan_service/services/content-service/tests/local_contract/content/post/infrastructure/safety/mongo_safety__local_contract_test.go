...
func TestPostSafetyRealMongo(t *testing.T) {
	...
	m, err := safety.New(runtime.Database, "alpha", []byte(strings.Repeat("s", 32)), authority)
	if err != nil {
		t.Fatal(err)
	}
	if _, err = safety.New(runtime.Database, "alpha", nil, authority); err == nil {
	...
	var row bson.M
	if err = runtime.Database.Collection(safety.Collection).FindOne(ctx, bson.M{}).Decode(&row); err != nil {
		t.Fatal(err)
	}
