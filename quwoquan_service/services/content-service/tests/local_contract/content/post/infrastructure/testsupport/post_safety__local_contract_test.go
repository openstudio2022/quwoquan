...
func TestTypedPostStoreSafetyRevisionAndAtomicity(t *testing.T) {
	s := testsupport.NewPostStore(nil)
	...
	if first.Post == nil || first.Post.Version != 1 {
	...
	if len(s.OutboxEvents()) != 2 {
		t.Fatal("restriction did not commit atomically")
	}
	c.ExpectedVersion = 2
	...
	if loaded, found := s.FindByID(ctx, "post"); !found || loaded.Version != 2 || len(s.OutboxEvents()) != 2 {
