...
func TestPrivateFenceTransactionMongo(t *testing.T) {
	...
	stage := func(id string) ImportedReleaseBinding {
		...
		post := PostDoc{PostRef: "posts/article/" + id + "/1", ContentID: "content-" + id, ContentVersion: 1, PoolSourceType: "data", VariantPurpose: "original", PoolStatus: "active", ContentType: "article", Title: id, AuthorID: "builtin_travel_blogger", ArticleMarkdown: "# " + id, Admission: ContentAdmission{ProcessResult: "completed", QualityResult: "passed", UsageScope: "production", EvidenceRef: "audit/attestation.json", EvidenceDigest: "sha256:" + strings.Repeat("a", 64)}, CreatedAt: now.Add(-time.Hour), UpdatedAt: now, PublishedAt: now}
	...
	first, err := activateForTest(ctx, db, "alpha", a, empty, now)
	if err != nil {
		t.Fatal("A", err)
	}
	...
	query := func(target ImportedReleaseBinding, before ActiveReleaseBinding) wire.ReadContentReleaseCommitReceiptQuery {
		return wire.ReadContentReleaseCommitReceiptQuery{Release: wire.ReleaseCandidateBinding{Environment: "alpha", SourceOwner: target.SourceOwner, ReleaseId: target.ReleaseID, ManifestDigest: target.ManifestDigest}, Expected: wireFence(before)}
	}
...
print("PYTHON_SIGNER_TO_GO_GUARD_REAL_MONGO_RECEIPT_PASS")`
	...
	replay, err := activateForTest(ctx, db, "alpha", a, empty, now.Add(time.Second))
	...
	for _, collection := range []string{"content_outbox", "data_release_stage_receipts"} {
		...
		if _, err = activateForTest(ctx, db, "alpha", b, expected, now.Add(2*time.Second)); err == nil {
	...
	second, err := activateForTest(ctx, db, "alpha", b, expected, now.Add(2*time.Second))
	if err != nil {
		t.Fatal("B", err)
	}
	var old bson.M
	...
	third, err := activateForTest(ctx, db, "alpha", a, expectedB, now.Add(3*time.Second))
	...
	for _, target := range []ImportedReleaseBinding{c, d} {
		...
		go func(target ImportedReleaseBinding) {
			defer wg.Done()
			_, e := activateForTest(ctx, db, "alpha", target, before, now.Add(4*time.Second))
			results <- e
		}(target)
	}
	wg.Wait()
