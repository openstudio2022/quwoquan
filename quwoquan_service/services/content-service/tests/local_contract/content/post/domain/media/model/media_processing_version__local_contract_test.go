...
func TestReadyVideoKeepsProcessingVersionAcrossIdempotentAutoCoverAndRestore(t *testing.T) {
	createdAt := time.Date(2026, 7, 21, 0, 0, 0, 0, time.UTC)
	asset, err := CreateMediaAsset(CreateMediaAssetParams{
	...
	if err := asset.RecordProcessingResult(
		...
		MediaProcessingDescriptor{Video: VideoProcessingDescriptor{
			ProcessorProfile:             "content_video_transcode",
	...
	if snapshot.Version != 2 || snapshot.ProcessingVersion != processingVersion {
		t.Fatalf(
			"version state=(aggregate=%d, processing=%d), want (2,%d)",
			snapshot.Version,
...
func TestVideoCoverSelectionDistinguishesProcessingAndRejectedAssets(
	t *testing.T,
) {
	createdAt := time.Date(2026, 7, 21, 0, 0, 0, 0, time.UTC)
	newVideo := func(id string) *MediaAsset {
		t.Helper()
		asset, err := CreateMediaAsset(CreateMediaAssetParams{
			ID: id, OwnerID: "owner-1", SourceSessionID: "session-" + id,
			ObjectKey: "private/" + id + "/source.mp4",
			SHA256:    "sha256:dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd",
			MediaType: "video", MimeType: "video/mp4", FileSize: 1024,
			AccessPolicy: AccessPolicyOwnerOnly, ProcessingRequired: true,
			Now: createdAt,
		})
		if err != nil {
			t.Fatalf("create video asset %s: %v", id, err)
		}
		return asset
	}

	processing := newVideo("media-cover-processing")
	if err := processing.SelectAutoCover(
		"owner-1",
		createdAt.Add(time.Second),
	); !errors.Is(err, ErrMediaNotReady) {
		t.Fatalf("processing cover error=%v, want ErrMediaNotReady", err)
	}

	rejected := newVideo("media-cover-rejected")
	if err := rejected.RecordProcessingResult(
		ProcessingStatusRejected,
		"unsupported video stream",
		MediaProcessingDescriptor{},
		createdAt.Add(time.Second),
	); err != nil {
		t.Fatalf("reject video processing: %v", err)
	}
	if err := rejected.SelectAutoCover(
		"owner-1",
		createdAt.Add(2*time.Second),
	); !errors.Is(err, ErrMediaProcessingRejected) {
		t.Fatalf(
			"rejected cover error=%v, want ErrMediaProcessingRejected",
			err,
		)
	}
}
