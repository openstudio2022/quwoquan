...
//
// 背景：RecommendationFeatureProfileView 随 typed reason 下发 edgeWeight。如果三个排序比较器都不读它，
...
func TestEdgeWeightParticipatesInObjectOrdering(t *testing.T) {
	...
	stale.EdgeWeight = 0.1
	recent.EdgeWeight = 0.9
	materialized := []IntersectionReasonView{stale, recent}
...
func TestEdgeWeightParticipatesInInboxOrdering(t *testing.T) {
	...
	sparse.EdgeWeight = 0.2
	dense.EdgeWeight = 0.8
	materialized := []IntersectionReasonView{sparse, dense}
