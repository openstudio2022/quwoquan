...
// assembleContentDomain 是 content 的领域装配入口：在骨架已装配的
// Mongo/Redis/auth/观测面之上打开仓储、编织内容投影与媒体/审核链路、注册
// 路由与后台 worker。runtime 承载两个只能在装配期确定、却要被装配之后的骨架
// 钩子读取的值（feed 并发预算、config sync 的 HotConfigStore）。
func assembleContentDomain(
	asm *servicekit.Assembly,
	cfg *config,
	runtime *contentAPIRuntime,
) error {
	ctx := asm.Context
	...
	runtime.feed = cfg.Feed
	runtime.assembled = true
	// 交集展示文案的运营态覆盖是 fail-safe 的：控制面不可达、未下发 key 或值为
	// 空串时回落 registry codegen 基线，文案不会因配置面故障而消失。
	intersectionapp.SetTextResolver(
		controlplane.NewIntersectionTextResolver(runtime.hotConfigStore),
	)
	logFeedQuotaPolicies(cfg.Feed)

	feedCursorCodec, err := feedapp.NewFeedCursorCodec(asm.Auth.AccessTokenConfig.Secret)
	if err != nil {
		return fmt.Errorf("feed cursor codec init failed: %w", err)
	}
	onboardingTaxonomy, err := buildOnboardingInterestTaxonomyValidator(*cfg)
	if err != nil {
		return fmt.Errorf("onboarding taxonomy validation config failed: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("account-closure privacy config failed: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("MediaAsset reference fence init failed: %w", err)
	}
	filterCatalogStore := filtercatalogpersistence.NewMongoStore(db)
	if err := filterCatalogStore.EnsureIndexes(ctx); err != nil {
		return fmt.Errorf("FilterCatalogRelease indexes init failed: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("FilterCatalogRelease composition failed: %w", err)
	}
	...
	if err := profileActivityStore.EnsureIndexes(ctx); err != nil {
		return fmt.Errorf("ProfileInteractionActivity indexes init failed: %w", err)
	}
	profileReadFactStore := profilereadfactinfra.NewMongoReadFactStore(db)
	if err := profileReadFactStore.EnsureIndexes(ctx); err != nil {
		return fmt.Errorf("ProfileInteractionReadFact indexes init failed: %w", err)
	}
	...
	if err := mediaStore.EnsureIndexes(ctx); err != nil {
		return fmt.Errorf("media runtime indexes init failed: %w", err)
	}
	if err := startMediaAssetOutboxRelay(
		...
		return fmt.Errorf("MediaAsset outbox relay init failed: %w", err)
	}
	...
	if err := mediaOriginalAccessStore.EnsureIndexes(ctx); err != nil {
		return fmt.Errorf("MediaOriginalAccessFact indexes init failed: %w", err)
	}
	...
	if err := originalAccessQuotaStore.EnsureIndexes(ctx); err != nil {
		return fmt.Errorf("OriginalAccessQuota indexes init failed: %w", err)
	}
	...
	if err := mediaImageReprocessStore.EnsureIndexes(ctx); err != nil {
		return fmt.Errorf("MediaImageReprocessRun indexes init failed: %w", err)
	}
	...
	if err := mediaUploadSessionStore.EnsureIndexes(ctx); err != nil {
		return fmt.Errorf("MediaUploadSession indexes init failed: %w", err)
	}
	if err := startMediaUploadSessionOutboxRelay(
		...
		return fmt.Errorf("MediaUploadSession outbox relay init failed: %w", err)
	}
	...
	if err := commentDataAdapter.EnsureIndexes(ctx); err != nil {
		return fmt.Errorf("Comment indexes init failed: %w", err)
	}
	...
	if err := moderationStore.EnsureIndexes(ctx); err != nil {
		return fmt.Errorf("PostModerationCase indexes init failed: %w", err)
	}
	...
	if err := personaAccessProjection.EnsureIndexes(ctx); err != nil {
		return fmt.Errorf("persona access projection startup failed: %w", err)
	}
	...
	if searchBuilt.Client != nil {
		...
		if err := searchBuilt.EnsureIndex(ctx); err != nil {
			return fmt.Errorf("ensure ES search index failed: %w", err)
		}
	...
	if err != nil {
		return fmt.Errorf("media object deletion fence assembly failed: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("UserAccountClosed store assembly failed: %w", err)
	}
	if err := accountClosureStore.EnsureIndexes(ctx); err != nil {
		return fmt.Errorf("UserAccountClosed indexes init failed: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("account restriction projection assembly failed: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("UserAccountClosed search assembly failed: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("UserAccountClosed cache assembly failed: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("subject-closure guard assembly failed: %w", err)
	}
	...
	if cfg.Embedding.Enabled && !contentSliceWorkload() {
		...
		if err != nil {
			return fmt.Errorf("embedding binding invalid: %w", err)
		}
	...
	if err != nil {
		return fmt.Errorf("account-closure recovery commands failed: %w", err)
	}
	accountClosureRecoveryHandler, err := closurehttp.NewHandler(accountClosureRecoveryCommands)
	if err != nil {
		return fmt.Errorf("account-closure recovery handler failed: %w", err)
	}
	handler, err = accountClosureRecoveryHandler.Mount(handler)
	if err != nil {
		return fmt.Errorf("account-closure recovery route failed: %w", err)
	}
	asm.Mux.Handle("/", handler)

	if handlers.internalGraphQL == nil {
		return fmt.Errorf("internal GraphQL handler is not configured")
	}
	// 以下三个面不是 generated public REST operation，因此挂在 operation guard
	// 之外、认证与观测栈之内：
	//   - owner-internal persisted GraphQL 在 handler 内校验 api-edge 服务主体、
	//     scope、persisted hash 与 ContractGraph digest；
	//   - 公开 SEO HTML 读面匿名可读、只输出公开已发布对象；
	//   - 边缘 forward_auth 验签端点纯 CPU 复算 HMAC 与到期（DEC-031）。
	unguarded := asm.Unguarded()
	unguarded.Handle("POST /internal/graphql", handlers.internalGraphQL)
	if handlers.publicWeb != nil {
		unguarded.Handle("/public-web/", handlers.publicWeb)
	}
	if mediaRuntime.mediaDeliveryAuthHandler != nil {
		unguarded.Handle(
			"GET /internal/media/delivery-auth",
			mediaRuntime.mediaDeliveryAuthHandler,
		)
	}

	log.Printf(
		"content-service assembled (feed_owner_max_inflight=%d recall_sources_maximum=%d recall_unterminated_per_source=%d)",
