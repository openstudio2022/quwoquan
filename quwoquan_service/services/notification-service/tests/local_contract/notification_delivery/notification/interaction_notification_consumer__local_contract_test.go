...
func (s *memoryFailureStore) RecordInteractionFailure(
	_ context.Context, stream, messageID, _ string, _ string, _ error,
) (int64, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *memoryFailureStore) IsInteractionDeadLettered(
	_ context.Context,
	stream string,
	messageID string,
) (bool, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.deadLettered[stream+"|"+messageID], nil
}

func (s *memoryFailureStore) MarkInteractionDeadLettered(
	_ context.Context,
	stream string,
	messageID string,
) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.deadLettered[stream+"|"+messageID] = true
	return nil
}
...
func (s *memoryFailureStore) ClearInteractionFailure(
	...
	delete(s.deadLettered, stream+"|"+messageID)
	return nil
}
...
func TestInteractionConsumerDeadLettersPoisonEventAndContinues(t *testing.T) {
	...
	if _, err := redis.XAdd(ctx, "events.content.comment_lifecycle", map[string]string{
		"eventId":    "evt-poison-user-secret",
		"eventType":  "CommentCreated",
		"payload":    `{"postAuthorId":"interaction-user-secret","personaIds":["interaction-persona-secret"]}`,
		"occurredAt": time.Now().UTC().Format(time.RFC3339Nano),
	}); err != nil {
	...
	for _, forbidden := range []string{
		...
		if _, exists := dlq[0].Values[forbidden]; exists {
			t.Fatalf("interaction DLQ leaked %s: %v", forbidden, dlq[0].Values)
		}
	}
	for _, required := range []string{
		...
		if dlq[0].Values[required] == "" {
			t.Fatalf("interaction DLQ missing %s: %v", required, dlq[0].Values)
		}
	}
	if dlq[0].Values["eventClass"] != "interaction_notification" ||
		dlq[0].Values["errorClass"] != "projection_failed" ||
		dlq[0].Values["attempts"] != "5" {
		t.Fatalf("interaction DLQ reference drifted: %v", dlq[0].Values)
	}
}
