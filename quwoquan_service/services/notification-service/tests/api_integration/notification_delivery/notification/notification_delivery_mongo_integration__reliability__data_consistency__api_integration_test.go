...
// singleEndpointLister 为通用 Deliver 的 alert 组装提供一个固定设备端点。
type singleEndpointLister struct{}

func (singleEndpointLister) ListPushDestinations(
	context.Context,
	string,
) ([]deliveryapplication.PushDestinationRef, error) {
	return []deliveryapplication.PushDestinationRef{
		{DeviceID: "device-real-1", EndpointRef: strings.Repeat("d", 64)},
	}, nil
}
...
func TestNotificationDeliveryPersistsRecipientLedgerAndSubmitsIntegrationRequest(t *testing.T) {
	...
	if !ok || payload["action"] != "alert" ||
		payload["deliveryKey"] != record.DedupeKey ||
		payload["targetPersonaId"] != record.RecipientIDs[0] ||
		payload["title"] != "李明" ||
		payload["targetType"] != "conversation" {
		t.Fatalf("notification request payload is not a contract alert: %+v", requests[0])
	}
...
func TestNotificationDeliveryReturnsStructuredRemoteFailure(t *testing.T) {
	...
	_, err = adapter.Deliver(
		...
		reliabletask.NotificationOutboxRecord{
			...
			DedupeKey:      "push:message-failure-001",
			Payload: map[string]string{
				"title":      "李明",
				"summary":    "周六的观星聚会记得带上三脚架",
				"targetType": "conversation",
				"targetId":   "conv-failure-001",
			},
		},
		"user-failure-001",
	)
...
func TestNotificationDeliveryOperatorFacadesListRecoverAndConvergeMetrics(t *testing.T) {
	...
	var deadLetters deliverydomain.NotificationDeliveryJobDeadLetterSlice
	...
	var recovered deliverydomain.RecoverNotificationDeliveryJobResult
	...
	var replayed deliverydomain.RecoverNotificationDeliveryJobResult
...
func newNotificationOperatorHTTPHandler(t *testing.T) http.Handler {
	...
	deliveryCommands, err := deliveryapplication.NewNotificationDeliveryJobCommandFacade(
		notificationReliableStore,
	)
	if err != nil {
		t.Fatalf("construct delivery command facade: %v", err)
	}
	handler, err := deliveryhttp.NewHandler(deliveryCommands, deliveryQueries)
	if err != nil {
...
func requestNotificationMetrics(
...
) deliverydomain.NotificationDeliveryJobMetricsSnapshot {
	t.Helper()
	...
	var snapshot deliverydomain.NotificationDeliveryJobMetricsSnapshot
