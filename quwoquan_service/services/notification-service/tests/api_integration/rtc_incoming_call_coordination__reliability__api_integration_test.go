...
func TestRTCIncomingCallCoordinationWithRealMongoAndRedis(t *testing.T) {
	...
	userServer := httptest.NewServer(http.HandlerFunc(
		func(w http.ResponseWriter, r *http.Request) {
			if r.URL.Path !=
				"/internal/user/personas/"+personaID+"/push-destinations" {
				http.NotFound(w, r)
				return
			}
...
func newIncomingCallHTTPHandler(
	...
	deliveryCommands, err :=
		application.NewNotificationDeliveryJobCommandFacade(
			notificationReliableStore,
		)
	if err != nil {
		t.Fatal(err)
	...
	deliveryQueries, err :=
		application.NewNotificationDeliveryJobQueryFacade(
			notificationReliableStore,
			notificationReliableStore,
		)
	if err != nil {
