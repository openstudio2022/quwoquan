...
// seedAssistantMembership 直插助手成员形态；assistant memberType 成员由系统
// 装配产生，测试树内无公开添加入口，构造该形态只能经本 harness。
func seedAssistantMembership(t *testing.T, conversationID string) {
	t.Helper()
...
// ---- selectable group 的直插前置状态收口（DEC-005 形态三）----
// 一般 contract 用例不得自带直写语句；构造可选群/圈群绑定形态只能
// 调用这里的命名 seed 函数。
