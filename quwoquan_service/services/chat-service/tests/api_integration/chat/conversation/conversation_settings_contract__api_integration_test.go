...
func TestUpdateConversationSettings_Mute(t *testing.T) {
	...
	code, result := doPatch(t, "/chat/conversations/"+convId+"/settings",
...
func TestUpdateConversationSettings_Pin(t *testing.T) {
	...
	code, _ := doPatch(t, "/chat/conversations/"+convId+"/settings",
...
func TestMarkAsRead(t *testing.T) {
	...
	result := doPost(t, "/chat/conversations/"+convId+"/messages/"+msgId+"/read",
		`{}`, "user_test_001", 200)
...
func TestGetReceipts(t *testing.T) {
	...
	doPost(t, "/chat/conversations/"+convId+"/messages/"+msgId+"/read",
		`{}`, "user_test_001", 200)

	code, result := doGet(t, "/chat/conversations/"+convId+"/messages/"+msgId+"/receipts", "user_test_001")
	if code != 200 {
		t.Fatalf("expected 200, got %d", code)
	}
	if result["items"] == nil {
