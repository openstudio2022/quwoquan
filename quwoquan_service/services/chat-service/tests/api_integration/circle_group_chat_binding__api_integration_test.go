...
func TestEnsureIndexesUpgradesLegacyCircleGroupIndexWithoutDataReset(t *testing.T) {
	ctx := context.Background()
	...
	if _, err := conversations.Indexes().CreateOne(ctx, mongo.IndexModel{
		...
		Options: options.Index().SetName("idx_conv_circle_group").SetSparse(true),
	}); err != nil {
		t.Fatalf("create legacy non-unique circle group index: %v", err)
	}
	if _, err := conversations.InsertMany(ctx, []any{
		bson.M{"_id": "legacy-empty-1", "circleGroupId": ""},
		bson.M{"_id": "legacy-empty-2", "circleGroupId": "  "},
		bson.M{"_id": "bound-conversation", "circleGroupId": "group-preserved"},
	}); err != nil {
		t.Fatalf("insert legacy circle group rows: %v", err)
	}

	store := persistence.NewMongoChatStore(upgradeDB)
	if err := store.EnsureIndexes(ctx); err != nil {
		t.Fatalf("upgrade legacy circle group index without resetting data: %v", err)
	}
	if err := store.EnsureIndexes(ctx); err != nil {
		t.Fatalf("replaying completed circle group index migration: %v", err)
	}
	...
	for _, index := range indexes {
		switch index["name"] {
		case "idx_conv_circle_group":
			t.Fatalf("legacy circle group index must be removed: %#v", index)
		case "uq_conv_circle_group":
			target = index
		}
	}
	if target == nil {
	...
	emptyBindings, err := conversations.CountDocuments(ctx, bson.M{
		"circleGroupId": bson.M{"$exists": true},
	})
	if err != nil {
		t.Fatalf("count normalized legacy bindings: %v", err)
	}
	if emptyBindings != 1 {
		t.Fatalf("only the non-empty binding may remain after migration, got %d", emptyBindings)
	}
}

func TestEnsureIndexesFailsClosedForDuplicateLegacyCircleGroupBindings(t *testing.T) {
	ctx := context.Background()
	...
	if _, err := conversations.InsertMany(ctx, []any{
		...
		t.Fatalf("insert duplicate legacy bindings: %v", err)
	}

	err := persistence.NewMongoChatStore(upgradeDB).EnsureIndexes(ctx)
	if err == nil || !strings.Contains(err.Error(), "duplicate non-empty circleGroupId") {
		t.Fatalf("duplicate legacy bindings must fail closed, got %v", err)
	}
	...
	if countErr != nil || count != 2 {
		t.Fatalf("failed migration must preserve duplicate evidence: count=%d err=%v", count, countErr)
	}
}

func TestEnsureIndexesSerializesConcurrentCircleGroupIndexMigration(t *testing.T) {
	ctx := context.Background()
	...
	if _, err := conversations.InsertMany(ctx, []any{
		bson.M{"_id": "concurrent-empty-1", "circleGroupId": ""},
		bson.M{"_id": "concurrent-empty-2", "circleGroupId": ""},
	}); err != nil {
		t.Fatalf("insert concurrent migration legacy rows: %v", err)
	}
