...
def main() -> int:
    ...
    command = [
        sys.executable,
        str(REPO_ROOT / "quwoquan_service/services/chat-service/tests/ops/ci/run_chat_avatar_device_matrix.py"),
        "--env",
        env_name,
        "--platform",
        args.platform,
        "--report",
        f".qwq_output/runs/{env_name}/device-matrix/chat-avatar-{args.platform}.json",
    ]
    report_path = (
        REPO_ROOT / ".qwq_output" / "runs" / env_name / "device-matrix" / f"chat-avatar-{args.platform}.json"
    )
