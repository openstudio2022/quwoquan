...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--report", default=".qwq_output/runs/beta/avatar-e2e/chat-avatar-report.json")
...
def should_rerun_for_failure(category: str) -> bool:
    return category in {
        "gateway_unreachable",
        "avatar_task_timeout",
...
"""
    if args.compose_mongo:
        cmd = [
            ...
            "compose",
            "-f",
            str(REPO_ROOT / "quwoquan_ops/environments/compose/docker-compose.gamma-local.yaml"),
            "exec",
            "-T",
            "mongodb",
            "mongosh",
...
def main() -> int:
    ...
    try:
        ...
        report["rerunRecommended"] = should_rerun_for_failure(exc.category)
        ...
        report["blockingReason"] = str(exc)
        report["rerunRecommended"] = should_rerun_for_failure(report["failureCategory"])
