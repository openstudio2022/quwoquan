...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--report",
        default=str(
            Path(os.environ.get("QWQ_OUTPUT_ROOT", ".qwq_output")) / "runs" / "app" / "local-gamma" / "avatar_e2e_report.json"
        ),
    )
...
def main() -> int:
    ...
    probe_cmd = [
        sys.executable,
        str(ROOT / "quwoquan_service/services/chat-service/tests/ops/smoke/run_chat_avatar_e2e_probe.py"),
        "--env",
        "local-gamma",
        "--base-url",
        args.base_url,
    ...
    if not args.skip_device_matrix and (report["probe"].get("report") or {}).get("status") == "passed":
        matrix_cmd = [
            sys.executable,
            str(ROOT / "quwoquan_service/services/chat-service/tests/ops/ci/run_chat_avatar_device_matrix.py"),
            "--env",
            "local-gamma",
            "--platform",
            args.platform,
