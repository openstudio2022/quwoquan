...
func TestCircleGroupChatSyncConsumerRetriesThenDeadLettersWithoutLeakingCause(t *testing.T) {
	...
	if got := client.expired[consumer.config.DLQ]; got != circleGroupChatSyncDLQTTL {
		t.Fatalf("DLQ retention=%v, want %v", got, circleGroupChatSyncDLQTTL)
	}
	pending, _, err := client.XAutoClaim(
		ctx,
		CircleGroupEventStream,
	...
	if err := client.XGroupCreateMkStream(ctx, consumer.config.DLQ, "dlq-observer", "0"); err != nil {
		t.Fatal(err)
	}
	dlq, err := client.XReadGroup(
		...
		"dlq-observer",
		"dlq-observer-1",
		map[string]string{consumer.config.DLQ: ">"},
		10,
		0,
	)
...
func newCircleGroupChatSyncConsumerForTest(
	...
	consumer, err := NewCircleGroupChatSyncConsumer(
		...
		CircleGroupChatSyncConsumerConfig{
			...
			DLQ:          "events.circle.groups.chat-circle-group-sync-test.dlq",
			BatchSize:    10,
