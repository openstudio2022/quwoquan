...
func TestUserAccountClosedConsumerAppliesAndAcknowledgesReplay(
	...
	if len(pending) != 0 {
		t.Fatalf("successful messages must be ACKed, pending=%d", len(pending))
	}
...
func TestUserAccountClosedConsumerRetriesWithoutAckThenDeadLetters(
	...
	projection := &memoryUserAccountClosedProjection{
		err: errors.New("projection failed for " + secret),
	}
	failures := &memoryUserAccountClosedFailures{}
	...
	if len(pending) != 1 || pending[0].ID != messageID {
		t.Fatalf("failed message must not be ACKed: %#v", pending)
	}
	...
	if len(pending) != 0 {
		t.Fatalf("dead-lettered message must be ACKed, pending=%d", len(pending))
	}
	if len(failures.attempts) != 0 {
		t.Fatalf("failure state must be cleared after ACK: %#v", failures.attempts)
	}
	...
	if len(deadLetters) != 1 {
		t.Fatalf("expected one DLQ message, got %d", len(deadLetters))
	}
	dlq := deadLetters[0].Values
	for _, forbidden := range []string{
		...
		if _, found := dlq[forbidden]; found {
			t.Fatalf("DLQ must not persist private field %q: %#v", forbidden, dlq)
		}
	}
	if dlq["errorDigest"] == "" || dlq["eventDigest"] == "" {
		t.Fatalf("DLQ must retain non-PII diagnostics: %#v", dlq)
	}
	if client.expired[userAccountClosedDLQ] != userAccountClosedDLQTTL {
		t.Fatalf(
			"DLQ TTL mismatch: got=%s want=%s",
			client.expired[userAccountClosedDLQ],
