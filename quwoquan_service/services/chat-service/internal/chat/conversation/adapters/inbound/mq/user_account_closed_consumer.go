...
// UserAccountClosedFailureStore 保存逐 stream message 的失败次数与 terminal
// marker。达到上限后原 source PEL 保持未 ACK，直到受控恢复显式释放 marker。
...
func (c *UserAccountClosedConsumer) processMessage(
	...
	startedAt := time.Now()
	deadLettered, stateErr := c.failures.IsUserAccountClosedDeadLettered(
		ctx,
		message.ID,
	)
	if stateErr != nil {
		return fmt.Errorf(
			"read chat UserAccountClosed dead-letter state: %w",
			stateErr,
		)
	}
	if deadLettered {
		// DLQ only retains an irreversible source reference. Keep the
		// original payload in the source PEL until explicit recovery clears
		// this marker.
...
// RecoverDeadLetter releases one terminal source PEL for bounded retry. The
// original event is never reconstructed from its sanitized DLQ record.
func (c *UserAccountClosedConsumer) RecoverDeadLetter(
	ctx context.Context,
	sourceStreamID string,
) error {
	if c == nil || c.failures == nil {
		return errors.New("chat UserAccountClosed consumer is not configured")
	}
	sourceStreamID = strings.TrimSpace(sourceStreamID)
	if sourceStreamID == "" {
		return errors.New("chat UserAccountClosed dead-letter source stream ID is required")
	}
	deadLettered, err := c.failures.IsUserAccountClosedDeadLettered(
		ctx,
		sourceStreamID,
	)
	if err != nil {
		return fmt.Errorf(
			"verify chat UserAccountClosed dead-letter state before recovery: %w",
			err,
		)
	}
	if !deadLettered {
	...
	if err := c.failures.ClearUserAccountClosedFailure(
		ctx,
		sourceStreamID,
	); err != nil {
		return fmt.Errorf(
			"release chat UserAccountClosed dead-letter state: %w",
			err,
		)
	}
	return nil
...
func (c *UserAccountClosedConsumer) recordFailure(err error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.lastFailureDigest = irreversibleUserAccountClosedDigest(err.Error())
}
