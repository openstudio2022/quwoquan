...
func (p *MongoUserAccountClosedProjection) RecordUserAccountClosedFailure(
	...
	err := p.failures.FindOneAndUpdate(
		ctx,
		bson.M{
			"_id":            userAccountClosedFailureID(messageID),
...
func (p *MongoUserAccountClosedProjection) IsUserAccountClosedDeadLettered(
	ctx context.Context,
	messageID string,
) (bool, error) {
	var document struct {
...
func (p *MongoUserAccountClosedProjection) MarkUserAccountClosedDeadLettered(
	ctx context.Context,
	messageID string,
) error {
	now := time.Now().UTC()
	result, err := p.failures.UpdateOne(
		ctx,
		bson.M{"_id": userAccountClosedFailureID(messageID)},
		bson.M{
			"$set": bson.M{
			...
			// A terminal marker owns the source PEL. It must survive the
			// transient retry TTL until an operator releases it explicitly.
	...
	if result.MatchedCount != 1 {
		return errors.New("chat UserAccountClosed failure state is missing")
	}
	return nil
}
