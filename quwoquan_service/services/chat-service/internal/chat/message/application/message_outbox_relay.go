...
// OutboxEventPublisher 是已提交 Message 事件的投递边界。实现必须在返回成功
// 前完成持久追加，因为该确认会推进 consumer checkpoint。
type OutboxEventPublisher interface {
	PublishRecordedDomainEvent(
		ctx context.Context,
		eventID, eventType, conversationID, actorID string,
...
type MessageOutboxRelay struct {
	reader      messageports.OutboxReader
	dispatch    messageports.OutboxDispatchStore
	checkpoints messageports.OutboxCheckpointStore
