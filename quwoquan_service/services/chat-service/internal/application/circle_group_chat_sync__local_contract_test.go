...
func TestCircleGroupChatSyncProjectsLifecycleAndRejectsLateMembership(t *testing.T) {
	...
	if err := syncService.Apply(context.Background(), CircleGroupChatSourceEvent{
		EventID: "group-1:created:1", EventType: circleGroupEventCreated,
	...
	if conv.MaxGroupSize != maxGroupSizeLimit || conv.CreatorId != "owner-1" ||
	...
	if err := syncService.Apply(context.Background(), CircleGroupChatSourceEvent{
		EventID: "membership-p2:active:1", EventType: circleGroupMembershipEventActivated,
		GroupID: "group-1", CircleID: "circle-1", Version: 1,
	...
	if err := syncService.Apply(context.Background(), CircleGroupChatSourceEvent{
		EventID: "membership-p2:left:2", EventType: circleGroupMembershipEventLeft,
	...
	if err := syncService.Apply(context.Background(), CircleGroupChatSourceEvent{
		EventID: "group-1:archived:2", EventType: circleGroupEventArchived,
	...
	if err := syncService.Apply(context.Background(), CircleGroupChatSourceEvent{
		EventID: "membership-p3:active:3", EventType: circleGroupMembershipEventActivated,
...
func TestCircleGroupChatSyncRejectsPublicChatGovernance(t *testing.T) {
	...
	store.conversations["conv-circle"] = &model.Conversation{
		ID: "conv-circle", Type: conversationTypeGroup, Status: model.ConversationStatusActive,
		CircleId: "circle-1", CircleGroupId: "group-1", MaxGroupSize: maxGroupSizeLimit,
	}
