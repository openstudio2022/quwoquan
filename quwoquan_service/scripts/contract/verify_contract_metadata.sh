#!/usr/bin/env bash
# Validate the shared metadata foundation and service-owned object contracts.
set -euo pipefail
...
mkdir -p "$CONTRACT_VIEW_PARENT"
CONTRACT_VIEW="$(mktemp -d "${CONTRACT_VIEW_PARENT}/cache-verify.XXXXXX")"
rmdir "$CONTRACT_VIEW"
trap 'rm -rf "$CONTRACT_VIEW"' EXIT
YAML_ROOTS=("$BASE")
for contracts_root in \
  "${ROOT}"/quwoquan_service/services/*/contracts \
  "${ROOT}"/quwoquan_service/control-plane/*/contracts; do
  [[ -d "$contracts_root" ]] && YAML_ROOTS+=("$contracts_root")
done
...
alias_hits="$(find "${YAML_ROOTS[@]}" -type f -name '*.yaml' -exec grep -En '(^|[[:space:]])[&*][[:alnum:]_-]+' {} + || true)"
...
while IFS= read -r -d '' p; do
  ruby -rpsych -e 'Psych.parse_file(ARGV.fetch(0))' "$p" \
    || { echo "[verify] FAIL: invalid YAML $p"; exit 1; }
done < <(find "${YAML_ROOTS[@]}" -type f -name '*.yaml' -print0)
...
if command -v go >/dev/null 2>&1; then
  ...
  PYTHONDONTWRITEBYTECODE=1 python3 \
    "${ROOT}/quwoquan_service/scripts/contracts/build_service_contract_view.py" \
    --output "$CONTRACT_VIEW" >/dev/null || exit 1
  (cd "${ROOT}/quwoquan_service" && go run ./tools/verify_metadata/ "$CONTRACT_VIEW") || exit 1
else
