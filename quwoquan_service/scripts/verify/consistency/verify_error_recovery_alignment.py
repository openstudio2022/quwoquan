...
def discover_factory_objects() -> dict[str, dict[str, list[str]]]:
    ...
    services_root = require_scan_root(
        repo_root / "quwoquan_service/services", "error-recovery-alignment services"
    )
    go_paths = require_nonempty(
        sorted(services_root.glob("*/generated/*/*/errors.go")),
        "error-recovery-alignment 对象级 errors.go",
        root=services_root,
    )
...
def main() -> int:
    ...
    except ScanRootUnusable as error:
        print(f"verify_error_recovery_alignment: GATE_BLOCK: {error}", file=sys.stderr)
        return 1
    if not factory_objects:
        print(
            "verify_error_recovery_alignment: GATE_BLOCK: 未发现对象级 errors.go；"
            "空扫描不构成通过证据",
            file=sys.stderr,
        )
        return 1
