#!/usr/bin/env python3
"""`quwoquan_service/scripts/verify/**` 的 root discovery bootstrap。

推导算法只有一份，位于 `quwoquan_ops/cli/lib/repository_root.py`。本文件是极薄转发层：
在「还不知道仓库根、因此还不能 import 包」的前提下按物理路径找到那份实现并转发它的符号，
自身不复制任何判定逻辑。

本目录的 verifier 分布在 `verify/<theme>/` 主题口袋里，历史上各自用
`Path(__file__).resolve().parents[N]` 推导根；主题口袋这一层目录加进来之后 `N` 全部失配，
扫描根落到 `quwoquan_service/scripts/**`，扫描结果为空却报告通过。改用本模块后，
根由标记判定而非目录层数决定。
"""
