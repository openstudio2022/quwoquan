#!/usr/bin/env python3
"""阻断 Registry/ContractGraph 版本信封、版本目录和交接 hash 漂移。"""
...
def main() -> int:
    ...
    registry_inputs = sorted(METADATA.glob("*/business_object_map.yaml"))
    readiness_inputs = sorted(METADATA.glob("*/*/readiness.yaml"))
    for path in registry_inputs + readiness_inputs:
    ...
    if failures:
        for failure in failures:
            print(f"[registry-contract-graph-single-track] FAIL: {failure}")
        return 1
    print(
        "[registry-contract-graph-single-track] OK: "
        f"registries={len(registry_inputs)}, readiness={len(readiness_inputs)}, "
        f"graph={graph_sha}"
    )
    return 0
