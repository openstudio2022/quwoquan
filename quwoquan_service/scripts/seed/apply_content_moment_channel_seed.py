...
createdAt 递减唯一且为全库最新，使推荐频道首刷多形态非空、连续下拉≥2 页曝光不重复。
...
用法（gamma-local）：
  python3 quwoquan_service/scripts/seed/apply_content_moment_channel_seed.py \
    --container quwoquan_service-mongodb-1 --redis-container quwoquan_service-redis-1 --db quwoquan_content \
    --report artifacts/home-rec-phase9/t4-patrol/seed_apply_report.json
"""
...
def build_docs(fixture: dict) -> list[dict]:
    base = _parse_base(fixture["createdAtBase"])
