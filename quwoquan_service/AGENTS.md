...
- 视图只投影契约文档。`_shared/test_fixtures` 是 canonical 树里唯一同时存放契约文档与「文档所描述的媒体字节」的地方：descriptor/manifest 等 `.yaml/.yml/.json` 是编译器真实读取集（`collectSourceDigests` 会摘要它们），而 jpg/png/mp4 载荷没有任何视图消费方——metadata loader 三处 WalkDir 都对 `test_fixtures` SkipDir，真正读字节的 gate 与环境脚本一律直接读 canonical 路径。载荷的 sha256 已记在随行文档里，视图保留文档即传递性钉住载荷，不必逐份拷贝（曾让单视图 680MB 中 670MB 是死重量）。新增 fixture 文件类型时按「只放行文档后缀」判断，不要改成屏蔽已知媒体后缀。
...
- nil 只表达「缺席」，不表达「失败」也不表达「空集合」：失败经 error 返回，领域端口的未命中用 sentinel 或 `AppError`，不用 `return nil, nil` 兼作未命中信号。出站列表必须序列化为 `[]`，HTTP wire 边界的值类型 `bool` 禁止 `omitempty`（`false` 会整个从 wire 消失，而端侧对必填 bool 是 fail-closed 校验，键一消失就是解码错误）；确需区分「未设置」与「false」时用 `*bool`。指针只用于 `infrastructure` 的可空列与需要三态的写入面（change set 范式），`domain`/`application`/出站 DTO 的可选标量用值类型。
- 字段可空性只由对象契约声明（`fields.yaml` 的 `NOT_NULL` / `NULLABLE`、投影 `nullable`，或 wire schema 的 `required` / `default`），新增字段必须显式声明；生成器不得补入契约未声明的默认值，也不得对未声明可空性的字段自行推定可空。必填字段缺失必须解码失败，该行为由 `local_contract` 锁定。
