...
// Facade 提供 IaC 配置只读快照查询。配置唯一真相源是服务自治环境入口生成的发布包；
// 平台不提供任何写路径。
type Facade struct {
...
func (f *Facade) GetSnapshot(_ context.Context, environment, service string) (ConfigSnapshotView, error) {
	...
	default:
		...
		releases, err := f.source.ReleaseFiles(environment, service)
		if err != nil {
			return ConfigSnapshotView{}, err
		}
		...
		for _, release := range releases {
			...
			if err := yaml.Unmarshal([]byte(release.Content), &document); err == nil {
				if config, ok := document["config"].(map[string]any); ok {
					if version := strings.TrimSpace(fmt.Sprint(config["version"])); version != "" {
						view.ReleaseVersions = append(view.ReleaseVersions, version)
					}
				}
			}
		}
	...
	return ConfigDomainSlice{Items: []ConfigDomainItem{
		{
			...
			Label:       "云侧领域服务",
			Services:    services,
			Description: "服务 config schema + 四环境差异生成的唯一只读有效配置",
		},
		{
			Domain:      "app",
			Label:       "端侧 App",
