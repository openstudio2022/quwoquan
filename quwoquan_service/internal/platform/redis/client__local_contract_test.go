...
func TestTrackedConsumerGroupSelfHealsAfterServerStateLoss(t *testing.T) {
	server := miniredis.RunT(t)
	raw := goredis.NewClient(&goredis.Options{Addr: server.Addr()})
	instance := &client{raw: raw}
	t.Cleanup(func() { _ = instance.Close() })
	...
	// FLUSHDB drops the stream and its groups server-side while the
	// in-process ensure cache still remembers them; the next read must heal
	// instead of failing with NOGROUP (TestMain FlushDBs between tests,
	// production failover).
	server.FlushDB()
	...
	if err != nil || len(messages) != 0 {
		t.Fatalf(
			"read after flush messages=%v err=%v, want healed empty read",
			messages,
			err,
		)
	}
	if _, err := instance.XAdd(
		ctx, stream, map[string]string{"kind": "post-flush"},
	); err != nil {
	...
	if err != nil || len(messages) != 1 ||
		messages[0].Values["kind"] != "post-flush" {
		t.Fatalf("read after heal messages=%v err=%v", messages, err)
	}
}

func TestUntrackedConsumerGroupStillSurfacesTypedNoGroup(t *testing.T) {
	server := miniredis.RunT(t)
	raw := goredis.NewClient(&goredis.Options{Addr: server.Addr()})
	instance := &client{raw: raw}
	t.Cleanup(func() { _ = instance.Close() })
	ctx := context.Background()
	const stream = "events:untracked"
	if _, err := raw.XAdd(ctx, &goredis.XAddArgs{
		Stream: stream,
		Values: map[string]interface{}{"kind": "seed"},
	}).Result(); err != nil {
		t.Fatal(err)
	}
	if _, err := instance.XReadGroup(
		ctx, "never-ensured", "consumer", map[string]string{stream: ">"}, 10, 0,
	); !isNoGroupError(err) {
		t.Fatalf("untracked group read error=%v, want typed NOGROUP", err)
	}
}
