...
func TestDerivedEvidenceGeneratedBoundaryAndFixedPoint(t *testing.T) {
	t.Parallel()
	repo := newSyntheticRepo(t, ast.ObjectKindAggregateRoot)
	appRoot := "quwoquan_app/lib/service/demo_service/demo_context/demo_object/domain/"
	testRoot := "quwoquan_app/test/local_contract/service/demo_service/demo_context/demo_object/"
	header := "// Code generated by tools/codegen_app_metadata from demo/schema.yaml. DO NOT EDIT.\n"
	generated := map[string]string{
		appRoot + "generated/model.g.dart":      header + "class Model {}\n",
		appRoot + "plain.dart":                  header + "class Plain {}\n",
		testRoot + "generated/bridge_test.dart": header + "void main() {}\n",
	}
	controls := map[string]string{
		appRoot + "generated/handwritten.dart":       "class Handwritten {}\n",
		appRoot + "generated/no_header.g.dart":       "class NoHeader {}\n",
		appRoot + "generated/unregistered.g.dart":    header + "class Unregistered {}\n",
		appRoot + "generated/stale.g.dart":           header + "class Changed {}\n",
		testRoot + "generated/handwritten_test.dart": "void main() {}\n",
	}
	for path, body := range controls {
		repo.write(t, filepath.Join(repo.root, path), body)
	}
	// 来源登记复用现有 code-health policy，不按 generated 目录或文件后缀豁免。
	repo.write(t, filepath.Join(repo.root, "quwoquan_ops/policies/code_health_policy.yaml"),
		"classification:\n  generated_manifests:\n"+
			"    - {path: quwoquan_app/tool/cloud_codegen/generated_manifest.json, root: quwoquan_app, generator: app-only-emitter}\n")
	emit := func(graphDigest string) {
		t.Helper()
		outputs := []map[string]string{}
		for path, body := range generated {
			body += "// graph: " + graphDigest + "\n"
			repo.write(t, filepath.Join(repo.root, path), body)
			sum := sha256.Sum256([]byte(body))
			outputs = append(outputs, map[string]string{"path": strings.TrimPrefix(path, "quwoquan_app/"), "sha256": hex.EncodeToString(sum[:])})
		}
		for _, name := range []string{"no_header.g.dart", "stale.g.dart"} {
			path := appRoot + "generated/" + name
			sum := sha256.Sum256([]byte(controls[path]))
			digest := hex.EncodeToString(sum[:])
			if name == "stale.g.dart" {
				digest = strings.Repeat("0", 64)
			}
			outputs = append(outputs, map[string]string{"path": strings.TrimPrefix(path, "quwoquan_app/"), "sha256": digest})
		}
		body, err := json.Marshal(map[string]any{"generator": "app-only-emitter", "contractGraphSha256": graphDigest, "outputs": outputs})
		if err != nil {
			t.Fatal(err)
		}
		repo.write(t, filepath.Join(repo.root, "quwoquan_app/tool/cloud_codegen/generated_manifest.json"), string(body))
	}
	compile := func() string {
		t.Helper()
		catalog := repo.load(t, load.WithRepoRoot(repo.root))
		evidence := onlyEvidence(t, catalog)
		paths := map[string]bool{}
		for _, artifact := range append(evidence.App.Domain, evidence.App.LocalContract...) {
			paths[artifact.Path] = true
		}
		for path := range generated {
			if paths[path] {
				t.Errorf("generated output became graph input: %s", path)
			}
		}
		for path := range controls {
			if !paths[path] {
				t.Errorf("unverified or handwritten input was excluded: %s", path)
			}
		}
		body, err := json.Marshal(graph.Build(catalog))
		if err != nil {
			t.Fatal(err)
		}
	...
	second := compile()
	emit(second)
	if third := compile(); first != second || second != third {
		t.Fatalf("graph -> derived App output did not reach a fixed point: %s / %s / %s", first, second, third)
	}
	for path, body := range controls {
		repo.write(t, filepath.Join(repo.root, path), body+"// real input changed\n")
		if compile() == second {
			t.Errorf("actual input drift was ignored: %s", path)
		}
		repo.write(t, filepath.Join(repo.root, path), body)
	}
	manifestPath := filepath.Join(repo.root, "quwoquan_app/tool/cloud_codegen/generated_manifest.json")
	for _, invalid := range []string{"not-json", `{"generator":"another-emitter","outputs":[]}`} {
		repo.write(t, manifestPath, invalid)
		evidence := onlyEvidence(t, repo.load(t, load.WithRepoRoot(repo.root)))
		if !strings.Contains(strings.Join(artifactPaths(evidence.App.Domain), "\n"), appRoot+"generated/model.g.dart") {
			t.Fatal("unverified manifest must not exclude a source")
		}
	}
}
