...
func (generator *DomainGenerator) buildObjectTemplateData(
	...
	for _, entityName := range entityNames {
		...
		for _, field := range entity.Fields {
			if field.GoTypeRef != "" {
				if field.GoImport == "" {
					return domainTemplateData{}, fmt.Errorf("%s.%s declares go_type_ref without go_import", entityName, field.Name)
				}
				importSet[field.GoImport] = struct{}{}
			}
		}
...
func (generator *DomainGenerator) fieldGoType(
	...
	if goTypeRef := strings.TrimSpace(field.GoTypeRef); goTypeRef != "" {
		if !regexp.MustCompile(`^[A-Za-z_][A-Za-z0-9_]*\.[A-Za-z_][A-Za-z0-9_]*$`).MatchString(goTypeRef) {
			return "", fmt.Errorf("invalid go_type_ref %q", goTypeRef)
		}
		if strings.TrimSpace(field.GoImport) == "" {
			return "", fmt.Errorf("go_type_ref %q requires go_import", goTypeRef)
		}
		return goTypeRef, nil
	}
