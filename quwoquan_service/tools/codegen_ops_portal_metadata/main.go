...
func main() {
	...
	for _, domain := range []string{"product", "platform"} {
		...
		if source.Has(configPath) {
			...
			configIndex := filepath.Join(outDir, fmt.Sprintf("%sConfig.generated.ts", domain))
			writeConfigTSModules(configIndex, domain+"Config", data)
...
const configChunkSize = 150

func writeConfigTSModules(indexPath, varName string, data configFile) {
	dir := filepath.Dir(indexPath)
	base := strings.TrimSuffix(filepath.Base(indexPath), ".generated.ts")
	imports := make([]string, 0, (len(data.Configs)+configChunkSize-1)/configChunkSize)
	spreads := make([]string, 0, cap(imports))
	for start, chunk := 0, 0; start < len(data.Configs); start, chunk = start+configChunkSize, chunk+1 {
		end := start + configChunkSize
		if end > len(data.Configs) {
			end = len(data.Configs)
		}
		chunkName := fmt.Sprintf("%sChunk%d", varName, chunk)
		chunkFile := fmt.Sprintf("%s.chunk%d.generated.ts", base, chunk)
		writeTSModule(filepath.Join(dir, chunkFile), chunkName, data.Configs[start:end])
		imports = append(imports, fmt.Sprintf("import { %s } from './%s';", chunkName, strings.TrimSuffix(chunkFile, ".ts")+".js"))
		spreads = append(spreads, "    ..."+chunkName)
	}
	content := strings.Join(imports, "\n") + "\n\n" +
		fmt.Sprintf("// Code generated by codegen_ops_portal_metadata. DO NOT EDIT.\n\nexport const %s = {\n  version: %d,\n  configs: [\n%s\n  ],\n} as const;\nexport type %s = typeof %s;\n", varName, data.Version, strings.Join(spreads, ",\n"), toPascalCase(varName), varName)
