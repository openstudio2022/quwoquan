...
func main() {
	...
	pythonOutput := flag.String("python-output", "services/recommendation-service/generated/recommendation/recommendation_feature_profile_view/intersection_policy.py", "生成 Python 不可变策略的输出文件")
	flag.Parse()
	...
	policyDigest, err := registryPolicyDigest(source, contractPath)
	if err != nil {
		fail(err)
	}
	out := renderGoTable(contractPath, registry, policyDigest)
	...
	pythonOut := renderPythonPolicy(contractPath, registry, policyDigest)
	if err := os.MkdirAll(filepath.Dir(*pythonOutput), 0o755); err != nil {
		fail(err)
	}
	if err := os.WriteFile(*pythonOutput, []byte(pythonOut), 0o644); err != nil {
		fail(err)
	}
	fmt.Printf("[codegen_rec_intersection] wrote %s and %s (%d kinds)\n", outPath, *pythonOutput, len(registry.Kinds))
}
...
// registryPolicyDigest 取注册表在 ContractGraph 里登记的源文件 sha256（documents[].sha256，
// 与 contract_graph.json 同一身份）作为交集策略身份（cohort）。注册表任何字节变化
// （kind、行动阶梯、模板、阈值）都产生新的策略版本，漏斗事件据此分桶。
func registryPolicyDigest(source *contractcodegen.Source, contractPath string) (string, error) {
	for _, document := range source.Graph().Documents {
		if document.Path == contractPath {
			if len(document.SHA256) != sha256.Size*2 {
				return "", fmt.Errorf("intersection kind registry digest is malformed: %q", document.SHA256)
			}
			if _, err := hex.DecodeString(document.SHA256); err != nil {
				return "", fmt.Errorf("intersection kind registry digest is not hex: %w", err)
			}
			return "sha256:" + document.SHA256, nil
		}
	}
	return "", fmt.Errorf("intersection kind registry %s not found in ContractGraph documents", contractPath)
}

func renderGoTable(sourcePath string, r *recintersectionmeta.Registry, policyDigest string) string {
	...
	// ── 策略身份（cohort）──
	b.WriteString("// IntersectionPolicyDigest 是交集策略身份：注册表 canonical 字节的 sha256。\n")
	b.WriteString("// Content 水合出口把它写入 IntersectionReason.cohort，端在曝光→点击→转化→清零\n")
	b.WriteString("// 全链原样回传（intersectionCohort），使漏斗可按策略版本（灰度 cohort）下钻；不是用户分桶。\n")
	b.WriteString(fmt.Sprintf("const IntersectionPolicyDigest = %q\n\n", policyDigest))
	...
	b.WriteString("// IntersectionWireObjectTypeByObjectKind: objectKind → canonical wire objectType\n")
	b.WriteString("// （registry.objectKinds[].objectType）。IntersectionTarget.objectType 归一化只查此表，\n")
	b.WriteString("// 服务端不再按 routeId / kind 手写 switch，也不从 objectId 前缀反推。\n")
	b.WriteString("var IntersectionWireObjectTypeByObjectKind = map[string]string{\n")
	for _, ok := range r.ObjectKinds {
		if strings.TrimSpace(ok.ObjectType) == "" {
			continue
		}
		b.WriteString(fmt.Sprintf("\t%q: %q,\n", ok.Kind, ok.ObjectType))
	}
	b.WriteString("}\n\n")
...
func renderPythonPolicy(sourcePath string, r *recintersectionmeta.Registry, policyDigest string) string {
	var b strings.Builder
	b.WriteString("# Code generated by tools/codegen_rec_intersection. DO NOT EDIT.\n")
	b.WriteString("# Source: ")
	b.WriteString(filepath.ToSlash(sourcePath))
	b.WriteString("\nfrom types import MappingProxyType\n\n")
	b.WriteString("# 交集策略身份（cohort）：注册表 canonical 字节的 sha256，与 Go 表 IntersectionPolicyDigest 同源。\n")
	b.WriteString(fmt.Sprintf("INTERSECTION_POLICY_DIGEST = %q\n\n", policyDigest))

	b.WriteString("OBJECT_KIND_BY_OBJECT_TYPE = MappingProxyType({\n")
	bindings := append([]recintersectionmeta.ObjectTypeBinding(nil), r.ObjectTypeBindings...)
	...
	b.WriteString("ROUTE_ID_BY_OBJECT_KIND = MappingProxyType({\n")
	objectKinds := append([]recintersectionmeta.ObjectKindDef(nil), r.ObjectKinds...)
	sort.Slice(objectKinds, func(i, j int) bool { return objectKinds[i].Kind < objectKinds[j].Kind })
	for _, objectKind := range objectKinds {
		if strings.TrimSpace(objectKind.RouteID) == "" {
			continue
		}
		b.WriteString(fmt.Sprintf("    %q: %q,\n", objectKind.Kind, objectKind.RouteID))
	}
	b.WriteString("})\n\n")
	b.WriteString("WIRE_OBJECT_TYPE_BY_OBJECT_KIND = MappingProxyType({\n")
	for _, objectKind := range objectKinds {
		if strings.TrimSpace(objectKind.ObjectType) == "" {
			continue
		}
		b.WriteString(fmt.Sprintf("    %q: %q,\n", objectKind.Kind, objectKind.ObjectType))
	}
	b.WriteString("})\n\n")

	b.WriteString("ACTION_POLICY_BY_KEY = MappingProxyType({\n")
	for _, key := range sortedActionKeyMetaKeys(r.ActionKeyMeta) {
		meta := r.ActionKeyMeta[key]
		b.WriteString(fmt.Sprintf("    %q: MappingProxyType({\n", key))
		b.WriteString(fmt.Sprintf("        \"label\": %q,\n", r.ActionLabelByKey[key]))
		b.WriteString(fmt.Sprintf("        \"tier\": %q,\n", meta.Tier))
		b.WriteString("        \"required_gates\": (")
		for _, gate := range meta.RequiredGates {
			b.WriteString(fmt.Sprintf("%q, ", gate))
		}
		b.WriteString("),\n")
		b.WriteString(fmt.Sprintf("        \"dispatch\": %q,\n", meta.Dispatch))
		b.WriteString("    }),\n")
	}
	b.WriteString("})\n\n")

	b.WriteString("ACTION_KEYS_BY_KIND = MappingProxyType({\n")
	for _, kind := range sortedKeys(r.ActionHintsByKind) {
		b.WriteString(fmt.Sprintf("    %q: (", kind))
		for _, key := range r.ActionHintsByKind[kind] {
	...
	// 结论句模板与主语语法：Python 生产者按同一份模板渲染 primaryText / primarySpans /
	// primaryTextL10nKey，不再在服务代码里拼中文字面量。
	st := r.StatementTemplates
	b.WriteString("STATEMENT_FORM_BY_KIND = MappingProxyType({\n")
	for _, kind := range sortedKeys(st.ByKind) {
		form := st.ByKind[kind]
		b.WriteString(fmt.Sprintf("    %q: MappingProxyType({\n", kind))
		b.WriteString(fmt.Sprintf("        \"template\": %q,\n", form.Template))
		b.WriteString(fmt.Sprintf("        \"l10n_key\": %q,\n", form.L10nKey))
		if form.Counted != nil {
			b.WriteString(fmt.Sprintf(
				"        \"counted\": MappingProxyType({\"template\": %q, \"l10n_key\": %q}),\n",
				form.Counted.Template, form.Counted.L10nKey))
		}
		if len(form.Variants) > 0 {
			b.WriteString("        \"variants\": MappingProxyType({\n")
			for _, name := range sortedKeys(form.Variants) {
				variant := form.Variants[name]
				b.WriteString(fmt.Sprintf(
					"            %q: MappingProxyType({\"template\": %q, \"l10n_key\": %q}),\n",
					name, variant.Template, variant.L10nKey))
			}
			b.WriteString("        }),\n")
		}
		b.WriteString("    }),\n")
	...
	subjects := r.PresentationText.SubjectPatterns
	b.WriteString("SUBJECT_PATTERN_BY_NAME = MappingProxyType({\n")
	for _, name := range sortedKeys(subjects.Patterns) {
		b.WriteString(fmt.Sprintf(
			"    %q: MappingProxyType({\"text\": %q, \"l10n_key\": %q}),\n",
			name, subjects.Patterns[name],
			recintersectionmeta.L10nKeyFor(subjects.L10nKeyPrefix, name)))
	}
	b.WriteString("})\n\n")

	labels := r.PresentationText.RelationLabels
	b.WriteString("# 代表人关系限定词：kind → 称谓（registry.presentationText.relationLabels）；未登记取 RELATION_LABEL_DEFAULT。\n")
	b.WriteString("RELATION_LABEL_BY_KIND = MappingProxyType({\n")
	for _, kind := range sortedKeys(labels.ByKind) {
		b.WriteString(fmt.Sprintf(
			"    %q: MappingProxyType({\"text\": %q, \"l10n_key\": %q}),\n",
			kind, labels.ByKind[kind],
			recintersectionmeta.L10nKeyFor(labels.L10nKeyPrefix, kind)))
	}
	b.WriteString("})\n")
	b.WriteString(fmt.Sprintf("RELATION_LABEL_DEFAULT = %q\n", labels.Default))
	return b.String()
}
