...
func renderEventCatalogGo(catalog eventCatalogFile, pages appPagesFile) string {
	...
	b.WriteString("type EventExtensionDefinition struct { Type string; Minimum *int; Maximum *int; MaxLength int; MaxItems int; ItemMaxLength int; Sensitive bool }\n")
	...
	for _, name := range extensionNames {
		...
		b.WriteString(fmt.Sprintf("%q:{Type:%q,Minimum:%s,Maximum:%s,MaxLength:%d,MaxItems:%d,ItemMaxLength:%d,Sensitive:%t},\n", name, definition.Type, goIntPointer(definition.Minimum), goIntPointer(definition.Maximum), definition.MaxLength, definition.MaxItems, definition.ItemMaxLength, definition.Sensitive))
	}
	b.WriteString("}\n")
