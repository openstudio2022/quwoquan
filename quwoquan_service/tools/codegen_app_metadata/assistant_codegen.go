...
func generateAssistantRuntimeEnumsGo(
	metadataDir string,
	outputPath string,
	check bool,
) error {
	...
	content := []byte(renderAssistantRuntimeEnumsGo(enumCatalog))
	if check {
		existing, err := os.ReadFile(outputPath)
		if err != nil {
			return fmt.Errorf("read generated assistant runtime enums %s: %w", outputPath, err)
		}
		if string(existing) != string(content) {
			return fmt.Errorf(
				"generated assistant runtime enums are stale: %s; run make codegen-assistant-runtime-enums",
				outputPath,
			)
		}
		return nil
	...
	if err := os.MkdirAll(filepath.Dir(outputPath), 0o755); err != nil {
		return fmt.Errorf("create assistant runtime enum output directory: %w", err)
	}
	if err := os.WriteFile(outputPath, content, 0o644); err != nil {
		return fmt.Errorf("write generated assistant runtime enums %s: %w", outputPath, err)
	}
	return nil
}
...
func renderAssistantRuntimeEnumsDart(catalog *assistantEnumCatalog) string {
	...
	b.WriteString("final class AssistantRuntimeEnumParseFailure implements FormatException {\n")
	b.WriteString("  const AssistantRuntimeEnumParseFailure(this.enumName, this.wireValue);\n\n")
	b.WriteString("  final String enumName;\n  final String wireValue;\n\n")
	b.WriteString("  @override\n")
	b.WriteString("  String get message => 'Unknown $enumName wire value: $wireValue';\n\n")
	b.WriteString("  @override\n")
	b.WriteString("  dynamic get source => wireValue;\n\n")
	b.WriteString("  @override\n")
	b.WriteString("  int? get offset => null;\n\n")
	b.WriteString("  @override\n")
	...
	for _, enumDef := range catalog.Enums {
		...
		b.WriteString(fmt.Sprintf("%s parse%sStrict(String raw) {\n", enumDef.Name, enumDef.Name))
		...
		b.WriteString(fmt.Sprintf("      throw AssistantRuntimeEnumParseFailure(%q, raw.trim());\n", enumDef.Name))
		b.WriteString("  }\n")
		b.WriteString("}\n\n")
...
func renderAssistantRuntimeEnumsGo(catalog *assistantEnumCatalog) string {
	var b strings.Builder
	b.WriteString("// Code generated by tools/codegen_app_metadata from assistant/_shared/enums.yaml. DO NOT EDIT.\n\n")
	b.WriteString("package generated\n\n")
	b.WriteString("import (\n\t\"fmt\"\n\t\"strings\"\n)\n\n")
	for _, enumDef := range catalog.Enums {
		if strings.TrimSpace(enumDef.Name) == "" {
			continue
		}
		...
		for _, value := range enumDef.Values {
			fmt.Fprintf(
				...
				"\t%s%s %s = %q\n",
				enumDef.Name,
				strings.ToUpper(value.Name[:1])+value.Name[1:],
				enumDef.Name,
				value.Wire,
			)
		}
		...
		fmt.Fprintf(
			&b,
			"func Parse%s(raw string) (%s, error) {\n",
			enumDef.Name,
			enumDef.Name,
		)
		b.WriteString("\tswitch strings.TrimSpace(raw) {\n")
		for _, value := range enumDef.Values {
			fmt.Fprintf(
				&b,
				"\tcase %q:\n\t\treturn %s%s, nil\n",
		...
		fmt.Fprintf(
			&b,
			"\tdefault:\n\t\treturn \"\", fmt.Errorf(\"unknown %s wire value %%q\", raw)\n\t}\n}\n\n",
			enumDef.Name,
		)
...
func validateAssistantEnumDefaults(catalog *assistantEnumCatalog) error {
	for _, enumDef := range catalog.Enums {
		...
		for _, value := range enumDef.Values {
			if owner, exists := wireOwners[value.Wire]; exists {
				return fmt.Errorf(
					"assistant enum %s has duplicate wire value %q for %s and %s",
					enumDef.Name,
					value.Wire,
					owner,
					value.Name,
...
func renderAssistantPreferenceSnapshotDart(schema *assistantSimpleSchema) string {
	var b strings.Builder
	b.WriteString("// Code generated by tools/codegen_app_metadata from assistant/assistant_preference_snapshot/schema.yaml. DO NOT EDIT.\n")
	b.WriteString("// ignore_for_file: prefer_const_constructors\n\n")
	b.WriteString("class AssistantPreferenceSnapshot {\n")
	b.WriteString("  const AssistantPreferenceSnapshot({\n")
	b.WriteString("    required this.preferenceId,\n    required this.scope,\n    required this.kind,\n    required this.value,\n")
	b.WriteString("    this.sourceType = '',\n    this.sourceSessionId = '',\n    this.confirmedAt = '',\n")
	b.WriteString("    this.version = 0,\n  });\n\n")
	b.WriteString("  final String preferenceId;\n  final String scope;\n  final String kind;\n  final String value;\n")
	b.WriteString("  final String sourceType;\n  final String sourceSessionId;\n  final String confirmedAt;\n")
	b.WriteString("  final int version;\n\n")
	b.WriteString("  Map<String, dynamic> toJson() => <String, dynamic>{\n")
	b.WriteString("    'preferenceId': preferenceId,\n    'scope': scope,\n    'kind': kind,\n    'value': value,\n")
	b.WriteString("    'sourceType': sourceType,\n    'sourceSessionId': sourceSessionId,\n")
	b.WriteString("    'confirmedAt': confirmedAt,\n    'version': version,\n  };\n\n")
	b.WriteString("  factory AssistantPreferenceSnapshot.fromJson(Map<String, dynamic> json) {\n")
	b.WriteString("    return AssistantPreferenceSnapshot(\n")
	b.WriteString("      preferenceId: (json['preferenceId'] as String?)?.trim() ?? '',\n")
	b.WriteString("      scope: (json['scope'] as String?)?.trim() ?? '',\n")
	b.WriteString("      kind: (json['kind'] as String?)?.trim() ?? '',\n")
	b.WriteString("      value: (json['value'] as String?)?.trim() ?? '',\n")
	b.WriteString("      sourceType: (json['sourceType'] as String?)?.trim() ?? '',\n")
	...
	b.WriteString("      version: (json['version'] as num?)?.toInt() ?? 0,\n")
	b.WriteString("    );\n")
	b.WriteString("  }\n")
	b.WriteString("}\n")
	return b.String()
...
func assistantEnumDefault(name string) string {
	...
	case "EvidenceStatus":
		return "unknown"
	case "ContextScopeExpansionPolicy":
		return "none"
	default:
		return "unknown"
	}
