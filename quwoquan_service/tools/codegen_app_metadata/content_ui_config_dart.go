...
func feedQueryDartLiteral(values map[string]string) string {
	if len(values) == 0 {
...
func renderContentMediaPlaybackPolicyDart(uc *uiConfigFile) (string, error) {
	const adaptivePlaybackFlag = "enable_hls_cmaf_abr"
	for _, flag := range uc.FeatureFlags {
		if strings.TrimSpace(flag.Flag) != adaptivePlaybackFlag {
			continue
		}
		var b strings.Builder
		b.WriteString("// Code generated by tools/codegen_app_metadata from content/content/post/ui_config.yaml. DO NOT EDIT.\n\n")
		b.WriteString("/// Media Asset 消费 Post 播放策略时使用的稳定公开边界。\n")
		b.WriteString("abstract final class ContentMediaPlaybackPolicy {\n")
		b.WriteString("  const ContentMediaPlaybackPolicy._();\n\n")
		fmt.Fprintf(
			&b,
			"  static const String adaptivePlaybackFeatureFlag = %s;\n",
	...
	return "", fmt.Errorf(
		"content media playback policy requires feature flag %q",
		adaptivePlaybackFlag,
	)
}
...
func renderContentUIConfigDart(uc *uiConfigFile) string {
	...
	b.WriteString("class OnboardingInterestDimensionConfig {\n")
	b.WriteString("  final String id;\n")
	b.WriteString("  final String tagRef;\n")
	b.WriteString("  final String displayLabel;\n")
	b.WriteString("  final int minSelections;\n")
	b.WriteString("  final int maxSelections;\n")
	b.WriteString("  final int order;\n\n")
	b.WriteString("  const OnboardingInterestDimensionConfig({\n")
	b.WriteString("    required this.id,\n")
	b.WriteString("    required this.tagRef,\n")
	b.WriteString("    required this.displayLabel,\n")
	b.WriteString("    required this.minSelections,\n")
	b.WriteString("    required this.maxSelections,\n")
	b.WriteString("    required this.order,\n")
	b.WriteString("  });\n")
	b.WriteString("}\n\n")
	...
	onboardingDimensions := append(
		[]onboardingInterestDimensionDef(nil),
		uc.OnboardingInterestCatalog.Dimensions...,
	)
	sort.Slice(onboardingDimensions, func(i, j int) bool {
		return onboardingDimensions[i].Order < onboardingDimensions[j].Order
	})
	...
	for _, dimension := range onboardingDimensions {
		b.WriteString(fmt.Sprintf("      OnboardingInterestDimensionConfig(id: %s, tagRef: %s, displayLabel: %s, minSelections: %d, maxSelections: %d, order: %d),\n",
			dartStringLiteral(dimension.ID),
			dartStringLiteral(dimension.TagRef),
			dartStringLiteral(dimension.DisplayLabel),
			dimension.MinSelections,
			dimension.MaxSelections,
			dimension.Order))
	}
	b.WriteString("    ],\n")
	b.WriteString("  );\n\n")
