...
func parseRuntimeLogs(raw string) ([]map[string]any, error) {
...
func TestHTTPServerMiddleware_EmitIOProcessException(t *testing.T) {
	...
	errorLogs, err := parseRuntimeLogs(errorBuf.String())
	if err != nil {
	...
	attrs, _ := errorLogs[0]["attributes"].(map[string]any)
	if attrs["module"] != "CHAT" ||
		attrs["kind"] != "SYSTEM" ||
		attrs["reason"] != "message_persist_failed" {
		t.Fatalf("expected canonical runtime attributes, got=%+v", errorLogs[0])
	}
}
...
func TestUAT_CorrelationAcrossThreeLogs(t *testing.T) {
	...
	standardLogs, _ := parseRuntimeLogs(standard.String())
	errorLogs, _ := parseRuntimeLogs(errorBuf.String())
	if len(standardLogs) == 0 || len(errorLogs) == 0 {
		t.Fatalf("expected both standard and error logs")
	}

	for _, obj := range standardLogs {
		correlation, _ := obj["correlation"].(map[string]any)
		if correlation["traceId"] != traceID || correlation["requestId"] != requestID {
			t.Fatalf("standard log correlation mismatch: %+v", obj)
		}
		if _, ok := correlation["sessionId"]; ok {
