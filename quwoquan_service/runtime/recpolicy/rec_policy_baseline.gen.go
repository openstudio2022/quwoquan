...
intersection:
  ...
  cooldownDays: 14
  # 交集负反馈冷却窗口天数（F 推荐差异化）：用户对交集条目显式负反馈（feedbackKinds：
  # notInterested / dismiss / rejectGreeting / leaveCircle）后，窗口内对该 subject
  # 「过滤不再推荐」（比曝光冷却更强）。默认 30 天 > 曝光 14 天；唯一 key 真相源
  # _shared/redis_keyspace.yaml: rec:ineg（TTL 60 天兜底）。
