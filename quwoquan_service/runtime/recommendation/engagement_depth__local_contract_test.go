...
func TestPublishResultReferralDoesNotTrainCreatorInterest(t *testing.T) {
	if got := computeEffectiveTagWeight(2, 0, "publish_result"); got != 0 {
		t.Fatalf("publish_result must be attribution-only, got effective weight %v", got)
	}
}

func TestEntityPageViewSignalWeightMatchesMetadata(t *testing.T) {
	if got := SignalWeights["entity_page_view"]; got != 1.5 {
		t.Fatalf("entity_page_view weight drift: got %v want 1.5", got)
	}
}
