...
func TestHTTPStatusFromErrorSupportsMetadataUserSubKinds(t *testing.T) {
	cases := []struct {
		...
		{name: "persona missing", raw: "USER.PERSONA.not_found", want: http.StatusNotFound},
		{name: "retired persona", raw: "USER.PERSONA.retired_guard", want: http.StatusBadRequest},
		{name: "persona handle taken", raw: "USER.PERSONA.handle_taken", want: http.StatusConflict},
