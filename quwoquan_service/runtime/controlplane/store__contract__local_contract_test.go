...
func TestFileStorePersistsDocumentsWorkflowAndAudit(t *testing.T) {
	store := testsupport.NewFileStore(filepath.Join(t.TempDir(), "control-plane.json"))
...
func TestPutDocumentIfAbsentPreservesFirstImmutableFact(t *testing.T) {
	store := testsupport.NewFileStore(filepath.Join(t.TempDir(), "control-plane.json"))

	first, inserted, err := store.PutDocumentIfAbsent(
		"experiment_assignment_facts",
		"assignment-1",
		Document{"id": "assignment-1", "variant": "control"},
	)
	if err != nil || !inserted || first["variant"] != "control" {
		t.Fatalf("first append: inserted=%v doc=%#v err=%v", inserted, first, err)
	}

	replayed, inserted, err := store.PutDocumentIfAbsent(
		"experiment_assignment_facts",
		"assignment-1",
		Document{"id": "assignment-1", "variant": "treatment"},
	)
	if err != nil || inserted {
		t.Fatalf("replay: inserted=%v err=%v", inserted, err)
	}
	if replayed["variant"] != "control" {
		t.Fatalf("append-only fact was overwritten: %#v", replayed)
	}
}
