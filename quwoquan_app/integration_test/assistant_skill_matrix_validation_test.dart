...
void main() {
  ...
  testWidgets(
    'simulator validates all 21 skills with multi-intent narrative quality',
    (tester) async {
      final skills = await const PersonalAssistantSkillLoader()
          .loadBundledSkills();
      final domainSkills = skills
          .where((skill) => skill.domainId.trim().isNotEmpty)
          .toList(growable: false);
      final skillsByDomain = <String, dynamic>{
        for (final skill in domainSkills) skill.domainId: skill,
      };
      final loadedDomains = skillsByDomain.keys.toList()..sort();
      final loadedSkillDebug = skills
          .map(
            (skill) => <String, String>{
              'id': skill.id,
      ...
      final router = AssistantDomainRouter();
      final routedDomains = await router.availableDomains(forceRefresh: true);
      final skillCatalogPrompt = await router.buildSkillCatalogPrompt();

      final toolRegistry = ToolMetadataRegistry();
      await toolRegistry.ensureLoaded();
      ...
      for (final skillCase in _skillCases) {
        ...
        expect(
          skill,
          isNotNull,
          reason: '${skillCase.domain} should be bundled',
        );

        final manifestErrors = skill.validate();
        expect(
          manifestErrors,
          isEmpty,
          reason: '${skillCase.domain} manifest must be valid',
        );
        expect(skill.description.trim(), isNotEmpty);
        expect(skill.skillInstructionMarkdown.trim(), isNotEmpty);
        expect(skill.allowedTools, isNotEmpty);
        expect(skill.executionShell.problemClass, isNotEmpty);

        expect(routedDomains, contains(skillCase.domain));
        expect(skillCatalogPrompt, contains('- ${skillCase.domain}:'));

        final matrixTools = toolRegistry.availableToolsForDomain(
          domainId: skillCase.domain,
          fallbackNames: const <String>[],
        );
        expect(matrixTools.toSet(), equals(skill.allowedTools.toSet()));

        final singleScript = await dialogueRuntime.buildRoundScript(
          domainId: skillCase.domain,
          userQuery: skillCase.singleQuery,
          contextScopeHint: const <String, dynamic>{},
          forceRefreshCatalog: true,
        );
        expect(singleScript.enabled, isTrue);
        expect(singleScript.domainId, equals(skillCase.domain));
        expect(singleScript.currentStateId.trim(), isNotEmpty);
        expect(singleScript.suggestedNextStateId.trim(), isNotEmpty);
        expect(
          singleScript.statePromptExcerpt.trim().isNotEmpty ||
              singleScript.stateMachineExcerpt.trim().isNotEmpty,
          isTrue,
          reason: '${skillCase.domain} should expose dialogue state guidance',
        );

        final singleRun = _buildValidatedRun(
          skillCase: skillCase,
          primarySkillName: skill.name,
          primaryDescription: skill.description,
        );
        ...
        report.add(<String, dynamic>{
          ...
          'secondaryDomain': skillCase.secondaryDomain,
          'state': singleScript.suggestedNextStateId,
          'tools': matrixTools,
