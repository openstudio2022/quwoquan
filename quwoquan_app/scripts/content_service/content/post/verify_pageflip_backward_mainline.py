...
UI_PAGEFLIP_DIRS = [
    APP_LIB / "design_system" / "pageflip",
    APP_LIB
    / "service"
    / "content_service"
    / "content"
    ...
    APP_TEST / "support" / "runtime" / "pageflip",
]
...
PAGEFLIP_WIDGET_TEST_PATH = (
    ...
    / "design_system"
    / "pageflip"
    / "pageflip_widget__local_contract_test.dart"
)
RENDER_FRAME_PATH = APP_LIB / "design_system" / "pageflip" / "render_frame.dart"
GEOMETRY_PATH = APP_LIB / "design_system" / "pageflip" / "geometry.dart"
RENDER_FRAME_BUILDER_PATH = (
    APP_LIB / "design_system" / "pageflip" / "backward_render_frame_builder.dart"
)
OLD_BACKWARD_LEAF_RENDERER_PATH = (
    APP_LIB / "design_system" / "pageflip" / "backward_leaf_renderer.dart"
)
...
def _check_recto_verso_split_in_host() -> list[str]:
    ...
    if split_body is None:
        ...
        for marker in (
            "backwardLeafFrame.sheetRectoCoverageNormalized",
        ...
        if "resolveBackwardCanonicalSheetFaces" not in diagnostic:
            violations.append(
                f"{diagnostic_path.relative_to(ROOT)}: diagnostics must consume "
                "the same canonical moving-sheet face resolver as paint."
            )
...
def _check_backward_texture_binding() -> list[str]:
    violations: list[str] = []
    snapshot_path = APP_LIB / "design_system" / "pageflip" / "page_surface_snapshot.dart"
