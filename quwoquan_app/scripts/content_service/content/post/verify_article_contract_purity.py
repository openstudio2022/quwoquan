...
POST_DTO_FILES = [
    ROOT
    / "quwoquan_app/lib/service/content_service/content/post/application/public/content_post_view_data.dart",
    ROOT
    / "quwoquan_app/packages/quwoquan_cloud_contracts/lib/src/content/content_operation_contracts.g.dart",
]
...
def main() -> int:
    ...
    for path in POST_DTO_FILES:
        if not path.exists():
            failures.append(f"missing canonical post projection: {path.relative_to(ROOT)}")
            continue
    ...
    if not ARTICLE_SURFACE_VIEW_MAPPER.exists():
        failures.append(
            f"{ARTICLE_SURFACE_VIEW_MAPPER.relative_to(ROOT)}: canonical article mapper missing"
        )
    else:
        wire_map = ARTICLE_SURFACE_VIEW_MAPPER.read_text(encoding="utf-8")
        if (
