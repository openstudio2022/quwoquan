#!/usr/bin/env python3
"""Report Material API surface usage under canonical App presentation roots.

Owner: design_system (manual analysis tool; not a gate).
Input: quwoquan_app/lib presentation roots.
Output (ephemeral under .qwq_output):
  - env/repo/runs/feature-tree/material_leaks.yaml
  - env/repo/runs/feature-tree/material_leaks.md
Write behavior: only writes disposable .qwq_output reports; no tracked inventory.

Re-run: python3 quwoquan_app/scripts/tools/design_system/scan_material_leaks.py
"""
...
def _scopes() -> tuple[tuple[str, Path], ...]:
    presentation = tuple(
        ("presentation", path)
        for path in sorted(APP_LIB.glob("service/*/*/*/presentation"))
    )
    return presentation + (
        ("runtime_shell", APP_LIB / "runtime/shell"),
        ("design_system", APP_LIB / "design_system"),
    )
...
def main() -> int:
    ...
    yl: list[str] = [
        "# Auto-generated by quwoquan_app/scripts/tools/design_system/scan_material_leaks.py — do not hand-edit.",
    ...
    md: list[str] = [
        "# Canonical App presentation Material 泄露清单",
        "",
        f"- **生成时间（UTC）**：{now}",
        "- **范围**：对象 `presentation`、`runtime/shell`与 `design_system` 下全部 `.dart`",
        "- **复跑**：`python3 quwoquan_app/scripts/tools/design_system/scan_material_leaks.py`",
