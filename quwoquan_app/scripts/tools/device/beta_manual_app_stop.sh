...
usage() {
  cat <<EOF
Usage:
  quwoquan_app/scripts/tools/device/beta_manual_app_stop.sh [options]
...
if [[ "$TERMINATE_APP" == "1" ]]; then
  if [[ -n "$FLUTTER_DEVICE_ID" ]]; then
    "$ROOT_DIR/quwoquan_app/scripts/device/run_stop_app_instance.sh" --env beta --device-id "$FLUTTER_DEVICE_ID" --quiet || true
  fi
