...
IdentityEntries = Callable[
    [dict[str, str], dict[str, str], dict[str, str], dict[str, str]],
    tuple[tuple[str, str], ...],
]
ResolvedBindings = Callable[
    [dict[str, str]],
    tuple[dict[str, str], dict[str, str], dict[str, str]],
]
ResolvedBinding = Callable[[dict[str, str]], dict[str, str]]
WorkspaceEntrypointBinding = Callable[[], dict[str, str]]
PrivateAtomicWrite = Callable[..., None]


class UserZshProjection:
    def __init__(
        self,
        *,
        carrier_path: Path,
        identity_environment_entries: IdentityEntries,
        required_workspace_entrypoint_binding: WorkspaceEntrypointBinding,
        workspace_entrypoint_binding: WorkspaceEntrypointBinding,
        resolved_bindings: ResolvedBindings,
        resolved_sdk_binding: ResolvedBinding,
        resolved_cocoapods_binding: ResolvedBinding,
        resolved_python_binding: ResolvedBinding,
        private_atomic_write: PrivateAtomicWrite,
        workspace_entrypoint_digest_keys: tuple[str, ...],
        resolution_error_types: tuple[type[BaseException], ...],
    ) -> None:
        self.carrier_path = carrier_path
        self.identity_environment_entries = identity_environment_entries
        self.required_workspace_entrypoint_binding = required_workspace_entrypoint_binding
        self.workspace_entrypoint_binding = workspace_entrypoint_binding
        self.resolved_bindings = resolved_bindings
        self.resolved_sdk_binding = resolved_sdk_binding
        self.resolved_cocoapods_binding = resolved_cocoapods_binding
        self.resolved_python_binding = resolved_python_binding
        self.private_atomic_write = private_atomic_write
        self.workspace_entrypoint_digest_keys = workspace_entrypoint_digest_keys
        self.resolution_error_types = resolution_error_types

    def literal_existing_home(self, home_path: Path) -> Path:
    ...
    def user_zsh_paths(
        ...
        *,
        ...
        home = self.literal_existing_home(home_path)
    ...
    def generated_user_zsh_projection(
        ...
        carrier = self.carrier_path.resolve(strict=True)
        body_lines = [
            f"export {key}={shlex.quote(value)}"
            for key, value in self.identity_environment_entries(
                sdk_binding,
                cocoapods_binding,
                python_binding,
                entrypoint_binding,
    ...
    def user_zsh_projection_is_recognized(self, content: bytes) -> bool:
        first_line = content.partition(b"\n")[0]
        return first_line.startswith((USER_ZSH_CONFIG_MARKER + " ").encode("utf-8"))


    def user_zsh_block(self, config_path: Path, *, inserted_prefix_newline: bool) -> bytes:
    ...
    def strip_user_zsh_block(self, original: bytes, *, path: Path) -> bytes:
    ...
    def with_user_zsh_block(self, original: bytes, *, config_path: Path, path: Path) -> bytes:
        base = self.strip_user_zsh_block(original, path=path)
        inserted = bool(base and not base.endswith(b"\n"))
        prefix = b"\n" if inserted else b""
        return base + prefix + self.user_zsh_block(config_path, inserted_prefix_newline=inserted)
    ...
    def activate_user_zsh(
        ...
        *,
        ...
        entrypoint_binding = self.required_workspace_entrypoint_binding()
        generated, zshrc, zprofile = self.user_zsh_paths(
            home_path=home_path,
        ...
        sdk_binding, cocoapods_binding, python_binding = self.resolved_bindings(env)
        expected_generated = self.generated_user_zsh_projection(
            sdk_binding,
            cocoapods_binding,
            python_binding,
            entrypoint_binding,
        ...
        if generated.exists():
            ...
            if not self.user_zsh_projection_is_recognized(generated.read_bytes()):
                raise SystemExit(
        ...
        updates = {
            zshrc: self.with_user_zsh_block(originals[zshrc], config_path=generated, path=zshrc),
            # 新契约只在 zshrc 维护单个 managed block；zprofile 里的 legacy 块被回收。
            zprofile: self.strip_user_zsh_block(originals[zprofile], path=zprofile),
        }
        ...
        if not changed:
            return "unchanged"
        self.private_atomic_write(generated, expected_generated, mode=0o600, private_parent=True)
        for startup in (zshrc, zprofile):
            if updates[startup] != originals[startup]:
                self.private_atomic_write(startup, updates[startup])
    ...
    def deactivate_user_zsh(
        ...
        *,
        ...
        updates = {
            zshrc: self.strip_user_zsh_block(originals[zshrc], path=zshrc),
            zprofile: self.strip_user_zsh_block(originals[zprofile], path=zprofile),
        }
        ...
        if generated_present:
            ...
            if (
                ...
                or not self.user_zsh_projection_is_recognized(generated.read_bytes())
            ):
    ...
    def user_zsh_status(
        ...
        *,
        ...
        try:
            expected_generated = self.generated_user_zsh_projection(
                self.resolved_sdk_binding(env),
                self.resolved_cocoapods_binding(env),
                self.resolved_python_binding(env),
                self.workspace_entrypoint_binding(),
            )
        except (
            *self.resolution_error_types,
        ) as error:
        ...
        if (
            ...
            try:
                zshrc_state = (
                    "active"
                    if self.with_user_zsh_block(zshrc_bytes, config_path=generated, path=zshrc)
                    == zshrc_bytes
                    else "drifted"
                )
            except SystemExit:
        ...
        if generated.exists():
            ...
            for line in generated_text.splitlines():
                for key in self.workspace_entrypoint_digest_keys:
                    prefix = f"export {key}="
