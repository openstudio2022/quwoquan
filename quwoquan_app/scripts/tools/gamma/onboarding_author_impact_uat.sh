...
usage() {
  cat <<'USAGE'
Usage: quwoquan_app/scripts/tools/gamma/onboarding_author_impact_uat.sh --device-id <id> [options]
...
if [[ "$api_status" -eq 0 ]]; then
  if ! bash "$ROOT/quwoquan_app/scripts/gamma/run_local_gamma_device_uat.sh" \
    --device-id "$DEVICE_ID" \
    --platform "$PLATFORM" \
    --target test/user_acceptance/service/content_service/content/post/interest_onboarding__user_acceptance_test.dart \
  ...
  if ! bash "$ROOT/quwoquan_app/scripts/gamma/run_local_gamma_device_uat.sh" \
    --device-id "$DEVICE_ID" \
    --platform "$PLATFORM" \
    --target test/user_acceptance/journeys/profile/profile_journey__user_acceptance_test.dart \
