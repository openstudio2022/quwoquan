...
Scans canonical object ``adapters`` and ``runtime/transport`` Dart sources for:
    ...
    for dirpath, _dirnames, filenames in os.walk(target_root):
        for name in filenames:
            ...
            if should_skip(path):
                continue
            rel = os.path.relpath(path, repo_root).replace("\\", "/")
            if "/adapters/" not in rel and "/runtime/transport/" not in rel:
                continue
            source = open(path, encoding="utf-8").read()
            is_remote_boundary = (
                name.endswith("_remote.dart")
                or "GeneratedCloudOperationClient" in source
                or "CloudRequestHeaders" in source
            )
            if not is_remote_boundary:
                continue
            scanned += 1
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Path containing canonical object adapters and runtime transport",
    )
    parser.add_argument(
        "--update-baseline",
        action="store_true",
    ...
    violations, scanned = collect_violations(target_root, repo_root)
    if scanned == 0:
        print(
            "verify_cloud_services_semantic: ERROR canonical adapter/transport scope is empty",
            file=sys.stderr,
        )
        return 2
