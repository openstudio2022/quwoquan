...
## 2. 范围
- Runtime：`assistant_agent_loop`、`local_phase_execution_owner`、`react_runtime`、`llm_provider`、`context_orchestrator`
...
- `tool_observation_v1`：工具观测必须包含 `ok/status/failureKind/recovery action/slotDelta/data`。
- `subagent_plan_v1/subagent_result_v1`：多代理编排必须结构化。
- `system_context_envelope`：系统默认注入时间、位置摘要、设备与权限状态，不通过额外工具读取。
