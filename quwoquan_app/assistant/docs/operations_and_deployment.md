...
```bash
mkdir -p ~/.assistant
cp assistant/config/config.example.json ~/.assistant/config.json
cp assistant/config/.env.example ~/.assistant/.env
...
1. `~/.assistant/config.json` + `~/.assistant/.env`（最高优先级，兼容回退 `~/.personal_assistant/...`）
2. 环境变量（`PERSONAL_ASSISTANT_MODEL_*`）
3. 项目内 bundled config（`assistant/config.json` + `assistant/.env`）
...
确认 Skill 资产已打包：`assets/assistant/skills/` 目录存在于 App bundle。
...
# 2. 一键 canary 脚本
bash assistant/scripts/assistant_canary_check.sh

# 3. 告警路由测试
bash assistant/scripts/assistant_alert_route_test.sh
```
