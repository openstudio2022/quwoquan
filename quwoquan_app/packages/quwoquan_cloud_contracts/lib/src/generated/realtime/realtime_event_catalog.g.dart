...
sealed class RealtimeEventEnvelope {
  ...
  factory RealtimeEventEnvelope.fromWire(
    ...
    String path = 'RealtimeEventEnvelope',
  ]) => decodeRealtimeEventEnvelope(wire, path);
...
final class ChatRealtimeEventEnvelope extends RealtimeEventEnvelope {
  const ChatRealtimeEventEnvelope({
    required super.wireType,
    super.eventId,
  ...
  @override
  Map<String, Object?> toWire() =>
      _realtimeEnvelopeToWire(wireType, eventId, occurredAt, payload.toWire());
}

final class RtcRealtimeEventEnvelope extends RealtimeEventEnvelope {
  const RtcRealtimeEventEnvelope({
    required super.wireType,
    super.eventId,
    required super.occurredAt,
    required this.payload,
...
final class UserSyncHintEventPayload {
  const UserSyncHintEventPayload({
...
final class UserSyncHintRealtimeEventEnvelope extends RealtimeEventEnvelope {
  const UserSyncHintRealtimeEventEnvelope({
    required super.wireType,
    super.eventId,
    required super.occurredAt,
    required this.payload,
...
final class FeedPatchRealtimeEventEnvelope extends RealtimeEventEnvelope {
  const FeedPatchRealtimeEventEnvelope({
    required super.wireType,
    super.eventId,
    required super.occurredAt,
    required this.payload,
...
RealtimeEventEnvelope decodeRealtimeEventEnvelope(
  Map<String, Object?> wire, [
  String path = 'RealtimeEventEnvelope',
]) {
  _realtimeRequireExactFields(wire, const <String>{
    'type',
    'eventId',
    'occurredAt',
    'payload',
  ...
  final occurredAtRaw = _realtimeRequiredString(
    wire,
    'occurredAt',
    '$path.occurredAt',
  );
  ...
  if (occurredAt == null)
    throw FormatException('$path.occurredAt must be ISO-8601');
  final rawPayload = wire['payload'];
  if (rawPayload is! Map || rawPayload.keys.any((key) => key is! String))
    throw FormatException('$path.payload must be an object');
  final payload = Map<String, dynamic>.from(rawPayload);
  switch (wireType) {
    case 'ConversationAvatarUpdated':
      return ChatRealtimeEventEnvelope(
        wireType: wireType,
        eventId: eventId,
        occurredAt: occurredAt,
        payload: decodeChatRealtimeEventPayload(
          eventType: wireType,
          payload: payload,
        ),
      );
    ...
    case 'call.answered':
      return RtcRealtimeEventEnvelope(
        wireType: wireType,
        eventId: eventId,
        occurredAt: occurredAt,
        payload: parseRtcWsPayload(wireType: wireType, payload: payload),
      );
    case 'call.connected':
      return RtcRealtimeEventEnvelope(
        wireType: wireType,
    ...
    case 'feed.patch':
      return FeedPatchRealtimeEventEnvelope(
        wireType: wireType,
        eventId: eventId,
        occurredAt: occurredAt,
        payload: parseFeedRealtimePatch(payload),
      );
    ...
    case 'sync_hint':
      _realtimeRequireExactFields(payload, const <String>{
        'userId',
        'latestSyncSeq',
      }, 'UserSyncHintEventPayload');
      final userId = _realtimeRequiredString(
        payload,
        'userId',
        'UserSyncHintEventPayload.userId',
      );
      final latestSyncSeq = payload['latestSyncSeq'];
      if (latestSyncSeq is! int || latestSyncSeq <= 0)
        throw FormatException(
          'UserSyncHintEventPayload.latestSyncSeq must be positive integer',
        );
      return UserSyncHintRealtimeEventEnvelope(
        wireType: wireType,
        eventId: eventId,
        occurredAt: occurredAt,
        payload: UserSyncHintEventPayload(
          userId: userId,
          latestSyncSeq: latestSyncSeq,
        ),
      );
...
void _realtimeRequireExactFields(
  Map<String, Object?> wire,
  Set<String> allowed,
  String path,
) {
  final unknown = wire.keys
...
String _realtimeRequiredString(
  Map<String, Object?> wire,
  String field,
  String path,
) {
...
String? _realtimeOptionalString(
  Map<String, Object?> wire,
  String field,
  String path,
) {
