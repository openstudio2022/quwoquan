...
/// Alpha-only Notification adapter backed by the immutable contract fixture.
final class AlphaAppMessageAdapter
    implements AppMessageQuery, AppMessageCommandWriter {
  AlphaAppMessageAdapter({AlphaFixtureBundle bundle = alphaFixtureBundle})
    : _messages = _readMessages(bundle);
  ...
  static List<AppMessage> _readMessages(AlphaFixtureBundle bundle) {
    final asset = bundle.assets['notification'];
    if (asset == null) {
      throw StateError('Notification alpha fixture asset is missing');
    }
    final decoded = jsonDecode(asset.sourceJson);
    if (decoded is! Map) {
      throw FormatException(
        'Notification alpha fixture root must be an object',
      );
    }
