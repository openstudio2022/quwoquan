...
/// Alpha 热词读模型：只消费 metadata seed manifest 生成的 immutable bundle。
final class AlphaHotQueryReader implements SearchHotQueryReader {
  ...
  static List<HotQuery> _loadItems() {
    final asset = alphaFixtureBundle.assets['search'];
    if (asset == null) {
      throw StateError('alpha fixture bundle is missing the search domain');
    }
...
final class AlphaCanonicalSearchFacet implements CanonicalSearchQueryFacet {
  ...
  static List<Map<String, Object?>> _contentPosts() {
    ...
    final decoded = json.decode(asset.sourceJson);
    if (decoded is! Map) {
      throw const FormatException('content fixture root must be an object');
    }
