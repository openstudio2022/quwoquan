...
final class AlphaProfileInteractionFacet
    implements
        ContentProfileInteractionQueryFacet,
        ContentProfileInteractionReadFactAppendFacet {
  AlphaProfileInteractionFacet({DateTime Function()? clock})
  ...
  Future<ContentProfileInteractionPage> listActivities(
    ...
    final matching = _rows
      ...
      ..sort(
        (left, right) => right.activity.occurredAt.compareTo(
  ...
  Future<ContentProfileInteractionReadFactAck> appendReadFact(
    ...
    _rows[rowIndex] = _AlphaProfileActivityRow(
  ...
  static List<_AlphaProfileActivityRow> _loadRows() {
    final asset = alphaFixtureBundle.assets['content'];
    if (asset == null) {
      throw StateError('alpha fixture bundle is missing the content domain');
    }
    final root = json.decode(asset.sourceJson);
  ...
  static _AlphaProfileActivityRow _decodeRow(Map<dynamic, dynamic> raw) {
    ...
    final actorID = _requiredText(raw['actorSubAccountId'], 'actorSubAccountId');
...
final class _AlphaProfileActivityRow {
  const _AlphaProfileActivityRow({
