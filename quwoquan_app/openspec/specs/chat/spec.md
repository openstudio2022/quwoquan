...
- **选择框（Checkbox）**
  - 视觉上略小：使用 `Transform.scale(scale: 0.82, alignment: Alignment.centerLeft)` 包裹；未选中边框与填充使用 `SettingsSemanticConstants.checkboxUnselectedBorderColor(isDark)`、`checkboxUnselectedFillColor(isDark)` 等，选中使用 `checkboxSelectedColor`；`fillColor` 使用 `WidgetStateProperty`。

### 设置页与选择页统一语义（SettingsSemanticConstants）

- **聊天信息页**：页面背景 `pageBackground(isDark)`，功能块背景 `blockBackground(isDark)`，功能块左右占满、块间距 `blockSpacing`；顶部 AppBar 背景与功能块一致（白），与第一块之间用细线分隔；块内分割线 `dividerColor(isDark)`、`dividerThickness`。退出群聊为独立白块、无红框、整行可点、文案色 `exitActionColor(isDark)`。
- **Switch**：未选中轨道 `switchInactiveTrackColor(isDark)`（明显灰，与白拇指对比度足够），拇指固定纯白（`thumbColor: WidgetStateProperty.all(Colors.white)`）；选中轨道蓝、拇指白。缩放 `Transform.scale(0.82)`。
- **选择页底部按钮**：未选时背景 `actionButtonDisabledBackground(isDark)`（深一点灰），高度 `actionButtonHeightMedium`（40）；选中时 `actionButtonPrimaryBackground`/`actionButtonPrimaryForeground`。圆角与内边距使用 `actionButtonBorderRadius`、`actionButtonPaddingHorizontal`/`Vertical`。
- **选择页 Checkbox**：未选中边框 `checkboxUnselectedBorderColor(isDark)`、填充 `checkboxUnselectedFillColor(isDark)` 使方框可见；选中 `checkboxSelectedColor`；缩放 `checkboxScale`（0.82）。与列表项、全选共用同一套 token，支持深色模式。
