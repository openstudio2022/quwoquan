...
///
/// 未收口路径使用 [SearchHitPayloadLegacy]；与 wire 对齐的帖子/圈子命中优先用 codegen 视图类型。
/// 序列化见各分支 [toWireMap]（仅边界/观测/助手工具）。
sealed class SearchHitPayload {
  const SearchHitPayload();

  /// 与历史 `SearchHit.toMap()['payload']` 形状一致的可编码 Map（JSON 边界）。
  Map<String, dynamic> toWireMap();
}

/// 过渡期：wire 仍为松散 Map 的命中（聊天、主页、POI、网页引用等）。
final class SearchHitPayloadLegacy extends SearchHitPayload {
  const SearchHitPayloadLegacy([Map<String, dynamic>? map])
...
/// 与 [AppSearchRepository._postHit] 历史手写字段表一致，避免与视图字段漂移。
Map<String, dynamic> postSearchItemViewToSearchHitWire(PostSearchItemView item) {
