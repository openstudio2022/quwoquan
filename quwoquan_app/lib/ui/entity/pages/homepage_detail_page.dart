...
class _HomepageDetailPageState extends ConsumerState<HomepageDetailPage> {
  ...
  void _handleIntersectionReasonTap(IntersectionReason reason) {
    // N9：实体页 D 面按结构化 actionHints 分发（metadata「端只读 actionKey/target」），
    // 不再用旧 `actionType == 'ask_xiaoqu'` 魔法字符串死分支（该值全仓从无产出）。
    final hint = _primaryActionHint(reason);
    if (hint != null &&
        !IntersectionActionKeys.isAssistant(hint.actionKey) &&
        hint.target != null) {
      // 结构化导航类动作（join_circle/open_route/follow_object/...）→ 统一导航到对象。
      _trackIntersectionReasonTap(reason);
      const IntersectionTargetNavigator().open(context, hint.target);
      return;
    }
    // 助手类 actionKey 或无结构化 target → 实体页打开小艺解释这条交集（消除「点击无反应」断点）。
    _openAssistantForIntersection(reason);
  }

  IntersectionActionHint? _primaryActionHint(IntersectionReason reason) {
