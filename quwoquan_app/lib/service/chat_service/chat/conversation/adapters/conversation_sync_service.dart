...
import 'package:quwoquan_cloud_contracts/quwoquan_cloud_contracts.dart'
    show UserSyncPatch, UserSyncPatchKind;
...
class ConversationSyncService {
  ...
  final ChatConversationRepository repo;
  final ConversationCacheService cache;
  final UserSyncRepository userSyncRepository;
  final ConversationAvatarSearchIndex store;
  final SearchActorScopeLoader personaContextLoader;
  ...
  Future<bool> syncAvatarPatches({
    ...
    int? observedLastSeq;
    try {
      await store.ensureConversationAvatarIndexReady();
      var lastSeq = await store.lastConversationAvatarSyncSeq(scope: namespace);
      ...
      while (hasMore && guard < 20) {
        ...
        if (result.requiresResync) {
          ...
          if (result.latestSyncSeq > lastSeq) {
            ...
            await store.saveConversationAvatarSyncSeq(
              scope: namespace,
              syncSeq: lastSeq,
            );
          }
  ...
  Future<void> _applyPatch(
    ...
    switch (patch.kind) {
      case UserSyncPatchKind.conversationAvatarUpdated:
        ...
        await store.updateConversationAvatarProjection(
          scope: namespace,
      ...
      case UserSyncPatchKind.userAvatarUpdated:
        ...
        await store.updateContactAvatarProjection(
          scope: namespace,
          userId: userId,
          avatarUrl: avatarUrl,
  ...
  void _activateNamespace(SearchActorScope namespace) {
