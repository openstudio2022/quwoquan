...
class ConversationCacheService extends ChangeNotifier
    implements ChatInboxCache, MessageHomeCache {
  ...
  void putMessageHomeRows(Iterable<MessageHomeRow> rows) {
    putAll(
      rows
          ...
          .map(ConversationCacheRecord.fromMessageHomeRow),
    );
  }

  @override
  List<MessageHomeRow> readMessageHomeRows() =>
      List<MessageHomeRow>.unmodifiable(
        getAll().map((record) => record.toMessageHomeRow()),
      );
