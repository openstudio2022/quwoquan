...
/// Conversation 对外公开的本地搜索/收件箱缓存投影。
class ConversationCacheRecord {
  ...
  factory ConversationCacheRecord.fromInboxEntry(ChatInboxCacheEntry dto) {
    return ConversationCacheRecord(
      id: dto.id.trim(),
      type: dto.type.trim(),
      title: dto.title.trim(),
  ...
  factory ConversationCacheRecord.fromMessageHomeRow(MessageHomeRow row) {
    return ConversationCacheRecord(
  ...
  ChatInboxCacheEntry toInboxEntry() {
  ...
  MessageHomeRow toMessageHomeRow() {
    return MessageHomeRow(
      ...
      avatarUrl: avatarUrl,
      groupAvatarVersion: groupAvatarVersion,
      lastActiveAt: _parseDateTime(lastMessageAt),
      unreadCount: unreadCount,
      mentionUnreadCount: mentionUnreadCount,
      muted: muted,
      pinned: pinned,
      notificationType: '',
      read: unreadCount == 0 && mentionUnreadCount == 0,
    );
  }
