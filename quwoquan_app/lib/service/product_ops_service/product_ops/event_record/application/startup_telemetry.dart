...
import 'package:quwoquan_app/service/product_ops_service/product_ops/event_record/application/startup_telemetry_support.dart';
...
final class StartupTelemetryEvent {
  ...
  Map<String, Object?> toJson() {
    return <String, Object?>{
      ...
      if (recoverySurface.isNotEmpty) 'recoverySurface': recoverySurface,
  ...
  Future<int> append(StartupTelemetryEvent event) async {
    ...
    for (final raw in rawEvents) {
      try {
        final decoded = jsonDecode(raw);
        if (decoded is Map) {
          knownEventIds.add(decoded['eventId']?.toString() ?? '');
        }
      } catch (_) {
...
final class StartupTelemetryReporter {
  ...
  factory StartupTelemetryReporter({
    ...
    required StartupTelemetryTransport transport,
  ...
  final StartupTelemetryTransport _transport;
...
final class StartupTelemetryRuntime {
  ...
  void configure(StartupTelemetryReporter reporter) {
    _reporter = reporter;
