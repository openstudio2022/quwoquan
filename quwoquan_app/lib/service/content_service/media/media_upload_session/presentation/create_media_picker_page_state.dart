...
class _CreateMediaPickerPageState extends State<CreateMediaPickerPage> {
  ...
  List<MediaPickerAlbumRef> _albums = const [];
  MediaPickerAlbumRef? _selectedAlbum;
  final GlobalKey _topBarKey = GlobalKey();
  final List<MediaPickerAssetRef> _assets = <MediaPickerAssetRef>[];
  ...
  Future<void> _loadInitial() async {
    ...
    final albums = await widget.mediaPickerPort.loadAlbums(
  ...
  Future<List<MediaPickerAlbumRef>> _prepareImageAlbums(
    List<MediaPickerAlbumRef> source,
  ) async {
    ...
    for (final album in source) {
      ...
      final count = await widget.mediaPickerPort.loadAlbumAssetCount(album);
  ...
  bool _isVideoOnlyAlbum(MediaPickerAlbumRef album) {
  ...
  bool _isCameraAlbum(MediaPickerAlbumRef album) {
    final normalized = album.name.trim().toLowerCase();
  ...
  int _albumPriority(MediaPickerAlbumRef album) {
    if (album.isAll) {
      return 2;
    }
  ...
  String _albumDisplayName(MediaPickerAlbumRef album) {
  ...
  MediaPickerRequestType _requestTypeByEntryMode() {
    return switch (widget.entryMode) {
      MediaPickerEntryMode.image => MediaPickerRequestType.image,
      MediaPickerEntryMode.video => MediaPickerRequestType.video,
      MediaPickerEntryMode.mixed => MediaPickerRequestType.common,
    };
  }
  ...
  Future<void> _loadMore() async {
    ...
    final next = await widget.mediaPickerPort.loadAssets(
      album: album,
      page: _page,
      pageSize: _pageSize,
    );
  ...
  bool _matchesEntryMode(MediaPickerAssetRef entity) {
    return switch (widget.entryMode) {
      MediaPickerEntryMode.image => entity.type == MediaPickerAssetType.image,
      MediaPickerEntryMode.video => entity.type == MediaPickerAssetType.video,
      MediaPickerEntryMode.mixed =>
        entity.type == MediaPickerAssetType.image ||
            entity.type == MediaPickerAssetType.video,
    };
  }

  Future<void> _toggleAsset(MediaPickerAssetRef entity) async {
    final key = entity.id;
    ...
    final item = await widget.mediaPickerPort.assetToMediaItem(entity);
  ...
  Future<void> _openCamera() async {
    ...
    final item = widget.mediaPickerPort.fileToMediaItem(
  ...
  Widget _buildCameraPage(BuildContext context) {
    ...
    return CameraCapturePage(
      ...
      filterRepository: widget.filterRepository,
    );
  }
  ...
  Future<void> _selectAlbum() async {
    ...
    final picked = await showAppTopAnchoredDropdown<MediaPickerAlbumRef>(
      context: context,
  ...
  Widget _buildAlbumCover(MediaPickerAlbumRef album, bool isDark) {
  ...
  Future<int> _albumCountFuture(MediaPickerAlbumRef album) {
    return _albumCountFutures.putIfAbsent(
      album.id,
      () => widget.mediaPickerPort.loadAlbumAssetCount(album),
    );
  }

  Future<Uint8List?> _albumCoverFuture(MediaPickerAlbumRef album) {
    return _albumCoverFutures.putIfAbsent(
      album.id,
      () => widget.mediaPickerPort.loadAlbumCover(album),
    );
  }
