...
class _PostCollectionEditorState extends State<PostCollectionEditor> {
    ...
    try {
      ...
      setState(() {
        _posts.addAll(
          result.items
              .map((item) => item.post)
              .whereType<wire.ContentPostProjection>()
              .where(
                (p) =>
                    p.contentType == wire.ContentType.video ||
                    p.contentType == wire.ContentType.image ||
                    p.contentType == wire.ContentType.article,
              ),
        );
  ...
  String? _imageAsset(wire.ContentPostProjection post) {
    ...
    return post.contentType == wire.ContentType.image ? post.mediaAssetId : null;
  }
