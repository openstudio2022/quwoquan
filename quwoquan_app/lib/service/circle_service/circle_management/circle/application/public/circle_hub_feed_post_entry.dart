import 'package:quwoquan_app/service/circle_service/circle_management/circle/application/public/circle_feed_post_projection_mapper.dart';
import 'package:quwoquan_app/service/content_service/content/post/application/public/content_post_view_data.dart';
import 'package:quwoquan_cloud_contracts/quwoquan_cloud_contracts.dart';

/// 圈子 Hub 帖子的公开 App read view。
///
/// 云侧内容事实由 [CircleFeedItemView] 提供；本类型只维护页面会话内互动快照，
/// 不依赖 concrete adapter、媒体查看器类型或动态 wire map。
final class CircleHubFeedPostEntry {
  ...
  void applyInteractionSnapshot({
    required Set<String> effectiveScopePostIds,
    required Set<String> effectiveScopeProfileIds,
    required Set<String> followingUsers,
    required Set<String> likedPosts,
    required Map<String, int> postLikesCount,
    required Map<String, int> postSharesCount,
    required Map<String, int> postCommentCount,
  }) {
    if (effectiveScopePostIds.isNotEmpty &&
        !effectiveScopePostIds.contains(postId)) {
      return;
    }

    _likeCount = postLikesCount[postId] ?? _likeCount;
    _commentCount = postCommentCount[postId] ?? _commentCount;
    _shareCount = postSharesCount[postId] ?? _shareCount;
    _isLiked = likedPosts.contains(postId);

    final authorId = authorRelationshipId.trim();
    if (authorId.isNotEmpty &&
        (effectiveScopeProfileIds.isEmpty ||
            effectiveScopeProfileIds.contains(authorId))) {
      _isFollowingAuthor = followingUsers.contains(authorId);
    }
  }
}
