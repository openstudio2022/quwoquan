...
class _SectionCreationsState extends ConsumerState<SectionCreations> {
  ...
  Widget _buildCreationFilterRow(
    ...
    final activeFilter = _creationFilters.firstWhere(
      (filter) => filter == circleState.activeSubTab,
      ...
      items: _creationFilters
          .map(
            (filter) => ObjectSecondaryFilterItem(
              id: filter.name,
              label: _creationSubTabLabel(filter),
            ),
          )
          .toList(growable: false),
      activeId: activeFilter.name,
      onSelect: (id) {
  ...
  CircleCreationSubTab _creationSubTabForId(String id) {
    switch (id) {
      case 'image':
        return CircleCreationSubTab.image;
      case 'video':
        return CircleCreationSubTab.video;
      case 'article':
        return CircleCreationSubTab.article;
      default:
        return CircleCreationSubTab.all;
    }
  }

  String _creationSubTabLabel(CircleCreationSubTab tab) => switch (tab) {
    CircleCreationSubTab.all => CommunityText.circleSubAll,
    CircleCreationSubTab.image => CommunityText.circleSubPhoto,
    CircleCreationSubTab.video => CommunityText.circleSubVideo,
      ...
      child: Row(
        ...
        children: [
          _ViewModeButton(
            ...
            tooltip: CommunityText.circleGridViewTooltip,
          ...
          _ViewModeButton(
            ...
            tooltip: CommunityText.circleListViewTooltip,
  ...
  Future<void> _openMediaViewer(
    ...
    final rawPostsById = circleHubMediaViewerRowsByPostId(viewerEntries);

    final interactionSnapshot = widget.participantSlots
        .buildMediaViewerSnapshot(ref: ref, posts: viewerDtos);
    widget.participantSlots.primeMediaViewerSnapshot(ref, interactionSnapshot);
    final navFeedRequestId = widget.participantSlots.newFeedRequestId(ref);
    final result = await context.push<Object?>(
      ...
      extra: MediaViewerExtra(
        posts: viewerDtos
            .map(widget.participantSlots.mapContentSurface)
            .toList(growable: false),
        dtoPosts: viewerDtos,
  ...
  void _applyViewerResult(MediaViewerResult result) {
    widget.participantSlots.applyMediaViewerResult(ref, result);
    setState(() {
      applyCircleHubMediaViewerResult(_feedEntries, result);
    });
  }
    ...
    try {
      final message = switch (action) {
        _CirclePostManagementAction.pin => () async {
          await widget.participantSlots.setPostPinned(
            ref,
        ...
        _CirclePostManagementAction.feature => () async {
          await widget.participantSlots.setPostFeatured(
            ref,
        ...
        _CirclePostManagementAction.remove => () async {
          await widget.participantSlots.removePost(
            ref,
  ...
  Widget _buildGridItem(
    ...
    return _managedEntrySurface(
      entry,
      showOverlay: true,
      child: widget.participantSlots.buildRecordPostCard(
        ...
        footer: Row(
          children: [
            ContentCardMetric(
              ...
              label: CommunityText.circleLikeCount(_entryLikeCount(entry)),
              color: fgSecondary,
  ...
  List<String> _recommendedArticleTemplatesForCircle() {
    ...
    return widget.participantSlots.recommendedArticleTemplates(categoryId);
  }
  ...
  String _articleRecommendationLabel(String articleTemplate) {
    ...
    if (templateId.isNotEmpty && recommended.contains(templateId)) {
      return CommunityText.circleArticleRecommendation(
        articleTemplatePresetFromString(templateId).label,
      );
    }
    ...
    return CommunityText.circleArticleRecommendation(labels);
  }
