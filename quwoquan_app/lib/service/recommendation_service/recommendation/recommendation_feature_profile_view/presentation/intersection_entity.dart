...
class _Avatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final kind = _objectKindFromWire(objectKind);
    final accent = AppColors.iosAccent(context);
    final radius = kind == IntersectionObjectKind.person
  ...
  IconData _iconFor(IntersectionObjectKind? kind) {
    switch (kind) {
      // 云侧登记了新 objectKind 而端侧尚未 codegen：给中性对象图标，不冒充任何已知类型。
      case null:
        return CupertinoIcons.square_stack_3d_up;
      case IntersectionObjectKind.person:
        return CupertinoIcons.person_crop_circle_fill;
      case IntersectionObjectKind.place:
        return CupertinoIcons.location_solid;
      case IntersectionObjectKind.circle:
        return CupertinoIcons.person_3_fill;
      case IntersectionObjectKind.school:
        return CupertinoIcons.book_fill;
      case IntersectionObjectKind.enterprise:
        return CupertinoIcons.building_2_fill;
      // §22.2 旅行摄影 objectKind（结构就位，本轮无数据流入；保持图标语义可辨识）。
      case IntersectionObjectKind.route:
        return CupertinoIcons.map_pin_ellipse;
      case IntersectionObjectKind.photoSpot:
        return CupertinoIcons.camera_fill;
      case IntersectionObjectKind.gear:
        return CupertinoIcons.bag_fill;
      case IntersectionObjectKind.content:
        return CupertinoIcons.doc_text_fill;
      // trip / meetup 已由领域模型收敛为唯一 Gathering 对象。
      case IntersectionObjectKind.gathering:
        return CupertinoIcons.person_2_fill;
      case IntersectionObjectKind.entity:
      case IntersectionObjectKind.tag:
        return CupertinoIcons.square_stack_3d_up;
    }
  }
}

IntersectionObjectKind? _objectKindFromWire(String objectKind) {
  try {
    return IntersectionObjectKind.fromWire(objectKind, 'objectKind');
  } on FormatException {
    return null;
  }
}
