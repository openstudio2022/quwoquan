...
/// 助手会话欢迎引导面板。
///
/// 语义是「引导开始对话」（上下文欢迎语 + 命令 chips + 建议），不是
/// GWT-016 的业务空态三类；前景色由宿主注入以适配沉浸/主题上下文。
class AssistantSessionWelcomePanel extends StatelessWidget {
  const AssistantSessionWelcomePanel({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    final chips = contextValue == null
        ...
        : AssistantPromptConfig.getChips(contextValue)
              .where((chip) => chip.actionType == 'command')
              .toList(growable: false);
    final suggestions = contextValue == null
        ? const <String>[]
        : AssistantPromptConfig.getSuggestionLines(contextValue);
    return Center(
          ...
          children: <Widget>[
            ...
            Text(
              contextValue == null
                  ...
                  : AssistantPromptConfig.getWelcomeMessage(contextValue),
