...
class AssistantModelConfigLoader {
  ...
  Future<List<AssistantModelRuntimeConfig>> loadFromBundledAsset() async {
    try {
      ...
      try {
        final envText = await rootBundle.loadString('personal_assistant/.env');
  ...
  List<AssistantModelRuntimeConfig> loadFromPersonalAssistantSync() {
    ...
    final configFile = File('$home/.personal_assistant/config.json');
    if (!configFile.existsSync()) return const <AssistantModelRuntimeConfig>[];
    final text = configFile.readAsStringSync();
    final decoded = jsonDecode(text);
    if (decoded is! Map<String, dynamic>) {
      return const <AssistantModelRuntimeConfig>[];
    }
    final envFileMap = _readDotEnvSync('$home/.personal_assistant/.env');
    return _extractConfigs(
      decoded,
      envFileMap: envFileMap,
      defaultProviderId: 'personal_assistant',
    );
  }
  ...
  Future<List<AssistantModelRuntimeConfig>> loadFromPersonalAssistant() async {
    ...
    final envFileMap = await _readDotEnv('$home/.personal_assistant/.env');
    return _extractConfigs(
      decoded,
      envFileMap: envFileMap,
      defaultProviderId: 'personal_assistant',
  ...
  Future<List<AssistantModelRuntimeConfig>> loadFromAppStorage() async {
    try {
      ...
      final paConfig = File('$basePath/.personal_assistant/config.json');
      if (await paConfig.exists()) {
        final configs = await _loadFromFile(
          paConfig,
      ...
      final projectConfig = File('$basePath/personal_assistant/config.json');
  ...
  List<_ProjectConfigCandidate> _projectConfigCandidatesSync() {
    ...
    return <_ProjectConfigCandidate>[
      _ProjectConfigCandidate(
        configPath: '$cwd/personal_assistant/config.json',
        envPath: '$cwd/personal_assistant/.env',
      ),
      _ProjectConfigCandidate(
        configPath: '$cwd/quwoquan_app/personal_assistant/config.json',
        envPath: '$cwd/quwoquan_app/personal_assistant/.env',
      ),
    ];
  }
