...
import 'package:quwoquan_app/assistant/internal_legacy/engine/aggregation_gate.dart';
...
import 'package:quwoquan_app/assistant/internal_legacy/engine/dialogue_state_runtime.dart';
import 'package:quwoquan_app/assistant/internal_legacy/engine/domain_router.dart';
import 'package:quwoquan_app/assistant/internal_legacy/engine/mode_decider.dart';
import 'package:quwoquan_app/assistant/internal_legacy/engine/process_journal_bus.dart';
import 'package:quwoquan_app/assistant/internal_legacy/engine/react_runtime.dart';
import 'package:quwoquan_app/assistant/internal_legacy/engine/recall_coordinator.dart';
...
import 'package:quwoquan_app/assistant/internal_legacy/debug/agent_loop_dev_logger.dart';
...
import 'package:quwoquan_app/assistant/internal_legacy/observability/logging/app_perf_probe.dart';
import 'package:quwoquan_app/assistant/internal_legacy/engine/llm_response_parser.dart';
import 'package:quwoquan_app/assistant/internal_legacy/protocol/assistant_content_filters.dart';
...
import 'package:quwoquan_app/assistant/internal_legacy/template_runtime/template_catalog_runtime.dart';
...
class PersonalAssistantAgentLoop {
  ...
  Future<AssistantRunResponse> _runImpl(
    ...
    if (subagentRuns.isNotEmpty) {
      ...
      mergedResult = ReactRuntimeResult(
        finalText: _ensureAssistantTurnEnvelopeText(subagentSynthesis.finalText),
        traces: <AssistantTraceEvent>[
          ...mergedResult.traces,
        ...
        failureCode:
            subagentSynthesis.failureCode.isNotEmpty
  ...
  Future<Map<String, dynamic>> _buildStructuredResponse({
    ...
    final processSummary = _resolveProcessSummary(
      answerPayload: answerPayload,
      domainId: dialogueRoundScript.domainId,
      request: request,
      uiReferences: uiReferences,
    ...
    final processReferenceCount = _resolveProcessReferenceCount(
      answerPayload: answerPayload,
      uiReferences: uiReferences,
    );
    ...
    final effectiveAggregationState = AggregationState(
      ...
      clarificationSource:
          stateDecision.askUser.slotId.trim().isNotEmpty ==
              true
  ...
  String _resolveProcessSummary({
    ...
    final isRealtimeLike = _isRealtimeLikeRequest(
      fallbackProblemClass:
          (answerPayload['problemClass'] as String?)?.trim() ?? '',
      answerPayload: answerPayload,
    );
    if (turn?.nextActionType == AssistantNextAction.askUser ||
        (turn == null &&
            parseNextAction(
                  ((answerPayload['decision'] as Map?)?['nextAction']
                              as String?)
                          ?.trim() ??
                      '',
                ) ==
    ...
    if (uiReferences.isNotEmpty) {
      if (isRealtimeLike) {
        return '已核对 ${uiReferences.length} 个高时效来源，正在整理会直接影响当前判断的结论。';
      }
      return '已核对 ${uiReferences.length} 个来源，正在整理可直接参考的结论。';
    }
    if (toolErrors.isNotEmpty) {
      if (isRealtimeLike) {
        return '已尝试获取最新信息，但这轮结果未通过展示门槛。';
      }
      return '已尝试获取相关信息，但这轮结果未通过展示门槛。';
    }
  ...
  bool _isRealtimeLikeRequest({
    ...
    if (payloadProblemClass.isNotEmpty) {
      return parseProblemClass(payloadProblemClass) == ProblemClass.realtimeInfo;
    }
  ...
  String _resolveFollowupPrompt({
    required Map<String, dynamic> answerPayload,
    required AssistantTurnAskUser askUser,
  }) {
    final prompt = (answerPayload['followupPrompt'] as String?)?.trim() ?? '';
    if (prompt.isNotEmpty) return prompt;
    return askUser.prompt.trim();
  }
  ...
  Future<String> _loadGlobalPolicyMarkdown() async {
    const path =
        'assets/assistant/prompts/global/stack.global_policy.md';
    try {
      final text = await rootBundle.loadString(path);
      return text.trim();
    } catch (_) {
