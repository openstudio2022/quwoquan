...
class DefaultProblemFramer {
  ...
  ProblemFrame frame(
    ...
    final negativeKeywords =
        _stringList(intentPayload['negativeKeywords']).isNotEmpty
  ...
  String _inferTargetObject(String normalized) {
    ...
    final englishBrand = RegExp(
      r'\b([A-Za-z][A-Za-z0-9._-]{1,40})\b',
    ).firstMatch(normalized);
    ...
    final stripped = normalized
        .replaceAll(
          RegExp(r'(帮我|请问|想问|想了解|想知道|能不能|是否|如果把|如果将|多给我|给我|关于|围绕|针对)'),
          ' ',
        )
        .replaceAll(RegExp(r'[，。！？,.!?]'), ' ')
  ...
  List<String> _extractExcludedScopes(String normalized) {
    ...
    final matches = <RegExpMatch>[
      ...RegExp(r'(?:不要|排除|去掉|不想要|别给我)\s*([^，。；;、]+)').allMatches(normalized),
