...
class SkillRegistry {
  ...
  void register(PersonalAssistantSkillManifest manifest) {
    ...
    if (existing != null &&
        !_isHigherVersion(manifest.version, existing.manifest.version)) {
      return;
    }
  ...
  List<PersonalAssistantSkillManifest> byCategory(String category) {
    return _entries.values
        .where((entry) => entry.manifest.category == category)
  ...
  List<PersonalAssistantSkillManifest> byTier(String tier) {
    return _entries.values
        .where((entry) => entry.manifest.tier == tier)
        .map((entry) => entry.manifest)
        .toList(growable: false);
  }
  ...
  static bool _isHigherVersion(String incoming, String existing) {
    ...
    for (int index = 0; index < 3; index++) {
      final incomingValue = index < iParts.length ? (iParts[index] ?? 0) : 0;
      final existingValue = index < eParts.length ? (eParts[index] ?? 0) : 0;
      if (incomingValue > existingValue) return true;
