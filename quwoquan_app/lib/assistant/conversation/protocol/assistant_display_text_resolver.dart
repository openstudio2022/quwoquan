...
abstract final class AssistantDisplayTextResolver {
  ...
  static String extractDisplayMarkdownFromStructuredText(String raw) {
    ...
    final uiAnswer =
        (payload['uiAnswer'] as Map?)?.cast<String, dynamic>() ??
    ...
    final nestedResult =
        ...
        const <String, dynamic>{};
    return _firstRenderableMarkdown(<String>[
  ...
  static String extractPlainTextFromStructuredText(String raw) {
    ...
    final nestedResult =
        (answerPayload['result'] as Map?)?.cast<String, dynamic>() ??
        const <String, dynamic>{};
    final plain = _firstRenderablePlain(<String>[
      (result['text'] as String?) ?? '',
      (result['summary'] as String?) ?? '',
      (nestedResult['text'] as String?) ?? '',
      (nestedResult['summary'] as String?) ?? '',
    ]);
    if (plain.isNotEmpty) return plain;
    final markdown = extractDisplayMarkdownFromStructuredText(raw);
    if (markdown.isEmpty) return '';
    return stripMarkdown(markdown);
  }
  ...
  static String _firstRenderableMarkdown(Iterable<String> candidates) {
    for (final candidate in candidates) {
      final sanitized = normalizeMarkdown(candidate);
      if (sanitized.isNotEmpty) return sanitized;
    }
    return '';
  }

  static String _firstRenderablePlain(Iterable<String> candidates) {
    for (final candidate in candidates) {
      final sanitized = normalizePlainText(candidate);
      if (sanitized.isNotEmpty) return sanitized;
    }
    return '';
  }
