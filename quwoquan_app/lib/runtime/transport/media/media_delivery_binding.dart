...
///
/// null/空串与未知值都保持 contract failure；只有具名 [previousPublic] adapter 能把
/// 已确认的 previous-version public 投影显式迁成 public。消费面不得从 URL 形态或字段缺席
/// 猜 public。
...
class MediaDeliveryBinding {
  ...
  ///
  /// 调用者必须先在所属 decoder/version 边界确认 previous-version 身份；本构造不会按 URL
  /// 形态猜测。正常新投影不得调用它。
  ...
  /// public builder 的唯一准入状态；具名 previous-version adapter 也已显式适配成该 typed 值。
