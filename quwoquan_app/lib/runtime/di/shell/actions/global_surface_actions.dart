...
/// 活动创建导航。路由只携带 typed Recommendation request，Host authority 与
/// command adapter 仍由目标 route host 独立校验并在缺失时结构化 fail-fast。
typedef StartGatheringNavigationBinding =
    Future<void> Function(BuildContext context);

/// Optional typed request keeps the generic shell entry and Recommendation
/// entry on one composition binding without leaking Flutter into the public
...
final startGatheringNavigationBindingProvider =
    Provider<StartGatheringNavigationBinding?>((ref) {
      Future<void> binding(
        BuildContext context, [
        GatheringCreateNavigationRequest? request,
      ]) async {
