...
import 'package:quwoquan_app/runtime/di/assistant_run_policy_dependencies.dart';
...
import 'package:quwoquan_app/runtime/shell/startup/startup_init_scheduler.dart';
...
import 'package:quwoquan_app/runtime/platform/permissions/app_permission_lifecycle_binding.dart';
...
class _QuWoQuanAppRootState extends ConsumerState<QuWoQuanAppRoot>
  ...
  void initState() {
    ...
    _startupInitScheduler = StartupInitScheduler(
      ...
      contentWarmupPort: ref.read(startupContentWarmupPortProvider),
      postFirstFrameTasks: _runDeferredPostFirstFrameTasks,
  ...
  Future<void> _runDeferredPostFirstFrameTasks() async {
    final tasks = <Future<void>>[
      ref.read(assistantRunPolicyLoaderProvider).load().then<void>((policy) {
        AssistantContentFilters.installProgressTextPolicy(policy);
      }),
    ];
    final postFirstFrameTasks = widget.postFirstFrameTasks;
    if (postFirstFrameTasks != null) {
      tasks.add(postFirstFrameTasks());
    }
      ...
      child: MaterialApp.router(
        ...
        builder: (context, child) => AppPermissionLifecycleBinding(
          child: wrapWithQuwoquanAppAppearance(
            context: context,
            snapshot: snapshot,
            child: child ?? const StartupRecoveryPage.routerError(),
          ),
        ),
      ),
    );
  ...
  Widget _buildStartupFallbackApp(
    ...
    required bool routerError,
  }) {
    final recoveryPage = routerError
        ? StartupRecoveryPage.routerError(
            failureCode: failure.runtimeFailure.code,
          )
        : StartupRecoveryPage.safeShell(
            failureCode: failure.runtimeFailure.code,
          );
    return MaterialApp(
      title: FoundationText.welcomeTitle,
