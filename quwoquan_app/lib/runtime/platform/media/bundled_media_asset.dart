...
import 'package:quwoquan_app/runtime/platform/media/bundled_media_materializer_stub.dart'
    if (dart.library.io) 'package:quwoquan_app/runtime/platform/media/bundled_media_materializer_io.dart'
    as materializer;
...
final class BundledMediaAsset {
  ...
  final void Function()? checkSource;
  ...
  Future<Uint8List> _read() async {
    ...
    if (bytes.length != byteLength ||
        'sha256:${sha256.convert(bytes)}' != digest) {
  ...
  /// 仅合并在途物化；成功路径下次仍由materializer重新验证磁盘字节。
  Future<String> materializeVideo() => _materializing ??= materializer
      .materializeBundledVideo(this)
      .whenComplete(() => _materializing = null);
}

/// 只含已验证 manifest 的目录；缺条目永不转网络。
final class BundledMediaCatalog {
  BundledMediaCatalog(Iterable<BundledMediaAsset> assets)
    : byReference = Map.unmodifiable({
        for (final asset in assets) asset.canonicalReference: asset,
      }),
      byAssetId = Map.unmodifiable({
  ...
  BundledMediaAsset? lookup(String reference, {String assetId = ''}) {
    ...
    if (asset == null || (assetId.isNotEmpty && asset.assetId != assetId)) {
      return null;
    }
    return asset;
  }
