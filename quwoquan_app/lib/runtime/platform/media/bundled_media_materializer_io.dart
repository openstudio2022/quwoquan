...
Future<String> materializeBundledVideo(BundledMediaAsset asset) async {
  if (asset.kind != 'video' || asset.mimeType != 'video/mp4') {
    throw const OfflineContentFailure(
      'bundled_video_format_unsupported',
      unavailable: true,
    );
  }
  // 即使命中私有物化文件，也不以路径缓存掩盖当前来源删除/篡改。
  final verifiedBytes = await asset.readVerifiedBytes();
  final root = await getApplicationSupportDirectory();
  final directory = Directory(
    '${root.path}/bundled_media/${asset.bundleDigest.substring(7)}',
  );
  await directory.create(recursive: true);
  final file = File('${directory.path}/${asset.digest.substring(7)}.mp4');
  if (await file.exists()) {
    final digest = await sha256.bind(file.openRead()).first;
    if (await file.length() != asset.byteLength ||
        'sha256:$digest' != asset.digest) {
      throw const OfflineContentFailure(
        'materialized_bundle_media_integrity_invalid',
      );
    }
    asset.checkSource?.call();
    return file.path;
  }
  final bytes = verifiedBytes;
  final attempt = await Directory.systemTemp.createTemp('bundled-video-');
  // 同一目标目录内 rename 原子发布；不覆盖其他 bundle 或网络缓存。
  final temporary = File(
    '${directory.path}/${asset.digest.substring(7)}.${attempt.path.split('/').last}.tmp',
  );
  try {
