...
├── appendLayer('stack.global_system')        // 5行身份约束（中英混写）
├── appendLayer('stack.runtime_policy')       // 执行策略 + {{domainSkillInstruction}} 嵌入
├── appendLayer('stack.recovery_policy')      // 4行恢复策略
├── appendLayer('stack.output_contract')      // 输出格式红线
└── stagePrompt (planner/synthesizer/...)     // 阶段模板 + 末尾 {{上下文变量}}
    合并方式: stackLayers.join('\n\n')        // 五层文本扁平拼接

agent_loop (额外注入的 system 消息，insert(0,...) 倒序)
├── system: "当前轮次状态机脚本（必须遵循）：" + JSON
├── system: "最小上下文锚点：" + anchor text
├── system: "可查询能力目录：" + catalog text
├── system: "记忆检索：" + recalled texts
├── system: "会话历史摘要：" + summary
...
| P5 | **输出契约不分阶段** | `stack.output_contract` 全局唯一，planning/synthesis 加载同一份 | planning 需要 JSON 格式，synthesis 需要 userMarkdown，互相干扰 |
| P6 | **system 消息过多且无序** | agent_loop 用 6 个 `insert(0,...)` 注入，最终顺序与语义优先级不一致 | 模型上下文窗口利用率低，重要信息被挤到中间 |
...
║                                                                                               ║
║  ┌─ L2 阶段前缀 ─────────────────────────────────────────────────────────────────────────┐   ║
║  │ §3  <task>            本轮任务定义（planner/synthesizer/postcondition）                │   ║
...
║  └────────────────────────────────────────────────────────────────── 缓存边界 L3 ──────┘   ║
╠═══════════════════════════════════════════════════════════════════════════════════════════════╣
║  MESSAGE 2: DATA（数据层——全部「基于什么做」集中在此，紧贴对话历史）                          ║
║                                                                                               ║
║  ┌─ L4 动态层（每轮变化，不缓存）───────────────────────────────────────────────────────┐   ║
...
当前问题：`## 反思与自检` 放在 prompt 末尾、`selfCheck` 是布尔打勾（无证据链）、`postcondition_check` 是额外 LLM 调用。这三种方式都是**事后补救**，模型已经生成了不合格的输出，再回头检查为时已晚。
...
**当前**：`stack.output_contract` 是全局唯一文件，所有阶段加载同一份。
...
- 历史检索反馈: {{historicalRetrievalFeedback}}
...
**优化建议**：
- 历史消息压缩（已有 `summarize_session`）：超过 5 轮时自动触发
- 工具调用历史不显示原始 JSON，仅保留摘要
...
| **引擎注入** | agent_loop / react_runtime 在收到模型输出后补入 | 模型不可能知道的运行时元数据 | domainId, stateId, promptTokens, latencyMs |

**删除的字段及原因**：

| 删除字段 | 原因 |
|---------|------|
| `contractVersion` | 常量，模型每次输出同一串字符浪费 token。引擎在收到响应后自动补入 |
...
**关于 phaseAwareLoaded**：
- 由 `agent_loop._loadPhaseAwareReferences()` 决定加载哪些 references
...
| 现有字段 | v2 变更 | 说明 |
|---------|---------|------|
| `contractVersion` | 移入 `_meta`（引擎注入） | 模型不再输出，省 token |
...
### 6.1 消息组装流程（`agent_loop.dart` 重构）

```
当前（7+ system messages，指令分散）:
  LLM provider prepend:
    system: stack.global_system + runtime_policy + recovery_policy + output_contract + stagePrompt
  agent_loop insert(0,...) × 6:
    system: "状态机脚本 JSON"
    system: "上下文锚点"
    system: "能力目录"
    system: "记忆检索"
...
v2（2 system messages + history，指令优先序）:
  ...
  MESSAGE 3+: 对话历史（user/assistant 交替）
```
...
```
当前:
  appendLayer('stack.global_system')          // ~50 token，身份
  appendLayer('stack.runtime_policy')         // ~120 token，含 {{domainSkillInstruction}} ← 问题：skill 被夹在策略规则中
  appendLayer('stack.recovery_policy')        // ~40 token，恢复
  appendLayer('stack.output_contract')        // ~200 token，全局唯一 ← 问题：plan/answer 不分
...
- 模型在 ~250 token 处就知道"这轮要做规划/回答/检查"
...
| `prompts/global/phase.output_contract.ask_user.md` | **新建** | §4 追问阶段输出规范（L2 阶段前缀） |
...
| `synthesizer.multi_skill_fusion.md` | **小改** | 去掉内联格式规范 |
| `planner.postcondition_check.md` | **保留** | 仅更新变量注入格式 |
...
| 文件 | 说明 |
|------|------|
| `stack.global_system.md` | 拆分为 identity + safety |
| `stack.global_policy.md` | 拆分为 persona + safety + tool_policy |
| `stack.runtime_policy.md` | 拆分为 tool_policy + 执行策略移入 stagePrompt |
| `stack.recovery_policy.md` | 合并入 safety |
| `stack.output_contract.md` | 拆分为 3 个 phase contract |
...
| `agent_loop.dart` | 消息注入从 6+ system messages 合并为 **2 条**（指令层+数据层） |
| `agent_loop.dart` | 6 个 `insert(0,...)` 合并为 `_buildDataMessage()`，输出 `<dialogue_state>` + `<context_slots>` |
| `agent_loop.dart` | `_mergeSkillInstructions` 输出 `<domain_skill>` XML 容器，persona 段独立注入 `<persona>` |
...
| ID | 任务 | 工时 | 输入 | 产出 | 验收标准 |
|----|------|------|------|------|----------|
| T1.1 | 新建 `stack.identity.md` | 0.5h | `stack.global_system.md` | §1 身份宣言 | 含命名「沃助理」、使命、信念 4 条 |
| T1.2 | 新建 `stack.safety.md` | 0.5h | `global_system` + `recovery_policy` + `global_policy` 安全段 | §2 安全边界 | 含硬性禁止 4 条 + 降级协议 3 条 + 危机识别 |
| T1.3 | 新建 `stack.persona.md` | 0.5h | `global_policy` 语气段 | §5 人设风格 + `{{skillPersona}}` 占位符 | 含语气基线 + 语言规则 + emoji 规范 |
| T1.4 | 新建 `stack.tool_policy.md` | 0.3h | `runtime_policy` 工具段 | §6b 工具规则 + `{{availableToolNames}}` | 含调用原则 4 条 |
...
| T1.7 | 新建 `phase.output_contract.ask_user.md` | 0.3h | 本文档 §4 设计 | ask_user 阶段输出规范 | 追问规则 + 选项化 |
| T1.8 | 重写 `planner.global_plan.md` | 1h | 现有文件 + §3 task 设计 | task 模板（去掉 `{{domainSkillInstruction}}`） | 含反思与自检嵌入任务定义（非末尾） |
| T1.9 | 精简 `synthesizer.final_answer.md` | 0.5h | 现有文件 | 去掉内联格式规范（移入 phase contract） | 格式规范零重复 |
| T1.10 | 19 个 SKILL.md 正文路径修正 | 0.5h | 全部 SKILL.md | `scripts/tool-call-guidance` → `references/tool-call-guidance` | `grep scripts/tool-call-guidance` 返回 0 |
...
| T2.2 | 消息注入合并为 2 条 | 2h | `agent_loop.dart` | 6 个 `insert(0,...)` → `_buildDataMessage()` 输出 `<dialogue_state>` + `<context_slots>` | messages 数组中 system 消息 ≤ 2 条 |
| T2.3 | skill.policy.md 拆分加载 | 1h | `agent_loop.dart` | `_splitSkillPolicy()` 按 `## ` 标题分 persona/constraint 两段 | persona 段注入 §5，constraint 段留 §6 |
| T2.4 | `toPromptMarkdown()` | 1h | `context_orchestrator.dart` | 新方法：contextEnvelope → `<context_slots>` 语义 Markdown | 输出含 📍📅👤🗂📱🧠 分区 |
| T2.5 | `<domain_skill>` XML 容器 | 0.5h | `agent_loop.dart` | `_mergeSkillInstructions` 输出带 `<domain_skill id="...">` 包裹 | XML 结构正确 |
| T2.6 | `<dialogue_state>` Markdown 化 | 1h | `agent_loop.dart` | JSON → 结构化 Markdown + `<dialogue_state>` 容器 | 无 jsonEncode 注入 |
| T2.7 | selfCheck 证据链引擎校验 | 1h | `agent_loop.dart` | `_mergeSelfCheck` 增加 evidence 字段处理，缺 evidence 时标记 warn | diagnostics 中可见 selfCheck 质量 |
...
| T3.2 | agent_loop emit 过程事件 | 2h | `agent_loop.dart` | 在规划/搜索/合成节点 emit 类型化事件 | trace 日志可见各阶段事件 |
...
| T3.4 | synthesizer 输出格式重构 | 1h | `phase.output_contract.answer.md` + `llm_provider.dart` | "结构化前缀 + 自由文本体" | userMarkdown 不再包裹在 JSON 中 |
| T3.5 | `reasonStream.onDelta` → `answerDelta` | 1h | `agent_loop.dart` | `onDelta: (_) {}` → emit answerDelta | 真实流式 token 传到 UI |
| T3.6 | capability_gateway 适配 | 1h | `capability_gateway.dart` | 删除 `_resolveChunkDisplayText` + `_chunkText` | 无伪流式代码 |
| T3.7 | chat_detail_page 事件路由 | 2h | `chat_detail_page.dart` | switch-case 替换启发式过滤 | 删除 `_looksLikeJsonEnvelope` + `contains("contractVersion")` |
...
enum AssistantStreamEventType {
  ...
  slotFilling,        // 槽位补全中：正在从上下文/GPS/历史中自动补齐
...
### 10.4 引擎侧 emit 点（agent_loop + react_runtime）

```
agent_loop._runSingleRound():
...
**核心矛盾**：synthesizer 要求输出 JSON（含 `decision`、`selfCheck` 等结构化字段），但 JSON 必须完整才能解析。

**解决方案——结构化前缀 + 自由文本体**：

```
v2 synthesizer output_contract 修改：

输出格式要求（严格按顺序）：
1. 第一行输出 JSON 头：{"decision":{"nextAction":"answer","confidence":0.9,"reasoning":"..."}}
2. 换行后直接输出 userMarkdown 纯文本（不包裹在 JSON 中）
3. 最后一行输出 JSON 尾：{"selfCheck":{"passed":true,"checks":[...]}}

引擎解析：
- 第一行：JSON.parse → 获取 decision
- 中间所有行：直接作为 userMarkdown → 流式 emit answerDelta
- 最后一行（以 {"selfCheck" 开头）：JSON.parse → 获取 selfCheck
- 引擎自动补入 `_meta`（contractVersion, domainId, stateId, latencyMs 等）
```

这样 `userMarkdown` 不再被包裹在 JSON 中，引擎逐行读取即可流式 emit，不需要增量 JSON 解析。

**备选方案（更保守）——JSON 完整但 userMarkdown 后置**：

```
模型仍输出完整 JSON，但 userMarkdown 放在 JSON 的最后一个字段。
引擎在 onDelta 中检测 `"userMarkdown":"` 开头后，开始 emit 后续 delta 作为 answerDelta。
遇到最终的 `"}` 时停止。

风险：仍然是增量 JSON 解析的变种，有转义/嵌套边界问题。
```

**推荐方案**：结构化前缀 + 自由文本体（方案一），彻底消除 JSON 包裹 userMarkdown 的矛盾。
...
switch (event.type) {
  ...
  case answerDelta:
    ...
    → 无需 contains("contractVersion") 过滤（协议保证是纯文本）
  case progressText:
    → 更新进度指示器文本
  case completed:
...
当前 6 个文件中有 13+ 个分散的 JSON 解析点。v2 协议下，**客户端和 capability_gateway 将不再需要解析 JSON**：

| 文件 | 当前解析点 | v2 变更 |
|------|-----------|---------|
| `agent_loop._decodeJsonObject` | 3 策略猜测 | 保留，但仅在引擎内部使用，不暴露给下游 |
| `agent_loop._extractJsonCandidateBlocks` | 括号深度扫描 | 同上 |
| `agent_loop._extractDisplayTextForStorage` | indexOf('{') | 直接用 `response.userMarkdown`，无需从 JSON 提取 |
| `react_runtime._extractToolCallsFromJsonText` | indexOf('{') | 升级为统一解析器 `LlmResponseJsonParser` |
| `capability_gateway._resolveChunkDisplayText` | 多路判断 | **删除**——不再需要，answer 由 answerDelta 事件流式传递 |
| `capability_gateway._chunkText` | 按句切分 | **删除**——不再需要伪流式 |
...
§4 <output_contract>    →    plan 阶段要求输出 JSON（引擎解析，不传给客户端）
                             answer 阶段要求"结构化前缀 + 自由文本体"（流式传给客户端）
...
当前:
prompts/global/
├── stack.global_system.md        ← 废弃（拆入 identity + safety）
├── stack.global_policy.md        ← 废弃（拆入 persona + safety + tool_policy）
├── stack.runtime_policy.md       ← 废弃（拆入 tool_policy + stage prompt）
├── stack.recovery_policy.md      ← 废弃（合入 safety）
├── stack.output_contract.md      ← 废弃（拆为 3 个 phase contract）
├── planner.global_plan.md        ← 保留，重写
├── planner.postcondition_check.md← 保留
├── synthesizer.final_answer.md   ← 保留，精简
├── synthesizer.multi_skill_fusion.md ← 保留，小改
...
├── planner.postcondition_check.md        ← 保留 §3（postcondition 阶段的 task）
├── synthesizer.final_answer.md           ← 精简 §3（answer 阶段的 task）
├── synthesizer.multi_skill_fusion.md     ← 小改 §3
├── phase.output_contract.plan.md         ← 新建 §4（L2 阶段前缀）
├── phase.output_contract.answer.md       ← 新建 §4
├── phase.output_contract.ask_user.md     ← 新建 §4
...
```json
{
  ...
  "phase.output_contract.ask_user": { "requiredVariables": [] }
}
```
