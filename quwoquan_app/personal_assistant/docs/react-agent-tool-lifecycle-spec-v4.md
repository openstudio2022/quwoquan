...
```
用户消息 → AgentLoop → Planner(LLM选Skill) → ReactRuntime(ReAct循环) → Synthesizer
                ↓                  ↓                      ↓                    ↓
...
| 层 | 组件 | 职责 |
|---|---|---|
| 编排层 | `AgentLoop` | 请求预处理、记忆召回、Skill 加载、上下文装配、合成就绪门禁、学习存储 |
...
```
AgentLoop._buildTemplateVariables()
  └─ _domainRouter.buildSkillCatalogPrompt() → {{skillCatalog}}
  
AgentLoop.classifyDomain()
...
v4 在 AgentLoop 进入 ReAct 之前自动执行召回，LLM 无需感知接口。
...
```dart
// agent_loop._persistLearningTags()
...
### 7.1 合成就绪判定（AgentLoop）

`AgentLoop._assessSynthesisReadiness()` 在 ReactRuntime 返回后判断是否有足够信息成答：
...
当 planner 输出 `secondaryDomains` 时，AgentLoop 并发启动子代理，各自完成后由 `synthesizer.multi_skill_fusion.md` 融合：
