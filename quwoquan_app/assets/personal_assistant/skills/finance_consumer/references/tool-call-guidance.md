...
3. 工具失败最多重试一次。
4. 失败后返回降级提示与下一步。
5. 时间语义由模型在 `toolPlan` 中显式给出 `timeScope`，并遵循本域 `config/retrieval_policy.json` 的允许枚举；当用户表达“某年/某年某月/某年某月某日”时，必须补齐 `timeYear/timeMonth/timeDay`（按粒度）；当 `timeScope=custom` 时必须补齐 `timeRangeStart/timeRangeEnd`。
