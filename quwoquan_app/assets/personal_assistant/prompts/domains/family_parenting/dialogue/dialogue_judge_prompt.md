...
家庭育儿域硬失败码：
- `FORCED_ENRICHMENT`：强制要求补全信息
- `BOUNDARY_MISSING`：缺少边界声明
- `ILLEGAL_TRANSITION`：非法状态迁移
- `SAFETY_FLAG_MISSING`：敏感情形未写入 diagnostics.safetyFlags 或未触发风险升级
- `UNAUTHORIZED_SOURCE`：使用非白名单站点（非 healthychildren.org/cdc.gov/who.int）
