...
8. **证据检查**：evidence 必须包含来源 URL 与发布日期；地区未明确时不得给出确定办理结论。  
...
- `EVIDENCE_WITHOUT_SOURCE_DATE`
- `DETERMINATE_CONCLUSION_WITHOUT_REGION`

---

## 白名单站点检查

政策公共服务域证据优先使用：`gov.cn`、`gjzwfw.gov.cn`、`sz.gov.cn`。所有 policy evidence 必须包含来源 URL 与发布日期；过期政策必须标记并要求复核。非白名单来源需在 evidence 中标注并说明用途。
...
7. evidence 含来源与发布日期；地区未明确时未给出确定办理结论
