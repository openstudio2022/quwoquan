...
- `URGENT_ESCALATION_MISSED`

---

## 白名单校验（health_wellness 域）

- 证据来源仅限：`who.int`、`nhc.gov.cn`、`cdc.gov`。
- 引用非白名单站点时，触发 `WHITELIST_VIOLATION`。
- 推荐具体药品或剂量时，触发 `MEDICATION_RECOMMENDATION`。
- 心理健康危机关键词未触发紧急引导时，触发 `URGENT_ESCALATION_MISSED`。
