...
Future<void> _resetLikeState(http.Client client, String postId) async {
  // unlike（即使未点赞也不报错）
  await client.delete(
    Uri.parse('$_stagingBase/v1/content/posts/$postId/like'),
    headers: {
      if (_testToken.isNotEmpty) 'Authorization': 'Bearer $_testToken',
    },
  );
...
void main() {
  ...
  patrolTest(
    ...
    ($) async {
      ...
      await $.pumpAndSettle();
      final countOptimistic = int.tryParse(
            $(TestKeys.likeCountText).text ?? '0',
          ) ??
          0;
      expect(countOptimistic, countBefore + 1,
          reason: 'Optimistic like count should increment immediately');

      // ── 等待 server 响应后确认计数一致 ──────────────────────────────
      await $(TestKeys.likeCountText)
  ...
  patrolTest(
    ...
    ($) async {
      ...
      expect(countAfter, countBefore + 1,
          reason:
              'Idempotent like: second tap should not double-increment count');
    },
  );
}
