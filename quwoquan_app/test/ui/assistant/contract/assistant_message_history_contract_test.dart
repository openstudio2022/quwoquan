...
void main() {
  group('AssistantMessageHistoryContract', () {
    ...
    for (final prefix in _degradedPrefixes) {
      test('Rule-2: message with degraded prefix "$prefix" is filtered out', () {
        final messages = <Map<String, dynamic>>[
          {'type': 'text', 'isSelf': true, 'content': '问题'},
          {'type': 'text', 'isSelf': false, 'content': '${prefix}详细错误信息'},
    ...
    test('Rule-4: non-text type messages are filtered out', () {
      final messages = <Map<String, dynamic>>[
        {'type': 'text', 'isSelf': true, 'content': '你好'},
        {'type': 'image', 'isSelf': false, 'content': 'https://example.com/img.png'},
    ...
    test('Rule-6: isSelf:true maps to user role, isSelf:false maps to assistant', () {
      final messages = <Map<String, dynamic>>[
    ...
    test('Rule-8: mixed scenario — only clean messages pass', () {
      final messages = <Map<String, dynamic>>[
        ...
        // 图片（应被过滤）
        {'type': 'image', 'isSelf': false, 'content': 'https://example.com/img'},
        // 空 content（应被过滤）
      ...
      expect(result.map((m) => m.content).toList(), equals([
        '第一个问题',
        '第二个问题',
        '这是正常回复',
      ]));
    ...
    test('Rule-9: typical multi-turn conversation without any degraded messages', () {
