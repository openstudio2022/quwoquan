...
IntersectionReason _reasonWithPoints(String displayText) {
  return IntersectionReason(
    dimension: 'interest',
    displayText: displayText,
    source: 'interest',
    intersectionClass: 'affinity',
    recommendedPointCount: 3,
    totalPointCount: 3,
  );
}
...
void main() {
  group('IntersectionReasonChip.primaryText 唯一口径', () {
    test('取首条 displayText（trim）', () {
      ...
      expect(IntersectionReasonChip.primaryText(reasons), '你和 TA 都来自同一校园');
    });

    test('有交集点摘要时优先展示点数 + 证据', () {
      expect(
        IntersectionReasonChip.primaryText(<IntersectionReason>[
          _reasonWithPoints('摄影内容相似'),
        ]),
        '3 个推荐交集点 · 摄影内容相似',
      );
    });
  ...
  testWidgets('双主题均渲染只读 displayText', (tester) async {
    for (final isDark in const <bool>[false, true]) {
      await tester.pumpWidget(
        _wrap(IntersectionReasonChip(text: '你和 TA 的交集', isDark: isDark)),
      );
