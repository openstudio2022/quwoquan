/// Production B10 two-device journey.
///
...
import 'package:quwoquan_app/rtc/rtc/call_session/presentation/pip_call_overlay.dart';
...
const _phase = String.fromEnvironment('QWQ_PROVIDER_UAT_B10_PHASE');
...
void main() {
  patrolTest(
    ...
    tags: const ['t4', 'provider', 'rtc', 'b10', 'prod_remote'],
    skip: !kRunPatrolT4,
...
Future<void> _runCaller(PatrolIntegrationTester $) async {
  ...
  _printMarker('QWQ_B10_REMOTE_MEDIA_CONNECTED');
...
Future<void> _runCallee(PatrolIntegrationTester $) async {
  ...
  // ignore: avoid_print
  print('QWQ_B10_REMOTE_CALLEE_READY:$_phase:callee');

  await _openNativeIncomingCall($);
  _printMarker('QWQ_B10_REMOTE_PUSH_PRESENTED');
  await $(
...
Future<void> _expectCallEnded(PatrolIntegrationTester $) async {
  ...
  while (DateTime.now().isBefore(deadline)) {
    ...
    if (find.byType(VideoCallPage).evaluate().isEmpty) {
      _printMarker('QWQ_B10_REMOTE_CALL_ENDED');
      return;
    }
  }
