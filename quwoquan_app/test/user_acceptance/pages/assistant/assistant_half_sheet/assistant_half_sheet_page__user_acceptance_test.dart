...
void main() {
  ...
  testWidgets('trace_context：打开半弹层先上报页面上下文且 snapshot 与来源一致', (tester) async {
    ...
    // 上下文快照与打开来源一致（同源 helper 投影）。
    expect(facet.lastContextSnapshot?['pageType'], 'discovery');
    expect(facet.lastContextSnapshot?['sourceSurfaceId'], 'discovery');
    expect(
      facet.lastContextSnapshot?['visitTarget'],
      const VisitTarget.page('discovery').targetKey,
    );
