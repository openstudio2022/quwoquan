...
///
/// 三维度覆盖：
///   常规契约  — 每个已知错误码正确解析，retryable 属性正确
...
void main() {
  ...
  group('ContentErrorCode — 常规契约', () {
    ...
    test('localized zh messages are set for known error codes', () {
      ...
      expect(ContentErrorMessages.zh[ContentErrorCode.upstreamTimeout], isNotNull);
    });
  });
  ...
  group('ContentErrorCode — HTTP 状态码映射契约', () {
    ...
    test('MIDDLEWARE error maps to 504 gateway timeout', () {
      expect(expectedHttpStatuses['CONTENT.MIDDLEWARE.upstream_timeout'], equals(504));
    });

    test('rate_limited maps to 429 (not 400 or 503)', () {
      // Explicitly guard against accidental change to rate_limited HTTP status
      expect(expectedHttpStatuses['CONTENT.USER.rate_limited'], equals(429),
  ...
  group('ContentErrorCode — 兼容性契约', () {
    ...
    test('unknown enum code is not retryable (backward compat default)', () {
      expect(ContentErrorCode.unknown.isRetryable, isFalse);
    });
  });
  ...
  group('ContentErrorCode — 异常/边界契约', () {
    ...
    test('partial code format (missing reason segment) falls back to unknown', () {
      expect(() => ContentErrorCode.fromCode('CONTENT.USER'), returnsNormally);
      expect(ContentErrorCode.fromCode('CONTENT.USER'), ContentErrorCode.unknown);
    });

    test('code with wrong module prefix falls back to unknown', () {
      expect(ContentErrorCode.fromCode('USER.USER.post_not_found'), ContentErrorCode.unknown);
    });
  });
}
