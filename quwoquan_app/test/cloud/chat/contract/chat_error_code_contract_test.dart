...
void main() {
  ...
  group('ChatErrorCode — 常规契约', () {
    test('parse conversation_not_found → conversationNotFound, not retryable',
        () {
      final code =
          ChatErrorCode.fromCode('CHAT.USER.conversation_not_found');
    ...
    test('parse message_too_long → messageTooLong, not retryable', () {
    ...
    test('parse internal_error → internalError, retryable', () {
  ...
  group('ChatErrorCode — 兼容性契约', () {
    ...
    test('other domain code → ChatErrorCode.unknown', () {
      final code =
          ChatErrorCode.fromCode('CONTENT.USER.post_not_found');
      expect(code, ChatErrorCode.unknown);
    });
