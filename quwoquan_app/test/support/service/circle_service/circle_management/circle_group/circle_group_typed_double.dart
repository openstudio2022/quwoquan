...
/// CircleGroup 的 suite-local 强类型 double。
final class CircleGroupTypedDouble
    implements CircleGroupCommands, CircleGroupQueries {
  ...
  Future<CircleGroupCommandResult> create(
    ...
    final groupId = 'group_fixture_${++_sequence}';
    final group = CircleGroupSlice(
      ...
      conversationId: 'conversation_fixture_$groupId',
  ...
  void _ensureDefault(String circleId) {
    final groupId = 'fixture_default_$circleId';
  ...
  CircleGroupSlice _required(String key, [int? expectedVersion]) {
    ...
    if (group == null) throw StateError('CircleGroup fixture not found');
    if (expectedVersion != null && group.version != expectedVersion) {
      throw StateError('CircleGroup fixture version conflict');
    }
    return group;
  }
