...
/// CircleGroupMembership 的 suite-local 强类型 double。
final class CircleGroupMembershipTypedDouble
  ...
  Future<CircleGroupMembershipCommandResult> apply(
    ...
    final membership = CircleGroupMembershipSlice(
      membershipId: 'group_membership_fixture_${command.groupId}_$_personaId',
  ...
  Future<CircleGroupMembershipCommandResult> updateRole(
    ...
    if (current.state != CircleGroupMembershipState.active) {
      throw StateError('CircleGroupMembership fixture is not active');
    }
    final updated = _copy(
      current,
  ...
  CircleGroupMembershipSlice _required(String key, [int? expectedVersion]) {
    ...
    if (membership == null) {
      throw StateError('CircleGroupMembership fixture not found');
    }
    if (expectedVersion != null && membership.version != expectedVersion) {
      throw StateError('CircleGroupMembership fixture version conflict');
    }
    return membership;
  }
  ...
  static const String _personaId = 'fixture_persona';
