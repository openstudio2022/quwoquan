...
final class InMemoryHomepageFacet
    implements
        HomepageQueryFacet,
  ...
  Future<HomepageDetailView> suggest(
    ...
    final now = _clock();
    final homepageId = 'fixture_homepage_candidate_${_records.length + 1}';
    ...
    _records[homepageId] = _InMemoryHomepageRecord(
      detail: detail,
      canonicalEntityId: '',
  ...
  Future<HomepageDetailView> updateClaimedBasics(
    ...
    _records[updated.homepageId] = _InMemoryHomepageRecord(
      detail: updated,
  ...
  Future<HomepageClaimRequestView> createClaimRequest({
    ...
    final record = HomepageClaimRequestView(
      ...
      claimTier: HomepageClaimTier.fromWire(
        draft.claimTier,
        'InMemoryHomepageFacet.claimTier',
      ),
...
Map<String, _InMemoryHomepageRecord> _recordsFromFixture(
  ...
  final records = <String, _InMemoryHomepageRecord>{};
  for (final raw in rawRows) {
    final row = _requiredObject(raw, 'entity fixture homepage');
    final detail = _detailFromFixture(row);
    records[detail.homepageId] = _InMemoryHomepageRecord(
      detail: detail,
...
HomepageType? _canonicalHomepageType(String raw) {
  try {
    return HomepageType.fromWire(raw, 'InMemoryHomepageFacet.homepageType');
  } on FormatException {
    return null;
  }
}
