...
final class InMemoryHomepageReviewFacet
    implements HomepageReviewCommandWriter, HomepageReviewQuery {
  InMemoryHomepageReviewFacet({
    this._activePersonaId = 'alpha-persona',
    DateTime Function()? clock,
  }) : _clock = clock ?? (() => DateTime.now().toUtc());

  final String _activePersonaId;
  final DateTime Function() _clock;
  final Map<String, _InMemoryReviewRecord> _records =
      <String, _InMemoryReviewRecord>{};
  ...
  Future<HomepageReviewView> create(CreateHomepageReviewCommand command) async {
    ...
    if (existing == null) {
      final record = _InMemoryReviewRecord(
        id: 'fixture_homepage_review_${_records.length + 1}',
  ...
  _InMemoryReviewRecord _findById(String reviewId) {
  ...
  void _requireAuthor(_InMemoryReviewRecord record) {
...
final class _InMemoryReviewRecord {
  const _InMemoryReviewRecord({
    required this.id,
