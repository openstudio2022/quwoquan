...
final class InMemoryContentMediaFacet implements ContentMediaFacet {
  InMemoryContentMediaFacet({
    this.completedAssetStatus = MediaAssetStatus.ready,
  });

  final Map<String, _InMemoryUpload> _uploads = <String, _InMemoryUpload>{};
  ...
  Future<MediaUploadSessionCommandResult> initUpload(
    ...
    final id = 'fixture_media_upload_${++_sequence}';
    final expiresAt = DateTime.utc(2030, 1, 1, 0, 15);
    _uploads[id] = _InMemoryUpload(command, expiresAt);
  ...
  Future<MediaUploadSessionCommandResult> completeUpload(
    ...
    final assetId = 'fixture_media_asset_${command.sessionId}';
  ...
  Future<MediaOriginalAccessGrant> requestOriginalAccess(
    ...
    return MediaOriginalAccessGrant(
      ...
      ttlSeconds: 300,
      auditId: 'fixture_audit_${asset.assetId}',
    );
  }
...
final class _InMemoryUpload {
  _InMemoryUpload(this.command, this.expiresAt);
