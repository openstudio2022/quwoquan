...
final class ReleaseHomepageUatCase {
  const ReleaseHomepageUatCase({
...
List<ReleaseHomepageUatCase> parseReleaseHomepageUatCases(
  String encodedPayload,
) {
  if (encodedPayload.isEmpty) {
    throw StateError('missing release-bound homepage UAT cases');
  }
  ...
  try {
    ...
    throw StateError(
      'invalid release-bound homepage UAT case encoding: $error',
    );
  }
  if (decoded is! Map<Object?, Object?>) {
    throw StateError('release homepage UAT cases must be an object');
  }
  ...
  for (final entry in decoded.entries) {
    if (entry.key is! String) {
      throw StateError('release homepage UAT cases contain a non-string key');
    }
  ...
  if (!_hasExactKeys(root.keys, rootKeys) ||
    ...
    throw StateError('release homepage UAT manifest contract mismatch');
  }
  for (final key in <String>[
    ...
    if (root[key] is! String || (root[key] as String).trim().isEmpty) {
      throw StateError('release homepage UAT manifest $key is invalid');
    }
  }
  final rawCases = root['cases'];
  if (rawCases is! List<Object?> || rawCases.isEmpty) {
    throw StateError('release homepage UAT manifest has no cases');
  }
  final entityRefs = <String>{};
  final homepageIds = <String>{};
  final cases = <ReleaseHomepageUatCase>[];
  for (final rawCase in rawCases) {
    if (rawCase is! Map<Object?, Object?>) {
      throw StateError('release homepage UAT case must be an object');
    }
    final row = <String, Object?>{};
    for (final entry in rawCase.entries) {
      if (entry.key is! String) {
        throw StateError('release homepage UAT case contains a non-string key');
      }
    ...
    if (!_hasExactKeys(row.keys, const <String>{
      ...
      throw StateError('release homepage UAT case contract mismatch');
    }
    ...
    if (!entityRefs.add(entityRef) || !homepageIds.add(homepageId)) {
      throw StateError('release homepage UAT case identity is duplicated');
    }
    cases.add(
      ReleaseHomepageUatCase(
        entityRef: entityRef,
...
String _requiredText(Map<String, Object?> row, String key) {
  ...
  if (value is! String || value.trim().isEmpty) {
    throw StateError('release homepage UAT case $key is invalid');
  }
  return value.trim();
}
