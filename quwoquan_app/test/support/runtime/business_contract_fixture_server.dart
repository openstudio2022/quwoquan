...
class _BusinessFixtures {
  ...
  static Map<String, dynamic> _canonicalCircleSeed(
    ...
    final seed = Map<String, dynamic>.from(source);
    seed['circles'] = (source['circles'] as List<dynamic>)
        .cast<Map<String, dynamic>>()
        .map((sourceRow) {
          final row = Map<String, dynamic>.from(sourceRow);
          row['category'] = row.remove('categoryId');
          return row;
        })
...
/// Deterministic in-process HTTP boundary for object-level local contracts.
///
/// It exposes only canonical HTTP wire seeded from service contract fixtures;
/// business assertions and object adapters remain in the owning test suites.
final class BusinessContractFixtureServer {
  BusinessContractFixtureServer._(this._server, this._fixtures);
  ...
  String get baseUrl => 'http://${address.host}:$port';

  String get expectedRtcSessionId =>
      ((_fixtures.rtcSeed['sessions'] as List<dynamic>).first
  ...
  GeneratedCloudOperationClient buildGeneratedClient() {
