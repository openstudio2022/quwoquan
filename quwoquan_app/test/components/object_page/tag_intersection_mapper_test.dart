...
void main() {
  group('sharedTagsToReasons', () {
    test('透传 label 为 displayText（不本地拼装，G2）并按 group 映射 dimension', () {
      ...
      expect(reasons[0].intersectionPoints.length, 1);
      expect(reasons[0].totalPointCount, 1);
      expect(reasons[0].factPointCount, 1);
      expect(reasons[0].totalPointCount, reasons[0].intersectionPoints.length);
