...
void main() {
  ...
  test('转化归因：follow/join_circle/add_contact 回流 dimension + sourceRef + tagRefs', () async {
    tracker.trackFollow(
      'u_lin',
      feedRequestId: 'feed_req_1',
      referralSource: ReferralSource.organicFeed,
    ...
    final byAction = {
      for (final e in repo.recorded) e.action: e,
    };
    ...
    expect(byAction[BehaviorAction.follow]!.toJson()['intersectionDimension'],
        'relationship');
    expect(byAction[BehaviorAction.joinCircle]!.toJson()['intersectionSourceRef'],
        'sharedInterest');
    expect(
