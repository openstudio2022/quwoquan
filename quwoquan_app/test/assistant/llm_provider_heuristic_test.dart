...
void main() {
  group('HeuristicLocalLlmProvider fallback contract', () {
    ...
    test(
      'weather query fail-closes instead of planning local tool calls',
      () async {
        ...
        expect(
          output.failureCode,
          equals(AssistantFailureCode.heuristicFallback),
        );
    ...
    test(
      'without usable context it still returns a safe user-facing fallback',
      () async {
        final output = await provider.reason(
          messages: const <Map<String, String>>[
    ...
    test(
      'synthesis stage also fail-closes instead of fabricating structured answer',
      () async {
        final output = await provider.reason(
  ...
  group('SwitchableAssistantLlmProvider model availability contract', () {
    test(
      'falls back to fail-closed degraded envelope when no remote model registered',
      () async {
