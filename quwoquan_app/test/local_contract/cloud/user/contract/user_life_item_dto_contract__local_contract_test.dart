...
void main() {
  group('UserLifeItemDto — wire 契约', () {
    test(
      'fromMap 按 wire 字段名解析（id/userId/category/title/subtitle/imageUrl/refId）',
      () {
    ...
    test('可空字段缺省安全（subtitle/imageUrl/refId 允许 null，sortOrder/时间有默认）', () {
      ...
      expect(dto.createdAt, DateTime(0));
    });
    ...
    test('LifeItemCategory 四枚举值均可解析并 round-trip', () {
      for (final category in <String>[
        ...
        final dto = UserLifeItemDto.fromMap(json);
        expect(dto.category, category);
        expect(dto.toMap()['category'], category);
      }
    });
  });
}
