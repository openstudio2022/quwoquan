...
void main() {
                  ...
                  onPressed: () async {
                    result = await Navigator.of(context)
                      ...
                      MaterialPageRoute<CreateMediaPickerResult>(
                        builder: (_) =>
                            DesktopImagePickerPage(maxSelection: maxSelection),
                      ),
                    );
                    popped = true;
                  },
  ...
  testWidgets('选目录 -> 扫描 -> 多选 -> 确认返回有序结果，并记忆目录', (tester) async {
                ...
                child: ElevatedButton(
                  onPressed: () async {
                    result = await Navigator.of(context)
                      ...
                      MaterialPageRoute<CreateMediaPickerResult>(
                        builder: (_) =>
                            const DesktopImagePickerPage(maxSelection: 9),
                      ),
                    );
                  },
                  child: const Text('open'),
    ...
    // 初始无记忆目录 -> 选文件夹空态。
    expect(find.byKey(TestKeys.desktopPickerChooseFolderButton), findsOneWidget);
    ...
    await tester.tap(find.byKey(const ValueKey<String>(
      'desktop-picker-tile-/pics/b.jpg',
    )));
    await tester.pump();
    await tester.tap(find.byKey(const ValueKey<String>(
      'desktop-picker-tile-/pics/a.jpg',
    )));
    await tester.pump();
    ...
    expect(
      result!.items.every((e) => e.type == CreateMediaType.image),
      isTrue,
    );
    expect(
  ...
  testWidgets('P0：网格缩略图走降采样 ImageProvider（targetPx 按显示像素，防回退全分辨率）',
    ...
    final tile = find.byKey(const ValueKey<String>(
      'desktop-picker-tile-/pics/a.jpg',
    ));
  ...
  testWidgets('能力位不支持本地文件系统时结构化降级为空态', (tester) async {
    final gateway = _FakeGateway(const <String, List<FileSystemEntry>>{},
        isSupported: false);
    final picker = _FakeDirectoryPicker(null);
    final memory = _FakeDirectoryMemory();
    ...
    await tester.pumpWidget(
      ProviderScope(
        ...
        child: const MaterialApp(
          home: DesktopImagePickerPage(maxSelection: 9),
        ),
      ),
    );
    await tester.pumpAndSettle();
