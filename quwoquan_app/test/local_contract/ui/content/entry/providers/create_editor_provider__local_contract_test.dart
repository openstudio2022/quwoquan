...
void main() {
  ...
  test('媒体正文更新不反向改写文章文档 nodes', () {
    final container = ProviderContainer();
    addTearDown(container.dispose);

    final notifier = container.read(createEditorProvider.notifier);
    notifier.setEditorKind(CreateEditorKind.media);
    final originalNodes = container
        .read(createEditorProvider)
        .articleDocument
        .nodes;

    notifier.updateMediaBody('媒体发布说明');

    final state = container.read(createEditorProvider);
    expect(state.body, '媒体发布说明');
    expect(state.articleDocument.nodes, same(originalNodes));
  });

  test('文章 node 更新时同步正文和活动节点', () {
    final container = ProviderContainer();
    addTearDown(container.dispose);
    ...
    final paragraphId = container
        ...
        .id;
    notifier.updateArticleNodeText(paragraphId, '开头');
    final orderedId = notifier.insertTextNodeAfter(
      paragraphId,
  ...
  test('插入正文 H2 会写入连续文档标题锚点并生成分页块', () {
    ...
    final paragraphId = container
        ...
        .firstWhere((node) => node.isBodyText)
        .id;
    notifier.updateArticleNodeText(paragraphId, '正文第一段');
    final headingId = notifier.insertTextNodeAfter(
      paragraphId,
  ...
  test('插入文章图片 node 时同步图片路径', () {
    ...
    final anchorId = container
        ...
        .firstWhere((node) => node.isBodyText)
        .id;
    final firstFigureId = notifier.insertImageAfterNode(anchorId, 'a.png');
    notifier.insertImageAfterNode(firstFigureId, 'b.png');

    final state = container.read(createEditorProvider);
    expect(state.imagePaths, <String>['a.png', 'b.png']);
    expect(
      state.articleDocument.nodes.where((node) => node.isFigure).length,
      2,
    );
  });

  test('文章图片 node 可更新环绕布局', () {
    final container = ProviderContainer();
    addTearDown(container.dispose);
    ...
    final anchorId = container
        ...
        .firstWhere((node) => node.isBodyText)
        .id;
    final figureId = notifier.insertImageAfterNode(anchorId, 'a.png');
    notifier.updateArticleNodeImageLayout(figureId, 'wrapLeft');

    final state = container.read(createEditorProvider);
    expect(
      state.articleDocument.nodes
          .firstWhere((node) => node.id == figureId)
          .imageLayout,
      'wrapLeft',
    );
  });
  ...
  test('开头锚点插图会落在正文最前方', () {
    ...
    final firstNode = container
        ...
        .nodes
        .firstWhere((node) => node.isBodyText);
    notifier.updateArticleNodeText(firstNode.id, '原始正文');
  ...
  test('文章封面选择在原图删除后会自动清空', () {
    ...
    final anchorId = container
        ...
        .firstWhere((node) => node.isBodyText)
        .id;
    final figureId = notifier.insertImageAfterNode(anchorId, 'cover.png');
    ...
    notifier.removeArticleNode(figureId);
  ...
  test('文章正文更新后可撤销与重做', () {
    ...
    notifier.updateArticleNodeText(paragraphId, '改动后');
  ...
  test('连续插图按 canonical nodes 顺序落位', () {
    final container = ProviderContainer();
    addTearDown(container.dispose);
    ...
    final figures = container
        .read(createEditorProvider)
        .articleDocument
        .nodes
        .where((node) => node.isFigure)
        .toList(growable: false);
    expect(figures.map((node) => node.imageUrl), <String>['a.png', 'b.png']);
  });

  test('图片前后正文通过 node 顺序直接物化', () {
    final container = ProviderContainer();
    addTearDown(container.dispose);
    ...
    final anchorId = container
        ...
        .id;
    final figureId = notifier.insertImageAfterNode(anchorId, 'a.png');
    final beforeId = notifier.insertTextNodeAfter(
      anchorId,
      initialText: '图前正文',
    );
    final afterId = notifier.insertTextNodeAfter(figureId, initialText: '图后正文');

    final state = container.read(createEditorProvider);
    final nodes = state.articleDocument.nodes;
    expect(
      nodes.indexWhere((node) => node.id == beforeId),
      lessThan(nodes.indexWhere((node) => node.id == figureId)),
    );
    expect(
      nodes.indexWhere((node) => node.id == afterId),
      greaterThan(nodes.indexWhere((node) => node.id == figureId)),
    );
    expect(state.articleDocument.body, contains('图前正文'));
    expect(state.articleDocument.body, contains('图后正文'));
  });
  ...
  test('insertImageAfterNode 后 nodes 序列正确', () {
    ...
    final paragraphId = container
        ...
        .firstWhere((node) => node.isBodyText)
        .id;
    notifier.updateArticleNodeText(paragraphId, '第一段');
