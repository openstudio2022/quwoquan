...
import 'package:quwoquan_app/runtime/shell/interest_match/interest_match_text_constants.dart';
...
final class _InMemoryVisitAppendPort implements VisitAppendPort {
  final List<VisitAppendInput> appended = <VisitAppendInput>[];
...
///
/// 边界先封闭 generated client，再把 suite-local 访问记录器同时交给 composition
/// 与可替换 Provider，避免测试装配向下解析真实 operation client。
List<Override> _boundaryOverrides({
  VisitRecorderService? visitRecorderService,
}) {
  return <Override>[
    ...sealedCloudBoundaryOverrides(),
    opsVisitAppendWriterProvider.overrideWithValue(_InMemoryVisitAppendPort()),
...
GoRouter _router({
  ...
  required VisitRecorderService visitRecorderService,
}) {
  return GoRouter(
...
Future<void> _pump(WidgetTester tester) async {
  ...
  await tester.pumpWidget(
    ProviderScope(
      overrides: _boundaryOverrides(visitRecorderService: visitRecorderService),
