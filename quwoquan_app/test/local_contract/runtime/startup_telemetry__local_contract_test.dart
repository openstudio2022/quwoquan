...
void main() {
  ...
  test('本地 journal 在落盘前剔除未声明的启动诊断字段值', () async {
    final store = _MemoryStartupJournalStore();
    final reporter = StartupTelemetryReporter(
      journal: StartupJournal(store),
      transport: _RecordingTransport(
    ...
    await reporter.record(
      phase: StartupTelemetryPhase.routerFailure,
      elapsedMs: 12,
    ...
    expect(event.recoverySurface, isNull);
  ...
  test('旧自由字符串 recovery journal 硬丢弃且 canonical typed 记录保留', () async {
    ...
    final event = (await StartupJournal(store).read()).single;
    expect(event.isRecoveryTelemetry, isTrue);
    expect(
  ...
  test('startup 与 recovery 永不进入同一 Remote batch', () async {
    final store = _MemoryStartupJournalStore();
    final transport = _AcknowledgingTransport();
    final reporter = StartupTelemetryReporter(
      journal: StartupJournal(store),
    ...
    await reporter.record(
      ...
      recoverySurface:
          ops_contracts.StartupRecoverySurface.pageAppStartupRecovery,
      recoveryLifecycle: ops_contracts.StartupRecoveryLifecycle.enter,
      recoveryMount: ops_contracts.StartupRecoveryMount.bootstrap,
      recoveryPhase: ops_contracts.StartupRecoveryPhase.startupChecking,
      recoveryAction: ops_contracts.StartupRecoveryAction.none,
    );
    ...
    expect(
      transport.batches.every(
        (batch) =>
            batch.every((event) => event.isRecoveryTelemetry) ||
            batch.every((event) => !event.isRecoveryTelemetry),
      ),
      isTrue,
    );
  ...
  test('RecoverySurfaceTelemetrySession 对同一 lifecycle tuple 去重', () async {
    final store = _MemoryStartupJournalStore();
    ...
    session.phaseChanged(ops_contracts.StartupRecoveryPhase.startupChecking);
