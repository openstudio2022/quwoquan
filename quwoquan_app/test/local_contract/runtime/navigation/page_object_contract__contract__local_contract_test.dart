...
void main() {
  test('canonical 页面对象契约通过阻断门禁', () {
    final appRoot =
        File('scripts/runtime/page/verify_page_object_contract.py').existsSync()
        ? Directory.current
        : Directory('quwoquan_app');
    final gate = File(
      '${appRoot.path}/scripts/runtime/page/verify_page_object_contract.py',
    );

    expect(gate.existsSync(), isTrue, reason: '未找到页面对象契约门禁脚本');

    final result = Process.runSync('python3', <String>[
      'scripts/runtime/page/verify_page_object_contract.py',
