...
final class _StubSessionLifecycleWriter
    implements AccountSessionLifecycleWriter {
...
void main() {
  ...
  testWidgets('内容预热只在 HTTPS trust 前置门通过后执行一次', (tester) async {
    suppressExpectedErrors();
    final prerequisites = Completer<void>();
    final warmup = _RecordingStartupContentWarmupPort();
      ...
      wrapRoot(
        ProviderScope(
          overrides: startupOverrides(
            ...
            extra: <Override>[
              startupContentWarmupPortProvider.overrideWithValue(warmup),
            ],
          ),
          child: QuWoQuanAppRoot(
            authNetworkPrerequisites: () => prerequisites.future,
    ...
    await pumpStartupThroughWelcome(tester);
    expect(warmup.callCount, 0);

    prerequisites.complete();
    await tester.pump();
    await tester.pump();
    expect(warmup.callCount, 1);
  ...
  testWidgets('前置门失败或组件销毁后不得触发内容预热', (tester) async {
    suppressExpectedErrors();
    final prerequisites = Completer<void>();
    final warmup = _RecordingStartupContentWarmupPort();
    ...
    prerequisites.completeError(StateError('trust unavailable'));
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 50));
    expect(warmup.callCount, 0);
  });
...
final class _RecordingStartupContentWarmupPort
