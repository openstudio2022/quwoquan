  ...
  group('startup native/Flutter handoff contract', () {
    test('Android 使用自适应渐变背景、同源品牌簇与贴底品牌名条，不拉伸整屏截图', () {
      ...
      final wrapper = _readAppFile(
        'scripts/tools/media/generate_native_launch_assets.py',
      );
    ...
    test('Flutter 是唯一动效所有者，生产没有最少等待 6 秒', () {
      ...
      final timeline = _readAppFile(
        'lib/runtime/shell/welcome/welcome_motion_timeline.dart',
      );
    ...
    test('欢迎页高保调色不外溢到 RTC 通话舞台', () {
      ...
      for (final path in const <String>[
        'lib/service/rtc_service/rtc/call_session/presentation/outgoing_call_page.dart',
        'lib/service/rtc_service/rtc/call_session/presentation/incoming_call_page.dart',
        'lib/service/rtc_service/rtc/call_session/presentation/call_stage_chrome.dart',
      ]) {
    ...
    test('三端从平台最早时钟计算 6 秒预算，只传时间不传动画状态', () {
      final bridge = _readAppFile(
        'lib/runtime/platform/startup_native_bridge.dart',
      );
      final runtime = _readAppFile(
        'lib/runtime/shell/startup/app_startup_runtime.dart',
      );
    ...
    test('公共 CA 单轨只允许 bootstrap 执行原生 attempt 握手', () {
      ...
      final scheduler = _readAppFile(
        'lib/runtime/shell/startup/startup_init_scheduler.dart',
      );
    ...
    test('iOS 裸 Debug 与 canonical launcher 共用完整 runtime handoff', () {
      ...
      final startupRuntime = _readAppFile(
        'lib/runtime/shell/startup/app_startup_runtime.dart',
      );
      final bootstrap = _readAppFile(
        'lib/runtime/shell/startup/app_bootstrap.dart',
      ...
      expect(
        ...
        contains("'configurationState': 'pending_native'"),
      );
    ...
    test('Android 插件由稳定应用注册器分层装配，生成注册器不在构建期修改', () {
      ...
      final policyVerifier = _readAppFile(
        'scripts/runtime/platform/verify_plugin_registration_policy.py',
      );
      ...
      expect(runtimePackageResolver, contains('nativeDriftKeys'));
      expect(runtimePackageResolver, contains(r'NATIVE_RUNTIME_PACKAGE.$key'));
      expect(runtimeConfig, contains('hydrateFromNativeRuntimePackage('));
      expect(
        runtimeConfig,
    ...
    test('production 启动装配匿名遥测 Remote，并只接受服务端持久化 ACK', () {
      ...
      final composition = _readAppFile('lib/runtime/di/ops_dependencies.dart');
      final telemetry = _readAppFile(
        'lib/runtime/observability/startup/startup_telemetry.dart',
      );
      final remote = _readAppFile(
        'lib/service/product_ops_service/product_ops/event_record/adapters/startup_telemetry_remote.dart',
      );
      expect(bootstrap, contains('initializeStartupTelemetryRuntime()'));
      expect(bootstrap, contains('attachStartupTelemetryTransport()'));
      expect(
        composition,
        contains('StartupTelemetryRuntime.instance.initialize'),
      );
      expect(
        composition,
        contains('StartupTelemetryRuntime.instance.attachTransport'),
      );
      expect(composition, contains('RemoteStartupTelemetryTransport'));
      expect(composition, contains('SharedPreferencesStartupJournalStore'));
      expect(remote, contains('opsEventRecordReportStartupEventBatch'));
      expect(remote, isNot(contains('requireAuth')));
      expect(remote, isNot(contains('OpsApiMetadata')));
