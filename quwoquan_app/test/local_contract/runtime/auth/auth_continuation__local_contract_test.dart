...
void main() {
  group('AuthContinuationController', () {
    ...
    testWidgets('发起活动续接精确调用组合根注入的 typed navigation binding', (tester) async {
      late BuildContext bindingContext;
      late WidgetRef bindingRef;
      ...
      final controller = bindingRef.read(authContinuationProvider.notifier);
      expect(
        controller.set(
          const OpenSheetContinuation(AuthContinuationSheet.startGathering),
        ),
        isTrue,
      );
      final pending = controller.take<OpenSheetContinuation>();
      expect(pending?.sheet, AuthContinuationSheet.startGathering);
      expect(bindingRef.read(authContinuationProvider), isNull);

      await GlobalQuickActionSheet.resumeSheetContinuation(
        bindingContext,
        bindingRef,
        pending!.sheet,
      );
