...
void main() {
  test('RTC 屏幕共享轨道经平台边界进入参与者模型并由视频页真实渲染', () {
    final participantsProvider = File(
      'lib/rtc/rtc/call_session/application/call_participants_provider.dart',
    ).readAsStringSync();
    final participantModel = File(
      'lib/rtc/rtc/call_session/domain/call_participant.dart',
    ).readAsStringSync();
    final videoPage = File(
      'lib/ui/rtc/pages/video_call_page.dart',
    ).readAsStringSync();
    final shareSurface = File(
      'lib/rtc/rtc/call_session/presentation/video_call_screen_share_surface.dart',
    ).readAsStringSync();
