...
Future<void> _pumpUntilPollBudgetConsumed(
  WidgetTester tester,
  _DataControlFacetFake dataControlFacet,
) async {
  for (
    var attempt = 0;
    attempt < 8 && dataControlFacet.getRequestIds.length < 6;
    attempt++
  ) {
...
void main() {
  ...
  testWidgets('bounded executing state is not presented as completion', (
    ...
    await tester.tap(find.text(AssistantText.assistantSkillDataControlConfirm));
    await tester.pumpAndSettle();
    await _pumpUntilPollBudgetConsumed(tester, dataControlFacet);
