...
import 'package:quwoquan_app/service/content_service/media/filter_catalog_release/adapters/image_editor_filter_digest.dart';
...
void main() {
  group('ImageEditorFilterConfig', () {
    ...
    test('loads local config and passes validation', () {
      expect(
        config.isValid(computeDigest: computeImageEditorFilterDigest),
        isTrue,
      );
    ...
    test('rejects duplicate category and preset display ordering', () {
      ...
      expect(
        _rehash(
          config,
          categories: categoriesWithDuplicateSort,
        ).isValid(computeDigest: computeImageEditorFilterDigest),
        isFalse,
      );
      ...
      expect(
        _rehash(
          config,
          presets: presetsWithDuplicateSort,
        ).isValid(computeDigest: computeImageEditorFilterDigest),
        isFalse,
      );
    });
    ...
    test('matches the cross-language decimal boundary digest vector', () {
      ...
      expect(
        boundaryConfig.computedCanonicalDigest(computeImageEditorFilterDigest),
        'fba38ede15295f3bbee31375d9955edc0baf722b8c204dbf0575f4ab25401242',
      );
      expect(
        boundaryConfig.isValid(computeDigest: computeImageEditorFilterDigest),
        isTrue,
      );
    });
  });
...
ImageEditorFilterConfig _decimalBoundaryVectorConfig() {
  ...
  return ImageEditorFilterConfig(
    ...
    canonicalDigest: draft.computedCanonicalDigest(
      computeImageEditorFilterDigest,
    ),
    categories: categories,
    presets: presets,
