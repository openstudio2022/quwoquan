...
void main() {
    ...
    final post = _videoPost(
      ...
      mediaItems: [
        for (var episode = 1; episode <= 2; episode++)
          PostMediaItem(
            kind: 'video',
            mediaAssetId: 'video-series-duplicate-$episode',
            mediaAssetVersion: 1,
            url: duplicateUrl,
            accessMode: MediaDeliveryAccessMode.public,
            durationMs: 125000,
  ...
  testWidgets('文章未知实体标签呈普通文字且不会把原始 entity id 推进主页错误页', (tester) async {
    ...
    final article = tester
        .widget<PostWorksViewerArticle>(find.byType(PostWorksViewerArticle))
        .article;
    expect(article.document.body, '未知地点');
    final span = article.document.nodes.expand((node) => node.spans).single;
  ...
  testWidgets('文章实体映射水合后更新同一阅读器，保留原文样式及其他合法链接', (tester) async {
    ...
    final source = ContentArticleRender(
      document: parsed.document.copyWith(
        nodes: [
    ...
    final beforeSpans = before.document.nodes
    ...
    for (var index = 1; index < beforeSpans.length; index++) {
      expect(
        beforeSpans[index],
        same(
          source.document.nodes.expand((node) => node.spans).elementAt(index),
        ),
      );
    }
    final entityText = find.byKey(
    ...
    final beforeChildren = (beforeText.text as TextSpan).children!
        .cast<TextSpan>();
    final unknownText = beforeChildren.singleWhere(
      (span) => span.text == '未知地点',
    );
    ...
    expect(
      unknownText.style?.decoration?.contains(TextDecoration.underline),
      isTrue,
    );
    expect(
      unknownText.style?.decoration?.contains(TextDecoration.lineThrough),
      isTrue,
    );
    for (final label in ['主页', '散步', '资料']) {
      expect(
        beforeChildren.singleWhere((span) => span.text == label).recognizer,
        isNotNull,
      );
    }
    final deckState = tester.state(find.byType(ArticleReadOnlyBookDeck));
    final beforePages = tester
        .widget<ArticleReadOnlyBookDeck>(find.byType(ArticleReadOnlyBookDeck))
        .pages;
    ...
    expect(
      tester
          .widget<ArticleReadOnlyBookDeck>(find.byType(ArticleReadOnlyBookDeck))
          .pages,
      isNot(same(beforePages)),
    );
    final afterText = tester.widget<RichText>(entityText.hitTestable().first);
    final restored = (afterText.text as TextSpan).children!
        .cast<TextSpan>()
        .singleWhere((span) => span.text == '未知地点');
