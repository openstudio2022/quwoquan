...
void main() {
  group('photo media picker commercial flow', () {
    ...
    testWidgets('相册下拉展示目录并切换当前图片集合，图片模式过滤视频', (tester) async {
      final service = _FakeMediaPickerPort(
        albums: <MediaPickerAlbumRef>[
          _album('recent', '最近项目', isAll: true),
          _album('travel', '旅行'),
        ],
        assetsByAlbumId: <String, List<MediaPickerAssetRef>>{
          'recent': <MediaPickerAssetRef>[
            _image('photo-one'),
            _image('photo-two'),
            _video('video-one'),
          ],
          'travel': <MediaPickerAssetRef>[_image('b1')],
        },
      );
    ...
    testWidgets('图片相册列表相机置顶、排除视频目录并按图片数降序', (tester) async {
      final service = _FakeMediaPickerPort(
        ...
        assetsByAlbumId: <String, List<MediaPickerAssetRef>>{
          'recent': <MediaPickerAssetRef>[_image('r1'), _image('r2')],
          'videos': <MediaPickerAssetRef>[
            _video('video-one'),
            _video('video-two'),
            _video('video-three'),
          ],
          'travel': <MediaPickerAssetRef>[
            _image('t1'),
            _image('t2'),
            _image('t3'),
          ],
          'camera': <MediaPickerAssetRef>[_image('c1')],
          'empty': const <MediaPickerAssetRef>[],
        },
      );
    ...
    testWidgets('较宽手机图片宫格保持三列，不因机型变宽升级为四列', (tester) async {
      ...
      final service = _FakeMediaPickerPort(
        albums: <MediaPickerAlbumRef>[_album('recent', '最近项目')],
        assetsByAlbumId: <String, List<MediaPickerAssetRef>>{
          'recent': <MediaPickerAssetRef>[
            _image('a1'),
            _image('a2'),
    ...
    testWidgets('图片选择器在浅色系统下仍强制深色 chrome，空选择时底部按钮不可点击', (tester) async {
      final service = _FakeMediaPickerPort(
        albums: <MediaPickerAlbumRef>[_album('recent', '最近项目')],
        assetsByAlbumId: const <String, List<MediaPickerAssetRef>>{
          'recent': <MediaPickerAssetRef>[],
        },
      );
    ...
    testWidgets('相册弹层在浅色系统下仍强制深色，且最高不越过顶部工具栏', (tester) async {
      final albums = <MediaPickerAlbumRef>[
        for (var i = 0; i < 18; i++) _album('album-$i', '相册$i'),
      ];
      final assetsByAlbumId = <String, List<MediaPickerAssetRef>>{
        for (var i = 0; i < 18; i++)
          'album-$i': <MediaPickerAssetRef>[_image('a$i')],
      };
      final service = _FakeMediaPickerPort(
        albums: albums,
    ...
    testWidgets('图片模式每次进入都重新选择，不沿用外部 initialSelection', (tester) async {
      final service = _FakeMediaPickerPort(
        albums: <MediaPickerAlbumRef>[_album('recent', '最近项目')],
        assetsByAlbumId: <String, List<MediaPickerAssetRef>>{
          'recent': <MediaPickerAssetRef>[_image('a1')],
        },
      );
    ...
    testWidgets('宫格选择、底部删除与完成回填保持当前顺序', (tester) async {
      ...
      final service = _FakeMediaPickerPort(
        albums: <MediaPickerAlbumRef>[_album('recent', '最近项目')],
        assetsByAlbumId: <String, List<MediaPickerAssetRef>>{
          'recent': <MediaPickerAssetRef>[_image('a1'), _image('a2')],
        },
      );
...
Widget _pickerApp({
  required MediaPickerPort service,
...
MediaPickerAlbumRef _album(String id, String name, {bool isAll = false}) {
...
MediaPickerAssetRef _image(String id) {
  return MediaPickerAssetRef(
    id: id,
    type: MediaPickerAssetType.image,
    mimeType: 'image/jpeg',
    width: 1200,
    height: 1600,
    durationMs: 0,
...
MediaPickerAssetRef _video(String id) {
  return MediaPickerAssetRef(
    id: id,
    type: MediaPickerAssetType.video,
    mimeType: 'video/mp4',
    width: 1200,
    height: 1600,
    durationMs: 8000,
...
class _FakeMediaPickerPort implements MediaPickerPort {
  _FakeMediaPickerPort({required this.albums, required this.assetsByAlbumId});

  final List<MediaPickerAlbumRef> albums;
  final Map<String, List<MediaPickerAssetRef>> assetsByAlbumId;
  ...
  @override
  Future<int> loadAlbumAssetCount(MediaPickerAlbumRef album) async {
    return _assetsForAlbum(album).length;
  }

  List<MediaPickerAssetRef> _assetsForAlbum(MediaPickerAlbumRef album) {
    final assets = assetsByAlbumId[album.id] ?? const <MediaPickerAssetRef>[];
    return switch (album.requestType) {
      MediaPickerRequestType.image =>
        assets
            .where((asset) => asset.type == MediaPickerAssetType.image)
            .toList(growable: false),
      MediaPickerRequestType.video =>
        assets
            .where((asset) => asset.type == MediaPickerAssetType.video)
            .toList(growable: false),
      _ => assets,
    };
  }
