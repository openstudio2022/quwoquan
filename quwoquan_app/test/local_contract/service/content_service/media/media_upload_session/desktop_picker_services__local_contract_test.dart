...
void main() {
  group('shouldUseDesktopImagePicker（能力位路由判据）', () {
    test('桌面（无系统相册 + 有本机文件系统）的图片入口走桌面选择器', () {
      expect(
        shouldUseDesktopImagePicker(
          hasMediaLibrary: CapabilityProfile.desktop.mediaLibrary,
          hasLocalFileSystem: CapabilityProfile.desktop.hasLocalFileSystem,
    ...
    test('移动端（有系统相册）图片入口不走桌面选择器', () {
      expect(
        shouldUseDesktopImagePicker(
          hasMediaLibrary: CapabilityProfile.mobile.mediaLibrary,
          hasLocalFileSystem: CapabilityProfile.mobile.hasLocalFileSystem,
          mode: MediaPickerEntryMode.image,
        ),
        isFalse,
      );
    ...
    test('web（有系统相册但无本机文件系统）不走桌面选择器', () {
      expect(
        shouldUseDesktopImagePicker(
          hasMediaLibrary: CapabilityProfile.web.mediaLibrary,
          hasLocalFileSystem: CapabilityProfile.web.hasLocalFileSystem,
          mode: MediaPickerEntryMode.image,
        ),
        isFalse,
      );
