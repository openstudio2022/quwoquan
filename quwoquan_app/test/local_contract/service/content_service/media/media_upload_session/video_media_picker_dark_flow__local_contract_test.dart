...
MediaPickerAlbumRef _album(String id, String name) {
  return MediaPickerAlbumRef(
    id: id,
    name: name,
    requestType: MediaPickerRequestType.video,
  );
}
...
class _FakeMediaPickerPort implements MediaPickerPort {
  ...
  Future<List<MediaPickerAssetRef>> loadAssets({
    ...
    return assetsByAlbumId[album.id] ?? const <MediaPickerAssetRef>[];
  }

  @override
  Future<int> loadAlbumAssetCount(MediaPickerAlbumRef album) async {
    return (assetsByAlbumId[album.id] ?? const <MediaPickerAssetRef>[]).length;
  }

  @override
  Future<Uint8List?> loadAlbumCover(MediaPickerAlbumRef album) async {
  ...
  Future<CreateMediaItem?> assetToMediaItem(
    ...
    return CreateMediaItem(
      id: entity.id,
      path: entity.type == MediaPickerAssetType.video
          ? '/tmp/${entity.id}.mp4'
          : '/tmp/${entity.id}.jpg',
