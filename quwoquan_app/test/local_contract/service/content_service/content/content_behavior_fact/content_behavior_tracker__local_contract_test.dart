...
///
/// 守护：Tracker 使用 RecordingContentBehaviorRepository，不发 HTTP。
/// 覆盖以下行为：
...
void main() {
  ...
  group('ContentBehaviorTracker', () {
    ...
    test('未达可见阈值只上报 visible，不进入 impressed 去重集合', () async {
      ...
      expect(event.toDurableStorageJson()['state'], equals('visible'));
    ...
    test('effective_play 仅上报实际播放证据并携带播放会话', () async {
      ...
      expect(event.toDurableStorageJson()['effectivePlayMs'], 8000);
      expect(
        event.toDurableStorageJson()['playbackSessionId'],
        'video-session-1',
      );
    });
    ...
    test('hide_author 事件上报 contentId + authorId + contentType', () async {
      ...
      expect(event.toDurableStorageJson()['action'], equals('hide_author'));
    });
    ...
    test('hide_content_type 事件上报 contentType，可带 authorId', () async {
      ...
      expect(
        event.toDurableStorageJson()['action'],
        equals('hide_content_type'),
      );
    });
    ...
    test('tag_click 交集证据组点击：独立动作（禁降级为 click）+ 完整交集归因回流', () async {
      ...
      expect(event.toDurableStorageJson()['action'], equals('tag_click'));
    ...
    test('wishlist_remove 写 removed 语义，空对象不上报', () async {
      ...
      expect(event.objectId, 'homepage_west_lake');
      expect(event.objectKind, 'homepage');
      expect(event.toDurableStorageJson()['action'], 'wishlist_remove');
    });
  });
}
