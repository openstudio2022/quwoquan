...
void main() {
  group('publish_draft_projection_bridge', () {
    ...
    test(
      'postReadPreviewFromCreateEditorState returns canonical read view',
      () {
        final state = CreateEditorState.initial().copyWith(title: 'NavTitle');
        final preview = postReadPreviewFromCreateEditorState(state);
        expect(preview.id, 'draft_preview');
        expect(preview.normalizedTitle, 'NavTitle');
      },
    );
    ...
    test('postReadPreviewFromPublishConfirmSummary work article branch', () {
      ...
      expect(preview.normalizedTitle, 'T');
      expect(preview.normalizedBody, 'B');
    });
    ...
    test(
      ...
      () {
        ...
        final state = CreateEditorState.initial().copyWith(
          ...
          body: 'x' * 200,
          articleDocument: document,
          settings: PublishSettings(tagRefs: <String>['Topic/旅行/城市漫步']),
        );
