...
import '../../../../../support/service/user_service/relationship/greeting_request/greeting_request_typed_double.dart';
...
void main() {
  ...
  group('InMemoryGreetingRequestFacet', () {
    late InMemoryGreetingRequestFacet facet;

    setUp(() {
      facet = InMemoryGreetingRequestFacet(
    ...
    test('reply 与 ignore 推进 inbox 终态', () async {
      ...
      final ignoreFacet = InMemoryGreetingRequestFacet(
        seedInbox: <GreetingRequestRecord>[
    ...
    test('cancel 后 pending 过滤消失、cancelled 过滤可见', () async {
      ...
      final pending = await facet.listGreetingOutbox(
        ListGreetingRequestsQuery(status: 'pending'),
      );
      final cancelledSlice = await facet.listGreetingOutbox(
        ListGreetingRequestsQuery(status: 'cancelled'),
      );
