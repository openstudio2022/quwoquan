...
/// canonical `user_profile_core` 当前用户场景；local_contract 不再把缺失的
/// `user_001` 当作默认档案并依赖已退役的 fallback 昵称。
const String _currentPersonaId = 'fixture_user_current';
const String _profileDisplayName = '新同学_260622_6698692';
...
class _TestAuthSessionStore implements AuthSessionStore {
  ...
  Future<StoredAuthSession> read() async {
    return StoredAuthSession(
      ...
      ownerId: authenticated ? _currentPersonaId : '',
      activePersonaId: authenticated ? _currentPersonaId : '',
...
class _AuthenticatedSessionController extends AuthSessionController {
  ...
  AuthSessionState build() => const AuthSessionState(
    ...
    ownerId: _currentPersonaId,
    activePersonaId: _currentPersonaId,
    accountState: 'active',
...
List<Override> _profilePageBoundaryOverrides() => <Override>[
...
void main() {
  ...
  Widget buildTestApp() {
    return ProviderScope(
      overrides: [
        ...
        currentUserIdProvider.overrideWithValue(_currentPersonaId),
  ...
  group('MyProfilePage — 我的主页数据加载 (A1)', () {
    ...
    testWidgets('进入后 displayName 非 "me"，展示真实昵称', (tester) async {
      ...
      // 当前用户直接来自 canonical `user_profile_core`，而不是缺失 id 的 fallback。
