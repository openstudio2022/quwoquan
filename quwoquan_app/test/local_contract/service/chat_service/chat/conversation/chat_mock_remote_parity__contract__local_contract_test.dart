...
void main() {
  group('chat mock remote parity contract', () {
    test(
      'ChatContactRepository rows only use media/avatar object keys',
      () async {
        final repo = ChatTestFacets();
        final contacts = await repo.contact.listContacts(limit: 100);
    ...
    test(
      ...
      () async {
        final repo = ChatTestFacets();
        final rows = await repo.contact.listContactHome(
          filter: 'all',
          limit: 500,
        );
    ...
    test(
      ...
      () async {
        final repo = ChatTestFacets();
        final rows = await repo.conversation.listMessageHome(
          filter: 'notification',
          limit: 50,
        );
        expect(rows, isEmpty);
    ...
    test('repository 与发送命令共享同一个 pure state engine', () async {
      ...
      final repo = ChatTestFacets(engine: engine);
      final writer = InMemoryChatMessageCommandWriter(engine: engine);
      final before = await repo.message.listMessages(
      ...
      final after = await repo.message.listMessages(
        conversationId: 'fixture_conv_direct',
        limit: 500,
      );
      final inbox = await repo.inbox.listInbox(limit: 500);
