...
import 'package:quwoquan_cloud_contracts/generated/entity_contracts.dart'
    show HomepageRelatedGroupSummary;
import '../../../../../support/service/entity_service/entity_homepage/homepage/homepage_test_adapter.dart';
import '../../../../../support/service/entity_service/entity_homepage/homepage/homepage_facets_typed_double.dart'
    show InMemoryHomepageFacet;
...
void main() {
  ...
  group('InMemoryHomepageFacet projection adapter', () {
  ...
  group('治理动作归属 Ops portal（B6 裁决）', () {
    test('App HomepageQuery / HomepageCommandWriter 均不超过 10 方法', () {
      ...
      expect(commandMethods, hasLength(2));
  ...
  group('RemoteHomepageRepository — related groups & detail JSON', () {
    ...
    test('getHomepageDetail 最小 JSON', () async {
      final client = MockClient((request) async {
        final detailPath = canonicalRemoteApiPath(
          AppCloudOperationIds.entityHomepageGetHomepageDetail,
          pathParameters: const <String, String>{'homepageId': 'h-min'},
        );
    ...
    test('getObjectPageBundle 解析 query 上下文和嵌套 projection', () async {
      final contractBundle = await InMemoryHomepageFacet().getObjectPageBundle(
        HomepageObjectPageBundleQuery(
          homepageId: 'homepage_sight_west_lake',
          referralSource: 'entity_page',
          feedRequestId: 'feed-1',
