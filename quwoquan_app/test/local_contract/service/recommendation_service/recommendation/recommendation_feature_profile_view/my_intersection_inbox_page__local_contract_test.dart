  ...
  testWidgets('filter=impact 时直达打动一级 tab', (tester) async {
    await tester.pumpWidget(
      ProviderScope(
        ...
        child: CupertinoApp.router(
          routerConfig: _router(
            page: MyIntersectionInboxPage(filter: 'impact'),
          ),
        ),
      ),
    );
...
GoRouter _router({Widget? page}) {
  final resolved = page ?? MyIntersectionInboxPage();
  return GoRouter(
    initialLocation: '/',
    routes: [
      GoRoute(path: '/', builder: (_, _) => resolved),
      GoRoute(
