...
const _endpointActionKeys = <String>{
...
void main() {
  group('IntersectionActionKeys', () {
    ...
    test('端侧常量集合与 codegen actionKey 闭集完全一致（无孤儿、无缺失）', () {
      const endpointConstants = _endpointActionKeys;
      expect(
        endpointConstants,
        IntersectionActionKey.values.map((key) => key.wireName).toSet(),
      );
    });
    ...
    test('isGatheringAction：仅同行/线下约伴类（dispatch==gathering）驱动「有人同行」', () {
      ...
      for (final retired in <String>[
        ...
        expect(_endpointActionKeys, isNot(contains(retired)));
      }
    });
    ...
    test('分类判定委托 codegen dispatch，端无第二真相源（M0.7）', () {
      ...
      for (final key in _endpointActionKeys) {
        expect(
