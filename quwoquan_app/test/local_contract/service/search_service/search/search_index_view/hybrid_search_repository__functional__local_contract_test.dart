...
void main() {
  test('suggest 合并 canonical Remote 与账号隔离本地命中', () async {
    ...
    final response = await repository.search(
      SearchRequest(
        query: '摄影',
        mode: CanonicalSearchMode.suggest,
  ...
  test('result 严格只走 canonical Remote，不读取本地命名空间', () async {
    ...
    final response = await repository.search(
      SearchRequest(query: '西湖', mode: CanonicalSearchMode.result),
    );
...
final class _CircleGroupIndexSpy implements CircleGroupLocalSearchIndex {
  int syncCalls = 0;
  int searchCalls = 0;
  ...
  Future<List<CircleGroupLocalSearchHit>> searchGroups({
    ...
    searchCalls += 1;
    return const <CircleGroupLocalSearchHit>[
      CircleGroupLocalSearchHit(
        groupId: 'group-photo',
