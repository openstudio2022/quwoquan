...
void main() {
  ...
  test('CallKit/FCM 是 startup-critical 且后台 handler 不直接拉起 Activity', () {
    ...
    final bootstrap = File(
      '${appRoot.path}/lib/app_bootstrap.dart',
    ).readAsStringSync();
    final pushEndpointGateway = File(
      '${appRoot.path}/lib/core/platform/push_endpoint_gateway.dart',
    ).readAsStringSync();
    final incomingCallPresenter = File(
      '${appRoot.path}/lib/core/platform/incoming_call_native_presenter.dart',
    ).readAsStringSync();
