...
Uint8List _jpegWithExif({
  ...
  if (fNumber != null) {
    exif.exifIfd['FNumber'] = img.IfdValueRational((fNumber * 100).round(), 100);
  }
...
void main() {
  group('extractMediaCaptureMetadata', () {
    test('从真实 JPEG APP1 段解析器材、参数、时间与坐标', () {
      ...
      final metadata = extractMediaCaptureMetadata(bytes);
    ...
    test('南纬西经取负号', () {
      ...
      expect(metadata.gpsLatitude, closeTo(-(33 + 51 / 60 + 30 / 3600), 0.0001));
      expect(
    ...
    test('无 EXIF 的 JPEG 返回空元数据而不是抛错', () {
      ...
      expect(extractMediaCaptureMetadata(bytes).isEmpty, isTrue);
    });
    ...
    test('相机时钟未初始化的年份被拒绝', () {
      ...
      expect(extractMediaCaptureMetadata(bytes).capturedAt, isNull);
    });

    test('缺少方向参考的坐标不落库', () {
      final bytes = _jpegWithExif(latitudeDms: <double>[30, 14, 30]);

      expect(extractMediaCaptureMetadata(bytes).gpsLatitude, isNull);
    });
  });
